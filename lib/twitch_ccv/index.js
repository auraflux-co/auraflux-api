...
async function listPeaksForBrand({ brandId, twitchLogin, limit = 10 } = {}) {
  ...
  const login = twitchLogin ? String(twitchLogin).trim().toLowerCase() : null;
  let result;
  if (brandId && login) {
    // Prefer brand rows, but also include streams captured under the brand's Twitch login
    // when brand_id was null or mismatched (common after OAuth vs manual save).
    result = await dbQuery(
      `SELECT id, brand_id, twitch_login, title, game_name, started_at, ended_at,
         ...
         FROM twitch_ccv_streams
        WHERE brand_id = $1 OR twitch_login = $2
...
/** Twitch player deep-link query: ?t=1h2m3s */
function formatTwitchTimestamp(sec) {
  if (sec == null || !Number.isFinite(Number(sec))) return null;
  const s = Math.max(0, Math.floor(Number(sec)));
  const h = Math.floor(s / 3600);
  const m = Math.floor((s % 3600) / 60);
...
/** Append peak seek to a Twitch VOD URL. */
function vodUrlAtPeak(vodUrl, peakOffsetSec) {
  if (!vodUrl) return null;
  const t = formatTwitchTimestamp(peakOffsetSec);
  if (!t) return vodUrl;
  try {
