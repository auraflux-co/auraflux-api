 ...
 *   sports-short  HOOK + CLIP + REACTION  (3 scenes only — no INTRO, no OUTRO)
 */
...
// Platform-level locked intro/outro fallbacks: none.
// Customers define their own sign-off and opening hook via customerConfig.voice.lockedIntro/lockedOutro.
// No intro or outro is required unless the customer explicitly configures one.
const LOCKED_INTROS_FALLBACK = {};
const LOCKED_OUTROS_FALLBACK = null;

/**
 * Get locked intro text for a template from customerConfig.
 * Returns null if customer has not configured a locked intro — no platform default.
 * @param {string} templateId - e.g. 'news-long', 'sports-long', 'clips-long'
 * @param {string} [customerId]
 * @returns {string|null}
 */
function getLockedIntro(templateId, customerId) {
  try {
  ...
  return LOCKED_INTROS_FALLBACK[templateId] || null;
}

/**
 * Get locked outro text from customerConfig.
 * Returns null if customer has not configured a locked outro — no platform default.
 * @param {string} [customerId]
 * @returns {string|null}
 */
function getLockedOutro(customerId) {
  try {
    const { loadCustomerConfig } = require('./customerConfig');
    const cfg = loadCustomerConfig(customerId, 'long-form');
    const outro = cfg?.designDefaults?.voice?.lockedOutro;
    if (outro !== undefined) return outro || null;
  } catch (e) {
    // non-fatal — fall through
  }
 ...
 * Per item: STREAM#_INTRO, STREAM#_CLIP1_SETUP, STREAM#_CLIP1_REACTION, ...
 * Uses STREAM# prefix so assembly groupSegmentsByLabel (Strategy A) groups correctly.
 * Streamer display name is embedded after the prefix: STREAM1_JASON_CLIP1_SETUP
 * No per-streamer OUTRO — Host transitions directly after last reaction.
 */
function buildClipsLong(items, clipsPerStreamer = 3) {
  ...
  for (let i = 0; i < items.length; i++) {
    const n = i + 1;
    // STREAM# prefix ensures assembly groups all scenes for this streamer together.
    // Streamer name embedded after prefix for human readability.
    const nameTag = normalizeHeaderPart(
      items[i].displayName || items[i].name || items[i].streamer || `STREAMER${n}`
    );
    const base = `STREAM${n}_${nameTag}`;
    scenes.push(scene(`${base}_INTRO`));
    for (let c = 1; c <= clipsPerStreamer; c++) {
      scenes.push(scene(`${base}_CLIP${c}_SETUP`, true));
      scenes.push(scene(`${base}_CLIP${c}_REACTION`));
    }
  }
...
function buildNbaClipTimingLockedBlock(clipTimingTargets) {
  ...
  if (!targets.length) {
    inner = [
      ...
      '- Write [DIALOGUE] as strict chronological play-by-play using ONLY moments from the Gemini analysis in GAME DATA.',
...
function buildSportsLong(items, nbaClipTimingByGame) {
  ...
  for (let i = 0; i < items.length; i++) {
    ...
    const targets =
      timingEntry && Array.isArray(timingEntry.clipTimingTargets)
...
// ─── Universal builder (C1+) ──────────────────────────────────────────────────

/**
 * universal-long: INTRO + (items × scenesPerItem) + OUTRO
 * Uses ITEM# prefix — no content-type vocabulary.
 * Assembly groups by ITEM# automatically via Strategy A.
 *
 * Default scene shape per item: ITEM#_SETUP, ITEM#_CLIP, ITEM#_REACTION
 * Override via jobSpec.order.inputs.sceneShape (array of suffix strings).
 * Clip marker placed on any scene whose suffix includes 'CLIP'.
 *
 * @param {Array}    items
 * @param {string[]} [sceneShape] - e.g. ['SETUP','CLIP','REACTION'] (default)
 * @param {string}   [customerId]
 */
function buildUniversalLong(items, sceneShape, customerId) {
  const shape =
    sceneShape && sceneShape.length > 0
      ? sceneShape.map((s) => normalizeHeaderPart(s))
      : ['SETUP', 'CLIP', 'REACTION'];

  const scenes = [];
  scenes.push(lockedScene('INTRO', getLockedIntro('news-long', customerId)));

  for (let i = 0; i < items.length; i++) {
    const n = i + 1;
    for (const suffix of shape) {
      const isClip = suffix === 'CLIP' || suffix.includes('CLIP');
      scenes.push(scene(`ITEM${n}_${suffix}`, isClip));
    }
  }

  scenes.push(lockedScene('OUTRO', getLockedOutro(customerId)));
  return scenes;
}

/**
 * universal-short: HOOK + CLIP + REACTION (3 scenes, ITEM prefix)
 * Same 3-scene shape as all other short templates but uses ITEM prefix for consistency.
 */
function buildUniversalShort() {
  return [scene('ITEM1_HOOK'), scene('ITEM1_CLIP', true), scene('ITEM1_REACTION')];
}
 ...
 * Split-screen: host top reacting, source clip bottom. Caption burned on video.
 * NO INTRO, NO OUTRO — per SHORT_FORM_SPEC.md
 */
function buildNewsShort(items) {
  return [scene('HOOK'), scene('CLIP', true), scene('REACTION')];
}
...
const TEMPLATE_BUILDERS = {
  ...
  // C1+ universal templates — content-type agnostic, ITEM# prefix
  'universal-long': buildUniversalLong,
  'universal-short': buildUniversalShort,
};
...
function inferTemplateId(jobSpec) {
  ...
  // Map C0 content type aliases to known template families
  let type = contentType;
  if (['twitch', 'clips', 'streamer'].some((t) => type.includes(t))) type = 'clips';
  else if (['nba', 'sports', 'basketball', 'football'].some((t) => type.includes(t)))
    type = 'sports';
  else if (['news', 'world', 'global'].some((t) => type.includes(t))) type = 'news';
  // C1+: any unrecognised content type falls through to the universal template
  else if (!['news', 'sports', 'clips'].includes(type)) type = 'universal';
...
function generateScaffold(jobSpec) {
  ...
  if (!builder) {
    throw new Error(
      `Unknown templateId "${templateId}". Supported: ${Object.keys(TEMPLATE_BUILDERS).join(', ')}`
    );
  }
  ...
  const scaffoldInstructions = isShortTemplate
    ? [
        ...
        `- Fill every [DIALOGUE] slot with spoken content in the host's voice`,
    ...
    : [
        ...
        `- All other scenes: write fresh dialogue in the host's voice`,
