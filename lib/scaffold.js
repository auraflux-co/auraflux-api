...
/**
 * Split inline scene headers onto their own line.
 * Gemini often concatenates headers: "text=== CINNA_INTRO ===First up..."
 */
function normalizeInlineSceneHeaders(script) {
  if (typeof script !== 'string' || !script.trim()) return script;
  let out = script.replace(/(^|[^\n\r])(===\s*[A-Z0-9_]+\s*===)/g, (match, pre, hdr) => {
    if (pre === '' || pre.endsWith('\n') || pre.endsWith('\r')) return match;
    return pre + '\n' + hdr;
  });
...
/**
 * Rewrite scene header names in document order to match jobSpec contract.
 * Fixes Gemini shortening (EMILY → EXTRAEMILY) when marker count already matches.
 */
function alignSceneHeadersToContract(script, expectedNames) {
  if (typeof script !== 'string' || !Array.isArray(expectedNames) || !expectedNames.length) return script;
  let idx = 0;
  return script.replace(/===\s*([A-Z0-9_]+)\s*===/g, (match) => {
    if (idx >= expectedNames.length) return match;
    const canonical = String(expectedNames[idx++] || '').trim();
    return canonical ? `=== ${canonical} ===` : match;
  });
}

/** Single entry for Gate 1 / post-Gemini script normalization. */
function normalizeScriptForGate1(script, expectedNames) {
  if (typeof script !== 'string' || !script.trim()) return script;
  let out = normalizeInlineSceneHeaders(script);
  const names = (expectedNames || []).map((h) => String(h || '').trim()).filter(Boolean);
  if (names.length) out = alignSceneHeadersToContract(out, names);
  return out;
}
...
/**
 * Replace Twitch logins and common shortened names with on-air display names in spoken text.
 */
function alignSpokenEntityNames(script, streamers = []) {
  if (typeof script !== 'string' || !streamers.length) return script;
  let out = script;
  for (const row of streamers) {
    const login = String(row.streamer || row.twitchUsername || '').trim();
    const display = String(row.displayName || row.onAirName || '').trim();
    if (!login || !display) continue;
    if (login !== display) {
      out = out.replace(new RegExp(`\\b${escapeRegExp(login)}\\b`, 'gi'), display);
    }
    if (login.toLowerCase() === display.toLowerCase() && login !== display) {
      out = out.replace(new RegExp(`\\b${escapeRegExp(login)}\\b`, 'g'), display);
    }
    const parts = display.split(/\s+/).filter(Boolean);
    if (parts.length > 1) {
      const shortName = parts[0];
      if (shortName.length >= 3 && shortName.toLowerCase() !== display.toLowerCase()) {
        out = out.replace(
          new RegExp(`\\b${escapeRegExp(shortName)}\\b(?!\\s*${escapeRegExp(parts.slice(1).join(' '))})`, 'gi'),
          display,
        );
      }
    }
    const compound = display.match(/^([A-Z][a-z]+)([A-Z][a-z].*)$/);
    if (compound && compound[2].length >= 3) {
      out = out.replace(new RegExp(`\\b${escapeRegExp(compound[2])}\\b`, 'gi'), display);
    }
  }
  return out;
}

/** Ensure locked intro/outro key phrases appear in INTRO / OUTRO sections (deterministic Gate 1 pass). */
function enforceLockedIntroOutro(script, lockedIntro, lockedOutro) {
  if (typeof script !== 'string' || !script.trim()) return script;
  let out = script;
  if (lockedIntro) {
    ...
    out = out.replace(
      ...
      (match, hdr, body) => {
        ...
        if (trimmed.includes(keyPhrase)) return match;
        return hdr + lockedIntro.trim() + (trimmed ? `\n\n${trimmed}` : '');
      },
    );
  }
  if (lockedOutro) {
    const keyPhrase = lockedOutro.substring(0, 50).trim();
    out = out.replace(/(===\s*OUTRO\s*===\s*\n?)([\s\S]*)$/i, (match, hdr, body) => {
      const trimmed = body.trim();
      if (trimmed.includes(keyPhrase)) return match;
      return hdr + lockedOutro.trim();
    });
  }
  return out;
}
...
/**
 * Full pre–Gate 1 script prep: structure normalize + locked intro/outro + entity names.
 */
function prepareScriptForGate1(script, opts = {}) {
  const names = (opts.expectedHeaders || opts.expectedNames || [])
    .map((h) => String(h || '').trim())
    .filter(Boolean);
  let out = normalizeScriptForGate1(script, names);
  if (opts.lockedIntro || opts.lockedOutro) {
    out = enforceLockedIntroOutro(out, opts.lockedIntro, opts.lockedOutro);
  }
  if (opts.streamers?.length) {
    out = alignSpokenEntityNames(out, opts.streamers);
  }
  return out;
}
 ...
 */
function buildClipsLong(items, clipsPerStreamer = 2) {
  const scenes = [];
...
function generateScaffold(jobSpec) {
  ...
  // Pass clipsPerStreamer for clips-long — UI default is 2; never silently assume 3.
...
module.exports = {
  ...
  alignSceneHeadersToContract,
  normalizeScriptForGate1,
  prepareScriptForGate1,
  alignSpokenEntityNames,
  enforceLockedIntroOutro,
  getLockedIntro,
  getLockedOutro,
