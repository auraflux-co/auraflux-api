...
/**
 * clips-long: 1 INTRO + (items × 7 scenes) + 1 OUTRO
 * Per item: ITEM#_INTRO, ITEM#_CLIP1, ITEM#_REACTION1, ITEM#_CLIP2, ITEM#_REACTION2, ITEM#_CLIP3, ITEM#_OUTRO
 * Universal naming — ITEM1 not streamer name (assembly reads streamer data from cardData, not scene name)
 */
function buildClipsLong(items) {
  ...
  for (let i = 0; i < items.length; i++) {
    ...
    scenes.push(scene(`ITEM${n}_CLIP1`, true));
    scenes.push(scene(`ITEM${n}_REACTION1`));
    scenes.push(scene(`ITEM${n}_CLIP2`, true));
    scenes.push(scene(`ITEM${n}_REACTION2`));
    scenes.push(scene(`ITEM${n}_CLIP3`, true));
    scenes.push(scene(`ITEM${n}_OUTRO`));
  }
...
/**
 * news-short: INTRO + SETUP + HOOK + OUTRO (avatar only, no [CLIP PLAYS HERE])
 * Universal scene names — no customer/content-type specific naming.
 * HeyGen only needs unique scene IDs, not content-type labels.
 */
...
/**
 * clips-short: INTRO + HOOK + CLIP + REACTION + OUTRO
 * Universal scene names — content-agnostic identifiers.
 */
...
/**
 * sports-short: INTRO + CLIP + NARRATION + OUTRO
 * Universal scene names — no NBA/ESPN/sport-specific naming.
 * [CLIP PLAYS HERE] is the marker that routes the source clip at assembly time.
 */
