...
function buildUniversalLong(items, sceneShape, customerId = 'c0') {
  const shape = (sceneShape && sceneShape.length > 0)
...
function buildUniversalShort() {
  return [
    scene('ITEM1_HOOK'),
    scene('ITEM1_CLIP', true),
    scene('ITEM1_REACTION')
  ];
}
...
function inferTemplateId(jobSpec) {
  ...
  else if (['nba', 'sports', 'basketball', 'football'].some(t => type.includes(t))) type = 'sports';
