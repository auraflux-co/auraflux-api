'use strict';
/**
 * lib/queue/worker.js — BullMQ pipeline worker (CPD-324)
 *
 * Picks up jobs from the 'pipeline' queue and runs them through
 * runPortalSequence — the same logic that was previously inline in
 * jobs_c1.js via setImmediate.
 *
 * Survives server restarts: unprocessed jobs stay in Redis and are
 * claimed by the worker when the server comes back up.
 *
 * Called from lib/server.js via startPipelineWorker().
 */

const { Worker } = require('bullmq');
const { redisConnection, QUEUE_NAME } = require('./index');
const { logError } = require('../error_logger');

// CPD-579: Default 1 — concurrent longform jobs (873s clips, caption burn-in) OOM the 2GB Standard instance.
// Override via PIPELINE_WORKER_CONCURRENCY env var if needed.
const CONCURRENCY = parseInt(process.env.PIPELINE_WORKER_CONCURRENCY || '1', 10);

let _worker = null;
let _memWatcher = null; // CPD-325: memory circuit breaker interval handle

/**
 * Process a single pipeline job from the queue.
 * This mirrors the setImmediate body from jobs_c1.js POST /jobs.
 */
async function processPipelineJob(bullJob) {
  const { jobSpec } = bullJob.data;
  const jobId = jobSpec?.jobId || bullJob.id;

  console.log(`[pipeline-worker] picking up job ${jobId} (attempt ${bullJob.attemptsMade + 1})`);

  // ── Credit + VOD hours deduction ─────────────────────────────────────────────
  const customerId = jobSpec?.customerId;
  const creditCost = jobSpec?.creditCost || 0;

  if (customerId) {
    const { consumeVodHours, hoursFromDurationSecs } = require('../services/vod_hours');
    const srcSecs = Number(jobSpec?.sourceDurationSecs || jobSpec?.order?.sourceDurationSecs || 0);
    const vodHours = hoursFromDurationSecs(srcSecs || 3600);
    const vodResult = await consumeVodHours(customerId, jobId, vodHours, jobSpec?.brandId || null);
    if (!vodResult.ok) {
      logError('VOD_HOURS_PAUSED', vodResult.reason, { jobId, customerId });
      const { saveJob } = require('../db');
      await saveJob(jobId, {
        ...jobSpec,
        status: 'credit_paused',
        pauseReason: vodResult.reason,
        updatedAt: new Date().toISOString(),
      }).catch(() => {});
      return { skipped: true, reason: 'vod_hours_paused' };
    }
  }

  if (creditCost > 0 && customerId) {
    const { consumeCredits } = require('../services/credits');
    const creditResult = await consumeCredits(customerId, jobId, creditCost, jobSpec?.brandId || null);

    if (!creditResult.ok) {
      if (creditResult.status === 'PAUSED') {
        logError('CPD120_CREDIT_PAUSED', creditResult.reason, { jobId, customerId });
        const { saveJob } = require('../db');
        await saveJob(jobId, { ...jobSpec, status: 'credit_paused', updatedAt: new Date().toISOString() }).catch(() => {});
        return { skipped: true, reason: 'credit_paused' };
      }
      if (creditResult.status !== 'ALREADY_CHARGED') {
        logError('CPD120_CREDIT_WARN', creditResult.reason || creditResult.status, { jobId });
      }
    }
  }

  // ── Pre-pipeline steps (mirrors jobs_c1.js) ─────────────────────────────────
  const sourceType = jobSpec?.order?.sourceType || jobSpec?.sourceType;

  if (sourceType === 'wan_gen') {
    const { _runWanPreGeneration } = require('../routes/jobs_c1');
    const ok = await _runWanPreGeneration(jobSpec, jobId);
    if (!ok) {
      throw new Error(`WAN pre-generation failed for job ${jobId}`);
    }
  }

  if (sourceType === 'research_query') {
    const { _runWebResearchPreStep } = require('../routes/jobs_c1');
    await _runWebResearchPreStep(jobSpec, jobId);
  }

  if (jobSpec.addOns?.clipSourcing?.active) {
    try {
      const { isFeatureEnabled } = require('../services/feature_gate');
      if (isFeatureEnabled('clip.sourcing', jobSpec.planTier)) {
        const clipSourcingSvc = require('../clip_sourcing');
        await clipSourcingSvc.runForJob(jobSpec).catch((e) =>
          console.warn(`[pipeline-worker] ${jobId}: clip sourcing failed (non-fatal) — ${e.message}`)
        );
      }
    } catch (_e) {
      console.warn(`[pipeline-worker] ${jobId}: clip sourcing unavailable — ${_e.message}`);
    }
  }

  // ── CPD-948: Gemini autonomous template selection ───────────────────────────
  try {
    const { selectTemplateForJob } = require('../services/template_selector');
    await selectTemplateForJob(jobSpec);
  } catch (_e) {
    console.warn(`[pipeline-worker] ${jobId}: template selection failed (non-fatal) — ${_e.message}`);
  }

  try {
    const { generateJobScript } = require('../script_gen_service');
    const { filledScript, orderedClipUrls, publishCopy } = await generateJobScript(jobSpec);
    // Write generated script back into the in-memory jobSpec so Portal 1 can read it.
    if (filledScript) {
      jobSpec.filledScript = filledScript;
    }
    if (orderedClipUrls && orderedClipUrls.length) {
      jobSpec.orderedClipUrls = orderedClipUrls;
    }
    if (publishCopy) {
      jobSpec.state = jobSpec.state || {};
      jobSpec.state.savedOutputs = jobSpec.state.savedOutputs || {};
      jobSpec.state.savedOutputs.publishCopy = publishCopy;
    }
  } catch (_e) {
    console.warn(`[pipeline-worker] ${jobId}: script pre-gen failed (non-fatal) — ${_e.message}`);
  }

  // ── CPD-898: Checkpoint resume — skip portals already completed on a prior attempt ──
  const _lastPassingPortal = jobSpec?.state?.checkpoints?.lastPassingPortal || null;
  let _resumeFromPortal = null;
  if (_lastPassingPortal && bullJob.attemptsMade > 0) {
    // Guard: if assembly was triggered by the checkpointed portal, the assembled file
    // must still exist on disk. Render has an ephemeral filesystem — if the file is gone
    // we must restart from the beginning despite the checkpoint.
    const _assembledPath = jobSpec?.assembledPath || jobSpec?.state?.savedOutputs?.assembledPath;
    const _fs = require('fs');
    const _assemblyPortals = new Set(['portal0', 'portal1']);
    const _assemblyWasTriggered = _assemblyPortals.has(_lastPassingPortal);
    const _assembledFileExists  = _assembledPath && _fs.existsSync(_assembledPath);

    if (!_assemblyWasTriggered || _assembledFileExists) {
      _resumeFromPortal = _lastPassingPortal;
      console.log(
        `[pipeline-worker] ${jobId}: attempt ${bullJob.attemptsMade + 1} — ` +
        `resuming from after ${_lastPassingPortal} (checkpoint)`
      );
    } else {
      console.log(
        `[pipeline-worker] ${jobId}: attempt ${bullJob.attemptsMade + 1} — ` +
        `checkpoint ${_lastPassingPortal} exists but assembled file missing — restarting from portal0`
      );
    }
  }

  // ── Portal sequence ─────────────────────────────────────────────────────────
  const { runPortalSequence } = require('../portal_policy_runner');
  const { _resolvePortalWorkers, _resolveExtensionWorkers } = require('../routes/jobs_c1');

  const { nrJobCreated, nrJobComplete, nrJobFailed, nrPortalStart, nrPortalPass, nrPortalFail } = require('../nr_events');
  const _jobStartMs = Date.now();
  nrJobCreated(jobSpec);

  const {
    runAssemblyAndPostProcess,
    isPortal1Active,
    runTtsMixAndChrome,
    ensureChromeApplied,
    runJobComplete,
  } = require('../services/pipeline_assembly');

  const db = require('../db');
  const _persist = (patch) => db.saveJob(jobId, { ...jobSpec, ...patch }).catch(() => {});

  const result = await runPortalSequence({
    jobSpec,
    portalWorkers:    _resolvePortalWorkers(jobSpec),
    extensionWorkers: _resolveExtensionWorkers(jobSpec),
    resumeFromPortal: _resumeFromPortal,
    onPortalStart: (portalKey) => {
      console.log(`[pipeline-worker] ${jobId}: portal ${portalKey} started`);
      nrPortalStart(jobSpec, portalKey);
      _persist({ status: 'running', currentPortal: portalKey, updatedAt: new Date().toISOString() });
    },
    onPortalPass: async (portalKey, r) => {
      console.log(`[pipeline-worker] ${jobId}: portal ${portalKey} passed`);
      nrPortalPass(jobSpec, portalKey, r?.score);

      // Write portal result into jobSpec.portalReports so downstream portals
      // (portal3b reads portal3a result) can access it via _adaptLegacyPortal.
      if (r && typeof r === 'object') {
        jobSpec.portalReports = jobSpec.portalReports || {};
        jobSpec.portalReports[portalKey] = r;
      }

      // CPD-485: assembly after portal1, or after portal0 when portal1 is inactive (clip-only jobs)
      if ((portalKey === 'portal1') || (portalKey === 'portal0' && !isPortal1Active(jobSpec))) {
        await runAssemblyAndPostProcess(jobSpec, jobId, {
          logPrefix: '[pipeline-worker]',
          persist:   _persist,
        });
      }

      // CPD-898: persist checkpoint to DB after each portal pass + any assembly step
      // runPortalSequence writes to jobSpec.state.checkpoints in-memory; flush here.
      _persist({
        state: jobSpec.state,
        updatedAt: new Date().toISOString(),
      });

      // CPD-485: TTS mix + chrome after tts_ext
      if (portalKey === 'tts_ext') {
        const ttsAudio = r?.audioPath || jobSpec.state?.tts?.audioPath;
        if (ttsAudio) {
          await runTtsMixAndChrome(jobSpec, jobId, ttsAudio, {
            logPrefix: '[pipeline-worker]',
            persist:   _persist,
          });
        }
      }

      // CPD-485: chrome safety net on portal3a
      if (portalKey === 'portal3a') {
        await ensureChromeApplied(jobSpec, jobId, { logPrefix: '[pipeline-worker]' });
      }
    },
    onPortalFail: (portalKey, r) => {
      console.log(`[pipeline-worker] ${jobId}: portal ${portalKey} failed`);
      nrPortalFail(jobSpec, portalKey, r?.failReason || r);
      _persist({ status: 'non-compliant', currentPortal: portalKey, failedPortal: portalKey, updatedAt: new Date().toISOString() });
    },
    // CPD-485: shared handler — grades, persists, notifies (was logging-only before)
    onJobComplete: () => runJobComplete(jobSpec, jobId, {
      jobStartMs:    _jobStartMs,
      logPrefix:     '[pipeline-worker]',
      nrJobComplete,
    }),
    onJobFailed: async (failedPortal, r) => {
      console.log(`[pipeline-worker] ${jobId}: failed at ${failedPortal}`);
      nrJobFailed(jobSpec, failedPortal, r);
      Object.assign(jobSpec, { status: 'failed', failedPortal, updatedAt: new Date().toISOString() });
      try {
        await db.updateJobSpec(jobId, jobSpec);
        await db.saveJob(jobId, jobSpec);
      } catch (_e) {}
    },
    persistJobStatus: ({ portalKey: pKey, phase }) => {
      _persist({ status: phase, currentPortal: pKey, updatedAt: new Date().toISOString() });
    },
  });

  return result;
}

function startPipelineWorker() {
  if (_worker) return _worker;

  let connection;
  try {
    connection = redisConnection();
  } catch (err) {
    console.warn('[pipeline-worker] REDIS_URL not set — worker not started. Jobs will not process.');
    return null;
  }

  _worker = new Worker(QUEUE_NAME, processPipelineJob, {
    connection,
    concurrency: CONCURRENCY,
    lockDuration: 30 * 60 * 1000, // 30 min — long enough for a full pipeline run
  });

  _worker.on('completed', (job, result) => {
    console.log(`[pipeline-worker] ✅ Job ${job.data?.jobSpec?.jobId || job.id} completed`);
  });

  _worker.on('failed', async (job, err) => {
    const jid = job?.data?.jobSpec?.jobId || job?.id;
    const jobSpec = job?.data?.jobSpec;
    console.error(`[pipeline-worker] ❌ Job ${jid} failed (attempt ${job?.attemptsMade}): ${err.message}`);
    logError('PIPELINE_WORKER_FAIL', err, { jobId: jid, attempt: job?.attemptsMade });

    // CPD-999: BullMQ throws without portal reports — persist rootCause for operator/debug
    if (jobSpec && jid) {
      try {
        const db = require('../db');
        const patch = {
          ...jobSpec,
          status: 'failed',
          rootCause: (err.message || 'pipeline_worker_uncaught').slice(0, 500),
          failedPortal: jobSpec.failedPortal || jobSpec.currentPortal || 'worker',
          updatedAt: new Date().toISOString(),
        };
        await db.saveJob(jid, patch);
        await db.updateJobSpec(jid, patch).catch(() => {});
      } catch (_persistErr) {
        console.warn(`[pipeline-worker] ${jid}: failed to persist worker failure — ${_persistErr.message}`);
      }
    }
  });

  _worker.on('error', (err) => {
    console.error('[pipeline-worker] Worker error:', err.message);
  });

  // CPD-325: Memory circuit breaker — pause the worker when heap exceeds
  // MEM_PAUSE_THRESHOLD_MB (default 1200 MB = ~78% of 1536 MB cap).
  // Resumes automatically when heap drops below the resume threshold.
  // CPD-579: Lowered from 1200MB to 800MB — on 2GB Standard, caption+colorgrade FFmpeg
  // can spike to ~1.5GB; 800MB threshold gives the circuit breaker enough headroom to pause
  // before the instance is killed by the OOM reaper.
  const PAUSE_THRESHOLD = parseInt(process.env.WORKER_MEM_PAUSE_MB || '800', 10) * 1024 * 1024;
  const RESUME_THRESHOLD = parseInt(process.env.WORKER_MEM_RESUME_MB || '900', 10) * 1024 * 1024;
  const MEM_POLL_MS = 10_000; // check every 10 seconds
  let _memPaused = false;

  _memWatcher = setInterval(() => {
    const heapUsed = process.memoryUsage().heapUsed;
    if (!_memPaused && heapUsed > PAUSE_THRESHOLD) {
      _memPaused = true;
      _worker.pause().catch(() => {});
      console.warn(
        `[pipeline-worker] ⚠️  Heap ${Math.round(heapUsed / 1024 / 1024)}MB exceeds ${Math.round(PAUSE_THRESHOLD / 1024 / 1024)}MB — worker paused until memory drains`
      );
      logError('WORKER_MEM_PAUSE', new Error('heap pressure'), {
        heapUsedMB: Math.round(heapUsed / 1024 / 1024),
        thresholdMB: Math.round(PAUSE_THRESHOLD / 1024 / 1024),
      });
    } else if (_memPaused && heapUsed < RESUME_THRESHOLD) {
      _memPaused = false;
      _worker.resume().catch(() => {});
      console.log(
        `[pipeline-worker] ✅ Heap ${Math.round(heapUsed / 1024 / 1024)}MB — worker resumed`
      );
    }
  }, MEM_POLL_MS);
  if (_memWatcher.unref) _memWatcher.unref();

  console.log(`[pipeline-worker] Started (concurrency=${CONCURRENCY}, memPause=${Math.round(PAUSE_THRESHOLD/1024/1024)}MB)`);
  return _worker;
}

function stopPipelineWorker() {
  if (_memWatcher) { clearInterval(_memWatcher); _memWatcher = null; }
  if (_worker) {
    _worker.close();
    _worker = null;
  }
}

module.exports = { startPipelineWorker, stopPipelineWorker };
