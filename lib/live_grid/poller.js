 ...
 *    highest-ranked live streamer not already on the grid (roster → follows → bench).
 ...
 * @param {Object} opts { challengeRatio, challengeStreak, exclude:Set, bench:Set, follows:Set }
...
function computeAssignments(current, live, streaks, opts = {}) {
  ...
  const follows = opts.follows || new Set();
  const operatorMode = !!opts.operatorMode;
  const slateLocked = opts.slateLocked || [];
  /** 0 = roster, 1 = Twitch follows, 2 = platform / discovery bench */
  const tier = (login) => {
    if (!bench.has(login)) return 0;
    if (follows.has(login)) return 1;
    return 2;
  };
  ...
  if (operatorMode) {
    // Operator locks the lineup — still evict offline and fill roster → follows → bench; no viewer-challenge reshuffles.
    dropOfflineAndFill();
    for (let q = 0; q < GRID_SIZE; q++) {
      if (slateLocked[q]) assignments[q] = null;
    }
    return { assignments: [...assignments], streaks: {}, swaps };
  }

  const ranked = dropOfflineAndFill();
  const onGrid = () => new Set(assignments.filter(Boolean));
...
class LiveGridPoller extends EventEmitter {
  constructor(options = {}) {
    ...
    this.slateLocked = [false, false, false, false]; // operator forced slate — no auto-fill
    this.benchFollows = [];
    this.allFollows = [];
  ...
  async fetchLive() {
    const seated = (this.assignments || []).filter(Boolean).map((l) => String(l).toLowerCase());
    const pinned = (this.pinned || []).filter(Boolean).map((l) => String(l).toLowerCase());
    const priority = [...new Set([...this.roster, ...seated, ...pinned])];
    const prioritySet = new Set(priority);
    const rest = [...new Set([
      ...this.bench,
      ...(this.benchFollows || []),
      ...(this.allFollows || []),
    ])].filter((l) => !prioritySet.has(l));
    const logins = [...priority, ...rest].slice(0, 100); // Helix cap — roster/seated always win
  ...
  /** Operator forced slate on quadrant q — auto pilot must not backfill. */
  lockSlate(q) {
    if (q < 0 || q > 3) return;
    this.slateLocked[q] = true;
    this.assignments[q] = null;
    this.pinned[q] = null;
  }

  unlockSlate(q) {
    if (q >= 0 && q < 4) this.slateLocked[q] = false;
  }

  /** Operator atomic seat — one feeder swap, no pollOnce auto-fill between remove and replace. */
  setOperatorSeat(q, login) {
    if (q < 0 || q > GRID_SIZE - 1) return null;
    const prev = this.assignments[q];
    const next = login ? String(login).toLowerCase() : null;
    this.assignments[q] = next;
    this.pinned[q] = next;
    if (next) this.exclude.delete(next);
    return { prev, next };
  }
  ...
  async pollOnce() {
    ...
    const { assignments, streaks, swaps } = computeAssignments(
      this.assignments, live, this.streaks,
      {
        ...
        bench: this.bench,
        follows: new Set(this.benchFollows || []),
        operatorMode: this.operatorMode,
        slateLocked: this.slateLocked,
      }
    );
    // Member pins override ranking only while the streamer stays live.
    for (let q = 0; q < GRID_SIZE; q++) {
      if (this.slateLocked[q]) {
        assignments[q] = null;
        continue;
      }
