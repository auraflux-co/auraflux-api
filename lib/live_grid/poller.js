...
class LiveGridPoller extends EventEmitter {
   ...
   * A login removed from both lists drops out of fetchLive, so the ranking
   * step evicts it naturally. Returns the updated lists.
   */
  updateRoster({ add = [], remove = [], benchAdd = [], benchRemove = [] } = {}) {
    ...
    for (const l of norm(remove)) this.roster = this.roster.filter(r => r !== l);
    for (const l of norm(benchAdd)) { if (!this.roster.includes(l)) this.bench.add(l); }
    for (const l of norm(benchRemove)) this.bench.delete(l);
    return { roster: [...this.roster], bench: [...this.bench] };
  }
  ...
  async pollOnce() {
    ...
    const { assignments, streaks, swaps } = computeAssignments(
      ...
      { challengeRatio: this.challengeRatio, challengeStreak: this.challengeStreak, exclude: this.exclude, bench: this.bench }
    );
