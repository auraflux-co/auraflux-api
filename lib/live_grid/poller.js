...
function computeAssignments(current, live, streaks, opts = {}) {
  ...
  if (operatorMode) {
    // Strict operator: grid is frozen — no offline drops, no bench fill, no viewer swaps.
    // Operator removes/replaces via dashboard only; feeders keep retrying offline streams.
...
class LiveGridPoller extends EventEmitter {
  ...
  async pollOnce() {
    ...
    // Member pins override ranking while the streamer stays live (operator: keep pin even if Helix blips offline)
    for (let q = 0; q < GRID_SIZE; q++) {
      const pin = this.pinned[q];
      if (!pin) continue;
      if (this.operatorMode || (live[pin] != null && !this.exclude.has(pin))) assignments[q] = pin;
