...
class SoloRosterOrchestrator {
  ...
  async start() {
    ...
    const slotMap = loginSlotMapForBindings(this.fleetId);
    for (const s of this.slots) {
      const listing = listingFromSlot(s.localPool);
      if (!listing?.rtmpUrl) {
        throw new Error(`pool slot ${s.localPool} (${s.login}) missing LIVE_GRID_SOLO_${s.localPool}_RTMP_URL`);
      }
    }
    applyLoginSlotMap(slotMap, this.slots.map((s) => s.login));
