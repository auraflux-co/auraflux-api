...
class SoloRosterOrchestrator {
  ...
  async startSlot(slotDef) {
    if (isSlotPaused(slotDef)) {
      const { fleetPausedReason } = require('./solo_roster_fleet');
      const why = slotDef.pausedReason || fleetPausedReason();
      this.log(`slot ${slotDef.slot} @${slotDef.login} paused — skip start (${why})`);
      return;
    }
    const q = slotDef.localIndex;
