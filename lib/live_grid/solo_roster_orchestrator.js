...
class SoloRosterOrchestrator {
  ...
  async _validateKickRoster() {
    ...
    for (const s of this.slots) {
      ...
      let data;
      try {
        data = await fetchKickChannelApi(slug);
      } catch (e) {
        throw new Error(`fleet slot ${s.slot}: Kick API failed for kick.com/${slug}: ${e.message}`);
      }
      if (!data) {
        throw new Error(
  ...
  _feederIngestReady(q) {
    ...
    return !!(st && st.kind === 'url' && (st.pids?.length || 0) > 0);
  }
  ...
  async startSlot(slotDef) {
    ...
    try {
      ...
      const ingestOk = await this._waitFeederReady(q);
      if (!ingestOk) {
