...
function soloEncodeSnapshot(q, login = null) {
  const seat = q + 1;
  const dims = soloOutputDims(q, login);
  const encodeSeat = require('./solo_streamer_registry').resolveSoloEncodeSeat(q, login);
  const h = dims.h;
  ...
  return {
    ...
    quadrant: seat,
    login: login || null,
    poolSlot: dims.poolSlot || null,
    w: dims.w,
    h: dims.h,
    fps: dims.fps,
    bitrateK: dims.bitrateK,
    maxrateK: readSoloSeatInt(encodeSeat, 'X264_MAXRATE_K', dims.bitrateK),
    bufsizeK: readSoloSeatInt(encodeSeat, 'X264_BUFSIZE_K', dims.bitrateK * 2),
...
function buildEncodeContractSnapshot(runtime = {}) {
  ...
  const solos = [0, 1, 2, 3].map((q) => ({
    ...soloEncodeSnapshot(q, runtime.assignments?.[q] || null),
