...
/** Kick live detection — OAuth API, then kick_fetch v2, then streamlink fallback. */
async function kickChannelLive(slug, timeoutMs = PROBE_TIMEOUT_MS) {
  const normalized = String(slug || '').trim().toLowerCase();
  if (!normalized) return false;

  try {
    const { fetchKickChannelOAuth } = require('../clients/kick_live_resolver');
    const oauth = await fetchKickChannelOAuth(normalized);
    if (oauth?.isLive) return true;
    if (oauth?.source === 'kick_oauth') return false;
  } catch {
    /* OAuth optional — fall through */
  }
