 ...
 * │         video Q1          video Q2   (flush — no gold divider)      │
...
/** Horizontal gold bar between top/bottom rows — default off (flush 2×2). Set LIVE_GRID_FRAME_HGUTTER=on to restore. */
function frameHgutterEnabled() {
  return String(process.env.LIVE_GRID_FRAME_HGUTTER || 'off').toLowerCase() === 'on';
}
...
function gridFrameMetrics(outW = 1920, outH = 1080) {
  ...
  /** Gap between top/bottom row blocks — only when horizontal gold gutter is on. */
  const rowGap = frameHgutterEnabled() ? borderW : 0;
  /** Two columns flush — no vertical center seam. */
  const cellW = Math.floor(innerW / 2);
  const rowSpanW = cellW * 2;
  const gridH = innerH - stripH;
  const rowBlockH = Math.floor((gridH - rowGap) / 2);
...
function cellBlockOrigin(q, m) {
  ...
  const col = q % 2;
  const y = m.innerY + m.stripH + row * (m.rowBlockH + m.rowGap);
  const x = colX(col, m);
  return {
    x,
    y,
...
function nameClusterHalfWidth(nameText, m) {
  const text = String(nameText || '').trim();
  if (!text) return m.nameClusterHalfMax;
  const fromChars = text.length * m.nameCharHalfW;
  return Math.min(m.nameClusterHalfMax, Math.max(28, fromChars));
}
...
function nameStripFlankPositions(q, m, nameText) {
  ...
  const clusterHalf = nameClusterHalfWidth(nameText, m);
...
function xstackLayout(m) {
  const x1 = colX(1, m);
  const y1 = m.innerY + m.stripH + m.rowBlockH + m.rowGap;
...
/** Optional gold row divider between top/bottom quads (off by default). */
function gutterFilters(m) {
  if (!frameHgutterEnabled()) return [];
...
/** Overlay chain after [grid]: on-air quad avatar + ON AIR badge flanking the name. */
function nameStripImageOverlays(m, audioQuad, opts, esc, inputIdx) {
  ...
  const pos = onAir ? nameStripFlankPositions(audioQuad, m, readNameText(audioQuad)) : null;
  const avSize = pos?.avatarSize ?? m.avatarSize;
  const badgeW = pos?.badgeW ?? m.badgeW;
  const badgeH = pos?.badgeH ?? m.badgeH;
  const avIn = onAir ? avBase + audioQuad : avBase;
  const parts = [];
  parts.push(buildAvatarFilter(avIn, avSize, 'avpic'));
  parts.push(buildOnAirBadgeFilter(badgeIdx, badgeW, badgeH));
  parts.push(
    `[grid][avpic]overlay@onairav=x=${pos ? pos.avatarX : hidden}:y=${pos ? pos.avatarY : 0}:format=auto[gav]`,
    `[gav][onairpic]overlay@onairbadge=x=${pos ? pos.badgeX : hidden}:y=${pos ? pos.badgeY : 0}:format=auto[gridout]`,
  );
  return parts.join(';');
}
...
function audioFrameZmqCommands(q, m, { muted, fallbackMusicActive } = {}) {
  ...
  if (frameHgutterEnabled()) {
    cmds += `cdrawbox@hgutter -1 color ${BRAND.accent}@1\n`;
  }
  const flank = onAir ? nameStripFlankPositions(q, m, readNameText(q)) : null;
  const hidden = -400;
  cmds += `coverlay@onairav -1 x ${flank ? flank.avatarX : hidden}\n`;
  cmds += `coverlay@onairav -1 y ${flank ? flank.avatarY : 0}\n`;
...
function ensureOnAirBadgeAsset() {
  const dir = path.dirname(ON_AIR_BADGE_PATH);
  if (!fs.existsSync(dir)) fs.mkdirSync(dir, { recursive: true });
  if (!fs.existsSync(ON_AIR_BADGE_PATH)) {
    console.warn(`[live-grid:overlay] missing ${ON_AIR_BADGE_PATH} — on-air badge/avatar flanks disabled`);
  }
}

function overlayAssetsReady(imageInputs) {
  ensureOnAirBadgeAsset();
  const badgeOk = fs.existsSync(ON_AIR_BADGE_PATH);
  const avatarOk = imageInputs.slice(0, 4).some(
    (img) => fs.existsSync(img.path) && fs.statSync(img.path).size > 200,
  );
  return badgeOk || avatarOk;
}
