...
function quadrantDisplayInfo(quad) {
  ...
  if (quad.login) displayName = quad.login;
  else if (quad.label && slug) displayName = `${quad.label} · ${slug}`;
  else if (quad.label) displayName = quad.label;
  else if (quad.feedUrl && slug) displayName = `FEED · ${slug}`;
...
class QuadrantFeeders {
  ...
  setQuadrant(q, login) {
    ...
    if (quad.login === login && quad.kind === nextKind && !quad.filePath && !quad.feedUrl) return;
    this.log(`quad${q + 1}: ${quad.label || quad.login || quad.feedUrl || quad.kind || 'empty'} → ${login || 'slate'}`);
    this._kill(q);
    quad.login = login;
    quad.filePath = null;
    quad.feedUrl = null;
  ...
  setQuadrantUrl(q, feedUrl, label = 'EVENT') {
  ...
  applySources(sources) {
    for (let q = 0; q < 4; q++) {
      ...
      if (s && typeof s === 'object' && s.type === 'file') {
        ...
        this.setQuadrantUrl(q, s.url, s.label);
      } else {
...
module.exports = { QuadrantFeeders, generateSlate, quadUrl, quadPublishUrl, nameFile, writeNameFile, GRID_DIR, SLATE_PATH, BRAND, channelFromFeedUrl, quadrantDisplayInfo };
