...
class QuadrantFeeders {
  ...
  setQuadrantUrl(q, feedUrl, label, opts = {}) {
    ...
    const quad = this.quads[q];
    const kick = isKickFeed({ kickSlug: opts.kickSlug || quad.kickSlug, url });
    if (quad.feedUrl === url && quad.kind === 'url' && quad.procs.length) return;
    if (kick && quad.kind === 'url' && quad.procs.length && kickPlaylistUrlsEquivalent(quad.feedUrl, url)) {
    ...
    quad.kickSlug = kick ? (opts.kickSlug || quad.login || null) : null;
    quad.filePath = null;
    quad.feedUrl = url;
  ...
  applySources(sources) {
    for (let q = 0; q < 4; q++) {
      ...
      if (s && typeof s === 'object' && s.type === 'file') {
        ...
        if (this.quads[q].kickSlug !== s.kickSlug) {
          this._kill(q);
          this.quads[q].kickSlug = s.kickSlug;
          this.quads[q].login = s.login || s.kickSlug;
          this.quads[q].label = s.label || s.kickSlug;
          writeNameFile(q, s.label || s.kickSlug);
          this._startSlate(q);
        }
