...
class QuadrantFeeders {
  ...
  /** Kick signed-URL token refresh — warm handoff: new ffmpeg before killing old (keeps RTSP alive). */
  refreshKickHlsUrl(q, feedUrl, opts = {}) {
    ...
    const oldProcs = quad.procs.slice();
    const oldGen = quad.gen;
    const gen = ++quad.gen;
    clearTimeout(quad.stallTimer);
    clearTimeout(quad.retryTimer);

    const kick = isKickFeed({ kickSlug: quad.kickSlug, url });
    const spawnEnv = kick
      ? { ...process.env, ...require('./kick_ingest').kickHlsFfmpegSpawnEnv() }
      : process.env;
    const newFf = spawn(FFMPEG, [
      ...hlsFfmpegArgs(url, { kickSlug: quad.kickSlug, url }),
      '-f', 'mpegts', quadPublishUrl(q),
    ], { stdio: ['ignore', 'ignore', 'pipe'], env: spawnEnv });

    let handoffDone = false;
    const WARM_MS = Number(process.env.LIVE_GRID_KICK_HLS_WARM_MS) || 20000;

    const finishHandoff = () => {
      if (handoffDone || quad.gen !== gen || this.stopped) {
        if (!handoffDone) { try { newFf.kill('SIGKILL'); } catch (_) {} }
        return;
      }
      handoffDone = true;
      clearTimeout(warmTimeout);
      for (const p of oldProcs) { try { p.kill('SIGKILL'); } catch (_) {} }
      quad.procs = [newFf];
      quad.kind = 'url';
      this._wireHlsFfmpeg(q, url, gen, newFf, { kickSlug: quad.kickSlug });
      this.log(`quad${q + 1}: Kick HLS warm handoff complete`);
    };

    const warmTimeout = setTimeout(() => {
      if (handoffDone || quad.gen !== gen) return;
      this.log(`quad${q + 1}: Kick HLS warm handoff timeout — cold restart`);
      try { newFf.kill('SIGKILL'); } catch (_) {}
      for (const p of oldProcs) { try { p.kill('SIGKILL'); } catch (_) {} }
      quad.procs = [];
      quad.gen = ++quad.gen;
      this._startHlsUrl(q, url, { kickSlug: quad.kickSlug, _refreshGen: quad.gen });
    }, WARM_MS);
    warmTimeout.unref?.();

    newFf.stderr.once('data', finishHandoff);
    newFf.on('exit', (code) => {
      if (handoffDone || quad.gen !== gen) return;
      clearTimeout(warmTimeout);
      quad.gen = oldGen;
      try { newFf.kill('SIGKILL'); } catch (_) {}
      this.log(`quad${q + 1}: Kick HLS warm refresh failed (${code}) — keeping previous feeder`);
    });
  }
  ...
  _wireHlsFfmpeg(q, url, gen, ff, opts = {}) {
    const quad = this.quads[q];
    ...
    const bumpStall = () => {
      ...
      quad.stallTimer = setTimeout(() => {
        if (quad.gen !== gen || this.stopped) return;
        if (kick && typeof this.onKickHlsStall === 'function' && this.onKickHlsStall(q, url, quad.kickSlug || null)) {
          this.log(`quad${q + 1}: Kick HLS stalled ${stallMs / 1000}s — fleet refresh (no slate)`);
          bumpStall();
          return;
        }
  ...
  _startHlsUrl(q, url, opts = {}) {
    ...
    quad.procs = [ff];
    this._wireHlsFfmpeg(q, url, gen, ff, { kickSlug: quad.kickSlug });
  }
