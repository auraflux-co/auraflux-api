...
function hostLoadPerCore() {
  const cores = Math.max(1, os.cpus().length);
  return +(os.loadavg()[0] / cores).toFixed(2);
}
...
function assessDelivery(input = {}) {
  ...
  const loadPerCore = master?.loadPerCore ?? hostLoadPerCore();

  // Direct RTMP — compositor feeds YouTube with no HLS tap or restreamer buffer.
  if (!outputMw && masterRunning) {
    if ((master.restarts || 0) > 0 && (master.restarts || 0) <= 2) {
      signals.push({
        key: 'master_restarts_rtmp',
        severity: 'warn',
        message: `Compositor restarted ${master.restarts} time(s) — RTMP blip likely`,
        metric: { restarts: master.restarts },
        selfHeal: null,
      });
      seeing.push('YouTube may buffer briefly after encoder recovery');
      score -= 12;
    }
    if (loadPerCore > 1.0) {
      const critical = loadPerCore > 1.85;
      signals.push({
        key: 'encode_cpu_saturated',
        severity: critical ? 'critical' : 'warn',
        message: `Host load ${loadPerCore}/core — libx264 may lag real time`,
        metric: { loadPerCore, cores: os.cpus().length },
        selfHeal: critical ? 'downshift_encode' : null,
      });
      if (critical) {
        seeing.push('Video freezing or spinning loader on YouTube');
        selfHealActions.push('downshift_encode');
        score -= 30;
      } else {
        seeing.push('Occasional stutter possible under CPU pressure');
        score -= 10;
      }
    }
  }
  ...
  if (encode.canvas && youtube?.deliveryWidth && youtube?.deliveryHeight) {
    const [cw, ch] = String(encode.canvas).split('×').map((n) => parseInt(n, 10));
    const yw = youtube.deliveryWidth;
    const yh = youtube.deliveryHeight;
    if (cw && ch && yw && yh && cw !== yw && Math.abs(cw - yw) > 80) {
      signals.push({
        key: 'youtube_aspect_mismatch',
        severity: 'warn',
        message: `Encode ${encode.canvas} but YouTube CDN ${yw}×${yh} — pillarbox risk`,
        metric: { encode: encode.canvas, youtube: `${yw}×${yh}` },
        selfHeal: null,
      });
      seeing.push('Stream may look small in the YouTube player');
      score -= 15;
    }
  }
  ...
  if (youtube?.staleLocal) {
    signals.push({
      ...
      selfHeal: outputMw ? 'restart_encode_pipeline' : null,
    });
    seeing.push('Watch page may show offline or ended');
    if (outputMw) selfHealActions.push('restart_encode_pipeline');
    score -= 35;
  }
