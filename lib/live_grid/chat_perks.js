...
class LikeTracker {
   ...
   */
  constructor({ videoId, log, postMessage, milestone, intervalMs } = {}) {
  ...
  async _tick() {
    ...
    if (bucket > this._lastAnnounced && this.postMessage) {
      this._lastAnnounced = bucket;
      this.log(`like milestone: ${likeCount}`);
      await this.postMessage(
        `❤️ ${likeCount.toLocaleString()} likes — thank you! Likes help us in the algorithm. ` +
        'Members: !listen 1-4 for instant audio · !swap N streamer to pick a live screen'
      );
    }
  }
}
