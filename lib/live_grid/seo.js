...
/** Fleet solo slot — live mirror of one Twitch creator (YouTube description block). */
const LIVE_MIRROR_DISCLAIMER = [
  '⚠️ COPYRIGHT & FORMAT NOTICE',
  'ClipzWorld News presents a live mirror / co-stream of the featured creator\'s own Twitch channel — transformative restream format for community watch-along only.',
  'We do NOT rebroadcast, simulcast, or replace licensed sports leagues, pay-per-view, network television, concert, premium fight/wrestling card, Olympic, or other copyrighted live event programming. This is not a primary broadcast of game feeds, stadium signals, TV carriage, or event-only streams.',
  'All trademarks, footage, and broadcast rights remain with their respective owners. Unauthorized redistribution is not intended; rights holders may contact ClipzWorld News with concerns.',
].join('\n');

/** 2×2 multiview grid — co-stream watch party (YouTube description block). */
const LIVE_GRID_CO_STREAM_DISCLAIMER = [
  '⚠️ COPYRIGHT & FORMAT NOTICE',
  'ClipzWorld\'s 2×2 multiview is a co-stream / reaction watch party — four Twitch creator feeds composited together, not a rebroadcast of licensed sports, pay-per-view, network TV, or other premium live event programming.',
  'We do NOT carry raw game feeds, stadium broadcasts, TV carriage, or event-only signals separate from each creator\'s own Twitch stream.',
  'All trademarks, footage, and broadcast rights remain with their respective owners; rights holders may contact ClipzWorld News with concerns.',
].join('\n');
...
function buildYoutubeTags(streamers = [], { mode = 'grid', headline = '' } = {}) {
  ...
  if (mode === 'solo') {
    pool.push('solo stream', 'full screen stream', 'twitch solo', 'clipzworld solo', 'multiview solo', 'screen stream');
  }
...
function buildGridLiveDescription({ streamers = [], hookLine } = {}) {
  ...
  return [
    ...
    '',
    LIVE_GRID_CO_STREAM_DISCLAIMER,
    '',
...
function buildLiveDescription({
  ...
  bodyParagraph,
  streamers = [],
  disclaimer = LIVE_GRID_CO_STREAM_DISCLAIMER,
  tags = [],
  hashtags = [],
  skipTagLine = true,
} = {}) {
