...
async function fetchPlatformTopLive(opts = {}) {
  ...
  try {
    ...
    if (resp.status === 429 || resp.status >= 400) {
      console.log(`[live-grid:discovery] platform top live unavailable: HTTP ${resp.status}`);
      return [];
    }
    ...
    console.log(`[live-grid:discovery] platform top live failed: ${e.response?.data?.message || e.message}`);
    return [];
  }
}
