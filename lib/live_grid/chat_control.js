...
const ANNOUNCE_EVERY_MS = parseInt(process.env.LIVE_GRID_CHAT_ANNOUNCE_MS || String(45 * 60_000), 10);
...
class ChatControl extends EventEmitter {
  ...
  async start() {
    ...
    this.log(`watching chat — !listen/!audio/!sound, subscriber !swap (chatId ${this.liveChatId.slice(0, 12)}…)`);

    if (this.announceText) {
      const lines = Array.isArray(this.announceText) ? this.announceText : [this.announceText];
      const announce = () => {
        (async () => {
          for (const line of lines) {
            await this.postMessage(line);
