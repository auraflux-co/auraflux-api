...
class LiveGridManager {
  ...
  async healDeliveryPipeline(reason = 'delivery_heal') {
    ...
    if (!hlsLive && this.master?.running) {
  ...
  _applyAudio(q, source) {
    ...
    this.audioQuad = q;
    if (this.master) {
      const { overlayAssetsReady, compositorImageInputs } = require('./brand_overlay');
      const overlaysOn = overlayAssetsReady(compositorImageInputs());
      if (overlaysOn) {
        this.refreshMasterEncoder('audio_pin');
        this.log(`on-air overlay refresh (~5s blip) — avatar/badge repositioned for quad${q + 1}`);
      } else {
