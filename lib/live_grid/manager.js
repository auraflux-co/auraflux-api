...
class LiveGridManager {
  ...
  async _prepareSoloBroadcasts() {
    ...
    for (let q = 0; q < 4; q++) {
      ...
      const login = this._lastAssignments?.[q] || listing.label || `Screen ${q + 1}`;
      let bid = listing.broadcastId;
      if (!(await this._canReuseBroadcastId(bid))) {
        const title = `🔴 LIVE: ${login} | ClipzWorld Screen ${q + 1}`;
        const broadcast = await yt.createLiveBroadcast({
          title,
          description: `Solo seat stream — ${login} on the ClipzWorld Live Grid (Screen ${q + 1}).`,
          privacyStatus: privacy,
      ...
      prepared.push({ quadrant: q + 1, broadcastId: bid, login });
    }
    return prepared;
  }
