...
const { resolveAvatarOverlay } = require('./avatar_overlay');
...
class LiveGridManager {
  ...
  async start(o = {}) {
    ...
    if (!bench) {
      ...
      refreshBench = async () => {
        const follows = await getFollowedBench({ roster: rosterList, exclude: o.exclude || [] });
        if (!usePlatform) return follows;
        const platform = await fetchPlatformTopLive();
        return mergePlatformBench({ roster: rosterList, follows: follows || [], platform });
      };
      bench = await refreshBench();
      if (bench?.length) {
        this.log(`bench: ${bench.length} channels (follows + platform top live)`);
      }
    }
    ...
    const avatarOverlay = o.avatarOverlay === false ? null : resolveAvatarOverlay({ path: o.avatarOverlay });
