...
const { QuadrantFeeders, generateSlate, quadUrl, writeNameFile } = require('./feeders');
...
const { existingIngestStream, allowNewIngestStream, resolveIngestForCreate } = require('./ingest_stream');
...
class LiveGridManager {
  ...
  async _canReuseBroadcastId(bid) {
    ...
    try {
      ...
      this.log(`broadcast status check failed (${e.message}) — will create fresh`);
      return false;
    }
  }
  ...
  async start(o = {}) {
    ...
    const resumeLocks = o._resumeRuntime?.operatorLocks;
    if (resumeLocks?.length) {
      for (let q = 0; q < 4; q++) this.feeders.setQuadrant(q, null);
    } else {
    ...
    if (localPreview.localOnly) {
      ...
      this.log(`LOCAL ONLY — composed grid → ${localPreview.hlsPath} (no YouTube RTMP)`);
    }
    const { verticalOutput: presetVertical, createVerticalBroadcast, legacyDual } =
      resolveVerticalStream({ verticalOutput: o.verticalOutput });
    ...
    if (!output) {
      ...
      const wantFresh = o.freshListing === true || (ALWAYS_FRESH_LISTING && ingest && yt.isConnected());

      if (wantFresh && ingest) {
        if (bid) {
          try {
            await yt.endLiveBroadcast(bid);
            this.log(`ended listing ${bid} — fresh listing on same RTMP key (16:9 / Studio dual-format)`);
          } catch (e) {
            this.log(`end listing ${bid} skipped: ${e.response?.data?.error?.message || e.message}`);
          }
        }
        ...
        if (canReuse) {
          ...
          const created = await this._createYoutubeBroadcast(o, layout, {
            existingStream: ingest || undefined,
          });
          output = created.output;
          if (created.verticalOutput) verticalOutput = created.verticalOutput;
          if (ingest) {
            this.log(`fresh listing on same RTMP ingest → ${this.broadcast?.watchUrl}`);
          } else {
            this.log(
              `fresh YouTube listing: ${this.broadcast?.watchUrl} — persist LIVE_GRID_RTMP_URL, ` +
              'LIVE_GRID_STREAM_ID, LIVE_GRID_BROADCAST_ID, LIVE_GRID_WATCH_URL in .env'
            );
          }
        } else if (o.broadcastId || process.env.LIVE_GRID_BROADCAST_ID) {
          const staleBid = o.broadcastId || process.env.LIVE_GRID_BROADCAST_ID;
          this.broadcast = {
            broadcastId: staleBid,
            watchUrl: o.watchUrl || process.env.LIVE_GRID_WATCH_URL || `https://youtube.com/live/${staleBid}`,
            streamId: o.streamId || process.env.LIVE_GRID_STREAM_ID || null,
          };
          this.log(`reusing YouTube broadcast ${staleBid}${output && !o.output ? ' (env RTMP — no API to verify status)' : ''}`);
    ...
    const localHlsPath = (!localPreview.localOnly && localPreview.hlsEnabled && output && /^rtmps?:/.test(output))
      ? localPreview.hlsPath
      : null;
    if (localHlsPath) {
      this.log(`local HLS tap → ${localPreview.hlsUrl} (same encode as RTMP — for localhost QA)`);
    }
    await this._guardRtmpLandscapeEncode(output, localHlsPath);
    ...
    if (this.poller?.operatorMode) this._lockCurrentGridAsOperator();
    if (this.broadcast?.broadcastId && this._programLayout) {
      await this._applyYoutubeSeoFromLayout(this._programLayout)
  ...
  _mergeOperatorLocks(sources, live = {}) {
    ...
    for (let q = 0; q < 4; q++) {
      ...
      if (lock.type === 'channel') {
        if (live[lock.login] != null || this.poller?.operatorMode) {
          out[q] = lock.login;
        } else {
  ...
  reloadEncodeSettings() {
    ...
    this.master.restart();
    return { ok: true, encode: cfg, broadcastId: this.broadcast?.broadcastId || null };
  }
  ...
  autoTuneEncodeIfNeeded() {
    ...
    if (cfg.bitrateK > 6500) { targetBitrate = 6500; changed = true; }
    else if (cfg.bitrateK > 4500 && loadPerCore > threshold * 1.8) { targetBitrate = 4500; changed = true; }
    ...
    this.log(`encode autotune: load ${loadPerCore.toFixed(2)}/core → ${next.fps}fps ${next.bitrateK}k`);
    this.master.restart();
  ...
  setOperatorMode(enabled) {
    ...
    this.log(this.poller.operatorMode
      ? 'operator mode ON — grid frozen; only you remove/replace (no auto viewer swaps or schedule reshuffles)'
  ...
  _benchUnhealthyQuadrant(q, info) {
    ...
    if (info.kind === 'url' && info.feedUrl) {
      ...
      this.poller?.updateRoster({ excludeAdd: [info.login] });
    }
