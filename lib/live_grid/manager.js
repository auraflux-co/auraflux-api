...
class LiveGridManager {
  ...
  async focusSoloQuadrant(quadrant, opts = {}) {
    ...
    const { readSoloListingForQuadrant } = require('./solo_listings_env');
    const listing = readSoloListingForQuadrant(seat);
    if (!listing?.rtmpUrl) throw new Error(`Q${q} solo listing not configured`);
    if (opts.broadcastId && listing.broadcastId && listing.broadcastId !== opts.broadcastId) {
      throw new Error(`Q${q} broadcastId mismatch (expected ${opts.broadcastId}, have ${listing.broadcastId})`);
    }
