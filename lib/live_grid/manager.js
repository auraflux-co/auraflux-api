...
class LiveGridManager {
  ...
  async focusSoloQuadrant(quadrant, opts = {}) {
    ...
    const { readSoloListingForQuadrant } = require('./solo_listings_env');
    const { streamerLockEnabled, getBinding, syncBindingsForAssignments } = require('./solo_streamer_registry');
    const heroLogin = normalizeSoloLogin(this._lastAssignments[seat] || opts.login);
    ...
    const heroLogin = this._lastAssignments[seat] || null;
    ...
    if (heroLogin) {
      try { this.setQuadrantChannel(seat, heroLogin); } catch (e) {
