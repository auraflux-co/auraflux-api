 ...
 * - Quadrant swaps use RTSP reconnect on master inputs (CPD-1005) — no
 *   planned master restart; a freeze watchdog restarts only if RTSP stays dead.
...
const MEMBER_INSTANT_AUDIO = String(process.env.LIVE_GRID_MEMBER_INSTANT_AUDIO || 'on').toLowerCase() !== 'off';
...
class LiveGridManager {
  ...
  async start(o = {}) {
    ...
    if (!output) {
      ...
      const title = o.title || seo?.title || gridTitle(assignments);
  ...
  _onChatCommand(cmd) {
    ...
    if (cmd.isMember && MEMBER_INSTANT_AUDIO) {
      ...
      if (this.setAudio(cmd.quadrant, 'chat')) {
        ...
        this.log(`audio → quad${cmd.quadrant + 1} (member instant: ${cmd.author})`);
        this.chat.postMessage(`⭐ Member perk — now hearing ${(login || '').toUpperCase()} (screen ${cmd.quadrant + 1})`)
          .catch(() => {});
      }
      return;
    }

    this._votes.set(cmd.author, { quadrant: cmd.quadrant, at: Date.now() });
    if (Date.now() - this._lastChatSwitch < CHAT_SWITCH_COOLDOWN_MS) return;
  ...
  _handleMemberSwap({ quadrant, login, author, isMember }) {
    if (!isMember) {
      this.chat?.postMessage('⭐ !swap is a member perk — subscribe to swap a live streamer onto a screen')
        .catch(() => {});
      return;
    }
    ...
    this._scheduleFreezeCheck(quadrant, login);
  ...
  _onAssignments(assignments) {
    ...
    const prev = [...this._lastAssignments];
    const changed = assignments.some((a, i) => a !== prev[i]);
    this.feeders.applyAssignments(assignments);
    this._lastAssignments = [...assignments];
    ...
    for (let q = 0; q < 4; q++) {
      if (assignments[q] !== prev[q]) this._scheduleFreezeCheck(q, assignments[q]);
    }
