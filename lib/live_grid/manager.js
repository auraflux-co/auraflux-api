...
class LiveGridManager {
  ...
  async focusSoloQuadrant(quadrant, opts = {}) {
    ...
    const { applySoloSeatEncodeOverrides } = require('./solo_focus');
    applySoloSeatEncodeOverrides(seat, {
      bitrateK: opts.bitrateK ?? 6000,
      maxrateK: opts.maxrateK ?? opts.bitrateK ?? 6000,
      bufsizeK: opts.bufsizeK ?? (opts.bitrateK ?? 6000) * 2,
      w: opts.w ?? 1920,
      h: opts.h ?? 1080,
      fps: opts.fps ?? parseInt(process.env.LIVE_GRID_FPS || '30', 10),
    });
    ...
    return {
      ...
      login: this._lastAssignments[seat] || null,
      encode,
