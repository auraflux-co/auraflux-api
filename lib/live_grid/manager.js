...
class LiveGridManager {
  ...
  /** Login for solo seat SEO — prefer live grid assignment, never generic "Screen N". */
  _resolveSoloLogin(q, listing) {
    const fromGrid = this._lastAssignments?.[q];
    const normalized = normalizeSoloLogin(fromGrid);
    if (normalized) return normalized;
    const fromLabel = normalizeSoloLogin(listing?.label);
    if (fromLabel) return fromLabel;
    return fromGrid ? String(fromGrid).trim().toLowerCase() : null;
  }

  /** Ensure each solo seat has an attachable YouTube broadcast (Studio Content + watch URL). */
  async _applySoloYoutubeSeo(q, broadcastId, login) {
    const mainUrl = this.broadcast?.watchUrl || process.env.LIVE_GRID_MAIN_WATCH_URL || '';
    const gridLogins = this._lastAssignments || [];
  ...
  async _prepareSoloBroadcasts() {
    const { readSoloListingForQuadrant, persistSoloListing } = require('./solo_listings_env');
    if (!yt.isConnected()) {
    ...
    for (let q = 0; q < 4; q++) {
      ...
      const login = this._resolveSoloLogin(q, listing) || `screen${q + 1}`;
      let bid = listing.broadcastId;
      if (!(await this._canReuseBroadcastId(bid))) {
        const seoPreview = buildSoloLiveSeo({ login, quadrant: q, gridLogins: this._lastAssignments || [] });
        const broadcast = await yt.createLiveBroadcast({
  ...
  async refreshSoloYoutubeSeo() {
    ...
    for (let q = 0; q < 4; q++) {
      const listing = readSoloListingForQuadrant(q);
      if (!listing?.broadcastId) continue;
      const login = this._resolveSoloLogin(q, listing) || `screen${q + 1}`;
      try {
        const { seo, result } = await this._applySoloYoutubeSeo(q, listing.broadcastId, login);
        seats.push({
          quadrant: q + 1,
        ...
        seats.push({ quadrant: q + 1, broadcastId: listing.broadcastId, error: e.message });
      }
    }
  ...
  async startSoloStreams() {
    ...
    await this._prepareSoloBroadcasts();
  ...
  _soloStatusSeats() {
    ...
    return [0, 1, 2, 3].map((q) => {
      ...
      return {
        quadrant: q + 1,
        login: this._lastAssignments[q] || null,
        running: false,
        restarts: 0,
  ...
  async _tryStartSoloStreamsWhenReady() {
    ...
    if (!this.running || this.soloPublishers?.started) return;
    const result = await this.startSoloStreams();
