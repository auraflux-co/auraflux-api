...
class LiveGridManager {
  ...
  async start(o = {}) {
    ...
    let bench = o.bench;
    if (!bench) {
      bench = await getFollowedBench({ roster: o.roster || DEFAULT_ROSTER, exclude: o.exclude || [] });
      if (bench) this.log(`bench from Twitch follows: ${bench.length} channels`);
    }
    this.poller = new LiveGridPoller({ roster: o.roster, bench: bench || undefined, exclude: o.exclude });
