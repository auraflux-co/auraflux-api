...
class LiveGridManager {
  ...
  async start(o = {}) {
    ...
    if (!forceLocalOnly) {
      if (!output) {
        ...
        const bid = attach?.broadcastId || o.broadcastId || process.env.LIVE_GRID_BROADCAST_ID;
        ...
        if (wantFresh && ingest) {
        ...
        } else if (attach && trustEnvBroadcast()) {
          this.broadcast = {
          ...
          if (canReuse) {
            ...
            this.log(
              `broadcast ${bid} not reusable — set freshListing:true to create via API, or create listing in Studio ` +
              'and update LIVE_GRID_BROADCAST_ID + LIVE_GRID_RTMP_URL in .env (recommended — saves quota)'
            );
            throw new Error(
              `YouTube listing ${bid} not attachable — update .env with a Studio listing or pass freshListing:true`
            );
          } else if (bid) {
            this.broadcast = {
              broadcastId: bid,
  ...
  async _onChannelWentOffline(q, login) {
    ...
    if (this.poller.assignments[q] === slug) this.poller.assignments[q] = null;
