...
const { QuadrantFeeders, generateSlate, quadUrl, writeNameFile, channelFromFeedUrl } = require('./feeders');
...
const { applyGoLiveDefaults, buildGoLiveSeo } = require('./go_live_template');
...
class LiveGridManager {
  ...
  async _createYoutubeBroadcast(o, layout, { scheduledStartTime, existingStream } = {}) {
    ...
    const templatePack = buildGoLiveSeo({}, o);
    ...
    if (seo && (SEO_ON_START || templatePack.fromTemplate)) {
  ...
  async start(o = {}) {
    ...
    if (!forceLocalOnly) {
      if (!output) {
        ...
        const studioAttach = o.createListing === false && !!bid;
        ...
        if (wantFresh && ingest) {
          ...
          this.log(`YouTube attach (env-only, 0 API calls) → ${attach.watchUrl}`);
    ...
    if (useOutputMiddleware && this._rtmpDeliveryUrl) {
  ...
  /**
   * Safe automated response to delivery signals (restreamer only).
   * Called from broadcast-sidecar heartbeat while grid is live.
   */
  async autoHealDelivery() {
    ...
    if (!this.running || !this.restreamer) {
      return { ok: false, reason: 'not live with middleware restreamer' };
    }

    const qa = this.buildDeliveryQa();
    const cooldownSec = parseInt(process.env.STREAM_DELIVERY_HEAL_COOLDOWN_SEC || '90', 10);
    const now = Date.now();
    if (this._lastDeliveryHealAt && (now - this._lastDeliveryHealAt) < cooldownSec * 1000) {
      return { ok: true, action: 'none', reason: 'cooldown', qa };
    }

    const actions = qa.selfHeal?.actions || [];
    if (!actions.includes('restart_restreamer')) {
      return { ok: true, action: 'none', qa };
    }

    const critical = qa.signals.some((s) =>
      s.key === 'hls_stale'
      || (s.key === 'restreamer_restarts' && s.severity === 'critical')
      || (s.key === 'restreamer_lagging_master' && s.severity === 'critical'));
    if (!critical) return { ok: true, action: 'none', qa };
  ...
  async refreshYoutubeSeo(reason = 'operator', opts = {}) {
    ...
    if (templatePack.fromTemplate) {
      ...
      streamers = templatePack.streamers;
      this.log(`go-live template SEO refresh (${streamers.map(s => s.login).join(', ')})`);
    } else {
      seo = await generateGridSeo(this._buildSeoContext(seoLayout));
      streamers = this._buildSeoContext(seoLayout).streamers;
    }
    ...
    const seoResult = await this._applyYoutubeSeo(
      ...
      { programMode: mode, layout: seoLayout, streamers },
    );
    return {
      refreshed: true,
      reason,
  ...
  async _applyYoutubeSeoFromLayout(layout, opts = {}) {
    ...
    if (templatePack.fromTemplate && !opts.forcePollerSeo) {
      ...
      seo = await generateGridSeo(this._buildSeoContext(layout));
