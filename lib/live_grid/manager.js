...
class LiveGridManager {
  ...
  /** Kick channel on quadrant q — resolve HLS via Apify/proxy, refresh while live (CPD-1065). */
  async setQuadrantKick(q, slug, opts = {}) {
    ...
    if (!resolved.playbackUrl) {
      if (opts.lockWhenOffline !== false && !resolved.isLive) {
        const kickPageUrl = `https://kick.com/${kickSlug}`;
        this.operatorLocks[q] = {
          type: 'url',
          kickSlug,
          login: kickSlug,
    ...
    const kickPageUrl = `https://kick.com/${kickSlug}`;
    this.log(`Kick Q${q + 1} → ${kickSlug} (${resolved.viewers || 0} viewers, ${resolved.source})`);
  ...
  _scheduleKickPlaybackRefresh(q, kickSlug) {
    ...
    const ms = parseInt(process.env.LIVE_GRID_KICK_PLAYBACK_REFRESH_MS || '180000', 10);
