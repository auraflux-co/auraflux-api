...
const { kickPlaylistUrlsEquivalent } = require('./kick_ingest');
const { kickStreamlinkIngestEnabled, kickPageUrl } = require('./kick_config');
const { kickChannelLive } = require('./stream_probe');
...
const { assertPoolIndex, fleetPoolSize } = require('./fleet_pool');

function assertOperatorQuadrant(q) {
  assertPoolIndex(q);
}
...
const SEO_SYNC_DEBOUNCE_MS = parseInt(
  process.env.LIVE_GRID_SEO_SYNC_DEBOUNCE_MS
  || process.env.LIVE_GRID_SOLO_SEO_SWAP_DEBOUNCE_MS
  || '30000',
  10,
);
...
/** Auto-rewrite YouTube titles on poller bench swaps — off prevents wrong titles in auto pilot (CPD-1065). */
const SEO_AUTO_SYNC = String(process.env.LIVE_GRID_SEO_AUTO_SYNC ?? 'off').toLowerCase() !== 'off';
...
const { mainGridEncodeEnabled } = require('./solo_focus');
...
class LiveGridManager {
  constructor(opts = {}) {
    ...
    /** @type {Array<NodeJS.Timeout|null>} */
    this._kickRefreshTimers = [null, null, null, null];
    /** @type {Array<{ type: 'channel'|'url'|'file', login?, url?, path?, label?, title? }|null>} */
    this.operatorLocks = [null, null, null, null];
    this._soloFocusConfig = null;
    this._lastHeroMaintainAt = 0;
  ...
  async _resolveLaunchLayout(o = {}) {
    ...
    this.programDirector = new ProgramDirector({
      log: this.log,
      mode: o.programMode || process.env.LIVE_GRID_PROGRAM_MODE || 'grid',
    });
  ...
  /** CPD-1067 — 5-slot solo roster per sidecar; no main grid compositor. */
  async startSoloRosterFleet(o = {}) {
    if (this.running) throw new Error('live grid already running');
    const { isFleetPaused, fleetPausedReason } = require('./solo_roster_fleet');
    if (isFleetPaused()) {
      throw new Error(`Fleet paused — ${fleetPausedReason()}`);
    }
    o = applyGoLiveDefaults(o || {});
    this.running = true;
    this.startedAt = Date.now();
    this.opts = { ...o, programMode: 'solo_roster' };
    process.env.LIVE_GRID_PROGRAM_MODE = 'solo_roster';
    this._programLayout = {
      mode: 'solo_roster',
      modeLabel: 'ClipzWorld Solo Roster',
      title: 'ClipzWorld News — Solo Roster',
      sources: [],
    };
    const { poolSize } = require('./solo_listings_env');
    this._lastAssignments = Array.from({ length: poolSize() }, () => null);
    ...
    this.feeders.onKickHlsStall = (q, url, kickSlug) => {
      if (!this.fleetOrchestrator || !kickSlug) return false;
      const st = this.fleetOrchestrator._slotState?.get(q);
      if (st?.phase !== 'live' && st?.phase !== 'starting') return false;
      this._refreshKickQuadrant(q, kickSlug).catch((e) => {
        this.log(`Kick HLS fleet refresh Q${q + 1} failed: ${e.message}`);
      });
      return true;
    };

    const { SoloRosterOrchestrator } = require('./solo_roster_orchestrator');
    this.fleetOrchestrator = new SoloRosterOrchestrator(this);
    const fleetStart = await this.fleetOrchestrator.start();
    ...
    this.log(`solo roster fleet live — sidecar ${fleetStart.fleetId}, ${fleetStart.slots} slots`);
    return { ...this.status(), fleet: fleetStart, programMode: 'solo_roster' };
  }

  async start(o = {}) {
    const { isSoloRosterMode } = require('./fleet_pool');
    if (isSoloRosterMode(o)) {
      return this.startSoloRosterFleet(o);
    }
    if (this.running) throw new Error('live grid already running');
    o = applyGoLiveDefaults(o || {});
    this.running = true;
    ...
    if (resumeRuntime?.operatorLocks?.length) {
      ...
      await applyResumeRuntime(this, resumeRuntime);
      ...
      // Locked seats restored above; unlocked seats still follow poller layout.
      this.feeders.applySources(this._programLayout.sources);
    } else {
    ...
    if (o.autoPilot === true) {
      ...
      for (let q = 0; q < 4; q++) {
        ...
        if (lock?.kickSlug || lock?.platform === 'kick') continue;
        this.operatorLocks[q] = null;
      }
    } else if (o.operatorMode === false && !resumeRuntime?.operatorLocks?.length) {
      this.poller.operatorMode = false;
      this.operatorLocks = [null, null, null, null];
    } else if (
      this.poller.operatorMode
      || o.operatorMode === true
      || resumeRuntime?.operatorMode
      || String(process.env.LIVE_GRID_OPERATOR_MODE || '').toLowerCase() === 'on'
    ) {
      this.poller.operatorMode = true;
    }
    ...
    this.poller.start();
    await this._reconcileFeedersFromPoller('grid_start').catch((e) => {
      this.log(`feeder reconcile on start failed: ${e.message}`);
    });
    ...
    await this._applyGoLiveKickLocksFromConfig().catch((e) => {
      this.log(`go-live Kick locks skipped: ${e.message}`);
    });
  ...
  async _applySoloYoutubeSeo(q, broadcastId, login, seoOpts = {}, extra = {}) {
    ...
    const fleetSlot = extra.fleetSlot ?? (extra.streamerLock ? (extra.poolSlot || q + 1) : null);
    const seo = buildSoloLiveSeo({
      login,
      quadrant: q,
      fleetSlot: Number.isInteger(fleetSlot) ? fleetSlot : undefined,
      mainWatchUrl: mainUrl,
      gridLogins,
      streamerLock: extra.streamerLock,
    });
    const playlistId = yt.resolveLivePlaylistIdFromConfig?.() || null;
    const seoOnStart = String(process.env.LIVE_GRID_SEO_ON_START ?? 'off').toLowerCase() === 'on';
    const setPublic = seoOpts.setPublic === true
      || (seoOpts.setPublic !== false && seoOnStart);

    let thumbnailPath = seoOpts.thumbnailPath || null;
    const thumbMode = String(process.env.LIVE_GRID_SOLO_THUMBNAIL || process.env.LIVE_GRID_LIVE_THUMBNAIL || 'stream_frame').toLowerCase();
    if (!thumbnailPath && thumbMode === 'stream_frame' && setPublic) {
      try {
        const { captureStreamFrameThumbnail } = require('./stream_frame_thumbnail');
        const cap = await captureStreamFrameThumbnail({ quadrant: q });
        if (cap.ok) {
          thumbnailPath = cap.path;
          this.log(`solo Q${q + 1}: stream-frame thumbnail ${require('path').basename(cap.path)}`);
        } else {
          this.log(`solo Q${q + 1}: stream-frame thumbnail skipped (${cap.error || 'no frame'})`);
        }
      } catch (e) {
        this.log(`solo Q${q + 1}: stream-frame thumbnail failed: ${e.message}`);
      }
    }
    ...
    const result = await yt.applyLiveBroadcastSeo(broadcastId, seo, {
      ...
      membersOnlyChat: seoOpts.membersOnlyChat !== false,
      setPublic,
      thumbnailPath,
    });
    if (result.tags) this.log(`solo Q${q + 1} YouTube tags set (${seo.tags?.length || 0})`);
    return { seo, result, thumbnailPath };
  }
  ...
  async _prepareSoloBroadcasts({ allowCreate = false } = {}) {
    ...
    if (streamerLockEnabled()) {
      syncBindingsForAssignments(this._lastAssignments);
      for (const binding of Object.values(activeBindings())) {
        const login = binding.login;
        const q = (binding.currentQuadrant || binding.slot) - 1;
        let bid = binding.broadcastId;
        const reusable = bid ? await this._canReuseBroadcastId(bid) : false;
        if (!reusable && !mayCreate) {
          if (!bid) {
            this.log(`solo @${login} missing broadcastId — pool slot ${binding.slot}`);
            prepared.push({ login, poolSlot: binding.slot, error: 'no broadcastId' });
            continue;
          }
          this.log(`solo @${login} broadcast ${bid} not active — SEO-only`);
        } else if (!reusable && mayCreate && binding.streamId) {
          const seoPreview = buildSoloLiveSeo({
            login,
            quadrant: q,
            gridLogins: this._feederLogins(),
            mainWatchUrl: this.broadcast?.watchUrl,
            streamerLock: true,
          });
          const broadcast = await yt.createLiveBroadcast({
          ...
          binding.watchUrl = broadcast.watchUrl;
          this.log(`solo @${login} fresh broadcast → ${broadcast.watchUrl}`);
        }
        try {
          const { seo } = await this._applySoloYoutubeSeo(q, bid, login, {}, { streamerLock: true });
          prepared.push({ login, poolSlot: binding.slot, quadrant: q + 1, broadcastId: bid, title: seo.title });
        } catch (e) {
          this.log(`solo @${login} SEO failed: ${e.message}`);
          prepared.push({ login, broadcastId: bid, seoError: e.message });
        }
      }
      return prepared;
    }
  ...
  /** @deprecated — use syncSoloListingsFromGrid (stream-key discover + grid login). */
  async refreshSoloYoutubeSeo(opts = {}) {
    const idMap = opts.soloBroadcastIds || opts.broadcastIds;
    const result = await this.syncSoloListingsFromGrid({
      discover: !idMap,
      persistDiscovered: false,
      mode: 'discoverable',
    });
    if (idMap && result.results) {
      for (const seat of result.results) {
        const bid = idMap[seat.quadrant] || idMap[String(seat.quadrant)];
        if (bid && seat.broadcastId !== bid) seat.broadcastId = bid;
      }
    }
    return {
      refreshed: result.ok,
  ...
  /** Hotswap streamer → pool slot (algorithm URL) without stopping grid. */
  async applySoloBindingMap(opts = {}) {
    if (!this.running) throw new Error('Live grid not running');
    const {
      streamerLockEnabled,
      applyLoginSlotMap,
    ...
    const poolUpdates = opts.poolBroadcastIds || opts.pool || {};
    for (const [rawSlot, spec] of Object.entries(poolUpdates)) {
      const slot = Number(rawSlot);
      const bid = typeof spec === 'string' ? spec : spec?.broadcastId;
      if (!bid) continue;
      updatePoolSlotListing(slot, {
        broadcastId: bid,
        watchUrl: typeof spec === 'object' ? spec.watchUrl : undefined,
        streamId: typeof spec === 'object' ? spec.streamId : undefined,
      });
    }

    const map = opts.bindings || opts.map;
    if (map && Object.keys(map).length) {
      applyLoginSlotMap(map, this._lastAssignments);
    }

    if (this.soloPublishers?.started) {
      for (let q = 0; q < 4; q++) {
        const login = this._lastAssignments[q];
        if (login) this.soloPublishers.restart(q, login);
      }
    } else {
      await this.startSoloStreams({ allowCreateBroadcasts: false });
    }

    let seo = null;
    if (opts.refreshSeo !== false) {
      seo = await this.refreshAllYoutubeSeo(opts.reason || 'solo_binding_hotswap', {
        discover: opts.discover !== false,
        persistDiscovered: opts.persistDiscovered !== false,
        skipThumbnail: opts.skipThumbnail !== false,
      });
    }
  ...
  async startSoloStreams({ allowCreateBroadcasts = false } = {}) {
    ...
    if (streamerLockEnabled()) {
      seedFromQuadrantEnv(this._lastAssignments);
      syncBindingsForAssignments(this._lastAssignments);
    }
    ...
    this.soloPublishers.startAll(this._lastAssignments);
    this._soloAnnouncer?.schedule();
  ...
  _soloStatusSeats() {
    ...
    const { streamerLockEnabled, getBinding, syncBindingsForAssignments } = require('./solo_streamer_registry');
    if (streamerLockEnabled()) syncBindingsForAssignments(this._lastAssignments);
    if (this.soloPublishers?.started) {
    ...
    return [0, 1, 2, 3].map((q) => {
      const listing = readSoloListingForQuadrant(q);
      const login = this._resolveSoloLogin(q, listing);
      const binding = streamerLockEnabled() && login ? getBinding(login) : null;
      return {
        quadrant: q + 1,
        login: login || null,
        running: false,
        ...
        configured: binding ? !!binding.rtmpUrl : !!listing?.rtmpUrl,
  ...
  async _startMasterEncoder() {
    if (!mainGridEncodeEnabled()) {
      this.log('main grid encoder skipped — LIVE_GRID_MAIN_ENCODE=off (solo focus)');
      this._scheduleSoloAutoStart();
      return { skipped: true, reason: 'main_encode_off' };
    }
  ...
  /** On-air channel logins only — slate with pending login does not count as seated. */
  _feederOnAirLogins() {
    return (this.feeders?.status() || []).map((f) => (
      f?.kind === 'channel'
        ? (normalizeSoloLogin(f.login) || normalizeSoloLogin(f.channelSlug) || null)
  ...
  /** True when quadrant feeders diverge from program layout (post-resume / deploy desync). */
  _feederLayoutMismatch(sources = this._programLayout?.sources) {
    if (!sources?.length || !this.feeders) return false;
    const want = this._sourcesToLogins(sources);
    const feeders = this.feeders.status();
    for (let q = 0; q < 4; q++) {
      const s = sources[q];
      const f = feeders[q];
      if (s && typeof s === 'object' && s.type === 'file') {
        if (f?.kind !== 'file' || path.resolve(f.filePath || '') !== path.resolve(s.path || '')) return true;
        continue;
      }
      if (s && typeof s === 'object' && s.type === 'url') {
        if (f?.kind !== 'url' || f.feedUrl !== s.url) return true;
        continue;
      }
      const w = want[q] || null;
      const h = this._feederOnAirLogins()[q] || null;
      if (w !== h) return true;
    }
    return false;
  }

  async _applyGoLiveKickLocksFromConfig() {
    const cfg = loadGoLiveConfig();
    if (!cfg?.operatorLocks?.length) return { applied: 0 };
    let applied = 0;
    for (const lock of cfg.operatorLocks) {
      if (!lock?.kickSlug && lock?.platform !== 'kick') continue;
      const q = (lock.quadrant ?? 1) - 1;
      if (q < 0 || q >= fleetPoolSize()) continue;
      if (this.operatorLocks[q]?.kickSlug === lock.kickSlug) continue;
      await this.setQuadrantKick(q, lock.kickSlug || lock.login, {
        label: lock.label || lock.kickSlug,
      });
      applied++;
    }
    if (applied) this.log(`go-live template: ${applied} Kick lock(s) applied`);
    return { applied };
  }

  /** Force feeders to match current poller + program layout (survives resume without poll delta). */
  async _reconcileFeedersFromPoller(reason = 'reconcile') {
    if (!this.running || !this.poller || !this.feeders) return { reconciled: false };
    const assignments = [...this.poller.assignments];
    const layout = await this._applyProgram(assignments);
    this._lastPollerAssignments = assignments;
    this._programLayout = layout;
    const sources = layout.sources;
    const mismatch = this._feederLayoutMismatch(sources);
    if (mismatch || !this.poller.operatorMode) {
      if (mismatch) {
        const seated = this._sourcesToLogins(sources).filter(Boolean).join(', ') || 'slate';
        this.log(`feeder/poller reconcile (${reason}) → ${seated}`);
      }
      this.feeders.applySources(sources);
      this._lastAssignments = this._sourcesToLogins(sources);
      return { reconciled: true, mismatch };
    }
    return { reconciled: false, mismatch: false };
  }
      ...
      if (lock.kickSlug) {
        if (lock.url) {
          out[q] = {
            type: 'url',
            url: lock.url,
            label: lock.label || lock.login || lock.kickSlug || '',
            title: lock.title || lock.login || '',
            login: lock.login || lock.kickSlug,
            kickSlug: lock.kickSlug,
          };
        } else {
          out[q] = {
            type: 'kick_hold',
  ...
  /** Re-apply render encode profile + restart main + solos (live hotswap). */
  async applyEncodeProfile(opts = {}) {
    if (!this.running) throw new Error('Live grid not running');
    const { applyRenderProfile } = require('./render_profile');
    const applied = applyRenderProfile((m) => this.log(m));
    if (!applied.applied) throw new Error(applied.reason || 'profile not applied');

    const main = this.reloadEncodeSettings();
    ...
    } else if (opts.startSolos !== false) {
      await this.startSoloStreams({ allowCreateBroadcasts: false });
    }

    const snap = this._encodeContractRuntime();
    const { buildEncodeContractSnapshot } = require('./encode_contract');
    return {
      ok: true,
      profilePath: applied.profilePath,
      main,
      encodeContract: buildEncodeContractSnapshot(snap),
      seats: this._soloStatusSeats(),
    };
  }
  ...
  autoTuneEncodeIfNeeded() {
    if (this._soloFocusConfig?.quadrant) {
      return { ok: true, action: 'none', reason: 'hero_solo_focus' };
    }
    if (!AUTOTUNE_LIVE) {
  ...
  /** CPD-1063 — apply benchmark step env and restart encoders to match (off-air capacity test). */
  async applyBenchmarkStep(step) {
    if (!this.running) throw new Error('Live grid not running');
    const { applyBenchmarkStepEnv, buildEncodeContractSnapshot } = require('./encode_contract');
    const spec = applyBenchmarkStepEnv(step);
    this._soloFocusConfig = null;

    if (spec.mainEncode && this.master && !this.master.proc) {
      await this._startMasterEncoder();
    } else if (!spec.mainEncode && this.master?.proc) {
      this.master.stop();
      this._rtmpEncoderStarted = false;
      this.restreamer?.stop();
      this.restreamer = null;
    } else if (spec.mainEncode && this.master?.proc) {
      this.reloadEncodeSettings();
    }

    if (this.soloPublishers) {
      for (let q = 0; q < 4; q++) {
        const seat = q + 1;
        if (!spec.soloSeats.includes(seat)) this.soloPublishers.stopSeat(q);
      }
    }

    if (spec.soloSeats.length) {
      if (!this.soloPublishers?.started) {
        await this.startSoloStreams({ allowCreateBroadcasts: false });
      } else {
        for (const seat of spec.soloSeats) {
          this.soloPublishers.restart(seat - 1, this._lastAssignments[seat - 1]);
        }
      }
    } else if (this.soloPublishers?.started) {
      this.soloPublishers.stopAll();
    }

    this.log(`benchmark step ${spec.step} (${spec.label}): ${spec.description}`);
    const snap = this._encodeContractRuntime();
    return {
      ok: true,
      step: spec.step,
  ...
  _encodeContractRuntime() {
    const soloRunning = [0, 1, 2, 3].map((q) => !!this.soloPublishers?.procs?.[q]);
    return {
      mainRunning: !!this.master?.proc,
      soloRunning,
      focusConfig: this._soloFocusConfig,
      assignments: this._lastAssignments,
    };
  }
  ...
  getEncodeContract() {
    const { buildEncodeContractSnapshot } = require('./encode_contract');
    return buildEncodeContractSnapshot(this._encodeContractRuntime());
  }

  /** Exit hero solo focus — restore render profile + main + all solos (CPD-1062). */
  async clearSoloFocusMode() {
    if (!this.running) throw new Error('Live grid not running');
    const { soloFocusSeat } = require('./solo_focus');
    const hadFocus = this._soloFocusConfig || soloFocusSeat();
    if (!hadFocus) return { ok: true, action: 'none', reason: 'not_in_focus' };
    return this.applyBenchmarkStep(0);
  }

  /** Allocate CPU to one solo seat — stop main + other solos, max encode on hero stream. */
  async focusSoloQuadrant(quadrant, opts = {}) {
    const q = Number(quadrant);
    if (!Number.isInteger(q) || q < 1 || q > 4) throw new Error('quadrant must be 1-4');
    const seat = q - 1;
    if (!this.running) throw new Error('Live grid not running');

    const { readSoloListingForQuadrant } = require('./solo_listings_env');
    const { streamerLockEnabled, getBinding, syncBindingsForAssignments } = require('./solo_streamer_registry');
    const heroLoginRaw = this._lastAssignments[seat] || opts.login || null;
    const heroLogin = normalizeSoloLogin(heroLoginRaw);
    if (streamerLockEnabled() && heroLogin) {
      syncBindingsForAssignments(this._lastAssignments);
    }
    const binding = streamerLockEnabled() && heroLogin ? getBinding(heroLogin) : null;
    const listing = binding || readSoloListingForQuadrant(seat);
    if (!listing?.rtmpUrl) throw new Error(`Q${q} solo listing not configured`);
    const expectBid = opts.broadcastId || binding?.broadcastId;
    if (expectBid && listing.broadcastId && listing.broadcastId !== expectBid) {
      throw new Error(`Q${q} broadcastId mismatch (expected ${expectBid}, have ${listing.broadcastId})`);
    }

    const { applySoloSeatEncodeOverrides, heroSoloEncodeDefaults } = require('./solo_focus');
    const heroDefaults = heroSoloEncodeDefaults();
    applySoloSeatEncodeOverrides(seat, {
      bitrateK: opts.bitrateK ?? heroDefaults.bitrateK,
      maxrateK: opts.maxrateK ?? opts.bitrateK ?? heroDefaults.maxrateK,
      bufsizeK: opts.bufsizeK ?? heroDefaults.bufsizeK,
      w: opts.w ?? heroDefaults.w,
      h: opts.h ?? heroDefaults.h,
      fps: opts.fps ?? parseInt(process.env.LIVE_GRID_FPS || String(heroDefaults.fps), 10),
      audioK: opts.audioK ?? heroDefaults.audioK,
    });
    process.env.LIVE_GRID_MAIN_ENCODE = 'off';

    if (opts.stopMain !== false && this.master?.proc) {
      this.master.stop();
      this._rtmpEncoderStarted = false;
      this.restreamer?.stop();
      this.restreamer = null;
      this.log(`main grid encoder stopped — CPU allocated to solo Q${q}`);
    }

    if (opts.stopOtherSolos !== false && this.soloPublishers) {
      for (let other = 0; other < 4; other++) {
        if (other !== seat) this.soloPublishers.stopSeat(other);
      }
    }

    if (opts.stopOtherQuadrants) {
      for (let other = 0; other < 4; other++) {
        if (other === seat) continue;
        this.relays?.stopQuadrant?.(other);
        this.feeders?.stopQuadrant?.(other);
      }
      this.log(`feeders/relays stopped for Q1–Q4 except Q${q}`);
    }

    this.audioQuad = seat;
    this.audioMode = 'manual';
    this.audioPinSource = 'solo_focus';
    this.log(`audio pinned to Q${q} (${this._lastAssignments[seat] || listing.label})`);

    if (!this.soloPublishers?.started) {
      await this.startSoloStreams({ allowCreateBroadcasts: false });
    } else {
      this.soloPublishers.restart(seat, this._lastAssignments[seat]);
    }

    const { soloOutputDims } = require('./solo_publishers');
    const encode = soloOutputDims(seat, heroLoginRaw || heroLogin);
    this._soloFocusConfig = {
      quadrant: q,
      broadcastId: listing.broadcastId,
      login: heroLoginRaw,
      stopMain: opts.stopMain !== false,
      stopOtherSolos: opts.stopOtherSolos !== false,
      stopOtherQuadrants: !!opts.stopOtherQuadrants,
      encode,
      lockedAt: new Date().toISOString(),
    };
    if (heroLoginRaw) {
      try { this.setQuadrantChannel(seat, heroLoginRaw); } catch (e) {
        this.log(`hero channel lock skipped: ${e.message}`);
      }
    }
    this.log(`solo focus Q${q} → ${encode.w}×${encode.h} ${encode.bitrateK}k (${listing.broadcastId})`);
    return {
      ok: true,
      quadrant: q,
      broadcastId: listing.broadcastId,
      ...
      login: heroLoginRaw,
      encode,
      mainEncodeStopped: opts.stopMain !== false,
      heroMode: true,
    };
  }

  /** Restore hero solo after auto-resume (persisted in resume JSON). */
  async applySoloFocusFromResume(cfg = {}) {
    if (!cfg?.quadrant || !this.running) return { ok: false, reason: 'no_focus_config' };
    this.log(`restoring hero solo focus Q${cfg.quadrant} (${cfg.broadcastId || 'listing'})`);
    return this.focusSoloQuadrant(cfg.quadrant, {
      broadcastId: cfg.broadcastId,
      bitrateK: cfg.encode?.bitrateK,
      maxrateK: cfg.encode?.maxrateK ?? cfg.encode?.bitrateK,
      bufsizeK: cfg.encode?.bufsizeK,
      w: cfg.encode?.w,
      h: cfg.encode?.h,
      fps: cfg.encode?.fps,
      audioK: cfg.encode?.audioK,
      stopMain: cfg.stopMain !== false,
      stopOtherSolos: cfg.stopOtherSolos !== false,
      stopOtherQuadrants: !!cfg.stopOtherQuadrants,
    });
  }

  /**
   * Heartbeat watchdog — keep hero stream isolated at target encode; restart if dropped.
   */
  async maintainSoloFocus() {
    const cfg = this._soloFocusConfig;
    if (!cfg?.quadrant || !this.running) return { ok: true, action: 'none' };

    const seat = cfg.quadrant - 1;
    const actions = [];
    const now = Date.now();
    const cooldownMs = parseInt(process.env.LIVE_GRID_HERO_MAINTAIN_COOLDOWN_MS || '20000', 10);
    if (this._lastHeroMaintainAt && (now - this._lastHeroMaintainAt) < cooldownMs) {
      return { ok: true, action: 'none', reason: 'cooldown' };
    }
    this._lastHeroMaintainAt = now;

    if (cfg.stopMain !== false && this.master?.proc) {
      this.master.stop();
      this._rtmpEncoderStarted = false;
      this.restreamer?.stop();
      this.restreamer = null;
      actions.push('stopped_main');
    }

    if (this.soloPublishers) {
      for (let other = 0; other < 4; other++) {
        if (other !== seat && this.soloPublishers.procs[other]) {
          this.soloPublishers.stopSeat(other);
          actions.push(`stopped_solo_q${other + 1}`);
        }
      }
      if (!this.soloPublishers.procs[seat]) {
        this.soloPublishers.restart(seat, cfg.login || this._lastAssignments[seat]);
        actions.push('restarted_hero_solo');
      }
    }

    if (cfg.stopOtherQuadrants) {
      for (let other = 0; other < 4; other++) {
        if (other === seat) continue;
        if (this.relays?.procs?.[other]) {
          this.relays.stopQuadrant(other);
          actions.push(`stopped_relay_q${other + 1}`);
        }
        const feederProcs = this.feeders?.quads?.[other]?.procs?.length || 0;
        if (feederProcs > 0) {
          this.feeders.stopQuadrant(other);
          actions.push(`stopped_feeder_q${other + 1}`);
        }
      }
    }

    const heroFeederUp = (this.feeders?.quads?.[seat]?.procs?.length || 0) > 0;
    const wantLogin = cfg.login || this._lastAssignments[seat];
    if (!heroFeederUp && wantLogin) {
      try {
        this.setQuadrantChannel(seat, wantLogin);
        actions.push('restored_hero_feeder');
      } catch (e) {
        this.log(`hero feeder restore failed: ${e.message}`);
      }
    }

    if (actions.length) {
      this.log(`hero maintain Q${cfg.quadrant}: ${actions.join(', ')}`);
    }
    return { ok: true, action: actions.length ? actions.join('+') : 'none', actions, heroRunning: !!this.soloPublishers?.procs?.[seat] };
  }
  ...
  async autoHealDelivery() {
    ...
    if (this._soloFocusConfig?.quadrant) {
      const maintained = await this.maintainSoloFocus();
      const seat = this._soloFocusConfig.quadrant - 1;
      const heroRunning = !!this.soloPublishers?.procs?.[seat];
      const qa = this.buildDeliveryQa();
      return {
        ok: heroRunning,
  ...
  /** Operator sync — Twitch grid login → YouTube solo discoverability SEO. */
  async syncSoloListingsFromGrid(opts = {}) {
    this._syncAssignmentsFromFeeders();
    let discovered = opts.discovered || null;
    if (!discovered && opts.discover !== false && yt.isConnected()) {
      try {
        discovered = await this.discoverYoutubeBroadcasts();
        if (opts.persistDiscovered !== false && discovered?.soloBroadcastIds) {
          const saved = this._persistDiscoveredSoloIds(discovered.soloBroadcastIds);
          if (saved.length) {
            this.log(`sync solo: pinned IDs ${saved.map((s) => `Q${s.quadrant}=${s.broadcastId}`).join(', ')}`);
          }
        }
      } catch (e) {
        this.log(`sync solo discover skipped: ${e.message}`);
      }
    }
    const { syncSoloListingsFromGrid } = require('./solo_listing_sync');
    return syncSoloListingsFromGrid({
      status: this.status(),
      discovered,
      yt,
      quadrants: opts.quadrants,
      dryRun: opts.dryRun === true,
      mode: opts.mode || 'discoverable',
      log: (m) => this.log(`sync solo ${m}`),
    });
  }
  ...
  async refreshAllYoutubeSeo(reason = 'operator', opts = {}) {
    ...
    const wantDiscover = opts.discover === true
    ...
    if (wantDiscover && yt.isConnected()) {
      try {
        ...
        if (opts.persistDiscovered === true && soloBroadcastIds) {
    ...
    const solo = await this.syncSoloListingsFromGrid({
      discover: false,
      persistDiscovered: false,
      discovered: discovered ? { soloBroadcastIds: discovered.soloBroadcastIds } : null,
      mode: 'discoverable',
    });
  ...
  _nudgeSoloAfterSwap(q, login) {
    if (!this.soloPublishers?.started) return;
    const { streamerLockEnabled, syncBindingsForAssignments } = require('./solo_streamer_registry');
    if (streamerLockEnabled()) syncBindingsForAssignments(this._lastAssignments);
  ...
  _applySourceLayout(layout, prevSources) {
    ...
    if (this.broadcast) {
      ...
      if (prevCount === 0 && nextCount > 0) {
        ...
        this._scheduleYoutubeSeoSync();
      }
    }
  }

  /**
   * Single debounced path: discover stream keys → update main + all solo YouTube listings.
   * Fires ~30s after grid assignments change (swap, bench fill, manual replace).
   */
  _scheduleYoutubeSeoSync() {
    if (!SEO_AUTO_SYNC) return;
    if (!this.broadcast?.broadcastId) return;
    clearTimeout(this._youtubeSeoSyncTimer);
    this._youtubeSeoSyncTimer = setTimeout(() => {
      if (!this.running) return;
      this.refreshAllYoutubeSeo('grid_swap', {
        discover: true,
        persistDiscovered: true,
        skipThumbnail: !SEO_ON_SWAP,
        thumbnail: SEO_ON_SWAP,
      }).catch((e) => this.log(`YouTube SEO sync failed: ${e.message}`));
    }, SEO_SYNC_DEBOUNCE_MS);
    this._youtubeSeoSyncTimer.unref?.();
  }
  ...
  async _applyYoutubeSeoFromLayout(layout, opts = {}) {
    ...
    const feederLogins = this._feederLogins();
    const templatePack = buildGoLiveSeo({}, {
      operatorLocks: this.operatorLocks,
      assignments: feederLogins.some(Boolean) ? feederLogins : this._sourcesToLogins(layout?.sources),
      streamers: seoCtx.streamers,
      programMode: layout?.mode,
      _resumeRuntime: this.opts,
    });
  ...
  setQuadrantUrl(q, feedUrl, label, opts = {}) {
    ...
    const kickSlug = opts.kickSlug || null;
    const prev = this.operatorLocks[q];
    const tokenRefresh = !!(kickSlug && prev?.kickSlug === kickSlug
      && kickPlaylistUrlsEquivalent(prev?.url, feedUrl));
    this.feeders.setQuadrantUrl(q, feedUrl, label, { login: slug, ...opts });
    this.operatorLocks[q] = {
      type: 'url',
      url: feedUrl,
      label: label || null,
      title: opts.title,
      login: slug,
      kickSlug: kickSlug || undefined,
      kickPageUrl: opts.kickPageUrl || undefined,
      platform: kickSlug ? 'kick' : undefined,
    };
    if (kickSlug) this._scheduleKickPlaybackRefresh(q, kickSlug);
    else this._clearKickRefreshTimer(q);
    this.relays?.setKickQuadrant?.(q, !!kickSlug);
    if (!tokenRefresh && !opts.skipSoloNudge) this._nudgeRelayAfterSwap(q, null);
    if (this._programLayout?.sources) {
      this._programLayout.sources[q] = {
        type: 'url',
        url: feedUrl,
        label: label || slug || '',
        title: opts.title || '',
  ...
  /** Kick channel on pool seat q — HLS on Render, streamlink optional locally (CPD-1065). */
  async setQuadrantKick(q, slug, opts = {}) {
    const { assertPoolIndex } = require('./fleet_pool');
    assertPoolIndex(q);
    ...
    this.operatorLocks[q] = {
      ...
      kickSlug,
      login: kickSlug,
      label: opts.label || kickSlug,
      platform: 'kick',
      kickPageUrl: pageUrl,
      url: pageUrl,
    };
    this.relays?.setKickQuadrant?.(q, true);
    this._scheduleKickPlaybackRefresh(q, kickSlug);
    writeNameFile(q, opts.label || kickSlug);

    if (kickStreamlinkIngestEnabled()) {
      this.log(`Kick Q${q + 1} → ${kickSlug} (streamlink — same health loop as Twitch)`);
      this.feeders.setQuadrantKick(q, kickSlug, { label: opts.label || kickSlug });
      if (this._programLayout?.sources) {
        this._programLayout.sources[q] = {
          type: 'kick_hold',
          kickSlug,
      ...
      this._lastAssignments = this._sourcesToLogins(this._programLayout?.sources || []);
      const st = this.feeders.status()[q];
      const offline = st.kind === 'slate' && !(st.pids?.length);
      if (!offline) this._nudgeRelayAfterSwap(q, kickSlug);
      return { ...st, locked: true, platform: 'kick', kickSlug, offline };
    }

    const resolved = await this._resolveKickPlayback(kickSlug);
    if (!resolved.playbackUrl) {
      if (opts.lockWhenOffline !== false && !resolved.isLive) {
        this.log(`Kick Q${q + 1} lock → ${kickSlug} (offline — auto-seat when live)`);
        this.feeders.setQuadrantKick(q, kickSlug, { label: opts.label || kickSlug });
        if (this._programLayout?.sources) {
          this._programLayout.sources[q] = {
            type: 'kick_hold',
        ...
        return { ...this.feeders.status()[q], locked: true, offline: true, kickSlug };
      }
      const msg = resolved.isLive
        ? `Kick ${kickSlug} live but playback URL unavailable (${resolved.source || 'unknown'})`
        : `Kick channel ${kickSlug} is offline`;
      throw new Error(msg);
    }
    this.log(`Kick Q${q + 1} → ${kickSlug} (${resolved.viewers || 0} viewers, ${resolved.source}${resolved.ingestMode ? `/${resolved.ingestMode}` : ''})`);
    return this.setQuadrantUrl(q, resolved.playbackUrl, opts.label || kickSlug, {
      login: kickSlug,
      title: opts.title || resolved.title || kickSlug,
      kickSlug,
      kickPageUrl: pageUrl,
      skipSoloNudge: opts.skipSoloNudge,
    });
  }

  async _resolveKickPlayback(kickSlug) {
    const { resolveKickLivePlayback } = require('../clients/kick_live_resolver');
    return resolveKickLivePlayback(kickSlug);
  }

  _clearKickRefreshTimer(q) {
    if (this._kickRefreshTimers?.[q]) {
      clearInterval(this._kickRefreshTimers[q]);
      this._kickRefreshTimers[q] = null;
    }
  }

  _scheduleKickPlaybackRefresh(q, kickSlug) {
    this._clearKickRefreshTimer(q);
    const ms = parseInt(
      process.env.LIVE_GRID_KICK_PLAYBACK_REFRESH_MS
      || (kickStreamlinkIngestEnabled() ? '300000' : '180000'),
      10,
    );
    if (!Number.isFinite(ms) || ms < 30000) return;
    this._kickRefreshTimers[q] = setInterval(() => {
      if (!this.running) return;
      const lock = this.operatorLocks[q];
      if (lock?.kickSlug !== kickSlug) return;
      this._refreshKickQuadrant(q, kickSlug).catch((e) => {
        this.log(`Kick periodic refresh Q${q + 1} failed: ${e.message}`);
      });
    }, ms);
    this._kickRefreshTimers[q].unref?.();
  }

  async _refreshKickQuadrant(q, kickSlug) {
    const lock = this.operatorLocks[q];
    if (!lock?.kickSlug || lock.kickSlug !== kickSlug) return { refreshed: false };

    if (kickStreamlinkIngestEnabled()) {
      const live = await kickChannelLive(kickSlug);
      if (!live) {
        this.log(`Kick Q${q + 1} ${kickSlug} offline — releasing lock`);
        this._clearKickRefreshTimer(q);
        this.operatorLocks[q] = null;
        this.relays?.setKickQuadrant?.(q, false);
        this.poller?.clearPin(q);
        this.feeders.setQuadrant(q, null);
        return { refreshed: false, offline: true, released: true };
      }
      const feederUp = !!(this.feeders?.quads?.[q]?.procs?.length
        && this.feeders.quads[q].kind === 'url');
      if (feederUp) {
        return { refreshed: false, unchanged: true, streamlink: true };
      }
      this.log(`Kick Q${q + 1} ${kickSlug} live — re-seating streamlink feeder`);
      this.feeders.setQuadrantKick(q, kickSlug, { label: lock.label || kickSlug });
      return { refreshed: true, reseated: true, streamlink: true };
    }
    ...
    if (!resolved.playbackUrl) {
      ...
      this.log(`Kick Q${q + 1} ${kickSlug} live but no playback URL — holding lock`);
      return { refreshed: false, offline: false };
    }
    if (!lock.url) {
      this.log(`Kick Q${q + 1} ${kickSlug} now live — seating`);
      return this.setQuadrantUrl(q, resolved.playbackUrl, lock.label || kickSlug, {
        login: kickSlug,
        kickSlug,
        title: resolved.title || lock.title,
      });
    }
    const tokenOnly = kickPlaylistUrlsEquivalent(lock.url, resolved.playbackUrl);
    if (lock.url === resolved.playbackUrl && this.feeders?.quads?.[q]?.procs?.length) {
      return { refreshed: false, unchanged: true };
    }
    if (tokenOnly && this.feeders?.quads?.[q]?.procs?.length) {
      this.log(`Kick Q${q + 1} HLS token refresh (${resolved.source})`);
      this.feeders.refreshKickHlsUrl(q, resolved.playbackUrl, {
        login: kickSlug,
        kickSlug,
        label: lock.label || kickSlug,
      });
      lock.url = resolved.playbackUrl;
      if (resolved.title) lock.title = resolved.title;
      if (this._programLayout?.sources?.[q] && typeof this._programLayout.sources[q] === 'object') {
        this._programLayout.sources[q].url = resolved.playbackUrl;
        if (resolved.title) this._programLayout.sources[q].title = resolved.title;
      }
      this._lastAssignments = this._sourcesToLogins(this._programLayout?.sources || []);
      return { refreshed: true, tokenOnly: true, playbackUrl: resolved.playbackUrl };
    }
    this.log(`Kick Q${q + 1} playback URL refreshed (${resolved.source})`);
    this.setQuadrantUrl(q, resolved.playbackUrl, lock.label || kickSlug, {
      login: kickSlug,
      kickSlug,
      title: resolved.title || lock.title,
    });
    return { refreshed: true, playbackUrl: resolved.playbackUrl };
  }
  ...
  _lockCurrentGridAsOperator() {
    ...
    for (let q = 0; q < 4; q++) {
      const lock = this.operatorLocks[q];
      if (lock?.type === 'url' || lock?.type === 'file' || lock?.type === 'slate') continue;
  ...
  setOperatorMode(enabled) {
    ...
    this.log(this.poller.operatorMode
      ...
      : 'auto pilot ON — bench fills empty quadrants; Kick/url locks on Q1+ stay until offline');
  ...
  _benchUnhealthyQuadrant(q, info) {
    ...
    if (lock?.kickSlug) {
      if (kickStreamlinkIngestEnabled()) {
        this.log(`feed guard: Kick Q${q + 1} (${lock.kickSlug}) blip — re-seating streamlink feeder`);
        const quad = this.feeders?.quads[q];
        if (quad) {
        ...
        this.feeders.setQuadrantKick(q, lock.kickSlug, { label: lock.label || lock.kickSlug });
        return;
      }
      this.log(`feed guard: Kick Q${q + 1} (${lock.kickSlug}) ingest blip — refreshing playback URL`);
      const quad = this.feeders?.quads[q];
      if (quad) {
        quad.feedUnhealthy = false;
        quad.feedFailures = 0;
      ...
      this._refreshKickQuadrant(q, lock.kickSlug).catch((e) => {
        this.log(`Kick feed refresh Q${q + 1} failed: ${e.message}`);
      });
      return;
    }
  ...
  _onAssignments(assignments) {
    ...
    this._applyProgram(assignments)
      .then((layout) => {
        ...
        const mismatch = this._feederLayoutMismatch(sources);
        if (changed || !this.poller?.operatorMode || mismatch) {
          if (mismatch && !changed) {
            const seated = this._sourcesToLogins(sources).filter(Boolean).join(', ') || 'slate';
            this.log(`feeder/poller reconcile (poll) → ${seated}`);
          }
          this.feeders.applySources(sources);
        }
        this._lastAssignments = this._sourcesToLogins(sources);
        ...
        if (changed && this.poller?.operatorMode) {
          for (let q = 0; q < 4; q++) {
            ...
            } else if (login && !lock?.kickSlug && lock?.platform !== 'kick') {
              this.operatorLocks[q] = { type: 'channel', login };
              this.poller?.pinQuadrant(q, login);
  ...
  async stop(opts = {}) {
    ...
    this._programTick = null;
    for (let q = 0; q < 4; q++) this._clearKickRefreshTimer(q);
    this.fleetOrchestrator?.stop();
    this.fleetOrchestrator = null;
    this.poller?.stop();
    this.chat?.stop();
  ...
  _statusSnapshot() {
    ...
    return {
      ...
      encodeContract: this.getEncodeContract(),
      ...
      soloStreams: (() => {
        ...
        return {
          ...
          seats: this._soloStatusSeats(),
          heroFocus: this._soloFocusConfig ? { ...this._soloFocusConfig } : null,
        };
      })(),
      fleetOrchestrator: this.fleetOrchestrator?.status?.() || null,
    };
  }
}
