...
function buildSoloLiveSeo(opts = {}) {
  ...
  const title = buildSoloLiveTitle(login || opts.login, opts.quadrant, new Date(), {
    streamerLock: opts.streamerLock,
  });
  ...
  const streamerLock = String(process.env.LIVE_GRID_SOLO_STREAMER_LOCK ?? 'on').toLowerCase() !== 'off';
  const screenLine = streamerLock && login
  ...
  const description = [
    `🔴 LIVE NOW — ${name} full-screen on ClipzWorld${streamerLock ? '' : ` Screen ${q}`}`,
    '',
    screenLine,
    '',
