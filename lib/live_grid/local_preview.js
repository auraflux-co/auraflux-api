...
function latestHlsSegmentPath() {
  try {
    if (!fs.existsSync(HLS_INDEX)) return null;
...
/** Age of newest HLS segment in ms, or null if unavailable. */
function hlsSegmentAgeMs() {
  try {
    const segPath = latestHlsSegmentPath();
    if (!segPath) return null;
    return Date.now() - fs.statSync(segPath).mtimeMs;
  } catch (_) {
    return null;
  }
}
