...
function buildRestreamerArgs(hlsPath, rtmpUrl) {
  ...
  if (isRemote) {
    // Remote HLS URL — pace at realtime
    args.push(
      '-re',
      '-reconnect', '1',
    ...
    // Local growing playlist from compositor — NO -re. Wall-clock pacing desyncs when
    // libx264 falls behind → HLS gaps → YouTube audio/video cutouts with 0 restarts.
    args.push(
