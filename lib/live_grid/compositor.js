...
function buildArgs(opts = {}) {
  ...
  if (bedIdx >= 0) {
    ...
    fc.push(`[${bedIdx}:a]aresample=async=1,volume@bed=${bedVol}[beda]`);
    fc.push(`[amx][beda]amix=inputs=2:duration=longest:normalize=0[amix]`);
    fc.push(vertOut ? `[amix]asplit=2[aout][aoutv]` : `[amix]anull[aout]`);
  } else {
...
class MasterCompositor extends EventEmitter {
  ...
  setFallbackMusic(active, { volume } = {}) {
    ...
    if (active) this.opts.muted = true;
    return this._writeGates();
  }
...
module.exports = { MasterCompositor, buildArgs, gridEncodeConfig, quadMasterInputArgs, USE_UDP_RELAY, isUdpInputNotReady };
