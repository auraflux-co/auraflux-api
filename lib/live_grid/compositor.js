 ...
 *   verticalOutput — optional second 9:16 output (CPD-1001): on-air quadrant
 *                crop (follows audio switches) stacked above the full grid,
 *                1080x1920, encoded in the same ffmpeg run. For the YouTube
 *                landscape + portrait dual-view pairing.
 */
function buildArgs(opts = {}) {
  ...
  // or none when muted (CPD-979 music guard mutes the whole mix).
  // aresample=async=1 (CPD-1001) absorbs the jittery timestamps live RTSP
  // feeds deliver (non-monotonic DTS) so audio stops drifting against video.
  for (let q = 0; q < 4; q++) {
  ...
  const gridConsumers = vertOut ? 3 : 1;
  fc.push(`[grid]split=${gridConsumers}[g0]${vertOut ? '[gv1][gv2]' : ''}`);
  if (logoIdx >= 0) {
  ...
  if (vertOut) {
    // 9:16 view: on-air quadrant on top (crop@vcrop x/y are runtime-commandable
    // — the crop follows audio switches), full grid below for context.
    fc.push(
  ...
  if (vertOut) {
    ...
    args.push(
      '-map', '[vert]', '-map', '[aoutv]',
      '-c:v', 'h264_videotoolbox', '-b:v', `${opts.verticalBitrateK || 4500}k`, '-g', '60',
      '-pix_fmt', 'yuv420p',
      '-c:a', 'aac', '-b:a', '160k', '-ac', '2', '-ar', '44100'
    );
    if (opts.durationSec) args.push('-t', String(opts.durationSec));
...
class MasterCompositor extends EventEmitter {
  ...
  /**
   * Mute/unmute the whole mix (CPD-979): close every volume gate without
   * moving the on-air border. Unmuting reopens the current audioQuad's gate.
   * Persisted in opts so respawns keep the muted state.
   */
  setMuted(muted) {
  ...
  _writeGates() {
    ...
    try {
      ...
      if (this.opts.verticalOutput) {
        // Keep the 9:16 view's top panel on the on-air quadrant (CPD-1001)
