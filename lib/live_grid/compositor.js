 ...
 *
 * Swap behaviour (CPD-1005): no planned master restart by default — RTSP
 * reconnect is not available on ffmpeg RTSP demuxer; a freeze watchdog
 * restarts the master if a quadrant stays unreadable. Set LIVE_GRID_SWAP_RESTART=on
 * to restore the pre-1005 debounced restart on every swap (brief YT blip).
 */
...
/** RTSP input — tcp only (ffmpeg 8.x RTSP demuxer rejects reconnect/rw_timeout). */
...
function buildArgs(opts = {}) {
  ...
  fc.push(`[grid]split=${vertOut ? 3 : 1}[g0]${vertOut ? '[gv1][gv2]' : ''}`);
  if (logoIdx >= 0) {
...
class MasterCompositor extends EventEmitter {
  ...
  status() {
    return {
      ...
      encode: gridEncodeConfig(this.opts),
    };
  }
}

module.exports = { MasterCompositor, buildArgs, gridEncodeConfig, rtspInputArgs };
