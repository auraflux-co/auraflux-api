...
const { hlsOutputArgs, hlsTeeSpec } = require('./hls_output');
...
/** Pass through on-air AAC without aresample/re-encode (RTSP direct only). */
function audioCopyEnabled() {
  if (String(process.env.LIVE_GRID_AUDIO_COPY || 'on').toLowerCase() === 'off') return false;
  // UDP relay + middleware HLS: copied AAC drifts from re-encoded video → deep/choppy on YouTube.
  if (USE_UDP_RELAY && String(process.env.LIVE_GRID_OUTPUT_MIDDLEWARE || '').toLowerCase() === 'on') {
    return false;
  }
  return true;
}

function audioOutputSampleRate() {
  return parseInt(process.env.LIVE_GRID_AUDIO_HZ || '48000', 10);
}
...
function finalizeAudioMix(label) {
  const hz = audioOutputSampleRate();
  return `[${label}]aformat=sample_rates=${hz}:channel_layouts=stereo,` +
...
function videoEncoderArgs(cfg, { isRtmp = false } = {}) {
  if (cfg.encoder === 'libx264') {
    const preset = process.env.LIVE_GRID_X264_PRESET || 'veryfast';
    const x264 = `keyint=${cfg.gop}:min-keyint=${cfg.gop}:scenecut=0:nal-hrd=cbr`;
    const args = [
    ...
    if (isRtmp) {
      const maxK = parseInt(process.env.LIVE_GRID_X264_MAXRATE_K || String(cfg.bitrateK), 10);
      const bufK = parseInt(process.env.LIVE_GRID_X264_BUFSIZE_K || String(cfg.bitrateK * 2), 10);
      args.push('-maxrate', `${maxK}k`, '-bufsize', `${bufK}k`);
    }
    return args;
  }
...
function describeEncodePlan(opts = {}) {
  ...
  const aspect = outW === outH ? 'square' : (outW > outH ? 'landscape' : 'portrait');
  return {
    canvas: `${outW}×${outH}`,
    localHls: localHls ? `${outW}×${outH}` : null,
    rtmp: sqPad ? '1080×1080 (square letterbox — wrong for landscape grid VODs)' : `${outW}×${outH} ${aspect}`,
    rtmpSquare: sqPad,
...
function buildArgs(opts = {}) {
  ...
  if (overlayAssetsReady(imageInputs)) {
  ...
  const gridOut = avatarIdx >= 0 ? '[gridav]' : (overlayAssetsReady(imageInputs) ? '[gridout]' : '[grid]');
  const useDirectAudio = audioDirectEnabled() && !fallbackMusicPath;
  const useAudioCopy = useDirectAudio && audioCopyEnabled() && !vertOut;
  ...
  if (useAmixGates && !fallbackMusicPath) {
    ...
    for (let q = 0; q < 4; q++) {
      fc.push(
        `[${q}:a]aresample=${hz}:async=1:first_pts=0,` +
        `aformat=sample_rates=${hz}:channel_layouts=stereo,` +
    ...
    fc.push(`anullsrc=r=${hz}:cl=stereo[sil]`);
    fc.push(`[amxraw][sil]amix=inputs=2:duration=longest:weights=1 0.001[amx]`);
  } else if (audioDirectEnabled() && fallbackMusicPath) {
    fc.push(`[${audioQuad}:a]aresample=${audioOutputSampleRate()}:async=0:first_pts=0,anull[axon]`);
  }
  if (bedIdx >= 0) {
    const bedVol = opts.fallbackMusicActive ? (opts.fallbackMusicVolume ?? 0.32) : 0;
    fc.push(`[${bedIdx}:a]aresample=${audioOutputSampleRate()}:async=0,volume@bed=${bedVol}[beda]`);
  ...
  const aenc = useAudioCopy
    ? ['-c:a', 'copy']
    : ['-c:a', 'aac', '-b:a', `${cfg.audioBitrateK}k`, '-ac', '2', '-ar', String(audioOutputSampleRate())];
  ...
  if (sqPad) {
    ...
    const ytRtmpEnc = rtmpVideoOut(cfg, 1080, 1080);
    args.push(
      '-map', '[v_hls]', ...audioMapHls, ...landscapeEnc, ...aenc,
      ...hlsOutputArgs(localHls),
      '-map', '[v_yt]', ...audioMapYt, ...ytRtmpEnc, ...aenc,
    );
    ...
    if (localHls && isRtmp) {
      fs.mkdirSync(path.dirname(localHls), { recursive: true });
      const tee = `[f=flv:flvflags=no_duration_filesize:onfail=ignore]${out}|${hlsTeeSpec(localHls)}`;
      args.push('-f', 'tee', tee);
    } else if (isRtmp) {
      args.push('-f', 'flv', out);
    } else if (isHlsOut) {
      args.push(...hlsOutputArgs(out));
    } else {
