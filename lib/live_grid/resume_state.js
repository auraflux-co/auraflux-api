...
function buildEnvResumeFallback() {
  ...
  if (String(process.env.LIVE_GRID_PROGRAM_MODE || '').toLowerCase() === 'solo_roster') {
    return {
      shouldResume: true,
      savedAt: new Date().toISOString(),
      source: 'env_fallback',
      startOpts: { programMode: 'solo_roster', autoPilot: true },
      runtime: {
        programMode: 'solo_roster',
        fleetId: process.env.LIVE_GRID_FLEET_ID || 'a',
        operatorMode: false,
        operatorLocks: [],
        soloStreamsStarted: true,
      },
    };
  }
...
function captureResumeSnapshot(mgr) {
  ...
  const st = mgr.status();
  const soloFleet = !!mgr.fleetOrchestrator
    || String(st.program?.requestedMode || st.program?.activeMode || '').toLowerCase() === 'solo_roster'
    || String(mgr.opts?.programMode || '').toLowerCase() === 'solo_roster';
  return {
    shouldResume: true,
    savedAt: new Date().toISOString(),
    source: 'manager',
    ...
    runtime: {
      ...
      programMode: soloFleet
        ? 'solo_roster'
        : (isOperatorChannelGrid(st.operatorLocks)
          ? 'grid'
      ...
      soloFocus: mgr._soloFocusConfig ? { ...mgr._soloFocusConfig } : null,
      fleetId: mgr.fleetOrchestrator?.fleetId || process.env.LIVE_GRID_FLEET_ID || null,
    },
  };
}
...
function buildResumeStartOpts(state) {
  ...
  const runtimeMode = state?.runtime?.programMode;
  if (runtimeMode === 'solo_roster') {
    opts.programMode = 'solo_roster';
  } else if (isOperatorChannelGrid(locks)) {
    opts.programMode = 'grid';
    opts.avatarOverlay = false;
  } else if (runtimeMode && runtimeMode !== 'auto') {
    opts.programMode = runtimeMode;
  }
  const bid = state?.runtime?.broadcastId;
  if (bid && runtimeMode !== 'solo_roster') {
    opts.broadcastId = bid;
    if (state.runtime.watchUrl) opts.watchUrl = state.runtime.watchUrl;
  }
  if (state?.runtime?.rtmpEncoderStarted && runtimeMode !== 'solo_roster') {
    opts._rtmpGo = true;
  }
  return opts;
}
...
async function applyResumeRuntime(mgr, runtime) {
  ...
  for (const lock of runtime.operatorLocks || []) {
    ...
    if (q < 0 || q > 3) continue;
    try {
      if (lock.kickSlug || lock.platform === 'kick') {
        await mgr.setQuadrantKick(q, lock.kickSlug || lock.login, {
          label: lock.label,
          title: lock.title,
        });
      } else if (lock.type === 'channel' && lock.login) {
