 ...
 *
 * The manager consumes flag changes: switch audio to a music-free quadrant,
 * or mute the whole mix when every live quadrant is playing music. Manual
 * audio pins are respected — the dashboard shows a MUSIC ON AIR warning
 * instead of overriding the operator.
 *
...
const CONFIRM_WINDOWS = 2;  // consecutive music windows before a quad is flagged (~30s)
const CLEAR_WINDOWS = 2;    // consecutive clean windows before the flag clears
const MUSIC_CONFIDENCE_MIN = 0.6;
 ...
 */
function pickAudioQuad(assignments, viewers = {}, musicFlags = []) {
  let best = -1, bestViewers = -1;
  assignments.forEach((login, q) => {
    if (!login || musicFlags[q]) return;
