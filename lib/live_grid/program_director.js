...
/** Ensure the same Twitch login does not appear on two quadrants. */
function dedupeLoginSources(sources, liveLogins = []) {
  const out = [...sources];
  const used = new Set();
  for (let q = 0; q < 4; q++) {
    const s = out[q];
    ...
    for (const l of liveLogins) {
      ...
      if (out.some((x, i) => i !== q && x === l)) continue;
      repl = l;
      break;
    }
    out[q] = repl;
...
function buildQuadrantSources(modeName, config, pollerAssignments, filePaths = {}, feedPaths = {}) {
  ...
  if (modeName !== 'grid') {
    ...
    if (modeName === 'event_night' && !out[0] && liveLogins.length) {
      const used = new Set(out.filter(s => typeof s === 'string'));
      const pick = liveLogins.find(l => !used.has(l));
      if (pick) out[0] = pick;
    }
  }

  out.splice(0, 4, ...dedupeLoginSources(out, liveLogins));
...
class ProgramDirector {
  ...
  resolveMode(explicitMode, date = new Date()) {
    const m = String(explicitMode || this.mode || 'grid').toLowerCase();
    if (m === 'auto') {
