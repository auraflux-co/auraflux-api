 ...
 * Reads quadN RTSP; restarts on grid swap without touching main compositor RTMP.
 */
...
function soloOutputDims() {
  return {
    w: parseInt(process.env.LIVE_GRID_SOLO_OUTPUT_W || '1920', 10),
    h: parseInt(process.env.LIVE_GRID_SOLO_OUTPUT_H || '1080', 10),
    fps: parseInt(process.env.LIVE_GRID_SOLO_FPS || process.env.LIVE_GRID_FPS || '30', 10),
    bitrateK: parseInt(process.env.LIVE_GRID_SOLO_BITRATE_K || '2500', 10),
...
class SoloPublishers {
  ...
  startAll() {
    ...
    let n = 0;
    for (let q = 0; q < 4; q++) {
      if (soloRtmpForQuadrant(q)) {
        this.start(q);
        n++;
      }
    }
    this.log(`solo publishers armed for ${n}/4 seats`);
  }
  ...
  async _startWhenReady(q, reason) {
    ...
    const url = quadUrl(q);
    const deadline = Date.now() + RTSP_WAIT_MS;
    ...
    if (!(await rtspHasVideo(url, 3000))) {
      this.log(`Q${q + 1} solo: RTSP not ready (${reason}) — retry later`);
      setTimeout(() => {
  ...
  _spawn(q, gen, rtmp) {
    ...
    this.log(`Q${q + 1} solo → ${rtmp.replace(/\/live2\/.+$/, '/live2/…')}`);
  }
