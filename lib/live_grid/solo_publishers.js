...
const { soloBrandFrameEnabled, buildSoloBrandVideoFilter } = require('./solo_brand_frame');
...
class SoloPublishers {
  ...
  _spawn(q, gen, rtmp) {
    ...
    const { w, h, fps } = soloOutputDims(q, login);
    const inputArgs = soloInputArgs(q, this.useUdpInput);
    let args;

    if (soloBrandFrameEnabled()) {
      const brand = buildSoloBrandVideoFilter({ w, h, fps });
      if (brand.mode === 'complex') {
        args = [
          '-hide_banner', '-loglevel', 'warning',
          ...inputArgs,
          ...brand.extraInputs.flatMap((p) => ['-i', p]),
          '-filter_complex', brand.filterComplex,
          '-map', brand.mapVideo,
          '-map', '0:a?',
          ...venc,
          '-pix_fmt', 'yuv420p',
