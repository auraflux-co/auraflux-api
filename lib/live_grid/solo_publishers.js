...
const { resolveRtmpForQuadrant, hasRtmpTarget, streamerLockEnabled } = require('./solo_streamer_registry');
...
function soloOutputDims(q = null) {
  const global = {
  ...
  if (q == null) return global;
  return {
    w: readSoloSeatInt(q, 'OUTPUT_W', global.w),
    h: readSoloSeatInt(q, 'OUTPUT_H', global.h),
    fps: readSoloSeatInt(q, 'FPS', global.fps),
    bitrateK: readSoloSeatInt(q, 'BITRATE_K', global.bitrateK),
    audioK: readSoloSeatInt(q, 'AUDIO_BITRATE_K', global.audioK),
  };
}

function soloVideoEncodeArgs(q = null) {
  const { w, h, fps, bitrateK, audioK } = soloOutputDims(q);
  const encoder = process.env.LIVE_GRID_ENCODER || 'libx264';
  const preset = process.env.LIVE_GRID_X264_PRESET || 'ultrafast';
  const gop = fps * 2;
  const maxrate = q != null
    ? readSoloSeatInt(q, 'X264_MAXRATE_K', readSoloSeatInt(q, 'BITRATE_K', bitrateK))
    : parseInt(process.env.LIVE_GRID_SOLO_X264_MAXRATE_K || String(bitrateK), 10);
  const bufsize = q != null
    ? readSoloSeatInt(q, 'X264_BUFSIZE_K', maxrate * 2)
...
class SoloPublishers {
  ...
  startAll(assignments = null) {
    ...
    if (Array.isArray(assignments)) {
      for (let q = 0; q < 4; q++) this._currentLogin[q] = assignments[q] || null;
    }
    ...
    this.log(`solo publishers armed for ${n}/4 seats (${mode}, ${STAGGER_MS / 1000}s stagger${lockNote})`);
  }
  ...
  _spawn(q, gen, rtmp) {
    ...
    const login = this._currentLogin[q];
    const { vf, venc, aenc } = soloVideoEncodeArgs(q);
    const args = [
