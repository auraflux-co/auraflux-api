...
const { readSoloListingForQuadrant, readSoloListingForPoolSlot, poolSize } = require('./solo_listings_env');
...
function poolSlotListing(slot) {
  ...
  return readSoloListingForPoolSlot(n);
}
...
/** Grid quadrant 0–3 for video input; pool slot 1–4 for encode + RTMP when streamer-locked. */
function resolveSoloEncodeSeat(q, login) {
  if (streamerLockEnabled() && login) {
    const slot = getBinding(login)?.slot;
    if (slot) return slot - 1;
  }
  if (Number.isInteger(q) && q >= 0 && q <= 3) return q;
  return 0;
}

function poolSlotForLogin(login) {
  const binding = getBinding(login);
  return binding?.slot || null;
}
...
function applyLoginSlotMap(slotMap = {}, assignments = []) {
  ...
  for (const [rawLogin, rawSlot] of Object.entries(slotMap)) {
    ...
    if (!Number.isInteger(slot) || slot < 1 || slot > poolSize()) {
...
function updatePoolSlotListing(slot, opts = {}) {
  ...
  const n = Number(slot);
  if (!Number.isInteger(n) || n < 1 || n > poolSize()) throw new Error(`slot must be 1-${poolSize()}`);
...
function syncBindingsForAssignments(assignments = []) {
  ...
  for (const lg of active) {
    ...
    const freeSlot = Array.from({ length: poolSize() }, (_, i) => i + 1)
      .find((s) => !usedSlots.has(s) && poolSlotListing(s)?.rtmpUrl);
