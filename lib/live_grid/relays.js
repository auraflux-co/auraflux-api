...
const KICK_RELAY_TRANSCODE = String(process.env.LIVE_GRID_KICK_RELAY_TRANSCODE ?? 'on').toLowerCase() !== 'off';

function relayVideoEncoderArgs() {
  ...
  const vf = `scale=${RELAY_SCALE_W}:${RELAY_SCALE_H}:flags=fast_bilinear:force_original_aspect_ratio=increase,crop=${RELAY_SCALE_W}:${RELAY_SCALE_H},fps=${RELAY_TRANSCODE_FPS}:round=near,setsar=1`;
  if (encoder === 'libx264') {
    return [
      '-map', '0:v', '-map', '0:a?',
      '-vf', vf,
      '-c:v', 'libx264', '-preset', preset, '-tune', 'zerolatency',
      '-profile:v', 'high', '-b:v', `${RELAY_BITRATE_K}k`, '-g', String(gop),
      '-c:a', 'copy',
...
function relayOutputArgs(q, kickQuads) {
  const kickTranscode = KICK_RELAY_TRANSCODE && kickQuads?.[q];
  if (kickTranscode || RELAY_TRANSCODE) return relayVideoEncoderArgs();
  return ['-c', 'copy'];
}
...
class QuadRelays {
  constructor(opts = {}) {
    ...
    this.kickQuads = [false, false, false, false];
  ...
  setKickQuadrant(q, isKick) {
    if (q < 0 || q > 3) return;
    this.kickQuads[q] = !!isKick;
  }
  ...
  startAll() {
    ...
    const kickN = this.kickQuads.filter(Boolean).length;
    this.log(`4× RTSP→UDP relays on ports ${UDP_BASE_PORT}–${UDP_BASE_PORT + 3}${RELAY_TRANSCODE || kickN ? ` (transcode${kickN ? `; kick×${kickN}` : ''})` : ' (copy)'}`);
  }
  ...
  _spawnRelay(q, gen) {
    ...
    const args = [
      ...
      '-err_detect', 'ignore_err',
      '-i', quadUrl(q),
      ...relayOutputArgs(q, this.kickQuads),
