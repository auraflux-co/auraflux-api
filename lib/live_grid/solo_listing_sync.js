...
function seatsFromSidecarStatus(status = {}) {
  ...
  if (streamerLockEnabled()) syncBindingsForAssignments(gridLogins);

  return soloSeats.map((seat) => {
    const q = seat.quadrant;
    const quad = byQuad[q] || {};
    const login = normalizeSoloLogin(quad.login || quad.channelSlug || seat.login);
    const binding = login && streamerLockEnabled() ? getBinding(login) : null;
    return {
      quadrant: q,
      login,
      broadcastId: binding?.broadcastId || seat.broadcastId || null,
      watchUrl: binding?.watchUrl || seat.watchUrl || null,
      label: binding ? `@${binding.login}` : (seat.label || `Screen ${q}`),
      twitchFeedUrl: quad.feedUrl || (login ? `https://www.twitch.tv/${login}` : null),
      configured: binding ? !!binding.rtmpUrl : (seat.configured !== false),
...
function resolveBroadcastIdForSeat(seat, idMap = {}) {
  // Env/status pin wins — discover map is fallback when seat has no broadcastId yet.
...
async function syncSoloListingsFromGrid(opts = {}) {
  ...
  for (const seat of seats) {
    ...
    const seo = buildSoloLiveSeo({
      login: seat.login || '',
      quadrant: (seat.streamerLocked && seat.login ? (getBinding(seat.login)?.currentQuadrant || seat.quadrant) : seat.quadrant) - 1,
      mainWatchUrl,
      gridLogins,
      streamerLock: seat.streamerLocked,
    });
