...
function streamersFromAssignments(assignments = []) {
  return (assignments || [])
    .map((item, i) => {
      if (!item) return null;
      const login = typeof item === 'string'
        ? item
        : (item.login || item.label || null);
      if (!login) return null;
      const slug = String(login).toLowerCase().replace(/^@/, '');
      return {
        login: slug,
...
function buildGoLiveSeo(cfg = {}, opts = {}) {
  ...
  let streamers = Array.isArray(opts.streamers) && opts.streamers.length
    ? opts.streamers
    : streamersFromLocks(locks);
...
function applyGoLiveDefaults(opts = {}) {
  ...
  if (out.operatorMode !== true) {
    ...
    const kickLocks = (cfg?.operatorLocks || [])
      .filter((l) => l?.kickSlug || l?.platform === 'kick')
      .map((l) => ({ ...l }));
    if (kickLocks.length) {
      out._resumeRuntime = {
        ...(out._resumeRuntime || {}),
        operatorMode: false,
        operatorLocks: kickLocks,
      };
    } else if (out._resumeRuntime) {
      out._resumeRuntime = {
        ...out._resumeRuntime,
        operatorMode: false,
        operatorLocks: (out._resumeRuntime.operatorLocks || [])
          .filter((l) => l?.kickSlug || l?.platform === 'kick'),
      };
    }
  }
