...
function isExcludedAvatarCandidate(filePath) {
  ...
  if (/apply_sources_test|test_feeder|test_loop/.test(base)) return true;
  return false;
}

/** Avatar PIP only on news_desk (Q3 slate) unless explicitly configured. */
function isOperatorChannelGrid(locks = []) {
  if (!Array.isArray(locks) || locks.length < 4) return false;
  return locks.every((l) => l?.type === 'channel' && l?.login);
}

function shouldUseAvatarPip(programMode, opts = {}) {
  if (opts.operatorChannelGrid) return false;
...
module.exports = { resolveAvatarOverlay, shouldUseAvatarPip, isExcludedAvatarCandidate, isOperatorChannelGrid };
