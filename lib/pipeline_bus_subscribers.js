...
// Owns the heygen:all_complete → Gate 2 QA → assembly → completion polling chain.
    ...
    async ({ jobId, contentType, segmentUrls, card, segmentData: rawSegmentData }) => {
      ...
      if (!segmentData) {
        ...
        if (shouldUseManualCheckpoint(segCard) && segCard.manualSegments?.holdKind === 'immediate') {
          ...
          logger.warn(
            { jobId, avatarCount: segmentData.filter((s) => s.type === 'avatar').length, clipCount: segmentData.filter((s) => s.type === 'source_clip').length },
          ...
          if (scriptScenes.length > 0) {
            ...
            for (const scene of scriptScenes) {
              if (scene.type === 'source_clip') {
                ...
                if (clip) {
                  segmentData.push({
                    type: 'source_clip', url: clip.clipUrl || clip.url || '',
                    ...
                    storyIndex: clip.storyIndex ?? clipIdx, sceneId: scene.name || scene.id,
                  });
                }
                clipIdx++;
              } else if (scene.type === 'avatar') {
                const sceneKey = scene.name || scene.id;
                const vj = avatarByName[sceneKey] || Object.values(avatarByName).find((v) => v.sceneName === sceneKey);
                if (vj) {
                  ...
                  if (segCard.contentType === 'news' && /STORY(\d+)_INTRO/i.test(vj.sceneName)) {
                    ...
                    if (item) seg.cardData = { title: item.title || `Story ${idx + 1}`, category: 'WORLD NEWS', storyId: `story_${idx + 1}`, imageUrl: item.thumbnailUrl || item.imageUrl || null, heroImageUrl: item.heroImageUrl || item.thumbnailUrl || null, source: item.source || '' };
                    ...
                    seg.cardData = { title: ni?.title || ni?.matchup || rawName || `Game ${gi + 1}`, matchup: ni?.matchup || rawName || `Game ${gi + 1}`, category: 'NBA GAME', storyId: `game_${gi + 1}`, gameId: ni?.gameId || null };
                    ...
                    const streamer = (segCard.streamers || []).find((s) => (s.displayName || '').toLowerCase() === namePart || (s.twitchUsername || '').toLowerCase() === namePart) || (segCard.streamers || [])[0];
                  ...
                  if (scene.hasClipInsert) {
                    const clip = sourceClips[clipIdx] || segCard.orderedClipUrls?.[clipIdx];
                    if (clip) {
                      segmentData.push({ type: 'source_clip', url: clip.clipUrl || clip.url || '', label: clip.label || `${sceneKey}_CLIP`, pageUrl: clip.pageUrl || '', clipTimingTargets: Array.isArray(clip.clipTimingTargets) ? clip.clipTimingTargets : [], clipTimingFormat: clip.clipTimingFormat || 'none', storyIndex: clip.storyIndex ?? clipIdx, sceneId: scene.name || scene.id });
                    }
                    clipIdx++;
                  }
                }
            ...
            segmentData = videoJobs.filter((vj) => vj.status === 'completed' && vj.video_url).sort((a, b) => (a.sceneIndex ?? 0) - (b.sceneIndex ?? 0)).map((vj) => ({ type: 'avatar', url: vj.video_url, label: vj.sceneName, sceneIndex: vj.sceneIndex }));
          }
          logger.warn({ jobId, avatarCount: segmentData.filter((s) => s.type === 'avatar').length, clipCount: segmentData.filter((s) => s.type === 'source_clip').length }, 'heygen:all_complete — segmentData rebuilt from card (startup resume)');
        }
      }
      ...
      if (manualApplied.overrideCount > 0) {
        logger.info({ jobId, overrideCount: manualApplied.overrideCount, manualDir: manualApplied.dir }, 'heygen:all_complete — using manual segment overrides from /tmp');
      }

      logger.info({ jobId, contentType, segmentCount: (segmentUrls || []).length }, 'heygen:all_complete — running Gate 2');
      nrEvent('PipelineHeyGenComplete', { jobId, contentType, customerId: (card && card.customerId) || (_liveCard && _liveCard.customerId) || 'c0', segmentUrlCount: (segmentUrls || []).length, scriptJobId: card.scriptJobId || null, jobSpecId: card.jobSpecId || null });
      ...
      const assemblyId      = _existingAsmId && _retryCount === 0 ? _existingAsmId : `asm_${jobId}${_retryCount > 0 ? `_r${_retryCount + 1}` : ''}`;
      ...
      try {
        await axios.post(
          `http://localhost:${PORT}/assemble`,
          {
            ...
            assemblyId, jobTitle: _humanTitle, contentType, jobId,
        ...
        nrEvent('AssemblyTriggered', { jobId, assemblyId, contentType, segmentCount: (segmentData || []).length, customerId: card.customerId || 'c0' });
        ...
        const pollAssemblyCompletion = () => {
          ...
          if (asmJob.status === 'done' || asmJob.status === 'manual_review') {
            ...
            if (asmJob.qaScore !== undefined) {
              finalCard.gate3 = { score: asmJob.qaScore, outcome: asmJob.qaOutcome || 'manual_review', report: asmJob.qaReport || '', checkedAt: new Date().toISOString() };
              if (asmJob.qaOutcome === 'pass' || asmJob.qaOutcome === 'manual_review') finalCard.stage = 'assembled';
            }
            ...
            nrEvent('PipelineRunTerminal', { jobId, assemblyId, contentType, customerId: _cid, stage: finalCard.stage, gate3Score: asmJob.qaScore ?? null, gate3Outcome: asmJob.qaOutcome || null, hasDriveUrl: !!(asmJob.driveUrl || finalCard.finalUrl) });
            ...
            nrEvent('AssemblyPersistSkipped', { jobId, assemblyId, contentType, asmStatus: asmJob.status || 'unknown', error: (asmJob.error || '').slice(0, 500) });
          }
        };
        ...
        nrEvent('AssemblyTriggerFailed', { jobId, contentType, customerId: (card && card.customerId) || 'c0', error: assembleErr?.message ? String(assembleErr.message).slice(0, 500) : 'unknown' });
      }
    }
  );
}
