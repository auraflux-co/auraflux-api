'use strict';
/**
 * Autonomous production — calendar-driven Generate without dashboard clicks (CPD-1053).
 * Cherry-picked from cwn-c0; uses localhost HTTP to /generate-clip-comp and /generate-full-script
 * (both mounted on Render auraflux-api).
 *
...
function resolveCustomerId(autoCfg = readAutoConfig()) {
  return process.env.PRODUCTION_CRON_CUSTOMER_ID || autoCfg.customerId || 'c0';
}
...
function templateStreamers(autoCfg = readAutoConfig()) {
  ...
  try {
    const ssPath = path.join(REPO_ROOT, 'config', 'streamerSources.json');
...
async function postClipComp(baseUrl, { clips, contentType, platforms, title, scheduledAt, calendarSlotId, autoCfg }) {
  const body = {
    clips,
    contentType,
    platforms,
    ...
    calendarSlotId,
    customerId: resolveCustomerId(autoCfg),
  };
...
async function postFullScript(baseUrl, { type, items, platforms, formType, scriptVariant, calendarSlotId, autoCfg }) {
  const body = {
    type,
    contentType: type,
    items,
...
async function dispatchShortSlot({ slot, dateKey, baseUrl, persistedJobs, autoCfg }) {
  ...
  if (contentType === 'twitch-short') {
    const streamers = templateStreamers(autoCfg);
...
async function dispatchLongformSlot({ slot, dateKey, baseUrl, autoCfg }) {
...
async function dispatchSlotProduction({ slot, dateKey, baseUrl, persistedJobs, state, log = console.log, autoCfg }) {
  ...
  if (kind === 'longform') {
    ...
    const result = await dispatchLongformSlot({ slot, dateKey, baseUrl, autoCfg });
  ...
  const result = await dispatchShortSlot({ slot, dateKey, baseUrl, persistedJobs, autoCfg });
  ...
  for (const slot of today.production || []) {
    ...
    try {
      ...
      const outcome = await dispatchSlotProduction({
        slot, dateKey: et.dateKey, baseUrl, persistedJobs, state, log, autoCfg,
      });
