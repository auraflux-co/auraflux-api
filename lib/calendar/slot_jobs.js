...
function getStreamWindows() {
  ...
  let tv = parseWindow(envTv && envTv !== 'off' ? envTv : tt, parseWindow(tt));
  let grid = parseWindow(envGrid || gridStr, parseWindow(gridStr));
  let livePaused = false;
  let pauseReason = null;
  try {
    const { loadOverrides } = require('./master_plan');
    const { nowET } = require('../live_grid/schedule_time');
    const liveOv = (loadOverrides().liveOverrides || []).find((o) => o.date === nowET().dateKey);
    if (liveOv) {
      if (liveOv.livePaused || liveOv.twitchTv?.window === 'off') tv = null;
      if (liveOv.livePaused || liveOv.youtubeGrid?.window === 'off') grid = null;
      livePaused = !tv && !grid;
      pauseReason = liveOv.reason || null;
    }
  } catch (_) { /* non-fatal */ }
  return {
    tv,
...
function getLiveGridStartBody(persistedJobs) {
  ...
  if (mode === 'news_desk') {
    try {
      ...
      if (latest?.path) {
        body.fileOverrides = { ...(body.fileOverrides || {}), produced_news: latest.path };
