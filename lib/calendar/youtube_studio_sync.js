...
async function fetchCalendarFromApi({ startDate, endDate, persistedJobs = null } = {}) {
  ...
  for (const [jobId, card] of Object.entries(jobsSource || {})) {
    ...
    const publishAt = card.scheduledPublishAt || card.publishedAt
      || card.publishRecord?.publishedAt || null;
    if (!publishAt) continue;
    const dateKey = dateKeyFromIso(publishAt);
    if (!dateKey || dateKey < rangeStart || dateKey > rangeEnd) continue;
    const ytUrl = card.gate5Result?.platforms?.youtube?.url
      || card.publish_results?.youtube?.url
      || card.youtubeUrl
      || null;
    const videoId = card.youtubeVideoId || (ytUrl ? String(ytUrl).match(/[?&]v=([^&]+)/)?.[1] : null);
    fromJobs.push({
      source: 'auraflux',
      jobId,
      videoId,
      title: card.publishCopy?.youtube?.title || card.title || card.scriptTitle || jobId,
      ...
      status: card.stage === 'published' ? 'published' : 'scheduled',
