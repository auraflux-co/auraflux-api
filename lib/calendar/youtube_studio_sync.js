...
function isActiveYoutubeLive(video = {}) {
  const snippet = video.snippet || {};
  const live = video.liveStreamingDetails || {};
  const lbc = String(snippet.liveBroadcastContent || '');
  if (lbc === 'live') return true;
  if (live.activeLiveChatId && !live.actualEndTime) return true;
  const dur = String(video.contentDetails?.duration || '');
  if (dur === 'P0D' && live.activeLiveChatId) return true;
  return false;
}

/** Grid / watch-party rebroadcasts — operational, not content-calendar publishes. */
function isOperationalGridStream(video = {}) {
  if (!isActiveYoutubeLive(video)) return false;
  const title = String(video.snippet?.title || '').toLowerCase();
  return /watch party|multiview|clipzworld watch|grid live|2x2|4-up/.test(title);
}

function inferContentKind(contentDetails = {}, snippet = {}, liveStreamingDetails = null) {
  const lbc = String(snippet.liveBroadcastContent || '');
  if (lbc === 'live') return 'live';
  if (liveStreamingDetails?.activeLiveChatId && !liveStreamingDetails?.actualEndTime) return 'live';
  if (String(contentDetails.duration || '') === 'P0D' && liveStreamingDetails?.activeLiveChatId) return 'live';
...
function videoEffectivePublishAt(video) {
  const live = video.liveStreamingDetails || {};
  if (isActiveYoutubeLive(video)) {
    return live.actualStartTime || video.snippet?.publishedAt || video.status?.publishAt || null;
  }
  const scheduled = video.status?.publishAt;
  const publicAt = video.snippet?.publishedAt;
  // Scheduled go-live in the future always wins — snippet.publishedAt is upload receipt time.
  if (scheduled) {
    const schedMs = new Date(scheduled).getTime();
    if (schedMs > Date.now()) return scheduled;
  }
  const privacy = video.status?.privacyStatus || '';
  if (privacy === 'public' || privacy === 'unlisted') {
    return publicAt || scheduled || null;
  }
  return publicAt || scheduled || null;
}

function calendarStatusFromVideo(video, publishAt) {
  if (isActiveYoutubeLive(video)) return 'live';
  const scheduled = video.status?.publishAt;
  ...
  if (ms > Date.now()) return 'scheduled';
  return 'published';
}

function videoToCalendarItem(video, rangeStart, rangeEnd) {
  if (isOperationalGridStream(video)) return null;
  ...
  const activeLive = isActiveYoutubeLive(video);
  ...
  return {
    ...
    status,
    liveBroadcast: activeLive || !!liveDetails?.scheduledStartTime,
  };
}
...
module.exports = {
  ...
  calendarStatusFromVideo,
  videoToCalendarItem,
  isActiveYoutubeLive,
  isOperationalGridStream,
  CACHE_PATH,
  CALENDAR_CACHE_PATH,
  DEFAULT_DAYS_AHEAD,
};
