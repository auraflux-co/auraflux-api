 ...
 * Set CALENDAR_LIVE_SYNC=on to hot-switch dayparts from content_calendar.json (off by default).
 */
...
function startCalendarLiveSync({ baseUrl, getPersistedJobs, intervalMs = 60 * 1000 } = {}) {
  if (String(process.env.CALENDAR_LIVE_SYNC || 'off').toLowerCase() === 'off') {
