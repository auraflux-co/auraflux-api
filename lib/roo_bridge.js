 ...
 *   logs/job_run_timeline.jsonl — **full** ordered bus stream (see lib/pipeline_events.js emit wrapper)
...
function attachToBus(pipelineBus) {
  ...
  pipelineBus.on('job:confirmed', (data) => {
    const { jobId, contentType, templateId, allReady, commitments, canonicalJobId } = data;
  ...
  pipelineBus.on('heygen:all_complete', (data) => {
    touchActiveJob(data, { pipelineStage: 'heygen_complete', segmentCount: Array.isArray(data.segmentUrls) ? data.segmentUrls.length : null });
  ...
  pipelineBus.on('heygen:poll_tick', (data) => {
    touchActiveJob(data, { pipelineStage: 'heygen_polling' });
  ...
  pipelineBus.on('heygen:poll_terminal', (data) => {
    touchActiveJob(data, { pipelineStage: 'heygen_poll_terminal' });
  ...
  pipelineBus.on('gate:pass', (data) => {
    ...
    activeJobs.set(k, job);
    logEvent('gate:pass', { jobId, canonicalJobId: canonicalJobId || null, gate, score, outcome, contentType, durationMs,
  ...
  pipelineBus.on('gate:sendback', (data) => {
    ...
    if (canonicalJobId) job.canonicalJobId = canonicalJobId;
    activeJobs.set(k, job);
    logEvent('gate:sendback', { jobId, canonicalJobId: canonicalJobId || null, gate, attempt, contentType, qaTier: 1,
  ...
  pipelineBus.on('gate:ops_sendback', (data) => {
    ...
    if (canonicalJobId) job.canonicalJobId = canonicalJobId;
    activeJobs.set(k, job);
    logEvent('gate:ops_sendback', { jobId, canonicalJobId: canonicalJobId || null, gate, attempt, contentType, qaTier: 2,
      fixDirectiveKeys: fixDirective ? Object.keys(fixDirective) : [] });
  });
  ...
  pipelineBus.on('gate:hard_fail', (data) => {
    ...
    if (canonicalJobId) job.canonicalJobId = canonicalJobId;
    activeJobs.set(k, job);
    logEvent('gate:hard_fail', { jobId, canonicalJobId: canonicalJobId || null, gate, reason, contentType,
      concerns: concerns || [], deductions: deductions || [] });
  });
  ...
  pipelineBus.on('gate:escalate', (data) => {
    ...
    if (canonicalJobId) job.canonicalJobId = canonicalJobId;
    activeJobs.set(k, job);
    logEvent('gate:escalate', { jobId, canonicalJobId: canonicalJobId || null, gate, reason,
