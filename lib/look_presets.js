...
const LOOK_PRESETS = {
  auto: {
    ...
    // Voice-based crisp/vivid/cool chosen in clip_comp_transform
...
function buildImpactTintFilter(flashes) {
  const list = (Array.isArray(flashes) ? flashes : [])
    .map((f) => {
      ...
      return {
        atSec,
        duration: Math.max(0.08, Math.min(1.2, Number(f.duration) || 0.28)),
        strength: Math.max(0.08, Math.min(0.45, Number(f.strength) || 0.22)),
      };
    })
    .filter(Boolean)
