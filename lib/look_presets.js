...
function buildImpactTintFilter(flashes) {
  const list = (Array.isArray(flashes) ? flashes : [])
    .map((f) => {
      ...
      return {
        atSec,
        duration: Math.max(0.08, Math.min(1.8, Number(f.duration) || 0.9)),
        strength: Math.max(0.08, Math.min(0.65, Number(f.strength) || 0.42)),
      };
    })
    .filter(Boolean)
  ...
  // Brightness+sat pulse (visible on red stage lighting) + light red push.
  // Pure colorbalance red was invisible on AGT/stage reds → music_beats 1/5 from stills.
  return list.map((f) => {
    const t0 = f.atSec.toFixed(3);
    const t1 = (f.atSec + f.duration).toFixed(3);
    const bright = Math.min(0.45, f.strength * 0.9).toFixed(3);
    const sat = (1 + Math.min(0.8, f.strength * 1.2)).toFixed(3);
    const rs = (f.strength * 0.55).toFixed(3);
    const gs = (-f.strength * 0.2).toFixed(3);
    const bs = (-f.strength * 0.25).toFixed(3);
    return (
      `eq=brightness=${bright}:saturation=${sat}:enable='between(t\\,${t0}\\,${t1})',` +
      `colorbalance=rs=${rs}:gs=${gs}:bs=${bs}:enable='between(t\\,${t0}\\,${t1})'`
    );
  }).join(',');
}
