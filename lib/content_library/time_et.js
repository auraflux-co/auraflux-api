...
/** End of current ISO week (Sunday 23:59:59 ET) for unused staged-clip expiry. */
function stagingExpiresAtMs(from = new Date()) {
  ...
  for (let i = 0; i < 8; i++) {
    if (fmt.format(d) === 'Sun') {
      const ymd = etYmd(d);
      const eow = findEtInstantMs(ymd, 23, 59, 59) + 999;
      if (fromMs <= eow) return eow;
    }
