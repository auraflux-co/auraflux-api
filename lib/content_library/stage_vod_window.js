 ...
 *
 * Uses the same extractVodClips / android/mweb path as production COMPACT/EXTRACT jobs.
 * Does not use the Peaks cookie-first --download-sections detour.
 */
...
async function stageVodWindowToR2(input, { log = console.log, force = false } = {}) {
  ...
  try {
    // Same extractor as COMPACT/EXTRACT jobs — android/mweb, optional proxy, no cookies-first.
    const [extracted] = await extractVodClips(vodUrl, {
      clipCount: 1,
      maxClipSecs: Math.max(15, endSec - startSec),
      jobId,
      isVertical: true,
      vodClipTimestamps: [{ start_s: startSec, end_s: endSec, title: 'peaks_window' }],
    });
    if (!extracted || !fs.existsSync(extracted)) {
      throw new Error('extractVodClips returned no file');
    }
    fs.copyFileSync(extracted, localPath);
