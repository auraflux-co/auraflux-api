...
async function analyzeVodHighlights({
  ...
  // CPD-1275: Twitch chat-rate peaks (no native Most Replayed)
  if (resolvedPlatform === 'twitch' || /twitch\.tv\/videos\//i.test(String(vodUrl || ''))) {
    try {
      ...
      if (heat.ok && heat.heatmap?.length) {
        ...
        if (segments.length) {
          saveVodSegments(sessionId, segments);
          log(`[vod-highlights] twitch_chat_heatmap: ${segments.length} peak(s) from ${heat.messageCount || 0} msgs`);
          return {
            ok: true,
            sessionId,
            segments,
        ...
        log(`[vod-highlights] twitch chat heatmap unavailable (${heat.reason || 'unknown'}): ${heat.message || ''}`);
      }
    } catch (chatErr) {
      log(`[vod-highlights] twitch chat heatmap failed: ${chatErr.message}`);
    }
  }
