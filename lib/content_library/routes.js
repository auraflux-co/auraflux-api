...
function registerContentLibraryRoutes(app) {
  ...
  // Import local MP4 (multipart) or JSON { localPath } → R2 → Compose
  ...
  app.post('/content-library/stage-local', (req, res) => {
    importUpload.single('file')(req, res, async (multerErr) => {
      ...
      try {
        ...
        const { stageLocalFile } = require('./stage_import');
        const body = req.body || {};
        let localPath = body.localPath || body.path || null;
        if (req.file && req.file.path) {
          const dest = pathMod.join(os.tmpdir(), `cwn_import_${Date.now()}${pathMod.extname(req.file.originalname || '.mp4')}`);
          fs.renameSync(req.file.path, dest);
          tmpUploaded = dest;
          localPath = dest;
        }
        if (!localPath) {
          return res.status(400).json({ ok: false, error: 'file or localPath required' });
        }
        const out = await stageLocalFile({
          localPath,
          title: body.title || (req.file && req.file.originalname) || undefined,
          streamer: body.streamer,
          sourceUrl: body.sourceUrl || body.url,
          platform: body.platform,
          force: body.force === '1' || body.force === true,
        }, { log: console.log });
        res.json({ ok: true, clip: out, staged: out });
      } catch (err) {
        res.status(500).json({
          ok: false,
          error: err.message,
          code: err.code || null,
          durationSec: err.durationSec || null,
          maxDurationSec: err.maxDurationSec || null,
        });
      } finally {
        if (tmpUploaded) {
          try { fs.unlinkSync(tmpUploaded); } catch (_) { /* ignore */ }
        }
      }
    });
  });

  // Pull YouTube / Facebook / etc. URL → full staged clip (no roster)
  app.post('/content-library/stage-url', async (req, res) => {
    try {
      const { stageRemoteUrl } = require('./stage_import');
      const body = req.body || {};
      const out = await stageRemoteUrl({
        url: body.url || body.vodUrl,
        title: body.title,
        streamer: body.streamer,
        platform: body.platform,
        startSec: body.startSec != null ? body.startSec : body.start_sec,
      ...
      const status = err.code === 'TOO_LONG' ? 400 : 500;
