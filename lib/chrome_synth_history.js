...
function chromeFingerprintHash(chrome) {
  ...
  return crypto.createHash('sha1').update(JSON.stringify(relevant)).digest('hex').slice(0, 12); // 12 hex chars — enough uniqueness, readable in logs
}
 ...
 */
function shouldSkipSynth(chrome, customerId = 'c1_default', threshold = DEFAULT_THRESHOLD) {
