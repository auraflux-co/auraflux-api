...
function inferFailureClass(evt = {}) {
  ...
  const prod =
    /\b(econn|etimedout|timeout|502|503|504\b|status code 5|puppeteer|ffmpeg|heygen|enotfound|eai_again|socket hang|download failed|assembly_preflight|no such file|sqlite error|json\.parse|rate limit|api error|connection refused)\b/i;
  const qa =
    /\b(clip analysis|authorized facts|cannot be verified against clip|portal 0|no score data|missing.*analysis|gemini.*clip|video metadata|no video analysis|qa input)\b/i;
  if (prod.test(blob)) return FAILURE_CLASS.PRODUCTION_DEFECT;
  if (qa.test(blob)) return FAILURE_CLASS.QA_INPUT_DEFECT;
  const spec =
    /\b(locked intro|locked outro|fabricat(?:ion|ed)?|prohibited|scene count|clip count|format mismatch|template|accuracy|intro text incorrect)\b/i;
...
function recordWhyLedger(p) {
  ...
  const contractDigest =
    p.contractDigest && typeof p.contractDigest === 'object' ? p.contractDigest : null;
  const evidenceDigest =
    p.evidenceDigest && typeof p.evidenceDigest === 'object' ? p.evidenceDigest : null;
