...
async function runUnifiedGatePolicy({
  ...
  // CPD-967: hard_fail handling depends on whether a repair intervention is wired.
  // Sendbacks never help on hard_fail (same file → same result), so we always skip them.
  // If no repair handler exists → unrecoverable, stop immediately.
  // If a repair handler IS wired → skip sendbacks, jump straight to the intervention loop
  // so the handler can mutate the source material (e.g. excise frozen segments) and
  // portal3a can re-run on the repaired file.
  if (result?.outcome === 'hard_fail') {
    if (!runInterventionAttempt) {
      policy.status = 'hard_stop';
      if (onHardStop) {
        try { await onHardStop({ gateKey, jobId, policy, result }); } catch (_e) { /* non-fatal */ }
      }
      await persist();
      return { result, policy };
    }
    // Skip the sendback loop — jump to interventions directly.
    policy.sendbackAttempts = maxSendbacks;
  }
...
const EXTENSION_AFTER_PORTAL = {
  ...
  thumbnail_ext:    'portal3b',        // Thumbnail approval stage — fires after Assembly QA (portal3b).
                                       // CPD-428: portal3b is active for both clips and commentary jobs;
                                       // portal4 is skipped for clips contentType so thumbnail_ext
                                       // never fired. Assembly is complete and valid at portal3b exit.
  gpt4o_qa_ext:     'portal3b',        // GPT-4o deep production quality + creative review (CPD-431).
                                       // Fires after Assembly QA, before publish. Runs Whisper
                                       // transcription + GPT-4o frame analysis. Non-blocking — skip
                                       // on any error so delivery is never held by this check.
  shoppable_ext:    'portal4',         // shoppable CTA bake-in fires after assembly, before publish
  twelvelabs_qa_ext: 'portal4',        // Twelve Labs Pegasus video QA (CPD-431). Fires after portal4
                                       // (r2VideoUrl available). Sends video URL to Pegasus 1.2 for
                                       // visual/audio/production/content quality score. Non-blocking.
};

// Portal order constant — used for checkpoint skip calculation.
...
async function runPortalSequence({
  ...
  onJobFailed,
  persistJobStatus,
  // CPD-898: resume portal key — skip all portals at/before this key on retry
  ...
  // CPD-898: compute skip threshold — all portals at/before resumeFromPortal are skipped
  const resumeIdx = resumeFromPortal ? PORTAL_ORDER.indexOf(resumeFromPortal) : -1;
  ...
  for (const portalKey of activePortals) {
    ...
    // CPD-898: skip portals already checkpointed on a prior attempt
    if (resumeIdx >= 0 && PORTAL_ORDER.indexOf(portalKey) <= resumeIdx) {
      console.log(`[portal-sequence:${jobId}] ${portalKey} — checkpoint skip (already passed on prior attempt)`);
      portalResults[portalKey] = { skipped: true, reason: 'checkpoint_already_passed' };
      continue;
    }
    ...
    if (policy.status === 'passed') {
      if (onPortalPass) {
        // CPD-492: Do NOT swallow — onPortalPass triggers assembly which must be fatal on failure.
        await onPortalPass(portalKey, result, policy);
      }

      // CPD-898: write checkpoint AFTER onPortalPass (including any assembly step)
      // so a retry can skip already-completed work.
      if (!jobSpec.state) jobSpec.state = {};
      if (!jobSpec.state.checkpoints) jobSpec.state.checkpoints = {};
      jobSpec.state.checkpoints.lastPassingPortal   = portalKey;
      jobSpec.state.checkpoints.lastPassingPortalAt = new Date().toISOString();
      ...
      for (const extKey of activeExtensions) {
        ...
        if (extPolicy.status === 'passed') {
          if (onPortalPass) {
            // CPD-492: propagate extension pass errors same as portal pass errors
            await onPortalPass(extKey, extResult, extPolicy);
          }
