...
function buildHookMachinePrompt(ctx, observation, { fixDirective = null, count = null } = {}) {
  ...
  const playbookBlock = buildPlaybookPromptBlock(loadHookPlaybook());
...
- MUST include at least one concrete noun from the observation (object, action, or quoted detail)
...
{"hooks":[{"text":"Hook line here","rank":1,"tensionScore":95,"why":"One sentence why it stops the scroll"},{"text":"Second option","rank":2,"tensionScore":88,"why":"..."}]}
...
async function callGeminiHookMachine(prompt) {
  return parseHookMachineResponse(await fetchGeminiHookMachineRaw(prompt));
}

/**
 * Generate + rank hook candidates via Gemini Hook Machine.
 * @returns {Promise<{ candidates: Array, rawCount: number, parseFailed?: boolean }>}
 */
