...
function gateSummaryFromCommitment(commitment) {
...
function buildGateContracts(jobSpec, commitments = {}) {
  ...
  for (const gate of GATE_ORDER) {
    gates[gate] = {
      ...
      prerequisites: GATE_PREREQS[gate] || [],
      expectedFromOrder: {
        templateId,
        contentType,
      ...
      commitSummary: gateSummaryFromCommitment(commitments?.[gate]?.commitment),
...
function baselineFromJobSpec(jobSpec) {
  return {
    ...
    ordered: {
      ...
      expectedClipCount: jobSpec?.designSpec?.sceneStructure?.expectedClipCount
        ?? jobSpec?.designSpec?.expectedClipCount
        ?? 0,
...
function gatePassedish(result) {
  ...
  return result.outcome === 'pass' || result.outcome === 'pass_with_notes' || result.outcome === 'proceed';
}
...
function softHealJobSpec(currentSpec, gate) {
  ...
  if ((!Array.isArray(inputs.items) || inputs.items.length === 0) && Array.isArray(sceneItems) && sceneItems.length > 0) {
    ...
    if (hydrated.length > 0) {
      ...
      heals.push(`hydrated order.inputs.items from designSpec.sceneStructure.items (${hydrated.length})`);
    }
  }
...
function auditIssuesForGate(spec, gate, result) {
  const issues = [];
  const prereqs = GATE_PREREQS[gate] || [];
  ...
  if (gate === 'gate4' || gate === 'gate5') {
    const assembled = spec?.state?.savedOutputs?.assembledPath || spec?.assembledPath || spec?.outputPath;
  ...
  if (gate === 'gate5') {
    ...
    if (baselinePlatforms.length > 0 && nowPlatforms.length > 0) {
      ...
      if (!same) issues.push(`delivery platforms drifted from baseline (${baselinePlatforms.join(',')} -> ${nowPlatforms.join(',')})`);
    }
  }
  return issues;
}
...
function auditAndRecordGateResult({ jobId, gate, result, fallbackJobSpec = null }) {
  ...
  const record = {
    ...
    cumulativeGateOrder: GATE_ORDER.slice(0, GATE_ORDER.indexOf(gate) + 1)
  };

  const existingContracts = (healedSpec?.state?.gateContracts) || {};
  const nextQaByGate = { ...(existingContracts.qaByGate || {}), [gate]: record };
  const cumulativeIssues = Object.values(nextQaByGate)
    .flatMap(r => (r?.issues || []).map(i => `${r.gate}: ${i}`));
  const cumulativeHeals = Object.values(nextQaByGate)
    .flatMap(r => (r?.softHeals || []).map(h => `${r.gate}: ${h}`));
  const cumulativeStatus = cumulativeIssues.length === 0
    ? 'pass'
...
function buildGateStatusSnapshot(jobSpec) {
  ...
  for (const gate of GATE_ORDER) {
    ...
    if (row.passed === false && row.outcome && row.outcome !== 'sendback') blockers.push(`${gate} outcome=${row.outcome}`);
  }
