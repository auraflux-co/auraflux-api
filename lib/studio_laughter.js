...
async function mixCrowdLaughOnReaction(reactionMp4, opts = {}) {
  ...
  const isClip1Reaction = /_CLIP1_REACTION$/i.test(String(opts.sceneLabel || ''));
  const isSceneResetReaction = isClip2Reaction || isClip1Reaction;
  ...
  const fadeOutSec = isSceneResetReaction ? 0.55 : 0.45;
  ...
  // No ducking — Bobby G is already silent during the HeyGen pause window;
  // ducking the main track causes an audible level blip on every reaction.
  let filter = [
    crowdChain,
    '[0:a][crowd]amix=inputs=2:duration=first:dropout_transition=0:normalize=0[aout]',
  ].join(';');
  if (isSceneResetReaction && segDur && segDur > 1) {
    const tailFade = 0.45;
    const tailSt = Math.max(0, segDur - tailFade);
    filter = `${filter};[aout]afade=t=out:st=${tailSt.toFixed(3)}:d=${tailFade.toFixed(3)}[aout2]`;
  }
  const audioMap = isSceneResetReaction && segDur && segDur > 1 ? '[aout2]' : '[aout]';
