...
async function assembleDualSourceStack({
  ...
  await concatTwo(bottomHoldPath, bottomPlayPath, bottomTimelinePath, true);

  // Extract bottom audio for phase 2 and delay it; mix with top audio (phase 1)
  ...
  try {
    ...
    hasBottomAudio = fs.existsSync(bottomAudioPath);
  } catch (_) {
  ...
  if (hasBottomAudio) {
    await runFfmpeg([
      ...
      '-filter_complex',
      `${labelFilter};`
      + `[2:a]adelay=${delayMs}|${delayMs},apad[ba];`
      + `[0:a]apad[ta];`
      + `[ta][ba]amix=inputs=2:duration=first:dropout_transition=0:normalize=0[aout]`,
      '-map', '[vout]',
      '-map', '[aout]',
      ...ffmpegEncodeArgs(true),
      '-c:a', 'aac', '-ar', '44100', '-ac', '2',
  ...
  log(`  ✅ dual_source_stack ready (${totalDur.toFixed(1)}s) · labels "${labels.top}" / "${labels.bottom}"`);
  return {
