...
async function enrichDualSourceSeoBrief(card) {
  ...
  // Never leave Local import / yt_paste on streamers — GPT + ensureStreamerInTitle prepend "Local:"
  const cleaned = [];
  if (ytClip?.sourceAuthor) cleaned.push(ytClip.sourceAuthor);
  for (const s of (card.streamers || [])) {
    const name = typeof s === 'string' ? s : (s?.displayName || s?.streamer || '');
    if (!isGarbageImportTalentName(name) && !cleaned.includes(name)) cleaned.push(name);
  }
  card.streamers = cleaned;
