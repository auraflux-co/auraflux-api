...
/**
 * Whisper-only C11 skips Hook Machine — rebuild SEO brief from Age labels + real clip titles
 * so publish copy is not "Local import's Twitch Antics".
 */
function enrichDualSourceSeoBrief(card) {
  ...
  const briefClips = clips.map((clip, i) => {
    const label = i === 0 ? labels.top : labels.bottom;
    const title = String(clip?.title || clip?.clipTitle || '').trim();
    const pageUrl = clip?.pageUrl || clip?.url || '';
    const platform = String(clip?.platform || '').toLowerCase()
      || detectUrlPlatform(pageUrl)
      || detectUrlPlatform(clip?.mp4Url || '')
      || (/youtube\s*import/i.test(title) ? 'youtube' : '')
      || (clip?.game === 'import' ? 'import' : 'unknown');
    return {
      index: i,
      streamer: label,
      displayName: label,
      ...
      platform,
      observation:
        `Then/Now pane labeled "${label}". Source clip: "${title || 'untitled'}". `
        + `This Short stacks ${labels.top} (TOP) over ${labels.bottom} (BOTTOM) — age/then-vs-now comparison, not a Twitch highlight reel.`,
      hook: label,
    };
  });
  if (!briefClips.length) {
    briefClips.push(
      { index: 0, streamer: labels.top, displayName: labels.top, platformTitle: '', platform: 'unknown', observation: `TOP pane ${labels.top}`, hook: labels.top },
      { index: 1, streamer: labels.bottom, displayName: labels.bottom, platformTitle: '', platform: 'unknown', observation: `BOTTOM pane ${labels.bottom}`, hook: labels.bottom },
    );
  }
  const hasYt = briefClips.some((c) => c.platform === 'youtube');
  card.clipCompBrief = {
    ...(card.clipCompBrief || {}),
    ...
    sourceChannel: null,
    leadTitleDraft: `${labels.top} vs ${labels.bottom} — Then vs Now`,
    leadReason: 'C11 dual_source_stack Then/Now Age comparison Short',
    dualSourceStack: true,
    sourceVideoPreferred: hasYt || briefClips.every((c) => c.platform !== 'twitch'),
    clips: briefClips,
