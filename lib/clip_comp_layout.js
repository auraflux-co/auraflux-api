...
function resolveFullBleedSourceRect(cropCx, cropCy, cropZoom = 1, sourceAspect = 16 / 9) {
  ...
  const w = isPortrait
    ? Math.min(1, 1 / zoom)
    : Math.min(1, FULL_BLEED_PORTRAIT_W_FRAC / zoom);
  const h = Math.min(1, 1 / zoom);
  const cx = Number.isFinite(Number(cropCx)) ? Number(cropCx) : 0.5;
  const cy = Number.isFinite(Number(cropCy)) ? Number(cropCy) : 0.5;
  return {
...
function buildFullBleedFilter(subjectCx = null, subjectCy = null, cropZoom = null, sourceAspect = 16 / 9) {
  ...
  if (!hasCx && !hasCy && !hasZoom) return FULL_BLEED_FILTER;
  ...
  return `crop=trunc(iw*${f(rect.w)}/2)*2:trunc(ih*${f(rect.h)}/2)*2:trunc(iw*${f(rect.x)}/2)*2:trunc(ih*${f(rect.y)}/2)*2,scale=1080:1920,format=yuv420p`;
}
...
async function applyPortraitFullBleed(inputPath, outputPath, {
  ...
  if (log) {
    const cxNote = filter === FULL_BLEED_FILTER ? '' : ` (subject cx=${Number(subjectCx).toFixed(2)} cy=${Number(subjectCy).toFixed(2)} zoom=${clampCropZoom(cropZoom).toFixed(2)})`;
...
async function applySquareCrop(inputPath, outputPath, { log = null } = {}) {
  await new Promise((res, rej) => {
    execFile(ffmpegPath(), [
      '-i', inputPath,
      '-vf', SQUARE_FILTER,
 ...
 */
function buildSplitScreenFilter(facecamRect, { topHeight = SPLIT_TOP_HEIGHT, contentCx = null, bottomCropPct = 0, bottomPaneRect = null } = {}) {
  ...
  const bottomRect = normalizeFacecamRect(bottomPaneRect);
  let bottomChain;
  if (bottomRect) {
    ...
    bottomChain = `[contentsrc]${contentPre}${botCrop},scale=1080:${bot}:force_original_aspect_ratio=increase,crop=1080:${bot},setsar=1[bot]`;
  } else {
...
async function applyPortraitSplitScreen(inputPath, outputPath, {
  ...
  const filter = buildSplitScreenFilter(facecamRect, { topHeight, contentCx, bottomCropPct, bottomPaneRect });
  await new Promise((res, rej) => {
    execFile(ffmpegPath(), [
      '-i', inputPath,
...
async function applyPortraitLayout(inputPath, outputPath, {
  ...
  if (mode === 'full_bleed_crop') {
    ...
    await applyPortraitFullBleed(inputPath, layoutOut, { log, bottomCropPct, subjectCx, subjectCy, cropZoom });
    return finishPortraitOutput(layoutOut, outputPath, deliveryAspect, log);
  }
  const { applyPortraitBlurPad } = require('./assembly_postprocess');
  await applyPortraitBlurPad(inputPath, layoutOut, { jobId: 'layout', log, bottomCropPct });
  return finishPortraitOutput(layoutOut, outputPath, deliveryAspect, log);
}

async function trimSourceRange(inputPath, outputPath, startSec, endSec, log) {
  ...
  await new Promise((resolve, reject) => {
    execFile(ffmpegPath(), [
      ...
      '-pix_fmt', 'yuv420p',
      '-y', outputPath,
    ], { timeout: 600_000 }, (err) => (err ? reject(err) : resolve()));
  });
...
async function applyPortraitLayoutTimed(inputPath, outputPath, {
  ...
  if (plan.length <= 1) {
    const segCreative = mergeCreativeForSegment(compCreative, plan[0]?.layout || {});
  ...
  for (let i = 0; i < plan.length; i++) {
    ...
    await trimSourceRange(inputPath, sliceIn, sliceStart, sliceEnd, log);
