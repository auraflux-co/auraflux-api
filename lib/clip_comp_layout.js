...
/**
 * CPD-1293: when sourceCleanup hides the Twitch chat rail (delogo → black),
 * clamp crop rects so they do not include that black column (pillarboxing).
 */
function insetRectAwayFromChatRail(rect, sourceCleanup = null) {
  const r = normalizeFacecamRect(rect);
  if (!r) return null;
  const sc = sourceCleanup && typeof sourceCleanup === 'object' ? sourceCleanup : null;
  if (!sc || sc.enabled === false || !sc.hideChatRail) return r;
  let chatLeft = 0.78;
  try {
    const { TWITCH_UI_PRESETS } = require('./source_cleanup');
    if (TWITCH_UI_PRESETS?.chat_rail?.x != null) chatLeft = Number(TWITCH_UI_PRESETS.chat_rail.x);
  } catch (_) { /* keep default */ }
  chatLeft = Math.max(0.5, Math.min(0.95, chatLeft));
  const right = r.x + r.w;
  if (right <= chatLeft + 0.005) return r;
  const w = Math.max(0.12, chatLeft - r.x - 0.01);
  return normalizeFacecamRect({ x: r.x, y: r.y, w, h: r.h });
}
...
function buildSplitScreenFilter(facecamRect, {
  ...
  sourceCleanup = null,
  honorOperatorCrops = false,
} = {}) {
  // CPD-1314: operator green/gold boxes (Save look here) are 1:1 — do not rewrite
  // them with desktop-chrome insets. Auto-detect still insets. Chat-rail clamp
  // stays when hideChatRail is on (black column after delogo).
  let rect = honorOperatorCrops
    ? normalizeFacecamRect(facecamRect)
    : insetFacecamAwayFromTopChrome(facecamRect, topChromeSafe);
  rect = insetRectAwayFromChatRail(rect, sourceCleanup);
  ...
  let bottomRect = honorOperatorCrops
    ? normalizeFacecamRect(bottomPaneRect)
    : insetRectAwayFromDesktopChrome(bottomPaneRect, {
      topSafe: Math.min(0.35, topChromeSafe + BOTTOM_PANE_TOP_EXTRA),
      bottomSafe: bottomChromeSafe,
    });
  bottomRect = insetRectAwayFromChatRail(bottomRect, sourceCleanup);
  const paneCropXY = honorOperatorCrops ? '0:0' : '(iw-1080)/2:0';
  const topPaneCrop = honorOperatorCrops ? `crop=1080:${top}:0:0` : `crop=1080:${top}`;
  let bottomChain;
  if (bottomRect) {
    ...
    // CPD-1317: Save-look boxes fill the pane (stretch). Cover-zoom was clipping the cam.
    bottomChain = honorOperatorCrops
      ? `[contentsrc]${contentPre}${botCrop},scale=1080:${bot},setsar=1[bot]`
      : `[contentsrc]${contentPre}${botCrop},scale=1080:${bot}:force_original_aspect_ratio=increase,crop=1080:${bot}:${paneCropXY},setsar=1[bot]`;
  } else {
  ...
  const topChain = honorOperatorCrops
    ? `[camsrc]${camCrop},scale=1080:${top},setsar=1[top]`
    : `[camsrc]${camCrop},scale=1080:${top}:force_original_aspect_ratio=increase,${topPaneCrop},setsar=1[top]`;
...
async function applyPortraitSplitScreen(inputPath, outputPath, {
  ...
  const filter = buildSplitScreenFilter(facecamRect, {
    ...
    sourceCleanup, honorOperatorCrops,
  });
  await new Promise((res, rej) => {
    execFile(ffmpegPath(), [
      '-i', inputPath,
...
async function applyPortraitLayout(inputPath, outputPath, {
  ...
  if (mode === 'split_screen') {
    ...
    if (rect) {
      ...
      const honorOperatorCrops = !!normalizeFacecamRect(compCreative?.layout?.facecamRect);
      await applyPortraitSplitScreen(inputPath, layoutOut, {
        ...
        bottomChromeSafe,
        previewFast,
        sourceCleanup: compCreative?.sourceCleanup || null,
        honorOperatorCrops,
      });
...
async function applyPortraitLayoutTimed(inputPath, outputPath, {
  ...
  if (!breakpoints.length) {
    // CPD-1293: when the source is NOT pre-trimmed and a real trim window is set,
    // trim first so applyPortraitLayout sees only the correct frames (prevents
    // black early frames and wrong crops from the full-source head).
    let layoutInput = inputPath;
    const needsTrim = !sourceFilePreTrimmed && (start > 0 || trimEnd != null);
    if (needsTrim) {
      const previewFast = !!compCreative?.previewFast;
      const trimmedPath = `${outputPath}.opening_trim.mp4`;
      await trimSourceRange(inputPath, trimmedPath, start, end, log, previewFast);
      layoutInput = trimmedPath;
    }
    if (openingLayout && openingLayout.mode) {
      const segCreative = mergeCreativeForSegment(compCreative, openingLayout);
      const { mode, facecamRect } = await resolveEffectiveLayoutMode(layoutInput, segCreative, log);
      if (log) log(`[layout] openingLayout-only → ${mode}`);
      const result = await applyPortraitLayout(layoutInput, outputPath, {
      ...
      if (needsTrim && layoutInput !== inputPath) {
        try { fs.unlinkSync(layoutInput); } catch (_) { /* ignore */ }
      }
      return result;
    }
    const result = await applyPortraitLayout(layoutInput, outputPath, { compCreative, log, deliveryAspect });
    if (needsTrim && layoutInput !== inputPath) {
      try { fs.unlinkSync(layoutInput); } catch (_) { /* ignore */ }
    }
    return result;
