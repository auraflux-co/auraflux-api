...
function buildSplitScreenFilter(facecamRect, {
  ...
  // CPD-1293: also pull both rects left of the delogo'd chat rail.
  let rect = insetFacecamAwayFromTopChrome(facecamRect, topChromeSafe);
  ...
  let bottomRect = insetRectAwayFromDesktopChrome(bottomPaneRect, {
...
async function applyPortraitSplitScreen(inputPath, outputPath, {
  ...
  const filter = buildSplitScreenFilter(facecamRect, {
    topHeight, contentCx, bottomCropPct, bottomPaneRect, topChromeSafe, bottomChromeSafe, sourceCleanup,
  });
  await new Promise((res, rej) => {
    execFile(ffmpegPath(), [
      '-i', inputPath,
