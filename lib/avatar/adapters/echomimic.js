...
function inferenceTuningFromEnv() {
  if (spikeProfileActive() && !process.env.ECHOMIMIC_PROMPT && !process.env.ECHOMIMIC_USE_DYNAMIC_CFG) {
    return {
      ...SPIKE_INFERENCE,
...
function buildRenderJobInput({ config, videoLength, imageGet, audioGet, outputPut }) {
  ...
  if (config.useIpMask != null) out.use_ip_mask = config.useIpMask;
  return out;
}
...
function resolveConfig({ contentType = 'twitch', format = 'landscape' } = {}) {
  ...
  const avatarId = spikeProfileActive() && !process.env.ECHOMIMIC_PORTRAIT && !process.env.ECHOMIMIC_IMAGE_KEY
    ? resolveSpikePortraitKey()
    : resolvePortraitKey();
  const defaultSteps = spikeProfileActive() ? SPIKE_STEPS : 8;
  return {
    avatarId,
    ...
    engine: 'echomimic',
    steps: parseInt(process.env.ECHOMIMIC_STEPS || String(defaultSteps), 10),
    sampleSize: (() => {
...
function chunkingEnabled() {
  const defaultChunk = spikeProfileActive() ? 'off' : 'on';
  const raw = process.env.ECHOMIMIC_CHUNK;
  if (raw == null || raw === '') return defaultChunk !== 'off';
...
async function submitSegment({ text, title, aspectRatio = '16:9', config, enhancedDelivery = false }) {
  ...
  const maxFrames = parseInt(
    process.env.ECHOMIMIC_MAX_FRAMES || String(spikeProfileActive() ? SPIKE_MAX_FRAMES : 81),
    10
  );
  const maxSec = (maxFrames - 1) / FPS;
