...
const DEFAULT_PROMPT = (
  ...
  + 'camera with a closed relaxed mouth when silent. Lip and jaw movement during '
  + 'speech stays anatomically correct — natural teeth, no exaggerated mouth opening. '
  + 'Hand and body movements are minimal. Preserve background integrity matching '
  + 'the reference image lighting and color temperature.'
);

const HEAD_ONLY_PROMPT = (
  'A bearded man speaks naturally to the camera in a streaming studio. '
  + 'Head and shoulders only. Lip and jaw movement during speech stays anatomically '
  + 'correct — natural teeth, no exaggerated mouth opening. Minimal head motion. '
...
function inferenceTuningFromEnv() {
  ...
  return {
    guidanceScale: parseFloat(process.env.ECHOMIMIC_GUIDANCE_SCALE || '4.0'),
    ...
    negScale: parseFloat(process.env.ECHOMIMIC_NEG_SCALE || (dynamicCfg ? '1.5' : '1.0')),
    negSteps: parseInt(process.env.ECHOMIMIC_NEG_STEPS || (dynamicCfg ? '2' : '0'), 10),
    negativePrompt: process.env.ECHOMIMIC_NEGATIVE_PROMPT || DEFAULT_NEGATIVE_PROMPT,
    prompt: process.env.ECHOMIMIC_PROMPT || DEFAULT_PROMPT,
...
function resolveConfig({ contentType = 'twitch', format = 'landscape' } = {}) {
  ...
  return {
    avatarId: process.env.ECHOMIMIC_IMAGE_KEY || 'spike/cpd881/inputs/bobbyg_heygen_frame.png',
