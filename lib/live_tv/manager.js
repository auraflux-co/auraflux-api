...
/** Default-scan eligibility — finished transformative videos only (CPD-957 / CPD-1023). */
function isPlayable(name, sizeBytes) {
  return isTwitchTvPlayable(name, sizeBytes);
}
...
class LiveTvManager {
  ...
  /**
   * CPD-958: append a newly published video to the running rotation.
   * The file is cached in the background and joins the NEXT playout pass
   * (the channel programs itself — same philosophy as the Live Grid).
   * @returns {boolean} true if the video was accepted
   */
  enqueue(file) {
    if (!this.running || !file || this.curated) return false;
    const abs = path.isAbsolute(file) ? file : path.join(this.outputDir, file);
    let size = 0;
    try { size = fs.statSync(abs).size; } catch (_) { return false; }
    if (!isPlayable(path.basename(abs), size)) return false;
    if (this.playlist.includes(abs)) return false;
    this.playlist.push(abs);
    this.log(`auto-enqueued: ${path.basename(abs)} (rotation: ${this.playlist.length})`);
    if (!this.cacheProc) this._cacheNext(); // caching was idle — kick it
    return true;
  }
