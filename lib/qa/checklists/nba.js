 ...
 */
function getLockedIntroCheck() {
  try {
    const { loadCustomerConfig } = require('../../customerConfig');
    const cfg = loadCustomerConfig('c0', 'long-form');
...
const sceneGuide = 'GAME#_INTRO, GAME#_NARRATION, GAME#_RECAP, GAME#_REACTION (where # = game number)';

/**
 * geminiContext — extra context appended to geminiQACheck EARLY/MIDDLE/LATE audio note.
 * For NBA: Bobby G narrates live over clips so mixed audio is correct — do not fail for it.
 */
