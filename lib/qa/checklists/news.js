 ...
 * For News: anchor reads scripted copy over B-roll. Mixed audio is NOT expected.
...
function getGeminiContext(customerId = undefined) {
  let speakerName = 'the anchor';
  try {
    const { loadCustomerConfig } = require('../../customerConfig');
    const cfg = loadCustomerConfig(customerId, 'long-form');
    speakerName = cfg?.voice?.speakerName || cfg?.designDefaults?.voice?.speakerName || speakerName;
  } catch (_e) {}
  return `\nAUDIO NOTE — News content: ${speakerName} reads scripted copy. Only ${speakerName}'s voiceover should be audible during B-roll segments. FAIL AUDIO if background clip audio is dominant or drowns out the voiceover.`;
}
const geminiContext = getGeminiContext();

/**
 * expectedScenesCount — 1 COLD OPEN + (stories × 4 scenes) + 1 OUTRO.
 * News uses: STORY#_INTRO, STORY#_SETUP, STORY#_SUMMARY, STORY#_REACTION.
 */
function expectedScenesCount(streamerCount) {
  return 1 + streamerCount * 4 + 1;
}

/** Scoring weights for geminiScriptQA DEDUCTION_MAP. News: items 1,2,8 = -15; 3 = -10; rest -5. */
const deductionMap = { 1: 15, 2: 15, 8: 15, 3: 10, 4: 5, 5: 5, 6: 5, 7: 5, 9: 5, 10: 5 };
const deductionLabels = {
  1: 'CLIP COUNT', 2: 'OUTRO', 3: 'STORY ACCURACY', 4: 'COLD OPEN',
  5: 'STORY SETUP', 6: 'BEAT PLACEMENT', 7: 'CLIP MATCH', 8: 'LOCKED INTRO',
  9: 'SOURCE ATTRIBUTION', 10: 'REACTION',
};
