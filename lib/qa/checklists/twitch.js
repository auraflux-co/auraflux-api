...
function checklist({ expectedScenes, expectedClips, customerId = undefined }) {
  return [
   ...
   - Are there exactly ${expectedScenes} === SCENE === markers?`,
    ...
    `6. REACTION LENGTH: Is each reaction exactly 1 spoken line? (FAIL only if 2+ sentences — a single word + period like "Classy." is PASS)`,
    ...
    `12. VOICE STYLE (-15 if wrong): Does every scene use Talk Soup / Joel McHale deadpan sarcasm (Bobby G host)? FAIL this item (-15) if any scene contains:
   ...
   - Neutral coroner-report narration in REACTION scenes ("Japan scored a goal.", "The game continued.", "He was very excited.")
   - Sentences that go up at the end — everything lands flat
   Good REACTION: "Classy." / "So... that happened." / "Chicks, man." / "Oh here go hell come."
   Good SETUP: pivot from normal premise to clip absurdity in 1-2 sentences, then clip
   ...
   Bad: "The word was spelled correctly." (neutral narration — not Soup)`,
  ].filter(Boolean);
}
...
/**
 * expectedScenesCount — how many === SCENE === markers to expect.
 * 1 INTRO + (streamers × (1 intro + clipsPerStreamer × 2)) + 1 OUTRO
 */
function expectedScenesCount(streamerCount, clipsPerStreamer = 2) {
  const scenesPerStreamer = 1 + clipsPerStreamer * 2;
  return 1 + streamerCount * scenesPerStreamer + 1;
}

/** Scoring weights for geminiScriptQA DEDUCTION_MAP (item# → points deducted on FAIL). */
const deductionMap = { 1: 15, 2: 15, 8: 15, 3: 10, 7: 10, 4: 5, 5: 5, 6: 5, 9: 5, 10: 5 };
