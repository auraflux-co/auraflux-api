...
async function fetchData({ items, type, jobId, geminiAnalyzeClip }, cfg) {
  ...
  const summaryPath = (item) => item.sportsApiPath || item.meta?.apiPath || 'basketball/nba';
  const usePickerClip = (item) => !!(item.pickerSelectedClip && item.clipUrl);
  ...
  const isShortForm = type === 'nba-short' || type === 'sports-short';
  ...
  await Promise.all(items.map(async (item) => {
    ...
    try {
      const summaryUrl = `https://site.api.espn.com/apis/site/v2/sports/${summaryPath(item)}/summary?event=${item.gameId}`;
      ...
      if ((isShortForm || usePickerClip(item)) && item.clipDuration) {
      ...
      } else if (usePickerClip(item)) {
        console.log(`[nba-fresh-url] Picker clip — keeping operator-selected URL for ${item.gameId || item.clipTitle || 'clip'}`);
      } else {
  ...
  const orderedClipUrls = items.map((item, i) => {
    ...
    return {
      ...
      streamer:        `game_${i + 1}`,
      title:           item.clipTitle || `${item.away || '?'} vs ${item.home || '?'}`,
      gameId:          item.gameId || '',
