...
async function fetchData({ items, type, jobId, geminiAnalyzeClip }, cfg) {
  ...
  await Promise.all(items.map(async (item) => {
    ...
    try {
      ...
      const hlVideo = topVideos.find(v => (v.headline||'').toLowerCase().includes('highlight'))
                   || topVideos[0]
                   || articleVideos[0];
      if (hlVideo) {
  ...
  if (nbaHits < items.length) {
    ...
    await Promise.all(items.map(async (item) => {
      ...
      try {
        ...
        const hlVideo2 = topVideos2.find(v => (v.headline||'').toLowerCase().includes('highlight'))
                      || topVideos2[0] || articleVideos2[0];
        if (hlVideo2) {
