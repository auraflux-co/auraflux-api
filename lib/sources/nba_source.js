...
async function fetchData({ items, type, jobId, geminiAnalyzeClip }, cfg) {
  ...
  if (missingClipUrl.length > 0) {
    const missingGames = missingClipUrl
      ...
      .join(', ');
    console.warn(
      `[nba_source] Gate 0: Dropping ${missingClipUrl.length}/${beforeDrop} NBA games with no clip URL: ${missingGames}`
    );
  ...
  if (nbaHits < items.length) {
    console.warn(
      `[nba_source] Gate 0: ${nbaHits}/${items.length} video analyses — HLS URLs likely expired. Re-fetching fresh URLs and retrying...`
    );
    await Promise.all(
      items.map(async (item) => {
        if (!item.gameId) return;
    ...
    if (nbaHits < items.length) {
      const failedGames = items
        ...
        .join(', ');
      throw new Error(
        `Gate 0 FAIL: Only ${nbaHits}/${items.length} NBA clips analyzed after retry. Failed: ${failedGames}. HLS streams may be unavailable. Try again in a few minutes.`
      );
    }
  }
