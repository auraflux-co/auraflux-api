 ...
 * ⚠️  C0-ONLY: This source fetches ESPN HLS clips for the ClipzWorld Sports show.
 ...
 * Drops games without clip URLs (Portal 0), re-fetches fresh ESPN HLS URLs,
...
function extractNbaTimingTargets(analysisText) {
  ...
  for (const line of lines) {
    ...
    if (
      ...
      (!Number.isFinite(endToken) && endToken !== 'end') ||
      !narration
    )
      continue;
...
async function fetchData({ items, type, jobId, geminiAnalyzeClip }, cfg) {
  ...
  console.log(
    `[nba_source] Gate0 strategy: phase=${strategyPhase} passType=${strategyPassType} passNumber=${strategyPassNumber}`
  );

  const selectHighlightVideo = (topVideos, articleVideos) => {
    const allVideos = [
      ...(Array.isArray(topVideos) ? topVideos : []),
      ...(Array.isArray(articleVideos) ? articleVideos : []),
    ].filter((v) => {
    ...
    // Longest duration = most complete game coverage (full highlights reel beats individual plays)
    return allVideos.slice().sort((a, b) => (b.duration || 0) - (a.duration || 0))[0];
  };
  // Portal 0: Drop games with no clipUrl, proceed with valid games only (count varies by day)
  const beforeDrop = items.length;
  const missingClipUrl = items.filter((item) => !item.clipUrl);
  if (missingClipUrl.length > 0) {
    const missingGames = missingClipUrl
      .map((i) => `${i.away || '?'} vs ${i.home || i.gameId || '?'}`)
      .join(', ');
    console.warn(
      `[nba_source] Portal 0: Dropping ${missingClipUrl.length}/${beforeDrop} NBA games with no clip URL: ${missingGames}`
    );
    items.splice(0, items.length, ...items.filter((item) => !!item.clipUrl));
  }
  if (items.length === 0) {
    const errMsg = `Portal 0 FAIL: No NBA games have highlight clip URLs today. Run SELECT GAMES → wait for scraper to complete → retry.`;
    console.error(`[nba_source] ${errMsg}`);
    throw new Error(errMsg);
  }
  console.log(
    `[nba_source] Analyzing ${items.length} NBA highlight clip${items.length !== 1 ? 's' : ''} (dropped ${beforeDrop - items.length} with no URL)...`
  );
    ...
    items.map(async (item) => {
      ...
      try {
        ...
        if (hlVideo) {
          ...
          const freshUrl =
            src.HLS?.HD?.href || src.HLS?.href || src.HD?.href || src.mezzanine?.href;
          if (freshUrl) {
            item.clipUrl = freshUrl;
            console.log(
              `[nba-fresh-url] ✅ Refreshed clip URL for ${item.gameId} [${freshUrl.includes('.m3u8') ? 'HLS' : 'MP4'}] "${hlVideo.headline || '?'}"`
            );
          }
        }
        ...
        if (Object.keys(playersByTeam).length > 0) {
          ...
          console.log(
            `[nba-players] ✅ ${item.gameId} roster context:`,
            JSON.stringify(playersByTeam)
          );
        }
      } catch (e) {
  ...
  let analyses = await Promise.all(
    items.map((item) =>
      geminiAnalyzeClip(item.localPath || item.clipUrl || '', item.thumbnailUrl || '', 'nba', item)
    )
  );
  let nbaHits = analyses.filter((a) => a && a.length > 50).length;
  console.log(
    `[nba_source] Got ${nbaHits}/${items.length} NBA analyses (${nbaHits} video, ${items.length - nbaHits} thumbnail/fallback)`
  );

  // Portal 0: All games must have video analysis — thumbnail fallback = fabricated narration
  if (nbaHits < items.length) {
    console.warn(
      `[nba_source] Portal 0: ${nbaHits}/${items.length} video analyses — HLS URLs likely expired. Re-fetching fresh URLs and retrying...`
    );
    await Promise.all(
      items.map(async (item) => {
        if (!item.gameId) return;
        try {
          ...
          if (hlVideo2) {
            ...
            const freshUrl =
              src2.HLS?.HD?.href || src2.HLS?.href || src2.HD?.href || src2.mezzanine?.href;
            if (freshUrl) {
              item.clipUrl = freshUrl;
              console.log(
                `[gate0-retry] ✅ Fresh URL for ${item.gameId} "${hlVideo2.headline || '?'}"`
              );
            }
          }
          ...
          console.warn(`[gate0-retry] Failed to refresh ${item.gameId}: ${e.message}`);
        }
      })
    );
    ...
    console.log(`[nba_source] Portal 0 retry: Got ${nbaHits}/${items.length} NBA analyses`);
    if (nbaHits < items.length) {
      const failedGames = items
        .filter((_, i) => !analyses[i] || analyses[i].length <= 50)
        .map((g) => `${g.away} vs ${g.home}`)
        .join(', ');
      throw new Error(
        `Portal 0 FAIL: Only ${nbaHits}/${items.length} NBA clips analyzed after retry. Failed: ${failedGames}. HLS streams may be unavailable. Try again in a few minutes.`
      );
    }
  }

  // ── Portal 0 roster cross-check — scan Gemini's analysis for wrong player names ──
  ...
  // Flag the poisoned analysis so Portal 1 knows the analysis is unreliable.
  analyses = analyses.map((analysis, i) => {
    const item = items[i];
    if (!analysis || !item?.playerContext) return analysis;
    const confirmedNames = new Set(
      Object.values(item.playerContext)
        .flat()
        .map((n) => n.toLowerCase())
    );
    ...
    for (const match of [...analysis.matchAll(namePattern)]) {
      ...
      if (
        !confirmedNames.has(lower) &&
        !/quarter|half|period|game\s+\d|first|second|third|fourth|final|overtime/i.test(full)
      ) {
        const count = (
          analysis.match(new RegExp(full.replace(/[.*+?^${}()|[\]\\]/g, '\\$&'), 'g')) || []
        ).length;
    ...
    if (wrongNames.length > 0) {
      ...
      const correctedNames = Object.entries(item.playerContext)
        ...
        .join(' | ');
      console.warn(
        `[nba_source] ⚠️ GATE0_ANALYSIS_WRONG_PLAYERS: ${item.away} vs ${item.home} — Gemini used non-roster names: ${badNames}. Confirmed: ${correctedNames}`
      );
      // Append a correction notice to the analysis so Portal 1 sees it
      return (
        analysis +
  ...
  return {
    analyses,
    orderedClipUrls,
    // NBA has no clipReportDataForQA — Portal 1 QA doesn't need it
    clipReportDataForQA: null,
  };
}
