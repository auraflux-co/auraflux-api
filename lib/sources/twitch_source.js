...
async function fetchData({ items, type, jobId, geminiAnalyzeClip }, cfg) {
  ...
  // Capture clip report data for Portal 1 why-doc (pass or fail)
  ...
  if (type === 'twitch') {
    ...
    if (filteredPairs.length < itemsBefore) {
      const droppedItems = items.filter(
        (item) => !filteredPairs.find((p) => p.item.streamer === item.streamer)
      );
      const dropped = droppedItems.map(
        (item) => `${item.streamer}(${videoAnalyzedByStreamer[item.streamer] || 0} clips)`
      );
      console.warn(
        `[twitch_source] ⚠️  Dropping ${itemsBefore - filteredPairs.length} streamers with <2 real clips: ${dropped.join(', ')}`
      );
      // Classify the failure mode for each dropped streamer so the clip report is diagnostic
      droppedItems.forEach((item) => {
        const streamerClips = allClips.filter((c) => c.streamer === item.streamer);
        const gqlFailedCount = streamerClips.filter((c) => c.gqlFailed).length;
        const videoAnalyzed = videoAnalyzedByStreamer[item.streamer] || 0;
        if (streamerClips.length === 0) {
          clipFailureReasons[item.streamer] = {
            cause: 'TWITCH_API_EMPTY',
            evidence: 'No clips were provided for this streamer — Helix API may have returned empty',
          };
        } else if (gqlFailedCount > 0 && gqlFailedCount === streamerClips.length) {
          const lastErr = streamerClips.find((c) => c.gqlError)?.gqlError || 'token expired or clip deleted';
          clipFailureReasons[item.streamer] = {
            cause: 'GQL_RESOLUTION_FAILED',
            evidence: `All ${gqlFailedCount} clip(s) failed GQL CDN resolution: ${lastErr}`,
          };
        } else if (videoAnalyzed === 0) {
          clipFailureReasons[item.streamer] = {
            cause: 'GEMINI_ANALYSIS_FAILED',
            evidence: `${streamerClips.length} clip(s) found but none produced usable video analysis (no signed CDN URLs)`,
          };
        } else {
          clipFailureReasons[item.streamer] = {
            cause: 'INSUFFICIENT_CLIPS',
            evidence: `Only ${videoAnalyzed}/${MIN_CLIPS_PER_STREAMER} real video clips resolved — need ≥${MIN_CLIPS_PER_STREAMER} for long-form`,
          };
        }
      });
  ...
  // ── Portal 0 analysis cross-check — verify Gemini's analysis mentions the confirmed streamer ──
  // Detects cases where Gemini analyzed the wrong clip or produced a generic/hallucinated description
  // that ignores the actual streamer being covered. Flags poisoned analyses for Portal 1.
