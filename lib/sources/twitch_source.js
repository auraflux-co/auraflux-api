 ...
 * @param {Object} cfg - Content-type config from configLoader (unused here, reserved for future)
 *
 * @returns {Promise<{
 *   analyses: Array,
...
async function fetchData({ items, type, jobId, geminiAnalyzeClip }, cfg) {
  ...
  if (earlyClips.length > 0) {
    console.log(`[twitch_source] 📥 Early-downloading ${earlyClips.length} high-expiry clips (Maya/Emily)...`);
  ...
  console.log(`[twitch_source] Gemini analyzed ${geminiHits}/${allClips.length} clips (${allClips.length - geminiHits} fell back to thumbnail)`);
