...
/** R2 / CDN staged library clips are full MP4 — not Twitch sig= URLs. */
function isStagedLibraryVideoUrl(url) {
  if (!url || typeof url !== 'string') return false;
  const u = url.toLowerCase();
  if (u.includes('r2.cloudflarestorage.com') || u.includes('cloudflarestorage.com')) return true;
  const domain = (process.env.R2_ASSETS_DOMAIN || '').replace(/^https?:\/\//, '').replace(/\/$/, '').toLowerCase();
  if (domain && u.includes(domain)) return true;
  const folder = (process.env.R2_LIBRARY_FOLDER || 'library-staging').toLowerCase();
  return u.includes(`/${folder}/`);
}

function isResolvableVideoUrl(url) {
  return !!(url && (url.includes('sig=') || isStagedLibraryVideoUrl(url)));
}
 ...
 */
async function fetchData({ items, type, jobId, geminiAnalyzeClip, scriptVariant }, cfg) {
  ...
  console.log(`[twitch_source] Gate0 strategy: phase=${strategyPhase} passType=${strategyPassType} passNumber=${strategyPassNumber}`);

  const forceGqlRefresh =
    strategyPassType === 'sendback' || strategyPassType === 'intervention';
  const sequentialGql =
    strategyPassType === 'intervention' || strategyPassNumber >= 2;
  ...
  // Apply display names to items FIRST so clips inherit the correct displayName
  ...
  items.forEach(item => {
    ...
    clips.forEach(clip => allClips.push({
      ...
      displayName:           item.displayName,  // propagate resolved display name onto every clip
      title:                 clip.title || '',
      game:                  clip.game || '',
  ...
  async function resolveClip(clip) {
    if (!forceGqlRefresh && clip.mp4UrlDash && isResolvableVideoUrl(clip.mp4UrlDash)) {
    ...
    try {
      if (sequentialGql) {
        ...
        console.log(`[gql] ✓ ${clip.streamer} (sequential): Gemini=${resultLow.quality} Assembly=${resultHigh.quality}`);
      } else {
  ...
  if (forceGqlRefresh) {
    ...
    if (weak.length > 0) {
      console.log(`[twitch_source] Gate0 ${strategyPassType}: second GQL pass for ${weak.length} clip(s) without signed URL`);
  ...
  streamerOrder.forEach(streamer => {
    ...
    const good = streamerClips.filter(c => c.videoUrl && isResolvableVideoUrl(c.videoUrl));
    const bad  = streamerClips.filter(c => !c.videoUrl || !isResolvableVideoUrl(c.videoUrl));
  ...
  console.log(`[twitch_source] GQL resolved ${totalResolved}/${analysisClips.length} final clips with signed URLs. Analyzing with Gemini...`);
  ...
  const orderedClipUrls = analysisClips.map(c => ({
    ...
    clipUrl:         c.assemblyUrl || c.videoUrl || c.mp4UrlDash || c.url || '',
    ...
    orientation:     'landscape', // Twitch clips are 16:9 — assembly uses default crop
  ...
  if (earlyClips.length > 0) {
    console.log(`[twitch_source] 📥 Early-downloading ${earlyClips.length} clip(s)${earlyDownloadAll ? ' (intervention: all signed)' : ' (high-expiry streamers)'}`);
  ...
  for (let wi = 0; wi < waves.length; wi++) {
    ...
    const waveResults = await Promise.all(
      waves[wi].map(c => geminiAnalyzeClip(c.videoUrl, c.thumbnailUrl, 'twitch', {
        ...
        // Video-analyzed = Twitch sig= URL or library-staged R2/CDN MP4.
        const isVideoAnalyzed = !!(c.videoUrl && isResolvableVideoUrl(c.videoUrl));
  ...
  if (type === 'twitch') {
    // CPD-997: top10 countdown sends exactly 1 clip per streamer by design,
    // so the 2-clip hallucination floor would drop every item.
    const MIN_CLIPS_PER_STREAMER = scriptVariant === 'top10' ? 1 : 2;
    ...
    if (filteredPairs.length < itemsBefore) {
      ...
      console.warn(`[twitch_source] ⚠️  Dropping ${itemsBefore - filteredPairs.length} streamers with <${MIN_CLIPS_PER_STREAMER} real clips: ${dropped.join(', ')}`);
      items.splice(0, items.length, ...filteredPairs.map(p => p.item));
      analyses = filteredPairs.map(p => p.analysis);
    }
    console.log(`[twitch_source] Streamers with ≥${MIN_CLIPS_PER_STREAMER} clips: ${items.length}/${itemsBefore} — ${items.map(i => i.streamer).join(', ')}`);
  } else {
    // twitch-short: 1 clip per streamer is correct
    console.log(`[twitch_source] twitch-short: ${items.length} streamer(s), 1 clip each — skipping ≥2 clip guard`);
  }
  ...
  analyses = analyses.map((analysis, i) => {
    ...
    const nameInAnalysis = new RegExp(`\\b${displayName.replace(/[.*+?^${}()|[\]\\]/g, '\\$&')}\\b`, 'i').test(analysis) ||
      (handle && new RegExp(`\\b${handle.replace(/[.*+?^${}()|[\]\\]/g, '\\$&')}\\b`, 'i').test(analysis));
    if (!nameInAnalysis) {
      console.warn(`[twitch_source] ⚠️ GATE0_STREAMER_MISSING: Analysis for "${displayName}" does not mention the streamer — may be wrong clip or thumbnail-only analysis`);
    ...
    if (handle && handle !== displayName.toLowerCase() && new RegExp(`\\b${handle.replace(/[.*+?^${}()|[\]\\]/g, '\\$&')}\\b`, 'i').test(analysis) && !new RegExp(`\\b${displayName.replace(/[.*+?^${}()|[\]\\]/g, '\\$&')}\\b`, 'i').test(analysis)) {
      console.warn(`[twitch_source] ⚠️ GATE0_HANDLE_IN_ANALYSIS: Analysis uses handle "${handle}" instead of display name "${displayName}"`);
