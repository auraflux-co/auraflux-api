...
async function fetchData({ items, type, jobId, geminiAnalyzeClip }, cfg) {
  ...
  if (type === 'twitch') {
    ...
    if (filteredPairs.length < itemsBefore) {
      ...
      console.warn(`[twitch_source] ⚠️  Dropping ${itemsBefore - filteredPairs.length} streamers with <2 real clips: ${dropped.join(', ')}`);
      items.splice(0, items.length, ...filteredPairs.map(p => p.item));
      analyses = filteredPairs.map(p => p.analysis);
    }
    console.log(`[twitch_source] Streamers with ≥2 clips: ${items.length}/${itemsBefore} — ${items.map(i => i.streamer).join(', ')}`);
  } else {
