 ...
 * ⚠️  C0-ONLY: This source scrapes AJ Puppeteer video pool for ClipzWorld News.
 ...
 * matches to AJ Puppeteer video pool, and enforces Portal 0 (all stories must have video).
 *
 ...
 *
 * PARTIAL EXTRACTION — Portal 0 HTTP response (lines ~1554-1586) is entangled with
...
async function fetchData(
  ...
  console.log(
    `[news_source] Gate0 strategy: phase=${strategyPhase} passType=${strategyPassType} passNumber=${strategyPassNumber}`
  );
  ...
  if (type === 'news' || type === 'news-short') {
    ...
    const priorityChange = prioritized
      .map((s, i) => `${i + 1}. ${(s.title || '').slice(0, 40)}`)
      .join(', ');
  ...
  const [ogImages, scrapedVideoUrls, analysesResult] = await Promise.all([
    ...
    Promise.all(
      items.map((item) =>
        geminiAnalyzeClip(item.videoUrl || '', item.thumbnailUrl || '', 'news', item)
      )
    ),
  ]);
  ...
  if (ajVideoPool && ajVideoPool.length > 0) {
    const portraitPool = ajVideoPool.filter(
      (v) => (v.orientation || '').toLowerCase() === 'portrait'
    );
    ...
    items.forEach((item) => {
      ...
      if (match) {
        if (!ALLOWED_AJ_ARTICLE_RE.test(String(match.articleUrl || ''))) {
          console.log(
            `[news-video-match] ⏭  Rejecting non-allowed AJ path for "${(item.title || '').slice(0, 40)}": ${String(match.articleUrl || '')}`
          );
          return;
        }
        if (/\/features\//i.test(String(match.articleUrl || ''))) {
          console.log(
            `[news-video-match] ⏭  Rejecting /features/ match for "${(item.title || '').slice(0, 40)}"`
          );
          return;
        }
        ...
        console.log(
          `[news-video-match] "${(item.title || '').slice(0, 40)}" → ${match.orientation} HLS from ${match.articleUrl.slice(-60)}`
        );
      }
    });
    ...
    if (type === 'news') {
      console.log(
        `[news-video-match] ${poolHits}/${items.length} stories matched to AJ PORTRAIT pool (${portraitPool.length}/${ajVideoPool.length} pool entries portrait)`
      );
    } else {
      console.log(
        `[news-video-match] ${poolHits}/${items.length} stories matched to AJ Puppeteer pool`
      );
    }
  }
  ...
  if (
    ...
    if (unresolved.length > 0) {
      console.log(
        `[news_source] Gate0 ${strategyPassType} pass ${strategyPassNumber}: retrying video scrape for ${unresolved.length} unresolved stor${unresolved.length === 1 ? 'y' : 'ies'}`
      );
  ...
  console.log(
    `[news_source] Got ${heroHits}/${items.length} og:image URLs (hero images for TV cards)`
  );
  console.log(
    `[news_source] Got ${videoHits}/${items.length} news video URLs (Fix 9 — Al Jazeera Brightcove scrape)`
  );

  const newsHits = analyses.filter((a) => a && a.length > 50).length;
  console.log(`[news_source] Got ${newsHits}/${items.length} news analyses`);

  // ── Portal 0 analysis cross-check — verify Gemini's analysis aligns with confirmed article data ──
  // Detects cases where Gemini analyzed the wrong video, hallucinated content, or analyzed a
  // thumbnail instead of the actual article topic. Flags the poisoned analysis so Portal 1 can reject.
  analyses = analyses.map((analysis, i) => {
    ...
    if (coverageRatio < 0.2) {
      ...
      console.warn(
        `[news_source] ⚠️ GATE0_TOPIC_MISMATCH: Analysis for "${item.title.slice(0, 50)}" has only ${Math.round(coverageRatio * 100)}% title keyword coverage — possible wrong-article or thumbnail-only analysis`
      );
      return (
        analysis +
  ...
  // PHASE4_INLINE: Portal 0 HTTP response is handled inline in handleGenerateFullScript
  ...
  if (type === 'news') {
    ...
    // If a story did not map to a confirmed portrait pool hit, clear fallback URLs so Portal 0
    ...
    const actualClipCount = items.filter(
      (i) => i.videoUrl && typeof i.videoUrl === 'string'
    ).length;
    if (actualClipCount < expectedClipCount) {
      ...
      if (jobId) {
        const reason = `Portal 0: News scraper found ${actualClipCount}/${expectedClipCount} clips with confirmed video. Missing stories: ${missingStories.slice(0, 3).join(' | ')}${missingStories.length > 3 ? ` (+${missingStories.length - 3} more)` : ''}. AJ sitemap may be down or returning no US content today.`;
        try {
          await axios.post(
            `http://localhost:${process.env.PORT || 3000}/job/${jobId}/stuck`,
            {
              ...
              detail: {
                actualClipCount,
                expectedClipCount,
                missingStories: missingStories.slice(0, 5),
              },
            },
            { timeout: 5000 }
          );
    ...
    console.log(
      `[news-clip-gate] ✅ PASS — ${actualClipCount}/${expectedClipCount} stories have video, proceeding to Gemini analysis`
    );
  }
  ...
  if (type === 'news') {
    ...
    console.log(
      `[news_source] Built News orderedClipUrls: ${clipsWithUrl}/${items.length} stories have clip URLs (${items.length - clipsWithUrl} null placeholders preserved for index alignment)`
    );
  }
