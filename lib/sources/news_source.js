...
function isRedditDeskItem(item) {
  if (!item) return false;
  return !!(item.postId || item.redditPostId || item.redditSource
    || item.source === 'reddit' || item.sourceType === 'reddit'
    || /reddit\.com\/r\/[^/]+\/comments\//i.test(item.link || item.url || item.permalink || ''));
}
...
async function fetchData({
  ...
  if (items.some(isRedditDeskItem)) {
    console.log('[news_source] Reddit desk — delegating to reddit_source');
    return require('./reddit_source').fetchData({
      items,
      type,
      jobId,
      geminiAnalyzeClip,
  ...
  if (ajVideoPool && ajVideoPool.length > 0) {
    ...
    items.forEach(item => {
      // AJ pool matching only applies to AJ-sourced items.
      // Non-AJ items (BBC, AP, Reuters) already have their own video URL set by
      // the scraper — do not overwrite with an unrelated AJ portrait clip.
      const srcLower = (item.source || '').toLowerCase();
      const isAlJazeera = !srcLower || srcLower.includes('al jazeera') || srcLower === 'aljazeera';
      if (!isAlJazeera && item.videoUrl) {
        console.log(`[news-video-match] ⏭  Skipping AJ pool for non-AJ "${(item.source||'?')}" story — keeping own URL`);
        return;
      }
      ...
      if (match) {
        ...
        item.pillarboxFilter  = match.pillarboxFilter || null;
  ...
  if (type === 'news') {
    // Orientation gate: AJ clips must be portrait (product lock for long-form quality).
    // BBC/other landscape clips are allowed — assembly.js pillarboxes them automatically.
    // Rule: drop a clip only if it's landscape AND from Al Jazeera (ajVideoPool match).
    // BBC landscape clips have item.source === 'bbc' or item.sourceOrientation === 'landscape'
    // but are still usable via pillarbox filter.
    items.forEach((item) => {
      const isPortrait  = (item.sourceOrientation || '').toLowerCase() === 'portrait';
      const srcLower    = (item.source || '').toLowerCase();
      // item.source is the human-readable label set by fetchCwnNewsVideos:
      //   'Al Jazeera' | 'BBC World' | 'AP' | 'Reuters' | ''
      // Portrait gate applies ONLY to AJ clips; BBC / AP / Reuters landscape is fine (pillarboxed).
      const isAlJazeera = !srcLower || srcLower.includes('al jazeera') || srcLower === 'aljazeera';
      const isNonAj     = !isAlJazeera;

      if (!isPortrait && isAlJazeera) {
        item.videoUrl    = null;
        item.pillarboxFilter = null;
      }
      // Non-AJ landscape (BBC/AP/Reuters): clips are 1920×1080 — they fit the assembly
      // output frame directly with no pillarbox/letterbox transformation needed.
      // assembly.js standard scale path handles them via force_original_aspect_ratio=increase.
      if (isNonAj && !isPortrait && item.videoUrl) {
        item.pillarboxFilter = null;
      }
    });
  ...
  // Build orderedClipUrls for News and News-Short — one entry per story.
  // For news-short: single entry from item.videoUrl (the user-selected clip from the picker).
  // For news: one entry per story, preserving null entries for index alignment.
  let orderedClipUrls = [];
  if (type === 'news' || type === 'news-short') {
    ...
    orderedClipUrls = items.map((item, i) => {
      ...
      return {
        ...
        pillarboxFilter: item.pillarboxFilter || null,
        orientation:    item.sourceOrientation || 'portrait',
        storyIndex:     i
      };
    });
