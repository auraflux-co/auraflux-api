'use strict';
/**
 * lib/gates/gate2.js — Gate 2 (Portal 2): Render Quality
 *
...
function checkIfStuck(jobId, segmentPaths, jobStartedAtMs) {
  ...
  if (stuck) {
    ...
    console.error(msg);
    logError('GATE2_STUCK_ALARM', new Error(msg), {
      jobId,
      gate: 2,
      elapsedMs,
...
function getThresholds(jobSpec) {
  ...
  const t = getGateThresholds(customerId, templateId, 'gate2', {
...
async function run(jobSpec, segmentPaths, gate0Report, gate1Report, segmentLabels = [], opts = {}) {
  ...
  if (jobStartedAtMs) {
    ...
    if (stuckCheck.stuck) {
      logError('GATE2_STUCK', new Error('Render is STUCK'), { jobId, gate: 2, ...stuckCheck });
    }
  }
  ...
  try {
    ...
    const _sCfg = loadCustomerConfig(jobSpec?.customerId || 'c0', 'long-form');
    speakerName =
    ...
    for (const check of cdnChecks) {
      if (check.expired) {
        ...
        logError(
          ...
          { jobId, gate: 2, statusCode: check.statusCode, url: check.url }
        );
      }
    }
  }
  ...
  for (let i = 0; i < segmentPaths.length; i++) {
    ...
    if (!result.sizeOk) {
      ...
      logError('GATE2_CORRUPT_SEGMENT', new Error(reason), {
        jobId,
        gate: 2,
        segmentPath: segPath,
        fileSize,
    ...
    try {
      ...
      if (videoMeta.valid) {
        ...
        if (isBadFrameRate) {
          ...
          logError('GATE2_BAD_FRAME_RATE', new Error(reason), {
            jobId,
            gate: 2,
            segmentPath: segPath,
      ...
      if (streamMeta.valid && streamMeta.audioDuration > 0) {
        ...
        if (streamMeta.isMismatched) {
          ...
          logError('GATE2_AV_DURATION_MISMATCH', new Error(reason), {
            jobId,
            gate: 2,
            segmentPath: segPath,
            ...streamMeta,
    ...
    if (result.audioOk) {
      try {
        ...
        if (audioLevel.isSilent) {
          ...
          logError('GATE2_SILENT_SEGMENT', new Error(reason), {
            jobId,
            gate: 2,
            segmentPath: segPath,
    ...
    if (videoMeta.valid && videoMeta.duration < minSeg && videoMeta.duration > 0) {
      ...
      logError('GATE2_SHORT_SEGMENT', new Error(reason), {
        jobId,
        gate: 2,
        segmentPath: segPath,
      ...
      try {
        ...
        if (result.freezeDetected) {
          ...
          if (isLikelyStaticAvatarFalsePositive) {
            ...
            logError('GATE2_FREEZE_SOFT_WARNING', new Error(reason), {
              jobId,
              gate: 2,
              segmentPath: segPath,
            ...
            logError('GATE2_FREEZE_DETECTED', new Error(reason), {
              jobId,
              gate: 2,
              segmentPath: segPath,
    ...
    if (!result.framingOk) {
      ...
      logError('GATE2_FRAMING_MISMATCH', new Error(reason), {
        jobId,
        gate: 2,
        segmentPath: segPath,
        expectedFormat,
  ...
  if (hardFailTriggered) {
    ...
    logError('GATE2_HARD_FAIL', new Error(`Batch stopped at ${batchStoppedAt}`), {
      jobId,
      gate: 2,
      batchStoppedAt,
    });
    ...
    outcome = 'review';
    passed = false;
    logError('GATE2_REVIEW', new Error(`Score ${totalScore} — operator review needed`), {
      jobId,
      gate: 2,
      totalScore,
      deductions,
    ...
    outcome = 'hard_fail';
    passed = false;
    logError('GATE2_HARD_FAIL', new Error(`Score ${totalScore} — hard fail`), {
      jobId,
      gate: 2,
      totalScore,
      deductions,
  ...
  const downstreamHeadsUp = concerns.length > 0 ? `Gate 2 concerns: ${concerns.join('; ')}` : null;
