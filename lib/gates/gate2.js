...
function probeVideoStream(filePath) {
  return new Promise((resolve) => {
    execFile(ffprobePath(), [
      '-v', 'quiet',
      '-select_streams', 'v:0',
      '-show_entries', 'stream=codec_name,duration,width,height,r_frame_rate,avg_frame_rate',
      '-of', 'json',
      filePath
...
async function run(jobSpec, segmentPaths, gate0Report, gate1Report, segmentLabels = []) {
  ...
  // Gate 0 already treats news format mismatches as non-blocking; keep Gate 2 aligned.
  // CPD-932: short-form avatar segments render at the DELIVERY aspect (9:16 portrait),
  // while Gate 0's confirmedFormat describes the SOURCE clips (often 16:9 landscape).
  // Validate short-form segments against the ordered delivery aspect, not the source format.
  const isShortForm = contentType.includes('-short') || String(jobSpec?.templateId || '') === 'short-form';
  const expectedFormat = contentType.startsWith('news') ? null
    : isShortForm ? (jobSpec?.order?.output?.aspectRatio || '9:16')
    : expectedFormatRaw;
  ...
  for (let i = 0; i < segmentPaths.length; i++) {
    ...
    try {
      ...
      if (videoMeta.valid) {
        ...
        const isBadFrameRate = videoMeta.frameRate && (videoMeta.frameRate < 23 || videoMeta.frameRate > 61);
        if (isBadFrameRate) {
          ...
          logError('GATE2_BAD_FRAME_RATE', new Error(reason), { jobId, gate: 2, segmentPath: segPath, frameRate: videoMeta.frameRate });
          totalScore -= 20;
      ...
      if (streamMeta.valid && streamMeta.audioDuration > 0) {
        ...
        if (streamMeta.isMismatched) {
          ...
          logError('GATE2_AV_DURATION_MISMATCH', new Error(reason), { jobId, gate: 2, segmentPath: segPath, ...streamMeta });
      ...
      logError('GATE2_STREAM_PROBE_ERROR', probeErr, { jobId, gate: 2, segmentPath: segPath });
    }
      ...
      try {
        ...
        if (result.freezeDetected) {
          ...
          if (isLikelyStaticAvatarFalsePositive) {
            ...
            logError('GATE2_FREEZE_SOFT_WARNING', new Error(reason), { jobId, gate: 2, segmentPath: segPath, meanVolume: result.meanVolume });
            deductions.push({ points: -5, reason });
            totalScore -= 5;
          } else {
            const reason = `Freeze detected in segment ${i}`;
            logError('GATE2_FREEZE_DETECTED', new Error(reason), { jobId, gate: 2, segmentPath: segPath, meanVolume: result.meanVolume ?? null });
            hardFailTriggered = true;
            batchStoppedAt = segPath;
            segmentResults.push(result);
            break;
  ...
  // Hard fail and let Gate 1 sendback fix the script before burning more HeyGen credits.
  ...
  if (hasShortSegments && outcome !== 'hard_fail') {
    outcome = 'hard_fail';
    passed = false;
    logError('GATE2_SHORT_SEGMENT_HARD_FAIL', new Error('Avatar segment too short — script text was too brief'), { jobId, gate: 2, totalScore });
  } else if (hasSilentSegments && outcome !== 'hard_fail') {
    outcome = 'rerender_needed';
    passed = false;
    logError('GATE2_RERENDER_NEEDED', new Error('Silent segments detected'), { jobId, gate: 2, totalScore });
  }
