...
function getMaxDurationSecs(jobSpec) {
  const customerId = jobSpec?.customerId || 'c0';
  const templateId = jobSpec?.order?.templateId || (jobSpec?.order?.formType?.includes('short') ? 'short-form' : 'long-form');
  const thresholds = getGateThresholds(customerId, templateId, 'gate0', {});
 ...
 * by the source module before Gate 0 runs.
...
function isPageUrl(url) {
  ...
  const hasCdnToken = url.includes('sig=') || url.includes('token=') || url.includes('fastly_token');
...
function probeSource(url) {
  return new Promise((resolve, reject) => {
    const isHLS = url.includes('.m3u8') || url.includes('/hls/') || url.includes('manifest.prod') || url.includes('boltdns.net');

    // HLS manifests often return width=0 height=0 at the manifest level.
    // Pass -analyzeduration and -probesize to force ffprobe to read stream segments
    // where actual dimension data lives. Universal — works for any HLS provider.
    const args = [
      '-v', 'quiet',
      ...(isHLS ? [
        '-protocol_whitelist', 'file,http,https,tcp,tls,crypto',
        '-analyzeduration', '10000000',  // 10s analyze window
        '-probesize', '10000000'          // 10MB probe size — reads into stream segments
      ] : []),
      '-select_streams', 'v:0',
    ...
    // HLS needs more time to read segments — 60s for HLS, 30s for direct MP4
    const timeout = isHLS ? 60000 : 30000;
    execFile(ffprobePath(), args, { timeout }, (err, stdout) => {
      ...
      try {
        ...
        let format = null;
        if (width > 0 && height > 0) {
          format = width >= height ? '16:9' : '9:16';
        } else if (isHLS) {
          // HLS manifest-level probe returned no dimensions — use jobSpec output format
          // as authoritative (set by the order). This is expected for some CDN HLS streams.
          format = 'hls_no_dims';
        }
...
/**
 * Check readiness to run Gate 0 — content-type-aware capability check.
...
function canProduce(jobSpec) {
  ...
  const normalizedBase = (() => {
    ...
    if (['nba', 'sports', 'basketball', 'football'].some(t => baseType.includes(t))) return 'sports';
  ...
  if (normalizedBase === 'news') {
    ...
    try { require('puppeteer'); } catch(e) {
      console.warn('[gate0] canProduce: puppeteer not installed — AJ scraper requires it if items not pre-scraped');
    }
  }

  if (normalizedBase === 'clips') {
    // Clips/Twitch: needs GQL token + Client ID for CDN URL resolution
  ...
  // Items must be present (customer must select content before generate)
  const items = jobSpec.order?.inputs?.items;
  if (!items || !Array.isArray(items) || items.length === 0) {
    reasons.push('No items in order — customer must select content before generate');
  }
...
function commit(jobSpec) {
  const items      = jobSpec?.order?.inputs?.items || [];
  const outputFormat = jobSpec?.order?.output?.format || jobSpec?.order?.output?.aspectRatio || 'unknown';
...
async function run(jobSpec) {
  ...
  for (const item of items) {
    ...
    // Step 0: Reject web page URLs — these must be resolved to media URLs by the source
    // module before Gate 0 runs. Universal: applies to any platform's clip page URL.
    if (isPageUrl(url)) {
      const reason = `Item ${itemId} URL is a web page, not a media file: ${url}. Source module must resolve to MP4/HLS before Gate 0.`;
      logError('GATE0_PAGE_URL', new Error(reason), { jobId, gate: 0, itemId, sourceUrl: url });
      return {
        ...baseOutput,
        failReason: reason,
        confirmedSources,
    ...
    if (!isHLS && MAX_DURATION_SECS && probeResult.duration > MAX_DURATION_SECS) {
      concerns.push(`Item ${itemId}: duration ${probeResult.duration.toFixed(1)}s exceeds target max ${MAX_DURATION_SECS}s — clip may be too long for short-form split-screen (assembly will cap at ${MAX_DURATION_SECS}s)`);
    }
    ...
    if (probeResult.format === 'hls_no_dims') {
      ...
      concerns.push(`Item ${itemId}: HLS manifest has no dimension data — using jobSpec format ${fallbackFormat}`);
  ...
  // Determine whether assembly normalises source format — must be computed BEFORE
  // the mixed-format check because news intentionally mixes portrait AJ clips and
  // landscape BBC/Reuters/AP clips (assembly pillarboxes/letterboxes as needed).
  ...
  if (uniqueFormats.length === 1) {
    ...
    if (assemblyConverts) {
      // Mixed formats are expected for news (portrait AJ + landscape BBC/Reuters/AP)
      // and short-form (source clips converted to delivery format by assembly).
      // Use the ordered output format as the authoritative format for downstream gates.
      confirmedFormat = orderedFormat || uniqueFormats[0];
      concerns.push(`Mixed source formats (${uniqueFormats.join(', ')}) — assembly will normalise to ${confirmedFormat}`);
    } else {
      // For content types where source and delivery format must match, mixed = hard fail
  ...
  // When assembly converts source format, confirmedFormat must reflect the DELIVERY format.
  // - Long-form news: always 16:9 (portrait clips are pillarboxed; orderedFormat may be unset)
  // - Short-form: always 9:16 (split-screen portrait delivery)
  // - Other: use detected source format
  const longFormNewsDelivery = assemblyConverts && !isShortForm ? '16:9' : null;
  const reportedFormat = assemblyConverts
    ? (orderedFormat || longFormNewsDelivery || confirmedFormat || null)
    : (confirmedFormat || orderedFormat || null);

  return {
    gate: 0,
    jobId,
    passed: true,
    confirmedFormat: reportedFormat,
    confirmedSources,
    failReason: null,
    upstreamContext: {
      reviewedReports: [],
