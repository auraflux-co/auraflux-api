  ...
  return new Promise((resolve, reject) => {
    execFile(ffprobePath(), [
      ...
      try {
        ...
        if (streams.length === 0) return reject(new Error('No video stream found'));
        const s = streams[0];
...
function commit(jobSpec) {
  ...
  return {
    committed: `I will confirm ${items.length} source(s) resolve, video is accessible, duration ≥ ${MIN_DURATION_SECS}s, format is [${outputFormat}], and titles match the order`,
