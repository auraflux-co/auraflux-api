...
async function run(jobSpec) {
  ...
  // Gate 0 confirms source is valid — assembly handles format conversion.
  ...
  // Universal rule: if assembly converts format, Gate 0 accepts any detected format.
  ...
  // When assembly converts source format (news portrait→landscape, short-form split-screen),
  // confirmedFormat must reflect the DELIVERY format, not the source clip format.
  // Gate 3b uses confirmedFormat to validate the output commitment — if we report the source
  // format it will always mismatch the committed delivery format for these content types.
  const reportedFormat = assemblyConverts
    ? (orderedFormat || confirmedFormat || null)
