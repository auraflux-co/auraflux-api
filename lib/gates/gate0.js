    ...
    const args = [
      ...
      ...(isHLS
        ? [
            ...
            '-analyzeduration',
            '10000000', // 10s analyze window
            '-probesize',
            '10000000', // 10MB probe size — reads into stream segments
          ]
        : []),
      '-select_streams',
      'v:0',
...
async function run(jobSpec) {
  ...
  for (const item of items) {
    ...
    if (!isHLS && probeResult.duration < MIN_DURATION_SECS) {
      ...
      logError('GATE0_DURATION_SHORT', new Error(reason), {
        jobId,
        gate: 0,
        itemId,
        sourceUrl: url,
  ...
  if (!assemblyConverts && orderedFormat && confirmedFormat && orderedFormat !== confirmedFormat) {
    ...
    logError('GATE0_FORMAT_ORDER_MISMATCH', new Error(reason), {
      jobId,
      gate: 0,
      orderedFormat,
      confirmedFormat,
  ...
  const downstreamHeadsUp =
    concerns.length > 0 ? `Gate 0 concerns (non-blocking): ${concerns.join('; ')}` : null;
