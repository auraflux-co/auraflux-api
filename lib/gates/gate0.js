'use strict';
/**
 * lib/gates/gate0.js — Gate 0: Gemini Source Confirmation
 *
...
function probeSource(url) {
  return new Promise((resolve, reject) => {
    ...
    const isHLS = url.includes('.m3u8') || url.includes('akamaized.net') || url.includes('manifest');
    const args = [
      '-v', 'quiet',
...
function canProduce(jobSpec) {
  ...
  const knownTypes = ['news', 'nba', 'twitch', 'clips', 'sports'];
  const contentType = jobSpec.order?.contentType;
  if (!contentType) {
    reasons.push('jobSpec.order.contentType missing');
  } else if (!knownTypes.some(t => contentType.toLowerCase().includes(t))) {
    reasons.push(`Unknown contentType "${contentType}" — recognized types: ${knownTypes.join(', ')}`);
  }
...
function commit(jobSpec) {
  ...
  return {
    committed: `I will confirm ${items.length} source(s) resolve, video is accessible, duration ≥ ${minDuration}s, format is [${outputFormat}], and titles match the order`,
...
async function run(jobSpec) {
  ...
  if (orderedFormat && confirmedFormat && orderedFormat !== confirmedFormat) {
