...
const { isOutcomeSpoilerHook } = require('../hook_training/hook_validators');

const PASS_THRESHOLD = Number(process.env.CLIP_HOOK_QA_PASS_SCORE || 92);
...
function buildClipHookQaPrompt({ observation, hook, platformTitle, streamer, candidateMeta = null }) {
  const metaBlock = candidateMeta
    ? `\nCANDIDATE METADATA (verify alignment):\nFormula: ${candidateMeta.formula || 'n/a'}\nVisual cue: ${candidateMeta.visualCue || 'n/a'}\nPayoff tease: ${candidateMeta.payoffTease || 'n/a'}\n`
    : '';
  return `${buildChannelVoiceBlock()}

You QA ONE burned-in on-screen hook for a Twitch clip YouTube Short.
...
7. Spoils the punchline — hook gives away the full joke or outcome (must be curiosity gap only); fail "Then the goal" / "Then he won" patterns
...
11. Formulaic two-beat template with no unique anchor (e.g. "Laughter to 'pure garbage'.")
12. visualCue (if provided) does not match what observation describes for seconds 0-3
...
async function claudeClipHookQA({
  ...
  if (isOutcomeSpoilerHook(hook)) {
    return {
      passed: false,
      score: 40,
      outcome: 'hard_fail',
      violations: ['hook spoils outcome — curiosity gap violated'],
      fixDirective: 'Tease setup only. Remove outcome words (goal, won, lost, then the…).',
    };
  }
  ...
  try {
    const resp = await callClaudeAPI({
      ...
      max_tokens: 512,
      temperature: 0.1,
      messages: [{ role: 'user', content: buildClipHookQaPrompt({ observation, hook, platformTitle, streamer, candidateMeta }) }],
    });
