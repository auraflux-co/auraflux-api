...
3. FFmpeg assembly: each clip blur-padded to full-frame 9:16 portrait, stitched end-to-end. Whisper captions burned from the clips' own audio. CWN logo centered in the TOP blur fold (scaled to fit). NO avatar, NO top bar, NO flag, NO sidebar, NO ticker.
...
${isClipComp ? `- Layout: full-frame portrait clip compilation. Blurred bars above/below the footage are the INTENTIONAL blur-pad background — never a defect.
- Chrome: NONE expected except the CWN logo in the top blur fold. If you see a top bar, flag, sidebar, or ticker — THAT is a violation. Their absence is correct.
- Captions: white burned captions transcribing the clips' own audio — these are expected.
- People on screen are streamers from the source clips. There is no studio host in this format — do NOT expect or look for an avatar.` : `- Station brand (always on ticker right side): ${chromeName} — this is CORRECT branding, never flag it
...
4. Layout: full-frame portrait clip with blur-pad background throughout? CWN logo visible centered in top blur band?
5. Captions: legible, reasonably synced to the clip audio, not covering key action?
