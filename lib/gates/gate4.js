...
async function analyzeFullVideo(fileUri, jobSpec, priorGateReports, jobId, platforms) {
  ...
  const clipCompFullBleed = (compCreative?.layout?.mode || jobSpec?.designSpec?.chrome?.clipCompLayoutMode || '') === 'full_bleed_crop';
...
3. FFmpeg assembly: each clip to 9:16 portrait (${clipCompFullBleed ? 'full-bleed crop' : 'blur-pad'}), hooks burned, logo applied, stitched end-to-end. Post-process burns Whisper captions from clip audio when enabled. NO avatar, NO top bar, NO flag, NO sidebar, NO ticker.
...
${isClipComp ? `- Layout: ${clipCompFullBleed ? '9:16 full-bleed portrait — source fills frame (NO blur bands)' : '9:16 blur-pad portrait — blurred bars above/below sharp footage are INTENTIONAL'}.
- Chrome: NONE expected except CWN logo (${clipCompFullBleed ? 'top-right corner' : 'top blur fold'}) and burned overlays per preset.
- Hook: ${clipCompFullBleed && !compCreative?.hooks?.rankedList?.enabled ? 'static burned hook in UPPER-MID frame (~y 620) — must NOT sit at bottom edge' : 'burned hook per preset (mid-frame for ranked, bottom blur band for classic)'}.
...
4. Layout: ${clipCompFullBleed ? 'full-bleed 9:16 with hook upper-mid and logo top-right?' : 'blur-pad 9:16 with hook in sharp zone and logo in top blur fold?'}
