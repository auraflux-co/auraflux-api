...
async function analyzeFullVideo(fileUri, jobSpec, priorGateReports, jobId) {
  ...
  const prompt = `You are Gate 4 — the final broadcast QA director and upload authorization gate.

YOUR ROLE: You watch the COMPLETE assembled video end-to-end. Your pass is the ONLY thing that authorizes upload. If you pass this video, it goes to YouTube/TikTok/Instagram. Take that responsibility seriously.

YOUR SCOPE: Evaluate overall broadcast readiness. Prior gates checked individual pieces — you confirm the whole is ready. Do not re-check what prior gates already confirmed clean unless something looks wrong.

Take your time. Watch the entire video before forming any judgment. Note issues but be fair — minor imperfections that don't affect viewer experience should not block upload.

PRIOR GATE REPORTS (what every upstream gate confirmed — confirms carried forward, concerns to watch):
${priorContext}
...
1. Pacing: appropriate end-to-end? (no dragging, no abrupt rushes)
2. Audio: consistent volume across full runtime? no dropouts? no silence gaps?
3. Compounding issues: anything recurring that the 3-sample check at Gate 3a might have missed?
4. Content accuracy: does commentary match what's shown in source clips?
5. Brand representation: consistent with show tone and style?
6. BROADCAST READY: would you approve this for immediate upload without changes?
...
Return JSON only:
{
  ...
  "broadcastReady": boolean,
  "notes": ["issues worth operator awareness — even if not blocking"],
  "blockers": ["issues that BLOCK upload — leave empty [] if none"],
  "score": 0-100
}`;
