'use strict';
/**
 * lib/gates/gate3b.js — Gate 3b: Commitment Verification (Analytical, No LLM)
 *
 * IDENTITY: Gate 3b does NOT watch the video. It is an analyst reading reports.
 * It compares what was PROMISED in the designSpec (commitments) against what
 * Gate 3a FOUND in its 3 sample points — and determines if they match.
 *
 ...
 *
 * OUTCOME DEFINITIONS:
 *   'pass'               — All committed fields match what Gate 3a delivered. Gate 4 proceeds.
 ...
 *
 * Only 'mismatch_escalate' blocks Gate 4. 'mismatch_fixable' passes through with a note.
 *
 * Output contract:
 * {
 *   gate: '3b',
...
function canProduce(jobSpec) {
  ...
  if (!hasCommitments && !hasDesignSpec) {
    reasons.push('jobSpec.commitments is empty and jobSpec.designSpec is empty — no committed spec to verify against');
  }
...
function commit(jobSpec) {
  ...
  return {
    committed: `I will verify assembly output matches committed spec for: ${fields}. Specifically: chrome skin, logo position, output dimensions, aspect ratio, clip count, caption style, audio mix mode. I do NOT check the thumbnail — thumbnail generation runs in parallel with Gate 3a and is verified by Gate 4.`
  };
}
...
async function run(jobSpec, gate3aReport, priorGateReports) {
  ...
  if (jobSpec?.expectedSynth) {
    return {
      ...
      upstreamContext: {
        ...
        downstreamHeadsUp: 'expectedSynth/lab — Gate 3b commitment comparison skipped (product policy)'
      },
      completedAt: now()
    };
  }
  ...
  // ── Chrome skin ────────────────────────────────────────────────────────
  const committedSkin = jobSpec.commitments?.assembly?.designSpec?.chrome?.skin
  ...
  // ── Logo position ──────────────────────────────────────────────────────
  const committedLogoPos = jobSpec.commitments?.assembly?.designSpec?.chrome?.logoPosition
  ...
  const committedFormat = jobSpec.commitments?.assembly?.designSpec?.aspectRatio
    || jobSpec.order?.output?.format
    || jobSpec.order?.output?.aspectRatio
    || commitments.format;
  const gate0Format = gate0Report?.confirmedFormat;

  // CPD-932: gate0.confirmedFormat is the SOURCE format. Short-form and news
  // assemblies intentionally convert sources to the delivery format (e.g.
  // landscape Twitch clip → 9:16 split-screen short), so committed≠source is
  // expected there — only flag when source format is contractually the output.
  const _ct3b = String(jobSpec?.contentType || '').toLowerCase();
  const assemblyConverts = _ct3b.includes('-short')
    || String(jobSpec?.templateId || '') === 'short-form'
    || _ct3b.startsWith('news');
  if (!assemblyConverts && committedFormat && gate0Format && committedFormat !== gate0Format) {
    mismatches.push({
      field: 'output.format',
  ...
  // ── Clip count ─────────────────────────────────────────────────────────
  const expectedClipCount = jobSpec?.designSpec?.sceneStructure?.expectedClipCount ?? jobSpec?.designSpec?.expectedClipCount ?? commitments.expectedClipCount ?? gate1Report?.clipCount;
...
function prepare(jobSpec) {
  ...
  try {
    ...
    const logoPos = designSpec?.chrome?.logoPosition || commitments?.logoPosition || 'top-left:20:20';
    const _rawRes = jobSpec?.designSpec?.resolution || jobSpec?.order?.output?.resolution || commitments?.resolution;
    ...
    console.log(`[gate3b] Ready for job ${jobId} — checklist: skin=${skin}, logo=${logoPos}, resolution=${w}x${h}, clips=${clipCount}`);
  } catch (e) {
