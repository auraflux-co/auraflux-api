...
async function submitUpload(platform, driveUrl, metadata, corrected, pipelineJobId = null) {
  ...
  const privacyStatus = isFutureSchedule ? 'private' : (corrected.privacyStatus || metadata.privacyStatus || 'private');
...
async function run(jobSpec, gate4Report) {
    ...
    if (platform === 'youtube' && process.env.YOUTUBE_DIRECT_PUBLISH === 'true') {
      try {
        ...
        if (ytDirect.isConnected()) {
          const isFutureSchedule = metadata.scheduledAt && new Date(metadata.scheduledAt).getTime() > Date.now();
          const direct = await ytDirect.publish({
            ...
            metadata: {
              ...
              privacyStatus: isFutureSchedule ? 'private' : 'private',
              publishAt: isFutureSchedule ? metadata.scheduledAt : undefined,
            },
          ...
          platformResults.youtube = {
            ...
            nativeScheduled: isFutureSchedule,
          };
          continue;
        }
        console.log(`[gate5] ${jobId}: YOUTUBE_DIRECT_PUBLISH=true but channel not connected — using Upload-Post`);
      } catch (e) {
        // CPD-982: include the YouTube API response body — a bare status code is undiagnosable
        const ytBody = e.responseBody || e.response?.data || null;
        logError('GATE5_YT_DIRECT_FAIL', e, { jobId, gate: 5, ...(ytBody ? { responseBody: JSON.stringify(ytBody).slice(0, 1000) } : {}) });
  ...
  return {
    ...
    nativeScheduled: !!(metadata.scheduledAt && new Date(metadata.scheduledAt).getTime() > Date.now()),
    completedAt: now()
  };
}
