...
/**
 * YouTube direct upload source — local MP4 when on C0 (matches /download preview).
 * Upload-Post (TikTok/IG) still needs the public R2 URL; YouTube does not.
 */
function resolveYouTubeVideoSource(jobSpec, r2AssetsUrl) {
  ...
  const candidates = [
    ...
    jobSpec.state?.savedOutputs?.outputPath,
  ].filter(Boolean);
  try {
    const { resolveLocalPublishPath } = require('../assembly_r2_persist');
    const fromCard = resolveLocalPublishPath(jobSpec);
    if (fromCard) candidates.unshift(fromCard);
  } catch (_e) { /* non-fatal */ }
  try {
    const { findJobFinalOutputPath } = require('../assembly_card_persist');
    const p = findJobFinalOutputPath(jobSpec);
 ...
 */
async function publishYouTubeDirect(jobSpec, videoSource, metadata, corrected, jobId) {
  ...
  try {
    ...
    const containsSyntheticMedia = resolveContainsSyntheticMediaForPublish(jobSpec, {
      ...metadata,
      heygenUsed,
    });
...
function canProduce(jobSpec) {
  ...
  const isTwitchLong = ct.includes('twitch') && !jobSpec.isShort && !jobSpec.clipsOnly;
  if (isTwitchLong && !isPreGenerate) {
    try {
      const { loadBookendsConfig } = require('../twitch_bookends');
      const bookCfg = loadBookendsConfig(jobSpec.customerId || 'c0');
      if (bookCfg.outroCredits?.enabled) {
        const card = global.persistedJobsRef?.[jobSpec.jobId];
        const creditsOk = !!(
        ...
        if (!creditsOk) {
          reasons.push('Twitch Soup long-form: credits outro not appended (creditsOutroAppended required)');
        }
      }
    } catch (_) { /* non-fatal */ }
  }
...
async function run(jobSpec, gate4Report) {
  ...
  const ct = String(jobSpec.contentType || '').toLowerCase();
  if (ct.includes('twitch') && !jobSpec.isShort && !jobSpec.clipsOnly) {
    try {
      const { assertTwitchSoupPublishReady } = require('../twitch_bookends');
      const card = global.persistedJobsRef?.[jobId];
      const mp4 = jobSpec.assembledPath
        || jobSpec.state?.savedOutputs?.assembledPath
        || card?.outputPath;
      const pubReady = await assertTwitchSoupPublishReady({
        card,
        jobSpec,
        mainMp4Path: mp4,
      ...
      if (!pubReady.ready) {
        const reason = `Gate 5 blocked — Twitch Soup not publish-ready: ${pubReady.reasons.join('; ')}`;
        logError('GATE5_CREDITS_OUTRO', new Error(reason), { jobId, gate: 5 });
        try {
          pipelineBus.emit('publish:failed_validation', {
            jobId,
        ...
        try {
          recordWhyLedger({
            ...
            kind: 'publish_validation_failure',
            passed: false,
            outcome: 'credits_outro_missing',
            reasons: pubReady.reasons.slice(0, 12),
            failureClass: FAILURE_CLASS.SPEC_VIOLATION,
            interventionType: INTERVENTION.NONE,
            interventionOutcome: 'blocked',
            source: 'lib/gates/gate5:run',
        ...
        return {
          ...baseOutput,
          prePublishValidation: { passed: false, violations: pubReady.reasons },
          completedAt: now(),
        };
      }
    } catch (credErr) {
      const reason = `Gate 5 credits check failed: ${credErr.message}`;
      logError('GATE5_CREDITS_CHECK', credErr, { jobId, gate: 5 });
      return {
        ...baseOutput,
        prePublishValidation: { passed: false, violations: [reason] },
        completedAt: now(),
  ...
  // CPD-971 / duplicate guard — skip only when THIS driveUrl was already uploaded
  try {
    const { isDriveUrlAlreadyPublished } = require('../publish_dedupe');
    if (isDriveUrlAlreadyPublished(jobId, driveUrl)) {
      console.log(`[gate5] ${jobId}: driveUrl already published — skipping duplicate upload`);
      return {
        ...baseOutput,
        passed: true,
  ...
  const { applyChaptersToMetadata } = require('../youtube_chapters');
  const metadata = applyChaptersToMetadata({
    ...
    containsSyntheticMedia: resolveContainsSyntheticMediaForPublish(jobSpec, {
      contentType: jobSpec.contentType,
      heygenUsed: !!(jobSpec.heygen?.videoJobs?.length || jobSpec.addOns?.heygen?.active),
    }),
  }, jobSpec);
  ...
  for (const platform of platforms) {
    ...
    if (platform === 'youtube') {
      const ytSource = resolveYouTubeVideoSource(jobSpec, driveUrl);
      if (ytSource !== driveUrl) {
        console.log(`[gate5] ${jobId}: YouTube direct from local file (${path.basename(ytSource)})`);
      }
      platformResults.youtube = {
        attempted: true,
        ...await publishYouTubeDirect(jobSpec, ytSource, metadata, corrected, jobId),
      };
      continue;
    }
  ...
  const card = global.persistedJobsRef?.[jobId] || null;

  // CPD-1191 — Content Memory on successful publish (non-blocking)
  if (anySuccess) {
    try {
      const intelligence = require('../intelligence');
      Object.entries(platformResults).forEach(([platform, r]) => {
        if (!r.attempted || r.failed) return;
        intelligence.recordFromPublish(jobSpec, card, {
          platform: platform === 'youtube' ? 'youtube' : platform,
          platformVideoId: r.jobId || null,
          title: metadata.title,
          driveUrl,
        });
      });
    } catch (e) {
      logError('INTELLIGENCE_RECORD_PUBLISH', e, { jobId, gate: 5 });
    }
  }
  ...
  if (anySuccess) {
    ...
    try {
      const { incrementAfterPublish } = require('../episode_counters');
      episodeIncrement = incrementAfterPublish({
        jobId,
        contentType: jobSpec.contentType,
        jobSpec,
        card,
        alreadyIncremented: !!(card?.episodeCounterIncremented || jobSpec?.episodeCounterIncremented),
      });
    } catch (epErr) {
      logError('GATE5_EPISODE_INCREMENT', epErr, { jobId, gate: 5 });
    }
  }
