...
function canProduce(jobSpec) {
  ...
  if (platforms.length === 0) reasons.push('No platforms specified in jobSpec.order.publish.platforms');
...
function commit(jobSpec) {
  const platforms = jobSpec?.order?.publish?.platforms || [];
  return {
    committed: `I will deliver to [${platforms.join(', ')}] as private drafts and confirm job_id returned for each platform. Will NOT fire without gate4.uploadSignal === true.`
  };
}
...
async function run(jobSpec, gate4Report) {
  ...
  const platforms = jobSpec.order?.publish?.platforms || [];
  const metadata = {
    title: jobSpec.order?.publish?.title || jobSpec.title || '',
    description: jobSpec.order?.publish?.description || '',
    tags: jobSpec.order?.publish?.tags || [],
    categoryId: jobSpec.order?.publish?.categoryId || '24',
    privacyStatus: jobSpec.order?.publish?.privacyStatus || 'private',
    scheduledAt: jobSpec.order?.publish?.scheduledAt || null,
    caption: jobSpec.order?.publish?.caption || '',
...
module.exports = { canProduce, commit, run, validatePrePublish };
