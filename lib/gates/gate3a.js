...
async function run(jobSpec, assembledPath, priorGateReports) {
  ...
  for (const { label, startFn } of samplePoints) {
    ...
    const prompt = `You are Gate 3a — the qualitative assembly reviewer for a broadcast video production pipeline.

YOUR ROLE: You review 3 sample points (EARLY/MIDDLE/LATE) of an assembled video. You do NOT watch the full video — Gate 4 does that. Your job is to confirm this ${label.toUpperCase()} sample is production quality.

YOUR SCOPE: Check for freeze, missing clips, audio dropout, chrome visibility. Do NOT check script content, names, or editorial choices — those were Gate 1's job.

Take your time. Watch carefully before scoring. Give the benefit of the doubt on minor issues. A freeze is the only immediate hard fail.

PRIOR GATE CONTEXT (what upstream gates confirmed — watch for anything flagged):
${priorContext}

CONFIRMED DETAILS FOR THIS JOB:
...
1. FREEZE: any still frame lasting > 2 seconds? → HARD FAIL if yes
2. Source clips: visible where expected? (not just talking head the entire time)
3. Audio: continuous? any dropout or sudden silence?
4. Chrome: broadcast overlay visible? correct skin color?
...
Return JSON only:
{
  ...
  "issues": ["specific issues found — be precise"],
  "downstreamHeadsUp": "one sentence for Gate 4 to watch for, or null if clean",
  "score": 0-100
}`;
