...
function prepare(jobSpec) {
  ...
  try {
    ...
    const expectedSkin = jobSpec?.designSpec?.chrome?.skin || skin;
    const format       = jobSpec?.order?.output?.format || jobSpec?.order?.output?.aspectRatio || jobSpec?.order?.designSpec?.aspectRatio || '16:9';
    const isShort      = (jobSpec?.order?.formType || '').includes('short') || (jobSpec?.contentType || '').includes('-short');

    // Pre-calculate sample timestamps from sceneStructure so they're ready when video arrives
    // (Scaffold ran at job creation — sceneHeaders are in spec before generation begins)
    const sceneHeaders       = jobSpec?.designSpec?.sceneStructure?.sceneHeaders || [];
    const clipScenePositions = sceneHeaders
      .map((h, i) => h.toUpperCase().includes('CLIP') ? i : null)
      .filter(i => i !== null);

    // Pre-read item titles for flag/sidebar accuracy context
    const itemTitles = (jobSpec?.order?.inputs?.items || [])
      .map((it, i) => `${i+1}. ${it.title || it.displayName || it.name || 'unknown'}`)
      .join(', ');
    const categoryLabel = jobSpec?.designSpec?.voice?.categoryLabel || jobSpec?.designSpec?.chrome?.categoryLabel || null;
