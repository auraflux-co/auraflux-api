...
function _clipCompFormatDescription(jobSpec, chromeCfg) {
  ...
  const layout = fullBleed ? 'full-bleed crop (9:16)' : 'blur-pad portrait (9:16)';
  ...
  const rankedDesc = ranked
    ...
    : '';
  return `portrait clip compilation — ${layout}. Source Twitch/IRL clips stitched end-to-end. NO avatar, NO broadcast top bar, NO broadcast flag, NO broadcast sidebar, NO broadcast ticker.${logo}${rankedDesc}${fullBleed ? ' Source fills frame width (no blur bands).' : ' Blur bands above/below sharp footage are intentional.'}`;
}
