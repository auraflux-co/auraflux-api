...
- chromeCorrect=true when ranked header + countdown + hooks are present and readable`;
  ...
  if (splitScreen) {
    return `- This is a 9:16 FACECAM SPLIT portrait clip compilation — the streamer's camera fills the TOP pane (~1/3 of frame) and the screen/gameplay content fills the BOTTOM pane. Both panes show the same real-time moment.
...
- Burned hook: at the pane seam (just below the cam pane).
...
- sourceClipsVisible=true when the cam pane shows a person AND the content pane shows screen/gameplay footage.
- chromeCorrect=true when both panes are present and readable; chromeCorrect=false if either pane is black/empty or the cam pane shows no person`;
  }
...
- chromeCorrect=true when layout matches spec; hook must NOT sit in the bottom UI dead zone unless ranked preset`;
  ...
  return `- This is a 9:16 portrait clip compilation. Source clip fills frame width; blurred copies pad top/bottom — blur-pad is INTENTIONAL.
- Per spec: NO avatar, NO broadcast top bar, NO flag, NO sidebar, NO ticker.
...
- chromeCorrect=true when blur-pad layout is clean and no broadcast chrome appears`;
}
