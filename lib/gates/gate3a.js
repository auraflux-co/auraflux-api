...
const { formatGate3aExpectedChromeItems, isTwitchLongForm } = require('../gate3a_chrome_items');
 ...
 */
function _clipCompFormatDescription(jobSpec, chromeCfg) {
  const cc = jobSpec?.designSpec?.compCreative || jobSpec?.compCreative || {};
  const ranked = chromeCfg?.rankedOverlay?.enabled || cc.hooks?.rankedList?.enabled;
  const layoutMode = chromeCfg?.clipCompLayoutMode || cc.layout?.mode || 'blur_pad';
  const fullBleed = layoutMode === 'full_bleed_crop';
  const splitScreen = layoutMode === 'split_screen';
  const gameTopSplit = splitScreen && (
    cc.preset === 'fableflow_speed'
    || cc.layout?.topPaneSubject === 'gameplay'
    || cc.layout?.topPaneSubject === 'content'
  );
  const layout = splitScreen
    ? (gameTopSplit
      ? 'split_screen (9:16 — gameplay/content top, cam bottom — operator Compose)'
      : 'facecam split (9:16 — cam pane top, content pane bottom)')
    : fullBleed ? 'full-bleed crop (9:16)' : 'blur-pad portrait (9:16)';
  const logo = chromeCfg?.hasLogo === false || cc.layout?.logo === 'off'
    ? ' Logo OFF by preset.'
    : cc.layout?.logo === 'corner'
      ? ' Logo corner.'
      : ' CWN logo in top blur fold per preset.';
  const rankedDesc = ranked
    ? ` Stream Serpent ranked overlay REQUIRED: top header (RANKING {streamer} {theme}), left countdown column 1–${chromeCfg?.rankedOverlay?.slotCount || cc.hooks?.rankedList?.slotCount || 5}, active rank highlighted, burned hook text mid-frame.`
    : '';
  return `portrait clip compilation — ${layout}. Source Twitch/IRL clips stitched end-to-end. NO avatar, NO broadcast top bar, NO broadcast flag, NO broadcast sidebar, NO broadcast ticker.${logo}${rankedDesc}${splitScreen ? ' Cam pane on top and content pane below are INTENTIONAL — two synced crops of the same source clip.' : fullBleed ? ' Source fills frame width (no blur bands).' : ' Blur bands above/below sharp footage are intentional.'}`;
}

function _clipCompVisualSpecBlock(jobSpec, chromeCfg) {
  const ranked = chromeCfg?.rankedOverlay;
  const fullBleed = (chromeCfg?.clipCompLayoutMode || '') === 'full_bleed_crop';
  const splitScreen = (chromeCfg?.clipCompLayoutMode || '') === 'split_screen';
  const hasLogo = chromeCfg?.hasLogo !== false;
  const logoSize = chromeCfg?.logoSize || 220;
  const logoPosition = chromeCfg?.logoPosition || 'top-blur-fold';
  const compCreative = jobSpec?.compCreative || jobSpec?.designSpec?.compCreative || {};
  const hooksMode = compCreative?.hooks?.mode || 'both';
  const whisperOn = compCreative?.captions?.whisper !== false && hooksMode !== 'hook_only';

  if (ranked?.enabled) {
    const headerExample = `RANKING ${(ranked.streamer || 'STREAMER').toUpperCase()} ${(ranked.theme || 'MOMENTS').replace(/_/g, ' ')}`;
    return `- This is a 9:16 Stream Serpent RANKED clip comp (${fullBleed ? 'full-bleed' : 'blur-pad'}).
- REQUIRED overlay (NOT broadcast chrome violations):
  • Top-center header like "${headerExample}" — this is the ranked title, NOT a broadcast top bar
  • Left countdown column showing ranks ${ranked.slotCount} down to 1 — active rank highlighted (yellow). This is NOT the broadcast sidebar
  • Burned hook text mid-frame on each clip — correct per preset
- Logo: OFF — do NOT expect CWN logo. chromeCorrect=false ONLY if ranked header/countdown are missing or unreadable
- NO broadcast avatar, NO broadcast top bar, NO broadcast flag, NO broadcast sidebar, NO broadcast ticker from our pipeline
- sourceClipsVisible=true when Twitch streamer cam, gameplay, or IRL footage is visible — people talking on camera in source clips ARE source clips (there is NO studio avatar in this format)
- Source clip's OWN UI (chat boxes, sub timers, score tickers, cooling break graphics) belongs to the Twitch clip — ignore them; do NOT treat as broadcast chrome violations
- chromeCorrect=true when ranked header + countdown + hooks are present and readable`;
  }

  if (splitScreen) {
    const gameTop = compCreative?.preset === 'fableflow_speed'
      || compCreative?.layout?.topPaneSubject === 'gameplay'
      || compCreative?.layout?.topPaneSubject === 'content';
    const paneOrder = gameTop
      ? 'TOP pane = gameplay/screen content; BOTTOM pane = streamer camera/reaction (C9 / operator Compose — NOT an error vs C8 cam-top)'
      : 'TOP pane = streamer camera/reaction; BOTTOM pane = screen/gameplay content (C8 default)';
    return `- This is a 9:16 SPLIT_SCREEN portrait clip compilation. ${paneOrder}. Both panes are crops of the same real-time source frame.
- The horizontal seam between the two panes is INTENTIONAL — NOT a rendering artifact.
- Do NOT fail for "inverted" panes when the operator/preset orders gameplay on top — that is a valid Compose look (CPD-1293).
- Per spec: NO avatar, NO broadcast top bar, NO flag, NO sidebar, NO ticker.
- Burned hook: at the pane seam when hooks are burned (whisper_only presets may have no static hook).
- Logo: ${hasLogo ? `${logoSize}px at top-right corner` : 'none expected — logo OFF'}.
- ${whisperOn ? 'Whisper captions REQUIRED at bottom of frame (dynamic speech transcription) — separate from any static hook line.' : 'Whisper captions OFF for this preset.'}
- sourceClipsVisible=true when at least one pane shows the streamer OR gameplay/IRL footage.
- chromeCorrect=true when both panes have visible content for most of the sample; a brief black flash at the very start of a sparse VOD window is NOT an automatic fail if later samples show content. chromeCorrect=false if a pane stays solid black across the samples or Whisper is missing when required`;
  }

  if (fullBleed) {
    const animOn = !!(compCreative?.animatedText?.enabled && (compCreative.animatedText.items || []).length);
    const burnedHookOn = hooksMode !== 'whisper_only' && !ranked?.enabled;
    const hookZone = ranked?.enabled
      ? 'mid-frame below ranked header (NOT at bottom)'
      : 'upper-mid frame (~30–35% from top) — NOT at the bottom edge';
    const logoZone = ranked?.enabled ? 'bottom-right corner' : 'top-right corner';
    let hookLine;
    if (ranked?.enabled) {
      hookLine = `Burned hook: ${hookZone}.`;
    } else if (animOn && !burnedHookOn) {
      hookLine = 'Animated punch text (WATCH THIS / WAIT FOR IT style) is the intended on-screen hook language — do NOT expect a separate burned CWN hook card, and do NOT fail for missing static hook.';
    } else if (animOn && burnedHookOn) {
      hookLine = `Burned hook may appear at ${hookZone}; animated punch text may also appear — competing layers are a creative note, not an automatic chrome fail.`;
    } else if (burnedHookOn) {
      hookLine = `Burned hook: ${hookZone}.`;
    } else {
      hookLine = 'No burned hook card expected (whisper_only).';
    }
    return `- This is a 9:16 full-bleed portrait clip compilation. Source clip fills the frame width.
- Per spec: NO avatar, NO broadcast top bar, NO flag, NO sidebar, NO ticker.
- ${hookLine}
- Logo: ${hasLogo ? `${logoSize}px at ${logoZone}` : 'none expected — logo OFF'}.
- ${whisperOn
      ? 'Whisper captions expected at bottom when there is clear speech. Musical performances / singing with little dialogue may have sparse or no Whisper — do NOT hard-fail solely for missing captions on a music-dominant sample.'
      : 'Whisper captions OFF for this preset.'}
- IGNORE source-clip watermarks and end-card CTAs (@handles, "like and subscribe", platform bugs) — those are in the raw Twitch/source clip, NOT pipeline chrome violations.
- sourceClipsVisible=true when source footage fills the frame.
- chromeCorrect=true when layout matches spec; hook must NOT sit in the bottom UI dead zone unless ranked preset`;
  }

  return `- This is a 9:16 portrait clip compilation. Source clip fills frame width; ClipzWorld navy (#0d1424) pads top/bottom — brand-pad is INTENTIONAL.
- Per spec: NO avatar, NO broadcast top bar, NO flag, NO sidebar, NO ticker.
- Logo: ${hasLogo ? `${logoSize}px visible near ${logoPosition}` : 'none expected'}.
- sourceClipsVisible=true when source footage is on screen for the entire sample.
- chromeCorrect=true when brand-pad layout is clean and no broadcast chrome appears`;
}

function commit(jobSpec) {
  const chrome = jobSpec?.designSpec?.chrome || {};
  const isSplitScreen = (chrome.layout || '').includes('split');
  const isClipComp = (chrome.layout || '') === 'clip-comp';
  if (isClipComp) {
    const ranked = chrome.rankedOverlay?.enabled;
    return {
      committed: ranked
        ? `I will watch 3 sample points and confirm: freeze-free playback, source Twitch clips visible, ranked header + countdown column readable, burned hooks present, NO missing ranked overlay, NO broadcast chrome. Logo OFF for this preset.`
        : `I will watch 3 sample points and confirm: freeze-free playback, source clips visible, layout matches compCreative preset (blur-pad or full-bleed), logo per spec, NO broadcast chrome.`,
    };
  }
  return {
...
function buildGate3aGeminiSamplePrompt(jobSpec, label, ctx) {
  ...
  // Absolute video time window for this sample — used to demand verifiable timestamps
  const _fmtTs = (s) => `${Math.floor(s / 60)}:${String(Math.floor(s % 60)).padStart(2, '0')}`;
  const sampleWindow = Number.isFinite(sampleStartSec)
    ? `${_fmtTs(sampleStartSec)}–${_fmtTs(sampleStartSec + SAMPLE_DURATION)}`
    : null;
  ...
  // CPD-935: clips-only compilation — full-frame portrait clips, no avatar, no split
  const isClipComp = (chromeCfg?.layout || '') === 'clip-comp';
  ...
  const clipCompFullBleed = isClipComp && (chromeCfg?.clipCompLayoutMode || '') === 'full_bleed_crop';

  // Build a plain-language description of the assembled video from the spec
  const assembledDesc = isClipComp
    ? _clipCompFormatDescription(jobSpec, chromeCfg)
    : isSplitScreen
...
YOUR ROLE: You review this ${label.toUpperCase()} sample (${SAMPLE_DURATION}s${sampleWindow ? `, covering absolute video time ${sampleWindow}` : ''}). Gate 4 watches the full video. Your scope is technical quality at this sample point — not editorial content.

TIMESTAMP REQUIREMENT (mandatory proof of review):
- EVERY issue you report MUST include the absolute video timestamp [M:SS] where you observed it.
- Compute it as: sample window start${sampleWindow ? ` (${sampleWindow.split('–')[0]})` : ''} + seconds into this sample clip where the problem appears.
- Also include a one-phrase description of what is VISIBLY on screen at that moment (e.g. "[12:03 — avatar talking, purple sidebar visible]").
- An issue without a timestamp and on-screen description will be treated as unverified and discarded.
...
- Layout: ${isClipComp ? 'full-frame clip compilation (portrait 9:16)' : isSplitScreen ? 'split-screen (portrait 9:16)' : 'full-screen (landscape 16:9)'}
...
- CRITICAL — scene label meanings: labels like NAME_CLIP2_SETUP and NAME_CLIP2_REACTION are AVATAR COMMENTARY scenes (the host sets up or reacts to a clip). They are NOT source clip playback. Avatar on screen with flag+sidebar visible during a *_SETUP or *_REACTION scene is CORRECT per spec. The actual source clip plays BETWEEN the SETUP and REACTION scenes. Do NOT report "source clip missing" or "chrome should be hidden" during SETUP/REACTION scenes.
- EARLY sample (10%) will show: ${earlySceneLabel}${isClipComp ? ' — a source clip segment; streamer/IRL footage on screen is CORRECT (no studio avatar in clip-comp format)' : ' — avatar only here is CORRECT per spec if this is not a clip scene'}
...
- CRITICAL — chrome during source clips: During source_clip segments the FLAG and SIDEBAR are intentionally hidden. The TICKER and LOGO are ALWAYS visible throughout the entire video including during source_clip segments. Do NOT flag ticker or logo as incorrect when they appear over a source clip — this is correct per spec. Only flag chromeCorrect=false if the ticker or logo are ABSENT when they should be present, or if the wrong skin color is shown.
- CRITICAL — source-stream UI inside clips: Source clips contain the STREAMER'S OWN on-screen UI — chat overlays, sub-leaderboards, subathon timers, donation tickers, viewer counters, webcam frames. These belong to the source content, NOT the broadcast chrome. NEVER report them as wrong flag or sidebar content. If you see a leaderboard, chat box, or timer where the sidebar/flag would normally be while a clip is playing, that is the streamer's own stream UI — our chrome is intentionally hidden at that moment.
...
- Expected items ordered: ${formatGate3aExpectedChromeItems(jobSpec)}
${isTwitchLongForm(jobSpec) ? '- TWITCH SOUP CHROME: sidebar and flag show STREAMER NAMES from sceneStructure.items[].label (e.g. Cinna, Emily). Do NOT expect raw Twitch clip titles (ICANT, wow, meme text) — those are source metadata only.' : ''}
...
- Sidebar uses a SLIDING WINDOW (shows up to 5 items at a time). With ${(jobSpec?.order?.inputs?.items || []).length} total items, earlier items WILL scroll off as the episode progresses. This is CORRECT BEHAVIOR. Set sidebarAccurate=true if: (a) the active/highlighted item matches the current scene position, AND (b) the visible items are in the correct relative order from the full list above. Do NOT set sidebarAccurate=false just because some items from the beginning of the list are no longer visible.
- During source_clip segments the flag and sidebar are intentionally hidden — do NOT mark flagAccurate=false or sidebarAccurate=false simply because they are absent during a clip. Only mark them inaccurate if they ARE visible but show the WRONG content.`}
...
1. FREEZE: ${isSplitScreen
  ? `is the ENTIRE frame (both halves together) static for > 2 seconds? → HARD FAIL if yes, report exact timestamp. CRITICAL — held top half is BY DESIGN: during the clip window the TOP half (${splitTop}) intentionally holds a still frame while the BOTTOM half (${splitBottom}) plays. A static top half with a moving bottom half is CORRECT per spec — do NOT report it as a freeze. Only freezeDetected=true if BOTH halves are simultaneously frozen.`
  : 'any still frame lasting > 2 seconds? → HARD FAIL if yes, report exact timestamp'}
...
4. ${isClipComp
  ? (chromeCfg?.rankedOverlay?.enabled
    ? 'Ranked overlay: header + countdown column readable? burned hook visible? Logo should be OFF.'
    : clipCompFullBleed
      ? 'Layout: full-bleed portrait clip visible edge-to-edge?'
      : `Layout: full-frame portrait clip visible (with intentional blurred background pad above/below)? Logo (${logoSize}px) visible at ${logoPosition}?`)
  : isSplitScreen
...
${isSplitScreen ? '5. Portrait layout: top/bottom split correct? caption visible near split line?\n' : ''}${!isSplitScreen && !isClipComp ? `5. Flag accuracy: Does the top-left flag show the correct category label AND the correct active item title for this scene position?
...
${isClipComp ? _clipCompVisualSpecBlock(jobSpec, chromeCfg) : isSplitScreen ? `- This is a 9:16 portrait split-screen. Top 50% = ${splitTop}. Bottom 50% = ${splitBottom}.
...
Return JSON only:
{
  ...
  "chromeDescription": "${isClipComp ? 'describe the full-frame clip layout (blur-pad background, logo visibility, any unexpected chrome)' : isSplitScreen ? `describe portrait split layout (top=${splitTop}, bottom=${splitBottom}) and logo visibility` : 'what you actually see in the chrome elements (top bar, flag, sidebar, logo) — be specific'}",
  ${!isSplitScreen && !isClipComp ? `"flagAccurate": boolean,
  ...
  "issues": ["specific issues — each MUST start with the absolute video timestamp and what is on screen, e.g. '[12:03 — avatar talking, sidebar visible] sidebar should be hidden during source clip', then what the fix should be"],
...
async function run(jobSpec, assembledPath, priorGateReports) {
  ...
  for (const { label, startFn } of samplePoints) {
    ...
    const { prompt, meta: g3PromptMeta } = buildGate3aGeminiSamplePrompt(jobSpec, label, { ...gate3aPromptCtx, sampleStartSec: startTime });
