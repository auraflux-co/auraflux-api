...
VISUAL SPEC CHECKS (chrome is now FFmpeg drawtext/drawbox — no Puppeteer artifacts possible):
- Top bar (y=0, h=48px): left side must show episode number, center must show show name matching skin "${expectedSkin}"
...
Return JSON only:
{
  ...
  "chromeDescription": "what you actually see in the chrome elements (top bar, flag, sidebar, logo) — be specific",
...
async function prepare(jobSpec) {
  ...
  try {
    ...
    // Only runs when the job spec contains a chrome overlay config.
    if (!chrome || (!chrome.skin && !chrome.templateFile)) {
      console.log(`[gate3a] No chrome config in spec for job ${jobId} — skipping synth prebuild`);
      return;
    }

    const threshold = jobSpec?.designSpec?.chrome?.synthVerifyThreshold
      || synthHistory.DEFAULT_THRESHOLD;
    const hash      = synthHistory.chromeFingerprintHash(chrome);
    const entry     = synthHistory.getEntry(chrome);

    if (synthHistory.shouldSkipSynth(chrome, customerId, threshold)) {
      console.log(`[gate3a] Synth prebuild SKIPPED for job ${jobId} — chrome hash ${hash} verified ${entry.successCount}/${threshold} times (last: ${entry.lastVerified})`);
      if (jobSpec.designSpec) jobSpec.designSpec.synthVerified = true;
      return;
    }

    const runCount = entry?.successCount ?? 0;
    console.log(`[gate3a] Synth prebuild REQUIRED for job ${jobId} — chrome hash ${hash} verified ${runCount}/${threshold} times (first-run threshold not yet met)`);

    // Run the synth assembly for this content type
    const synthResult = await runSynthAssembly({ jobSpec, contentType, format, jobId });

    if (synthResult.ok) {
      const updated = synthHistory.recordSuccess({
        chrome,
        customerId,
        contentType,
        jobId,
      ...
      console.log(`[gate3a] ✅ Synth prebuild PASSED for job ${jobId} — chrome hash ${hash} (${newCount}/${threshold} successes)`);
      if (jobSpec.designSpec) jobSpec.designSpec.synthVerified = true;
      ...
      console.error(`[gate3a] ❌ Synth prebuild FAILED for job ${jobId} — chrome hash ${hash}: ${synthResult.error}`);
 ...
 * Returns { ok: true, outputMb } on success, { ok: false, error } on failure.
 */
async function runSynthAssembly({ jobSpec, contentType, format, jobId }) {
  ...
  const STATIC_HOST = 'http://localhost:8765/tmp/synth_prebuild';
  ...
  async function makeSynth(filename, source, size) {
    ...
    if (fs.existsSync(outPath) && fs.statSync(outPath).size > 50000) return outPath;
    await new Promise((res, rej) => {
      _execFile(FFMPEG, [
        '-f', 'lavfi', '-i', `${source}=duration=5:size=${size}:rate=30`,
        '-f', 'lavfi', '-i', 'sine=f=440:b=4:duration=5',
        '-c:v', 'libx264', '-preset', 'ultrafast', '-crf', '28',
        '-c:a', 'aac', '-b:a', '64k', '-pix_fmt', 'yuv420p',
        '-y', outPath
      ], { timeout: 20000 }, (err) => err ? rej(err) : res(outPath));
    });
    return outPath;
  }

  try {
    await makeSynth(avFile,   'testsrc',   avSize);
    await makeSynth(clipFile, 'smptebars', clipSize);
  } catch (e) {
    return { ok: false, error: `FFmpeg synth generation failed: ${e.message}` };
  }

  const avUrl   = `${STATIC_HOST}/${avFile}`;
  const clipUrl = `${STATIC_HOST}/${clipFile}`;
  ...
  for (let i = 0; i < 60; i++) {
    ...
    try {
      ...
      if (d.outputPath && fs.existsSync(d.outputPath)) {
        ...
        return { ok: true, outputMb: Math.round(outputMb * 10) / 10 };
      }
      if (d.status === 'failed' && !d.outputPath) {
        return { ok: false, error: d.error || 'Assembly failed — no output file' };
      }
