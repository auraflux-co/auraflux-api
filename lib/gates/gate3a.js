...
function buildGate3aGeminiSamplePrompt(jobSpec, label, ctx) {
  ...
  const assembledDesc = isClipComp
    ? `portrait full-frame clip compilation (9:16) — source clips blur-padded to fill the frame and stitched end-to-end. NO avatar, NO top bar, NO flag, NO sidebar, NO ticker. CWN logo centered in the TOP blur fold above the sharp footage (scaled to fit the blur band — intentional placement, not a defect). Blurred bars above/below the source footage are the intentional blur-pad background, NOT a layout break.`
    : isSplitScreen
