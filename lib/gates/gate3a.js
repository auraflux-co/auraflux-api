...
function buildGate3aGeminiSamplePrompt(jobSpec, label, ctx) {
  ...
  const assembledDesc = isClipComp
    ? (() => {
      ...
      const layout = cc.layout?.mode === 'full_bleed_crop' ? 'full-bleed crop (9:16)' : 'blur-pad portrait (9:16)';
      const ranked = cc.hooks?.rankedList?.enabled ? ' Ranked-list overlay (1–5 column) MUST be readable.' : '';
      const logo = cc.layout?.logo === 'off' ? ' Logo OFF by preset.' : cc.layout?.logo === 'corner' ? ' Logo corner.' : ' CWN logo in top blur fold or corner per preset.';
      return `portrait clip compilation — ${layout}. Source clips stitched end-to-end. NO avatar, NO top bar, NO flag, NO sidebar, NO ticker.${logo}${ranked} Blur bands (if present) are intentional, not defects.`;
    })()
    : isSplitScreen
...
${isClipComp ? `- This is a 9:16 portrait clip compilation. The source clip fills the frame width; blurred copies of the footage pad the top/bottom — this blur-pad is INTENTIONAL, never a layout break or black-bar violation.
- Per spec: NO avatar, NO top bar, NO flag, NO sidebar, NO ticker. There is no studio host in this format — people on screen are streamers from the source clips.
- Logo: ${hasLogo ? `${logoSize}px visible near ${logoPosition} of the portrait frame` : 'none expected'}.
- chromeVisible=true means the logo is visible (as specified). chromeCorrect=true means the clip fills the frame cleanly with the blur-pad background and no broadcast chrome elements.
- If you see a top bar, flag, sidebar, or ticker — those are NOT in spec. Report them as violations.
- Set sourceClipsVisible=true if source footage (gameplay, streamer cam, IRL) is on screen — in this format it should be the ENTIRE video.` : isSplitScreen ? `- This is a 9:16 portrait split-screen. Top 50% = ${splitTop}. Bottom 50% = ${splitBottom}.
