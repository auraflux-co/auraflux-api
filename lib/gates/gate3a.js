 ...
 * @param {object} ctx - precomputed context from run(): SAMPLE_DURATION, confirmedFormat, expectedSkin, isShort, clipCount, totalScenes, sceneHeaders, clipSceneIndices, earlySceneLabel, priorContext
 */
...
4. FFmpeg assembled: ${isShort
  ? 'portrait split-screen (9:16) — top half: avatar, bottom half: source clip. NO broadcast chrome (no top bar, no flag, no sidebar, no ticker). Logo only (80px, top-right).'
  : 'avatar segments + source clips + broadcast chrome overlay + ticker'}
...
${isShort ? `- Short-form layout: NO flag, NO sidebar, NO ticker. Check portrait split only.` : `- Flag should show: category "${jobSpec?.designSpec?.voice?.categoryLabel || jobSpec?.designSpec?.chrome?.categoryLabel || 'CATEGORY'}" + active item title for current scene position
- Sidebar should show: all ${(jobSpec?.order?.inputs?.items || []).length} item(s) with correct titles stacked vertically`}
${isShort && (jobSpec?.state?.savedOutputs?.filledScript || '').length > 10 ? `
CLIP CONTENT VERIFICATION (short-form only):
...
4. ${isShort
  ? 'Layout: portrait split-screen correct? Top half = avatar, bottom half = source clip? Logo visible top-right?'
  : `Chrome: broadcast overlay visible? correct skin color (${expectedSkin})?`}
${isShort ? '5. Portrait layout: top/bottom split correct? caption visible near split line?\n' : ''}${isShort ? '' : `5. Flag accuracy: Does the top-left flag show the correct category label AND the correct active item title for this scene position?
6. Sidebar accuracy: Does the sidebar show the correct item titles matching the ordered items?`}

VISUAL SPEC CHECKS${isShort ? ' (SHORT-FORM — no broadcast chrome)' : ' (chrome is now FFmpeg drawtext/drawbox — no Puppeteer artifacts possible)'}:
${isShort ? `- This is a 9:16 portrait split-screen. Top 50% = Bobby G avatar. Bottom 50% = source clip. NO top bar, NO flag, NO sidebar, NO ticker.
- Logo: small (80px) visible near top-right corner of the portrait frame.
- Caption: text captions should be visible near the split line or on the lower half.
- chromeVisible=true means the logo is visible. chromeCorrect=true means the portrait split is clean (avatar top, clip bottom) with no black bars or layout break.
- If the video shows a full-screen avatar with NO clip on screen, chromeCorrect=false and sourceClipsVisible=false.` : `- Top bar (y=0, h=48px): left side must show episode number, center must show show name matching skin "${expectedSkin}"
- Flag (top-left, x=0 y=48, w=620 h=88px): must be visible on avatar scenes — row 1 is category label, row 2 is story/streamer/game title
- Sidebar (right side, x=1468, y=120): story/streamer/game cards stacked vertically, active card highlighted with colored left border
- Logo: visible bottom-right area (x~1725 y~910)
- Flag must NOT cover Bobby G's face — flag ends at y=136, Bobby G face starts at y~120+ so only top edge may overlap briefly
- If chrome appears correct (top bar visible, flag visible, sidebar visible, logo visible): set chromeCorrect=true
- If ANY chrome element is missing or wrong: set chromeCorrect=false AND describe exactly what you see vs what was expected
...
Return JSON only:
{
  ...
  "chromeDescription": "${isShort ? 'describe portrait split layout (top=avatar, bottom=clip) and logo visibility' : 'what you actually see in the chrome elements (top bar, flag, sidebar, logo) — be specific'}",
  ${isShort ? '' : `"flagAccurate": boolean,
  ...
  ${isShort ? '"portraitSplitCorrect": boolean, "captionVisible": boolean, "clipContentMatchesScript": boolean, "clipContentDescription": "what player/team/event you actually see in the clip vs what the HOOK/REACTION script named",' : ''}
...
async function run(jobSpec, assembledPath, priorGateReports) {
  ...
  const isShort = (jobSpec?.order?.formType || '').includes('short') || (jobSpec?.contentType || '').includes('-short');
  ...
  // ── Short-form clip content semantic check ───────────────────────────────
  ...
  const isShortFormJob = (jobSpec?.order?.formType || '').includes('short') || (jobSpec?.contentType || '').includes('-short');
  if (isShortFormJob && !freezeFound) {
  ...
  if (chromeStrict && chromeCfg && !isShort && !freezeFound) {
...
async function prepare(jobSpec) {
  ...
  try {
    ...
    const format       = isShort ? 'portrait' : 'mp4';
