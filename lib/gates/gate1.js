...
const SENDBACK_THRESHOLD = 70;

// Hype words that violate flat delivery rule
const HYPE_WORDS = [
  'incredible', 'amazing', 'crazy', 'wild', 'insane', 'unbelievable',
  'mind-blowing', 'stunning', 'epic', 'legendary', 'phenomenal', 'extraordinary'
];

// Prohibited patterns: phonetic hints, mid-script CTAs
...
// Required outro lines by form type
const REQUIRED_OUTROS = {
  long: "I'm Bobby G. See you tomorrow.",
  short: "Subscribe. Appreciate you.",
  compilation: "I'm Bobby G. See you tomorrow."
};
 ...
 */
function findHypeViolations(sections) {
  const violations = [];
  for (const { header, body } of sections) {
...
function checkOutro(script, jobSpec) {
  const formType = jobSpec?.order?.formType || 'long';
  const required = REQUIRED_OUTROS[formType] || REQUIRED_OUTROS.long;
...
async function run(jobSpec, filledScript, gate0Report) {
  ...
  const hypeViolations = findHypeViolations(sections);
  ...
  // Check outro (-15 if wrong/missing)
  const outroCheck = checkOutro(script, jobSpec);
  if (!outroCheck.ok) {
    score -= 15;
  ...
  if (score > 0 && process.env.ANTHROPIC_API_KEY) {
    ...
    try {
      const qaPrompt = `You are reviewing a script for factual accuracy. The script was written about these confirmed source items:

${confirmedTitles || '(no confirmed source URLs available)'}

Script to review:
${script.substring(0, 8000)}

Check ONLY for outright fabricated facts — specific statistics, scores, dates, or events that cannot possibly be inferred from the source titles/URLs. Do NOT penalize for reasonable commentary or opinion.

Return JSON only:
{
  "fabricationFound": boolean,
  "examples": ["list of specific fabricated claims if any"]
}`;
