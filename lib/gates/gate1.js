...
/**
 * Parse Claude Gate 1 JSON — models sometimes wrap JSON in fences, add a preamble,
 * or append prose after the closing brace. Never throw; returns null on total failure.
 */
function tryParseGate1ClaudeJson(rawText) {
...
Return JSON only:
{
  ...
  "softAccuracyIssues": ["NBA/sports only — specific claims that couldn't be verified against clip analysis, each costs -10"]
      ...
      let parsed = tryParseGate1ClaudeJson(text);
      if (!parsed) {
        logError('GATE1_JSON_PARSE_FAIL', new Error('Claude Gate 1 response was not valid JSON after repair attempts'), {
        ...
        try {
          ...
          recordWhyLedger({
            ...
            reasons: ['Claude Gate 1 returned non-JSON; neutral JSON fallback applied before scoring'],
            evidenceDigest: { head: text.slice(0, 200), tail: text.slice(-100) },
            contractDigest: { showName: jobSpec?.designSpec?.voice?.showName || null },
            source: 'lib/gates/gate1.js:tryParseGate1ClaudeJson'
          });
        } catch (_e) { /* non-fatal */ }
        parsed = { ...GATE1_CLAUDE_FALLBACK };
      }
