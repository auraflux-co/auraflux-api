...
function buildConfirmedEntities(jobSpec) {
  ...
  for (const item of (jobSpec?.order?.inputs?.items || [])) {
    if (isSportsContent) {
      ...
      if (displayName) {
        authorizedFactsLines.push(`Streamer: "${displayName}" (use this exact name — NOT handle "${handle || displayName}")`);
      ...
      if (item.title) {
        const titleWords = item.title.split(/\s+/).filter(w => w.length > 4).slice(0, 5);
        confirmedArticleTitles.push({ title: item.title, source: item.source || 'unknown', keywords: titleWords });
        authorizedFactsLines.push(`Story: "${item.title}" (source: ${item.source || 'Al Jazeera'})`);
        if (item.desc) {
...
async function run(jobSpec, filledScript, gate0Report) {
  ...
  if (cTypeCheck.includes('twitch') && requiredDisplayNames.length > 0 && script) {
    for (const { wrong, correct } of requiredDisplayNames) {
      const handleOccurrences = (script.match(new RegExp(`\\b${wrong.replace(/[.*+?^${}()|[\]\\]/g, '\\$&')}\\b`, 'gi')) || []).length;
      if (handleOccurrences >= 2) {
        score -= 50;
        deductions.push({ points: -50, reason: `ENTITY_VIOLATION (deterministic): Twitch handle "${wrong}" used ${handleOccurrences}x in script — must use display name "${correct}". Customer sees the handle, not the brand name.` });
        logError('GATE1_TWITCH_HANDLE_VIOLATION', new Error(`Handle "${wrong}" instead of display name "${correct}"`), { jobId, gate: 1, wrong, correct, occurrences: handleOccurrences });
      }
    }
  }
  ...
  if (cTypeCheck.includes('news') && confirmedArticleTitles.length > 0 && script) {
    const confirmedSources = new Set(confirmedArticleTitles.map(a => (a.source || '').toLowerCase()).filter(Boolean));
    const knownNewsOrgs = ['al jazeera', 'bbc', 'cnn', 'reuters', 'ap news', 'npr', 'fox news', 'nbc news', 'abc news', 'cbs news', 'the guardian'];
    for (const org of knownNewsOrgs) {
      if (!confirmedSources.has(org) && new RegExp(`\\b${org.replace(/[.*+?^${}()|[\]\\]/g, '\\$&')}\\b`, 'i').test(script)) {
        score -= 30;
        deductions.push({ points: -30, reason: `SOURCE_VIOLATION: Script attributes content to "${org}" but confirmed sources are: ${[...confirmedSources].join(', ') || 'unknown'}. Do not fabricate source attribution.` });
        logError('GATE1_NEWS_SOURCE_VIOLATION', new Error(`Wrong source org "${org}"`), { jobId, gate: 1, wrongOrg: org, confirmedSources: [...confirmedSources] });
      }
    }
    let topicCoveredCount = 0;
    for (const article of confirmedArticleTitles) {
      const kw = article.keywords || [];
      const covered = kw.some(word => new RegExp(`\\b${word.replace(/[.*+?^${}()|[\]\\]/g, '\\$&')}\\b`, 'i').test(script));
    ...
    if (uncoveredArticles > 0 && confirmedArticleTitles.length >= 2) {
      ...
      if (uncoveredArticles > Math.floor(confirmedArticleTitles.length / 2)) {
        score -= 25;
        deductions.push({ points: -25, reason: `TOPIC_DRIFT: Script does not mention keywords from ${uncoveredArticles}/${confirmedArticleTitles.length} confirmed articles. Possible hallucination or wrong-article content.` });
        logError('GATE1_NEWS_TOPIC_DRIFT', new Error(`${uncoveredArticles} articles have zero keyword coverage in script`), { jobId, gate: 1, uncoveredArticles, totalArticles: confirmedArticleTitles.length });
      }
    }
  }
  const hypeWords = getHypeWords(jobSpec);
  ...
  if (score > 0 && process.env.GEMINI_API_KEY) {
    ...
    try {
      ...
      if (isSports && Array.isArray(parsed.softAccuracyIssues) && parsed.softAccuracyIssues.length > 0) {
        ...
        if (parsed.softAccuracyIssues.length > 0) {
          logError('GATE1_SPORTS_ACCURACY_SOFT', new Error(`${parsed.softAccuracyIssues.length} soft accuracy issues`), { jobId, gate: 1, issues: parsed.softAccuracyIssues });
        }
      }

      // HARD FAIL: roster violations when confirmed ESPN data is available
      if (isSports && Array.isArray(parsed.rosterViolations) && parsed.rosterViolations.length > 0) {
      ...
      if (cTypeCheck.includes('twitch') && Array.isArray(parsed.entityNameErrors) && parsed.entityNameErrors.length > 0) {
        for (const e of parsed.entityNameErrors.slice(0, 5)) {
          score -= 30;
          deductions.push({ points: -30, reason: `ENTITY_VIOLATION (Gemini): Script uses "${e.wrong}" instead of confirmed display name "${e.correct}".` });
        }
        logError('GATE1_TWITCH_ENTITY_VIOLATION', new Error(`${parsed.entityNameErrors.length} entity name violations`), { jobId, gate: 1, violations: parsed.entityNameErrors });
      }

      // News: source attribution violations from Gemini cross-check (-40 each — wrong source is a factual error)
      if (cTypeCheck.includes('news') && Array.isArray(parsed.sourceViolations) && parsed.sourceViolations.length > 0) {
        for (const v of parsed.sourceViolations.slice(0, 3)) {
          score -= 40;
          deductions.push({ points: -40, reason: `SOURCE_VIOLATION (Gemini): ${v}` });
        }
        logError('GATE1_NEWS_SOURCE_VIOLATION_GEMINI', new Error(`${parsed.sourceViolations.length} source violations`), { jobId, gate: 1, violations: parsed.sourceViolations });
      }
    } catch (err) {
