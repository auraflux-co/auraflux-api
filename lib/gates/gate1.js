...
async function run(jobSpec, filledScript, gate0Report) {
  ...
  if (score > 0 && process.env.ANTHROPIC_API_KEY) {
    ...
    try {
      const qaPrompt = `You are a senior script quality reviewer. Your job is to carefully and thoroughly read a broadcast script and check ONLY for outright fabricated facts.

Take your time. Read the full script carefully before forming any judgment. A slow, accurate review is far better than a fast, wrong one.

These are the confirmed source items the script was written about:
${confirmedTitles || '(no confirmed source URLs available — use extra caution before flagging fabrication)'}
