...
EXPECTED SCENES: ${expectedScenes}`;
  ...
  const qaPrompt = isShortForm ? `You are a senior QA reviewer for ClipzWorld News short-form videos.
Gemini wrote a short script (HOOK + CLIP + REACTION + CAPTION). Your job is STRUCTURE ONLY.
Take your time and read carefully before scoring. Give the script the benefit of the doubt — only fail what is clearly wrong.
You are NOT checking whether the reaction matches the clip — non-sequitur reactions are intentional.
You are NOT checking content accuracy, clip match, or fabricated content.
...
If all 4 checks pass, all four arrays are [].`
  ...
  : `You are Gate 1 — the script style QA reviewer for a broadcast video production pipeline.

YOUR ROLE: Check voice, entity names, commentary accuracy, and outro. That is your entire scope.

YOUR SCOPE — what you check:
- Voice style: flat delivery, no hype words, Jon Stewart/Norm MacDonald tone
...
- Outro: must end with the exact required closing line

YOU DO NOT CHECK: scene count, header format, structure, clip count, scene order. Those were handled before you. The script structure is system-guaranteed. Do not penalize anything structural.

Take your time. Read the full script carefully before scoring anything. A thorough, accurate review is far more valuable than a fast one. When in doubt, give the script the benefit of the doubt — false positives waste HeyGen credits and delay production.
...
EXPECTED [CLIP PLAYS HERE] COUNT: ${expectedClips}`;
  ...
  const qaPrompt = `You are a senior QA reviewer for ClipzWorld News. Claude just wrote a script. You watched the clips. Cross-check the script against what you know about each clip.

Take your time. Read the full script carefully before scoring anything. A thorough accurate review is worth far more than a fast one. Give the script the benefit of the doubt — only flag real, clear problems. False positives waste credits and delay production.
