...
// Pipeline note (2026-04): geminiQACheck / geminiSegmentQA are LEGACY — production
// assembly uses lib/gates/gate2 + lib/portals/portal3a. Kept for script QA helpers,
// tests (test/pipeline_qa.test.js), and tooling — not mounted on the hot assembly path.
...
async function geminiQACheck(videoPath, opts = {}) {
  const { contentType, avatarCount, clipCount, downloadedClipCount, expectedClips, expectedTicker, totalDuration } = opts;
  ...
  if (_contractExpected > 0 && _contractDelivered === 0) {
    ...
    return {
      score: 0, passed: false, outcome: 'fail', outcomeLabel: '❌ HARD FAIL',
      report: msg, freezeDetected: false,
      deductions: [{ points: 100, reason: `CONTRACT: ${_contractExpected} clips expected, 0 delivered` }]
    };
  }
    ...
    try {
      ...
      try {
        ...
        if (ctCfg && ctCfg.qa && ctCfg.qa.checklistModule) {
          ...
          if (gMod && gMod.geminiContext) audioNote = gMod.geminiContext;
        }
      } catch(e) {
        // Fallback: inline isNBA check
        if ((contentType || '').startsWith('nba')) {
          audioNote = `\nAUDIO NOTE — NBA content: Bobby G narrates LIVE over the highlight clip. Mixed audio (Bobby G voice + game audio playing simultaneously) is CORRECT and expected. Do NOT fail AUDIO for two simultaneous audio sources.`;
        }
      }
...
async function geminiQACheckShort(videoPath, opts = {}) {
  const { contentType, totalDuration } = opts;
  ...
  const sampleStart = Math.max(0, dur * 0.40 - 7);
  const tmpPath = path.join(TMP_DIR, `qa_short_sample_${Date.now()}.mp4`);

  let geminiFile, segReport;
  try {
    await new Promise((res, rej) => {
      const args = ['-ss', sampleStart.toFixed(0), '-i', videoPath, '-t', '15', '-c', 'copy', '-y', tmpPath];
      const proc = execFile(ffmpegPath(), args, { maxBuffer: 10 * 1024 * 1024 });
      proc.on('close', code => code === 0 ? res() : rej(new Error(`Sample extract failed: ${code}`)));
      proc.on('error', rej);
    });
    ...
    const sampleSize = fs.statSync(tmpPath).size;
    if (sampleSize < 1000) {
      return { score: 90, report: 'QA skipped — sample too small', passed: true, outcome: 'pass', outcomeLabel: '✅ PASS (skipped)', deductions: [] };
    }

    geminiFile = await waitForGeminiFile(await uploadToGeminiFiles(tmpPath));

    const qaPrompt = `You are a QA reviewer for ClipzWorld News SHORT-FORM videos (9:16 portrait, ~45 seconds).
...
2. TOP HALF (Bobby G): Is the Bobby G avatar visible in the TOP half of the frame? (yes/partial/no) — CRITICAL if no
...
5. AUDIO: Is only Bobby G's voice audible — clean, single-track, no source clip audio? (yes/partial/no) NOTE: clip audio is intentionally stripped in short-form. FAIL if you hear two simultaneous audio sources (clip audio bleed) or complete silence during spoken sections.
...
SCORING — start at 100, deduct:
  ...
  -20 if TOP HALF (Bobby G) is no (critical)
...
2. TOP HALF (Bobby G): PASS/FAIL — [what you see]
...
SUMMARY: [one sentence]`;
    ...
    segReport = (genResp.data?.candidates?.[0]?.content?.parts || []).map(p => p.text||'').join('').trim();
    ...
    try { fs.unlinkSync(tmpPath); } catch(e2) {}
    if (geminiFile) { try { await axios.delete(`https://generativelanguage.googleapis.com/v1beta/${geminiFile.name}?key=${GEMINI_APIKEY}`); } catch(e) {} }
  }
  ...
  const bobbyGFail   = /TOP HALF.*Bobby.*FAIL/i.test(segReport);
  const clipFail     = /BOTTOM HALF.*Clip.*FAIL/i.test(segReport);
  const freezeFail   = /VIDEO FREEZE:.*FAIL/i.test(segReport) && /yes/i.test((segReport.match(/VIDEO FREEZE:.*/) || [''])[0]);
  const hasCriticalFail = portraitFail || bobbyGFail || clipFail || freezeFail;

  const deductions = [];
  if (portraitFail) deductions.push({ points: 25, reason: 'PORTRAIT FORMAT: video not in 9:16 portrait — CRITICAL' });
  if (bobbyGFail)   deductions.push({ points: 20, reason: 'TOP HALF (Bobby G): avatar not visible in top half — CRITICAL' });
  if (clipFail)     deductions.push({ points: 20, reason: 'BOTTOM HALF (Clip): source clip not visible in bottom half — CRITICAL' });
  if (freezeFail)   deductions.push({ points: 25, reason: 'VIDEO FREEZE detected — CRITICAL' });
  if (avgScore < 80) deductions.push({ points: 80 - avgScore, reason: `Sample scored ${avgScore}/100 — see report for specifics` });
  ...
  const whyDoc = [
    ...
    `Bobby G top half:  ${bobbyGFail   ? '🚨 YES' : '✅ No'}`,
    ...
    `── RECOMMENDED ACTION ───────────────────────────`,
    outcome === 'pass'          ? 'Auto-proceed to Upload-Post publish.' :
    outcome === 'manual_review' ? 'Review sample report above. Approve manually or reject to re-assemble.' :
  ...
  try { fs.writeFileSync(logFile, whyDoc); console.log(`[qa] Gate 3 short why-doc saved: ${logFile}`); } catch(e) {}

  return { score: avgScore, report: whyDoc, passed, outcome, outcomeLabel, freezeDetected: freezeFail, deductions };
}
...
function parseScriptIntoScenes(script, opts = {}) {
  ...
  for (let i = 0; i < matches.length; i++) {
    ...
    const sanitizeSceneText = (raw) => String(raw || '')
    ...
    const spokenText = (sceneName && cleanText.length && !hasClipType && !(hasClipMarker && isShortScene && cleanText))
      ? require('./heygen_spoken_sanitize').sanitizeSpokenTextForScene(cleanText, sceneName)
      : require('./heygen_spoken_sanitize').collapseParentheticalPronunciationGuides(cleanText);
    ...
    if (hasClipType) {
      ...
      const hookText = require('./heygen_spoken_sanitize').sanitizeSpokenTextForScene(
        sanitizeSceneText(parts[0] || ''), sceneName
      );
      const reactionText = require('./heygen_spoken_sanitize').sanitizeSpokenTextForScene(
        sanitizeSceneText(parts.slice(1).join(' ') || ''), `${sceneName}_REACTION`
      );
      if (hookText) {
      ...
      scenes.push({ name: currentMatch.name, text: spokenText, type: 'avatar', hasClipInsert: true });
    } else if (hasClipMarker && cleanText.length === 0) {
      // [CLIP PLAYS HERE] with no spoken text — pure clip position
      scenes.push({ name: currentMatch.name, text: '', type: 'source_clip' });
    } else if (spokenText.length > 0) {
      scenes.push({ name: currentMatch.name, text: spokenText, type: 'avatar' });
    }
  }
...
async function claudeScriptQA(script, clipAnalyses, opts = {}) {
  ...
  const isNBA = isSportsContentType(contentType);
  ...
  try {
    ...
    console.warn(`[qa-gate1] checklist module load failed for ${contentType}: ${e.message} — falling back to inline defaults`);
  }

  const contextHeader = checklistMod
    ? checklistMod.contextHeader({ displayNames, clipsPerStreamer, expectedClips, expectedScenes, streamers })
    : isTwitch
...
EXPECTED SCENES: ${expectedScenes}`;
  ...
  const checklist = checklistMod
    ? checklistMod.checklist({ expectedScenes, expectedClips })
    : isShortForm ? [
   ...
   - Are there exactly ${expectedScenes} === SCENE === markers?`,
    ...
    `10. LOCKED INTRO (-15 if wrong): The === INTRO === scene MUST contain: "Welcome to Twitch Soup. I'm your host Bobby G, and for the next few minutes I'll be your guide through the world of livestreaming. Let's get into it." FAIL (-15) if the intro deviates from this wording.`,
   ...
   - Are there exactly ${expectedScenes} === SCENE === markers?`,
    ...
    `6. CLIP INTRO: Does each *_CLIP scene have a 50–80 word pre-clip callout (Bobby G speaks BEFORE the clip plays)? Must include [beat][CLIP PLAYS HERE][beat]. FAIL if under 40 words or over 100 words.`,
    `7. POST-CLIP RECAP: Does each *_NARRATION scene have 1-2 factual sentences describing what the audience just watched? No opinion — facts only. Must NOT contain [CLIP PLAYS HERE].`,
    `8. BEAT PLACEMENT: Is [beat] present before AND after every [CLIP PLAYS HERE]?`,
    `9. CLIP MATCH (most important): Does each *_CLIP callout accurately reflect what Gemini saw in the highlight? No invented plays, no generic "ball handler" vagueness — name the player and the play.`,
    `10. LOCKED INTRO (-15 if wrong): The === INTRO === scene MUST contain: "What's up, ClipzWorld! Grab your shades, because we're heading to the Other Side of the Pillow. I'm Bobby G, and we're breaking down the highlights that weren't just good—they were cold." FAIL (-15) if the intro deviates from this wording.`,
    `11. CLIP INTRO WORD COUNT (-15 if wrong): Does each *_CLIP scene have 50–80 words of spoken text (before [CLIP PLAYS HERE])? Count the words. FAIL (-15) if any *_CLIP scene is under 40 words or over 100 words.`,
    `12. OUTRO REACTION: Is each *_OUTRO exactly 1 sentence — deadpan, no recap?`,
    `13. CLIP VOICE STYLE (-15 if wrong): Does each *_CLIP scene use Stuart Scott cadence — short declarative bursts, specific player names and actions, economy of language? FAIL this item if any *_CLIP scene has any of these:
   ...
   - Are there exactly ${expectedScenes} === HEADER === markers?`,
    ...
    `9. LOCKED INTRO (-15 if wrong): The === INTRO === scene MUST contain: "I'm Bobby G, and this is Because the Light was on. I'm told this is the news." FAIL (-15) if the intro deviates from this wording.`,
  ...
  const qaPrompt = isShortForm ? `You are Gate 1 — the script style QA reviewer for short-form content in the AuraFlux production pipeline.
...
- structuralIssues: list failures with exact required text. Example: "OUTRO wrong — must end with exactly: I'm Bobby G. See you tomorrow."
...
- Output VALID JSON only — no trailing commas, no comments inside json block.`;
  ...
  if (!isShortForm && missingAppreciateYou) {
    preCheckDeductions.push({ points: 15, reason: `OUTRO: ${outroLabel} missing from script — CRITICAL` });
  ...
  const hasCriticalFail = wrongSceneCount || wrongClipCount || (!isShortForm && missingAppreciateYou) || hasWrongSceneNames || adjustedScore < 60;
  let outcome, passed;
  ...
  const whyDoc = [
    ...
    isShortForm ? `No outro required (short-form)` : `Missing sign-off: ${missingAppreciateYou ? '🚨 YES' : '✅ No'}`,
    ``,
    `── SCORE BREAKDOWN ───────────────────────────────`,
    `STARTING SCORE: 100`,
    ``,
...
async function claudeScriptFix(script, clipAnalyses, opts = {}) {
  ...
  const isNews = contentType === 'news' || contentType === 'news-short';
  const isNBA  = contentType === 'nba'  || contentType === 'nba-short' || contentType === 'sports-short';
  ...
  try {
    ...
    console.warn(`[qa-fix] checklist module load failed for ${contentType}: ${e.message} — falling back to inline defaults`);
  }
  ...
  if (fixChecklistMod) {
    ...
    sceneGuide = 'STORY#_INTRO, STORY#_SETUP, STORY#_SUMMARY, STORY#_REACTION (where # = story number 1-5)';
  } else if (isNBA) {
    ...
    sceneGuide = 'GAME#_INTRO, GAME#_NARRATION, GAME#_RECAP, GAME#_REACTION (where # = game number)';
  } else {
...
async function geminiScriptQA(script, clipAnalyses, opts = {}) {
  ...
  } else if (isSportsContentType(contentType) || contentType === 'news') {
...
EXPECTED [CLIP PLAYS HERE] COUNT: ${expectedClips}`;
  ...
  const checklist = isTwitch ? [
    ...
    `9. LOCKED INTRO (-15 if wrong): The === INTRO === scene MUST contain: "Welcome to Twitch Soup. I'm your host Bobby G, and for the next few minutes I'll be your guide through the world of livestreaming. Let's get into it." FAIL (-15) if the intro deviates from this wording.`,
    ...
    `8. LOCKED INTRO (-15 if wrong): The === INTRO === scene MUST contain: "What's up, ClipzWorld! Grab your shades, because we're heading to the Other Side of the Pillow. I'm Bobby G, and we're breaking down the highlights that weren't just good—they were cold." FAIL (-15) if the intro deviates from this wording.`,
    ...
    `8. LOCKED INTRO (-15 if wrong): The === INTRO === scene MUST contain: "I'm Bobby G, and this is Because the Light was on. I'm told this is the news." FAIL (-15) if the intro deviates from this wording.`,
...
[list]`;
  ...
  const hasCriticalFail = wrongSceneCount || wrongClipCount || (!isShortForm && missingAppreciateYou) || adjustedScore < 60;
  let outcome, passed;
  if (hasCriticalFail || adjustedScore < MANUAL_THRESHOLD) {
    outcome = 'fail'; passed = false;
  } else if (adjustedScore >= PASS_THRESHOLD) {
  ...
  return new Promise((resolve) => {
    execFile(ffprobePath(), [
      ...
      try {
        ...
        if (streams.length === 0) return resolve({ hasAudio: false, durationSecs: 0, bitrate: 0 });
        const s = streams[0];
        resolve({
...
async function geminiSegmentQA(segmentPaths, opts = {}) {
  ...
  for (const segPath of toCheck) {
    ...
    try {
      ...
      console.log(`[qa-gate2] ${label} segment ffprobe: hasAudio=${probe.hasAudio} codec=${probe.codec||'none'} dur=${probe.durationSecs}s bitrate=${probe.bitrate}`);
...
ISSUES: <comma-separated list of issues, or "None">`;
      ...
      let segReport = (genResp.data?.candidates?.[0]?.content?.parts || []).map(p => p.text||'').join('').trim();
      ...
      const geminiSaysNoAudio = /LIP SYNC:.*\[?FAIL/i.test(segReport) || /AUDIO QUALITY:.*\[?FAIL/i.test(segReport) || /AUDIO:.*\[?FAIL/i.test(segReport);
      let claudeCorrected = false;
      let claudeCorrectionNote = '';
      if (geminiSaysNoAudio && probe.hasAudio) {
        console.log(`[qa-gate2] ${label}: Gemini flagged audio/lip-sync fail but ffprobe confirms audio present — asking Claude to correct`);
        try {
...
Format your response exactly like the original Gemini report but with corrections applied.`
          ...
          const claudeCorrection = (correctionResp.content || []).map(b => b.text || '').join('').trim();
          if (claudeCorrection && claudeCorrection.includes('CORRECTED: YES')) {
            ...
            console.log(`[qa-gate2] ${label}: Claude correction applied — Gemini audio false positive overridden`);
          }
        } catch(e) {
          console.warn(`[qa-gate2] ${label}: Claude correction failed (${e.message}) — falling back to ffprobe override`);
          ...
          segReport = segReport
            .replace(/1\.\s*LIP SYNC:\s*\[?FAIL\]?/i, '1. LIP SYNC: [PASS] (ffprobe confirmed audio present)')
            .replace(/2\.\s*AUDIO QUALITY:\s*\[?FAIL\]?/i, '2. AUDIO QUALITY: [PASS] (ffprobe confirmed audio present)');
        }
      }
      ...
      else if (!claudeCorrected && (/AUDIO QUALITY:.*\[?FAIL/i.test(segReport) || /AUDIO:.*\[?FAIL/i.test(segReport)))  {
        audioMissing = true;
        scores.push(30);
  ...
  if (!scores.length) {
    ...
    const outPath = p.join(p.dirname(__dirname), 'output', 'qa_failures', `gate2_segments_fail_${Date.now()}.txt`);
    const whyDoc = [
    ...
    try { fs.writeFileSync(outPath, whyDoc); } catch(e) {}
    return { score: 0, passed: false, outcome: 'fail', outcomeLabel: '❌ HARD FAIL',
      hasCriticalFail: true, lipSyncFail: false, audioMissing: false,
      deductions: [{ points: 100, reason: 'No avatar segments sampled — cannot pass Gate 2 without reviewing segments' }],
      reports: [], jobId, segmentsChecked: 0, whyDocPath: outPath };
  }
...
async function callClaudeAPI(params) {
  ...
  try {
    ...
    const promptText = typeof firstMsg?.content === 'string'
      ? firstMsg.content
...
function autoAction(gate, score, context = {}) {
  ...
  if (gate === 1) {
    ...
    const ct = String(context.contentType || '');
    const isTwitchLong = ct.includes('twitch') && !ct.includes('-short');
    if (isTwitchLong && (context.gate1Outcome === 'pass' || context.gate1Outcome === 'sendback') && context.gate1Passed) {
      return { action: 'proceed', directive: 'AUTO_PROCEED_TO_HEYGEN', reason: 'Gate 1 passed (held)' };
    }
