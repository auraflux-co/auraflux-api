...
async function geminiQACheck(videoPath, opts = {}) {
  ...
  // If Gate 1 said N clips were expected and assembly delivered 0, this is a hard fail.
  ...
  if (_contractExpected > 0 && _contractDelivered === 0) {
    const msg = `Pipeline contract violated: Gate 1 expected ${_contractExpected} source clips but 0 were delivered to the assembled video. This is a hard fail — Gemini QA skipped.`;
...
SUMMARY: [one sentence. Either "No issues found — video looks clean." or describe the main problem.]`;
  ...
  try {
    ...
    console.log(`[qa] Gate 3 why-doc saved: ${logFile}`);
  } catch (e) {}
...
SUMMARY: [one sentence]`;
  ...
  try {
    fs.writeFileSync(logFile, whyDoc);
    console.log(`[qa] Gate 3 short why-doc saved: ${logFile}`);
  } catch (e) {}
...
EXPECTED SCENES: ${expectedScenes}`;
  ...
  const qaPrompt = isShortForm
    ? `You are Gate 1 — the script style QA reviewer for short-form content in the AuraFlux production pipeline.
...
If all 4 checks pass, all four arrays are [].`
    : `You are Gate 1 — the script style QA reviewer for the AuraFlux broadcast video production pipeline.
...
- Output VALID JSON only — no trailing commas, no comments inside json block.`;
  ...
  return {
    ...
    fixDirective,
    tokenUsage,
    // Pipeline contract: carry expectedClips forward so Gates 2, assembly, and Gate 3
    // can assert clips were actually delivered. Never undefined after Gate 1.
    expectedClips,
  };
}
...
EXPECTED [CLIP PLAYS HERE] COUNT: ${expectedClips}`;
  ...
  const qaPrompt = `You are Gate 1 (Gemini) — the script quality reviewer for the AuraFlux broadcast video production pipeline.

── HOW THIS WORK WAS PRODUCED ────────────────────────────────────────────────
1. Gate 0 confirmed source videos are valid and accessible
...
// Used by Gate 2 to override Gemini false positives (Gemini sometimes reports "no audio"
...
Format your response exactly like the original Gemini report but with corrections applied.`,
  ...
  if (!scores.length) {
    ...
    const whyDoc = [
      ...
      `Outcome:    HARD FAIL`,
      ``,
      `REASON: No avatar segments were available to sample. Gate 2 cannot pass without`,
    ...
    return {
      ...
      deductions: [
        {
          points: 100,
          reason: 'No avatar segments sampled — cannot pass Gate 2 without reviewing segments',
        },
      ],
      reports: [],
      jobId,
...
function autoAction(gate, score, context = {}) {
  ...
  // Must respect that — do not regenerate when Gate 1 already marked passed (score can be <90).
  if (gate === 1) {
    ...
    if (retryCount < 3) {
      return {
        ...
        reason: `Gate 1 score ${score} — auto-retry ${retryCount + 1}/3`,
      };
    }
    ...
    return {
      ...
      reason: `Gate 1 score ${score} after 3 retries — manual review required`,
    };
  }
  ...
  if (gate === 2) {
    ...
    if (score >= 65 && score < 85) {
      return {
        ...
        directive: 'MANUAL_REVIEW',
        reason: 'Gate 2 score 65-84 — continue assembly with warning',
      };
    }
    ...
    return {
      ...
      reason: 'Gate 2 score <65 — re-submit failed segments only',
    };
  }
  ...
  if (gate === 3) {
    ...
    if (score >= 60 && score < 70) {
      return {
        action: 'proceed_with_warning',
        directive: 'MANUAL_REVIEW',
        reason: 'Gate 3 score 60-69 — upload anyway, flag for review',
      };
    }
    ...
    if (missingClips && missingClips.length > 0) {
      return {
        ...
        reason: `Gate 3 score <60 — missing clips detected: ${missingClips.join(', ')}`,
      };
    }
    ...
    return {
      ...
      reason: 'Gate 3 score <60 — rollback to all_sent stage',
    };
  }
