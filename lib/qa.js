...
EXPECTED SCENES: ${expectedScenes}`;
  ...
  const checklist = isTwitch ? [
   ...
   - Are there exactly ${expectedScenes} === HEADER === markers?${isShortForm ? ' (Short-form: expect fewer scenes - typically 3-4 total)' : ''}`,
    `2. CLIP COUNT: Are there exactly ${expectedClips} === STORY#_CLIP === markers (one per story) with empty spokenText?${isShortForm ? ' (Short-form: MUST be exactly 1 clip)' : ''}`,
    `3. OUTRO: Does the script end with "Appreciate you!"?`,
    ...
    `8. STORY MATCH (most important): Does each story's narration accurately reflect the clip analysis? Check STORY#_INTRO and STORY#_SUMMARY against the clip content.`,
...
async function claudeScriptFix(script, clipAnalyses, opts = {}) {
  ...
  const sceneGuide = isNews
    ...
    : isNBA
    ? 'GAME#_INTRO, GAME#_NARRATION, GAME#_REACTION (where # = game number)'
