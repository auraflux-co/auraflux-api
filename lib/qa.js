...
function parseScriptIntoScenes(script) {
  ...
  for (let i = 0; i < matches.length; i++) {
    ...
    // Detect source_clip scenes BEFORE stripping markers
    const hasClipMarker  = /\[CLIP PLAYS HERE\]/i.test(text);
    const hasClipType    = /type:\s*source_clip/i.test(text);
    const isSourceClip   = hasClipMarker || hasClipType;

    // Clean up markers from text
    text = text.replace(/\[beat\]/g, '').trim();
    text = text.replace(/\[CLIP PLAYS HERE\]/g, '').trim();
    text = text.replace(/^type:\s*(source_clip|avatar)\s*$/gim, '').trim();
    text = text.replace(/^spokenText:\s*/gim, '').trim();

    // Always include source_clip scenes (they have no spoken text by design)
    // Only skip non-clip scenes that are truly empty
    if (isSourceClip) {
      scenes.push({
