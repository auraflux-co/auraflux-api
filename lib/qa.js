...
function parseScriptIntoScenes(script) {
  ...
  for (let i = 0; i < matches.length; i++) {
    ...
    text = text.replace(/type:\s*avatar/gi, '').trim();
...
EXPECTED SCENES: ${expectedScenes}`;
  ...
  const checklist = checklistMod
   ...
   - Are there exactly ${expectedScenes} === SCENE === markers?`,
    ...
    `10. LOCKED INTRO (-15 if wrong): The === INTRO === scene MUST contain this EXACT text: "Welcome to Twitch Soup. I'm your host Bobby G, and for the next few minutes, I'll be your guide through the digital dumpster fire that is livestreaming. Let's jump right into it." FAIL (-15) if the intro deviates from this wording.`,
  ...
  const qaPrompt = isShortForm ? `You are a QA reviewer for ClipzWorld News short-form videos.
...
If all 4 checks pass, all four arrays are [].`
  ...
  : `You are a QA reviewer for ClipzWorld News. Gemini just wrote a script for a video that will be generated using HeyGen's Bobby G avatar. You watched the clips. Cross-check the script against what you know about each clip.
