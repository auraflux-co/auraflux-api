...
async function geminiQACheck(videoPath, opts = {}) {
  ...
  try {
    ...
    const _sCfg = loadCustomerConfig(customerId, 'long-form');
    speakerName =
  ...
  if (!fs.existsSync(videoPath))
    return { score: 0, report: 'QA failed — video file not found', passed: false };

  // ── Pipeline contract hard check — runs before Gemini, costs nothing ──────────
  // If Portal 1 said N clips were expected and assembly delivered 0, this is a hard fail.
  // Gemini cannot catch this reliably by watching a short sample. We assert it here.
  const _contractExpected = expectedClips || 0;
  const _contractDelivered = downloadedClipCount || clipCount || 0;
  if (_contractExpected > 0 && _contractDelivered === 0) {
    const msg = `Pipeline contract violated: Portal 1 expected ${_contractExpected} source clips but 0 were delivered to the assembled video. This is a hard fail — Gemini QA skipped.`;
    console.error(`[gate3] ❌ CONTRACT FAIL — ${msg}`);
    return {
      score: 0,
      passed: false,
      outcome: 'fail',
      outcomeLabel: '❌ HARD FAIL',
      report: msg,
      freezeDetected: false,
      deductions: [
        { points: 100, reason: `CONTRACT: ${_contractExpected} clips expected, 0 delivered` },
      ],
    };
  }
  ...
  // SHORT-FORM: single portrait split-screen check (not 3-point EARLY/MIDDLE/LATE)
  if (isShortForm) {
    return geminiQACheckShort(videoPath, opts);
  }
  ...
  const samplePoints = [
    { label: 'EARLY', start: Math.max(0, dur * 0.1 - 10) },
    { label: 'MIDDLE', start: Math.max(0, dur * 0.5 - 10) },
  ...
  for (const point of samplePoints) {
    ...
    try {
      await new Promise((res, rej) => {
        ...
        proc.on('close', (code) =>
          code === 0 ? res() : rej(new Error(`Sample extract failed: ${code}`))
        );
        proc.on('error', rej);
      });
      ...
      const checklist =
        point.label === 'EARLY'
          ? [
              ...
              `6. AVATAR FRAMING: Is ${speakerName} fully visible and properly centered in the frame? (yes/partial/no)`,
            ]
          : point.label === 'MIDDLE'
            ? [
                ...
                `4. AVATAR VISIBLE: ${speakerName} clearly visible and properly framed? (yes/no)`,
                `5. AUDIO: Audio clear and continuous? (yes/partial/no)`,
                ...((downloadedClipCount ?? clipCount) > 0
                  ? [
                      `6. SOURCE CLIPS: Are source clips (non-avatar footage) visible and playing? (yes/no)`,
                    ]
                  : []),
              ]
            : [
                ...
                `3. OUTRO: Does the OUTRO scene play cleanly — avatar visible, "Goodnight and good luck." audible? (yes/no) — NOTE: this 20-second sample window may end BEFORE the video ends; if the sample cuts off mid-sentence that is a sample-window boundary, NOT an OUTRO failure. Only mark FAIL if the OUTRO scene itself is broken (freeze, missing avatar, audio dropout).`,
                `4. AUDIO: Audio clear through to the end of this sample? (yes/partial/no)`,
              ];

      // geminiContext: extra audio/context note from the pluggable checklist module.
      // For NBA, this warns Gemini that mixed audio is correct (Bobby G narrates live).
      // Falls back to isNBA check if module is unavailable.
      let audioNote = '';
      try {
        const { getContentTypeConfig } = require('./configLoader');
        const ctCfg = getContentTypeConfig(contentType);
        if (ctCfg && ctCfg.qa && ctCfg.qa.checklistModule) {
          const gMod = require(path.join(__dirname, ctCfg.qa.checklistModule));
          if (gMod && gMod.getGeminiContext) audioNote = gMod.getGeminiContext(customerId);
          else if (gMod && gMod.geminiContext) audioNote = gMod.geminiContext;
        }
      } catch (e) {
        // Fallback: inline isNBA check
        if ((contentType || '').startsWith('nba')) {
...
Context: ${avatarCount} avatar segments, ${clipCount} source clips requested, ${downloadedClipCount ?? clipCount} downloaded.${audioNote}
...
SUMMARY: [one sentence. Either "No issues found — video looks clean." or describe the main problem.]`;
      ...
      if (!hasFailures && segScore < 70) {
        console.log(
          `[qa-gate3] ${point.label} sample - All checks passed but score is ${segScore}, raising to 70`
        );
        segScore = 70;
      }
  ...
  // ── Portal 3: Assembly QA thresholds ──────────────────────────────
  ...
  const tickerMissing =
    reports.filter((r) => /TICKER:\s*(FAIL|no\b)/i.test(r) && !/TICKER:\s*PASS/i.test(r)).length ===
    reports.length;
  ...
  const outroCutOff =
    /OUTRO:.*FAIL/i.test(lateReport) && !/abrupt.*cut|cut.*abrupt|sample.*end/i.test(lateReport);
  ...
  const clipsExpectedButMissing =
    (clipCount > 0 && effectiveClipCount === 0) ||
    (effectiveClipCount > 0 &&
      /SOURCE CLIPS:\s*(FAIL|no\b)/i.test(fullReport) &&
      !/SOURCE CLIPS:\s*PASS/i.test(fullReport));
  const hasCriticalFail =
    freezeDetected || tickerMissing || outroCutOff || avDeSync || clipsExpectedButMissing;

  // Build structured deduction list for why-doc
  const deductions = [];
  if (freezeDetected)
    deductions.push({ points: 30, reason: 'VIDEO FREEZE detected — critical failure' });
  if (tickerMissing)
    deductions.push({
      points: 20,
      reason: 'TICKER missing from all sample points — critical failure',
    });
  if (outroCutOff)
    deductions.push({
      points: 20,
      reason: 'OUTRO cut off — "Appreciate you!" not present in late sample',
    });
  if (avDeSync) deductions.push({ points: 15, reason: 'A/V DESYNC detected in sample' });
  if (clipsExpectedButMissing)
    deductions.push({
      points: 25,
      reason: 'SOURCE CLIPS missing — expected clips but none detected in video',
    });
  scores.forEach((s, i) => {
    if (s < 80)
      deductions.push({
        points: 80 - s,
        reason: `${samplePoints[i].label} sample scored ${s}/100 — see report for specifics`,
      });
  });
  ...
  const whyDoc = [
    ...
    `── RECOMMENDED ACTION ───────────────────────────`,
    outcome === 'pass'
      ? 'Auto-proceed to Upload-Post publish.'
      : outcome === 'manual_review'
        ? 'Review sample reports above. Approve manually in dashboard to proceed, or reject to re-assemble.'
  ...
  try {
    fs.writeFileSync(logFile, whyDoc);
    console.log(`[qa] Portal 3 why-doc saved: ${logFile}`);
  } catch (e) {}
...
// ─── GATE 3 — SHORT-FORM: Portrait split-screen check ─────────────────────────
// Replaces the long-form EARLY/MIDDLE/LATE 3-sample checklist.
// Checks: portrait 9:16, Bobby G top half, clip bottom half, caption visible, audio, freeze.
async function geminiQACheckShort(videoPath, opts = {}) {
  const { contentType, totalDuration, customerId = 'c0' } = opts;
  let speakerName = 'the host';
  try {
    const { loadCustomerConfig } = require('./customerConfig');
    const _sCfg = loadCustomerConfig(customerId, 'short-form');
    speakerName =
      _sCfg?.voice?.speakerName || _sCfg?.designDefaults?.voice?.speakerName || speakerName;
  } catch (_e) {}
  const dur = totalDuration || 45;

  const PASS_THRESHOLD = opts.passThreshold || 70;
  const MANUAL_THRESHOLD = opts.manualThreshold || 60;

  // Extract a 15s clip from the middle (where Bobby G reacts over clip — most likely to expose layout issues)
  const sampleStart = Math.max(0, dur * 0.4 - 7);
  const tmpPath = path.join(TMP_DIR, `qa_short_sample_${Date.now()}.mp4`);

  let geminiFile, segReport;
  try {
    await new Promise((res, rej) => {
      const args = [
        '-ss',
        sampleStart.toFixed(0),
        '-i',
        videoPath,
        '-t',
        '15',
    ...
    if (sampleSize < 1000) {
      return {
        score: 90,
        report: 'QA skipped — sample too small',
        passed: true,
        outcome: 'pass',
    ...
    geminiFile = await waitForGeminiFile(await uploadToGeminiFiles(tmpPath));

    const qaPrompt = `You are a QA reviewer for short-form videos (9:16 portrait, ~45 seconds).

Watch this 15-second clip from the middle of a short-form episode and check these 6 items:

CHECKLIST:
1. PORTRAIT FORMAT: Is the video in 9:16 portrait orientation (tall, not wide)? (yes/no) — CRITICAL if no
2. TOP HALF (${speakerName}): Is the ${speakerName} avatar visible in the TOP half of the frame? (yes/partial/no) — CRITICAL if no
3. BOTTOM HALF (Clip): Is the source clip (streamer/highlight footage) playing in the BOTTOM half? (yes/partial/no) — CRITICAL if no
4. CAPTION: Is a text caption/overlay visible and readable anywhere on screen? (yes/no)
5. AUDIO: Is only ${speakerName}'s voice audible — clean, single-track, no source clip audio? (yes/partial/no) NOTE: clip audio is intentionally stripped in short-form. FAIL if you hear two simultaneous audio sources (clip audio bleed) or complete silence during spoken sections.
6. VIDEO FREEZE: Does either half of the screen appear frozen while the other plays? (yes/no) — CRITICAL if yes

SCORING — start at 100, deduct:
  -25 if PORTRAIT FORMAT is no (critical)
  -20 if TOP HALF (${speakerName}) is no (critical)
  -20 if BOTTOM HALF (Clip) is no (critical)
  -15 if CAPTION is no
  -15 if AUDIO is partial or no (clip audio bleed = FAIL)
  -25 if VIDEO FREEZE is yes (critical)

REQUIRED FORMAT:
CHECKLIST RESULTS:
1. PORTRAIT FORMAT: PASS/FAIL — [what you see]
2. TOP HALF (${speakerName}): PASS/FAIL — [what you see]
3. BOTTOM HALF (Clip): PASS/FAIL — [what you see]
4. CAPTION: PASS/FAIL — [what you see]
5. AUDIO: PASS/FAIL — [what you see]
6. VIDEO FREEZE: PASS/FAIL — [what you see]

DEDUCTIONS: [list deductions or "None"]
SCORE: [0-100]
SUMMARY: [one sentence]`;
    ...
    segReport = (genResp.data?.candidates?.[0]?.content?.parts || [])
      .map((p) => p.text || '')
      .join('')
      .trim();
    console.log(`[qa-gate3-short] Raw Gemini response:\n${segReport}\n---`);
  } catch (e) {
    console.error(`[qa-gate3-short] Sample check failed: ${e.message}`);
    segReport = 'Sample check failed — auto-pass';
  } finally {
    try {
      fs.unlinkSync(tmpPath);
    } catch (e2) {}
  ...
  // Parse score
  let avgScore = parseInt((segReport.match(/SCORE:\s*(\d+)/i) || [])[1] || '85');
  if (avgScore < 0) avgScore = 0;
  if (avgScore > 100) avgScore = 100;

  // Detect critical failures
  const portraitFail = /PORTRAIT FORMAT:.*FAIL/i.test(segReport);
  const bobbyGFail = /TOP HALF.*FAIL/i.test(segReport);
  const clipFail = /BOTTOM HALF.*Clip.*FAIL/i.test(segReport);
  const freezeFail =
    /VIDEO FREEZE:.*FAIL/i.test(segReport) &&
    /yes/i.test((segReport.match(/VIDEO FREEZE:.*/) || [''])[0]);
  const hasCriticalFail = portraitFail || bobbyGFail || clipFail || freezeFail;

  const deductions = [];
  if (portraitFail)
    deductions.push({
      points: 25,
      reason: 'PORTRAIT FORMAT: video not in 9:16 portrait — CRITICAL',
    });
  if (bobbyGFail)
    deductions.push({
      points: 20,
      reason: `TOP HALF (${speakerName}): avatar not visible in top half — CRITICAL`,
    });
  if (clipFail)
    deductions.push({
      points: 20,
      reason: 'BOTTOM HALF (Clip): source clip not visible in bottom half — CRITICAL',
    });
  if (freezeFail) deductions.push({ points: 25, reason: 'VIDEO FREEZE detected — CRITICAL' });
  if (avgScore < 80)
    deductions.push({
      points: 80 - avgScore,
      reason: `Sample scored ${avgScore}/100 — see report for specifics`,
    });
  ...
  const whyDoc = [
    `=== CWN GATE 3: ASSEMBLY QA (SHORT-FORM) — ${outcomeLabel} ===`,
    `Gate:       3 of 4 — Assembly QA`,
    `Format:     Short-form 9:16 portrait split-screen`,
    `Scored by:  Gemini (did not assemble)`,
    `Time:       ${new Date().toISOString()}`,
    `Video:      ${path.basename(videoPath)}`,
    `Score:      ${avgScore}/100`,
    ...
    ``,
    `── CRITICAL FAILURES ────────────────────────────`,
    `Portrait format:   ${portraitFail ? '🚨 YES' : '✅ No'}`,
    `${speakerName} top half:  ${bobbyGFail ? '🚨 YES' : '✅ No'}`,
    `Clip bottom half:  ${clipFail ? '🚨 YES' : '✅ No'}`,
    `Video freeze:      ${freezeFail ? '🚨 YES' : '✅ No'}`,
    ``,
    `── SCORE BREAKDOWN ───────────────────────────────`,
    `STARTING SCORE: 100`,
    ``,
    ...
    outcome === 'pass'
      ? 'Auto-proceed to Upload-Post publish.'
      : outcome === 'manual_review'
        ? 'Review sample report above. Approve manually or reject to re-assemble.'
        : 'Hard fail — re-run short assembly. Check vstack filter, portrait dimensions, caption burn.',
  ]
  ...
  const qaLogDir = path.join(__dirname, '..', 'output', 'qa_failures');
  if (!fs.existsSync(qaLogDir)) fs.mkdirSync(qaLogDir, { recursive: true });
  const logFile = path.join(qaLogDir, `gate3_short_${outcome}_${Date.now()}.txt`);
  try {
    fs.writeFileSync(logFile, whyDoc);
    console.log(`[qa] Portal 3 short why-doc saved: ${logFile}`);
  } catch (e) {}
  ...
  return {
    ...
    outcome,
    outcomeLabel,
    freezeDetected: freezeFail,
    deductions,
  };
}

function parseScriptIntoScenes(script, opts = {}) {
  ...
  for (let i = 0; i < matches.length; i++) {
    ...
    const text = script.substring(startIndex, endIndex).trim();

    // Detect scene type BEFORE stripping markers
    const hasClipMarker = /\[CLIP PLAYS HERE\]/i.test(text);
    const hasClipType = /type:\s*source_clip/i.test(text);
    const sceneName = String(currentMatch.name || '');
    const shortSceneByName = /_SHORT$/i.test(sceneName) || /^SHORT_/i.test(sceneName);
    const isShortScene = contentType.includes('-short') || shortSceneByName;

    const sanitizeSceneText = (raw) =>
      String(raw || '')
        .replace(/<<<CWN_NBA_TS_LOCK>>>[\s\S]*?<<<CWN_NBA_TS_UNLOCK>>>/gi, '')
        .replace(/\[beat\]/gi, '')
        .replace(/^type:\s*(source_clip|avatar)\s*$/gim, '')
        .replace(/^spokenText:\s*/gim, '')
        .replace(/^HOOK:\s*/gim, '')
        .replace(/^REACTION:\s*/gim, '')
        .replace(/^CAPTION:\s*.+$/gim, '')
        .replace(/\[CLIP PLAYS HERE\]/gi, '')
        .replace(/\n{3,}/g, '\n\n')
        .trim();

    // Strip metadata lines and beat markers
    const cleanText = sanitizeSceneText(text);

    if (hasClipType) {
      // Explicit source_clip type declaration — pure clip scene, no dialogue
      scenes.push({ name: currentMatch.name, text: '', type: 'source_clip' });
    } else if (hasClipMarker && isShortScene) {
      // Short-form single-header scripts: split into HOOK avatar → source clip → REACTION avatar.
      const parts = text.split(/\[CLIP PLAYS HERE\]/i);
      const hookText = sanitizeSceneText(parts[0] || '');
      const reactionText = sanitizeSceneText(parts.slice(1).join(' ') || '');
      if (hookText) {
        scenes.push({ name: sceneName, text: hookText, type: 'avatar' });
      }
      scenes.push({ name: `${sceneName}_CLIP`, text: '', type: 'source_clip' });
      if (reactionText) {
        scenes.push({ name: `${sceneName}_REACTION`, text: reactionText, type: 'avatar' });
      }
    } else if (hasClipMarker && cleanText.length > 0) {
      // SETUP scene: has spoken dialogue AND [CLIP PLAYS HERE] marker
      // Bobby G speaks THEN clip plays — submit dialogue to HeyGen as avatar
      // The clip is inserted by assembly via orderedClipUrls after this avatar segment
      scenes.push({
      ...
      // [CLIP PLAYS HERE] with no spoken text — pure clip position
      scenes.push({ name: currentMatch.name, text: '', type: 'source_clip' });
    } else if (cleanText.length > 0) {
      scenes.push({ name: currentMatch.name, text: cleanText, type: 'avatar' });
    }
  }
...
function getFixSuggestion(cause) {
  switch (cause) {
    case 'TWITCH_API_EMPTY':
      return 'No clips returned in 48h window — streamer may not have been live recently';
    case 'TWITCH_API_ERROR':
      return 'Twitch API request failed — check TWITCH_CLIENT_ID/TOKEN in .env';
    case 'STREAMER_NOT_FOUND':
      return 'Twitch user lookup returned empty — verify streamer username spelling';
    case 'GQL_RESOLUTION_FAILED':
      return 'GQL CDN URL resolution failed — clip may be deleted or token expired';
    case 'CDN_DOWNLOAD_BLOCKED':
      return 'CDN download was blocked — clip URL may be expired or geo-restricted';
    case 'GEMINI_ANALYSIS_FAILED':
      return 'Gemini could not analyze clip — thumbnail or video URL may be invalid';
    case 'GEMINI_ANALYSIS_TRUNCATED':
      return 'Gemini analysis was truncated — clip may be too long or upload failed';
    case 'NO_CLIPS_AFTER_FILTERING':
      return 'All clips had non-real titles (spam/bot clips filtered) — check clip quality';
    case 'INSUFFICIENT_CLIPS':
      return 'Fewer than 2 signed video clips resolved — add more clips for this streamer in the dashboard';
    default:
      return 'Check server logs for details';
  }
}

function generateClipAvailabilityReport(
  items,
  allClips,
  streamerOrder,
  analysisClips,
  ...
  const targetPerStreamer =
    items && items[0] && items[0].clips && items[0].clips.length > 0 ? items[0].clips.length : 2;
  const expectedStreamers = streamerOrder
    ? streamerOrder.length
    : Object.keys(STREAMER_DISPLAY_NAMES).length;
  ...
  report.push(
    `Target: ${expectedTotal} clips (${expectedStreamers} streamers × ${targetPerStreamer} clips each)`
  );
  ...
  // Normalize streamerOrder to lowercase for dedup comparison
  const streamerOrderLower = (streamerOrder || []).map((s) => s.toLowerCase());
  // Show episode streamers first (in order), then roster streamers not already covered
  const allStreamersToShow = [
    ...(streamerOrder || []),
    ...rosterStreamers.filter((s) => !streamerOrderLower.includes(s.toLowerCase())),
  ];
  allStreamersToShow.forEach((streamer) => {
    ...
    if (!streamerOrder.includes(streamer)) {
      ...
      if (failure && failure.cause) {
        ...
        // Infer a diagnostic mode from clip data rather than a generic fallback
        const unclassifiedClips = allClips.filter((c) => c.streamer === streamer);
        if (unclassifiedClips.length === 0) {
          reason = `⚠️ Not included — cause: TWITCH_API_EMPTY\n       evidence: no clips in allClips for this streamer\n       fix: ${getFixSuggestion('TWITCH_API_EMPTY')}`;
        } else {
          const signed = unclassifiedClips.filter(
            (c) => c.videoUrl && String(c.videoUrl).includes('sig=')
          );
          if (signed.length === 0) {
            reason = `⚠️ Not included — cause: GQL_RESOLUTION_FAILED\n       evidence: ${unclassifiedClips.length} clip(s) found, none resolved to a signed CDN URL\n       fix: ${getFixSuggestion('GQL_RESOLUTION_FAILED')}`;
          } else {
            reason = `⚠️ Not included — cause: INSUFFICIENT_CLIPS\n       evidence: ${signed.length} signed clip(s) resolved but below the 2-clip floor\n       fix: ${getFixSuggestion('INSUFFICIENT_CLIPS')}`;
          }
        }
      }
...
async function claudeScriptQA(script, clipAnalyses, opts = {}) {
  ...
  if (!client)
    return {
      ...
      passed: true,
      outcome: 'pass',
      outcomeLabel: '✅ PASS (skipped — no key)',
      deductions: [],
    };
  ...
  // Long-form outro check: all shows end with "Goodnight and good luck."
  // Shorts have no outro (checked separately via isShortForm guard)
  const missingAppreciateYou = !/goodnight and good luck/i.test(script);
  const outroLabel = '"Goodnight and good luck."';
  ...
  // Build content-type-aware context and checklist via pluggable checklist module.
  // The module exports contextHeader(opts) and checklist(opts) — both functions.
  // Fallback to inline defaults if config or module is unavailable.
  ...
  let checklistMod = null;
  try {
    const { getContentTypeConfig } = require('./configLoader');
    const ctCfg = getContentTypeConfig(contentType);
    if (ctCfg && ctCfg.qa && ctCfg.qa.checklistModule) {
      checklistMod = require(path.join(__dirname, ctCfg.qa.checklistModule));
    }
  } catch (e) {
    console.warn(
      `[qa-gate1] checklist module load failed for ${contentType}: ${e.message} — falling back to inline defaults`
    );
  }

  const contextHeader = checklistMod
    ? checklistMod.contextHeader({
        displayNames,
        clipsPerStreamer,
        expectedClips,
        expectedScenes,
...
EXPECTED SCENES: ${expectedScenes}`;
  ...
  // isShortForm already declared above — used here for QA hint text

  const checklist = checklistMod
    ? checklistMod.checklist({ expectedScenes, expectedClips, customerId })
    : isShortForm
      ? [
          // Fix 5: Updated short-form checklist for HOOK + CLIP + REACTION structure (no INTRO, no OUTRO)
          `1. HOOK present: Is there an === HOOK === scene with exactly 1 sentence that sets up the clip without spoiling it?`,
          `2. CLIP marker: Is there exactly 1 [CLIP PLAYS HERE] marker, inside the === CLIP === scene?`,
          `3. REACTION present: Is there a === REACTION === scene with exactly 1 flat sentence delivered after the clip? No hype, no exclamation points.`,
          `4. CAPTION: Is there a CAPTION: line in the REACTION scene suitable for a burned-in screen overlay?
   ...
   NOTE: Do NOT check for INTRO or OUTRO scenes — short-form has neither. A missing INTRO/OUTRO is CORRECT per spec.`,
        ]
      : isTwitch
        ? [
   ...
   - Are there exactly ${expectedScenes} === SCENE === markers?`,
            ...
            `10. LOCKED INTRO (-15 if wrong): The === INTRO === scene MUST contain: "Welcome to Twitch Soup. I'm your host ${speakerName}, and for the next few minutes I'll be your guide through the world of livestreaming. Let's get into it." FAIL (-15) if the intro deviates from this wording.`,
            `11. WORD COUNT: Is each streamer section approximately 80-100 words?`,
            `12. VOICE STYLE (-15 if wrong): Does every scene use ${speakerName}'s Norm MacDonald + Jon Stewart deadpan voice? FAIL this item (-15) if any scene contains:
   ...
   - Are there exactly ${expectedScenes} === SCENE === markers?`,
              ...
              `10. LOCKED INTRO (-15 if wrong): The === INTRO === scene MUST contain: "What's up, ClipzWorld! Grab your shades, because we're heading to the Other Side of the Pillow. I'm ${speakerName}, and we're breaking down the highlights that weren't just good—they were cold." FAIL (-15) if the intro deviates from this wording.`,
   ...
   - Are there exactly ${expectedScenes} === HEADER === markers?`,
                ...
                `9. LOCKED INTRO (-15 if wrong): The === INTRO === scene MUST contain: "I'm ${speakerName}, and this is Because the Light was on. I'm told this is the news." FAIL (-15) if the intro deviates from this wording.`,
                `10. SOURCE ATTRIBUTION (STRICT): Does any scene contain spoken source attribution? Check for "According to Al Jazeera", "Sources report", "Al Jazeera's coverage". FAIL hard (-25) if any found — ${speakerName} must NEVER speak the source name.`,
                `11. REACTION: Does each STORY#_REACTION scene have a flat, deadpan 1-sentence reaction?`,
                `12. VOICE STYLE (-15 if wrong): Does every scene use ${speakerName}'s Jon Stewart + Norm MacDonald news anchor voice — facts stated plainly, one flat observation, done? FAIL this item (-15) if any scene contains:
  ...
  const qaPrompt = isShortForm
    ? `You are Portal 1 — the script style QA reviewer for short-form content in the AuraFlux production pipeline.

── HOW THIS WORK WAS PRODUCED ────────────────────────────────────────────────
1. The system generated a short-form scaffold with exactly 3 scene headers — HOOK, CLIP, REACTION:
   - All short types (clips-short, sports-short, news-short): HOOK → CLIP → REACTION
   - NO INTRO scene. NO OUTRO scene. 3 scenes total. This is correct per spec.
   - A missing INTRO or OUTRO is NOT an error — do not check for them.
   Scene names are UNIVERSAL (HOOK, CLIP, REACTION) not content-specific.
   ACCEPT only these 3 headers as valid. Do NOT require STORY1_INTRO, INTRO, OUTRO, or similar old names.
2. Gemini filled the [DIALOGUE] slots with spoken content
3. You are reviewing Gemini's dialogue for structural correctness only

YOUR GOAL: Help this job pass. Identify exactly what is wrong and what the fix is. A precise fix directive is success.

YOUR SCOPE: STRUCTURE ONLY — hook length, clip marker placement, reaction format, caption length, outro line.
You are NOT checking whether the reaction matches the clip — non-sequitur reactions are intentional.
You are NOT checking content accuracy, clip match, or fabricated content — those are other gates' jobs.

HOW TO FIX WHAT YOU FIND:
- Hook too long: report word count and exact required count → Gemini trims to spec
- Missing [CLIP PLAYS HERE]: report which scene is missing it → system re-inserts
- Caption too long: report character count and limit → Gemini shortens
- Wrong outro: report what was written and exact required line → Gemini fixes in one pass

SHOW: ${contentType} (${opts?.showName || (isTwitch ? 'Twitch Soup' : isNBA ? 'Other Side of the Pillow' : 'Because the Light Was On')})

── THE SCRIPT ────────────────────────────────────────
${script}

── YOUR QA CHECKLIST (4 items only) ─────────────────
...
── SCORING ───────────────────────────────────────────
Start at 100. Deduct:
  - Items 1, 2, 3: -15 each (structure is non-negotiable)
  - Item 4: -10 (caption must follow per-show rules)

PASS threshold: 90. Anything below = FAIL, Gemini rewrites.
...
Rules: fabricatedContent is ALWAYS []. missingScenes is ALWAYS []. nameErrors is ALWAYS [].
Only use structuralIssues for HOOK/CLIP/REACTION/CAPTION structural problems (wrong sentence count, missing marker, caption too long, hype words).
If all 4 checks pass, all four arrays are [].`
    : `You are Portal 1 — the script style QA reviewer for the AuraFlux broadcast video production pipeline.

── HOW THIS WORK WAS PRODUCED ────────────────────────────────────────────────
1. The system generated a structural scaffold (scene headers, clip positions) from the Job Spec
2. Gemini (AI) filled the [DIALOGUE] slots with spoken content
3. You (Claude) are now reviewing Gemini's dialogue for style quality

YOUR GOAL: Help this job pass to the next gate. Your job is not to reject — it is to identify exactly what needs to change so the script can be fixed and pass. A sendback with a precise fix directive is success. A rejection without a fix directive is failure on your part.

YOUR SCOPE — what you check:
- Voice style: flat delivery, no hype words (incredible/amazing/crazy/wild), Jon Stewart/Norm MacDonald tone
- Entity names: display names used correctly (not handles or usernames)
- Commentary accuracy: does dialogue match what the source clips actually show?
- Outro: must end with the exact required closing line for this show

YOU DO NOT CHECK: scene count, header format, structure, clip count, scene order. The system generated the scaffold — structure is guaranteed. Do not penalize anything structural.

HOW TO FIX WHAT YOU FIND:
- Wrong name: specify "used: X → correct: Y" — Gemini can fix in one pass
- Voice violation (hype word): specify exact scene and word, suggest replacement phrasing
- Fabricated fact: specify scene, what was fabricated, what the clip analysis actually shows — Gemini rewrites that scene using your provided facts
- Wrong outro: specify exact required closing line
- If the script is borderline (70-89): send it back with your fix directive — one retry is allowed
- If the script is hard fail (<70): escalate with full fix directive — do not send back, human reviews

Take your time. Read the full script before scoring. Give benefit of the doubt — only flag what is clearly and specifically wrong.
...
SCORE: [0-100]
ISSUES:
- [CHECK NAME]: [what's wrong] → [exactly what Gemini must write instead]
...
Rules for filling FIX_DIRECTIVE — be surgical, not generic:
- missingScenes: list every scene header absent from script (structure issues only — system scaffold may have shifted)
- fabricatedContent: for each scene with wrong content: { "scene": "STORY2_INTRO", "problem": "describes CEO resignation but clip is about wildfire", "fix": "rewrite using these facts from the clip: [paste exact clip analysis text here so Gemini has what it needs]" }
- nameErrors: for each wrong name: { "used": "Jaycinco", "correct": "Jay Cinco" }
- structuralIssues: list failures with exact required text. Example: "OUTRO wrong — must end with exactly: I'm ${speakerName}. See you tomorrow."
- If script passes all checks, all four arrays are empty [].
- Output VALID JSON only — no trailing commas, no comments inside json block.`;
  ...
  if (wrongClipCount) {
    preCheckDeductions.push({
      points: 25,
      reason: `CLIP COUNT: Found ${clipMarkers} [CLIP PLAYS HERE] markers, expected ${expectedClips} — CRITICAL`,
    });
    adjustedScore = Math.max(0, adjustedScore - 25);
  }
  if (!isShortForm && missingAppreciateYou) {
    preCheckDeductions.push({
      points: 15,
      reason: `OUTRO: ${outroLabel} missing from script — CRITICAL`,
    });
  ...
  if (hasWrongSceneNames) {
    preCheckDeductions.push({
      points: 25,
      reason: `SCENE NAMING: Used generic scene_NN names instead of === INTRO ===, === STORY1_INTRO === etc. — CRITICAL`,
    });
    adjustedScore = Math.max(0, adjustedScore - 25);
  }
  const hasCriticalFail =
    wrongSceneCount ||
    wrongClipCount ||
    (!isShortForm && missingAppreciateYou) ||
    hasWrongSceneNames ||
    adjustedScore < 60;
  let outcome, passed;
  ...
  const whyDoc = [
    ...
    isShortForm
      ? `No outro required (short-form)`
      : `Missing sign-off: ${missingAppreciateYou ? '🚨 YES' : '✅ No'}`,
    ``,
    `── SCORE BREAKDOWN ───────────────────────────────`,
    `STARTING SCORE: 100`,
    ``,
    ...
    `── RECOMMENDED ACTION ───────────────────────────`,
    outcome === 'pass'
      ? 'Auto-proceed to HeyGen segment generation.'
      : outcome === 'manual_review'
        ? 'Review issues above. Edit script in dashboard, then manually approve to send to HeyGen.'
  ...
  try {
    fs.writeFileSync(logFile, whyDoc);
    console.log(`[qa-gate1] Script QA why-doc saved: ${logFile}`);
  } catch (e) {}
  ...
  if (clipReportData && contentType === 'twitch') {
    try {
      const {
        items: rItems,
        allClips: rAllClips,
        streamerOrder: rOrder,
        analysisClips: rAnalysis,
      } = clipReportData;
      const clipReport = generateClipAvailabilityReport(
        rItems,
        rAllClips,
        rOrder,
        rAnalysis,
      ...
      console.warn(`[qa-gate1] Clip report append failed: ${e.message}`);
    }
  }
  ...
  return {
    ...
    fixDirective,
    tokenUsage,
    // Pipeline contract: carry expectedClips forward so Gates 2, assembly, and Portal 3
    // can assert clips were actually delivered. Never undefined after Portal 1.
    expectedClips,
  };
}
...
async function claudeScriptFix(script, clipAnalyses, opts = {}) {
  ...
  // Build clip reference block + scene guide via pluggable checklist module.
  // Falls back to inline implementation if module is unavailable.
  let fixChecklistMod = null;
  try {
    const { getContentTypeConfig } = require('./configLoader');
    const ctCfg = getContentTypeConfig(contentType);
    if (ctCfg && ctCfg.qa && ctCfg.qa.checklistModule) {
      fixChecklistMod = require(path.join(__dirname, ctCfg.qa.checklistModule));
    }
  } catch (e) {
    console.warn(
      `[qa-fix] checklist module load failed for ${contentType}: ${e.message} — falling back to inline defaults`
    );
  }
  ...
  let sceneGuide;
  if (fixChecklistMod) {
    clipRef = fixChecklistMod.clipReferences(streamers, clipAnalyses);
    sceneGuide = fixChecklistMod.sceneGuide;
  } else if (isNews) {
    ...
    clipRef = streamers
      ...
      .join('\n');
    sceneGuide = 'streamer INTRO, CLIP#_SETUP, CLIP#_REACTION sections';
  }
...
3. Keep ${speakerName} voice: flat, deadpan, Norm MacDonald delivery. Never say "incredible", "amazing", "shocking".
...
async function geminiScriptQA(script, clipAnalyses, opts = {}) {
  const { contentType = 'twitch', streamers = [], clipsPerStreamer = 2, jobId = 'unknown', customerId } = opts;
  ...
  const expectedClips =
    contentType === 'twitch' ? streamers.length * clipsPerStreamer : clipAnalyses.length;
  const wrongClipCount = Math.abs(clipMarkers - expectedClips) > 1; // allow ±1 tolerance
  // All long-form shows end with "Goodnight and good luck." — not "Appreciate you"
  const missingAppreciateYou = !/goodnight and good luck/i.test(script);
  const outroLabel = '"Goodnight and good luck."';

  // Build Gemini prompt with clip analyses for content verification
  const clipSummaries = clipAnalyses
    .map((a, i) => {
  ...
  // Load checklist module for this contentType — same mechanism as geminiGate1QA / claudeScriptFix.
  let scriptQAMod = null;
  try {
    const { getContentTypeConfig } = require('./configLoader');
    const ctCfg = getContentTypeConfig(contentType);
    if (ctCfg && ctCfg.qa && ctCfg.qa.checklistModule) {
      scriptQAMod = require(path.join(__dirname, ctCfg.qa.checklistModule));
    }
  } catch (_e) { /* non-fatal — fall back to generic defaults below */ }

  // expectedScenes: delegate to module formula; fall back to 0 (no scene-count validation).
  let expectedScenes = 0;
  if (scriptQAMod && typeof scriptQAMod.expectedScenesCount === 'function') {
    expectedScenes = scriptQAMod.expectedScenesCount(streamers.length, clipsPerStreamer);
  }

  const wrongSceneCount = expectedScenes > 0 && sceneMarkers !== expectedScenes;

  // contextHeader and checklist: delegate to module; fall back to generic.
  const contextHeader = scriptQAMod && typeof scriptQAMod.contextHeader === 'function'
    ? scriptQAMod.contextHeader({ displayNames, clipsPerStreamer, expectedClips, expectedScenes, streamers })
    : `ITEMS: ${streamers.length}\nEXPECTED [CLIP PLAYS HERE] COUNT: ${expectedClips}`;

  const checklist = scriptQAMod && typeof scriptQAMod.checklist === 'function'
    ? scriptQAMod.checklist({ expectedScenes, expectedClips, customerId: opts.customerId })
    : [
        `1. CLIP COUNT: Are there exactly ${expectedClips} [CLIP PLAYS HERE] markers?`,
        `2. OUTRO: Does the script end with "Appreciate you!"?`,
        `3. STRUCTURE: Does the script follow the expected format?`,
  ...
  const qaPrompt = `You are Portal 1 (Gemini) — the script quality reviewer for the AuraFlux broadcast video production pipeline.

── HOW THIS WORK WAS PRODUCED ────────────────────────────────────────────────
1. Portal 0 confirmed source videos are valid and accessible
2. Gemini analyzed each source clip and produced the clip summaries below
3. Claude wrote this script based on Gemini's clip analysis
4. This script will be sent to HeyGen to render Bobby G (avatar) speaking each scene
5. Each === SCENE_NAME === marker becomes a separate HeyGen video segment

YOUR GOAL: Help this job pass to HeyGen. Identify exactly what needs to change so the script can be fixed and approved. A sendback with a precise fix directive is success. A vague rejection is failure on your part.

HOW TO FIX WHAT YOU FIND:
- Wrong clip match: specify exact scene, what Claude wrote, and what the clip actually shows — include specific facts from clip analysis so Claude can rewrite accurately
- Wrong name: specify "used: X → correct: Y" — Claude fixes in one pass
- Missing marker: specify which === HEADER === is absent — system re-inserts
- Wrong outro: specify exact required closing line
- Scene count wrong: specify expected vs actual count and which scenes are missing/extra
- If borderline (70-89): send back with fix directive — one retry allowed
- If hard fail (<70): escalate with full fix directive

Take your time. Read carefully before scoring. Give benefit of the doubt — only flag real, clear problems. False positives waste HeyGen credits and delay production.

CONTENT TYPE: ${contentType}
${contextHeader}

── HEYGEN GENERATION CONTEXT (downstream dependency) ─────────────────────────
This script goes directly to HeyGen after you approve it:
  Avatar ID:    ${HEYGEN_AVATAR_ID.slice(0, 8)}... (${speakerName} avatar)
  Voice ID:     ${HEYGEN_VOICE_ID.slice(0, 8)}... (${speakerName} voice)
  Speak Speed:  ${HEYGEN_SPEAK_SPEED}x
  Expected Scenes: ${sceneMarkers} scenes (each === SCENE_NAME === marker = one HeyGen video)

IMPORTANT: Scene markers (=== SCENE_NAME ===) must be correctly formatted or HeyGen splits fail.
...
── SCORING ───────────────────────────────────────────
SCORE: [0-100]
For each failed check, deduct points per the scoring guide in the checklist above.
All other items: -5 each
...
[list]`;
  ...
  try {
    ...
    geminiReport = (genResp.data?.candidates?.[0]?.content?.parts || [])
      .map((p) => p.text || '')
      .join('')
      .trim();
  } catch (e) {
  ...
  // Use per-content-type deduction weights from the checklist module; fall back to Twitch weights.
  const DEDUCTION_MAP = (scriptQAMod && scriptQAMod.deductionMap) ||
    { 1: 15, 2: 15, 8: 15, 3: 10, 7: 10, 4: 5, 5: 5, 6: 5, 9: 5, 10: 5 };
  const DEDUCTION_LABELS = (scriptQAMod && scriptQAMod.deductionLabels) || {
    1: 'CLIP COUNT', 2: 'OUTRO', 3: 'DISPLAY NAMES', 4: 'INTRO LENGTH',
    5: 'REACTION LENGTH', 6: 'SETUP LENGTH', 7: 'BEAT PLACEMENT', 8: 'CLIP MATCH',
    9: 'LOCKED INTRO', 10: 'WORD COUNT',
  };
  ...
  const whyDoc = [
    ...
    geminiDeductions.length
      ? geminiDeductions.map((d) => `  -${d.points}  ${d.reason}`).join('\n')
      : '',
    geminiDeductions.length ? `` : '',
    preCheckDeductions.length ? `PRE-CHECK DEDUCTIONS:` : '',
    preCheckDeductions.length
...
// ffprobe pre-check: returns { hasAudio, durationSecs, bitrate } for a video file.
// Used by Portal 2 to override Gemini false positives (Gemini sometimes reports "no audio"
// on valid HeyGen segments with AAC audio tracks).
function ffprobeAudioCheck(filePath) {
  return new Promise((resolve) => {
    execFile(
      ...
      { timeout: 10000 },
      (err, stdout) => {
        if (err) return resolve({ hasAudio: false, durationSecs: 0, bitrate: 0 });
        try {
          const data = JSON.parse(stdout);
          const streams = data.streams || [];
          if (streams.length === 0)
            return resolve({ hasAudio: false, durationSecs: 0, bitrate: 0 });
          const s = streams[0];
          resolve({
            hasAudio: !!s.codec_name,
            codec: s.codec_name || '',
            durationSecs: parseFloat(s.duration || '0'),
            bitrate: parseInt(s.bit_rate || '0', 10),
          });
        } catch (e) {
          resolve({ hasAudio: false, durationSecs: 0, bitrate: 0 });
        }
      }
    );
  });
...
async function geminiSegmentQA(segmentPaths, opts = {}) {
  const { jobId = 'unknown', contentType = 'twitch', customerId = 'c0' } = opts;
  let speakerName = 'the host';
  try {
    const { loadCustomerConfig } = require('./customerConfig');
    const _sCfg = loadCustomerConfig(customerId, 'long-form');
  ...
  // Short-form: 1 segment only. Long-form: sample first, middle, last.
  const avatarSegs = segmentPaths.filter((p) => p && fs.existsSync(p));
  const toCheck = isShortForm
    ? [avatarSegs[0]].filter(Boolean)
    : [
        avatarSegs[0],
  ...
  for (const segPath of toCheck) {
    const label =
      segPath === toCheck[0]
        ? 'FIRST'
        : segPath === toCheck[toCheck.length - 1]
          ? 'LAST'
          : 'MIDDLE';
    try {
      // ── Step 1: ffprobe ground-truth check (runs locally, never wrong) ──────
      const probe = await ffprobeAudioCheck(segPath);
      console.log(
        `[qa-gate2] ${label} segment ffprobe: hasAudio=${probe.hasAudio} codec=${probe.codec || 'none'} dur=${probe.durationSecs}s bitrate=${probe.bitrate}`
      );
      ...
      const shortFormNote = isShortForm
        ? `
NOTE — SHORT-FORM SEGMENT: This is a 9:16 portrait avatar segment from a short-form video.
The avatar should be cropped/framed for portrait (tall) format, not wide landscape.
Do NOT fail for portrait cropping — that is expected and correct.
Do NOT check for ticker bar or outro ("Appreciate you!") — shorts don't have these.
`
        : '';

      // ── Step 2: ffprobe context injected into Gemini prompt ─────────────────
      // Gemini sometimes fails to detect the audio track in HeyGen's AAC encoding.
      // We give it the ground truth so it can focus on visual QA instead of guessing.
      const probeContext = probe.hasAudio
        ? `\n── FFPROBE GROUND TRUTH (authoritative) ──────────────\nAudio stream confirmed present by ffprobe: codec=${probe.codec}, duration=${probe.durationSecs}s, bitrate=${probe.bitrate}bps.\nDo NOT report "no audio" — the audio track exists. Focus your audio assessment on quality (clarity, distortion, crackling), not presence.\n`
        : `\n── FFPROBE GROUND TRUTH (authoritative) ──────────────\nffprobe found NO audio stream in this file. Audio missing is confirmed.\n`;

      const segPrompt = `You are a QA reviewer for HeyGen avatar segments.

Watch this ${speakerName} avatar segment and provide a detailed quality assessment.
${shortFormNote}${probeContext}
── HEYGEN GENERATION CONTEXT ─────────────────────────
This segment was generated by HeyGen with:
  Avatar ID:    ${HEYGEN_AVATAR_ID.slice(0, 8)}... (${speakerName} avatar — should be a professional news anchor)
  Voice ID:     ${HEYGEN_VOICE_ID.slice(0, 8)}... (${speakerName} voice — deep, authoritative male voice)
  Speak Speed:  ${HEYGEN_SPEAK_SPEED}x (slightly faster than normal for news pacing)

Expected quality: Clean video, smooth lip sync, professional avatar framing, clear audio.
...
3. AVATAR VISIBILITY: [PASS/FAIL]
   - Is ${speakerName} properly framed in the shot?
   - Is the face clearly visible throughout?
...
5. BACKGROUND: [PASS/FAIL]
   - ${speakerName}'s background is a warm home-office/studio setting (bookshelf, lamp) — this is CORRECT and expected
   - FAIL only if there are visual artifacts, glitches, green screen bleed, or the avatar is missing entirely
   - Do NOT fail for the bookshelf/room background — that is ${speakerName}'s standard HeyGen background
...
ISSUES: <comma-separated list of issues, or "None">`;
      ...
      let segReport = (genResp.data?.candidates?.[0]?.content?.parts || [])
        .map((p) => p.text || '')
        .join('')
        .trim();

      // ── Step 3: Claude correction layer ────────────────────────────────────
      // If Gemini flags audio/lip-sync failure but ffprobe confirms audio exists,
      // Claude reviews both reports and issues a corrected score.
      const geminiSaysNoAudio =
        /LIP SYNC:.*\[?FAIL/i.test(segReport) ||
        /AUDIO QUALITY:.*\[?FAIL/i.test(segReport) ||
        /AUDIO:.*\[?FAIL/i.test(segReport);
      let claudeCorrected = false;
      let claudeCorrectionNote = '';
      if (geminiSaysNoAudio && probe.hasAudio) {
        console.log(
          `[qa-gate2] ${label}: Gemini flagged audio/lip-sync fail but ffprobe confirms audio present — asking Claude to correct`
        );
        try {
          const correctionResp = await client.messages.create({
            model: 'claude-sonnet-4-6',
            max_tokens: 800,
            messages: [
              {
                role: 'user',
                content: `You are the QA arbiter for a video production pipeline. Gemini reviewed a HeyGen avatar segment and flagged audio/lip-sync failure. But ffprobe (the ground-truth tool) confirms the audio track IS present.

FFPROBE FACTS:
- Audio codec: ${probe.codec}
- Duration: ${probe.durationSecs}s
- Bitrate: ${probe.bitrate}bps
- Has audio: YES

GEMINI'S REPORT:
${segReport}

Your job: Gemini was wrong about audio being absent — the file has a valid audio track. Correct Gemini's report:
1. Override LIP SYNC to [PASS] (audio confirmed present by ffprobe)
2. Override AUDIO QUALITY to [PASS] (audio track confirmed present)
3. Keep all other assessments (AVATAR VISIBILITY, VIDEO FREEZE, BACKGROUND) as Gemini reported
4. Issue a corrected OVERALL SCORE based only on real issues (visual freeze, avatar visibility, background)
5. End with: CORRECTED: YES

Format your response exactly like the original Gemini report but with corrections applied.`,
              },
            ],
          });
          const claudeCorrection = (correctionResp.content || [])
            .map((b) => b.text || '')
            .join('')
            .trim();
          if (claudeCorrection && claudeCorrection.includes('CORRECTED: YES')) {
            segReport = claudeCorrection;
            claudeCorrected = true;
            claudeCorrectionNote = `\n[Claude correction applied: Gemini falsely reported no audio — ffprobe confirmed ${probe.codec} audio present at ${probe.bitrate}bps]`;
            console.log(
              `[qa-gate2] ${label}: Claude correction applied — Gemini audio false positive overridden`
            );
          }
        } catch (e) {
          console.warn(
            `[qa-gate2] ${label}: Claude correction failed (${e.message}) — falling back to ffprobe override`
          );
          // If Claude API fails, apply ffprobe override directly without Claude
          claudeCorrectionNote = `\n[ffprobe override: audio confirmed present (${probe.codec}, ${probe.bitrate}bps) — Gemini audio/lip-sync fail overridden without Claude review]`;
          claudeCorrected = true;
          // Rewrite just the failing lines in segReport
          segReport = segReport
            .replace(
              /1\.\s*LIP SYNC:\s*\[?FAIL\]?/i,
              '1. LIP SYNC: [PASS] (ffprobe confirmed audio present)'
            )
            .replace(
              /2\.\s*AUDIO QUALITY:\s*\[?FAIL\]?/i,
              '2. AUDIO QUALITY: [PASS] (ffprobe confirmed audio present)'
            );
        }
      }

      const segScore = parseInt(
        (segReport.match(/OVERALL SCORE:\s*\[?(\d+)\]?/i) ||
          segReport.match(/SCORE:\s*\[?(\d+)\]?/i) ||
          [])[1] || '80'
      );

      // ── Step 4: Score based on corrected report ─────────────────────────────
      // After Claude correction, re-evaluate failure patterns on the updated report
      const segDeductions = [];
      if (!claudeCorrected && /LIP SYNC:.*\[?FAIL/i.test(segReport)) {
        lipSyncFail = true;
        scores.push(20);
        ...
        !claudeCorrected &&
        (/AUDIO QUALITY:.*\[?FAIL/i.test(segReport) || /AUDIO:.*\[?FAIL/i.test(segReport))
      ) {
        audioMissing = true;
        scores.push(30);
        segDeductions.push('Audio missing/broken');
      } else {
        scores.push(claudeCorrected ? Math.max(segScore, 85) : segScore);
        // Track minor issues that reduce score
        if (segScore < 100) {
          if (/AVATAR VISIBILITY:.*\[?FAIL/i.test(segReport))
            segDeductions.push('Avatar framing issue');
          if (/BACKGROUND:.*\[?FAIL/i.test(segReport)) segDeductions.push('Background artifacts');
        }
      }

      reports.push(
        `=== ${label} SEGMENT ===\n${segReport}${claudeCorrectionNote}${segDeductions.length ? '\n\nISSUES: ' + segDeductions.join(', ') : ''}`
      );
  ...
  // If no scores — HARD FAIL. Auto-passing with no evidence is worse than failing.
  // This catches the case where segmentData had no avatar segments to sample.
  if (!scores.length) {
    const fs = require('fs'),
      p = require('path');
    const outPath = p.join(
      ...
      'output',
      'qa_failures',
      `gate2_segments_fail_${Date.now()}.txt`
    );
    const whyDoc = [
      `=== CWN GATE 2: SEGMENT QA — ❌ HARD FAIL ===`,
      `Gate:       2 of 4 — HeyGen Segment QA`,
      `Time:       ${new Date().toISOString()}`,
      `Job:        ${jobId}`,
      `Segments:   0 avatar segments sampled`,
      `Score:      0/100`,
      `Outcome:    HARD FAIL`,
      ``,
      `REASON: No avatar segments were available to sample. Portal 2 cannot pass without`,
      `reviewing at least one segment. This means segmentData had no avatar content,`,
      `or all segments were source_clips. Check HeyGen render completed correctly.`,
    ].join('\n');
    try {
      fs.writeFileSync(outPath, whyDoc);
    } catch (e) {}
    return {
      score: 0,
      passed: false,
      ...
      deductions: [
        {
          points: 100,
          reason: 'No avatar segments sampled — cannot pass Portal 2 without reviewing segments',
        },
      ],
      reports: [],
      jobId,
  ...
  const deductions = [];
  if (lipSyncFail)
    deductions.push({ points: 30, reason: 'LIP SYNC broken on avatar segment — CRITICAL' });
  if (audioMissing)
    deductions.push({ points: 25, reason: 'AUDIO missing on avatar segment — CRITICAL' });
  ...
  const whyDoc = [
    ...
    `Gate:       2 of 4 — HeyGen Segment QA`,
    `Scored by:  Gemini reviews → Claude corrects Gemini false positives via ffprobe ground truth`,
    `Time:       ${new Date().toISOString()}`,
    `Job:        ${jobId}`,
    `Segments:   ${avatarSegs.length} avatar segments checked (${isShortForm ? '1 sampled — short-form' : '3 sampled'})`,
    `Score:      ${avgScore}/100`,
    ...
    `FINAL SCORE: ${avgScore}/100`,
    ``,
    `── SEGMENT REPORTS (Gemini + Claude corrections) ─────────────────`,
    fullReport,
    ``,
    `── RECOMMENDED ACTION ───────────────────────────`,
    outcome === 'pass'
      ? 'Auto-proceed to CapCut/FFmpeg assembly.'
      : outcome === 'manual_review'
        ? 'Review segment issues above. Approve manually or reject to re-generate affected segments.'
  ...
  try {
    fs.writeFileSync(logFile, whyDoc);
    console.log(`[qa-gate2] Segment QA why-doc saved: ${logFile}`);
  } catch (e) {}
...
async function callClaudeAPI(params) {
  ...
  try {
    const firstMsg = Array.isArray(params?.messages) ? params.messages[0] : null;
    const promptText =
      typeof firstMsg?.content === 'string'
        ? firstMsg.content
        : Array.isArray(firstMsg?.content)
          ? firstMsg.content.map((p) => p?.text || '').join('\n')
          : '';
    ...
    if (e.status === 429) {
      throw new Error(
        `Claude API rate limited. Retry after ${e.headers?.['retry-after'] || '60'} seconds`
      );
    }
...
async function uploadToGeminiFiles(filePath, maxRetries = 3) {
  ...
  for (let attempt = 0; attempt < maxRetries; attempt++) {
    try {
      ...
      if (attempt > 0) {
        console.log(
          `[gemini-upload] Retry ${attempt}/${maxRetries - 1} for ${path.basename(filePath)} (${fileSize}MB)`
        );
      }
      ...
      console.warn(
        `[gemini-upload] Upload failed (attempt ${attempt + 1}): ${e.message}. Retrying in ${backoffMs / 1000}s...`
      );
...
async function deleteGeminiFile(fileName) {
  try {
    await axios.delete(
      `https://generativelanguage.googleapis.com/v1beta/${fileName}?key=${GEMINI_APIKEY}`,
      { timeout: 10000 }
    );
 ...
 *
 * CRITICAL CONSTRAINT: Portal 1 auto-retry max ONCE — no infinite loops burning HeyGen credits
 *
...
function autoAction(gate, score, context = {}) {
  ...
  // ── Portal 1: Script QA ──
  // gate1Worker sets `passed` from customer qaThresholds (e.g. short-form pass 70, long-form 90).
  // Must respect that — do not regenerate when Portal 1 already marked passed (score can be <90).
  if (gate === 1) {
    if (context.gate1Passed === true) {
      return { action: 'proceed', directive: 'AUTO_PROCEED_TO_HEYGEN', reason: 'Portal 1 passed' };
    }
    if (score >= 90) {
      return { action: 'proceed', directive: 'AUTO_PROCEED_TO_HEYGEN', reason: 'Portal 1 passed' };
    }
    ...
    if (retryCount < 3) {
      return {
        ...
        reason: `Portal 1 score ${score} — auto-retry ${retryCount + 1}/3`,
      };
    }
    ...
    return {
      ...
      reason: `Portal 1 score ${score} after 3 retries — manual review required`,
    };
  }

  // ── Portal 2: HeyGen Segment QA (pass ≥85) ──
  if (gate === 2) {
    if (score >= 85) {
      return { action: 'proceed', directive: 'AUTO_PROCEED_TO_ASSEMBLY', reason: 'Portal 2 passed' };
    }
    if (score >= 65 && score < 85) {
      return {
        action: 'proceed_with_warning',
        directive: 'MANUAL_REVIEW',
        reason: 'Portal 2 score 65-84 — continue assembly with warning',
      };
    }
    // Score <65: identify failed segments, re-submit only those (not full re-render)
    return {
      action: 'rerender_segments',
      directive: 'RERENDER_SEGMENTS',
      reason: 'Portal 2 score <65 — re-submit failed segments only',
    };
  }

  // ── Portal 3: Assembly QA (pass ≥70) ──
  if (gate === 3) {
    if (score >= 70) {
      return { action: 'proceed', directive: 'AUTO_UPLOAD_TO_DRIVE', reason: 'Portal 3 passed' };
    }
    if (score >= 60 && score < 70) {
      return {
        action: 'proceed_with_warning',
        directive: 'MANUAL_REVIEW',
        reason: 'Portal 3 score 60-69 — upload anyway, flag for review',
      };
    }
    ...
    if (missingClips && missingClips.length > 0) {
      return {
        ...
        directive: 'RETRY_ASSEMBLY_WITH_CLIPS',
        reason: `Portal 3 score <60 — missing clips detected: ${missingClips.join(', ')}`,
      };
    }
    // Score <60 other: rollback to all_sent stage (not full HeyGen re-render)
    return {
      action: 'rollback',
      directive: 'ROLLBACK_TO_HEYGEN',
      reason: 'Portal 3 score <60 — rollback to all_sent stage',
    };
  }
...
--${boundary}--`),
  ...
  try {
    ...
    const analysis = (genResp.data?.candidates?.[0]?.content?.parts || [])
      .map((p) => p.text || '')
      .join('')
      .trim();
