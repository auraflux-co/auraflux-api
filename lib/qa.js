...
async function geminiQACheck(videoPath, opts = {}) {
  const { contentType, avatarCount, clipCount, downloadedClipCount, expectedClips, expectedTicker, totalDuration, customerId = 'c0' } = opts;
  ...
  try {
    ...
    speakerName = _sCfg?.voice?.speakerName || _sCfg?.designDefaults?.voice?.speakerName || speakerName;
  } catch (_e) {}
...
SUMMARY: [one sentence]`;
  ...
  if (bobbyGFail)   deductions.push({ points: 20, reason: `TOP HALF (${speakerName}): avatar not visible in top half — CRITICAL` });
...
function parseScriptIntoScenes(script, opts = {}) {
  ...
  for (let i = 0; i < matches.length; i++) {
    ...
    if (hasClipType) {
      ...
      scenes.push({ name: currentMatch.name, text: cleanText, type: 'avatar', hasClipInsert: true });
    } else if (hasClipMarker && cleanText.length === 0) {
