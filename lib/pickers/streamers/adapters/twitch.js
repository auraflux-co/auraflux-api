...
async function fetchTwitchClips(login, { clipsPer = 3, pubHours = 24, strictWindow = true, sort = 'popular' } = {}) {
  ...
  const endedAt = startedAt ? new Date().toISOString() : null;

  const first = Math.min(Math.max(clipsPer * 2, pickerCap), 100);
  const all = [];
  let cursor = null;
  let pages = 0;
  const maxPages = pubHours == null ? 10 : (pubHours >= 720 ? 10 : pubHours >= 168 ? 8 : 5);
