...
async function fetchTwitchClips(login, { clipsPer = 3, pubHours = 24 } = {}) {
  ...
  // Helix first= max is 100 per request — one HTTP call regardless of N (pagination only if N>100).
  const pickerCap = Math.min(Math.max(parseInt(process.env.TWITCH_PICKER_CLIP_CAP, 10) || 30, 8), 100);
  const fetchWindow = Math.min(Math.max(clipsPer * 2, pickerCap), 100);
  ...
  const clips = raw.slice(0, returnCap).map(c => ({
    title: c.title || 'Clip',
    url: c.url || '',
