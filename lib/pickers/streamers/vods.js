...
async function fetchTwitchVods(login, { limit = 5, pubBand = null } = {}) {
  ...
  const fetchN = Math.min(Math.max(limit, 5), 500);
  const headers = { 'Client-Id': clientId, Authorization: `Bearer ${token}` };
  const rawRows = [];
  let cursor = null;
  while (rawRows.length < fetchN) {
    const pageSize = Math.min(100, fetchN - rawRows.length);
    ...
    const resp = await axios.get(
      `https://api.twitch.tv/helix/videos?${qs.toString()}`,
      { headers, timeout: 15000 }
    );
    const page = resp.data?.data || [];
    if (!page.length) break;
    rawRows.push(...page);
    cursor = resp.data?.pagination?.cursor || null;
    if (!cursor) break;
  }
  let vods = rawRows.map((v) => ({
    title: v.title || 'VOD',
...
async function fetchKickVods(login, { limit = 5, pubBand = null } = {}) {
  ...
  const fetchN = Math.min(Math.max(limit, 5), 200);
...
async function fetchYoutubeVods(handle, { limit = 5, channelId = null, pubBand = null } = {}) {
  ...
  // Over-fetch: Shorts filtered out; date bands drop more; playlist pages up to 500.
  const fetchN = Math.min(Math.max(limit * 3, limit, 30), 500);
  const contentOpts = { type: 'video' }; // long-form via duration in getChannelUploads
  if (pubBand?.startedAt) contentOpts.publishedAfter = pubBand.startedAt;
  const raw = await client.getChannelUploads(channel.id, fetchN, contentOpts);
  let vods = (raw || [])
