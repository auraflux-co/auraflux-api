...
function markHookSelection(card, clipIndex, hookText) {
  const alreadyBuilt = !!(card.assembledAt || card.driveUrl);
  card.hooksPendingReassemble = alreadyBuilt;
...
function assertReadyToPublish(card = {}) {
  if (!card.clipsOnly) return { ok: true };
...
function resolveGate5PublishCopy(card = {}) {
  ...
  if (card.operatorTitleLocked && card.publishCopy) {
    pc = card.publishCopy;
  } else if (Array.isArray(card.titleCandidates)) {
    const sel = card.titleCandidates.find((c) => c && c.selected);
    if (sel?.text && card.publishCopy) pc = card.publishCopy;
  }
  if (pc && card.publishCopy && pc !== card.publishCopy) {
    pc = card.publishCopy;
  }
  return pc || {};
}
