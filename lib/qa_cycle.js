...
function limits() {
  ...
  const intervention = Math.max(1, parseInt(process.env.QA_MAX_INTERVENTION_ATTEMPTS || '3', 10) || 3);
...
function getByGateFromSpec(jobSpec) {
  return (jobSpec && jobSpec.state && jobSpec.state.automation && jobSpec.state.automation.qaCycle && jobSpec.state.automation.qaCycle.byGate) || {};
}
