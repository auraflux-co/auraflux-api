 ...
 * Portal contract violations are logged via console.warn so broken upstream stages are visible
...
const CONTRACTS = {
  '0': {
    ...
    requires: [],
    // CPD-485: Portal 0 writes clip plans to order.inputs.items (not designSpec.clips —
    // that was a stale reference). jobSpec.clipSpec carries trim points from the clip editor.
    produces: ['order.inputs.items', 'designSpec.expectedClipCount'],
  },
  '1': {
    description: 'Script generation (AI commentary or research)',
    // CPD-485: portal1 reads source context from order.inputs.items, not designSpec.clips
    requires: ['order.inputs.items'],
  ...
  '2': {
    ...
    // CPD-485: assembly reads clip URLs from order.inputs.items; clipSpec (optional) carries
    // editor trim points forwarded from the dashboard fetchSpec.clipSpec.
    requires: ['order.inputs.items'],
 ...
 *
 * Non-fatal — logs a warning. Does not throw.
 */
