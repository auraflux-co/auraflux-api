 ...
 *
 * All routes require Better Auth JWT. Platform: 'youtube' | 'tiktok' | 'instagram'
 */
...
async function requireAuthOrQueryToken(req, res, next) {
  ...
  if (queryToken) {
    try {
      // 60s grace for slow OAuth popup loads / minor clock drift
...
router.get('/social/callback/:platform', async (req, res) => {
  ...
  try {
    ...
    try {
      await assertCanConnectPlatform(stateData.customerId, platform, stateData.brandId || null);
    } catch (limitErr) {
      if (limitErr.code === 'CHANNEL_LIMIT') {
        const settingsUrl = process.env.APP_URL
          ? `${process.env.APP_URL}/settings`
          : 'https://app.auraflux.co/settings';
        return res.redirect(
          `${settingsUrl}?social_error=${encodeURIComponent(limitErr.message)}&platform=${platform}`
        );
      }
      throw limitErr;
    }
