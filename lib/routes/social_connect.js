...
router.get('/social/connect/:platform', requireAuthOrQueryToken, async (req, res) => {
  ...
  if (!DIRECT_OAUTH_PLATFORMS.has(platform)) {
    try {
      ...
      const accessUrl = await generateConnectUrl(customerId, {
        redirectUrl,
        platforms: [platform],
      });
  ...
  _pkceStore.set(state, { customerId, platform });
...
router.get('/social/callback/:platform', async (req, res) => {
  ...
  if (oauthError) {
    return res.redirect(
      `${process.env.NEXT_PUBLIC_APP_URL || ''}/settings/social?social_error=${oauthError}&platform=${platform}`
    );
  }
...
// For Upload-Post managed platforms (tiktok, instagram) we also reset the
// Upload-Post profile so the next connect flow starts with a clean slate.
// Without this the old account stays linked in Upload-Post, the user has to
// manually disconnect inside Upload-Post's hosted UI, and the JWT session
// becomes exhausted — causing a redirect loop or error page on reconnect.
router.delete('/social/accounts/:platform', requireAuth, async (req, res) => {
  try {
    ...
    // Remove our stored tokens first (non-fatal if platform wasn't in DB)
    await deleteTokens(customerId, platform);

    // For Upload-Post managed platforms, reset the UP profile so reconnect
    // starts with a clean OAuth session (no stale account blocking the flow).
    if (!DIRECT_OAUTH_PLATFORMS.has(platform)) {
      try {
        await resetProfile(customerId);
      } catch (upErr) {
        // Non-fatal — log but don't block the disconnect response
        console.warn('[social-connect] Upload-Post profile reset failed (non-fatal):', upErr?.message);
      }
    }
  ...
  try {
    ...
    if (event === 'social_account_disconnected' && customerId && platform) {
      ...
      try {
        ...
        console.warn('[uploadpost-webhook] deleteTokens failed (non-fatal):', dbErr?.message);
      }
    }
