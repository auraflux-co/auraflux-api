...
const {
  ...
  resetProfile: resetUploadPostProfile,
...
router.delete('/social/accounts/:platform', requireAuth, resolveBrandContext, async (req, res) => {
  try {
    ...
    // For Upload-Post managed platforms, reset the entire profile so the next
    // connect flow starts with a genuinely clean OAuth session.
    // Upload-Post has no per-platform disconnect API — the only documented
    // endpoint is DELETE /api/uploadposts/users (full profile reset).
    // After the reset, all Upload-Post-managed tokens (tiktok + instagram) are
    // invalid, so we remove them from our DB too.
    if (!DIRECT_OAUTH_PLATFORMS.has(platform)) {
      await resetUploadPostProfile(customerId).catch((err) => {
        console.warn(`[social-connect] Upload-Post profile reset failed (non-fatal):`, err?.message);
      });
      // Clear all Upload-Post-managed tokens — the profile is now empty
      const UP_PLATFORMS = ['tiktok', 'instagram'];
      await Promise.all(
        UP_PLATFORMS.map((p) => deleteAllTokensForPlatform(customerId, p).catch(() => {}))
      );
    }
