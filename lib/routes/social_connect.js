...
router.get('/social/connect/:platform', requireAuthOrQueryToken, resolveBrandContext, async (req, res) => {
  ...
  // Encode the source flow in the state so the callback can redirect correctly
  // even if the _pkceStore entry is lost (server restart, multi-instance).
  const source = brandId ? 'admin' : 'settings';
...
router.get('/social/callback/:platform', async (req, res) => {
  ...
  // Derive redirect target from state suffix (:admin or :settings) so errors
  // land on the correct page even if _pkceStore was cleared.
  const [, stateSource] = (state || ':settings').split(':');
  const appUrl = process.env.NEXT_PUBLIC_APP_URL || 'https://app.auraflux.co';
  const errorRedirectPath = stateSource === 'admin' ? '/admin/connect-brands' : '/settings/social';
  ...
  try {
    ...
    // If coming from admin connect-brands page (has brand_id in state), redirect back there
    const redirectPath = stateData.brandId ? '/admin/connect-brands' : '/settings/social';
    res.redirect(`${appUrl}${redirectPath}?social_connected=${platform}`);
  } catch (err) {
    console.error(`[social-connect] ${platform} callback error:`, err.message);
    // If coming from admin connect-brands page, redirect back there on error too
    const redirectPath = stateData?.brandId ? '/admin/connect-brands' : '/settings/social';
    res.redirect(
      `${appUrl}${redirectPath}?social_error=${encodeURIComponent(err.message)}&platform=${platform}`
    );
  }
});
