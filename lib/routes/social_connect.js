...
router.get('/social/connect/:platform', requireAuthOrQueryToken, async (req, res) => {
  ...
  if (!DIRECT_OAUTH_PLATFORMS.has(platform)) {
    try {
      ...
      // Do NOT filter by platform — show all Upload-Post managed platforms in
      // one session. This lets the user disconnect an old TikTok/Instagram
      // account AND reconnect a new one in a single JWT session, avoiding the
      // "missing code or verifier" error that occurs when a stale account blocks
      // a reconnect in a separate session.
      const accessUrl = await generateConnectUrl(customerId, { redirectUrl });
...
router.get('/social/callback/:platform', async (req, res) => {
  ...
  if (oauthError) {
    return res.redirect(
      `${process.env.NEXT_PUBLIC_APP_URL || 'https://app.auraflux.co'}/settings/social?social_error=${oauthError}&platform=${platform}`
    );
  }
