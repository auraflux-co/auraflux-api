...
async function requireAuthOrQueryToken(req, res, next) {
  ...
  if (queryToken) {
    try {
      const payload = await verifyToken(queryToken, _clerkVerifyOptions());
...
router.get('/social/connect/:platform', requireAuthOrQueryToken, async (req, res) => {
  ...
  if (!DIRECT_OAUTH_PLATFORMS.has(platform)) {
    try {
      ...
      const redirectUrl = `${appUrl}/dashboard/settings/social-connect?social_connected=${platform}`;
...
router.get('/social/callback/:platform', async (req, res) => {
  ...
  try {
    ...
    createNotification(stateData.customerId, {
      ...
      actionUrl: '/dashboard/settings/social-connect',
    });
