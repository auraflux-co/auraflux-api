...
router.get('/social/accounts', requireAuth, resolveBrandContext, async (req, res) => {
  try {
    ...
    // Fetch Upload-Post accounts for this brand. If the brand profile has no
    // accounts yet (e.g. user connected before the brandId-keying migration),
    // fall back to the legacy customerId-keyed profile so they aren't locked out.
    // Once they reconnect under this brand the fallback becomes a no-op.
    let uploadPostAccounts = [];
    if (brandId) {
      uploadPostAccounts = await listUploadPostAccounts(brandId).catch(() => []);
      if (uploadPostAccounts.length === 0 && customerId && customerId !== brandId) {
        uploadPostAccounts = await listUploadPostAccounts(customerId).catch(() => []);
        if (uploadPostAccounts.length > 0) {
          console.log(`[social/accounts] using legacy customerId profile for brandId=${brandId} — user should reconnect`);
        }
      }
    }

    const [directAccounts] = await Promise.all([
