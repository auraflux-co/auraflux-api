 ...
 *   POST /support/sms-webhook       — Twilio inbound SMS (public, Twilio-signed)
 *   POST /support/sms-status        — Twilio delivery status callback (public)
 *
 * Feature gating:
...
// ─── Twilio setup (lazy — only initialised if env vars present) ───────────────
...
// ─── POST /support/sms-webhook — Twilio inbound SMS ─────────────────────────
// Public route — Twilio posts here when customer texts +15715001787.
// Validates Twilio signature, matches phone to user, stores message,
// generates AI reply via Gemini, replies via TwiML.
...
router.post('/support/sms-webhook', async (req, res) => {
  ...
  const from    = req.body?.From;     // e.g. +12025551234
  ...
  if (!from || !body) {
    twiml.message('Sorry, we received an empty message. Please try again.');
    return res.type('text/xml').send(twiml.toString());
  }

  try {
    // Find session by phone number
    let session = await db.findSessionByPhone(from);

    // No session → can't match to user without a phone record; ask them to register
    if (!session) {
      twiml.message(
    ...
    logError('SUPPORT_SMS_WEBHOOK_FAIL', err, { from });
    twiml.message('Sorry, something went wrong on our end. Please try again or visit app.auraflux.co/dashboard/support.');
    return res.type('text/xml').send(twiml.toString());
  }
});

// ─── POST /support/sms-status — Twilio delivery status callback ───────────────

router.post('/support/sms-status', (req, res) => {
  // Log delivery status — non-blocking, best-effort
  const { MessageSid, MessageStatus, To } = req.body || {};
  if (MessageStatus === 'failed' || MessageStatus === 'undelivered') {
    logError('SUPPORT_SMS_DELIVERY_FAIL', new Error(`SMS ${MessageStatus}`), { MessageSid, To });
  }
  res.sendStatus(204);
});
