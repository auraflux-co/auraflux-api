...
// Outbound SMS from-number (operator → customer replies)
const SUPPORT_FROM = process.env.SUPPORT_SMS_NUMBER || process.env.NEXT_PUBLIC_SUPPORT_SMS_NUMBER || '';
...
const adminAuth = [requireAuth, requireRole({ minLevel: ROLES.OPERATOR })];
...
router.post('/support/sessions/:id/reply', adminAuth, apiLimit, async (req, res) => {
  ...
  try {
    ...
    if (session.phone_number && SUPPORT_FROM) {
      try {
        await sms.sendSms({ to: session.phone_number, from: SUPPORT_FROM, body: message.trim() });
      } catch (smsErr) {
