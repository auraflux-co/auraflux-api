...
router.post('/support/sms-webhook', async (req, res) => {
  ...
  // Safety: if the 'to' number is a brand number but routing failed, ACK and drop
  // rather than routing through the customer support AI.
  if (to && to !== process.env.TELNYX_NUMBER && to !== process.env.SUPPORT_SMS_NUMBER) {
    return res.sendStatus(200);
  }
