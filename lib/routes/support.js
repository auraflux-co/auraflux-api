...
// ─── POST /support/slack-events — Slack Events API (thread replies → SMS) ───
// Slack App → Event Subscriptions → Request URL:
//   https://auraflux-api.onrender.com/support/slack-events
// Subscribe to: message.channels (or message.groups for private channels)
// Requires: SLACK_BOT_TOKEN, SLACK_SIGNING_SECRET, SLACK_SMS_CHANNEL_ID

router.post('/support/slack-events', async (req, res) => {
  const { verifySlackRequest } = require('../services/slack_bot');
  const { handleSlackMessageEvent } = require('../services/slack_sms_reply');

  // url_verification handled in server.js (early) — duplicate guard for safety
  if (req.body?.type === 'url_verification' && req.body?.challenge) {
    return res.status(200).type('application/json').send(
      JSON.stringify({ challenge: req.body.challenge }),
    );
  }
...
router.post('/support/voice-webhook', async (req, res) => {
  ...
  const { parseVoiceEvent, notifyVoiceEvent } = require('../services/telnyx_slack_notify');
  const voiceEvt = parseVoiceEvent(req);
  if (voiceEvt) {
    notifyVoiceEvent(voiceEvt).catch(() => {});
  }
  return res.sendStatus(200);
});
