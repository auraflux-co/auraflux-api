...
router.post('/support/sms-webhook', async (req, res) => {
  ...
  // Always persist for SMS Inbox (brand numbers + operator 437/571 lines).
  const { persistInboundSms } = require('../services/sms_inbox_store');
  await persistInboundSms({ from, to, body });

  // ── Brand inbox routing ────────────────────────────────
  // If the 'to' number matches a brand's telnyx_number, skip the support flow
  // (verification codes live in SMS Inbox; Slack already notified via thread helper).
  if (to) {
    try {
