...
router.post('/support/sms-webhook', async (req, res) => {
  ...
  if (to) {
    try {
      const { rows: [brand] } = await db.query(
        `SELECT id FROM brands WHERE telnyx_number = $1 LIMIT 1`, [to]
      );
      if (brand) {
        await db.query(
  ...
  // Safety: if 'to' is not a recognised support number, ACK and drop rather than
  // routing through the customer support AI. getSupportNumbers() covers TELNYX_NUMBER,
  // SUPPORT_SMS_NUMBER, and any ported numbers in SUPPORT_SMS_NUMBERS.
  ...
  try {
    ...
    _notifyOperatorsNewSms(session, body).catch(() => {});
...
async function _notifyOperatorsNewSms(session, body) {
  try {
  ...
  // Slack notification — fires when SLACK_SUPPORT_WEBHOOK_URL is set
  _notifySlackNewSms(session, body).catch(() => {});
}

/** Post an inbound SMS notification to Slack via incoming webhook. */
async function _notifySlackNewSms(session, body) {
  const webhookUrl = process.env.SLACK_SUPPORT_WEBHOOK_URL;
  if (!webhookUrl) return;

  try {
    const axios = require('axios');
    const inboxUrl = `${process.env.NEXT_PUBLIC_APP_URL || 'https://app.auraflux.co'}/dashboard/admin/support?session=${session.id}`;
    const from = session.phone_number || 'unknown number';

    await axios.post(webhookUrl, {
      text: `📱 *New support SMS* from ${from}`,
      blocks: [
        {
          type: 'section',
          text: {
            type: 'mrkdwn',
            text: `📱 *New support SMS*\n*From:* ${from}\n*Message:* ${body.slice(0, 300)}`,
          },
        },
        {
          type: 'actions',
    ...
    logError('SLACK_SMS_NOTIFY_FAIL', err, { sessionId: session.id });
  }
}
