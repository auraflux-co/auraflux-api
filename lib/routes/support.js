 ...
 *   POST /support/voice-webhook           — inbound voice / call events (Telnyx)
...
router.post('/support/sms-webhook', async (req, res) => {
  ...
  if (to) {
    try {
      ...
      if (brand) {
        ...
        const { notifyInboundSms } = require('../services/telnyx_slack_notify');
        ...
        notifyInboundSms({
          ...
          inboxUrl: `${appUrl}/admin/sms-inbox`,
        }).catch(() => {});
        return res.sendStatus(200);
      }
    } catch (err) {
  ...
  if (to && !getSupportNumbers().includes(to)) {
    const { notifyInboundSms, shouldNotifyForNumber } = require('../services/telnyx_slack_notify');
    if (shouldNotifyForNumber(to)) {
      notifyInboundSms({ from, to, body }).catch(() => {});
    }
    return res.sendStatus(200);
  }
