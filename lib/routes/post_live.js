...
router.get('/post-live/published-jobs', (req, res) => {
  ...
  const items = Object.entries(jobs)
    .filter(([, card]) => isPublishedTalkSoupJob(card))
    .map(([jobId, card]) => ({
      jobId,
...
router.post('/post-live/sessions/register-from-job', (req, res) => {
  ...
  if (!isPublishedTalkSoupJob(card)) {
    return res.status(409).json({ ok: false, error: 'Job is not a published Talk Soup episode' });
  }
  try {
