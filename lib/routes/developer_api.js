...
// Content types that are NOT simple aliases of a productionProfile and must never be
// overridden by the profile → contentType mapping. When a caller explicitly sets one
// of these, the contentType wins over productionProfile.
const NON_ALIAS_CONTENT_TYPES = new Set(['show_commentary', 'custom']);
        ...
        // at the top-level spec.sourceConfig), so check both paths.
        {
          ...
          if (_isTwitchVod) {
            try {
              ...
              if (_spec.contentType === 'show_commentary' && _extractedPaths.length > 1) {
                try {
