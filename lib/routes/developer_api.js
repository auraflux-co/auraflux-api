 ...
 *   POST   /v1/account/api-keys       — create API key (invite-approved only)
 *   DELETE /v1/account/api-keys/:keyId — revoke API key
 *   GET    /v1/peaks/vods             — list VODs for Peaks
 *   POST   /v1/peaks/analyze          — YouTube Most Replayed / Twitch chat peaks
 *   GET    /v1/peaks/sessions/:id/segments
 *   POST   /v1/peaks/stage            — stage VOD window to R2
 *   GET    /v1/peaks/kick             — Kick CCV peaks
 *   GET    /v1/peaks/twitch-ccv       — Twitch CCV peaks
 *   GET    /v1/peaks/presets          — C1–C11 Short compose presets (edits after Peaks)
 *   POST   /v1/jobs                   — also accepts productionPath short_compile_clips + featureConfig.compose.preset
 */
...
const { requireApiInviteApproved } = require('../auth/api_invite');
...
// Peaks platform-signal wrappers (analyze → stage → jobs → approve-publish)
router.use(require('./developer_peaks'));
...
router.post('/jobs', async (req, res) => {
  ...
  try {
    ...
    // Peaks → Short: bake C1–C11 compose preset (parity with jobs_c1 / dashboard Peaks UI)
    if (b.productionPath) jobSpec.productionPath = b.productionPath;
    if (b.featureConfig && typeof b.featureConfig === 'object') {
      jobSpec.order.featureConfig = { ...(jobSpec.order.featureConfig || {}), ...b.featureConfig };
    }
    if (b.productionPath === 'short_compile_clips') {
      const {
        applyClipCompProfileToJobSpec,
        resolveClipCompSourceContentType,
      } = require('../clip_comp');
      const clipItems = b.items || b.sourceLibrary || jobSpec.order?.inputs?.items || [];
      ...
      const sourceContentType = resolveClipCompSourceContentType({
        contentType: b.contentType || resolvedContentType,
      ...
      applyClipCompProfileToJobSpec(jobSpec, {
        ...
        clipCount,
        sourceContentType,
        platforms: b.platforms || jobSpec.order?.publish?.platforms || ['youtube'],
        scheduledAt: b.scheduledAt || null,
        clipsOnly: true,
      });
      try {
        const { mergeCompCreative } = require('../clip_comp_creative');
        const presetKey = b.featureConfig?.compose?.preset
          || b.compCreativePreset
          || b.compositionSpec?.compCreativePreset
          || 'fableflow_speed';
        const compCreative = mergeCompCreative({
          ...(b.compositionSpec?.compCreative || {}),
          preset: presetKey,
        });
        ...
        console.warn('[v1/jobs] compCreative merge skipped:', creativeErr.message);
      }
      if (!jobSpec.staging) {
        try {
          const { resolveActivePortals } = require('../job_spec');
      ...
      try {
        ...
        const _storeReport = (pk, result) => {
          ...
          if (pk === 'thumbnail_ext' && result.thumbnail) {
            ...
            const _bestUrl = _thumb.designedUrl ||
...
router.post('/account/api-keys', requireOperateTier, requireApiInviteApproved, async (req, res) => {
  const { name } = req.body || {};
  try {
...
//   POST /v1/operator/jobs/:id/sendback    — parse directions → assembly + optional auto-redo
//   POST /v1/operator/jobs/:id/retry       — redo job with optional directions
...
const _REVISION_ELIGIBLE = new Set(['complete', 'staged', 'held', 'processing', 'operator_review']);

// POST /v1/operator/jobs/:id/sendback — operator sends revision directions to assembly
router.post('/operator/jobs/:id/sendback', _requireOperator, async (req, res) => {
  try {
    ...
    const spec = _parseSpec(row);
    const operatorId = req.user.id;
    const { directions, categories = [], autoRedo = true } = req.body || {};
    const text = String(directions || '').trim();
    if (!text) {
      return res.status(400).json({ error: 'directions_required', message: 'Provide directions for what to change.' });
    }

    const status = spec.status || row.status || '';
    const hasOutput = !!(spec.state?.savedOutputs?.r2VideoUrl || spec.outputUrl);
    if (!hasOutput || !_REVISION_ELIGIBLE.has(status)) {
      return res.status(409).json({ error: 'not_reviewable', status });
    }
    ...
    if (autoRedo !== false) {
      prepareJobForRetry(spec, { operatorId, action: 'operator_sendback', note: text.slice(0, 500) });
    ...
    logError('CPD1231_OP_SENDBACK_FAIL', err, { jobId: req.params.id });
    res.status(500).json({ error: 'sendback_failed' });
  }
});

// POST /v1/operator/jobs/:id/retry — redo pipeline with optional directions
router.post('/operator/jobs/:id/retry', _requireOperator, async (req, res) => {
  try {
    const jobId = req.params.id;
    const row = await db.loadJobRow(jobId);
    if (!row) return res.status(404).json({ error: 'not_found' });
    ...
    if (!RETRYABLE_STATUSES.has(currentStatus)) {
      return res.status(409).json({ error: 'not_retryable', status: currentStatus });
    }

    const { directions, categories = [] } = req.body || {};
    const directionsText = String(directions || spec.state?.customerRevision?.feedback || spec.state?.operatorRevision?.directions || '').trim();
    ...
    res.status(202).json({
      ...
      directionsApplied: !!directionsText,
    });
  } catch (err) {
    logError('CPD1231_OP_RETRY_FAIL', err, { jobId: req.params.id });
