...
function resolveProductionProfileAndContentType(body) {
  ...
  if (b.contentType && NON_ALIAS_CONTENT_TYPES.has(_normalizeLegacyContentType(b.contentType) || b.contentType)) {
    ...
    // CPD-486: if caller explicitly sets format:longform, honour it — don't force vertical_reel
    // just because contentType is 'clips'. Longform clip compilations (youtube_deep_dive,
    // reaction_cut) should produce 16:9 output, not 9:16.
    let profile = b.productionProfile || LEGACY_CONTENT_TYPE_TO_PRODUCTION_PROFILE[base] || 'broadcast_desk';
    if ((b.format === 'longform' || b.format === 'long') && profile === 'vertical_reel') {
...
function resolveTemplateIdFromBody(b, contentTypeStr) {
  ...
  if (b.format === 'long'  || b.format === 'longform') return 'long-form';
