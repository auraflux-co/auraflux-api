...
router.post('/jobs', async (req, res) => {
  ...
  try {
    ...
    if (streamerVal) jobSpec.order.inputs.streamer = streamerVal;
      ...
      try {
        ...
        runPortalSequence({
          ...
          onPortalPass:   async (pk, result) => {
            ...
            if (pk === 'portal1' && !_spec.assembledPath) {
              ...
              if (_hasSourceClips && !_spec.assembledPath) {
                try {
                  ...
                  // CPD-431: Apply post-processing effects (loudnorm, LUT, square crop, etc.)
                  // AFTER assembly but BEFORE chrome overlay so branding lands on the
                  // processed video. portrait blur-pad skipped if assembly already applied
                  // the vertical crop (layoutPortraitApplied flag is set by assembly_service).
                  // Non-fatal — returns original path on any failure.
                  if (_spec.assembledPath) {
                    try {
                      const { applyPostProcessingEffects: _applyPP } = require('../assembly_postprocess');
                      await _applyPP(_spec, _spec.assembledPath, (msg) => console.log(`[v1] ${_jobId}: ${msg}`));
                      // If loudnorm, square crop, or other audio/visual effects ran, re-upload
                      // the processed file to R2 so outputUrl reflects the final quality.
                      const _ppModified = _spec.state?.savedOutputs?.loudnormApplied ||
                        _spec.state?.savedOutputs?.layoutSquareApplied ||
                        (_spec.state?.savedOutputs?.postProcessEffects?.length > 0);
                      if (_ppModified) {
                        try {
                          const { uploadToR2: _ppR2 } = require('../storage');
                          const _ppFileName = `${Date.now()}_assembled_${_jobId}_fx.mp4`;
                          const _ppUrl = await _ppR2(_spec.assembledPath, _ppFileName, { folder: `outputs/${_jobId}` });
                          _spec.assembledVideoUrl = _ppUrl;
                          if (!_spec.state) _spec.state = {};
                          if (!_spec.state.savedOutputs) _spec.state.savedOutputs = {};
                          _spec.state.savedOutputs.r2VideoUrl = _ppUrl;
                          console.log(`[v1] ${_jobId}: post-processing applied and re-uploaded to R2`);
                        } catch (_ppR2Err) {
                          console.warn(`[v1] ${_jobId}: postprocess R2 re-upload failed (non-fatal) — ${_ppR2Err.message}`);
                        }
                      }
                    } catch (_ppErr) {
                      console.warn(`[v1] ${_jobId}: postprocess effects failed (non-fatal) — ${_ppErr.message}`);
                    }
                  }
                  ...
                  if (_spec.addOns?.branding?.active === true && !_spec.addOns?.tts?.active && _spec.assembledPath) {
                    try {
                      ...
                      const _showName   = _spec.designSpec?.chrome?.name || _spec.designSpec?.chrome?.showName || _spec.order?.showName || 'AuraFlux';
                      ...
                      const _portraitDone1 = _spec.state?.savedOutputs?.layoutPortraitApplied;
                      const _dur1 = (_spec.clipDurations || []).reduce((s, d) => s + (d || 0), 0) || (_spec.durationMins || 0) * 60;
                      await _applyChr(_spec.assembledPath, _chromedPath, { showName: _showName, isVertical: _isVert, streamerName: _streamer, needsPortraitReframe: _isVert && !_portraitDone1, durationSecs: _dur1 }); // CPD-479 CPD-481
                      require('fs').renameSync(_chromedPath, _spec.assembledPath);
                      if (!_spec.state) _spec.state = {};
                      _spec.state.chromeApplied = true;
                      if (_isVert && !_portraitDone1) { if (!_spec.state.savedOutputs) _spec.state.savedOutputs = {}; _spec.state.savedOutputs.layoutPortraitApplied = true; }
            ...
            if (pk === 'tts_ext' && _spec.assembledPath) {
              ...
              if (_ttsAudio) {
                ...
                if (mixed) {
                  ...
                  // CPD-426: Gate on addOns.branding — skip chrome when explicitly false.
                  if (_spec.addOns?.branding?.active === false) {
                    console.log(`[v1] ${_jobId}: branding disabled — skipping chrome overlay (TTS path)`);
                    if (!_spec.state) _spec.state = {};
                    _spec.state.chromeApplied = false;
                    _spec.state.chromeSkipped = true;
                  }
                  if (_spec.addOns?.branding?.active !== false) try {
                    const { applyChrome } = require('../assembly_service');
                    const chromedPath = mixed.replace('.mp4', '_chrome.mp4');
                    const _showName = _spec.designSpec?.chrome?.name || _spec.designSpec?.chrome?.showName ||
                    ...
                    const _isVert = _spec.productionProfile === 'vertical_reel' ||
                      (_spec.format === 'short' && (_spec.order?.publish?.platforms || [])
                        .some((p) => ['tiktok', 'instagram'].includes(String(p).toLowerCase())));
                    const _portraitDone2 = _spec.state?.savedOutputs?.layoutPortraitApplied;
                    const _dur2 = (_spec.clipDurations || []).reduce((s, d) => s + (d || 0), 0) || (_spec.durationMins || 0) * 60;
                    await applyChrome(mixed, chromedPath, { showName: _showName, isVertical: _isVert, streamerName: _streamerName, needsPortraitReframe: _isVert && !_portraitDone2, durationSecs: _dur2 }); // CPD-479 CPD-481
                    const fs = require('fs');
                    fs.renameSync(chromedPath, mixed);
                    if (!_spec.state) _spec.state = {};
                    if (_isVert && !_portraitDone2) { if (!_spec.state.savedOutputs) _spec.state.savedOutputs = {}; _spec.state.savedOutputs.layoutPortraitApplied = true; }
                    _spec.state.chromeApplied = true;
                    _spec.assembledPath = mixed;
                    _spec.outputPath    = mixed;
            ...
            // CPD-426: Skip when addOns.branding is explicitly false.
            if (pk === 'portal3a' && _spec.assembledPath && !(_spec.state?.chromeApplied) && _spec.addOns?.branding?.active !== false) {
              try {
                const { applyChrome } = require('../assembly_service');
                ...
                const _isVert = _spec.productionProfile === 'vertical_reel' ||
                  (_spec.format === 'short' && (_spec.order?.publish?.platforms || [])
                    .some((p) => ['tiktok', 'instagram'].includes(String(p).toLowerCase())));
                const _portraitDone3 = _spec.state?.savedOutputs?.layoutPortraitApplied;
                const _dur3 = (_spec.clipDurations || []).reduce((s, d) => s + (d || 0), 0) || (_spec.durationMins || 0) * 60;
                await applyChrome(_spec.assembledPath, chromedPath, { showName: _showName, isVertical: _isVert, streamerName: _streamerName2, needsPortraitReframe: _isVert && !_portraitDone3, durationSecs: _dur3 }); // CPD-479 CPD-481
                const fs = require('fs');
                fs.renameSync(chromedPath, _spec.assembledPath);
                _spec.state.chromeApplied = true;
                if (_isVert && !_portraitDone3) { if (!_spec.state.savedOutputs) _spec.state.savedOutputs = {}; _spec.state.savedOutputs.layoutPortraitApplied = true; }
          ...
          onJobComplete:  async (allResults) => {
            ...
            // Set outputUrl from R2 before grading so output_exists check passes.
            Object.assign(_spec, {
              status:    'complete',
              outputUrl: _spec.outputUrl || _spec.state?.savedOutputs?.r2VideoUrl || _spec.assembledVideoUrl || null,
              updatedAt: new Date().toISOString(),
            });
            // CPD-422: Grade AFTER status='complete' and outputUrl are set so grader sees
            // the correct terminal state (not the mid-flight 'running' state it was called in before).
            try {
              const { gradeJob } = require('../services/job_grader');
              const _gr = gradeJob(_spec);
              const _gradeAt = new Date().toISOString();
              Object.assign(_spec, {
                grade:       _gr.grade,
                gradeResult: { grade: _gr.grade, passed: _gr.passed, gaps: _gr.gaps, warnings: _gr.warnings, summary: _gr.summary, gradedAt: _gradeAt },
              });
              console.log(`[v1] ${_jobId}: grade ${_gr.grade}/100 — ${_gr.passed ? 'PASSED ✅' : 'GAPS: ' + _gr.gaps.map((g) => g.checkId).join(', ')}`);

              // CPD-431: Operator review queue — jobs < 100/100 go to operator_review status.
              // Customers never see a sub-100 grade or are asked to fix anything themselves.
              // Operator resolves issues (auto-remediation or manual fix) then re-grades and
              // sets status back to 'complete' via PUT /admin/jobs/:id/release.
              if (!_gr.passed) {
                _spec.status = 'operator_review';
                const _gpt4oNotes = _spec.state?.savedOutputs?.gpt4oQA || null;
                const _autoFixable = _gpt4oNotes?.fixableIssues?.filter((i) => i.autoFixable) || [];
                const _manualFix   = _gpt4oNotes?.fixableIssues?.filter((i) => !i.autoFixable) || [];
                if (!_spec.state) _spec.state = {};
                _spec.state.operatorReview = {
                  enteredAt:     _gradeAt,
                  grade:         _gr.grade,
                  gaps:          _gr.gaps,
                  warnings:      _gr.warnings,
                  gpt4oScore:    _gpt4oNotes?.score || null,
                  autoFixable:   _autoFixable,
                  manualFix:     _manualFix,
                  creativeNotes: _gpt4oNotes?.creativeNotes || [],
                };
                console.log(`[v1] ${_jobId}: grade ${_gr.grade}/100 → operator_review (${_gr.gaps.length} gaps, ${_autoFixable.length} auto-fixable)`);
              }
            } catch (_ge) {
