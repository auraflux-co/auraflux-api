...
router.post('/jobs', async (req, res) => {
  ...
  try {
    ...
    // Accept topic/tone at top level (e.g. {"topic":"...", "tone":"..."}) OR nested under order.
    const orderFields = b.order || {};
    const topic = b.topic || orderFields.topic;
    const tone  = b.tone  || orderFields.tone;
    if (topic)    jobSpec.order.topic    = topic;
    if (tone)     jobSpec.order.tone     = tone;
...
function _adaptLegacyPortal(mod, jobSpec) {
  const portalKey = mod.portalKey || 'unknown';
  return {
    runWorker: async (_opts) => {
      try {
        return await mod.run(jobSpec);
      } catch (err) {
        logError('CPD126_PORTAL_UNHANDLED_THROW', err, { jobId: jobSpec?.jobId, portalKey });
        return {
          passed:       false,
          failReason:   `Portal threw: ${err.message}`,
          outcome:      'hard_fail',
          score:        0,
