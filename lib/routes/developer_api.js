...
router.post('/jobs', async (req, res) => {
  ...
  try {
    ...
    await db.saveJob(jobSpec.jobId, jobSpec);
    ...
    setImmediate(async () => {
      try {
        ...
        runPortalSequence({
          ...
          onPortalStart:  (pk) => console.log(`[v1] ${_jobId}: portal ${pk} started`),
          onPortalPass:   (pk) => console.log(`[v1] ${_jobId}: portal ${pk} passed`),
          onPortalFail:   (pk, r) => logError('CPD126_PORTAL_FAIL', r?.reason || 'non-compliant', { jobId: _jobId, pk }),
          onJobComplete:  () => console.log(`[v1] ${_jobId}: pipeline complete`),
          onJobFailed:    (fp, r) => logError('CPD126_JOB_FAILED', r?.reason || fp, { jobId: _jobId }),
          persistJobStatus: ({ portalKey: pKey, phase }) => {
            db.saveJob(_jobId, { ...jobSpec, status: phase, currentPortal: pKey, updatedAt: new Date().toISOString() })
              .catch((e) => console.error('[v1] persistJobStatus failed:', e.message));
          },
        });
      } catch (err) {
