  ...
  try {
    ...
    setImmediate(async () => {
      try {
        ...
        // CPD-181: EXTRACT flow — if source URL is a Twitch VOD, download and split into clips
        // before running the portal sequence. Replace sourceConfig.urls with extracted clip paths.
        {
          const _sourceUrls = _spec.sourceConfig?.urls || [];
          ...
          if (_isTwitchVod) {
            try {
              ...
              const _vodUrl = _sourceUrls.find((u) => /twitch\.tv\/videos\/\d+/.test(String(u)));
              const _extractedPaths = await extractVodClips(_vodUrl, {
                clipCount: 3,
                maxClipSecs: 60,
                jobId: _jobId,
              });
              // Replace sourceConfig with local clip paths so assembleForJob picks them up
        ...
        runPortalSequence({
          ...
          onPortalPass:   async (pk, result) => {
            ...
            if (pk === 'tts_ext' && _spec.assembledPath) {
              ...
              if (_ttsAudio) {
                const { mixTtsIntoVideo } = require('../assembly_service');
                try {
                  const mixed = await mixTtsIntoVideo(_spec.assembledPath, _ttsAudio, _jobId);
                  if (mixed) {
                    ...
                    try {
                      ...
                      await applyChrome(mixed, chromedPath, { showName: _showName, isVertical: _isVert });
                      const fs = require('fs');
            ...
            // CPD-185: For jobs without TTS (tts_ext not ordered), apply chrome after assembly
            // so the output video always has branding, even without voiceover.
            if (pk === 'portal3a' && _spec.assembledPath && !(_spec.extensions?.tts_ext?.ordered)) {
              try {
                const { applyChrome } = require('../assembly_service');
                ...
                const _isVert = _spec.productionProfile === 'vertical_reel' ||
                  (_spec.format === 'short' && (_spec.order?.publish?.platforms || [])
                    .some((p) => ['tiktok', 'instagram'].includes(String(p).toLowerCase())));
                await applyChrome(_spec.assembledPath, chromedPath, { showName: _showName, isVertical: _isVert });
                const fs = require('fs');
