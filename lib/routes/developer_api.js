        ...
        runPortalSequence({
          ...
          onPortalPass:   async (pk, result) => {
            ...
            if (pk === 'portal1' && !_spec.assembledPath) {
              // Topic-only jobs (no source video clips) skip assembly and
              // video-dependent portals — the output is the script + metadata only.
              ...
              if (!_hasSourceClips) {
                console.log(`[v1] ${_jobId}: topic-only job — skipping assembly and video portals`);
                // Mark video-dependent portals as inactive so runPortalSequence skips them
                _spec.portals = _spec.portals || {};
                ['portal2', 'portal3a', 'portal1b'].forEach((vpk) => {
                  if (_spec.portals[vpk]) _spec.portals[vpk].active = false;
                  else _spec.portals[vpk] = { active: false, reason: 'topic_only — no source video' };
                });
                await db.updateJobSpec(_jobId, _spec);
              } else {
                try {
