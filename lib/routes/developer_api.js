                ...
                try {
                  ...
                  if (_spec.addOns?.branding?.active === true && !_spec.addOns?.tts?.active && _spec.assembledPath) {
                    try {
                      ...
                      // CPD-479: reframe only if portrait layout hasn't been applied yet
                      const _portraitDone1 = _spec.state?.savedOutputs?.layoutPortraitApplied;
                      await _applyChr(_spec.assembledPath, _chromedPath, { showName: _showName, isVertical: _isVert, streamerName: _streamer, needsPortraitReframe: _isVert && !_portraitDone1 });
