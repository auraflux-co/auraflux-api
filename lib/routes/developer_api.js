...
router.get('/account', async (req, res) => {
  try {
    ...
    res.json({
      ...
      credits: { balance: (credits?.includedRemaining || 0) + (credits?.packCredits || 0), used: credits?.includedUsed || 0, included: credits?.creditsIncluded || 0, packs: credits?.packCredits || 0 },
      rateLimits: limits,
    });
  } catch (err) {
