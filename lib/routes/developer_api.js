...
router.post('/jobs', async (req, res) => {
  ...
  try {
    ...
    // Mirror jobs_c1.js: extract post-processing fields from b.addOns so assembly_postprocess
    // and the pipeline can apply them. createJobSpec only maps extension add-ons (heygen, tts,
    // branding, etc.) — captions, colorGrade, effects, and audioOpts are stored directly.
    const _ao = b.addOns || {};
    if (_ao.captions   && typeof _ao.captions   === 'object' && !jobSpec.captions)   jobSpec.captions   = _ao.captions;
    if (_ao.colorGrade && typeof _ao.colorGrade === 'object' && !jobSpec.colorGrade) jobSpec.colorGrade = _ao.colorGrade;
    if (_ao.effects    && typeof _ao.effects    === 'object' && !jobSpec.effects)    jobSpec.effects    = _ao.effects;
    if (_ao.audio      && typeof _ao.audio      === 'object' && !jobSpec.audioOpts)  jobSpec.audioOpts  = _ao.audio;
    // Also accept top-level body fields (b.captions, b.effects, etc.) as fallback
