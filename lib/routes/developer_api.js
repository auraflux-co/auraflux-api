          ...
          onPortalPass:   async (pk, result) => {
                ...
                try {
                  ...
                  if (_spec.addOns?.branding?.active === true && !_spec.addOns?.tts?.active && _spec.assembledPath) {
                    try {
                      ...
                      await _applyChr(_spec.assembledPath, _chromedPath, { showName: _showName, isVertical: _isVert, streamerName: _streamer, needsPortraitReframe: _isVert && !_portraitDone1 }); // CPD-479
              ...
              if (_ttsAudio) {
                ...
                if (mixed) {
                  ...
                  if (_spec.addOns?.branding?.active !== false) try {
                    ...
                    await applyChrome(mixed, chromedPath, { showName: _showName, isVertical: _isVert, streamerName: _streamerName, needsPortraitReframe: _isVert && !_portraitDone2 }); // CPD-479
                    const fs = require('fs');
            ...
            if (pk === 'portal3a' && _spec.assembledPath && !(_spec.state?.chromeApplied) && _spec.addOns?.branding?.active !== false) {
              try {
                ...
                await applyChrome(_spec.assembledPath, chromedPath, { showName: _showName, isVertical: _isVert, streamerName: _streamerName2, needsPortraitReframe: _isVert && !_portraitDone3 }); // CPD-479
                const fs = require('fs');
