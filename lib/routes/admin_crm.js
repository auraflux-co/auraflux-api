...
router.get('/admin/permissions', requireAuth, requireRole(ROLES.SUPERADMIN), async (req, res) => {
  try {
    ...
    const accounts = await Promise.all(ownerRows.map(async ({ account_id: accountId }) => {
      ...
      // Fetch owner's first/last name from Clerk for a human-readable identifier
      let ownerName = null;
      if (ownerMember?.member_id) {
        try {
          const cu = await clerk.users.getUser(ownerMember.member_id);
          const fn = cu.firstName?.trim();
          const ln = cu.lastName?.trim();
          if (fn || ln) ownerName = [fn, ln].filter(Boolean).join(' ');
        } catch { /* degrade */ }
      }
...
router.post('/admin/warp/:userId', requireAuth, requireRole(ROLES.SUPERADMIN), async (req, res) => {
  ...
  // Optional: brandId pre-selects a sub-brand in the warped session
  const { brandId, redirectTo } = req.body || {};
  try {
    const clerk = getClerk();
    ...
    await logAudit(req.user.id, 'warp', userId, { token: 'created', brandId: brandId || null });

    // Append brandId + redirectTo as query params on the sign-in URL so the
    // app can pick them up after the Clerk sign-in completes and pre-select
    // the correct brand context automatically.
    let url = tokenResp.url;
    const extras = new URLSearchParams();
    if (brandId)    extras.set('warp_brand_id', brandId);
    if (redirectTo) extras.set('warp_redirect', redirectTo);
    if (extras.toString()) {
      url += (url.includes('?') ? '&' : '?') + extras.toString();
    }

    res.json({ ok: true, url, token: tokenResp.token });
  } catch (err) {
...
router.get('/admin/crm/:accountId', requireAuth, requireRole({ minLevel: ROLES.SUPERADMIN }), async (req, res) => {
  ...
  try {
    ...
    const [
      ...
      // Sub-brands + their connected YouTube channels
      pool.query(
        `SELECT b.id, b.name, b.created_at,
                pot.platform_handle AS youtube_handle,
                pot.created_at      AS youtube_connected_at
           FROM brands b
           LEFT JOIN platform_oauth_tokens pot
             ON pot.brand_id = b.id AND pot.platform = 'youtube'
    ...
    const brandsRows  = (val(brandsResult)?.rows || []);
    ...
    res.json({
      ...
      brands: brandsRows.map((r) => ({
        ...
        youtubeConnectedAt: r.youtube_connected_at || null,
      })),
    });
  } catch (err) {
...
const RENDER_SERVICES = [
  ...
  { id: 'crn-d8g650t8nd3s7399gm10', name: 'auraflux-pipeline-review', type: 'cron' },
  // auraflux-app → Vercel (app.auraflux.co), not Render
];
...
// ── POST /admin/sources/bypass ───────────────────────────────────────────────
// Set a Twitch/Kick source handle on a customer account without OAuth.
// Body: { customerId, platform, handle }          — single
//   or: { entries: [{ customerId, platform, handle }, ...] } — bulk (≤50)
//
// Platform map: twitch → twitchLogin  |  kick → kickUsername
const SOURCE_PLATFORM_KEY = { twitch: 'twitchLogin', kick: 'kickUsername' };

router.post(
  '/admin/sources/bypass',
  requireAuth, requireRole(ROLES.SUPERADMIN),
  async (req, res) => {
    const body = req.body || {};

    // Normalise single vs bulk
    const entries = body.entries
      ? body.entries
      : [{ customerId: body.customerId, platform: body.platform, handle: body.handle }];

    if (!Array.isArray(entries) || entries.length === 0 || entries.length > 50) {
      return res.status(400).json({ ok: false, error: 'entries must be 1–50 items' });
    }

    const results = [];
    for (const entry of entries) {
      const { customerId, platform, handle } = entry || {};
      if (!customerId || !platform || !handle) {
        results.push({ customerId, ok: false, error: 'missing customerId / platform / handle' });
        continue;
      }
      const channelKey = SOURCE_PLATFORM_KEY[platform.toLowerCase()];
      if (!channelKey) {
        results.push({ customerId, ok: false, error: `unknown platform "${platform}" — use twitch or kick` });
        continue;
      }
      ...
      try {
        const result = await pgQuery(
          ...
          [JSON.stringify({ [channelKey]: cleanHandle }), customerId]
        );
        if (!result.rows.length) {
          results.push({ customerId, ok: false, error: 'no active plan found for this customerId' });
        } else {
          results.push({ customerId, platform, handle: cleanHandle, ok: true, sourceChannels: result.rows[0].source_channels });
        }
      } catch (err) {
        logError('ADMIN_SOURCE_BYPASS', err);
