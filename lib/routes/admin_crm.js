 ...
 */
router.get('/admin/crm', requireAuth, requireRole({ minLevel: ROLES.ADMIN }), async (req, res) => {
  try {
    const pool = db.getPool();
    ...
    const accounts = await Promise.all(ownerRows.map(async (row) => {
      ...
      // Clerk lookup — grab email + publicMetadata.planTier as a fallback
      // for accounts that have no client_plans row yet (e.g. E2E test accounts).
      try {
        const clerkUser = await clerk.users.getUser(row.account_id);
        ownerEmail    = clerkUser.emailAddresses?.[0]?.emailAddress || ownerEmail;
        clerkPlanTier = normaliseTier(clerkUser.publicMetadata?.planTier) || null;
      } catch { /* degrade */ }
 ...
 * Operator or admin.
 */
router.get('/admin/crm/:accountId', requireAuth, requireRole({ minLevel: ROLES.ADMIN }), async (req, res) => {
  const { accountId } = req.params;
  const pool  = db.getPool();
  const clerk = getClerk();
...
router.post(
  ...
  requireAuth, requireRole({ minLevel: ROLES.ADMIN }),
  async (req, res) => {
    const { accountId } = req.params;
    const { body }      = req.body || {};
 ...
 */
router.get('/admin/system-health', requireAuth, requireRole(ROLES.ADMIN), async (req, res) => {
  try {
    const [
      renderServices,
      nrIncidents,
 ...
 */
router.get('/admin/activity-overview', requireAuth, requireRole(ROLES.ADMIN), async (req, res) => {
  const pool  = db.getPool();
  const clerk = getClerk();
