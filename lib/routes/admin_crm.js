 ...
 */
router.get('/admin/users', requireAuth, requireRole(ROLES.ADMIN), async (req, res) => {
  const clerk = getClerk();
  const pool  = db.getPool();
  const PAGE  = 500;
  const MAX   = 2000;
...
router.patch('/admin/users/:userId/role', requireAuth, requireRole(ROLES.ADMIN), async (req, res) => {
  const { userId } = req.params;
  const { role }   = req.body;
  const VALID      = ['customer', 'operator', 'admin'];
