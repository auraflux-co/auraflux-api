...
const { getVoiceGuide } = require('../script_gen');
...
router.post('/nba/scrape-game-highlight', async (req, res) => {
  ...
  try {
    ...
    if (articleVideos.length) {
      ...
      if (hlUrl) {
        console.log(
          `[nba-scrape] ✅ Portal 0 PASS: Game Highlights from article.video: "${highlight.headline}" (${highlight.duration}s) [${hlsUrl ? 'HLS' : 'direct MP4'}]`
        );
    ...
    if (!videos.length) {
      // Portal 0 FAIL: Puppeteer failed and API has no videos either
      return res.json({
        ok: false,
        gate0: 'fail',
    ...
    // Portal 0: Validate the selected URL is usable
    // Puppeteer already ran first and failed, so no further fallback is available.
    if (!videoUrl) {
      console.error(`[nba-scrape] Portal 0 FAIL: No usable video URL found for game ${gameId}`);
      return res.json({
        ok: false,
        gate0: 'fail',
    ...
    // Portal 0: Validate duration meets minimum threshold (30s for short-form, 10s for long-form)
    if (maxDuration > 0 && maxDuration < minDurationSecs) {
      console.warn(
        `[nba-scrape] Portal 0 WARN: Best video for game ${gameId} is only ${maxDuration}s — below ${minDurationSecs}s minimum (formType: ${formType || 'long'})`
      );
      return res.json({
        ok: false,
        gate0: 'fail',
    ...
    console.log(
      `[nba-scrape] ✅ Portal 0 PASS: Selected longest duration video: "${highestDurationVideo.headline || highestDurationVideo.title || 'Game Highlights'}" (${maxDuration}s)`
    );
...
router.get('/news/us-canada-videos', async (req, res) => {
  try {
    ...
    // ── Portal 0: Gemini recovery when scraper returns 0 videos ────────────────
    ...
    if (ajVideos.length === 0) {
      console.log(
        '[news/us-canada-videos] Portal 0: 0 videos — asking Gemini to select best candidates for retry...'
      );
      try {
        const [todayUrls, yestUrls] = await Promise.all([
        ...
        if (allUrls.length > 0) {
          ...
          if (match) {
            ...
            console.log(
              `[news/us-canada-videos] Portal 0 Gemini picked ${candidateUrls.length} candidates — retrying scrape...`
            );
            ajVideos = await scrapeAjNewsVideos(5, candidateUrls);
            console.log(`[news/us-canada-videos] Portal 0 retry: ${ajVideos.length} videos found`);
          }
        }
      } catch (e) {
        console.warn(`[news/us-canada-videos] Portal 0 Gemini recovery failed: ${e.message}`);
      }
    }
...
// POST /generate-full-script — canonical handler lives in server.js only.
// Do not re-register here: c0_sources mounts before server.js inline routes, so
// Express first-match would hit a broken copy missing createJobSpec / handleGenerateFullScript.
...
// ── GET /reddit/status ────────────────────────────────────────────
router.get('/reddit/status', (req, res) => {
  const RedditClient = require('../clients/reddit_client');
  res.json({ ok: true, ...RedditClient.configStatus() });
});

// ── Reddit OAuth (one-time refresh token setup) ───────────────────
const REDDIT_OAUTH_SCOPES = 'read';
const REDDIT_REDIRECT_URI = process.env.REDDIT_REDIRECT_URI
  || 'http://localhost:3000/reddit/oauth/callback';

router.get('/reddit/oauth/authorize', (req, res) => {
  const clientId = process.env.REDDIT_CLIENT_ID;
  if (!clientId) {
    return res.status(400).type('html').send(
      `<h1>Reddit OAuth</h1>
<p><strong>Easiest (Script app — no redirect needed in browser):</strong></p>
<ol>
<li><a href="https://www.reddit.com/prefs/apps">reddit.com/prefs/apps</a> → create app → type <strong>script</strong></li>
<li>Redirect URI: <code>http://localhost:8080</code> (Reddit requires a value; script apps ignore it)</li>
<li>Add CLIENT_ID + SECRET + your existing USERNAME/PASSWORD to .env</li>
<li>Set <code>REDDIT_USE_PULLPUSH=0</code> → deploy → <a href="/reddit/oauth/script-test">/reddit/oauth/script-test</a></li>
</ol>
<p><strong>Web app (refresh token):</strong> redirect must match <code>REDDIT_REDIRECT_URI</code> exactly.
If Reddit rejects <code>localhost:3000</code>, try <code>http://127.0.0.1:3000/reddit/oauth/callback</code> in both Reddit prefs and .env.</p>`,
    );
  }
...
router.get('/reddit/oauth/callback', async (req, res) => {
  const code = req.query.code;
  const err = req.query.error;
  if (err) {
    return res.status(400).send(`<h1>Reddit OAuth denied</h1><p>${err}</p>`);
  }
  if (!code) {
    return res.status(400).send('<h1>Missing code</h1><p>No authorization code in callback.</p>');
  }
  try {
    const RedditClient = require('../clients/reddit_client');
    const client = new RedditClient({ usePullpush: false });
    const tok = await client.exchangeAuthorizationCode(code, REDDIT_REDIRECT_URI);
    const refresh = tok.refresh_token || '';
    const html = `<!DOCTYPE html><html><head><title>Reddit OAuth OK</title></head><body style="font-family:monospace;padding:24px;background:#111;color:#eee;">
<h1 style="color:#c7af4f;">Reddit OAuth — success</h1>
<p>Add to <code>~/cwn-c0/.env</code>:</p>
<pre style="background:#000;padding:12px;border:1px solid #333;">REDDIT_REFRESH_TOKEN=${refresh || '(none — use script app + password grant)'}
REDDIT_USE_PULLPUSH=0</pre>
<p>Then: <code>bash scripts/deploy_c0.sh</code></p>
<p>Test: <code>curl http://localhost:3000/reddit/status</code> → mode oauth</p>
</body></html>`;
    res.type('html').send(html);
  } catch (e) {
    res.status(500).send(`<h1>Token exchange failed</h1><pre>${e.message}</pre>`);
  }
});

// Script app — password grant test (no browser redirect). Use when Reddit rejects localhost callback URLs.
router.get('/reddit/oauth/script-test', async (req, res) => {
  const RedditClient = require('../clients/reddit_client');
  const status = RedditClient.configStatus();
  if (!status.hasSecret || !process.env.REDDIT_CLIENT_ID) {
    return res.status(400).json({
      ok: false,
      error: 'Set REDDIT_CLIENT_ID and REDDIT_CLIENT_SECRET (script app from reddit.com/prefs/apps)',
      status,
    });
  }
  if (!status.hasPassword) {
    return res.status(400).json({
      ok: false,
      error: 'Script app needs REDDIT_USERNAME + REDDIT_PASSWORD in .env',
      status,
    });
  }
  try {
    ...
    const raw = await client.listSubreddit('PublicFreakout', { sort: 'top', window: '24h', limit: 5 });
    ...
    res.json({
      ...
      ms: Date.now() - t0,
      posts: picks.length,
      hint: 'Set REDDIT_USE_PULLPUSH=0 in .env and redeploy to use official API for THE THREAD',
      sample: picks[0] ? { id: picks[0].id, title: picks[0].title?.slice(0, 80) } : null,
    });
  } catch (e) {
    res.status(500).json({
      ok: false,
      error: e.message,
      hint: 'Use app type "script" at reddit.com/prefs/apps. Redirect URI can be http://localhost:8080 (unused).',
      status,
    });
  }
});
...
// ── POST /reddit/stage ────────────────────────────────────────────
// Download post video → R2 (same pattern as /content-library/stage for Twitch clips).
router.post('/reddit/stage', async (req, res) => {
  try {
    const body = req.body || {};
    const { stageRedditPostToR2 } = require('../content_library/stage_reddit');
    const { getStagedClipByPlatformId, formatStagedClip } = require('../content_library/staged_store');
    const { attachPlaybackUrl } = require('../content_library/playback_url');
    const out = await stageRedditPostToR2(body, { force: !!body.force });
    const postId = body.postId || body.id;
    const row = getStagedClipByPlatformId('reddit', postId);
    const staged = row ? await attachPlaybackUrl(out, row) : out;
    res.json({ ok: true, staged, clip: staged, cached: !!out.cached });
  } catch (err) {
    res.status(500).json({ ok: false, error: err.message });
  }
});
...
// ── POST /wire/stage ──────────────────────────────────────────────
router.post('/wire/stage', async (req, res) => {
  try {
    const body = req.body || {};
    const { stageWireStoryToR2, wireClipId } = require('../content_library/stage_wire');
    const { getStagedClipByPlatformId } = require('../content_library/staged_store');
    const { attachPlaybackUrl } = require('../content_library/playback_url');
    const out = await stageWireStoryToR2(body, { force: !!body.force });
    const clipId = wireClipId(body);
    const row = getStagedClipByPlatformId('wire', clipId);
    const staged = row ? await attachPlaybackUrl(out, row) : out;
    res.json({ ok: true, staged, clip: staged, cached: !!out.cached });
  } catch (err) {
    res.status(500).json({ ok: false, error: err.message });
...
// ── GET /reddit/candidates ────────────────────────────────────────
// Top video posts from curated subs. Default comments=0 (fast list); load comments per thread via GET /reddit/thread/:postId.
router.get('/reddit/candidates', async (req, res) => {
  const RedditClient = require('../clients/reddit_client');
  const { resolveRedditTimeWindow } = require('../clients/reddit_client');
  const subs = (req.query.subs || 'PublicFreakout,InstantKarma,Whatcouldgowrong,videos')
    .split(',').map((s) => s.trim()).filter(Boolean);
  const minScore = Math.max(100, parseInt(req.query.minScore, 10) || 500);
  const perSub = Math.max(1, Math.min(50, parseInt(req.query.perSub, 10) || 3));
  const rawLimit = Math.min(Math.max(perSub * 4, 25), 100);
  const fetchTimeoutMs = perSub >= 20 ? 120000 : 90000;
  const tw = resolveRedditTimeWindow(req.query.window || req.query.t || '24h');
  const includeComments = req.query.comments !== '0' && req.query.light !== '1';
    ...
    const subResults = await Promise.all(subs.map(async (sub) => {
      try {
        const raw = await Promise.race([
          client.listSubreddit(sub, { sort: 'top', window: tw.window, t: tw.t, limit: rawLimit }),
          new Promise((_, reject) => setTimeout(() => reject(new Error('sub fetch timeout')), fetchTimeoutMs)),
        ]);
        const picks = client.filterVideoCandidates(raw, { minScore }).slice(0, perSub);
        const bundles = [];
        for (const p of picks) {
          if (includeComments) {
            try {
              bundles.push(await client.buildPostBundle(p.id, { commentLimit: 30 }));
            } catch (e) {
              bundles.push({ ...p, topComments: [], commentError: e.message });
            }
          } else {
            bundles.push({ ...p, topComments: [] });
          }
        }
        return { sub, bundles, error: null };
      } catch (e) {
        return { sub, bundles: [], error: e.message };
      }
    }));
    const subErrors = [];
    for (const { sub, bundles, error } of subResults) {
      if (error) subErrors.push({ sub, error });
      bySubreddit[sub] = bundles;
      posts.push(...bundles);
    }
    res.json({
      ok: true,
      source: client.dataSource || RedditClient.resolveDataSource(),
      window: tw.window,
      subs,
      perSub,
      minScore,
      comments: includeComments,
      subErrors: subErrors.length ? subErrors : undefined,
...
// ── GET /reddit/thread/:postId ──────────────────────────────────
// Full post + top comments (lazy load when operator selects a thread).
router.get('/reddit/thread/:postId', async (req, res) => {
  const RedditClient = require('../clients/reddit_client');
  const postId = String(req.params.postId || '').trim();
  if (!postId) return res.status(400).json({ ok: false, error: 'postId required' });
  const commentLimit = Math.max(5, Math.min(50, parseInt(req.query.commentLimit, 10) || 30));
  const permalink = String(req.query.permalink || req.query.postUrl || '').trim();
  try {
    const client = new RedditClient();
    const bundle = await client.buildPostBundle(postId, { commentLimit, permalink });
    res.json({ ok: true, post: bundle });
  } catch (err) {
    res.status(500).json({ ok: false, error: err.message });
  }
});
