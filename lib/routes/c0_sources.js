...
// Top video posts from curated subs + OP + top comments (PullPush or OAuth).
router.get('/reddit/candidates', async (req, res) => {
  ...
  try {
    for (const sub of subs) {
      const raw = await client.listSubreddit(sub, { sort: 'top', window: tw.window, t: tw.t, limit: 25 });
      ...
      for (const p of picks) {
        try {
          ...
          posts.push(bundle);
          bySubreddit[sub].push(bundle);
        } catch (e) {
          const fallback = { ...p, topComments: [], commentError: e.message };
          posts.push(fallback);
          bySubreddit[sub].push(fallback);
        }
      }
    }
    res.json({
