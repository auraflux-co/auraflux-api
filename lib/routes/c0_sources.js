...
router.get('/reddit/candidates', async (req, res) => {
  ...
  const perSub = Math.max(1, Math.min(8, parseInt(req.query.perSub, 10) || 3));
    ...
    const subResults = await Promise.all(subs.map(async (sub) => {
      try {
        const raw = await Promise.race([
          client.listSubreddit(sub, { sort: 'top', window: tw.window, t: tw.t, limit: 25 }),
          new Promise((_, reject) => setTimeout(() => reject(new Error('sub fetch timeout')), 90000)),
        ]);
