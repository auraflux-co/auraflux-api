...
router.get('/reddit/oauth/authorize', (req, res) => {
  ...
  if (!clientId) {
    return res.status(400).send(
      '<h1>Reddit OAuth</h1><p>Set REDDIT_CLIENT_ID and REDDIT_CLIENT_SECRET in .env first, then create a web app at '
      + '<a href="https://www.reddit.com/prefs/apps">reddit.com/prefs/apps</a> with redirect URI: '
      + `<code>${REDDIT_REDIRECT_URI}</code></p>`,
    );
  }
...
router.get('/reddit/candidates', async (req, res) => {
  ...
  try {
    const subResults = await Promise.all(subs.map(async (sub) => {
      try {
        const raw = await Promise.race([
          ...
          new Promise((_, reject) => setTimeout(() => reject(new Error('sub fetch timeout')), 40000)),
        ]);
    ...
    res.json({
      ok: true,
      source: client.usePullpush ? 'pullpush' : 'oauth',
      window: tw.window,
      subs,
      perSub,
      minScore,
