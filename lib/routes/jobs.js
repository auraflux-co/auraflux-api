...
const { updateJobPublishSchedule, updateJobScheduledStart } = require('../db');
...
// GET /job-cards — return in-flight job cards for C0 dashboard recovery
...
// GET /content-type-status — CPD-320: auth required (internal system state)
router.get('/content-type-status', requireAuth, (req, res) => {
...
// PUT /jobs/:id/schedule — set or update publish/start schedule (CPD-48 / CPD-118)
router.put('/jobs/:id/schedule', requireAuth, apiLimit, async (req, res) => {
  ...
  // scheduledStartAt — set/clear the pipeline start time (CPD-118)
  if (scheduledStartAt !== undefined) {
    if (scheduledStartAt === null) {
      try {
        await updateJobScheduledStart(id, null);
      } catch (err) {
        return res.status(500).json({ error: err.message });
      }
    } else {
      const ts = typeof scheduledStartAt === 'number' ? scheduledStartAt : Date.parse(scheduledStartAt);
      if (isNaN(ts)) return res.status(400).json({ error: 'scheduledStartAt must be a valid ISO date or epoch ms' });
      if (ts < Date.now()) return res.status(400).json({ error: 'scheduledStartAt must be in the future' });
      try {
        await updateJobScheduledStart(id, new Date(ts).toISOString());
      } catch (err) {
        return res.status(500).json({ error: err.message });
      }
    }
  ...
  // publishMode = 'immediate' — clear publish schedule
  try {
    await updateJobPublishSchedule(id, 'immediate', null);
    return res.json({ ok: true, jobId: id, publishMode: 'immediate', scheduledPublishAt: null, scheduledStartAt: null });
  } catch (err) {
    return res.status(500).json({ error: err.message });
  }
});
