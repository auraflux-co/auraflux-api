  ...
  if (stage === 'script_ready') {
    ...
    try {
      pipelineBus.emit('job:advance', {
        ...
        from: 'script_ready',
        to: 'gate1_forced',
        message: 'Gate 1 force-passed — video generation is now unlocked.',
      });
    } catch (_e) {}
    return res.json({
      ok: true,
...
router.put('/jobs/:id/schedule', requireAuth, apiLimit, (req, res) => {
  const { id } = req.params;
  ...
  if (publishMode === 'scheduled') {
    ...
    try {
      updateJobPublishSchedule(id, 'scheduled', ts);
  ...
  try {
    updateJobPublishSchedule(id, 'immediate', null);
