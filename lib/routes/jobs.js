...
router.get('/jobs', (req, res) => {
  ...
  const jobsWithGateStatus = actionableJobs.map((job) => {
    ...
    for (const id of candidates) {
      try { spec = getJobBySpec(id); if (spec) break; } catch (_e) {}
    }
...
router.delete('/job/:id', (req, res) => {
  ...
  try { fs.writeFileSync(JOBS_FILE, JSON.stringify(persistedJobs, null, 2)); } catch (e) {
...
// POST /job/:id/rollback
router.post('/job/:id/rollback', validateJobId, (req, res) => {
  const jobId = req.params.id;
  ...
  if (before === 'published') {
    ...
    try { pipelineBus.emit('job:rollback', { jobId, before: 'published', after: 'assembled', message: 'Publish record cleared — re-approve to re-publish.' }); } catch (_e) {}
    return res.json({ ok: true, jobId, before: 'published', after: 'assembled', message: 'Publish record cleared — re-approve to re-publish.' });
  }

  if (before === 'assembled') {
    delete card.assembledAt; delete card.finalUrl; delete card.outputPath;
    delete card.gate5; delete card._gate5Done; delete card._gate5Running;
    delete card._gate3Approved; delete card._gate3Rejected;
    // C0: card.heygen.videoJobs — C1+: card.videoJobs
    if (card.heygen?.videoJobs) card.heygen.videoJobs.forEach((vj) => { vj._url = null; });
    if (card.videoJobs)         card.videoJobs.forEach((vj) => { vj._url = null; });
    ...
    try { pipelineBus.emit('job:rollback', { jobId, before: 'assembled', after: 'all_sent', message: 'Assembly cleared — click REFRESH IDs then ASSEMBLE again.' }); } catch (_e) {}
  ...
  if (before === 'all_sent') {
    ...
    if (card.heygen?.videoJobs) card.heygen.videoJobs.forEach((vj) => { delete vj.video_id; });
    if (card.videoJobs)         card.videoJobs.forEach((vj) => { delete vj.video_id; });
    delete card.gate2;
    ...
    try { pipelineBus.emit('job:rollback', { jobId, before: 'all_sent', after: 'script_ready', message: 'Segment IDs cleared — edit script and re-send to video generation.' }); } catch (_e) {}
    return res.json({ ok: true, jobId, before: 'all_sent', after: 'script_ready', message: 'Segment IDs cleared — edit script and re-send to video generation.' });
  }
...
// POST /job/:id/advance
router.post('/job/:id/advance', validateJobId, (req, res) => {
  const jobId = req.params.id;
  const card  = persistedJobs[jobId];
  if (!card) return res.json({ ok: false, error: 'Job not found: ' + jobId });
  ...
  if (stage === 'script_ready') {
    ...
    try { pipelineBus.emit('job:advance', { jobId, from: 'script_ready', to: 'gate1_forced', message: 'Gate 1 force-passed — video generation is now unlocked.' }); } catch (_e) {}
    return res.json({ ok: true, jobId, before: 'script_ready', after: 'gate1_forced', message: 'Gate 1 force-passed — video generation is now unlocked.' });
  }
  ...
  if (stage === 'all_sent') {
    ...
    let forced = 0;
    videoJobs.forEach((vj) => { if (!vj._url && vj.video_id) { vj._forcedComplete = true; forced++; } });
    ...
    try { pipelineBus.emit('job:advance', { jobId, from: 'all_sent', to: 'gate2_forced', message: `Gate 2 force-passed — ${forced} segment(s) marked. Click REFRESH IDs to get real URLs, then ASSEMBLE.` }); } catch (_e) {}
  ...
  if (stage === 'assembled') {
    ...
    try { pipelineBus.emit('job:advance', { jobId, from: 'assembled', to: 'gate5_forced', message: 'Gate 5 force-passed — APPROVE & UPLOAD button is now unlocked.' }); } catch (_e) {}
...
// POST /job/:id/manual-segments/resume
router.post('/job/:id/manual-segments/resume', validateJobId, (req, res) => {
  const jobId = req.params.id;
  const card  = persistedJobs[jobId];
  ...
  if (stage === 'published') {
    return res.status(400).json({ ok: false, error: 'Job is published — POST /job/:id/rollback first, then resume.' });
  }
  ...
  if (stage !== 'awaiting_manual_segments' && stage !== 'all_sent') {
    return res.status(400).json({ ok: false, error: `Job is at stage "${stage}" — expected awaiting_manual_segments or all_sent` });
  }
  ...
  card.manualSegments = { ...(card.manualSegments || {}), status: 'resume_requested', resumedAt: new Date().toISOString() };
  card.stage = 'all_sent';
  saveJobCard(jobId, card);

  try {
    pipelineBus.emit('heygen:all_complete', { jobId, contentType: card.contentType || 'general', segmentUrls, card, segmentData: null });
    return res.json({ ok: true, jobId, message: 'Resume accepted — Gate 2 + assembly handoff emitted.', segmentCount: segmentUrls.length });
  } catch (e) {
...
// POST /job/:id/dismiss
router.post('/job/:id/dismiss', validateJobId, (req, res) => {
  const { id } = req.params;
...
// POST /job/:id/stuck
router.post('/job/:id/stuck', validateJobId, (req, res) => {
  const { id } = req.params;
...
// POST /job/:id/qa-confirm-generate
router.post('/job/:id/qa-confirm-generate', validateJobId, (req, res) => {
  const { id } = req.params;
  try {
...
router.get('/job-spec/:jobId', async (req, res) => {
  try {
    ...
    const { buildGateStatusSnapshot, validateGateContractConsistency } = require('../job_spec_contracts');
    const spec = await getJobSpec(req.params.jobId);
    if (!spec) return res.status(404).json({ error: 'Job spec not found' });
    res.json({ ok: true, jobSpec: spec, gateStatus: buildGateStatusSnapshot(spec), gateContractValidation: validateGateContractConsistency(spec) });
  } catch (e) {
