...
router.post('/job/:id/rollback', (req, res) => {
  ...
  if (before === 'all_sent') {
    ...
    try { pipelineBus.emit('job:rollback', { jobId, before: 'all_sent', after: 'script_ready', message: 'HeyGen IDs cleared — edit script and re-send to HeyGen.' }); } catch (_e) {}
...
router.post('/job/:id/advance', (req, res) => {
  ...
  if (stage === 'script_ready') {
    ...
    try { pipelineBus.emit('job:advance', { jobId, from: 'script_ready', to: 'gate1_forced', message: 'Gate 1 force-passed — SEND TO HEYGEN is now unlocked.' }); } catch (_e) {}
...
router.post('/job/:id/manual-segments/resume', (req, res) => {
  ...
  try {
    pipelineBus.emit('heygen:all_complete', { jobId, contentType: card.contentType || 'twitch', segmentUrls, card, segmentData: null });
