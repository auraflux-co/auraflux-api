'use strict';
/**
 * lib/routes/account.js — Dashboard account management (Better Auth JWT)
 *
 * Provides API key management for the dashboard at /account/api-keys.
 * Uses requireAuth so the dashboard can bootstrap and manage
...
const { requireApiInviteApproved, isApiAccessApproved } = require('../auth/api_invite');
...
router.get('/account/api-access', requireAuth, (req, res) => {
  res.json({
    ok: true,
    apiAccess: isApiAccessApproved(req.user) ? 'approved' : (req.user?.apiAccess || null),
    approved: isApiAccessApproved(req.user),
    inviteUrl: 'https://auraflux.co/contact?topic=api_invite',
  });
});

// ─── POST /account/api-keys ───────────────────────────────────────────────────
router.post('/account/api-keys', requireAuth, requireOperatePlan, requireApiInviteApproved, async (req, res) => {
  const { name } = req.body || {};
  if (!name || !name.trim()) {
...
router.patch('/account/source-channels', requireAuth, resolveBrandContext, async (req, res) => {
  ...
  try {
    if (req.brandId && req.user?.id) {
      const { ensureClientPlanForBrand } = require('../db/postgres');
      await ensureClientPlanForBrand(req.brandId, req.user.id, req.user.planTier || 'operate');
    }
    ...
    // CPD-1231: YouTube channel avatar for sub-brands (testing) when no logo uploaded yet
    if (update.youtubeHandle && req.brandId && req.user?.id) {
      const { syncBrandFromYouTube } = require('../services/brand_youtube_sync');
      syncBrandFromYouTube({
        brandId:       req.brandId,
        accountId:     req.user.id,
        youtubeHandle: update.youtubeHandle,
        force:         false,
      }).catch((err) => logError('ACCOUNT_YOUTUBE_AVATAR_SYNC_FAIL', err));
    }
...
// Permanently dismisses the setup checklist. Better Auth has no Clerk metadata;
// prefer setup_dismissed on user_profiles when migration 038 is applied.
router.post('/account/setup-status/dismiss', requireAuth, async (req, res) => {
  try {
    try {
      await query(
        `UPDATE user_profiles SET setup_dismissed = true, updated_at = NOW() WHERE account_id = $1`,
        [req.user.id],
      );
    } catch (colErr) {
      // Column may not exist yet — still succeed so UI can hide locally
      console.warn('[account] setup_dismissed column missing or update failed:', colErr.message);
    }
    res.json({ ok: true });
  } catch (err) {
