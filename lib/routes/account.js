...
//   firstJobSubmitted  — at least one row in jobs table for this customer
router.get('/account/setup-status', requireAuth, async (req, res) => {
  const customerId = req.user.id;
  try {
    const [channelsResult, connectedPlatforms, jobsResult] = await Promise.all([
      query(
        'SELECT source_channels FROM client_plans WHERE client_id = $1 AND active = TRUE LIMIT 1',
        [customerId]
      ),
      ...
      query(
        'SELECT 1 FROM jobs WHERE customer_id = $1 LIMIT 1',
        [customerId]
      ),
    ]);
