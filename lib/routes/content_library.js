...
const {
  ...
  MAX_VOD_LIMIT,
} = require('../content_library/pub_window');
...
router.get('/content-library/vods', requireAuth, resolveBrandContext, async (req, res) => {
  try {
    ...
    const discovery = parseVodListQuery(req.query);
    const { band, minDurationSec, maxDurationSec, sort, limit } = discovery;
    // Over-fetch raw rows so window/duration filters still fill `limit`.
    const fetchN = Math.min(Math.max(limit * 3, 50), MAX_VOD_LIMIT * 2);
    ...
    if (platform === 'twitch') {
      ...
      const videos = await client.getVideos(user.id, fetchN, { type: 'archive' });
      ...
      const vods = applyVodDiscoveryFilters(mapped, discovery);
      ...
      return res.json({
        ...
        window: band.pubWindow,
        windowLabel: band.label,
        sort,
        minDurationSec,
        maxDurationSec,
    ...
    const listed = await client.getRecentLongVideos(channel.id, {
      limit: Math.min(fetchN, MAX_VOD_LIMIT),
      minDurationSec,
      maxDurationSec,
      publishedAfter: band.publishedAfter,
    });
    const mapped = (listed.videos || []).map((v) => ({
      platform: 'youtube',
    ...
    // Duration already partially applied in client when min>0; we passed 0 then filter here.
    const vods = applyVodDiscoveryFilters(mapped, discovery);
    ...
    res.json({
      ...
      minDurationSec,
      maxDurationSec,
      scanned: listed.scanned || mapped.length,
