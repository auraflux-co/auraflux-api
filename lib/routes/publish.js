...
const { requireFields, validateBodySize, validateStringLength, validateJobId } = require('../validation');
...
router.get('/publish/upload-status', (req, res) => {
  ...
  res.json({ total: db.uploads.length, filtered: uploads.length, uploads: uploads.slice(0, limit) });
});
...
router.post('/publish', validateBodySize(), requireFields('jobId'), handlePublish);

// POST /generate-publish-copy
router.post('/generate-publish-copy', validateBodySize(), requireFields('jobId'), handleGeneratePublishCopy);
...
router.get('/publish/status', async (req, res) => {
  ...
  try {
    ...
    const response = await axios.get(`https://api.upload-post.com/api/uploadposts/status?${param}`, { headers: { Authorization: `Apikey ${UPLOADPOST_API_KEY}` } });
...
router.get('/publish/history', async (req, res) => {
  ...
  if (!UPLOADPOST_API_KEY) return res.status(400).json({ error: 'UPLOADPOST_API_KEY not set' });
  try {
    const response = await axios.get('https://api.upload-post.com/api/uploadposts/history?limit=20', { headers: { Authorization: `Apikey ${UPLOADPOST_API_KEY}` } });
    res.json(response.data);
  } catch (e) {
...
router.get('/publish/queue', async (req, res) => {
  ...
  if (!UPLOADPOST_API_KEY) return res.status(400).json({ error: 'UPLOADPOST_API_KEY not set' });
  try {
    const response = await axios.get(`https://api.upload-post.com/api/uploadposts/queue/settings?profile_username=${UPLOADPOST_PROFILE}`, { headers: { Authorization: `Apikey ${UPLOADPOST_API_KEY}` } });
    res.json(response.data);
  } catch (e) {
    res.status(500).json({ error: e.message });
  }
...
// POST /publish/queue
router.post('/publish/queue', validateBodySize(), async (req, res) => {
  const UPLOADPOST_API_KEY = process.env.UPLOADPOST_API_KEY;
  const UPLOADPOST_PROFILE = process.env.UPLOADPOST_PROFILE || '';
  if (!UPLOADPOST_API_KEY) return res.status(400).json({ error: 'UPLOADPOST_API_KEY not set' });
  try {
...
router.post('/publish/youtube', async (req, res) => {
  ...
  try {
    ...
    if (scheduledAt) { status.privacyStatus = 'private'; status.publishAt = new Date(scheduledAt).toISOString(); }

    console.log(`[youtube] Uploading ${filename} (${(fs.statSync(filePath).size / 1024 / 1024).toFixed(1)}MB)...`);
    const uploadRes = await youtube.videos.insert({
      part: ['snippet', 'status'],
      requestBody: {
        snippet: { title: title || filename, description: description || '', tags: tags || [], categoryId: '24', defaultLanguage: 'en', defaultAudioLanguage: 'en' },
        status,
      },
...
router.post('/publish/tiktok', async (req, res) => {
  ...
  try {
    ...
    await axios.put(upload_url, fileBuffer, { headers: { 'Content-Type': 'video/mp4', 'Content-Range': `bytes 0-${fileSize - 1}/${fileSize}`, 'Content-Length': fileSize }, maxBodyLength: Infinity });
...
router.post('/publish/instagram', async (req, res) => {
  ...
  if (!process.env.INSTAGRAM_ACCESS_TOKEN || !process.env.INSTAGRAM_ACCOUNT_ID)
    return res.status(400).json({ error: 'INSTAGRAM_ACCESS_TOKEN and INSTAGRAM_ACCOUNT_ID required' });
  const filePath = path.join(OUTPUT_DIR, path.basename(filename));
  if (!fs.existsSync(filePath)) return res.status(404).json({ error: 'File not found' });
  ...
  try {
    ...
    const containerResp = await axios.post(`${BASE}/${IG_ID}/media`, { video_url: driveUrl, caption: caption || '', media_type: 'REELS', access_token: IG_TOKEN });
    ...
    const publishResp = await axios.post(`${BASE}/${IG_ID}/media_publish`, { creation_id: containerId, access_token: IG_TOKEN });
