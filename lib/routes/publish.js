...
router.post('/publish/youtube', async (req, res) => {
  const planTier = req.body?.planTier || req.user?.planTier || 'operate';
  if (!isFeatureEnabled('publish.direct_youtube', planTier)) {
    return res.status(403).json({ error: 'publish.direct_youtube requires dwy+ plan tier', code: 'PLAN_GATE' });
  }
...
router.post('/publish/tiktok', async (req, res) => {
  const planTier = req.body?.planTier || req.user?.planTier || 'operate';
  if (!isFeatureEnabled('publish.direct_tiktok', planTier)) {
    return res.status(403).json({ error: 'publish.direct_tiktok requires dfy+ plan tier', code: 'PLAN_GATE' });
  }
...
router.post('/publish/instagram', async (req, res) => {
  const planTier = req.body?.planTier || req.user?.planTier || 'operate';
  if (!isFeatureEnabled('publish.direct_tiktok', planTier)) {
    return res.status(403).json({ error: 'publish.direct_tiktok/instagram requires dfy+ plan tier', code: 'PLAN_GATE' });
  }
  const { filename, caption } = req.body;
  if (!filename) return res.status(400).json({ error: 'filename required' });
