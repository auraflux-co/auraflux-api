 ...
 * Headers: x-auraflux-api-secret: <AUTH_JWT_SECRET or BETTER_AUTH_SECRET>
 * Body: { to, subject, text, html? }
 *
 * Accepts any of AUTH_JWT_SECRET / BETTER_AUTH_SECRET / AURAFLUX_API_SECRET
 * so Vercel and Render can use either shared secret name.
 */
...
function providedMatchesAny(provided) {
  return expectedSecrets().some((expected) => secretsMatch(provided, expected));
}
...
router.post('/internal/auth-email', rateLimit(60), async (req, res) => {
  ...
  if (!secrets.length) {
    return res.status(503).json({ ok: false, error: 'Auth secret not configured' });
  }
  const provided =
    req.headers['x-auraflux-api-secret'] || req.headers['x-api-secret'];
  if (!providedMatchesAny(provided)) {
