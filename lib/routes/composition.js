...
router.post('/composition/timeline-preview', requireAuth, requireNearFinalPreview, async (req, res) => {
  ...
  try {
    ...
    let overrides = body.compCreative || {};
    // Merge brand creative profile into near-final burn when brandId provided
    if (body.brandId && req.user?.id && isFeatureEnabled('brand.creative_profile', planTierFromReq(req))) {
      try {
        const { getBrand } = require('../db/postgres');
        const { mergeBrandCreativeProfile } = require('../services/brand_creative_profile');
        const brand = await getBrand(body.brandId, req.user.id);
        let profile = brand?.creative_profile || null;
        if (typeof profile === 'string') profile = JSON.parse(profile);
        if (profile) overrides = mergeBrandCreativeProfile(overrides, profile);
      } catch (_e) { /* non-fatal */ }
    }
