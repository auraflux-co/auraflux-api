...
const { renderCompositionPreview, renderVodSegmentPreview, cleanupOldPreviews, PREVIEW_DIR } = require('../composition_preview');
...
function registerCompositionRoutes(app, deps = {}) {
  ...
  app.get('/composition/preview/file/:name', (req, res) => {
    try {
      ...
      if (!/^prev_[\w.-]+\.jpg$/.test(name) && !/^vodprev_[\w.-]+\.jpg$/.test(name)) {
