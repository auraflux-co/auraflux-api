...
function registerCompositionRoutes(app, deps = {}) {
  ...
  app.post('/composition/timeline-preview', async (req, res) => {
    ...
    try {
      ...
      const { isDualSourceStackMode } = require('../clip_comp_dual_source');
      const dualCreative = { preset, ...(overrides || {}), layout: { ...(overrides?.layout || {}) } };
      if (isDualSourceStackMode(dualCreative) || isDualSourceStackMode({ preset, layout: overrides?.layout })) {
        if (clips.length < 2) {
          return res.status(400).json({
            ok: false,
            error: 'dual_source_stack Review needs 2 clips (TOP + BOTTOM) — lineup is short',
          });
        }
        const result = await renderDualSourceTimelinePreview({
      ...
      const clip = body.clip || clips[0] || null;
      if (!clip?.url && !clip?.pageUrl && !clip?.mp4Url && !clip?.stagedUrl) {
        return res.status(400).json({ ok: false, error: 'clip or vodSegment required' });
      }
      const result = await renderCompositionTimelinePreview({
  ...
  // iss_SpdIbNmLEjQ8 — Compose QA intent vs near-final burn tokens (wiring, not Gemini)
  app.post('/composition/intent-vs-burned', (req, res) => {
    try {
      const { compareComposerIntentVsBurned } = require('../composer_intent_vs_burned');
      const body = req.body || {};
      const report = compareComposerIntentVsBurned(body.intent || {}, body.near || {});
  ...
  // iss_H_DvoUgXhdQc — Gemini watches full SOURCE + SHORT videos (stills fallback)
  app.post('/composition/compare-original', async (req, res) => {
    const { isFeatureEnabled } = require('../services/feature_gate');
    const planTier = (req.body && req.body.planTier) || 'operate';
    if (!isFeatureEnabled('ai.transform_compare', planTier)) {
      return res.status(403).json({
        ok: false,
        error: 'ai.transform_compare not enabled for this plan (needs GEMINI_API_KEY)',
      });
    }
    const { compareShortVsOriginal } = require('../composition_compare_original');
    const body = req.body || {};
    try {
      let sourcePath = body.sourcePath || body.sourceFilePath || body.originalPath;
      let shortPath = body.shortPath || body.shortFilePath || body.previewPath || body.filePath;
      if (body.shortName && !shortPath) {
        shortPath = path.join(PREVIEW_DIR, path.basename(String(body.shortName)));
      }
      if (body.sourceName && !sourcePath) {
        sourcePath = path.join(PREVIEW_DIR, path.basename(String(body.sourceName)));
      }
      const mode = body.mode === 'stills' ? 'stills' : 'video';
      const result = await compareShortVsOriginal({
        sourcePath,
        shortPath,
        mode,
        ratios: Array.isArray(body.ratios) ? body.ratios : undefined,
        width: body.width != null ? Number(body.width) : 640,
        includeRaw: !!body.includeRaw,
      });
      res.json(result);
    } catch (err) {
      console.error('[composition/compare-original]', err.message);
      res.status(400).json({ ok: false, error: err.message });
    }
  });

  // CPD-1282 / CPD-1286: energy peaks → punch/shake/SFX; optional music-bed source
  app.post('/composition/analyze-beats', async (req, res) => {
    const { analyzeBeatsOnFile, analyzeBeatsOnMusicBed } = require('../beat_detect');
    const body = req.body || {};
    try {
      const isMusicBed = body.source === 'music_bed' || !!body.musicBed;
      const opts = {
        ...
        // Music bed: short window + cache (Compose trim). Clip audio: longer ok.
        maxSec: body.maxSec != null
          ? Number(body.maxSec)
          : (isMusicBed ? 20 : 60),
      ...
      let result;
      if (isMusicBed) {
        const bedKey = body.musicBed || body.bed || 'low_trap';
        result = await analyzeBeatsOnMusicBed(bedKey, opts);
      } else {
        const filePath = body.filePath || body.localPath;
        if (!filePath) return res.status(400).json({ ok: false, error: 'filePath or musicBed required' });
        result = await analyzeBeatsOnFile(filePath, opts);
      }
      res.json({ ok: true, ...result });
    } catch (err) {
      res.status(400).json({ ok: false, error: err.message });
  ...
  } = require('../composer_operator_session');

  app.get('/composition/operator-session', (_req, res) => {
    try {
      const session = readOperatorComposerSession();
      if (!session) return res.json({ ok: false, session: null });
  ...
  app.post('/composition/operator-session', (req, res) => {
    try {
      const session = writeOperatorComposerSession(req.body || {});
      res.json({ ok: true, session });
    } catch (err) {
      res.status(400).json({ ok: false, error: err.message });
    }
