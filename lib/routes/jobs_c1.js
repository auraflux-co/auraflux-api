...
const { resolveProductionProfileAndContentType } = require('../pipeline_routing');
const { extractFeatureConfig, validateFeatureInputs } = require('../services/feature_input_schema');
...
function normaliseDashboardPayload(req, _res, next) {
  ...
  // CPD-485: fetchSpec.clipSpec → clipSpec (clip editor trim points; was silently dropped)
  if (b.fetchSpec?.clipSpec && !b.clipSpec) {
    b.clipSpec = b.fetchSpec.clipSpec;
  }
  ...
  // Dashboard wizard signature — used to tag createdBy on the job spec
  if (b.entryType || b.fetchSpec || b.uploadSpec || b.createSpec) {
    b.createdVia = 'dashboard';
  }
  ...
  // CPD-505: publish metadata forwarding — wizard sends publishMeta or top-level fields
  if (!b.publishMeta) {
    const pm = b.publishMeta || {};
    if (b.publishTitle)       pm.title          = b.publishTitle;
    if (b.publishDescription) pm.description    = b.publishDescription;
    if (b.publishTags)        pm.tags            = b.publishTags;
    if (b.publishPrivacy)     pm.privacyStatus  = b.publishPrivacy;
    if (b.tiktokCaption)      pm.tiktokCaption      = b.tiktokCaption;
    if (b.instagramCaption)   pm.instagramCaption   = b.instagramCaption;
    if (b.scheduledPublishAt) pm.scheduledPublishAt = b.scheduledPublishAt;
    if (Object.keys(pm).length) b.publishMeta = pm;
  }
...
router.post(
  ...
  async (req, res) => {
    ...
    // CPD-444 / CPD-1006: Branding gate — sync Twitch avatar when logo missing.
    const jobWarnings = [];
    const brandIdForGate = req.body.brandId || req.brandId || null;
    let resolvedBrandName = null;
    if (brandIdForGate) {
      try {
        const brandRow = await getBrand(brandIdForGate, customerId);
        resolvedBrandName = brandRow?.name || brandRow?.slug || null;
      } catch (_e) { /* non-fatal */ }
    }
    if (resolvedAddOns?.branding?.active) {
      let logoPresent = false;
      if (brandIdForGate) {
        try {
          const { ensureBrandLogo } = require('../services/brand_twitch_sync');
          logoPresent = await ensureBrandLogo({
            brandId:   brandIdForGate,
            accountId: customerId,
            twitchLogin: req.body.sourceChannels?.twitchLogin || req.body.twitchLogin,
          });
          if (!logoPresent) {
            const brandRow = await getBrand(brandIdForGate, customerId);
            logoPresent = !!(brandRow?.image_url);
          }
        } catch (_err) {
          // Non-fatal: treat lookup failure as missing logo
        }
      }
      if (!logoPresent) {
        resolvedAddOns.branding = { active: false };
        jobWarnings.push({
          code: 'BRANDING_NO_LOGO',
          message: 'Connect a Twitch channel or upload a logo to enable branding.',
        });
      }
    }

    // CPD-872 fix: resolve productionProfile before createJobSpec so it's baked into requiredFeatures.
    const { productionProfile: resolvedProductionProfile } = resolveProductionProfileAndContentType(req.body);
    ...
    try {
      jobSpec = createJobSpec({
        ...
        publishMeta: req.body.publishMeta || {},  // CPD-505: customer-provided publish metadata
        productionProfile: resolvedProductionProfile,  // CPD-872 fix: pass resolved profile into job spec
      });
    } catch (err) {
    ...
    // Persist wizard preset template name (e.g. 'TikTok Clutch') so job cards and
    // script generation can use it for display and content context.
    console.log(`[jobs/c1] ${jobId}: templateName in body = ${JSON.stringify(req.body.templateName)}`);
    if (req.body.templateName && typeof req.body.templateName === 'string') {
      jobSpec.templateName = req.body.templateName.slice(0, 100);
      console.log(`[jobs/c1] ${jobId}: templateName set to "${jobSpec.templateName}"`);
    }

    // Apply template-specific feature defaults when the wizard doesn't explicitly
    // set colorGrade / effects / captions. Templates advertise these features —
    // they must always be present in the output regardless of source material.
    const TEMPLATE_FEATURE_DEFAULTS = {
      'TikTok Clutch':       { colorGrade: { active: true, preset: 'crisp'  }, captions: { active: true, style: 'animated' }, effects: { zoom: true }, branding: { active: true }, hasLogo: true },
      'IRL Story Time':      { colorGrade: { active: true, preset: 'warm'   }, captions: { active: true, style: 'burnin'   }, branding: { active: true }, hasLogo: true },
      'Montage Hype Reel':   { colorGrade: { active: true, preset: 'vivid'  }, effects: { zoom: true }, branding: { active: true }, hasLogo: true },
      'Quick Guide':         { colorGrade: { active: true, preset: 'cool'   }, captions: { active: true, style: 'animated' }, branding: { active: true }, hasLogo: true },
      'YouTube Deep Dive':   { colorGrade: { active: true, preset: 'clean'  }, captions: { active: true, style: 'burnin'   }, branding: { active: true }, hasLogo: true },
      'Reaction Cut':        { colorGrade: { active: true, preset: 'neutral'}, branding: { active: true }, hasLogo: true },
    };
    const _templateDefaults = jobSpec.templateName ? TEMPLATE_FEATURE_DEFAULTS[jobSpec.templateName] : null;
    if (_templateDefaults) {
      if (_templateDefaults.colorGrade && !jobSpec.colorGrade)   jobSpec.colorGrade = _templateDefaults.colorGrade;
      if (_templateDefaults.captions   && !jobSpec.captions)     jobSpec.captions   = _templateDefaults.captions;
      if (_templateDefaults.effects    && !jobSpec.effects)      jobSpec.effects    = _templateDefaults.effects;
      else if (_templateDefaults.effects && jobSpec.effects)     jobSpec.effects    = { ..._templateDefaults.effects, ...jobSpec.effects };
      if (_templateDefaults.branding) {
        jobSpec.addOns = jobSpec.addOns || {};
        if (!jobSpec.addOns.branding?.active) jobSpec.addOns.branding = _templateDefaults.branding;
      }
      if (_templateDefaults.hasLogo) {
        jobSpec.designSpec = jobSpec.designSpec || {};
        jobSpec.designSpec.chrome = jobSpec.designSpec.chrome || {};
        jobSpec.designSpec.chrome.hasLogo = true;
      }
      console.log(`[jobs/c1] ${jobId}: applied template defaults for "${jobSpec.templateName}" — colorGrade=${JSON.stringify(jobSpec.colorGrade?.preset)} captions=${JSON.stringify(jobSpec.captions?.active)} zoom=${JSON.stringify(jobSpec.effects?.zoom)} branding=${JSON.stringify(jobSpec.addOns?.branding?.active)} hasLogo=${JSON.stringify(jobSpec.designSpec?.chrome?.hasLogo)}`);
    }

    // CPD-485 / CPD-872: productionProfile is now baked into the job spec by createJobSpec.
    // Ensure it's also set at the top level for assembly_service.js (backwards compat).
    if (resolvedProductionProfile && !jobSpec.productionProfile) {
      jobSpec.productionProfile = resolvedProductionProfile;
    }

    // Brand creative profile → designSpec.compCreative (creator portal Brand Profile)
    if (brandIdForGate) {
      try {
        const { isFeatureEnabled: featOn } = require('../services/feature_gate');
        const planForBrand = bodyPlanTier || req.user?.planTier || 'operate';
        if (featOn('brand.creative_profile', planForBrand)) {
          const brandForCreative = await getBrand(brandIdForGate, customerId);
          const profile = brandForCreative?.creative_profile;
          const parsed = typeof profile === 'string' ? JSON.parse(profile) : profile;
          if (parsed && typeof parsed === 'object') {
            const { mergeBrandCreativeProfile } = require('../services/brand_creative_profile');
            jobSpec.designSpec = jobSpec.designSpec || {};
            jobSpec.designSpec.compCreative = mergeBrandCreativeProfile(
              jobSpec.designSpec.compCreative || {},
              parsed,
            );
            // Carry into featureConfig.compose so playlist strategy + Peaks handoff see audioBed/captionStyle
            jobSpec.featureConfig = jobSpec.featureConfig || {};
            jobSpec.featureConfig.compose = {
              ...(jobSpec.featureConfig.compose || {}),
              ...(req.body.featureConfig?.compose || {}),
              ...(parsed.captionStyle ? { captionStyle: parsed.captionStyle } : {}),
              ...(parsed.audioBed ? { audioBed: parsed.audioBed } : {}),
            };
            if (parsed.audioBed) {
              jobSpec.tags = Array.from(new Set([...(jobSpec.tags || []), parsed.audioBed]));
            }
          }
        }
      } catch (_brandCreativeErr) { /* non-fatal */ }
    }

    // Detect source platform from URLs so chrome overlay uses brand colors.
    if (!jobSpec.sourcePlatform) {
      try {
        ...
        const _c1Platform = detectSourcePlatform(_c1Urls);
        if (_c1Platform) {
          jobSpec.sourcePlatform = _c1Platform;
          const { loadCustomerConfig: _lcc } = require('../job_spec');
          const _cc = _lcc(customerId);
          const _ct = jobSpec.designSpec?.chrome?.resolvedContentType || contentType || 'clips';
          const _cfg = resolveChromeCfg(_cc, _ct, _c1Platform);
          jobSpec.designSpec                       = jobSpec.designSpec || {};
          jobSpec.designSpec.chrome                = jobSpec.designSpec.chrome || {};
          jobSpec.designSpec.chrome.resolvedCfg    = _cfg;
          jobSpec.designSpec.chrome.resolvedHash   = fingerprintResolvedChromeCfg(_cfg);
          jobSpec.designSpec.chrome.sourcePlatform = _c1Platform;
        }
      } catch (_e) { /* non-fatal — chrome falls back to customer defaults */ }
    }

    // CPD-485: carry post-processing effects chosen in the wizard (loudnorm, LUT, square crop, etc.)
    if (req.body.effects && typeof req.body.effects === 'object') {
      jobSpec.effects = req.body.effects;
    }

    // CPD-485: format (short / long) influences vertical crop detection and chrome sizing.
    if (req.body.format && !jobSpec.format) {
      jobSpec.format = req.body.format;
    }

    // CPD-485 / CPD-QA: Extract all feature config from addOns (or top-level body fields)
    // using the canonical feature_input_schema so dashboard and API stay in parity.
    // addOns is the preferred path (matches developer_api.js); top-level fields are fallback.
    const _addOns = req.body.addOns || {};
    ...
    if (_featureConfig.layout     && !jobSpec.effects) {
      jobSpec.effects = jobSpec.effects || {};
      jobSpec.effects.layout = _featureConfig.layout;
    }
    // Top-level body fallback (backwards compat with older dashboard payloads)
    if (req.body.captions   && typeof req.body.captions   === 'object' && !jobSpec.captions)   jobSpec.captions   = req.body.captions;
    if (req.body.colorGrade && typeof req.body.colorGrade === 'object' && !jobSpec.colorGrade) jobSpec.colorGrade = req.body.colorGrade;
    if (req.body.effects    && typeof req.body.effects    === 'object' && !jobSpec.effects)    jobSpec.effects    = req.body.effects;
    if (req.body.audioOpts  && typeof req.body.audioOpts  === 'object' && !jobSpec.audioOpts)  jobSpec.audioOpts  = req.body.audioOpts;

    // clipSpec forwarded by normaliseDashboardPayload (fetchSpec.clipSpec → b.clipSpec);
    // copy to jobSpec so assembly_service can apply trim points.
    if (req.body.clipSpec && !jobSpec.clipSpec) {
      jobSpec.clipSpec = req.body.clipSpec;
    }

    // CPD-485: streamer — wired on v1 path since CPD-193; missing on dashboard path until now.
    // Used by chrome overlay (showName/streamer) and publish copy generation.
    const _streamer = req.body.streamer || req.body.order?.inputs?.streamer;
    if (_streamer) {
      jobSpec.order = jobSpec.order || {};
      jobSpec.order.inputs = jobSpec.order.inputs || {};
      if (!jobSpec.order.inputs.streamer) jobSpec.order.inputs.streamer = _streamer;
    }
    ...
    // CPD-513: staging flag — customer requested review before Portal 5 auto-publishes
    if (req.body.staging === true) { jobSpec.staging = true; }

    if (req.body.productionPath) {
      jobSpec.productionPath = req.body.productionPath;
    }

    // CPD-1042: clip-comp stack for short_compile_clips (parity with cwn-c0)
    if (req.body.productionPath === 'short_compile_clips') {
      const {
        applyClipCompProfileToJobSpec,
        resolveClipCompSourceContentType,
      } = require('../clip_comp');
      const clipItems = req.body.items || req.body.sourceLibrary || jobSpec.order?.inputs?.items || [];
      const clipCount = Math.max(1, clipItems.length || sourceConfig?.urls?.length || 1);
      const sourceContentType = resolveClipCompSourceContentType({
        contentType: req.body.contentType || contentType,
        templateName: jobSpec.templateName,
      });
      applyClipCompProfileToJobSpec(jobSpec, {
        customerId,
        clipCount,
        sourceContentType,
        platforms: req.body.platforms || jobSpec.order?.publish?.platforms || ['youtube'],
        scheduledAt: scheduledAt || null,
        clipsOnly: true,
      });

      // Peaks / Compose: bake C1–C11 creative preset onto compositionSpec
      try {
        ...
        const compCreative = mergeCompCreative({
          ...(req.body.compositionSpec?.compCreative || {}),
          preset: presetKey,
        });
        ...
        console.log(`[jobs/c1] ${jobId}: compCreative preset=${presetKey}`);
      } catch (creativeErr) {
        console.warn(`[jobs/c1] ${jobId}: compCreative merge skipped:`, creativeErr.message);
      }

      console.log(`[jobs/c1] ${jobId}: clip-comp profile applied (${sourceContentType}, ${clipCount} clips)`);
    }

    // CPD-1044: build portals map so staging disables portal5 (parity with developer_api)
    try {
      const { resolveActivePortals } = require('../job_spec');
      resolveActivePortals(jobSpec);
    } catch (_portalErr) {
      console.warn(`[jobs/c1] portal map build failed (non-fatal): ${_portalErr.message}`);
    }
    ...
    // Preflight: Growth VOD hours remaining (40h/mo)
    try {
      const { getVodHoursBalance, hoursFromDurationSecs } = require('../services/vod_hours');
      const vodBal = await getVodHoursBalance(customerId);
      if (vodBal && !vodBal.unlimited) {
        const need = hoursFromDurationSecs(
          Number(req.body.sourceDurationSecs || jobSpec.sourceDurationSecs || 3600),
        );
        if (vodBal.hoursRemaining < need) {
          return res.status(402).json({
            ok: false,
            error: 'vod_hours_exhausted',
            message: vodBal.hoursRemaining <= 0
              ? 'Monthly VOD processing hours exhausted. Upgrade to Pro Operator for unlimited processing.'
              : `Not enough VOD hours remaining (${vodBal.hoursRemaining.toFixed(2)}h left this period).`,
            balance: vodBal,
          });
        }
      }
    } catch (vodPreErr) {
      console.warn('[jobs/c1] vod hours preflight skipped:', vodPreErr.message);
    }
    ...
    if (futureStart) {
      ...
      return res.status(202).json({
        ...
        ...(jobWarnings.length ? { warnings: jobWarnings } : {}),
      });
    }
    ...
    if (_useQueue) {
      try {
        ...
        const { queuePriorityForTier } = require('../services/plan_entitlements');
        const _prio = queuePriorityForTier(jobSpec.planTier || bodyPlanTier || 'operate');
        await q.add(jobId, { jobSpec }, { jobId, priority: _prio });
    ...
    setImmediate(async () => {
      try {
        ...
        const { consumeVodHours, hoursFromDurationSecs } = require('../services/vod_hours');
        // VOD hours gate (Growth 40h/mo) — estimate from source duration when known
        const _srcSecs = Number(jobSpec.sourceDurationSecs || jobSpec.order?.sourceDurationSecs || 0);
        const _vodHours = hoursFromDurationSecs(_srcSecs || 3600); // default 1h if unknown
        if (customerId) {
          const vodResult = await consumeVodHours(customerId, jobId, _vodHours, jobSpec.brandId || null);
          if (!vodResult.ok) {
            logError('VOD_HOURS_PAUSED', vodResult.reason, { jobId, customerId });
            const { saveJob } = require('../db');
            await saveJob(jobId, {
              ...jobSpec,
              status: 'credit_paused',
              pauseReason: vodResult.reason,
        ...
        if (creditCost > 0 && customerId) {
          const creditResult = await consumeCredits(customerId, jobId, creditCost, jobSpec.brandId || null);
          if (!creditResult.ok) {
        ...
        runPortalSequence({
          ...
          extensionWorkers: _resolveExtensionWorkers(jobSpec),
          ...
          onPortalPass: async (portalKey, result) => {
            ...
            const _persist = (patch) => _sj(jobId, { ...jobSpec, ...patch }).catch(() => {});

            // CPD-485: assembly fires after portal1 for script jobs, or after portal0
            // for clip-only jobs where portal1 is inactive (stageMap.script.active=false).
            if (
              (portalKey === 'portal1') ||
              (portalKey === 'portal0' && !isPortal1Active(jobSpec))
            ) {
              // CPD-539: generate publishCopy from the approved script if not already present.
              // The script is available at portal1 pass — generate AI copy before assembly so
              // portal5 reads real titles instead of falling back to the topic slug heuristic.
              if (!jobSpec.state?.savedOutputs?.publishCopy) {
                try {
                  const _script = jobSpec.state?.savedOutputs?.filledScript || jobSpec.filledScript || null;
                  if (_script) {
                    const { generatePublishCopyFromScript } = require('../publish');
                    const _platforms = (jobSpec.order?.publish?.platforms || []).length
                      ? jobSpec.order.publish.platforms
                      : ['youtube', 'tiktok', 'instagram'];
                    const _pcResult = await generatePublishCopyFromScript({
                      script:      _script,
                      contentType: jobSpec.contentType || 'clips',
                      customerId:  jobSpec.customerId  || 'c1',
                      formType:    jobSpec.order?.formType || 'compilation',
                      items:       (jobSpec.order?.inputs?.items || []).map((it) => ({
                        title: it.title || it.displayName || '', headline: it.headline || '', url: it.url || '',
                      })),
                      jobId:       jobId || null,
                      platforms:   _platforms,
                      designSpec:  jobSpec.designSpec || {},
                    });
                    if (_pcResult && !_pcResult.error) {
                      if (!jobSpec.state) jobSpec.state = {};
                      if (!jobSpec.state.savedOutputs) jobSpec.state.savedOutputs = {};
                      jobSpec.state.savedOutputs.publishCopy = _pcResult;
                      console.log(`[jobs/c1] ${jobId}: publishCopy locked at portal1 pass: "${_pcResult.youtube?.title || 'n/a'}"`);
                    }
                  }
                } catch (_pcErr) {
                  console.warn(`[jobs/c1] ${jobId}: publishCopy generation failed (non-fatal): ${_pcErr.message}`);
                }
              }

              await runAssemblyAndPostProcess(jobSpec, jobId, {
                logPrefix: '[jobs/c1]',
                persist:   _persist,
              });
            }

            // CPD-485: TTS mix + chrome after tts_ext — was v1-only
            if (portalKey === 'tts_ext') {
              const ttsAudio = result?.audioPath || jobSpec.state?.tts?.audioPath;
              if (ttsAudio) {
                await runTtsMixAndChrome(jobSpec, jobId, ttsAudio, {
                  logPrefix: '[jobs/c1]',
                  persist:   _persist,
                });
              }
            ...
            // CPD-485: chrome safety net on portal3a — was v1-only
            if (portalKey === 'portal3a') {
              await ensureChromeApplied(jobSpec, jobId, { logPrefix: '[jobs/c1]' });
            }

            // CPD-896: portal3b mismatch_fixable — chrome re-burn then re-verify portal3a.
            // cwn-c0 re-runs gate3a after every chrome re-burn to confirm the fix worked.
            if (portalKey === 'portal3b' && result?.outcome === 'mismatch_fixable') {
              try {
                const _fs = require('fs');
                const { applyChrome } = require('../assembly_service');
                const _assembledPath = jobSpec.assembledPath;
                if (_assembledPath && _fs.existsSync(_assembledPath)) {
                  const _reburnPath = _assembledPath.replace('.mp4', '_reburn.mp4');
                  const { showName: _sn, streamerName: _str } = resolveChromeName(jobSpec);
                  const _isVert = jobSpec.productionProfile === 'vertical_reel' ||
                    (jobSpec.format === 'short' && (jobSpec.order?.publish?.platforms || [])
                      .some((p) => ['tiktok', 'instagram'].includes(String(p).toLowerCase())));
                  const _dur = (jobSpec.clipDurations || []).reduce((s, d) => s + (d || 0), 0) || (jobSpec.durationMins || 0) * 60;
                  await applyChrome(_assembledPath, _reburnPath, {
                    showName: _sn, streamerName: _str, isVertical: _isVert, durationSecs: _dur,
                    hasTopBar: jobSpec.designSpec?.chrome?.hasTopBar === true || jobSpec.chromeCfg?.hasTopBar === true,
                  });
                  _fs.renameSync(_reburnPath, _assembledPath);
                  console.log(`[jobs/c1] ${jobId}: portal3b chrome re-burn complete — re-running portal3a to verify`);
                  // Re-run portal3a on the re-burned file to verify the fix (CPD-896)
                  const _p3aMod = require('../portals/portal3a');
                  const _p3aResult = await _p3aMod.run(jobSpec);
                  const _p3aPassed = _p3aResult?.passed;
                  if (!jobSpec.state) jobSpec.state = {};
                  if (!jobSpec.state.savedOutputs) jobSpec.state.savedOutputs = {};
                  jobSpec.state.savedOutputs.portal3aReburnResult = _p3aResult;
                  if (!_p3aPassed) {
                    console.warn(`[jobs/c1] ${jobId}: portal3a re-verify after reburn FAILED — routing to operator_review`);
                    logError('CPD896_PORTAL3A_REBURN_FAIL', new Error('portal3a re-verify failed after chrome re-burn'), { jobId });
                    if (!jobSpec.state.savedOutputs.operator_review) {
                      jobSpec.state.savedOutputs.operator_review = 'chrome_reburn_unverified';
                    }
                  } else {
                    console.log(`[jobs/c1] ${jobId}: portal3a re-verify PASSED after chrome re-burn (CPD-896)`);
                  }
                  await _persist({ ...jobSpec, updatedAt: new Date().toISOString() });
                } else {
                  console.warn(`[jobs/c1] ${jobId}: portal3b chrome re-burn skipped — assembledPath not found`);
                }
              } catch (_reburnErr) {
                console.warn(`[jobs/c1] ${jobId}: portal3b chrome re-burn failed (non-fatal) — ${_reburnErr.message}`);
                logError('CPD896_CHROME_REBURN_FAIL', _reburnErr, { jobId });
              }
            }
          },
          ...
          // CPD-485: shared runJobComplete — grades, persists, notifies (was partial here)
          onJobComplete: () => runJobComplete(jobSpec, jobId, {
        ...
        // CPD-492: catch assembly/onPortalPass failures that now propagate
        }).catch((err) => logError('CPD67_DISPATCH_FAIL', err, { jobId }));
      } catch (err) {
 ...
 */
/**
 * CPD-491: Wrap an extension worker module so portal_policy_runner injects
 * jobSpec into every runWorker call. Extension workers expect ({ jobSpec, ... })
 * but portal_policy_runner calls runWorkerAttempt({ workerAttempt, phase }).
 * This adapter merges jobSpec into the opts so the worker always has it.
 */
function _adaptExtensionWorker(mod, jobSpec) {
  return {
    runWorker: (opts) => mod.runWorker({ ...opts, jobSpec }),
    runIntervention: mod.runIntervention
      ? (opts) => mod.runIntervention({ ...opts, jobSpec })
      : null,
    isPass: mod.isPass || ((r) => !!r?.passed),
  };
}
...
function _resolveExtensionWorkers(jobSpec) {
  return {
    ...
    // CPD-506: thumbnail_ext added to C1 dashboard path (was missing — developer_api.js had it, jobs_c1.js did not)
    ...
    twelvelabs_qa_ext:  _adaptExtensionWorker(require('../portals/portal_twelvelabs_qa'),     jobSpec),
  };
}
 ...
 */
function _adaptLegacyPortal(mod, jobSpec, timeoutMs, isPassOverride) {
  return {
    runWorker: (_opts) => {
    ...
    // CPD-485: per-portal isPass override allows portal3b mismatch_fixable to pass
    // instead of hard-failing on the dashboard path.
    isPass: isPassOverride || ((result) => !!(result?.passed)),
  };
}

// CPD-967: portal3a wrapper that adds a repair intervention for frozen-frame hard_fails.
// When portal_policy_runner detects hard_fail AND runIntervention is wired, it skips
// sendbacks (useless on hard_fail) and calls this handler instead. The handler excises
// the frozen segments from the assembled video and updates jobSpec.assembledPath so
// the next runWorkerAttempt (portal3a re-run) sees the repaired file.
function _wrapPortal3aWithRepair(mod, jobSpec) {
  const base  = _adaptLegacyPortal(mod, jobSpec);
  const jobId = jobSpec?.jobId || 'unknown';
  return {
    ...base,
    runIntervention: async ({ interventionAttempt, lastResult }) => {
      const defects = lastResult?.ffmpegDefects || [];
      const fixable = defects.filter((d) => d.type === 'frozen_frame' || d.type === 'black_frame');
      if (!fixable.length) {
        return `Intervention ${interventionAttempt}: no fixable defects in ffmpegDefects — hard_fail cannot be repaired autonomously`;
      }

      const assembledPath = jobSpec.assembledPath || jobSpec.state?.savedOutputs?.assembledPath;
      if (!assembledPath || !require('fs').existsSync(assembledPath)) {
        return `Intervention ${interventionAttempt}: assembled file not found — cannot repair`;
      }

      try {
        const { repairFrozenSegmentsInAssembled } = require('../assembly_service');
        const repairedPath = await repairFrozenSegmentsInAssembled(assembledPath, jobId);
        if (!repairedPath) {
          return `Intervention ${interventionAttempt}: freezes detected but all in skip windows or repair returned nothing — retrying as-is`;
        }

        jobSpec.assembledPath = repairedPath;
        if (!jobSpec.state) jobSpec.state = {};
        if (!jobSpec.state.savedOutputs) jobSpec.state.savedOutputs = {};
        jobSpec.state.savedOutputs.assembledPath = repairedPath;

        return `CPD-967 repair attempt ${interventionAttempt}: excised ${fixable.length} frozen segment(s) from assembled video → ${require('path').basename(repairedPath)}`;
      } catch (err) {
        return `Intervention ${interventionAttempt}: repair threw — ${err.message.slice(0, 100)}`;
      }
    },
  };
}

// CPD-895: portal1 wrapper that saves fixDirective after sendback and re-runs script_gen on retry.
// cwn-c0 injects fix directives into subsequent Gemini script prompts; this ports that behaviour
// to the Render portal pipeline.
function _wrapPortal1WithFixDirective(mod, jobSpec) {
  const base = _adaptLegacyPortal(mod, jobSpec, 10 * 60 * 1000);
  const jobId = jobSpec?.jobId || 'unknown';
  return {
    ...base,
    runWorker: async (_opts) => {
      const _shouldRegenScript = jobSpec.state?.portal1FixDirective && (
        jobSpec.state?.forceScriptRegen || (_opts?.workerAttempt || 1) > 1
      );
      if (_shouldRegenScript) {
        try {
          const { generateJobScript: _regen } = require('../script_gen_service');
          const _fd = jobSpec.state.portal1FixDirective;
          console.log(`[portal1-wrapper] ${jobId}: regenerating script with fixDirective (attempt ${_opts?.workerAttempt || 1})`);
          const _res = await _regen(jobSpec, { fixDirective: _fd });
          if (_res?.filledScript) {
            jobSpec.filledScript = _res.filledScript;
            if (!jobSpec.state.savedOutputs) jobSpec.state.savedOutputs = {};
            jobSpec.state.savedOutputs.filledScript = _res.filledScript;
            console.log(`[portal1-wrapper] ${jobId}: script regenerated (${_res.filledScript.length} chars) for portal1 retry`);
          }
          if (jobSpec.state) delete jobSpec.state.forceScriptRegen;
        } catch (_e) {
          console.warn(`[portal1-wrapper] ${jobId}: script regen failed (non-fatal) — retrying with prior script: ${_e.message}`);
        }
      }
      const result = await base.runWorker(_opts);
      if (result?.fixDirective && result.passed === false) {
        if (!jobSpec.state) jobSpec.state = {};
        jobSpec.state.portal1FixDirective = result.fixDirective;
      }
      return result;
    },
  };
...
function _resolvePortalWorkers(jobSpec) {
  return {
    ...
    portal1:  _wrapPortal1WithFixDirective(require('../portals/portal1'), jobSpec),
    portal1b: _adaptLegacyPortal(require('../portals/portal1_video_reviewer'), jobSpec),
    // CPD-485: portal2 requires segmentPaths which are only present on the v1 API path.
    // On the dashboard path, portal2 is expected to be inactive (clips-only jobs skip it
    // via the portal map). If it runs anyway, return a soft non-fail rather than throwing.
    portal2:  _adaptLegacyPortal(require('../portals/portal2'),                jobSpec, undefined, (result) => {
      if (result?.outcome === 'skipped' || result?.skipped) return true; // not active — treat as pass
      return !!(result?.passed);
    }),
    portal3a: _wrapPortal3aWithRepair(require('../portals/portal3a'),           jobSpec),
    // CPD-485: mismatch_fixable means the portal found issues but can self-correct.
    // On the v1 path this is handled by developer_api. Dashboard path previously treated
    // it as hard_fail. Accept it as a pass — portal3a/portal4 catch genuine failures.
    portal3b: _adaptLegacyPortal(require('../portals/portal3b'),               jobSpec, undefined, (result) => {
      return !!(result?.passed) || result?.outcome === 'mismatch_fixable';
    }),
  ...
  async (req, res) => {
    ...
    try {
      ...
      const rows = allJobs
        ...
        : await db.listJobsByCustomer(userId, 50, brandId);

      const isOperatorView = req.user.role === 'superadmin';
      ...
      const shapeRow = (row, customerName) => {
        ...
        return {
          ...
          entryType:           _resolveEntryType(spec),
          status:              _resolveCustomerJobStatus(row, spec, { isOperator: isOperatorView }),
...
router.get(
  ...
  async (req, res) => {
    ...
    try {
      ...
      // If a brand context is active, the job must belong to that brand
      if (!isOperator && req.brandId && row.brand_id && row.brand_id !== req.brandId) {
        return res.status(403).json({ ok: false, error: 'Job does not belong to the active brand' });
      }
      ...
      res.json({
        ...
        job: {
          ...
          contentType:         spec.contentType || null,
          entryType:           _resolveEntryType(spec),
          status:              _resolveCustomerJobStatus(row, spec, { isOperator }),
          customerId:          row.customer_id,
          planTier:            spec.planTier || 'operate',
          publishMode:         spec.order?.publish?.publishMode || 'immediate',
          scheduledPublishAt:  spec.order?.publish?.scheduledPublishAt || null,
...
// ── GET /jobs/:jobId/grade — spec compliance grade (CPD-422) ─────────────────
router.get(
  '/jobs/:jobId/grade',
  requireAuth,
  requireRole({ minLevel: ROLES.CUSTOMER }),
  async (req, res) => {
    ...
    try {
      ...
      // Authorization: customer can only grade their own jobs
      if (!isOp && spec.customerId !== userId) {
        return res.status(403).json({ ok: false, error: 'Forbidden' });
      }

      // Return cached grade if already computed
      if (spec.specGrade) {
        return res.json({ ok: true, jobId, ...spec.specGrade, cached: true });
      }

      const { gradeJob } = require('../services/job_grader');
      const result = gradeJob(spec);

      // Persist grade to spec
      try {
  ...
  async (req, res) => {
    ...
    try {
      ...
      res.json({
        ...
        publishStatus: spec.publishStatus || null,
        publishResults: spec.publishResults || null,
        input: {
          contentType: spec.contentType   || null,
          sourceType:  spec.sourceType    || null,
          tone:        spec.order?.tone   || null,
          topic:       spec.order?.topic  || spec.order?.userPrompt || null,
          duration:    spec.order?.duration || spec.order?.durationMins || null,
          ...
          wizardConfig:  _buildWizardConfig(spec),
          orderSource:   _buildOrderSourceSummary(spec),
          featureCompliance: _buildFeatureComplianceSummary(spec),
        ...
        output: {
          ...
          script:       scriptText,
          publishCopy:  _sanitizePublishCopy(
            spec.state?.savedOutputs?.publishCopy || spec.state?.publishCopy || null
          ),
          savedOutputs: spec.state?.savedOutputs || null,
        },
        // CPD-505: customer-provided publish metadata — pre-fills the editable review fields
        publishMeta: {
          title:            spec.order?.publish?.title           || null,
          description:      spec.order?.publish?.description     || null,
          tags:             spec.order?.publish?.tags            || [],
          privacyStatus:    spec.order?.publish?.privacyStatus   || 'public',
          tiktokCaption:    spec.order?.publish?.tiktokCaption   || null,
          instagramCaption: spec.order?.publish?.instagramCaption|| null,
        },
        portalReports,
...
router.post(
  ...
  async (req, res) => {
    ...
    try {
      ...
      // Creator portal default: private on ClipzWorld News / YouTube unless explicitly set
      const bodyPrivacy = req.body?.publishMeta?.privacyStatus || req.body?.privacyStatus;
      if (!bodyPrivacy) {
        req.body = {
          ...req.body,
          publishMeta: {
            ...(req.body.publishMeta || {}),
      ...
      // Playlist strategy — auto-match when creator did not override playlist id
      try {
        const { isFeatureEnabled: featOn } = require('../services/feature_gate');
        if (featOn('publish.playlist_strategy', spec.planTier || req.user?.planTier || 'operate')) {
          const already = spec.order?.publish?.youtubePlaylistId || req.body?.youtubePlaylistId
            || req.body?.publishMeta?.youtubePlaylistId;
          if (!already && spec.brandId) {
            const { getBrand: gb } = require('../db/postgres');
            const { matchPlaylistStrategy } = require('../services/playlist_strategy');
            const brand = await gb(spec.brandId, userId);
            let rules = brand?.publish_strategy_rules || [];
            if (typeof rules === 'string') rules = JSON.parse(rules);
            let profile = brand?.creative_profile || null;
            if (typeof profile === 'string') profile = JSON.parse(profile);
            const tags = [
              ...(spec.tags || []),
              ...(spec.order?.tags || []),
              ...(spec.order?.publish?.tags || []),
              spec.featureConfig?.compose?.preset,
              profile?.audioBed,
              profile?.captionStyle,
            ].filter(Boolean);
            const audioBed = spec.designSpec?.compCreative?.audio?.musicBed
              || spec.featureConfig?.compose?.audioBed
              || profile?.audioBed
              || null;
            const match = matchPlaylistStrategy(rules, { tags, audioBed, platform: 'youtube' });
            if (match.matched && match.playlistId) {
              applyPublishRequestToSpec(spec, {
                youtubePlaylistId: match.playlistId,
                youtubePlaylistTitle: match.playlistTitle,
                playlistStrategyBadge: match.badge,
              });
            }
          }
        }
      ...
      const privacy = spec.order?.publish?.privacyStatus || 'private';
      const readiness = assertPublishReadiness(spec, {
        // Private ClipzWorld / creator test publishes may skip grade/chrome holds
        forceApprove: privacy === 'private'
          || (isOp && (req.body.forceApprove === true || req.body.operatorOverride === true)),
      });
      if (!readiness.ok) {
      ...
      // CPD-505: merge customer-edited publish metadata from the review screen before Portal 5 runs.
      clearFailedPublishState(spec, {
        forceRepublish: req.body.forceRepublish === true || req.body.clearSchedule === true,
      });
      ...
      let platforms = req.body.platforms || spec.order?.publish?.platforms || [];
      // Private creator tests default to YouTube (@clipzworldnews) when no platforms set
      if (!platforms.length) platforms = ['youtube'];
      if (privacy === 'private' && !platforms.includes('youtube') && !platforms.includes('YouTube')) {
      ...
      if (!creds.ok) {
        return res.status(422).json({ ok: false, ...creds });
      }
      ...
      return res.status(202).json({
        ...
        platforms: platforms.reduce((acc, p) => ({ ...acc, [p]: { status: 'pending' } }), {}),
      });
    } catch (err) {
...
/** Portal-policy phases that leak into jobs.status via worker persistJobStatus (CPD-869). */
const _PORTAL_POLICY_STATUSES = new Set([
  'passed', 'sendback', 'intervention', 'hard_stop', 'non-compliant',
]);

const _TERMINAL_JOB_STATUSES = new Set([
  'complete', 'published', 'operator_review', 'failed', 'cancelled', 'credit_paused', 'held',
]);

const _STALE_JOB_MS = 90 * 60 * 1000;

function _jobOutputUrl(spec) {
  return spec?.state?.savedOutputs?.r2VideoUrl || spec?.state?.savedOutputs?.driveUrl || null;
}

/**
 * Resolve customer-facing job status for dashboard/review UI (CPD-869).
 * jobs.status column often holds portal-policy phases; job_spec.status is canonical.
 */
function _resolveCustomerJobStatus(row, spec, { isOperator = false } = {}) {
  const outputUrl = _jobOutputUrl(spec);
  let rawStatus = spec?.status || row?.status || 'queued';

  if (_PORTAL_POLICY_STATUSES.has(rawStatus) || _PORTAL_POLICY_STATUSES.has(row?.status)) {
    if (spec?.status && !_PORTAL_POLICY_STATUSES.has(spec.status)) {
      rawStatus = spec.status;
    } else if (outputUrl) {
      rawStatus = (spec?.gradeResult?.passed === false || spec?.state?.operatorReview)
        ? 'operator_review'
        : 'complete';
    } else if (['hard_stop', 'non-compliant'].includes(row?.status || rawStatus)) {
      rawStatus = 'failed';
    } else {
      rawStatus = spec?.status || 'running';
    }
  }

  // Assembly finished but spec never advanced — surface for review queue
  if (
    outputUrl &&
    ['queued', 'running'].includes(rawStatus) &&
    !spec?.status
  ) {
    rawStatus = 'operator_review';
  } else if (
    outputUrl &&
    ['queued', 'running'].includes(rawStatus) &&
    spec?.status &&
    !_TERMINAL_JOB_STATUSES.has(spec.status)
  ) {
    rawStatus = spec.state?.operatorReview || spec.status === 'operator_review'
      ? 'operator_review'
      : (spec.grade != null && spec.gradeResult?.passed !== false ? 'complete' : 'operator_review');
  }

  const updatedMs = row?.updated_at ? Number(row.updated_at) : null;
  if (
    ['queued', 'running'].includes(rawStatus) &&
    !outputUrl &&
    updatedMs &&
    Date.now() - updatedMs > _STALE_JOB_MS
  ) {
    rawStatus = 'failed';
  }

  if (!isOperator && rawStatus === 'operator_review') {
    return 'processing';
  }
  return rawStatus;
}

function _sanitizePublishCopy(raw) {
  if (!raw || typeof raw !== 'object') return null;
  const PLATFORMS = ['youtube', 'tiktok', 'instagram', 'twitter', 'facebook', 'linkedin'];
  const out = {};
  for (const p of PLATFORMS) {
    if (raw[p] && typeof raw[p] === 'object') out[p] = raw[p];
  }
  return Object.keys(out).length ? out : null;
}

/**
 * How this job was submitted — drives Review Queue "what was ordered" labelling.
 * @param {import('express').Request} req
 * @returns {'dashboard'|'e2e_script'|'api'|'agent'}
 */
function _resolveCreatedBy(req) {
  if (req.headers['x-e2e-secret']) return 'e2e_script';
  if (req.body?.createdVia === 'dashboard' || req.body?.entryType || req.body?.fetchSpec || req.body?.uploadSpec) {
    return 'dashboard';
  }
  if (req.body?.createdVia === 'agent') return 'agent';
  return 'api';
}

/**
 * Source clips submitted vs clip auto-selected for assembly (Review "what was ordered").
 * @param {Object} spec
 * @returns {{ candidates: Array, selected: object|null, selectionSource: string|null, clipSourcing: boolean }}
 */
function _buildOrderSourceSummary(spec) {
  const library = spec?.order?.inputs?.sourceLibrary || [];
  const items = spec?.order?.inputs?.items || [];
  const urls = spec?.order?.inputs?.sourceConfig?.urls
    || (items.length ? items.map((i) => i.url).filter(Boolean) : [])
    || [];

  let candidates = [];
  if (library.length) {
    candidates = library.map((item, i) => ({
      url: item.url || null,
      title: item.title || item.label || `Clip ${i + 1}`,
  ...
  const manifest = spec?.state?.clipManifest;
  const ordered = Array.isArray(spec?.orderedClipUrls) ? spec.orderedClipUrls : [];
  const selectionLog = spec?.state?.clipSelectionLog || [];

  let selected = null;
  let selectionSource = manifest?.source || null;

  if (manifest?.clips?.length) {
    const primary = manifest.clips[0];
    selected = {
      url: primary.url || primary.path || null,
      title: primary.label || primary.title || 'Selected clip',
    ...
    if (typeof first === 'string') {
      selected = { url: first, title: 'Selected clip', duration: null, confidence: null };
    } else {
      selected = {
        url: first.url || first.clipUrl || null,
        title: first.label || first.title || 'Selected clip',
  ...
  if (!selectionSource && selectionLog[0]?.source) {
    selectionSource = selectionLog[0].source;
  }
  ...
  return {
    ...
    selected,
    selectionSource,
    clipSourcing: spec?.addOns?.clipSourcing?.active === true,
  };
}

/**
 * Customer-facing feature compliance summary from GPT-4o QA (when present).
 * @param {Object} spec
 * @returns {Array<{ feature: string, status: string, note?: string }>}
 */
function _buildFeatureComplianceSummary(spec) {
  const rows = spec?.state?.savedOutputs?.gpt4oQA?.featureCompliance;
  if (!Array.isArray(rows) || !rows.length) return [];
  return rows
    .filter((r) => r && typeof r === 'object')
    .map((r) => ({
      feature: r.feature || r.name || 'Feature',
      status:  r.status || 'unknown',
      note:    r.note || r.reason || undefined,
    }));
}
...
function _buildPortalReports(spec) {
  ...
  const currentPortal = spec?.currentPortal || null;
  ...
  return PORTALS.map((key) => {
    ...
    if (!r) {
      ...
      // If this portal is actively running right now, surface 'running' so the stage indicator advances.
      if (active && currentPortal === key) return { portal: key, status: 'running', passed: false };
...
/**
 * Normalise the entry type from various storage formats.
 * sourceType 'url_list' is the internal name for the 'fetch' entry method;
 * map it back to 'fetch' so the frontend entryTypeLabel helper matches.
 */
function _resolveEntryType(spec) {
  const raw = spec?.order?.inputs?.entryType || spec?.sourceType || null;
  if (raw === 'url_list') return 'fetch';
  if (raw === 'wan_gen')  return 'create';
  return raw;
}
...
function _buildWizardConfig(spec) {
  ...
  // Captions / colorGrade stored at top-level spec (set by developer_api extractFeatureConfig)
  if ((spec?.captions?.active || spec?.addOns?.captions?.active) && !activeAddOns.includes('captions')) {
    activeAddOns.push('captions');
  }
  if ((spec?.colorGrade?.active || spec?.addOns?.colorGrade?.active) && !activeAddOns.includes('colorGrade')) {
    activeAddOns.push('colorGrade');
  }
  // Effects uses { zoom: true } shape, not { active: true }
  const eff = spec?.effects || spec?.addOns?.effects || {};
  if ((eff.zoom || eff.ken_burns || eff.slowmo || eff.vignette) && !activeAddOns.includes('effects')) {
    activeAddOns.push('effects');
  }
  ...
  const isClips = spec?.contentType === 'clips';

  return {
    // For clips, formFactor comes from the assembled output if known, not the job creation default.
    // Clips have their own aspect ratio; don't surface a preset default.
    formFactor:    isClips ? (spec?.state?.savedOutputs?.formFactor || null) : formFactor,
    ...
    // For clips, durationMins is a billing/credit default, not the actual clip duration.
    // Don't surface it — the video player shows real duration.
    durationMins:  isClips ? null : (spec?.order?.output?.durationMins || spec?.durationMins || null),
    planTier:      spec?.planTier || null,
...
const REVISION_ELIGIBLE_STATUSES = new Set(['complete', 'staged', 'held', 'processing']);

/**
 * POST /jobs/:jobId/request-revision
 * Customer sends output back with feedback — job held for operator/Collab redo.
 * Parses feedback into assembly directives and optionally auto-redispatches pipeline.
 */
router.post(
  '/jobs/:jobId/request-revision',
  requireAuth,
  requireRole({ minLevel: ROLES.CUSTOMER }),
  async (req, res) => {
    const db = require('../db');
    const { jobId } = req.params;
    const userId = req.user.id;
    const { feedback, categories = [], autoRedo = true } = req.body || {};
    const text = String(feedback || '').trim();
    if (!text) {
      return res.status(400).json({ ok: false, error: 'feedback_required', message: 'Describe what needs to change.' });
    }
    if (text.length > 4000) {
      return res.status(400).json({ ok: false, error: 'feedback_too_long', message: 'Feedback must be 4000 characters or fewer.' });
    }

    try {
      const row = await db.loadJobRow(jobId);
      if (!row) return res.status(404).json({ ok: false, error: 'Job not found' });
      if (row.customer_id !== userId && req.user.role !== 'superadmin') {
        return res.status(403).json({ ok: false, error: 'Forbidden' });
      }
      ...
      const status = spec.status || row.status || '';
      const hasOutput = !!(spec.state?.savedOutputs?.r2VideoUrl || spec.outputUrl);
      if (!hasOutput || !REVISION_ELIGIBLE_STATUSES.has(status)) {
        return res.status(409).json({
          ok: false,
          error: 'not_reviewable',
          message: 'This job is not ready for revision feedback yet.',
          status,
        });
      }
      ...
      if (!spec.state) spec.state = {};
      const prev = spec.state.customerRevision || {};

      const { ingestRevisionFeedback } = require('../services/revision_directive');
      const directive = await ingestRevisionFeedback(spec, {
        feedback: text,
      ...
      spec.state.customerRevision = {
        ...
        assemblyFixDirective: directive,
      };

      const { prepareJobForRetry, dispatchJobRetry } = require('../services/job_retry_dispatch');
      const { nrJobComplete, nrJobFailed, nrPortalFail } = require('../nr_events');

      if (autoRedo !== false) {
        prepareJobForRetry(spec, {
          operatorId: userId,
          action: 'customer_revision_redo',
          note: text.slice(0, 500),
        });
        await db.updateJobSpec(jobId, spec);
        dispatchJobRetry(jobId, spec, {
          resolvePortalWorkers: _resolvePortalWorkers,
          resolveExtensionWorkers: _resolveExtensionWorkers,
          logPrefix: '[revision]',
          nrPortalFail,
          nrJobComplete,
          nrJobFailed,
      ...
      createNotification(userId, {
        ...
        title:     autoRedo !== false
          ? 'Changes received — revising your video now'
          : 'Changes requested — we\'ll notify you when it\'s ready',
        body:      text.slice(0, 120),
        actionUrl: '/myjobs/active',
      }).catch(() => {});

      res.json({
        ok: true,
        jobId,
        status: autoRedo !== false ? 'queued' : 'held',
        autoRedo: autoRedo !== false,
        assemblyFixDirective: directive,
        message: autoRedo !== false
          ? 'Feedback applied — your video is being revised now. We\'ll notify you when it\'s ready to review again.'
          : 'Feedback sent. Your team will revise the video and notify you when it\'s ready to review again.',
      });
    } catch (err) {
      logError('CPD1200_REQUEST_REVISION_FAIL', err, { jobId, userId });
      res.status(500).json({ ok: false, error: 'request_revision_failed' });
    }
  }
);
...
router.post(
  ...
  async (req, res) => {
    ...
    try {
      ...
      const { RETRYABLE_STATUSES, prepareJobForRetry, dispatchJobRetry } = require('../services/job_retry_dispatch');
      ...
      const { directions, categories = [] } = req.body || {};
      const directionsText = String(directions || spec.state?.customerRevision?.feedback || '').trim();

      if (directionsText) {
        const { ingestRevisionFeedback } = require('../services/revision_directive');
        await ingestRevisionFeedback(spec, {
          feedback: directionsText,
          categories: Array.isArray(categories) ? categories : (spec.state?.customerRevision?.categories || []),
          source: 'operator',
          actorId: operatorId,
        });
      }
      ...
      prepareJobForRetry(spec, {
        operatorId,
        action: 'retry',
        note: directionsText || spec.state?.customerRevision?.feedback || null,
      });
      ...
      res.json({ ok: true, jobId, action: 'retry', previousStatus: currentStatus, directionsApplied: !!directionsText });
    } catch (err) {
...
/**
 * POST /jobs/:jobId/reapply-brand-chrome
 * Operator-only (CPD-1020). Fix brandId/streamer/logo on a completed job and re-burn chrome.
 * Optional body: { brandId, twitchLogin } — otherwise inferred from clip URLs.
 */
router.post(
  '/jobs/:jobId/reapply-brand-chrome',
  requireAuth,
  requireRole({ minLevel: ROLES.SUPERADMIN }),
  async (req, res) => {
    const db = require('../db');
    ...
    try {
      ...
      const { brandId, twitchLogin } = req.body || {};
      const { reapplyBrandChrome } = require('../services/brand_job_repair');
      const result = await reapplyBrandChrome(jobId, spec, {
      ...
      res.json({ ok: true, jobId, action: 'reapply-brand-chrome', ...result });
    } catch (err) {
      logError('CPD1020_REAPPLY_BRAND_CHROME_FAIL', err, { jobId, operatorId });
      res.status(422).json({ ok: false, error: 'reapply_brand_chrome_failed', message: err.message });
    }
  }
);
...
router.post(
  ...
  async (req, res) => {
    ...
    try {
      ...
      const { runPortalSequence } = require('../portal_policy_runner');
      const {
        runAssemblyAndPostProcess: _advAssemble,
        isPortal1Active: _advIsP1Active,
        runTtsMixAndChrome: _advTtsMix,
        ensureChromeApplied: _advChrome,
        runJobComplete: _advJobComplete,
      } = require('../services/pipeline_assembly');
      const workers = _resolvePortalWorkers(spec);
      const _advanceStartMs = Date.now();
      setImmediate(() => {
        // CPD-493: operator advance must mirror main dispatch — assembly, TTS mix, grading, notifications
        runPortalSequence({
          jobSpec: spec,
          portalWorkers: workers,
          extensionWorkers: _resolveExtensionWorkers(spec),
          onPortalPass: async (portalKey, result) => {
            const { saveJob: _sj } = require('../db');
            const _persist = (patch) => _sj(jobId, { ...spec, ...patch }).catch(() => {});
            if ((portalKey === 'portal1') || (portalKey === 'portal0' && !_advIsP1Active(spec))) {
              await _advAssemble(spec, jobId, { logPrefix: '[advance]', persist: _persist });
            }
            if (portalKey === 'tts_ext') {
              const ttsAudio = result?.audioPath || spec.state?.tts?.audioPath;
              if (ttsAudio) await _advTtsMix(spec, jobId, ttsAudio, { logPrefix: '[advance]', persist: _persist });
            }
            if (portalKey === 'portal3a') {
              await _advChrome(spec, jobId, { logPrefix: '[advance]' });
            }
          },
          onPortalFail: (pk, r) => { logError('CPD104_ADVANCE_PORTAL_FAIL', r?.reason || pk, { jobId }); nrPortalFail(spec, pk, r?.reason || r?.failReason); },
          onJobComplete: () => _advJobComplete(spec, jobId, { jobStartMs: _advanceStartMs, logPrefix: '[advance]', nrJobComplete }),
        ...
        // CPD-492: catch assembly failures propagated from onPortalPass
        }).catch((err) => logError('CPD104_ADVANCE_FAIL', err, { jobId }));
      });
...
module.exports._resolveCreatedBy       = _resolveCreatedBy;
module.exports._buildOrderSourceSummary = _buildOrderSourceSummary;
module.exports._buildFeatureComplianceSummary = _buildFeatureComplianceSummary;
