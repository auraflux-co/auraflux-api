  ...
  async (req, res) => {
    ...
    setImmediate(async () => {
      try {
        ...
        const { runAssemblyAndPostProcess } = require('../services/pipeline_assembly');
        ...
        runPortalSequence({
          ...
          onPortalPass: async (portalKey, result) => {
            ...
            // CPD-485: mirror the developer_api assembly step — assemble after portal1
            // so portal3a receives assembledPath. Previously missing on this path.
            if (portalKey === 'portal1') {
              const { saveJob: _sj } = require('../db');
              await runAssemblyAndPostProcess(jobSpec, jobId, {
                logPrefix: '[jobs/c1]',
                persist: (patch) => _sj(jobId, { ...jobSpec, ...patch }).catch(() => {}),
              });
            }
          },
          ...
          onJobComplete: () => {
            ...
            // CPD-422: grade job against spec before persisting
            try {
              const { gradeJob } = require('../services/job_grader');
              const gradeResult = gradeJob(jobSpec);
              jobSpec.specGrade = {
                grade:   gradeResult.grade,
                passed:  gradeResult.passed,
                gaps:    gradeResult.gaps,
                summary: gradeResult.summary,
                gradedAt: new Date().toISOString(),
              };
              console.log(`[jobs/c1] ${jobId}: spec grade ${gradeResult.grade}/100 — ${gradeResult.passed ? 'PASSED' : 'GAPS: ' + gradeResult.gaps.map((g) => g.checkId).join(', ')}`);
            } catch (_ge) {
              console.error(`[jobs/c1] ${jobId}: grader error —`, _ge.message);
            }
