...
function normaliseDashboardPayload(req, _res, next) {
  ...
  if (b.productionPath) {
    ...
    if (!feats.includes('script')) {
      // Opt out of script generation — skip portal1
  ...
  async (req, res) => {
    ...
    setImmediate(async () => {
      try {
        ...
        // consumeCredits is idempotent: ALREADY_CHARGED = safe to ignore.
        // PAUSED = overage cap hit — abort and mark job credit_paused.
...
async function _runWanPreGeneration(jobSpec, jobId) {
  ...
  // Ensure the ComfyUI pod is running before submitting the workflow.
  // Auto-starts a stopped pod or spins up a replacement on a new machine if the
  // current host is out of GPUs — no manual intervention required.
  try {
...
router.get(
  ...
  async (req, res) => {
    ...
    const { jobId } = req.params;
    const userId    = req.user.id;
    const isOp      = req.user.role === 'operator' || req.user.role === 'admin';
    ...
    try {
      ...
      setImmediate(() => {
        runPortalSequence({
          ...
          portalWorkers: workers,
          extensionWorkers: _resolveExtensionWorkers(),
          onPortalFail: (pk, r) => logError('CPD104_RETRY_PORTAL_FAIL', r?.reason || pk, { jobId }),
          onJobFailed: (fp, r) => logError('CPD104_RETRY_JOB_FAILED', r?.reason || fp, { jobId }),
    ...
    try {
      ...
      setImmediate(() => {
        runPortalSequence({
          ...
          portalWorkers: workers,
          extensionWorkers: _resolveExtensionWorkers(),
          onPortalFail: (pk, r) => logError('CPD104_ADVANCE_PORTAL_FAIL', r?.reason || pk, { jobId }),
          onJobFailed: (fp, r) => logError('CPD104_ADVANCE_JOB_FAILED', r?.reason || fp, { jobId }),
          persistJobStatus: ({ portalKey: pKey, phase }) => {
            try {
              const { saveJob } = require('../db');
              saveJob(jobId, { ...spec, status: phase, currentPortal: pKey, updatedAt: new Date().toISOString() })
...
// Export WAN pre-generation helper so developer_api.js can reuse it
