...
router.post(
  ...
  async (req, res) => {
    ...
    // CPD-444: Branding hard gate — if branding is requested but the brand has no
    // uploaded logo (image_url is null), auto-disable and surface a warning.
    // Silently falling back to the operator CWN logo is a product correctness bug.
    const jobWarnings = [];
    ...
    if (resolvedAddOns?.branding?.active) {
      ...
      if (!logoPresent) {
        ...
        jobWarnings.push({ code: 'BRANDING_NO_LOGO', message: 'Upload a logo to enable branding.' });
      }
    }
    ...
    const TEMPLATE_FEATURE_DEFAULTS = {
      'TikTok Clutch':       { colorGrade: { active: true, preset: 'crisp'  }, captions: { active: true, style: 'animated' }, effects: { zoom: true } },
      'IRL Story Time':      { colorGrade: { active: true, preset: 'warm'   }, captions: { active: true, style: 'burnin'   } },
      'Montage Hype Reel':   { colorGrade: { active: true, preset: 'vivid'  }, effects: { zoom: true } },
      'Quick Guide':         { colorGrade: { active: true, preset: 'cool'   }, captions: { active: true, style: 'animated' } },
      'YouTube Deep Dive':   { colorGrade: { active: true, preset: 'clean'  }, captions: { active: true, style: 'burnin'   } },
      'Reaction Cut':        { colorGrade: { active: true, preset: 'neutral'} },
    };
    ...
    if (_templateDefaults) {
      ...
      console.log(`[jobs/c1] ${jobId}: applied template defaults for "${jobSpec.templateName}" — colorGrade=${JSON.stringify(jobSpec.colorGrade?.preset)} captions=${JSON.stringify(jobSpec.captions?.active)} zoom=${JSON.stringify(jobSpec.effects?.zoom)}`);
    }
