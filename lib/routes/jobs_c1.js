...
router.post(
  ...
  async (req, res) => {
    ...
    // CPD-485: resolve productionProfile (vertical_reel, broadcast_desk, live_event) from
    // the wizard payload so assembly_service.js can apply the correct crop / reframe.
    // developer_api.js has done this since CPD-448; dashboard path was missing it.
    {
      const { productionProfile: _pp } = resolveProductionProfileAndContentType(req.body);
      if (_pp) jobSpec.productionProfile = _pp;
    }
