...
const { createJobSpec, getJobSpec, updateJobSpec } = require('../job_spec');
...
router.get(
  ...
  (req, res) => {
    ...
    try {
      const rows = allJobs
        ? db.prepare(
            'SELECT id, customer_id, job_spec, created_at, updated_at FROM jobs ORDER BY created_at DESC LIMIT 100'
          ).all()
        : db.prepare(
            'SELECT id, customer_id, job_spec, created_at, updated_at FROM jobs WHERE customer_id = ? ORDER BY created_at DESC LIMIT 50'
          ).all(userId);

      const jobs = rows.map((row) => {
        const spec = row.job_spec ? JSON.parse(row.job_spec) : {};
        return {
          jobId:               row.id,
...
router.get(
  ...
  (req, res) => {
    ...
    try {
      const row = db.prepare('SELECT id, customer_id, job_spec, created_at, updated_at FROM jobs WHERE id = ?').get(jobId);
...
// ── PUT /jobs/:jobId/schedule — update scheduled publish time (CPD-E) ──────────
router.put(
  '/jobs/:jobId/schedule',
  apiLimit,
  requireAuth,
  async (req, res) => {
    ...
    if (!scheduledPublishAt && scheduledPublishAt !== null) {
      return res.status(400).json({ ok: false, error: 'scheduledPublishAt required (ISO string or null to clear)' });
    }

    try {
      const spec = getJobSpec(jobId);
      if (!spec) return res.status(404).json({ ok: false, error: 'Job not found' });
      if (String(spec.customerId) !== String(userId) && req.user?.role !== 'admin' && req.user?.role !== 'operator') {
        return res.status(403).json({ ok: false, error: 'Forbidden' });
      }
      ...
      updateJobSpec(jobId, spec);

      res.json({ ok: true, jobId, scheduledPublishAt });
    } catch (err) {
      logError('UPDATE_JOB_SCHEDULE_FAIL', err, { jobId, userId });
