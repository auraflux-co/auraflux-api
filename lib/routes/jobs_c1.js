...
function _wrapPortal1WithFixDirective(mod, jobSpec) {
  ...
  return {
    ...
    runWorker: async (_opts) => {
      if ((_opts?.workerAttempt || 1) > 1 && jobSpec.state?.portal1FixDirective) {
        try {
          const { generateJobScript: _regen } = require('../script_gen_service');
          const _fd = jobSpec.state.portal1FixDirective;
          console.log(`[portal1-wrapper] ${jobId}: attempt ${_opts.workerAttempt} — regenerating script with fixDirective`);
 ...
 * Does not re-run pipeline; operator uses POST /jobs/:id/retry after reading feedback.
 */
router.post(
  ...
  async (req, res) => {
    ...
    const { jobId } = req.params;
    const userId = req.user.id;
    const { feedback, categories = [] } = req.body || {};
    ...
    try {
      ...
      createNotification(userId, {
        ...
        title:     'Changes requested — we\'ll notify you when it\'s ready',
      ...
      res.json({
        ...
        jobId,
        status: 'held',
        message: 'Feedback sent. Your team will revise the video and notify you when it\'s ready to review again.',
      });
    } catch (err) {
...
router.post(
  ...
  async (req, res) => {
    ...
    try {
      ...
      if (!['failed', 'held', 'complete', 'staged'].includes(currentStatus)) {
      ...
      const revisionNote = spec.state.customerRevision?.feedback || null;
      spec.state.operatorActions = [
        ...(spec.state.operatorActions || []),
        { action: 'retry', operatorId, at: spec.updatedAt, customerRevision: revisionNote },
      ];
      ...
      setImmediate(() => {
        // CPD-493: operator retry must mirror main dispatch — assembly, TTS mix, grading, notifications
        runPortalSequence({
          jobSpec: spec,
          portalWorkers: workers,
          ...
          onPortalPass: async (portalKey, result) => {
            ...
            if ((portalKey === 'portal1') || (portalKey === 'portal0' && !isPortal1Active(spec))) {
              await runAssemblyAndPostProcess(spec, jobId, { logPrefix: '[retry]', persist: _persist });
            }
            if (portalKey === 'tts_ext') {
              const ttsAudio = result?.audioPath || spec.state?.tts?.audioPath;
              if (ttsAudio) await runTtsMixAndChrome(spec, jobId, ttsAudio, { logPrefix: '[retry]', persist: _persist });
            }
            if (portalKey === 'portal3a') {
              await ensureChromeApplied(spec, jobId, { logPrefix: '[retry]' });
            }
          },
          onPortalFail: (pk, r) => { logError('CPD104_RETRY_PORTAL_FAIL', r?.reason || pk, { jobId }); nrPortalFail(spec, pk, r?.reason || r?.failReason); },
          onJobComplete: () => runJobComplete(spec, jobId, { jobStartMs: _retryStartMs, logPrefix: '[retry]', nrJobComplete }),
        ...
        }).catch((err) => logError('CPD104_RETRY_FAIL', err, { jobId }));
      });
