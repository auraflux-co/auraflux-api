...
router.post(
  ...
  async (req, res) => {
    ...
    try {
      ...
      // This lets customers correct AI-generated title/description/captions without a separate PATCH.
      const _reviewMeta = req.body.publishMeta || {};
      if (Object.keys(_reviewMeta).length) {
        spec.order = spec.order || {};
        spec.order.publish = spec.order.publish || {};
        if (_reviewMeta.title       !== undefined) spec.order.publish.title           = _reviewMeta.title;
        if (_reviewMeta.description !== undefined) spec.order.publish.description     = _reviewMeta.description;
        if (_reviewMeta.tags        !== undefined) spec.order.publish.tags            = _reviewMeta.tags;
        if (_reviewMeta.privacyStatus !== undefined) spec.order.publish.privacyStatus = _reviewMeta.privacyStatus;
        if (_reviewMeta.tiktokCaption !== undefined) spec.order.publish.tiktokCaption = _reviewMeta.tiktokCaption;
        if (_reviewMeta.instagramCaption !== undefined) spec.order.publish.instagramCaption = _reviewMeta.instagramCaption;
      }
      ...
      const { publishResultsHadSuccess } = require('../services/approve_publish');
      ...
      if (!publishResultsHadSuccess(results)) {
        ...
        return res.status(422).json({
          ...
          message: 'No platform uploads succeeded.',
          platforms: results,
        });
      }
