...
router.post('/public/review-share/:token/approve', async (req, res) => {
  try {
    ...
    // Guest approve always publishes private — allow past grade/chrome holds for draft publish
    const readiness = assertPublishReadiness(jobSpec, { forceApprove: true });
    if (!readiness.ok) {
