 ...
 * One account can own multiple brands.
 *
 * Routes:
 ...
 *
 * All routes require Better Auth JWT auth (requireAuth).
 */
...
router.patch('/:id', requireAuth, async (req, res) => {
  const { id } = req.params;
  const {
    name, image_url, description, intro_card_url, outro_card_url,
    creative_profile, publish_strategy_rules,
  } = req.body || {};
  ...
  if (creative_profile !== undefined) {
    const { normalizeCreativeProfile } = require('../services/brand_creative_profile');
    fields.creative_profile = normalizeCreativeProfile(creative_profile);
  }
  if (publish_strategy_rules !== undefined) {
    if (!Array.isArray(publish_strategy_rules)) {
      return res.status(400).json({ ok: false, error: 'invalid_publish_strategy_rules', message: 'Must be an array' });
    }
    fields.publish_strategy_rules = publish_strategy_rules;
  }
...
const ASSET_MIME = {
  ...
  caption_font: ['font/ttf', 'font/otf', 'application/x-font-ttf', 'application/x-font-otf', 'application/octet-stream'],
};
...
router.post('/:id/upload', requireAuth, upload.single('file'), async (req, res) => {
  ...
  if (!ASSET_MIME[assetType]) {
    return res.status(400).json({ ok: false, error: 'invalid_asset_type', message: 'assetType must be logo, intro_card, outro_card, or caption_font.' });
  }

  if (assetType === 'caption_font') {
    ...
    const defaults = getPlanDefaults(plan?.tier || 'operate');
    const allowed = plan?.tier
      ? (getPlanDefaults(plan.tier).caption_font_upload || defaults.caption_font_upload)
      : false;
    // Operate+ (Pro) and above — Growth cannot upload custom fonts
    if (!allowed || (plan?.tier || '').toLowerCase() === 'growth') {
      return res.status(403).json({
        ok: false,
        error: 'plan_upgrade_required',
        message: 'Custom caption font uploads require Pro Operator / Agency or higher.',
      });
    }
  }
  if (!req.file) {
  ...
  try {
    ...
    if (assetType === 'caption_font') {
      const fontName = (req.body?.fontName || req.file.originalname || 'Custom Font').slice(0, 120);
      const { getPool } = require('../db/postgres');
      const pool = getPool();
      await pool.query(
        `INSERT INTO brand_caption_fonts (brand_id, account_id, font_name, r2_key)
         VALUES ($1, $2, $3, $4)`,
        [id, req.user.id, fontName, key],
      );
      // Brand template pointer — latest custom caption font for this brand
      try {
        await pool.query(
          `UPDATE brands SET caption_font_key = $1, caption_font_url = $2 WHERE id = $3 AND account_id = $4`,
          [key, assetUrl, id, req.user.id],
        );
      } catch (_colErr) {
        // columns may not exist until migration 044 — non-fatal; ledger row is enough
      }
    }
