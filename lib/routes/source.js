...
router.get('/source/twitch/:username/content', auth, apiLimit, async (req, res) => {
  ...
  try {
    ...
    // CPD-339: Use clip page URLs as primary URL for Twitch clips.
    // Direct CDN URLs (cloudfront.net/nauth/*) are blocked from Render's datacenter IPs
    // because they are unsigned — CloudFront enforces residential-IP-only access.
    // portal0's yt-dlp probe handles page URLs natively: it authenticates with Twitch's
    // GQL API from Render, obtains a signed CDN URL, and probes metadata.
    // The signed URL portal0 returns via directUrl IS accessible from Render.
    // assembly_service uses yt-dlp to download clip page URLs directly.
    //
    // directCdnUrl is kept as a metadata field for UI display / analytics only —
    // it is NOT used as the submission URL.
    ...
    const normalizedClips = clips.map((c, i) => {
      const directCdnUrl = clipUrlResults[i]?.status === 'fulfilled' ? clipUrlResults[i].value : null;
      return {
        id:           c.id,
