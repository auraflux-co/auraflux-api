...
router.get('/source/twitch/:username/content', auth, apiLimit, async (req, res) => {
  ...
  try {
    ...
    const normalizedClips = clips.map((c) => {
      // CPD-292: Portal 0 rejects watch-page URLs (twitch.tv/user/clip/...) as
      // "URL is a web page, not a media file". Try to resolve to a direct CDN MP4
      // via thumbnailToMp4(). Only use the result if it ends in .mp4 — older clips
      // (clips-media-assets2.twitch.tv/{id}-preview-NxN.jpg) convert cleanly;
      // newer thumbnails (static-cdn.jtvnw.net) don't match the pattern and would
      // produce a non-MP4 URL that Portal 0 also rejects.
      // Fall back to clips.twitch.tv short URL (yt-dlp resolves these fine, but
      // Portal 0 still needs to accept it — the downloader pre-check is the blocker).
      const candidate = c.thumbnail_url ? client.thumbnailToMp4(c.thumbnail_url) : '';
      const directUrl = candidate.endsWith('.mp4') ? candidate : null;
      return {
        id:           c.id,
        ...
        url:          directUrl || c.url || `https://www.twitch.tv/clip/${c.id}`,
