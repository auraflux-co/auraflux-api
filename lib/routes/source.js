...
const KickUserApiClient = require('../clients/kick_user_api');
  ...
  try {
    ...
    const normalizedClips = clips
      .map((c, i) => {
        ...
        // Use the page URL as the primary `url` — Twitch CDN URLs (cloudfront.net/nauth/)
        // are IP-blocked from Render datacenter even when signed with GQL sig/token params.
        // portal0 and assembly_service use yt-dlp on the page URL to bypass this restriction.
        // signedUrl is preserved for UI preview / future CDN proxy approach (CPD-348).
        const pageUrl = c.url || `https://clips.twitch.tv/${c.id}`;
        // CPD-349: Derive public CDN URL from thumbnail (clips-media-assets2.twitch.tv)
        // This bypasses Twitch GQL entirely for portal0 probe + assembly downloads.
        // Falls back gracefully if thumbnail pattern doesn't match.
        const cdnUrl = client.thumbnailToMp4(c.thumbnail_url) || null;
        return {
          id:           c.id,
          ...
          url:          pageUrl,
          cdnUrl,
          signedUrl:    signedCdnUrl || null,
          viewCount:    c.view_count || 0,
          platform:     'twitch',
          contentType:  'clip',
        };
      }); // No filter — page URL always available even if GQL resolve fails
...
router.get('/source/kick/:username/content', auth, apiLimit, async (req, res) => {
  ...
  try {
    ...
    // CPD-353: Use connected OAuth token (Kick public API v1) when available —
    // no Cloudflare bypass needed, no Apify cost, no TLS fingerprint tricks.
    const storedToken = customerId
      ? await tokenStore.loadTokens(customerId, null, 'kick').catch(() => null)
      : null;
    ...
    if (storedToken?.accessToken) {
      const userClient = new KickUserApiClient(storedToken, customerId);
      console.log(`[source/kick] using OAuth token for ${username} (customer ${customerId})`);
      raw = await userClient.getContent(username, Math.min(50, limit * 2), { type: kickType });
      channel = {
        id:          username.toLowerCase(),
        slug:        username.toLowerCase(),
        username:    username.toLowerCase(),
        displayName: storedToken.platformHandle || username,
        avatarUrl:   null,
        isLive:      false,
        followersCount: 0,
      };
    } else {
      // Apify / TLS-fingerprint fallback
      const client = new KickClient();
    ...
    if (err.isKickUnavailable) {
      ...
      return res.status(503).json({
        ...
        workaround: 'Connect your Kick account in Settings → My Channels to bypass this.',
      });
    }
...
function parseVodDuration(str) {
  const TwitchClient = require('../clients/twitch_client');
  return TwitchClient.parseVodDuration(str);
}
