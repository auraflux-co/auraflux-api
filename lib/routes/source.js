...
router.get('/source/twitch/:username/content', auth, apiLimit, async (req, res) => {
  ...
  try {
    ...
    const normalizedClips = clips.map((c) => {
      ...
      // "URL is a web page, not a media file". Resolve to the direct CDN MP4 URL
      // from the thumbnail URL (clips-media-assets2.twitch.tv/{id}.mp4) which
      // Portal 0 accepts. Fall back to watch-page URL only if thumbnail is absent.
      const directUrl = c.thumbnail_url ? client.thumbnailToMp4(c.thumbnail_url) : null;
      return {
        id:           c.id,
