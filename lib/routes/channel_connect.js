...
const { verifyAuthJwt } = require('../auth/jwt');
...
function getSourceAdapters() {
  const adapters = { kick: kickOAuth };
  const twitchSecret = process.env.TWITCH_CLIENT_SECRET || process.env.TWITCH_OAUTH_CLIENT_SECRET;
  if (twitchSecret) {
    adapters.twitch = require('../publish/adapters/twitch_oauth');
  }
  return adapters;
}
...
async function requireAuthOrQueryToken(req, res, next) {
  ...
  if (queryToken) {
    try {
      const payload = verifyAuthJwt(String(queryToken), { clockSkewSec: 60 });
      if (!payload?.sub) throw new Error('no sub');
      req.user = {
        id:       payload.sub,
        role:     payload.role || 'customer',
        planTier: payload.planTier || 'operate',
...
router.get('/channels/connect/:platform', requireAuthOrQueryToken, async (req, res) => {
  ...
  const adapter = getSourceAdapters()[platform];
  if (!adapter) {
    return res.status(400).json({ error: `Unknown source platform: ${platform}. Supported: ${Object.keys(getSourceAdapters()).join(', ')}` });
  }
...
router.get('/channels/callback/:platform', async (req, res) => {
  ...
  try {
    ...
    const { assertCanConnectPlatform } = require('../services/channel_limits');
    try {
      await assertCanConnectPlatform(stateData.customerId, platform);
    } catch (limitErr) {
      if (limitErr.code === 'CHANNEL_LIMIT') {
        return res.redirect(
          `${settingsUrl}?channel_error=${encodeURIComponent(limitErr.message)}&platform=${platform}`
        );
      }
      throw limitErr;
    }
...
router.get('/channels/connections', requireAuth, async (req, res) => {
  try {
    ...
    const platforms  = Object.keys(getSourceAdapters());
    const results = await Promise.all(
      platforms.map(async (p) => {
        let tokens = await loadTokens(customerId, null, p).catch(() => null);
        if (!tokens) return null;
        if (p === 'kick') {
          tokens = await kickOAuth.syncKickProfile(customerId, tokens);
        }
        if (p === 'twitch' && tokens.platformHandle) {
          const clean = String(tokens.platformHandle).replace(/^@/, '').toLowerCase();
          await dbQuery(
            `UPDATE client_plans
                SET source_channels = source_channels || $1::jsonb
              WHERE client_id = $2 AND active = TRUE`,
            [JSON.stringify({ twitchLogin: clean }), customerId],
          ).catch(() => {});
        }
        return {
          platform:       p,
    ...
    res.json({ ok: true, connections: results.filter(Boolean), oauthPlatforms: platforms });
  } catch (err) {
