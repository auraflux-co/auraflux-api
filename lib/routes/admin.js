...
const GITHUB_REPO  = process.env.GITHUB_REPO  || 'clipzworldnews/auraflux-api';

// Search open/recent PRs for a given Jira issue key and post a status comment
async function postJiraStatusToGitHub(issueKey, status, summary, actor) {
  ...
  const body = `${statusEmoji} **Jira status update**\n\n` +
  ...
  return new Promise((resolve) => {
    const searchQuery = encodeURIComponent(`repo:${owner}/${repo} is:pr head:${issueKey.toLowerCase()} in:head`);
    const options = {
      hostname: 'api.github.com',
      path: `/search/issues?q=${searchQuery}&per_page=5`,
      headers: { 'User-Agent': 'auraflux-api', Authorization: `Bearer ${GITHUB_TOKEN}`, Accept: 'application/vnd.github+json' },
    };
...
module.exports = function createAdminRouter({ _healthCache, BUILD_INFO }) {
  ...
  router.post('/api/jira-webhook', webhookLimit, validateBodySize(65536), (req, res) => {
    try {
      ...
      if (!isAider && !isCursor && !isRovo && !isReviewRequest && !isApproved) {
        return res.json({ queued: false, reason: 'no aider/cursor/rovo label and no recognised action' });
      }
      ...
      const entry = {
        ...
        receivedAt: new Date().toISOString(), processed: false,
      };
      ...
      if (isApproved || isReviewRequest) {
        postJiraStatusToGitHub(key, isApproved ? 'Approved' : 'In Review', summary).catch(() => {});
      }
  ...
  router.post('/api/github-sync', webhookLimit, validateBodySize(16384), async (req, res) => {
    try {
      ...
      const { issueKey, status, summary, actor } = req.body || {};
      if (!issueKey || !status) return res.status(400).json({ error: 'issueKey and status required' });

      const posted = await postJiraStatusToGitHub(issueKey, status, summary || '', actor || '');
  ...
  router.post('/api/admin/migrate-pg', async (req, res) => {
    ...
    try {
      ...
      execFile('node', [scriptPath], { env: process.env, cwd: path.join(__dirname, '../..') },
        (err, stdout, stderr) => {
