...
async function postJiraStatusToGitHub(issueKey, status, summary, actor, commentBody) {
  ...
  if (status === 'comment' && commentBody) {
    ...
    body =
      `💬 **Jira comment** on [${issueKey}](https://aurafluxco.atlassian.net/browse/${issueKey})` +
      `${actor ? ` from **${actor}**` : ''}:\n\n---\n\n${commentBody}`;
  } else {
    const statusEmoji =
      {
    ...
    body =
      ...
      (summary ? `_${summary}_\n` : '') +
      `\nhttps://aurafluxco.atlassian.net/browse/${issueKey}`;
  }
...
module.exports = function createAdminRouter({ _healthCache, BUILD_INFO }) {
  ...
  // ── POST /admin/api/generate-test-key ───────────────────────────────────────
  // One-shot endpoint: create an API key for any Clerk user ID.
  // Protected by X-Admin-Secret. Intended for CI/CD and automated QA test setup.
  router.post('/api/generate-test-key', async (req, res) => {
    const secret = process.env.ADMIN_SECRET || process.env.NEW_RELIC_LICENSE_KEY;
    if (!secret || req.headers['x-admin-secret'] !== secret) {
      return res.status(403).json({ ok: false, error: 'forbidden' });
    }
    const { userId, planTier = 'operate', name = 'QA Test Key' } = req.body || {};
    if (!userId) return res.status(400).json({ ok: false, error: 'userId required' });
    try {
      const { createApiKey } = require('../services/api_keys');
      const { key, record } = await createApiKey(userId, planTier, name);
      res.json({ ok: true, key, prefix: record.key_prefix, planTier: record.plan_tier });
    } catch (err) {
      res.status(500).json({ ok: false, error: err.message });
    }
  });

  // Temporary portal diagnostic — returns canProduce state + ffprobe URL probe result
  router.get('/api/portal0-creds', async (req, res) => {
    const secret = process.env.ADMIN_SECRET || process.env.NEW_RELIC_LICENSE_KEY;
    if (!secret || req.headers['x-admin-secret'] !== secret) {
      return res.status(403).json({ ok: false, error: 'forbidden' });
    }
    const { canProduce } = require('../portals/portal0');
    const testUrl = req.query.url || 'https://samplelib.com/mp4/sample-30s.mp4';
    const mockSpecs = {
      news:  { jobId: 'diag', contentType: 'news',  order: { inputs: { items: [{ id: 'i0', url: testUrl }] } } },
      clips: { jobId: 'diag', contentType: 'clips', order: { inputs: { items: [{ id: 'i0', url: testUrl }] } } },
    };
    const cpResults = {};
    for (const [ct, spec] of Object.entries(mockSpecs)) {
      cpResults[ct] = canProduce(spec);
    }

    // Probe the URL with ffprobe directly
    let probeResult = null;
    let probeError = null;
    const { ffprobePath } = require('../ffmpeg_utils');
    const { execFile } = require('child_process');
    await new Promise((resolve) => {
      const args = ['-v','error','-select_streams','v:0','-show_entries','stream=width,height,duration','-show_entries','format=duration','-of','json', testUrl];
      execFile(ffprobePath(), args, { timeout: 20000 }, (err, stdout) => {
        if (err) { probeError = `${err.message} | stderr: ${(err.stderr||'').slice(0,200)}`; }
        else {
          try {
            const d = JSON.parse(stdout);
            const s = (d.streams||[])[0]||{};
            probeResult = { width: +s.width||0, height: +s.height||0, duration: parseFloat(s.duration||d.format?.duration||'0') };
    ...
    res.json({
      ...
      canProduce: cpResults,
      probe: probeResult,
      probeError,
    });
  });

  // POST /api/clerk-webhook — Clerk user lifecycle events
  // Assigns default planTier:operate to all new signups so they can access the platform.
  router.post('/api/clerk-webhook', webhookLimit, validateBodySize(65536), async (req, res) => {
    try {
      ...
      if (webhookSecret && svixId && svixSignature) {
        ...
        try {
          ...
          console.warn('[clerk-webhook] signature verification failed:', verifyErr.message);
      ...
      if (type === 'user.created') {
        ...
        const existing = data?.public_metadata || {};
        if (existing.planTier) {
          // Already has a plan — don't overwrite
          return res.json({ ok: true, action: 'skipped', reason: 'planTier already set' });
        }

        const { createClerkClient } = require('@clerk/express');
        const clerk = createClerkClient({ secretKey: process.env.CLERK_SECRET_KEY });
        await clerk.users.updateUser(userId, {
          publicMetadata: { planTier: 'operate', role: 'customer' },
        });
        console.log(`[clerk-webhook] user.created ${userId} — assigned operate/customer`);
        return res.json({ ok: true, action: 'assigned', planTier: 'operate' });
      }

      return res.json({ ok: true, action: 'ignored', type });
    } catch (err) {
