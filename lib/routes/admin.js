...
module.exports = function createAdminRouter({ _healthCache, BUILD_INFO }) {
  ...
  // Temporary portal credential readiness diagnostic — returns which creds are set without values
  router.get('/api/portal0-creds', (req, res) => {
    const secret = process.env.ADMIN_SECRET || process.env.NEW_RELIC_LICENSE_KEY;
    if (!secret || req.headers['x-admin-secret'] !== secret) {
      return res.status(403).json({ ok: false, error: 'forbidden' });
    }
    ...
    const mockSpecs = {
      news:  { jobId: 'diag', contentType: 'news',  order: { inputs: { items: [{ id: 'i0', url: 'https://samplelib.com/mp4/sample-30s.mp4' }] } } },
      clips: { jobId: 'diag', contentType: 'clips', order: { inputs: { items: [{ id: 'i0', url: 'https://samplelib.com/mp4/sample-30s.mp4' }] } } },
    };
    const results = {};
    for (const [ct, spec] of Object.entries(mockSpecs)) {
      results[ct] = canProduce(spec);
    }
    res.json({
      ok: true,
      env: {
