...
module.exports = function createAdminRouter({ _healthCache, BUILD_INFO }) {
  ...
  router.get('/api/portal0-creds', async (req, res) => {
    ...
    await new Promise((resolve) => {
      const args = ['-v','quiet','-select_streams','v:0','-show_entries','stream=width,height,duration','-show_entries','format=duration','-of','json', testUrl];
      execFile(ffprobePath(), args, { timeout: 20000 }, (err, stdout) => {
        if (err) { probeError = err.message; }
        else {
          try {
