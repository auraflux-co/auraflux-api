...
function buildPrompt({ schemaLines, currentLines, designBrief, framerContent, pageFilesSection, workerContent, instruction }) {
  ...
  return [
    ...
    'DEFAULT RESPONSE: execute the change and return html_patch or changes.',
    'ONLY use type "message" when: (a) you genuinely cannot determine which page to edit, OR (b) the user is explicitly asking a question.',
    'NEVER restate the request back. NEVER ask "can you clarify". NEVER say "great!" or "thanks for the direction".',
    'If the user says "do it", "yes", "proceed", "go", or repeats a request — execute immediately, no questions.',
    'If a request touches multiple pages, return html_patches (array) covering all of them in one shot.',
    '',
    '## CONTEXT',
    ...
    '',
    'Option C — one page HTML rewrite:',
    ...
    '',
    'Option D — multiple pages rewritten at once (use when request touches >1 page):',
    '{ "type": "html_patches", "patches": [{ "page": str, "description": str, "html": str }, ...] }',
    '',
    '## HTML PATCH RULES',
