...
${designBrief ? `## Design Migration Brief\n${designBrief}\n\n` : ''}${framerContent ? `## Framer Published Content (live, read-only)\n${framerContent}\n\n` : ''}${pageFilesSection ? `## Worker Page Source Files (live, editable — these are what you can rewrite)\n${pageFilesSection}\n\n` : (workerContent ? `## Worker-Owned Pages (current live HTML)\n${workerContent}\n\n` : '')}
...
- For html_patch: produce complete, valid HTML. Preserve ${FRAMER_FONTS || ''}, ${FRAMER_NAV || FALLBACK_NAV}, ${FRAMER_FOOTER || FALLBACK_FOOTER} expressions in pricing.html. Preserve data-editable attributes. Do NOT invent new page names — only "pricing", "contact-content", or "roadmap-content".
