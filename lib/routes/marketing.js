 ...
 *
 * SINGLE SOURCE OF TRUTH: after every successful Cloudflare deploy, commitToGit()
 * pushes the changed files back to the git repo (with [skip ci] so GitHub Actions
 * does not re-deploy). This ensures git and the live site are always in sync
 * regardless of whether a change came from code or from this in-app editor.
 */
...
const WORKER_CONSTANTS = {
  FRAMER_FOOTER:   'Primary footer HTML — shown on ALL pages (home, blog, pricing, contact, roadmap, legal, developer-api). This is what actually renders. Edit this to change the footer.',
  FRAMER_NAV:      'Primary nav HTML — shown on ALL pages. Edit this to change navigation links or branding.',
  FALLBACK_FOOTER: 'Emergency fallback footer — only used if framer-shell/footer.html is missing or empty. Rarely needed.',
  FALLBACK_NAV:    'Emergency fallback nav — only used if framer-shell/nav.html is missing or empty. Rarely needed.',
...
/**
 * Framer-shell files that back FRAMER_FOOTER and FRAMER_NAV.
 * deploy.sh reads these files and injects them as FRAMER_FOOTER / FRAMER_NAV.
 * Editing _worker.js constants for these has no effect — edits must go to these files.
 */
const FRAMER_SHELL_FILES = {
  FRAMER_FOOTER: path.join(CLOUDFLARE_DIR, 'framer-shell', 'footer.html'),
  FRAMER_NAV:    path.join(CLOUDFLARE_DIR, 'framer-shell', 'nav.html'),
};

// ── Git commit-back ───────────────────────────────────────────────────────────
// After every successful Cloudflare deploy triggered from the in-app editor,
// push the changed files back to git so GitHub Actions CI never overwrites them.
// Uses the GitHub Contents API with [skip ci] to prevent a redundant CI deploy.
const GITHUB_REPO  = 'auraflux-co/auraflux-api';
const GITHUB_BRANCH = 'main';

async function commitToGit(files, description) {
  const token = process.env.GITHUB_API_TOKEN;
  if (!token) {
    const msg = '[marketing] GITHUB_API_TOKEN not set — git and Cloudflare are now out of sync. Add GITHUB_API_TOKEN to Render env vars.';
    console.error(msg);
    const Sentry = (() => { try { return require('@sentry/node'); } catch { return null; } })();
    if (Sentry) Sentry.captureMessage(msg, 'error');
    throw new Error(msg);
  }
  ...
  for (const { repoPath, localPath } of files) {
    try {
      ...
      // Fetch current SHA (needed by GitHub API to update an existing file)
      const shaRes = await new Promise((resolve, reject) => {
        const req = https.request(
          ...
          { headers: { Authorization: `Bearer ${token}`, 'User-Agent': 'auraflux-api', Accept: 'application/vnd.github+json' } },
      ...
      const payload = JSON.stringify({
        message: `chore(marketing): ${description} [skip ci]`,
        content: encoded,
      ...
      console.log(`[marketing] committed ${repoPath} to git`);
    } catch (err) {
      console.error(`[marketing] git commit-back failed for ${repoPath}:`, err.message);
    }
  }
}

/** Read current values of editable worker constants from _worker.js (or framer-shell files for FRAMER_*) */
function loadWorkerConstants() {
  try {
    const workerPath = path.join(CLOUDFLARE_DIR, '_worker.js');
    const src = fs.existsSync(workerPath) ? fs.readFileSync(workerPath, 'utf8') : '';
    const result = {};
    for (const name of Object.keys(WORKER_CONSTANTS)) {
      // FRAMER_FOOTER and FRAMER_NAV are backed by framer-shell files, not _worker.js
      if (FRAMER_SHELL_FILES[name]) {
        try {
          const filePath = FRAMER_SHELL_FILES[name];
          if (fs.existsSync(filePath)) {
            result[name] = fs.readFileSync(filePath, 'utf8').slice(0, 3000);
          }
        } catch { /* ok */ }
        continue;
      }
      if (src) {
        ...
        if (m) result[name] = m[1].slice(0, 3000);
      }
    }
    return result;
  } catch { return {}; }
...
const PAGE_FILES = {
  home:              'home.html',
  blog:              'blog.html',
  pricing:           'pricing.html',        // /plans page
  about:             'about.html',          // /our-story
  system:            'system.html',         // /our-system
...
function buildPrompt({ schemaLines, currentLines, designBrief, framerContent, pageFilesSection, workerContent, workerConstants, instruction }) {
  ...
  return [
    ...
    'For footer changes: constant = "FRAMER_FOOTER". For nav changes: constant = "FRAMER_NAV". NEVER use FALLBACK_FOOTER or FALLBACK_NAV for user-visible edits — those are emergency-only.',
    ...
    '{ "type": "worker_edit", "description": "what changed", "edits": [{ "constant": "FRAMER_FOOTER"|"FRAMER_NAV"|"FALLBACK_FOOTER"|"FALLBACK_NAV"|"INJECTED_CSS", "value": "new HTML or CSS string — no backticks inside" }] }',
    'USE THIS FOR: site footer (FRAMER_FOOTER), site nav (FRAMER_NAV), global CSS (INJECTED_CSS). FRAMER_FOOTER and FRAMER_NAV are what actually render — always prefer these over FALLBACK_* variants.',
    '',
  ...
  async (req, res) => {
    ...
    try {
      ...
      for (const { constant, value } of edits) {
        // FRAMER_FOOTER and FRAMER_NAV are backed by framer-shell files —
        // writing to _worker.js has no effect because deploy.sh always re-injects
        // from these files. Write directly to the framer-shell file instead.
        if (FRAMER_SHELL_FILES[constant]) {
          const shellPath = FRAMER_SHELL_FILES[constant];
          const shellBackup = shellPath + '.bak';
          if (fs.existsSync(shellPath)) fs.copyFileSync(shellPath, shellBackup);
          fs.writeFileSync(shellPath, value, 'utf8');
          console.log(`[marketing] worker-edit: wrote ${constant} to ${shellPath}`);
          continue;
        }
    ...
    // Commit changed files back to git so CI never overwrites in-app edits.
    // FRAMER_FOOTER/FRAMER_NAV edits go to framer-shell files, not _worker.js.
    const desc = `worker-edit: ${edits.map(e => e.constant).join(', ')}`;
    const gitFiles = [];
    const workerTouched = edits.some(e => !FRAMER_SHELL_FILES[e.constant]);
    if (workerTouched) {
      gitFiles.push({ repoPath: 'cloudflare/marketing/_worker.js', localPath: workerPath });
    }
    for (const { constant } of edits) {
      if (FRAMER_SHELL_FILES[constant]) {
        const shellPath = FRAMER_SHELL_FILES[constant];
        const relPath = `cloudflare/marketing/framer-shell/${path.basename(shellPath)}`;
        gitFiles.push({ repoPath: relPath, localPath: shellPath });
      }
    }
    if (gitFiles.length) {
      commitToGit(gitFiles, desc)
        .catch(e => console.error('[marketing] worker-edit commit-back failed:', e.message));
    }
...
router.post(
  ...
  async (req, res) => {
    ...
    // Commit all patched page files back to git
    const gitFiles = patches.map(({ page }) => ({
      repoPath: `cloudflare/marketing/pages/${PAGE_FILES[page]}`,
      localPath: path.join(PAGES_DIR, PAGE_FILES[page]),
    }));
    commitToGit(gitFiles, `html-patches: ${patches.map(p => p.page).join(', ')}`)
      .catch(e => console.error('[marketing] html-patches commit-back failed:', e.message));
...
router.post(
  ...
  async (req, res) => {
    ...
    // Commit the patched page file back to git
    commitToGit([{ repoPath: `cloudflare/marketing/pages/${PAGE_FILES[page]}`, localPath: filePath }],
      `html-patch: ${page}`)
      .catch(e => console.error('[marketing] html-patch commit-back failed:', e.message));
