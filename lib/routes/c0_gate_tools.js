...
const { SYSTEM_FONT, CWN_LOGO_PATH } = require('../services/branding_assets');
...
// POST /gate2-segment-qa — Portal 2: Gemini reviews completed HeyGen segments
...
router.post('/nba/generate-intro-card', async (req, res) => {
  ...
  // Validate gameId is a safe numeric ESPN game ID — prevents SSRF via puppeteer page.goto()
  if (!/^\d+$/.test(String(gameId))) {
    return res.status(400).json({ ok: false, error: 'Invalid gameId: must be a numeric ESPN game ID' });
  }

  const { assertNumericGameId, gotoLocalTemplateWithParams } = require('../puppeteer_safe');
