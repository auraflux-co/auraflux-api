...
const { getCreditBalance, getClientPlan, getOrCreateClientPlan, getPool, insertCreditPack, hasStripeEvent, recordStripeEvent, updateClientPlanTier, updateBrandPlanTier, setManagedAddon, setAutoTopupEnabled } = require('../db');
const { findUnclaimedPendingBySession, hasPaidSubscription } = require('../auth/paywall');
const { updateProfileMeta } = require('../auth/profiles');
const { createNotification } = require('../services/notifications');
const { PACKS, PLANS, getPlans, createCheckoutSession, createSubscriptionCheckoutSession, createBillingPortalSession, constructWebhookEvent, resolveSubscriptionTier, resolveSubscriptionState, addManagedAddonToSubscription, getOrCreateStripeCustomer, upgradeExistingSubscription } = require('../services/stripe_billing');
...
router.get(
  ...
  async (req, res) => {
    ...
    try {
      ...
      const balance = await getCreditBalance(clientId, brandId);
      if (!balance || !plan) {
      ...
      return res.json({
        ...
        tier: balance.tier,
        managed_addon: !!(plan.managed_addon || balance.tier === 'managed'),
...
router.get(
  ...
  async (req, res) => {
    ...
    try {
      const pool = getPool();
      const brandFilter = brandId ? 'AND brand_id = $4' : '';
      const brandArgs   = brandId ? [clientId, limit, offset, brandId] : [clientId, limit, offset];
      const countArgs   = brandId ? [clientId, brandId] : [clientId];

      const [{ rows }, { rows: countRows }] = await Promise.all([
        pool.query(
          `SELECT id, job_id, credits_used AS credits, type, pack_id, brand_id, created_at
           FROM credit_ledger
           WHERE client_id = $1 ${brandFilter}
           ORDER BY created_at DESC
           LIMIT $2 OFFSET $3`,
          brandArgs
        ),
        pool.query(
          `SELECT COUNT(*) AS total FROM credit_ledger WHERE client_id = $1 ${brandId ? 'AND brand_id = $2' : ''}`,
          countArgs
        ),
      ]);
  ...
  async (req, res) => {
    ...
    try {
      if (event.type === 'checkout.session.completed') {
        ...
        // CPD-401 / CPD-328: New subscription checkout (mode=subscription, no packId)
        if (session.mode === 'subscription' && clientId && !packId) {
          ...
          // Managed add-on checkout (does not replace base plan)
          if (session.metadata?.addon === 'managed' || planId === 'managed_addon') {
            try {
              await setManagedAddon(clientId, true, stripeSubscriptionId, session.customer || null);
              console.log(`[stripe-webhook] checkout.completed: client=${clientId} → managedAddon=true`);
              createNotification(clientId, {
                type:      'managed_addon_activated',
                title:     'Managed add-on activated',
                body:      'Done-for-you production is now attached to your plan.',
                actionUrl: '/billing',
              });
            } catch (addonErr) {
              logError('[credits] setManagedAddon failed', addonErr, { clientId });
            }
            return res.json({ ok: true });
          }

          const PLAN_LABELS = {
          growth: 'AuraFlux Creator',
          operate: 'AuraFlux Studio',
          guided: 'AuraFlux Studio',
          managed: 'AuraFlux Managed',
        };
          const planLabel = PLAN_LABELS[planId] || planId;

          if (brandId) {
            // Add-brand flow: activate the brand plan immediately — don't wait for subscription.updated
            try {
              await updateBrandPlanTier(brandId, planId, stripeSubscriptionId, null);
              console.log(`[stripe-webhook] checkout.completed: brand=${brandId} → tier=${planId}`);
            } catch (planErr) {
              logError('[credits] updateBrandPlanTier failed', planErr, { brandId, planId });
            }
            // Notify the account owner (not a welcome email — brand addition)
            createNotification(clientId, {
              type:      'brand_subscription_activated',
              title:     `Brand plan activated — ${planLabel}`,
              body:      `Your new brand subscription is confirmed.`,
              actionUrl: '/billing/add-brand/success',
            });
            return res.json({ ok: true });
          }

          // First subscription — send welcome email
          try {
        ...
        const { tier, managedAddon } = await resolveSubscriptionState(sub);
        if (tier) {
          if (brandId) {
            // Multi-brand path: update the specific brand's plan row
            await updateBrandPlanTier(brandId, tier, sub.id, stripeCustomerId);
            if (clientId) await setManagedAddon(clientId, managedAddon, sub.id, stripeCustomerId);
            console.log(`[stripe-webhook] subscription.updated: brand=${brandId} → tier=${tier} managedAddon=${managedAddon}`);
          } else if (clientId) {
            await updateClientPlanTier(clientId, tier, sub.id, stripeCustomerId, { managedAddon });
            console.log(`[stripe-webhook] subscription.updated: client=${clientId} → tier=${tier} managedAddon=${managedAddon}`);
          } else {
            console.warn(
              `[stripe-webhook] subscription.updated: could not resolve tier ` +
                `(clientId=${clientId}, brandId=${brandId}) — no plan update applied`
            );
          }
        } else if (clientId && managedAddon) {
          // Add-on-only update (base tier already set)
          await setManagedAddon(clientId, true, sub.id, stripeCustomerId);
          console.log(`[stripe-webhook] subscription.updated: client=${clientId} → managedAddon=true (base tier unchanged)`);
        } else {
          console.warn(
            `[stripe-webhook] subscription.updated: could not resolve tier ` +
              `(clientId=${clientId}) — no plan update applied`
          );
        }
        ...
        if (brandId) {
          ...
          await updateClientPlanTier(clientId, 'operate', null, null, { managedAddon: false });
...
router.get('/plans', apiLimit, async (req, res) => {
  try {
    ...
    // Merge Stripe rows over static fallback so Growth always appears with a dollar price
    const source = { ...PLANS, ...livePlans };
    res.json({
      ok: true,
      ...
      plans: Object.values(source).map((p) => {
        ...
        const { getPlanDefaults } = require('../services/plan_entitlements');
        const entitlements = getPlanDefaults(p.id);
        return {
          id:               p.id,
          label:            p.label,
          credits:          p.credits || entitlements.credits_included,
...
router.post(
  ...
  requireAuth,
  requireRole({ minLevel: ROLES.CUSTOMER }),
  body('planId').isIn(['growth', 'operate', 'guided', 'managed']).withMessage('planId must be growth, operate, guided, or managed'),
  body('successUrl').isURL().withMessage('successUrl must be a valid URL'),
  body('cancelUrl').isURL().withMessage('cancelUrl must be a valid URL'),
  async (req, res) => {
    const errors = validationResult(req);
...
// ── POST /plans/managed-addon — attach Managed to Creator or Studio ───────────
...
router.post(
  ...
  async (req, res) => {
    ...
    try {
      const plan = await getOrCreateClientPlan(clientId, req.user?.planTier || 'operate');
      const base = (plan?.tier || 'operate').toLowerCase();
      if (!['growth', 'operate', 'guided'].includes(base) && base !== 'managed') {
        return res.status(400).json({
          ok: false,
          error: 'Managed can be added to Creator or Studio plans only.',
        });
      }
      if (plan?.managed_addon || base === 'managed') {
        return res.json({ ok: true, alreadyAttached: true });
      }

      const email = await resolveEmailForUser(clientId, req.user?.email);
      if (!email) {
        return res.status(400).json({ ok: false, error: 'Account email required to add Managed.' });
      }

      const result = await addManagedAddonToSubscription(clientId, email, successUrl, cancelUrl);
      if (result.ok) {
        return res.json({
          ok: true,
          attached: true,
      ...
      if (result.reason === 'checkout_required' && result.checkoutUrl) {
        return res.json({ ok: true, url: result.checkoutUrl, sessionId: result.sessionId });
      }
      return res.status(400).json({
        ok: false,
        error: result.message || result.reason || 'Could not add Managed',
        reason: result.reason,
      });
    } catch (err) {
      logError('MANAGED_ADDON_FAIL', err, { clientId });
      return res.status(500).json({ ok: false, error: err.message });
    }
  }
);
...
// CPD-403: After a marketing-site pay-first customer creates their account,
// the app calls this endpoint with the Stripe session_id from the success URL.
// Finds the matching pending_subscriptions row, activates the plan, syncs profile.
router.post(
  ...
  async (req, res) => {
    ...
    try {
      ...
      const pending = await findUnclaimedPendingBySession(session_id);
      if (!pending) {
      ...
      const PLAN_LABELS = {
          growth: 'AuraFlux Growth / Creator',
          operate: 'AuraFlux Pro Operator / Agency',
          guided: 'AuraFlux Guided',
          managed: 'AuraFlux Managed',
        };
      const planLabel = PLAN_LABELS[plan] || plan;

      // Ensure client_plans row exists, then apply Stripe ids + tier
      await getOrCreateClientPlan(clientId, plan);
      ...
      // Keep JWT / feature gate source of truth in sync
      try {
        await updateProfileMeta(clientId, { planTier: plan });
      } catch (metaErr) {
        console.warn('[claim-checkout] updateProfileMeta failed', metaErr.message);
      }
...
// ── GET /credits/access ──────────────────────────────────────────────────────
// App paywall probe: does this account have an active Stripe subscription?
router.get(
  '/credits/access',
  apiLimit,
  requireAuth,
  async (req, res) => {
    try {
      const role = req.user?.role || req.user?.publicMetadata?.role;
      if (role === 'superadmin') {
        return res.json({ ok: true, paid: true, reason: 'superadmin' });
      }
      const paid = await hasPaidSubscription(req.user.id);
      let vodHours = null;
      try {
        const { getVodHoursBalance } = require('../services/vod_hours');
        vodHours = await getVodHoursBalance(req.user.id);
      } catch (_) {}
      return res.json({
        ok: true,
        paid,
        reason: paid ? 'subscription' : 'unpaid',
        vodHours,
      });
    } catch (err) {
      logError('ACCESS_CHECK_FAIL', err, { clientId: req.user?.id });
      return res.status(500).json({ ok: false, error: err.message });
    }
  }
);
