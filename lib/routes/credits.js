...
const { PACKS, PLANS, createCheckoutSession, createSubscriptionCheckoutSession, createBillingPortalSession, constructWebhookEvent, resolveSubscriptionTier } = require('../services/stripe_billing');
...
// ── GET /plans — list available subscription plans (CPD-100) ─────────────────

router.get('/plans', apiLimit, (req, res) => {
  res.json({
    ok: true,
    plans: Object.values(PLANS).map((p) => ({
      ...
      priceConfigured: !!(process.env[`STRIPE_PRICE_${p.id.toUpperCase()}`]),
    })),
  });
});
...
router.post(
  ...
  requireAuth,
  requireRole({ minLevel: ROLES.CUSTOMER }),
  body('planId').isIn(['diy', 'dwy', 'dfy']).withMessage('planId must be diy, dwy, or dfy'),
  body('successUrl').isURL().withMessage('successUrl must be a valid URL'),
  body('cancelUrl').isURL().withMessage('cancelUrl must be a valid URL'),
  async (req, res) => {
    const errors = validationResult(req);
