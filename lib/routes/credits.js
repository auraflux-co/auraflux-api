...
const { getCreditBalance, getClientPlan, getOrCreateClientPlan, getPool, insertCreditPack, hasStripeEvent, recordStripeEvent, updateClientPlanTier, updateBrandPlanTier } = require('../db');
      ...
      if (event.type === 'checkout.session.completed') {
        ...
        if (tier) {
          if (brandId) {
            ...
            await updateBrandPlanTier(brandId, tier, sub.id);
  ...
  async (req, res) => {
    ...
    try {
      ...
      const { checkoutUrl, sessionId } = await createSubscriptionCheckoutSession(
        clientId, planId, successUrl, cancelUrl, brandId || null
      );
