...
// Returns the last 100 inbound SMS across ALL brands for the authenticated customer.
// Used by the superadmin live feed.
router.get('/api/admin/sms-inbox', requireAuth, async (req, res) => {
  try {
    const customerId = req.auth?.userId || req.user?.id;
    if (!customerId) return res.status(401).json({ ok: false, error: 'Unauthorized' });
