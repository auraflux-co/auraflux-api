...
router.post('/api/public/chat', rateLimit(20), async (req, res) => {
  ...
  try {
    ...
    // Always log first message to console for superadmin visibility
    if (safeHistory.length === 0) {
      console.log(`[public/chat] new visitor: "${message.slice(0, 120)}"`);
    }

    return res.json({ reply, escalated: escalate });
  } catch (err) {
