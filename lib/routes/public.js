...
router.get('/api/public/plans', rateLimit(60), async (req, res) => {
  ...
  try {
    ...
    // Fallback to hardcoded values if Stripe is unreachable (must match live Stripe prices)
    return res.json({
      growth:  '$299/mo',
      operate: '$999/mo',
      guided:  '$999/mo',
...
//       → Stripe Checkout → success_url → app.auraflux.co/sign-in?checkout=success&plan=guided
router.get('/api/public/checkout', rateLimit(10), async (req, res) => {
  ...
  if (!['growth', 'operate', 'guided'].includes(plan)) {
    return res.status(400).json({ error: `Unknown plan: ${plan}. Valid: growth, operate, guided` });
  }

  const priceId = PLAN_PRICES[plan];
  // Growth $299/mo — use Price ID when configured; otherwise price_data so checkout ships without waiting on Doppler.
    ...
    : plan === 'growth'
      ? {
          price_data: {
            ...
            product_data: {
              name: 'AuraFlux Growth / Creator',
              description: '40h VOD/mo · 3 channels · 1080p60 · social dispatch',
            },
          },
          quantity: 1,
        }
  ...
  try {
    const session = await stripe.checkout.sessions.create({
      mode: 'subscription',
      line_items: [lineItem],
      // Post-payment → create account / sign in, then claim plan on /home
      success_url: `https://app.auraflux.co/sign-in?checkout=success&plan=${plan}&session_id={CHECKOUT_SESSION_ID}`,
...
// ── GET /api/public/pending-checkout ─────────────────────────────────────────
// Validate an unclaimed marketing checkout session before allowing sign-up.
router.get('/api/public/pending-checkout', rateLimit(60), async (req, res) => {
  setCors(req, res);
  ...
  try {
    const { findUnclaimedPendingBySession } = require('../auth/paywall');
    const pending = await findUnclaimedPendingBySession(sessionId);
    if (!pending) {
      return res.status(404).json({ ok: false, error: 'No unclaimed checkout found (expired or already claimed)' });
    }
    return res.json({
      ok: true,
      plan: pending.plan,
    ...
    console.error('[public/pending-checkout]', err.message);
...
AuraFlux is an automated content production platform that turns stream VODs and peaks into
published short-form video — 9:16 framing, captions, and direct publish to YouTube Shorts,
TikTok, and Instagram Reels.

Purchase is required before app access. Customers buy on auraflux.co/pricing, then create
their account on app.auraflux.co with the checkout session.

Plans (self-serve checkout):
- Operate (~$999/mo): Self-serve platform — peaks, Shorts pipeline, publish, monthly credits, team + invite-gated API.
- Guided: Operate features plus operator monitoring and guidance (higher credit pool).
- Managed: Done-for-you / enterprise — dedicated account management (sales; not self-serve checkout).

Exports today: vertical Shorts typically 720×1280 at 30fps. Metering is credit-based (not unlimited VOD hours).
...
Do not answer questions about competitors. Do not invent features (no free trial, no custom font uploads yet, no guaranteed <2 min SLA).
...
router.post('/api/public/contact', rateLimit(5), async (req, res) => {
  setCors(req, res);
  const { name, email, message, topic, source } = req.body || {};
  ...
  const isApiInvite = topicNorm === 'api_invite' || sourceNorm === 'api_invite'
    || /^\[api_invite\]/i.test(String(message));
  const subject = isApiInvite
    ? `[API INVITE] ${name}`
    : `Pre-sales inquiry from ${name}`;
  ...
  try {
    if (process.env.SMTP_HOST && process.env.SMTP_USER && process.env.SMTP_PASS) {
      ...
      await transporter.sendMail({
        ...
        replyTo: email,
        subject,
        text:    `Name: ${name}\nEmail: ${email}\nTopic: ${topicNorm || '(none)'}\nSource: ${sourceNorm || '(none)'}\n\n${message}`,
      });
    } else {
      console.log('[public/contact] SMTP not configured — lead:', {
        name, email, topic: topicNorm, source: sourceNorm, message: message.slice(0, 100),
      });
    }
