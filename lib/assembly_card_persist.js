...
function reconcileStuckAssemblingJob(job) {
  ...
  if (!job.gate3) {
    job.gate3 = {
      ...
      report: 'Assembly finished but Gate 3 Gemini QA did not run (server restart or poll timeout). MP4 is on disk — preview it. If it looks good, use APPROVE & PUBLISH; only RE-ASSEMBLE FROM FILES if you need a fresh Gate 3 pass.',
      checkedAt: new Date().toISOString(),
    };
    ...
    job.gate3.report = 'Assembly finished but Gate 3 Gemini QA did not run (server restart or poll timeout). MP4 is on disk — preview it. If it looks good, use APPROVE & PUBLISH; only RE-ASSEMBLE if you need a fresh Gate 3 pass.';
  } else if (job.gate3.outcome === 'fail' && !job.gate3.score && String(job.gate3.report || '').includes('did not run')) {
    job.gate3.outcome = 'manual_review';
  }
  if (!job.stage || job.stage === 'assembling') job.stage = 'awaiting_review';
  return job;
}
