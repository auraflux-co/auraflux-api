...
function enrichFromRunMetrics(job) {
  ...
  try {
    ...
    let matches = fs.readdirSync(OUTPUT_DIR).filter(
      (f) => f.includes(job.id) && f.endsWith('.mp4') && !/_cap\.mp4$/i.test(f)
    );
    if (!matches.length) {
      matches = fs.readdirSync(OUTPUT_DIR).filter((f) => f.includes(job.id) && f.endsWith('.mp4'));
    }
    if (matches.length) {
      matches.sort((a, b) => fs.statSync(path.join(OUTPUT_DIR, b)).size - fs.statSync(path.join(OUTPUT_DIR, a)).size);
    ...
    job.qaOutcome = g3.outcome === 'pass' ? 'pass' : 'fail';
...
function reconcileStuckAssemblingJob(job) {
    ...
    try {
      ...
      if (!matches.length) {
        matches = fs.readdirSync(OUTPUT_DIR).filter(
          (f) => f.includes(job.id) && f.endsWith('.mp4')
        );
      }
      if (matches.length) {
        matches.sort((a, b) => {
          const sa = fs.statSync(path.join(OUTPUT_DIR, a)).size;
          const sb = fs.statSync(path.join(OUTPUT_DIR, b)).size;
          return sb - sa;
        });
  ...
  if (!job.gate3) {
    job.gate3 = {
      score: null,
      outcome: 'fail',
      report: 'Gate 3 failed — job was stuck in assembling. Use RE-ASSEMBLE FROM FILES after QA fix.',
      checkedAt: new Date().toISOString(),
    };
  }
  return job;
