...
function enrichFromRunMetrics(job) {
  ...
  try {
    ...
    const score = g3.score ?? g3.gate3aScore ?? null;
    const staleManual = String(job.gate3?.report || '').includes('Gate 3 Gemini QA did not run');
    // CPD-1293: never keep the "did not run" banner once real Gate 3 scores exist.
    // Keep prior Gate 3a concerns/deductions — metrics JSON only stores score/outcome.
    const priorConcerns = Array.isArray(job.gate3?.concerns) ? job.gate3.concerns : [];
    const priorDeductions = Array.isArray(job.gate3?.deductions) ? job.gate3.deductions : [];
    const detailLines = [
      ...priorConcerns,
      ...priorDeductions.map((d) => (typeof d === 'string' ? d : d?.reason)).filter(Boolean),
    ].filter((line, i, arr) => line && arr.indexOf(line) === i);
    let report;
    if (g3.outcome === 'pass') {
      report = [`Gate 3 passed (${score}/100)`, ...detailLines.slice(0, 6)].join('\n');
    } else if (score != null && (staleManual || !job.gate3?.report)) {
      report = [`Gate 3 ${g3.outcome || 'fail'} (${score}/100)`
        + (g3.gate3aOutcome ? ` · Gate 3a: ${g3.gate3aOutcome}` : ''), ...detailLines.slice(0, 6)].join('\n');
    } else {
      report = job.gate3?.report || `Gate 3 ${g3.outcome || 'fail'} (${score ?? 'n/a'}/100)`;
    }
    job.gate3 = {
      score,
      ...
      report,
      concerns: priorConcerns,
      deductions: priorDeductions,
      sampleFindings: job.gate3?.sampleFindings || null,
...
function wireAssemblyOutputToJob(job, outputPath) {
  ...
  if (!job.stage || job.stage === 'assembling' || job.stage === 'heygen_done' || job.stage === 'all_sent'
    || (job.manualFinalIngest && job.stage === 'assembled')) {
    job.stage = 'awaiting_review';
  }
  return job;
}
