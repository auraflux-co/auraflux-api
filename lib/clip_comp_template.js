...
function shouldApplyClipCompTransform(clipsOnlyComp, designSpec = null, compCreative = null) {
  ...
  const creative = compCreative || designSpec?.compCreative || null;
  const look = creative?.look?.preset || designSpec?.lookPreset;
  if (look && look !== 'auto') return true;
  if (creative?.effects?.transform === true) return true;
  return false;
}
