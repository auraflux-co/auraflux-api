...
function normalizeOverlayTexts(cfg) {
  if (!cfg) return [];
  ...
  return raw.map((item, i) => {
    ...
    if (!text) return null;
    const startSec = Number(item.startSec ?? item.atSec ?? 0);
...
function buildAnimatedTextFilter(cfg) {
  const items = normalizeOverlayTexts(cfg);
