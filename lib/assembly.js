...
function soupAvatarDialogueJoin(fromLabel = '', toLabel = '') {
  ...
  if (soupIsStreamerHandoff(from, to)) {
    return soupSceneResetXfade({ fadeReactionTail: true, streamerHandoff: true, incomingLabel: to, sceneLabel: from });
  }
  return {
    useXfade: true,
    videoDur: 0.22,
...
function soupJoinTransition(fromType, toType, fromLabel = '', toLabel = '') {
  if (fromType === 'source_clip') {
    return { useXfade: false, videoDur: 0, audioDur: 0, fadeReactionTail: false, transition: 'cut', audioFadeSec: 0.05 };
  }
...
async function mergeTwoWithCut(leftPath, rightPath, outputPath, {
  ...
  logLine(`  ✂️  hard-cut pair (video cut, audio ${audioFadeSec.toFixed(2)}s${tailNote}${prepStableTail ? ', stable-tail prep' : ''})`);
}
...
async function concatMediaWithTransition(inputFiles, outputPath, {
  ...
  const joinSpecs = [];
  for (let i = 0; i < n - 1; i++) {
    joinSpecs.push(soupJoinTransition(types[i], types[i + 1], labels[i], labels[i + 1]));
  }
  ...
  if (!useXfade) {
    if (allPlainCut && n >= 2 && encodeVideo) {
      ...
      for (let i = 1; i < n; i++) {
        ...
        await mergeTwoWithCut(current, inputFiles[i], stepOut, { encodeVideo, logFn });
        if (current !== inputFiles[0] && temps.includes(current)) {
          try { fs.unlinkSync(current); } catch (_) { /* ignore */ }
        }
        if (stepOut !== outputPath) temps.push(stepOut);
      ...
      for (let i = 0; i < segsToProcess.length; i++) {
        ...
        try {
          ...
          if (fileSize < minSegmentBytes) {
            log(asmId, `❌ Segment ${i+1} too small (${fileSize} bytes, minimum ${minSegmentBytes}${isSynthPrebuild ? ', synth prebuild' : ', remote'}) — likely error page or corrupt`);
