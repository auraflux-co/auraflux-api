...
const { geminiQACheck, geminiSegmentQA } = require('./qa');
...
async function handleAssemble(req, res, saveJobCard) {
  ...
  const run = async () => {
    try {
      ...
      // ── Gate Worker System: Gate 2 (render quality) on downloaded segments ──
      // Runs after all segments are downloaded. Additive — does NOT replace existing Gate 2.
      // Saves result to job_spec DB for observability + monitoring.
      try {
        const gate2 = require('./gates/gate2');
        const { saveGateResult: gwSaveG2, getJobSpec: gwGetJobSpec } = require('./job_spec');
        const gwAsmJobSpec = await (async () => {
          if (assemblyJobId) {
            try { return await gwGetJobSpec(assemblyJobId); } catch(e) { return null; }
          }
          return null;
        })();
        const gwG2JobSpec = gwAsmJobSpec || {
          jobId: asmId,
          customerId: 'c0',
        ...
        // Only run on avatar segments (skip source_clips for render quality check)
        const avatarFilePaths = localFiles.filter((_, i) => {
          const seg = segsToProcess[i];
          return !seg || seg.type !== 'source_clip';
        });
        if (avatarFilePaths.length > 0 && gate2.canProduce(gwG2JobSpec).ready) {
          const gwG0Report = gwAsmJobSpec?.state?.gateResults?.gate0 || null;
          const gwG1Report = gwAsmJobSpec?.state?.gateResults?.gate1 || null;
          const gwG2Result = await gate2.run(gwG2JobSpec, avatarFilePaths, gwG0Report, gwG1Report);
          try { await gwSaveG2(gwG2JobSpec.jobId, 'gate2', gwG2Result); } catch(e) { /* ignore */ }
          log(asmId, `[gate-worker] Gate 2: ${gwG2Result.outcome} (score ${gwG2Result.score}/100)`);
        } else {
          log(asmId, `[gate-worker] Gate 2 skipped (canProduce not ready or no avatar segments)`);
        }
      } catch (gwG2Err) {
        log(asmId, `[gate-worker] Gate 2 error (non-blocking): ${gwG2Err.message}`);
      }
      ...
      if (isShortForm) {
        ...
        if (captionText && captionStyle) {
          ...
          // Canvas is 1080×1920. Clip half = bottom 960px, Avatar half = top 960px.
          let dtX, dtY;
          if (captionStyle.position === 'top-center') {
            dtX = '(W-text_w)/2';
            dtY = '60';
...
// Gap #45: OUTRO freeze-hold -- clone last frame for 0.75s (long-form only)
      ...
      while (qaAttempt < MAX_QA_RETRIES) {
        ...
        try {
          qaResult = await geminiQACheck(outPath, {
            ...
            expectedClips: contractExpectedClips, // Pipeline contract from Gate 1
          ...
          if (qaResult.outcome === 'pass' || qaResult.outcome === 'manual_review') {
            ...
            // Wire Gate 3 stuck alarm — call POST /job/:id/stuck
            try {
              const stuckReason = `Gate 3: Video failed QA after ${MAX_QA_RETRIES} surgical retries. Final score: ${qaResult.score}/100. Issues: ${qaResult.report.split('\n').slice(0, 3).join('; ')}`;
              await axios.post(`http://localhost:${process.env.PORT || 3000}/job/${assemblyJobId}/stuck`, {
                gate: 'gate3',
                reason: stuckReason,
                detail: { score: qaResult.score, retries: MAX_QA_RETRIES, outcome: qaResult.outcome }
              });
              log(asmId, `🚨 Job marked STUCK — Gate 3 hard fail after ${MAX_QA_RETRIES} retries`);
            } catch(stuckErr) {
              logError('STUCK_ALARM_FAIL', `Failed to mark job stuck: ${stuckErr.message}`, { asmId, jobId: assemblyJobId });
            }
      ...
      // ── Gate Worker System: Gate 3a + Gate 3b ──────────────────────────────
      // Runs after existing Gate 3. Additive — does NOT replace existing Gate 3.
      // gate3a: Gemini qualitative review of assembled video
      // gate3b: Claude commitment verification (analytical)
      let gwG3aResult = null;
      let gwG3bResult = null;
      try {
        const gate3a = require('./gates/gate3a');
        const gate3b = require('./gates/gate3b');
        const { saveGateResult: gwSaveG3, getJobSpec: gwGetJobSpec3 } = require('./job_spec');
        const gwAsmJobSpec3 = await (async () => {
          if (assemblyJobId) {
            try { return await gwGetJobSpec3(assemblyJobId); } catch(e) { return null; }
          }
          return null;
        })();
        const gwG3JobSpec = gwAsmJobSpec3 || {
          jobId: asmId,
          customerId: 'c0',
          assembledPath: outPath,
          outputPath: outPath,
          order: { output: { format: format === 'portrait' ? '9:16' : '16:9' }, formType: format === 'portrait' ? 'short' : 'long' },
          state: { gateResults: {} },
          commitments: {}
        };
        // Attach assembled path for gate3a.canProduce()
        const gwG3JobSpecWithPath = { ...gwG3JobSpec, assembledPath: outPath, outputPath: outPath };

        if (outPath && fs.existsSync(outPath)) {
          const priorReports3a = [
            gwAsmJobSpec3?.state?.gateResults?.gate0 || null,
            gwAsmJobSpec3?.state?.gateResults?.gate1 || null,
            gwAsmJobSpec3?.state?.gateResults?.gate2 || null
          ];

          // Gate 3a — Gemini qualitative review
          if (gate3a.canProduce(gwG3JobSpecWithPath).ready) {
            gwG3aResult = await gate3a.run(gwG3JobSpecWithPath, outPath, priorReports3a);
            try { await gwSaveG3(gwG3JobSpec.jobId, 'gate3a', gwG3aResult); } catch(e) { /* ignore */ }
            log(asmId, `[gate-worker] Gate 3a: ${gwG3aResult.outcome} (score ${gwG3aResult.score}/100)`);
          } else {
            log(asmId, `[gate-worker] Gate 3a canProduce not ready — skipping`);
          }

          // Gate 3b — Claude commitment verification (only if 3a passed or has findings)
          if (gwG3aResult && (gwG3aResult.passed || gwG3aResult.sampleFindings)) {
            const priorReports3b = [
              gwAsmJobSpec3?.state?.gateResults?.gate0 || null,
              gwAsmJobSpec3?.state?.gateResults?.gate1 || null,
              gwAsmJobSpec3?.state?.gateResults?.gate2 || null
            ];
            if (gate3b.canProduce(gwG3JobSpec).ready) {
              gwG3bResult = await gate3b.run(gwG3JobSpec, gwG3aResult, priorReports3b);
              try { await gwSaveG3(gwG3JobSpec.jobId, 'gate3b', gwG3bResult); } catch(e) { /* ignore */ }
              log(asmId, `[gate-worker] Gate 3b: ${gwG3bResult.outcome} — ${gwG3bResult.mismatches?.length || 0} mismatch(es)`);
            } else {
              log(asmId, `[gate-worker] Gate 3b canProduce not ready (commitments empty) — skipping`);
            }
          }
        } else {
          log(asmId, `[gate-worker] Gate 3a/3b skipped — assembled path not available`);
        }
      } catch (gwG3Err) {
        log(asmId, `[gate-worker] Gate 3a/3b error (non-blocking): ${gwG3Err.message}`);
      }
      ...
      try {
        ...
        if (driveUrl) {
          ...
          // ── Gate Worker System: Gate 4 + Gate 5 ──────────────────────────────
          // Gate 4: Gemini full-video broadcast ready check
          // Gate 5: Upload confirmation (only fires if gate4.uploadSignal === true)
          // Both are additive — do NOT block existing auto-publish (Gate 6) flow.
          try {
            const gate4 = require('./gates/gate4');
            const gate5 = require('./gates/gate5');
            const { saveGateResult: gwSaveG45, saveOutput: gwSaveOutput45, getJobSpec: gwGetJobSpec45 } = require('./job_spec');
            const gwAsmJobSpec45 = await (async () => {
              if (assemblyJobId) {
                try { return await gwGetJobSpec45(assemblyJobId); } catch(e) { return null; }
              }
              return null;
            })();
            const gwG45JobSpec = gwAsmJobSpec45 || {
              jobId: asmId,
              customerId: 'c0',
              assembledPath: outPath,
              outputPath: outPath,
              ...
              order: {
                ...
                publish: { platforms: [], driveUrl }
              },
              state: { gateResults: {} },
              commitments: {}
            };
            const gwG45JobSpecWithPath = { ...gwG45JobSpec, assembledPath: outPath, outputPath: outPath, driveUrl };
            // Save driveUrl to job spec
            try { await gwSaveOutput45(gwG45JobSpec.jobId, 'driveUrl', driveUrl); } catch(e) { /* ignore */ }

            const priorReports45 = [
              gwAsmJobSpec45?.state?.gateResults?.gate0 || null,
              gwAsmJobSpec45?.state?.gateResults?.gate1 || null,
              gwAsmJobSpec45?.state?.gateResults?.gate2 || null,
              gwG3aResult || gwAsmJobSpec45?.state?.gateResults?.gate3a || null,
              gwG3bResult || gwAsmJobSpec45?.state?.gateResults?.gate3b || null
            ];

            // Gate 4 — full video broadcast ready
            if (gate4.canProduce(gwG45JobSpecWithPath).ready) {
              const gwG4Result = await gate4.run(gwG45JobSpecWithPath, outPath, priorReports45);
              try { await gwSaveG45(gwG45JobSpec.jobId, 'gate4', gwG4Result); } catch(e) { /* ignore */ }
              log(asmId, `[gate-worker] Gate 4: ${gwG4Result.passed ? 'PASS' : 'FAIL'} uploadSignal=${gwG4Result.uploadSignal}`);

              // Gate 5 — upload confirmation (only if gate4.uploadSignal is true)
              if (gwG4Result.uploadSignal && gate5.canProduce(gwG45JobSpecWithPath).ready) {
                const gwG5Result = await gate5.run(gwG45JobSpecWithPath, gwG4Result);
                try { await gwSaveG45(gwG45JobSpec.jobId, 'gate5', gwG5Result); } catch(e) { /* ignore */ }
                log(asmId, `[gate-worker] Gate 5: ${gwG5Result.passed ? 'PASS' : 'FAIL'}`);
              } else {
                log(asmId, `[gate-worker] Gate 5 skipped (gate4 uploadSignal=${gwG4Result.uploadSignal} or canProduce not ready)`);
              }
            } else {
              log(asmId, `[gate-worker] Gate 4 canProduce not ready — skipping Gate 4+5`);
            }
          } catch (gwG45Err) {
            log(asmId, `[gate-worker] Gate 4/5 error (non-blocking): ${gwG45Err.message}`);
          }
