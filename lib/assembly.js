...
function loadAssemblyJobs() {
  try {
    if (fs.existsSync(ASSEMBLY_JOBS_FILE)) {
      const data = JSON.parse(fs.readFileSync(ASSEMBLY_JOBS_FILE, 'utf8'));
      const now = Date.now();
      const DAY_MS = 24 * 60 * 60 * 1000;
      
      Object.entries(data).forEach(([asmId, job]) => {
        // Mark interrupted jobs
        if (job.status === 'assembling' || job.status === 'running') {
          job.status = 'interrupted';
          job.interruptedAt = new Date().toISOString();
          logger.warn({ asmId, status: 'interrupted' }, 'Assembly job interrupted by restart');
        }
        
        // Prune jobs older than 24h
        const jobAge = now - (job.startedAt ? new Date(job.startedAt).getTime() : now);
        if (jobAge < DAY_MS) {
          assemblyJobs[asmId] = job;
        } else {
          logger.debug({ asmId, age: Math.round(jobAge / 1000 / 60 / 60) }, 'Pruned old assembly job');
        }
      });
      
      logger.info({ count: Object.keys(assemblyJobs).length }, 'Assembly jobs loaded from disk');
    }
  } catch (err) {
    logger.error({ err }, 'Failed to load assembly jobs from disk');
  }
}

// Save single assembly job to disk (called on every mutation)
function saveAssemblyJob(asmId) {
  try {
    ...
    if (fs.existsSync(ASSEMBLY_JOBS_FILE)) {
      try {
        allJobs = JSON.parse(fs.readFileSync(ASSEMBLY_JOBS_FILE, 'utf8'));
      } catch (e) {
        logger.warn({ err: e }, 'Failed to read existing assembly jobs, creating new file');
      }
    }
    
    // Update this job (exclude large log field unless job is complete/failed)
    const jobToSave = {
      asmId: job.asmId || asmId,
      status: job.status,
      pct: job.pct,
      outputPath: job.outputPath,
      filename: job.filename,
      gate2Score: job.gate2Score,
      gate2Outcome: job.gate2Outcome,
      gate3Score: job.qaScore,
      gate3Outcome: job.qaOutcome,
      startedAt: job.startedAt || new Date().toISOString(),
      completedAt: job.status === 'done' || job.status === 'failed' ? new Date().toISOString() : job.completedAt,
    ...
    // Include log only on completion/failure
    if (job.status === 'done' || job.status === 'failed' || job.status === 'interrupted') {
      jobToSave.log = job.log;
      jobToSave.error = job.error;
    }
    
    allJobs[asmId] = jobToSave;
    
    // Write atomically
    const tmpFile = ASSEMBLY_JOBS_FILE + '.tmp';
    fs.writeFileSync(tmpFile, JSON.stringify(allJobs, null, 2));
    fs.renameSync(tmpFile, ASSEMBLY_JOBS_FILE);
  } catch (err) {
    logger.error({ err, asmId }, 'Failed to save assembly job to disk');
  }
}
