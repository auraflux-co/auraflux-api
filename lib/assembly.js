...
function soupAvatarDialogueJoin(fromLabel = '', toLabel = '') {
  ...
  if (/_CLIP1_REACTION$/i.test(from) && /_CLIP2_SETUP$/i.test(to)) {
    return {
      useXfade: true,
      videoDur: 0.22,
      audioDur: 0.22,
...
async function mergeTwoWithXfade(leftPath, rightPath, outputPath, {
  ...
  const leftDur = await probeDuration(leftPath);
  ...
  logLine(`  🎞  xfade pair ${transitionName} ${transitionDur.toFixed(2)}s / audio ${audioXfadeDur.toFixed(2)}s${tailNote}`);
}
...
async function mergeTwoWithCut(leftPath, rightPath, outputPath, {
  ...
  logLine(`  ✂️  hard-cut pair (video cut, audio ${audioFadeSec.toFixed(2)}s${tailNote})`);
}

/** Intro→setup: freeze last frame briefly, hard video cut, audio crossfade only. */
async function mergeTwoWithHoldCut(leftPath, rightPath, outputPath, {
  ...
  const fadeOutSt = Math.max(0, leftDur - axf);
  const args = [
    '-fflags', '+genpts', '-i', leftPath,
    '-fflags', '+genpts', '-i', rightPath,
    '-filter_complex',
    `[0:v]${norm},tpad=stop_mode=clone:stop_duration=${hold.toFixed(3)}[v0];`
    + `[1:v]${norm}[v1];`
    + `[v0][v1]concat=n=2:v=1:a=0[v];`
    + `[0:a]afade=t=out:st=${fadeOutSt.toFixed(3)}:d=${axf.toFixed(3)}[a0];`
    + `[1:a]afade=t=in:st=0:d=${axf.toFixed(3)}[a1];`
  ...
  logLine(`  🖼  hold-cut pair (hold ${hold.toFixed(2)}s video, audio xfade ${axf.toFixed(2)}s)`);
}
...
async function concatMediaWithTransition(inputFiles, outputPath, {
  ...
  const allCut = joinSpecs.every((s) => !s.useXfade);
