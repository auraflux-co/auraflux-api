...
const { getAssemblyConfig } = require('./configLoader');
  ...
  const run = async () => {
    try {
      ...
      for (let i = 0; i < localFiles.length; i++) {
        ...
        // ── Config-driven chrome burn dispatch (Phase 4 universal arch) ──────────────────
        // Replaces the hardcoded if/else chain (twitch/news/nba) that existed here.
        // asmCfg.chromeModule = relative path to the chrome handler module, or null for short-form.
        // Each module exports burnChrome(ctx) → Promise<string> (returns updated inputForTS).
        // PHASE4_INLINE: The chrome modules receive all loop-local state via ctx because
        // the burn logic is deeply entangled with the segment loop (inputForTS, label, segsToProcess,
        // localFiles, i, streamerRoster, segTypes). Passing ctx is the lowest-risk extraction.
        if ((segTypes[i] || 'avatar') === 'avatar') {
          const _asmCfg = getAssemblyConfig(contentType);
          const _chromeModulePath = _asmCfg.chromeModule;
          if (_chromeModulePath) {
            let _chromeMod = null;
            try {
              _chromeMod = require(path.join(__dirname, _chromeModulePath));
            } catch(e) {
              log(asmId, `  ⚠️  Chrome module load failed (${_chromeModulePath}): ${e.message} — skipping chrome burn`);
            }
            if (_chromeMod && typeof _chromeMod.burnChrome === 'function') {
              const _ctx = {
                inputForTS,
                label,
                streamerRoster,
              ...
              inputForTS = await _chromeMod.burnChrome(_ctx);
            }
          }
        }
...
// Gap #45: OUTRO freeze-hold -- clone last frame for 0.75s (long-form only)
        ...
        try {
          await new Promise((res, rej) => {
            ...
            // Phase 4: sourceCrop now read from config/contentTypes.json via getAssemblyConfig().
            // Falls back to the previous twitch/non-twitch default if config lookup returns nothing.
            const _asmCfgCrop = getAssemblyConfig(contentType);
            const sourceCropSize = _asmCfgCrop.sourceCrop || ((contentType === 'twitch') ? 'crop=1880:1040,' : 'crop=1920:1080,');
      ...
      // Phase 4: voiceover flag now driven by config/contentTypes.json assembly.voiceover.
      // Falls back to contentType === 'nba' check if config is unavailable.
      const _asmCfgVo = getAssemblyConfig(contentType);
      const _voiceoverEnabled = typeof _asmCfgVo.voiceover === 'boolean'
        ? _asmCfgVo.voiceover
        : (contentType === 'nba');
      if (_voiceoverEnabled && tsFiles.length > 0) {
