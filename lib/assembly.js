...
/** Gold-standard avatar→avatar crossfade — matches CLIP2_REACTION→next_INTRO (operator-validated). */
function soupAvatarDialogueJoin(fromLabel = '', toLabel = '') {
  const from = String(fromLabel || '');
  const afterReactionSetup = /_CLIP1_REACTION$/i.test(from) && /_CLIP2_SETUP$/i.test(toLabel || '');
  return {
    useXfade: true,
    videoDur: 0.22,
    audioDur: 0.22,
    fadeReactionTail: afterReactionSetup,
    reactionTailFadeSec: afterReactionSetup ? 0.35 : 0,
    transition: 'crossfade',
    sceneReset: soupIsSceneResetJoin(from, toLabel),
  };
}
...
async function concatMediaWithTransition(inputFiles, outputPath, {
  ...
  if (!allXfade && !allCut) {
    ...
    logLine(`  🎞  mixed stitch: ${n} segments (${xfadeN} xfade, ${joinSpecs.length - xfadeN} cut${sceneResetN ? `, ${sceneResetN} scene-reset` : ''})`);
    return {
      mode: 'mixed',
      segments: n,
      xfadeJoins: xfadeN,
