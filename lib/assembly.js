...
const { burnSceneChromeFromDirective, generateChromeOverlay } = require('./chrome_overlay');
const {
  resolveChromeCfg,
  buildAndBurnChrome,
...
// Legacy HTML/Puppeteer chrome overlay flag. C1+ (Render) defaults OFF — FFmpeg drawtext path.
// C0 (localhost, no DATABASE_URL) defaults ON for backward compat with local dashboard.
// Override: set LEGACY_OVERLAY_ONLY=true on C1+ to force legacy path; =false on C0 to use FFmpeg.
const _IS_C1 = !!process.env.DATABASE_URL;
const C0_LEGACY_OVERLAY_ONLY = _IS_C1
  ? process.env.LEGACY_OVERLAY_ONLY === 'true'
...
async function burnLegacyHtmlOverlay({
  ...
  await generateChromeOverlay(storyData, overlayPng, activeIdx, {
    showLowerThird: true,
    hideSidebar: false,
    episodeNumber,
    activeCategory,
...
// Synth prebuild (Portal 3a): lavfi patterns can be <100KB while still valid MP4; ftyp/mdat probe below is authoritative.
...
// - publishCopy (1 location): Portal 6 metadata
//
...
function groupSegmentsByLabel(segsToProcess) {
  ...
  for (let i = 0; i < labels.length; i++) {
    ...
    // itemIdx 0 = first item (INTRO/COLD_OPEN shows first card); last existing group index for OUTRO.
    if (STANDALONE.test(label)) {
      ...
      groups.push({
        ...
        indices: [i],
        itemIdx: isOutro ? Math.max(0, itemCounter - 1) : 0,
      });
      continue;
    }
...
async function handleAssemble(req, res, saveJobCard) {
  ...
  /** Lab/synthetic assemblies: skip broadcast-style Gemini QA in Portal 3a/3b/4; withhold auto-publish (Portal 4 uploadSignal). */
  ...
  assemblyJobs[asmId] = {
    ...
    // Store script metadata for Portal 2 HeyGen re-rendering
  ...
  const run = async () => {
    try {
      ...
      // Carry in-process Portal 2 result forward so Portal 3 preflight does not race
      ...
      // ── Portal 2: Render quality check (gate worker system) ────────────────────────
      ...
      // and blocks Portal 3a when avatarSegsForQA is empty (local file paths, not URLs).
      // Future customers (Upload My Content, Link Content) will use Portal 2 fully —
      // their renders arrive without prior human review.
      {
        const gate2Timer = new StageTimer(asmId, 'Portal 2 QA');
        try {
          ...
          if (_isC0Manual) {
            log(
              asmId,
              `⏭  Portal 2 skipped — c0 manual workflow (operator reviewed segments before assembly)`
            );
            gate2To3aReady = true;
            ...
            try {
              ...
              if (avatarSegsForQA.length > 0) {
                // Download ALL avatar segments for Portal 2 — silent detection requires full check
                ...
                for (let idx = 0; idx < avatarSegsForQA.length; idx++) {
                  ...
                  try {
                    ...
                    if (sz > 5000) {
                      ...
                      log(asmId, `  ✓ Portal 2 segment: ${seg.label} (${(sz / 1024).toFixed(0)}KB)`);
                    } else {
                      try {
                    ...
                    log(
                      asmId,
                      `  ⚠️  Portal 2 segment download failed for ${seg.label}: ${e.message}`
                    );
                  }
                ...
                if (g2TmpPaths.length > 0) {
                  ...
                  const runGate2Attempt = async () => {
                    ...
                    if (g2Preflight.softHeals.length > 0) {
                      log(
                        asmId,
                        `🩹 Portal 2 preflight soft-heal: ${g2Preflight.softHeals.join('; ')}`
                      );
                    }
                    if (!g2Preflight.ready) {
                      log(
                        asmId,
                        `⚠️  Portal 2 prerequisites warning: ${g2Preflight.reasons.join('; ')}`
                      );
                    }
                  ...
                  const runGate2Intervention = async ({ interventionAttempt, lastResult }) => {
                    if (
                      ...
                      log(
                        asmId,
                        `🛠 Portal 2 intervention ${interventionAttempt}: re-render ${badSegments.length} segment(s)`
                      );
                  ...
                  try {
                    const { saveAssemblyJob: saveAsmJobDb } = require('./db');
                    saveAsmJobDb(asmId, assemblyJobId || asmId, { gate2Score: g2Result.score }).catch(() => {});
                  } catch (e) {}

                  log(
                    asmId,
                    `📋 Portal 2: ${g2Result.outcome} (${g2Result.score}/100) — ${g2Result.segmentResults?.length || 0} segments checked`
                  );

                  if (!g2Result.passed && g2Result.outcome === 'hard_fail') {
                    log(
                      asmId,
                      `❌ Portal 2 hard fail — ${g2Result.batchStopped ? `batch stopped at ${path.basename(g2Result.batchStoppedAt || '')}` : 'segments failed QA'} — continuing assembly`
                    );
                  ...
                  if (
                    ...
                    log(
                      asmId,
                      `🔄 Portal 2: ${badSegments.length} segment(s) need re-render — ${badSegments.map((s) => s.label).join(', ')}`
                    );
                  ...
                  // Clean up tmp files after Portal 2 completes (and after re-render logic)
                  ...
                  log(asmId, `⚠️  Portal 2: No segments downloaded successfully — skipping`);
                  gate2To3aReady = false;
                }
              } else {
                log(asmId, `⚠️  Portal 2: No avatar segments to check — skipping`);
                gate2To3aReady = false;
              }
            } catch (g2Err) {
              log(asmId, `⚠️  Portal 2 error: ${g2Err.message} — continuing assembly`);
              gate2To3aReady = false;
            }
          } // end non-c0-manual else
        } catch (_outerErr) {
          log(asmId, `⚠️  Portal 2 outer error: ${_outerErr.message}`);
        }
      ...
      // CPD-419: Store per-segment labeled durations for YouTube chapter markers.
      // Must happen here while durations[] and localFileMeta[] are both populated
      // and before localFiles are deleted after assembly.
      assemblyJobs[asmId].segmentLabelsAndDurations = durations.map((dur, idx) => ({
        label: localFileMeta[idx]?.label || `Segment ${idx + 1}`,
      ...
      // Declare outPath/outFile/totalDur/tickerType in outer scope so Portal 3 QA + Drive upload can access them
      // regardless of whether short-form or long-form branch ran.
      let outPath = '';
      let outFile = '';
      let totalDur = '0';
      let downloadedClipCount = 0; // set inside download block, read by Portal 3 — must be outer scope
      ...
      if (isShortForm) {
        ...
        // Assign outer-scope counter so Portal 3 QA has the real downloaded clip count
        ...
        // Skip to end of assembly endpoint (Portal 3 QA, Drive upload, etc.)
        ...
        // ── Portal 3 Pre-Flight: deterministic structural check, no Gemini tokens ─────────
        ...
        if (preFlightCriticals.length > 0) {
          ...
          log(
            asmId,
            `❌ Portal 3 pre-flight failed — ${preFlightCriticals.length} critical issue(s). Aborting before Gemini upload.`
          );
        ...
        // Assign to outer-scope let so Portal 3 QA can read it (was const — out of scope bug)
        ...
        // g3JobSpec is loaded later (Portal 3 QA), so we pre-load here for chrome burns.
        // Uses the same lookup pattern as Portal 2 and Portal 3.
        ...
        const processSegmentToTs = async (inputFilePath, segIdx, localSegIdx) => {
          ...
          const _customerIdForChrome = chromeBurnJobSpec?.customerId || reqCustomerId;
          ...
          if (!_directiveHandled) {
            try {
              ...
              try {
                ...
                const _ccDef = _lcDef(chromeBurnJobSpec?.customerId || reqCustomerId);
              ...
              if (_frozenChromeCfg && typeof _frozenChromeCfg === 'object') {
                ...
                const _customerId = chromeBurnJobSpec?.customerId ;
                const _custConfig = _loadCC(_customerId);
                // Detect source platform: explicit field first, then infer from chromeSkin,
                // then fall back to sniffing the source URLs.
                const _srcPlatform = chromeBurnJobSpec?.sourcePlatform
                  || (chromeSkin === 'twitch' ? 'twitch' : null)
                  || detectSourcePlatform([
                      ...(chromeBurnJobSpec?.sourceConfig?.urls || []),
                      ...(chromeBurnJobSpec?.order?.inputs?.sources || []),
                    ]);
                _chromeCfg = resolveChromeCfg(_custConfig, _ctForChrome, _srcPlatform);
              }
              try {
                const _cfgHash =
        ...
        for (let gIdx = 0; gIdx < segGroups.length; gIdx++) {
          ...
          for (let k = 0; k < groupLocalFiles.length; k++) {
            ...
            // populate flat tsFiles for Portal 3a and legacy code paths
        ...
        // Keep tsSegTypes in sync with tsFiles for Portal 3a / downstream code
        ...
        if (groupMp4Paths.length === 1) {
          ...
          // CPD-418: Scene transitions — use xfade when feature is enabled, ≤30 groups,
          // and a non-cut transition is requested. Falls back to concat copy otherwise.
          const _reqJobSpec = req.body?.jobSpec || null;
          // CPD-427: dynamicOverlays addOn enables scene transitions; also honoured via
          // featureFlags, features array, or explicit transition type on the request.
          const _hasSceneTrans = _reqJobSpec?.addOns?.dynamicOverlays?.active === true ||
            _reqJobSpec?.featureFlags?.scene_transitions ||
            (_reqJobSpec?.features && _reqJobSpec.features.includes('scene_transitions')) ||
            transition === 'crossfade' || transition === 'dissolve' || transition === 'fade';
          const _transType = _reqJobSpec?.designSpec?.transitions?.type || transition || 'crossfade';
          const _useXfade = _hasSceneTrans && groupMp4Paths.length <= 30 && _transType !== 'cut';

          if (_useXfade) {
            // Probe group durations for accurate xfade offsets
            const _groupDurations = [];
            for (const gmp4 of groupMp4Paths) {
              try {
                const d = await probeDuration(gmp4);
                _groupDurations.push(Number.isFinite(d) && d > 0 ? d : 60);
              } catch (_) {
                _groupDurations.push(60);
              }
            }
            const _tName = _transType === 'dissolve' ? 'dissolve' : 'fade';
            const _tDur = _transType === 'dissolve' ? 0.7 : 0.3;
            // Build xfade filter chain with real offsets
            const _inputArgs = [];
            groupMp4Paths.forEach((f) => _inputArgs.push('-i', f));
            let _elapsed = 0;
            const _fparts = [];
            let _vPrev = '[0:v]';
            let _aPrev = '[0:a]';
            for (let _gi = 1; _gi < groupMp4Paths.length; _gi++) {
              _elapsed += _groupDurations[_gi - 1] - _tDur;
              const _vOut = _gi === groupMp4Paths.length - 1 ? '[vout]' : `[v${_gi}]`;
              const _aOut = _gi === groupMp4Paths.length - 1 ? '[aout]' : `[a${_gi}]`;
              _fparts.push(
                `${_vPrev}[${_gi}:v]xfade=transition=${_tName}:duration=${_tDur}:offset=${_elapsed.toFixed(3)}${_vOut}`
              );
              _fparts.push(`${_aPrev}[${_gi}:a]acrossfade=d=${_tDur}${_aOut}`);
              _vPrev = _vOut;
              _aPrev = _aOut;
            }
            const _xfadeArgs = _inputArgs.concat([
              '-filter_complex', _fparts.join(';'),
              '-map', '[vout]',
              '-map', '[aout]',
            ...
            log(asmId, `  🎞  Scene transitions: ${_tName} (${_tDur}s) across ${groupMp4Paths.length} groups`);
            await new Promise((res, rej) => {
              const ff = execFile(ffmpegPath(), _xfadeArgs, { maxBuffer: 100 * 1024 * 1024 });
              ff.stderr.on('data', (d) => {
                const l = d.toString();
                if (l.includes('Error') || l.includes('error') || l.includes('Invalid')) {
                  log(asmId, `  [xfade] ${l.trim()}`);
                }
              });
              ff.on('close', (code) => {
                if (code === 0) res();
                else rej(new Error(`xfade stitch FFmpeg exited with code ${code}`));
              });
              ff.on('error', rej);
            });
          } else {
          ...
          } // end else (non-xfade path)
          try {
            fs.unlinkSync(stitchListPath);
          } catch (_) {}
        }

        // CPD-167: TTS narration mix for source-clip-only jobs.
        // applyVoiceoverToGroup only handles avatar+source_clip pairs. For jobs where all segments
        // are source_clips (e.g. all Twitch clip jobs), the TTS audio path is never consumed.
        // Here we duck the original audio and mix in the ElevenLabs narration track.
        const _ttsAudioPath = chromeBurnJobSpec?.state?.tts?.audioPath;
        if (_ttsAudioPath && fs.existsSync(_ttsAudioPath)) {
          const _narrationVol  = 2.2;  // narration boost so it sits above music-level clip audio
          const _duckVol       = 0.12; // duck original clip audio to 12% while narration plays
          const _narratedPath  = outPath.replace('.mp4', '_narrated.mp4');
          log(asmId, `\n🎙  Mixing TTS narration: ${path.basename(_ttsAudioPath)}`);
          try {
            await new Promise((res, rej) => {
              const _ff = execFile(
                ffmpegPath(),
                [
                  ...
                  '-i', _ttsAudioPath,
                  '-filter_complex',
                  `[0:a]volume=${_duckVol}[duck];[1:a]volume=${_narrationVol},apad[narr];[duck][narr]amix=inputs=2:duration=first:dropout_transition=2[aout]`,
                  '-map', '0:v',
                  '-map', '[aout]',
                  '-c:v', 'copy',
                  '-c:a', 'aac',
                ...
                (err) => {
                  if (err) { log(asmId, `  ⚠️  TTS mix failed (non-fatal) — ${err.message}`); res(); }
                  else {
                    try { fs.renameSync(_narratedPath, outPath); } catch (_) {}
                    log(asmId, '  ✅ TTS narration mixed into output');
                    res();
                  }
                }
              );
            ...
            log(asmId, `  ⚠️  TTS mix error (non-fatal) — ${_ttsErr.message}`);
          }
        }
        ...
        assemblyJobs[asmId].pct = 100;
        assemblyJobs[asmId].status = 'ffmpeg_done'; // Portal 3 + Drive still pending — poller must not fire yet
        assemblyJobs[asmId].outputPath = outPath;
        ...
        // Cleanup long-form intermediate segment files before handing off to Portal 3+
      ...
      // ── Portal 3a + Portal 3b: Assembly QA (gate worker system) ─────────────────────
      // Portal 3a: Gemini watches 3 sample points of assembled video (qualitative)
      // Portal 3b: Claude commitment verification (analytical — reads reports, not video)
      // Both are non-fatal for now — hard fails surface to monitoring for escalation.
      const gate3Timer = new StageTimer(asmId, 'Portal 3 QA');
      ...
      // Fire thumbnail generation in parallel with Portal 3a — Puppeteer render is independent of Gemini video review
      // By the time Portal 3a finishes (~45-60s), thumbnail is already on Drive
      ...
      log(asmId, `🖼  Thumbnail generation started in parallel with Portal 3a...`);
      ...
      // Only set to 'pass' when Portal 3a actually runs AND returns a pass or pass_with_notes outcome
      let qaResult = {
        portal: 'portal3a',
        jobId: asmId,
        passed: false,
        ...
        score: 65,
        sampleFindings: {},
        report: 'Portal 3a/3b gate worker system — awaiting gate run',
      };
      ...
      try {
        if (outPath && fs.existsSync(outPath)) {
          if (!gate2To3aReady) {
            log(asmId, `⛔ Portal 3a/3b blocked — Portal 2→3a handoff review did not pass`);
          } else {
            // Portal 3a — Gemini qualitative review
            if (gate3aWorker.canProduce(g3JobSpecWithPath).ready) {
              ...
              if (g3aPreflight.softHeals.length > 0) {
                log(asmId, `🩹 Portal 3a preflight soft-heal: ${g3aPreflight.softHeals.join('; ')}`);
              }
              if (!g3aPreflight.ready) {
                log(asmId, `⚠️  Portal 3a prerequisites warning: ${g3aPreflight.reasons.join('; ')}`);
              }
              ...
              log(asmId, `📋 Portal 3a: ${gwG3aResult.outcome} (${gwG3aResult.score}/100)`);
              ...
              try {
                const { saveAssemblyJob: saveAsmJobDb } = require('./db');
                saveAsmJobDb(asmId, assemblyJobId || asmId, { gate3aScore: gwG3aResult.score }).catch(() => {});
              } catch (e) {}
              ...
              if (gwG3aResult.passed || gwG3aResult.outcome === 'pass_with_notes') {
                qaResult = {
                  ...
                  outcome: 'pass',
                  passed: true,
                  report: `Portal 3a: ${gwG3aResult.outcome}`,
                };

                // Portal 3b — commitment verification (only if 3a passed)
                if (gate3bWorker.canProduce(g3JobSpec).ready) {
                  ...
                  if (g3bPreflight.softHeals.length > 0) {
                    log(
                      asmId,
                      `🩹 Portal 3b preflight soft-heal: ${g3bPreflight.softHeals.join('; ')}`
                    );
                  }
                  if (!g3bPreflight.ready) {
                    log(
                      asmId,
                      `⚠️  Portal 3b prerequisites warning: ${g3bPreflight.reasons.join('; ')}`
                    );
                  }
                  ...
                  log(
                    asmId,
                    `📋 Portal 3b: ${gwG3bResult.outcome} — ${gwG3bResult.mismatches?.length || 0} mismatch(es)`
                  );
                  ...
                  // Portal 3b self-healing: attempt targeted chrome re-burn on fixable mismatches
                  if (
                    gwG3bResult &&
                    ...
                    if (chromeRelated) {
                      log(
                        asmId,
                        `🔧 Portal 3b: attempting targeted chrome re-burn for ${fixableMismatches.length} fixable mismatch(es)`
                      );
                      try {
                        pipelineBus.emit('pipeline:retry_attempt', {
                          jobId: g3JobSpec.jobId,
                      ...
                      try {
                        ...
                        const _chromeCfgReBurn =
                          _frozenReBurnCfg && typeof _frozenReBurnCfg === 'object'
                            ...
                            : (() => {
                                ...
                                const custConfig = loadCustomerConfig(g3JobSpec.customerId );
                                const _reburnPlatform = g3JobSpec?.sourcePlatform
                                  || (baseContentType === 'twitch' ? 'twitch' : null)
                                  || detectSourcePlatform([
                                      ...(g3JobSpec?.sourceConfig?.urls || []),
                                      ...(g3JobSpec?.order?.inputs?.sources || []),
                                    ]);
                                return resolveChromeCfg(custConfig, _ctForReBurn, _reburnPlatform);
                              })();
...

                        {
                          if (
                            ...
                            log(
                              asmId,
                              `✅ Portal 3b: chrome re-burn complete — re-running Portal 3a to confirm`
                            );

                            // Re-run Portal 3a to confirm fix
                            try {
                              ...
                              if (reRunG3a.passed || reRunG3a.outcome === 'pass_with_notes') {
                                ...
                                log(
                                  asmId,
                                  `✅ Portal 3b: self-heal confirmed — Portal 3a re-check: ${reRunG3a.score}/100`
                                );
                                try {
                                ...
                                log(
                                  asmId,
                                  `⚠️  Portal 3b: re-burn applied but Portal 3a re-check still failed (${reRunG3a.score}/100) — proceeding`
                                );
                              }
                            } catch (reRunErr) {
                              log(
                                asmId,
                                `⚠️  Portal 3b: Portal 3a re-check error: ${reRunErr.message} — using original result`
                              );
                            }
                          }
                        ...
                        log(
                          asmId,
                          `⚠️  Portal 3b: chrome re-burn failed (non-fatal): ${reburnErr.message} — proceeding with original`
                        );
                  ...
                  log(asmId, `⚠️  Portal 3b: commitments empty — skipping`);
                  gate3To4Ready = false;
                }
              } else if (gwG3aResult.outcome === 'hard_fail') {
                log(
                  asmId,
                  `❌ Portal 3a hard fail — ${gwG3aResult.ffmpegAlarm?.issue || 'quality check failed'} — monitoring will escalate`
                );
                qaResult = {
                  score: gwG3aResult.score,
                  outcome: 'fail',
                  passed: false,
                  report: `Portal 3a hard fail: ${gwG3aResult.ffmpegAlarm?.issue || 'quality check failed'}`,
                };
                gate3To4Ready = false;
              }
            } else {
              log(
                asmId,
                `⚠️  Portal 3a: GEMINI_API_KEY not set or assembled path unavailable — skipping`
              );
              gate3To4Ready = false;
            }
          }
        } else {
          log(asmId, `⚠️  Portal 3a/3b: assembled video not found at ${outPath} — skipping`);
          gate3To4Ready = false;
        }
      } catch (g3Err) {
        log(asmId, `⚠️  Portal 3a/3b error: ${g3Err.message} — downstream gates will be blocked`);
        logError('GATE3_QA_ERROR_FALLBACK', g3Err.message, { asmId });
        gate3To4Ready = false;
      }

      // Store portal 3 score on assemblyJobs for downstream checks
      ...
      // Finalize all job metrics now that Portal 3 is complete
      ...
      // Log final Portal 3 outcome
      if (qaResult.outcome === 'fail') {
        log(asmId, `❌ Portal 3 HARD FAIL (score: ${qaResult.score}/100) — Drive upload blocked`);
      } else {
        log(asmId, `✅ Portal 3 PASSED (score: ${qaResult.score}/100) — proceeding to Drive upload`);
      }
      ...
      if (process.env.SKIP_DRIVE_UPLOAD === 'true') {
        ...
        // CPD-431: Apply post-processing effects (loudnorm, LUT, portrait reframe, etc.)
        // before upload — non-fatal, always returns outPath even on failure.
        try {
          const { applyPostProcessingEffects } = require('./assembly_postprocess');
          const _ppJobSpec = req.body?.jobSpec || null;
          if (_ppJobSpec) {
            await applyPostProcessingEffects(_ppJobSpec, outPath, (msg) => log(asmId, msg));
          }
        } catch (_ppErr) {
          log(asmId, `  ⚠️  [postprocess] Hook error: ${_ppErr.message}`);
        }

        // C1+ path: upload to R2 if credentials are present (DATABASE_URL + R2_ACCESS_KEY_ID)
        // C0 path: upload to Google Drive
        // Fallback: localhost URL (dev/test only — Upload-Post cannot reach this)
        const useR2 = !!(process.env.DATABASE_URL && process.env.R2_ACCESS_KEY_ID);

        if (useR2) {
          log(asmId, `\n☁️  Uploading assembled video to R2 (C1+ path)...`);
          nrAssembly('AssemblyR2UploadStart', {
            assemblyId: asmId,
            sourceJobId: assemblyJobId || null,
            contentType: contentType || null,
            fileName: outFile,
          ...
          try {
            const r2Url = await uploadToR2(outPath, outFile, {
              folder: 'output',
              contentType: 'video/mp4',
            });
            assemblyJobs[asmId].r2VideoUrl = r2Url;
            assemblyJobs[asmId].driveUrl = r2Url; // portal5 reads driveUrl — R2 URL is compatible
            log(asmId, `✅ R2 upload complete: ${r2Url}`);
            nrAssembly('AssemblyR2UploadOk', {
              assemblyId: asmId,
              sourceJobId: assemblyJobId || null,
              contentType: contentType || null,
              hasR2Url: true,
            ...
            log(asmId, `⚠️  R2 upload failed: ${r2Err.message}`);
            nrAssembly('AssemblyR2UploadFail', {
              assemblyId: asmId,
              sourceJobId: assemblyJobId || null,
              contentType: contentType || null,
              reason: 'exception',
              error: (r2Err.message || '').slice(0, 500),
            });
      ...
      if (assemblyJobs[asmId].status === 'ffmpeg_done') {
        ...
        if (qaOutcome === 'fail') {
          assemblyJobs[asmId].status = 'failed'; // Portal 3 hard fail — poller will record this
          saveAssemblyJob(asmId);
        } else {
          assemblyJobs[asmId].status = 'done'; // Portal 3 pass or no QA result
          saveAssemblyJob(asmId);
        }
      }

      // (Intermediate file cleanup and else-block close moved above Portal 3+ section
      //  so Portal 3 → Portal 4 → Portal 5 runs for both short-form AND long-form jobs.)

      // ── Portal 4 + Portal 5: run after Drive upload, regardless of Drive outcome ─────────────
      // assemblyJobs[asmId].driveUrl is always set: real Drive URL, localhost fallback, or null.
      // Portal 4 only needs the local assembled file (outPath); Portal 5 needs a public URL.
      // Portal 5 will gracefully fail if Drive is unavailable — Portal 4 still gives broadcast signal.
      if (assemblyJobs[asmId].qaOutcome !== 'fail' && outPath) {
        ...
        // Generate publish copy BEFORE Portal 4/5 — Portal 5 needs title for Upload-Post
        // Skip if already locked at Portal 1 pass (publishCopy in jobSpec or assemblyJobs card)
        const _existingPublishCopy = assemblyJobs[asmId]?.publishCopy || null;
        if (_existingPublishCopy) {
          log(asmId, `  ✅ Publish copy already locked from Portal 1 — skipping regeneration`);
          log(
            asmId,
        ...
        // ── Portal 4 + Portal 5 (gate worker system) ─────────────────────────────────
        // Portal 4: Gemini full-video broadcast ready check (only if gate3b passed)
        // Portal 5: Upload confirmation (only fires if gate4.uploadSignal === true)
        try {
          const gate4Worker = require('./portals/portal4');
          // Ensure thumbnail promise is settled before Portal 4 checks thumbnailDriveUrl
            ...
            g45JobSpec = {
              ...
              state: {
                ...
                savedOutputs: {
                  assembledPath: outPath,
                  driveUrl,
                  r2VideoUrl: assemblyJobs[asmId]?.r2VideoUrl || null,
          ...
          // Inject publishCopy, thumbnailDriveUrl, and segmentDurations into g45JobSpec.state.savedOutputs
          // so Portal 5 can read them without requiring a DB round-trip
          const _publishCopy = assemblyJobs[asmId]?.publishCopy || null;
          const _thumbDriveUrl = assemblyJobs[asmId]?.thumbnailDriveUrl || null;
          const _segDurs = assemblyJobs[asmId]?.segmentLabelsAndDurations || null;
          if (g45JobSpec.state?.savedOutputs) {
            ...
            if (_segDurs && _segDurs.length > 1) g45JobSpec.state.savedOutputs.segmentDurations = _segDurs;
          }
          ...
          // Portal 4 — full video broadcast ready
          if (!gate3To4Ready) {
            log(asmId, `⛔ Portal 4/5 blocked — Portal 3b→4 handoff review did not pass`);
            ...
            if (g4Preflight.softHeals.length > 0) {
              log(asmId, `🩹 Portal 4 preflight soft-heal: ${g4Preflight.softHeals.join('; ')}`);
            }
            if (!g4Preflight.ready) {
              log(asmId, `⚠️  Portal 4 prerequisites warning: ${g4Preflight.reasons.join('; ')}`);
            }
            ...
            log(
              asmId,
              `📋 Portal 4: ${g4Result.passed ? 'PASS' : 'FAIL'} (uploadSignal=${g4Result.uploadSignal})`
            );

            // Persist gate4 result to DB (non-fatal; score stored as 1=pass, 0=fail)
            try {
              const { saveAssemblyJob: saveAsmJobDb } = require('./db');
              saveAsmJobDb(asmId, assemblyJobId || asmId, { gate4Score: g4Result.passed ? 1 : 0 }).catch(() => {});
            } catch (e) {}

            // Portal 5 — upload confirmation (only if gate4.uploadSignal is true)
            const g4PostSpec = {
            ...
            if (
              ...
              if (g5Preflight.softHeals.length > 0) {
                log(asmId, `🩹 Portal 5 preflight soft-heal: ${g5Preflight.softHeals.join('; ')}`);
              }
              if (!g5Preflight.ready) {
                log(asmId, `⚠️  Portal 5 prerequisites warning: ${g5Preflight.reasons.join('; ')}`);
              }
              ...
              log(asmId, `📋 Portal 5: ${g5Result.passed ? 'PASS' : 'FAIL'}`);
            } else {
              log(
                asmId,
                `⚠️  Portal 5 skipped (gate4 uploadSignal=${g4Result.uploadSignal}, handoffPass=${g4Handoff.passed})`
              );
            }
          } else {
            log(asmId, `⚠️  Portal 4 not ready — assembled file may be too large or missing`);
          }
        } catch (g45Err) {
          log(asmId, `⚠️  Portal 4/5 error (non-blocking): ${g45Err.message}`);
        }
      }
    } catch (err) {
