...
async function handleAssemble(req, res, saveJobCard) {
  ...
  const run = async () => {
    try {
      ...
      // Align segment types with downloaded files for pre-flight + Gate 3 counts.
      // Prefer localFileTypes[] (filled alongside each successful download) — it stays correct
      // when a middle segment is skipped. Legacy filename matching desyncs (e.g. synth prebuild).
      let segTypes = [];
        ...
        if (!_directiveHandled && (segTypes[i] || 'avatar') === 'avatar') {
          try {
            ...
            if (specItems.length > 0) {
              ...
              // specItems is empty — log error and default to 0. This should not happen
              // if req.body.items was correctly written to gwJobSpec.order.inputs.items.
              console.error(`[chrome-burn] specItems empty for job — items injection failed upstream. label=${label}`);
              activeIdx = 0;
            }
            ...
            if (specItems.length > 0) {
              ...
              // specItems is empty — chrome sidebar will be blank.
              // This means req.body.items was not injected into job spec. Fix upstream.
              console.error(`[chrome-burn] specItems empty — sidebar will be blank. contentType=${contentType}`);
              allStories = [];
              overlayTitle = label || 'CONTENT';
              activeCategory = contentType.toUpperCase();
            }
...
// Gap #45: OUTRO freeze-hold -- clone last frame for 0.75s (long-form only)
        ...
        try {
          await new Promise((res, rej) => {
            ...
            // Change 2: source crop filter — read from job spec (assemblyConfig per content type)
            // designSpec.assembly = assemblyConfig = { news: {...}, clips: {...}, sports: {...} }
            ...
            // Change 3: Al Jazeera watermark box — read from job spec (assemblyConfig per content type)
            ...
            // Change 4: clip trim params — read from job spec (assemblyConfig per content type)
      ...
      // mixMode comes from designSpec.assembly[asmCtKey].mixMode (assemblyConfig in c0.json)
      const _asmCtKey = contentType === 'twitch' ? 'clips' : contentType === 'nba' ? 'sports' : (contentType || 'news').replace(/-short$/, '');
      const voiceoverMode = chromeBurnJobSpec?.designSpec?.assembly?.[_asmCtKey]?.mixMode === 'voiceover';
