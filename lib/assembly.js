  ...
  const run = async () => {
    ...
    try {
      ...
      const concatTsToMp4 = async (tsList, outMp4, encodeAudio = true) => {
        ...
        const inputArgs = tsList.length === 1
          ? ['-i', tsList[0]]
          : (() => {
              fs.writeFileSync(listPath, tsList.map(f => `file '${f.replace(/'/g, "'\\''")}'`).join('\n'));
              return ['-f', 'concat', '-safe', '0', '-i', listPath];
            })();
        // Force consistent 30fps and 90000 timescale on every group MP4 so the final
        // -c copy stitch can concatenate across groups without time_base conflicts.
        // -shortest (CPD-885): a group mp4 must never carry audio longer than video —
        // the final -c copy stitch converts per-group A/V imbalance into progressive lip-sync drift.
        const encArgs = encodeAudio
          ? [...ffmpegEncodeArgs(false), '-r', '30', '-video_track_timescale', '90000',
             ...
             '-shortest']
          : ['-c:v', 'copy', '-c:a', 'aac', '-ar', '44100', '-ac', '2', '-bsf:a', 'aac_adtstoasc'];
        const args = [...inputArgs, ...encArgs, '-movflags', '+faststart', '-y', outMp4];
        await new Promise((res, rej) => {
          const proc = execFile(ffmpegPath(), args, { maxBuffer: 50 * 1024 * 1024 });
          proc.on('close', code => code === 0 ? res() : rej(new Error(`Group concat failed: ${code}`)));
          proc.on('error', rej);
        });
        if (tsList.length > 1) { try { fs.unlinkSync(listPath); } catch(_) {} }
      }; // end concatTsToMp4
      ...
      if (stitchPaths.length === 1) {
        ...
        // Multi-group stitch: remux segments to MPEG-TS then concat.
        // MP4 concat demuxer + -c copy produces bogus video duration / non-monotonic DTS
        // (browser preview fails) even when each group MP4 probes clean individually.
