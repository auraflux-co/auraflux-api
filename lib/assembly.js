...
async function handleAssemble(req, res, saveJobCard) {
  ...
  const run = async () => {
    ...
    try {
      ...
      // Compute episode number for the thumbnail — reads the same counters file that the chrome burn uses.
      // Must be done here (before the parallel promise) so thumbnail.js receives it via savedOutputs.episodeNumber.
      const _thumbEpCountersPath = require('path').join(__dirname, '..', 'data', 'episode_counters.json');
      let _thumbEpNumRaw = 1;
      try {
        const _thumbEpC = JSON.parse(fs.readFileSync(_thumbEpCountersPath, 'utf8'));
        _thumbEpNumRaw = _thumbEpC[contentType] || _thumbEpC.twitch || 1;
      } catch(_e) {}
      const _thumbEpisodeNumber = assemblyJobs[asmId]?.episodeNumber || `Episode ${_thumbEpNumRaw}`;
      ...
      if (assemblyJobs[asmId].qaOutcome !== 'fail' && outPath) {
        ...
        if (_isTwitchLong && assemblyJobId && outPath && fs.existsSync(outPath)) {
          try {
            const { verifyTwitchSoupAssembly } = require('./twitch_bookends');
