...
/** HeyGen scene change within same studio — cut video, blend audio only. */
...
function soupJoinTransition(fromType, toType, fromLabel = '', toLabel = '') {
  ...
  // New HeyGen scene, same studio chrome — never xfade video (causes blip/ghost).
  if (soupIsSceneResetJoin(from, to)) {
    const afterReaction = /_CLIP1_REACTION$/i.test(from);
    return {
      useXfade: false,
      videoDur: 0,
      audioDur: 0,
      fadeReactionTail: afterReaction,
      reactionTailFadeSec: afterReaction ? 0.35 : 0,
      transition: 'cut',
      audioFadeSec: afterReaction ? 0.15 : 0.12,
      sceneReset: true,
    };
  }
  return {
...
async function mergeTwoWithXfade(leftPath, rightPath, outputPath, {
  ...
  const tailFade = fadeReactionTail ? Math.min(0.4, Math.max(0.2, audioXfadeDur)) : 0;
