...
async function concatMediaWithTransition(inputFiles, outputPath, {
  ...
  const useXfade = transition !== 'cut' && n <= 30;
  if (!useXfade) {
    if (transition !== 'cut' && n > 30) {
  ...
  for (let i = 1; i < n; i++) {
    ...
    filterParts.push(
      `${prevV}[${i}:v]xfade=transition=${transitionName}:duration=${transitionDur}:offset=${cumOffset.toFixed(3)}${vOut}`,
    );
  ...
  const run = async () => {
    ...
    try {
      ...
      const applyVoiceoverToGroup = async (groupTsFiles, groupSegTypes, groupSegMeta) => {
        ...
        if (_voAsmCfg.mixMode !== 'voiceover') return { files: groupTsFiles, types: groupSegTypes, meta: groupSegMeta };
      ...
      const concatTsToMp4 = async (tsList, outMp4, encodeAudio = true, trans = transition) => {
