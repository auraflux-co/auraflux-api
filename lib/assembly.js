...
// ── New Relic custom event helper (Gate 3 + Assembly events) ─────────────────
const _nrAgentAsm = process.env.NEW_RELIC_LICENSE_KEY ? require('newrelic') : null;
function nrEvent(name, attrs) {
  if (_nrAgentAsm) {
    try {
      _nrAgentAsm.recordCustomEvent(name, { ...attrs, env: process.env.NODE_ENV || 'development' });
    } catch (_) {
    ...
    try {
      ...
      if (isShortForm) {
        ...
        if (hasLogo) {
          await new Promise((res, rej) => {
            const args = [
              '-i', captionedPath,   // ← was stackedPath, now captionedPath
              '-i', logoPath,
      ...
      for (let i = 0; i < localFiles.length; i++) {
        ...
        if (contentType === 'twitch' && (segTypes[i] || 'avatar') === 'avatar') {
          ...
          try {
            ...
            const isOutroOrColdOpen = /^(OUTRO|COLD_OPEN)$/i.test(label.trim());
            const burnedPath = inputForTS.replace('.mp4', '_twitch_burned.mp4');

            if (isOutroOrColdOpen) {
              // OUTRO / COLD_OPEN: no chrome per spec §5.6 — skip burn entirely
            } else {
              // All other avatar segments (INTRO, REACTION, CLIP_SETUP):
              // Single-state — flag visible, sidebar visible with active streamer highlighted.
              const overlayPath = path.join(TMP_DIR, `twitch_overlay_${Date.now()}.png`);

              await generateNewscastOverlay(overlayBase, overlayPath, activeIdx, {
              ...
              await new Promise((res, rej) => {
                ...
                proc.on('close', code => {
                  if (code === 0) res();
                    ...
                    console.error(`[twitch-chrome] FFmpeg exit ${code}: ${reason}`);
                    rej(new Error(`Twitch chrome burn failed: ${code} — ${reason}`));
                  }
                });
                proc.on('error', rej);
              });

              try { if (fs.existsSync(overlayPath)) fs.unlinkSync(overlayPath); } catch(e) {}

              if (fs.existsSync(burnedPath) && fs.statSync(burnedPath).size > 10000) {
                inputForTS = burnedPath;
                log(asmId, `  🎮 Twitch chrome burned [${activeIdx + 1}/${allStories.length}]: ${label}`);
              }
            }
          } catch(e) {
          ...
          try {
            ...
            if (isStoryIntro) {
              // Two-state burn: PNG A (lower-third + sidebar visible) for t=0..introDur,
              //   PNG B (lower-third hidden, sidebar still visible) for t>introDur
              ...
              // COLD_OPEN / OUTRO: no chrome per spec §4.7 — skip burn entirely
            }
          } catch(e) {
          ...
          try {
            ...
            if (isOutroOrColdOpen) {
              // OUTRO / COLD_OPEN: no chrome per spec §6.7 — skip burn entirely
            } else {
              // All other avatar segments (INTRO, NARRATION, REACTION):
              // Single-state — flag visible, sidebar visible with active game highlighted.
              const overlayPath = path.join(TMP_DIR, `nba_overlay_${Date.now()}.png`);

              await generateNewscastOverlay(overlayBase, overlayPath, activeGameIndex, {
              ...
              await new Promise((res, rej) => {
                ...
                proc.on('close', code => {
                  if (code === 0) res();
                    ...
                    const reason = burnStderr.slice(-300).replace(/\n/g, ' ').trim();
                    console.error(`[nba-chrome] FFmpeg exit ${code}: ${reason}`);
                    rej(new Error(`NBA chrome burn failed: ${code} — ${reason}`));
                  }
                });
                proc.on('error', rej);
              });
              ...
              if (fs.existsSync(burnedPath) && fs.statSync(burnedPath).size > 10000) {
                inputForTS = burnedPath;
                log(asmId, `  🏀 NBA chrome burned [game ${activeGameIndex + 1}/${allStories.length}]: ${label}`);
              }
            }
          } catch(e) {
...
// Gap #45: OUTRO freeze-hold -- clone last frame for 0.75s (long-form only)
        ...
        try {
          await new Promise((res, rej) => {
            ...
            // For news source clips: use pillarboxFilter if set (portrait 9:16 → navy pillarbox + gold seams)
            // otherwise fall back to zoom-to-fill for landscape clips.
            const seg = segsToProcess[tsFiles.length];
            const segPillarbox = (!isAvatarSeg && contentType === 'news' && seg?.pillarboxFilter) ? seg.pillarboxFilter : null;
            const vfFilter = isAvatarSeg
              ? 'scale=1920:1080:flags=lanczos,pad=1920:1080:(ow-iw)/2:(oh-ih)/2:color=black,fps=fps=30'
              : segPillarbox
                ? `${segPillarbox},fps=fps=30,drawbox=x=1710:y=900:w=210:h=180:color=0x0d1424@1.0:t=fill`
                : `scale=1920:1080:force_original_aspect_ratio=increase,${sourceCropSize}scale=1920:1080,fps=fps=30` +
                  (contentType === 'news' ? ',drawbox=x=1710:y=900:w=210:h=180:color=0x0d1424@1.0:t=fill' : '');
      ...
      while (qaAttempt < MAX_QA_RETRIES) {
        ...
        try {
          ...
          if (qaResult.outcome === 'pass' || qaResult.outcome === 'manual_review') {
            ...
            nrEvent('PipelineGatePass', { gate: 3, jobId: assemblyJobId, score: qaResult.score, contentType, outcome: qaResult.outcome });
            break;
            ...
            nrEvent('PipelineGateFail', { gate: 3, jobId: assemblyJobId, score: qaResult.score, contentType, reason: 'max_retries' });
            nrEvent('QAUnresolvable', { gate: 3, jobId: assemblyJobId, score: qaResult.score, reason: qaResult.report?.split('\n').slice(0, 2).join('; ') || 'max_retries_exhausted' });
