    ...
    try {
      ...
      for (let i = 0; i < segsToProcess.length; i++) {
        ...
        if (segType === 'source_clip') {
          ...
          if (storedIsTwitchPage && twitchPageForFallback) {
            log(asmId, `📥 Twitch page URL for ${label} — yt-dlp download (skip CDN refresh)`);
      ...
      if (isShortForm) {
        ...
        if (isClipsOnlyComp) {
          ...
          const portraitClips = [];
          const clipHookTitles = resolvedHooks.slice(0, clipFiles.length);
          for (let ci = 0; ci < clipFiles.length; ci++) {
            ...
            const hookTitle = clipHookTitles[ci] || '';
            let segPath = pc;
            if (hookTitle) {
          ...
          if (assemblyJobId && clipHookTitles.some(Boolean)) {
            const _hookCard = global.persistedJobsRef && global.persistedJobsRef[assemblyJobId];
            if (_hookCard) {
              _hookCard.clipHookTitles = clipHookTitles;
              if (typeof saveJobCard === 'function') {
      ...
      if (assemblyJobs[asmId].qaOutcome !== 'fail' && outPath) {
        ...
        if (_reuseGate1Copy) {
          ...
          const _cardHooks = assemblyJobId && global.persistedJobsRef?.[assemblyJobId]?.clipHookTitles;
          if (clipsOnlyCompJob && _cardHooks?.length) {
            publishScript = _cardHooks.filter(Boolean).map((h, i) => `CLIP ${i + 1}: ${h}`).join('\n');
          }
