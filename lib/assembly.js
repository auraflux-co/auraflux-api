...
const C0_LEGACY_OVERLAY_ONLY = process.env.LEGACY_OVERLAY_ONLY !== 'false' && process.env.C0_LEGACY_OVERLAY_ONLY !== 'false';
...
function getPinnedComment(contentType, customerId = 'c0') {
  try {
    ...
    const baseType = (contentType || 'news').replace(/-short$/, '').replace(/^nba$/, 'sports').replace(/^twitch$/, 'clips');
    const cfg = loadCustomerConfig(customerId);
    const comment = cfg?.delivery?.pinnedComments?.[baseType] || cfg?.delivery?.pinnedComments?.[contentType];
      ...
      const processSegmentToTs = async (inputFilePath, segIdx, localSegIdx) => {
        ...
        if (!useLegacyChrome && USE_DIRECTIVE_CHROME && assemblyJobId && hasDirectiveForJob(assemblyJobId) && (segTypes[segIdx] || 'avatar') === 'avatar') {
          try {
            ...
            if (scene) {
              try {
                const _forceSidebarForC0 = process.env.FORCE_SIDEBAR_VISIBLE !== 'false' && process.env.C0_FORCE_SIDEBAR_VISIBLE !== 'false';
        ...
        if (!_directiveHandled) {
          try {
            ...
            const _ctKeyForDefault = chromeSkin === 'twitch' ? 'clips' : chromeSkin === 'nba' ? 'sports' : chromeSkin || 'news';
            let defaultCategory;
            try {
            ...
            if (!defaultCategory) defaultCategory = chromeSkin === 'twitch' ? 'ON STREAM' : chromeSkin === 'nba' ? 'NBA GAME' : 'WORLD NEWS';
