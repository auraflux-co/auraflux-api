                  ...
                  try {
                    ...
                    const reRenderBody = {
                      ...
                      video_inputs: [{
                        ...
                        voice: { type: 'text', input_type: 'ssml', input_text: sceneText, voice_id: process.env.HEYGEN_VOICE_ID, speed: parseFloat(process.env.HEYGEN_SPEAK_SPEED || '0.85') }
                      }],
                      dimension: { width: isPortrait ? 720 : 1280, height: isPortrait ? 1280 : 720 },
                      dynamic_duration: true,
                      test: false
                    };

                    const submitResp = await axios.post('https://api.heygen.com/v2/video/generate', reRenderBody,
                    ...
                    for (let attempt = 0; attempt < 18; attempt++) {
                      ...
                      const statusResp = await axios.get(`https://api.heygen.com/v1/video_status.get?video_id=${newVideoId}`,
                      ...
                      if (status === 'failed') throw new Error(`HeyGen re-render failed for ${badSeg.label}`);
                    }
        ...
        const buildTsArgs = async () => {
          if (clipIntroSkipSecs > 0 && !isAvatarSeg) {
            try {
              ...
              const finalTrim = (silenceTrimDur && silenceTrimDur > 0 && silenceTrimDur < NEWS_CLIP_MAX_SECONDS)
                ? silenceTrimDur : NEWS_CLIP_MAX_SECONDS;
              return ['-ss', String(clipIntroSkipSecs), '-i', inputForTS, '-t', finalTrim.toFixed(3), ...baseArgs];
            } catch(trimErr) {
              return ['-ss', String(clipIntroSkipSecs), '-i', inputForTS, '-t', String(NEWS_CLIP_MAX_SECONDS), ...baseArgs];
            }
          }
      ...
      const concatTsToMp4 = async (tsList, outMp4, encodeAudio = true) => {
        if (tsList.length === 1) {
          const args = [
            '-i', tsList[0],
            '-c:v', 'copy', '-c:a', 'aac', '-ar', '44100', '-ac', '2',
            '-bsf:a', 'aac_adtstoasc', '-movflags', '+faststart', '-y', outMp4
          ];
          await new Promise((res, rej) => {
            const proc = execFile(ffmpegPath(), args, { maxBuffer: 50 * 1024 * 1024 });
            proc.on('close', code => code === 0 ? res() : rej(new Error(`Single-segment group encode failed: ${code}`)));
            proc.on('error', rej);
          });
          return;
        }
        ...
        const encArgs = encodeAudio
          ? [...ffmpegEncodeArgs(false), '-c:a', 'aac', '-ar', '44100', '-ac', '2', '-af', 'aresample=async=1']
          : ['-c:v', 'copy', '-c:a', 'aac', '-ar', '44100', '-ac', '2', '-bsf:a', 'aac_adtstoasc'];
        const args = ['-f', 'concat', '-safe', '0', '-i', listPath, ...encArgs, '-movflags', '+faststart', '-y', outMp4];
        await new Promise((res, rej) => {
          const proc = execFile(ffmpegPath(), args, { maxBuffer: 50 * 1024 * 1024 });
