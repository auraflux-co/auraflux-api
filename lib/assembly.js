    ...
    try {
      ...
      if (isShortForm) {
        ...
        if (isClipsOnlyComp) {
          ...
          const resolvedHooks = await resolveHooksForAssembly({
            hookItems,
            hookClips,
      ...
      if (assemblyJobs[asmId].qaOutcome !== 'fail' && outPath) {
        ...
        if (_reuseGate1Copy) {
          ...
          if (clipsOnlyCompJob && _cardHooks?.length) {
          ...
          if (clipsOnlyCompJob && outPath && fs.existsSync(outPath)) {
            try {
              ...
              if (transcript) {
                publishScript = `CLIPS IN THIS COMPILATION:\n${publishScript}\n\nFULL TRANSCRIPT OF THE VIDEO:\n${transcript}`;
                log(asmId, `  📜 Publish copy will use the Whisper transcript (${transcript.length} chars, CPD-939)`);
              }
            } catch (_te) { /* fall back to clip-title script */ }
          }
          try {
