    ...
    try {
      ...
      if (isShortForm) {
        ...
        if (isClipsOnlyComp) {
          ...
          const { applyPortraitBlurPad, burnClipCompHookCaption } = require('./assembly_postprocess');
          ...
          const useEditorial = isEditorialContentType(ct);
          let editorialAccent = '#1CE8FF';
          if (useEditorial) {
            try {
              editorialAccent = buildSpokenScripts(ct, { topic: '' }).accentColor;
            } catch (_) {}
          }
          ...
          for (let ci = 0; ci < clipFiles.length; ci++) {
            const pc = path.join(TMP_DIR, `${asmId}_comp_${ci}.mp4`);
            shortTempPaths.push(pc);
            await applyPortraitBlurPad(clipFiles[ci], pc, { jobId: asmId, log: (m) => log(asmId, `  ${m}`) });
            const hookIdx = clipSourceIndices[ci];
            const hookTitle = (hookIdx != null ? resolvedHooks[hookIdx] : resolvedHooks[ci]) || '';
            let segPath = pc;
            if (hookTitle) {
              const pcHook = path.join(TMP_DIR, `${asmId}_comp_${ci}_hook.mp4`);
              shortTempPaths.push(pcHook);
              await burnClipCompHookCaption(pc, pcHook, {
                text: hookTitle,
            ...
            if (useEditorial) {
              const pcChip = path.join(TMP_DIR, `${asmId}_comp_${ci}_chip.mp4`);
              shortTempPaths.push(pcChip);
              const chipText = hookTitle.split(/[|–-]/)[0].trim().slice(0, 36)
                || buildSpokenScripts(ct, { topic: '' }).categoryLabel;
          ...
          clipsOnlyCompPath = path.join(TMP_DIR, `${asmId}_comp_concat.mp4`);
          ...
          if (useEditorial) {
            try {
              ...
              if (edOut && fs.existsSync(edOut)) {
                fs.copyFileSync(edOut, clipsOnlyCompPath);
                editorialBuilt = true;
                log(asmId, '  🎬 Editorial sports/news comp assembled (intro + bridges + outro)');
              }
            } catch (edErr) {
              log(asmId, `  ⚠️  Editorial timeline failed (non-fatal): ${edErr.message.slice(0, 140)}`);
            }
          }

          if (!editorialBuilt) {
          if (portraitClips.length === 1) {
            fs.copyFileSync(portraitClips[0], clipsOnlyCompPath);
          } else {
            // Re-encode on concat — keeps A/V aligned for whisper captions + loudnorm
            const listPath = path.join(TMP_DIR, `${asmId}_comp_concat_list.txt`);
            shortTempPaths.push(listPath);
            fs.writeFileSync(
              listPath,
            ...
            await new Promise((res, rej) => {
              // Do NOT use vf fps=30 here — it truncates video ~30% short while audio
              // runs full length (90s v / 128s a), breaking whisper captions + hooks.
              const args = [
                '-f', 'concat', '-safe', '0', '-i', listPath,
              ...
              const proc = execFile(ffmpegPath(), args, { maxBuffer: 50 * 1024 * 1024 });
              proc.on('close', code => code === 0 ? res() : rej(new Error(`comp_concat failed: ${code}`)));
              proc.on('error', rej);
            });
          }
          }
        ...
        const logoLabel = isClipsOnlyComp
          ? `top blur fold (max ${clipCompLogo.maxWidth}×${clipCompLogo.maxHeight})`
          : `${shortLogoPos.size}px bottom-right mug`;
        log(asmId, `  🏷  Applying CWN logo (${logoLabel})...`);

        if (hasLogo) {
          const logoFilter = isClipsOnlyComp
            ? `[1:v]scale=w='min(${clipCompLogo.maxWidth}\\,iw*${clipCompLogo.maxHeight}/ih)':h='min(${clipCompLogo.maxHeight}\\,ih*${clipCompLogo.maxWidth}/iw)':force_original_aspect_ratio=decrease,format=rgba,colorchannelmixer=aa=${clipCompLogo.opacity}[logo];[0:v][logo]overlay=x=(W-w)/2:y=(${clipCompLogo.topBlurBand}-h)/2:format=auto,format=yuv420p[vout]`
      ...
      try {
        ...
        if (assemblyJobId && ppApplied) {
          ...
          if (_ppCard) {
            _ppCard.captionsApplied = !!ppApplied.captions;
      ...
      if (assemblyJobs[asmId].qaOutcome !== 'fail' && outPath) {
        ...
        if (_reuseGate1Copy) {
          ...
          if (clipsOnlyCompJob && _clipCompBrief) {
            ...
            publishScript = buildClipCompSeoInput(_clipCompBrief);
