      ...
      if (assemblyJobs[asmId].qaOutcome !== 'fail' && outPath) {
        ...
        try {
          ...
          if (!gate3To4Ready) {
            ...
            // CPD-924: deferred publish — if the operator scheduled a future
            // publish time, hold here; scheduling_cron fires Gate 5 when due.
            ...
            if (g4Result.uploadSignal && g4Handoff.passed && _schedAt && new Date(_schedAt).getTime() > Date.now()) {
              ...
              if (_schedLiveCard) {
                ...
                _schedLiveCard.driveUrl = driveUrl;
                _schedLiveCard.state = _schedLiveCard.state || {};
                _schedLiveCard.state.savedOutputs = {
                  ...(_schedLiveCard.state.savedOutputs || {}),
                  driveUrl,
                  publishCopy: assemblyJobs[asmId]?.publishCopy || _schedLiveCard.state.savedOutputs?.publishCopy || null,
                };
                if (assemblyJobs[asmId]?.thumbnailDriveUrl) {
                  _schedLiveCard.thumbnailDriveUrl = assemblyJobs[asmId].thumbnailDriveUrl;
                  _schedLiveCard.state.savedOutputs.thumbnailDriveUrl = assemblyJobs[asmId].thumbnailDriveUrl;
                }
                if (typeof saveJobCard === 'function') {
            ...
            } else if (g4Result.uploadSignal && g4Handoff.passed && gate5Worker.canProduce(g45JobSpecWithPath).ready) {
              ...
              if (!_metaCheck.passed) {
                ...
                if (assemblyJobId) {
                  const _holdCard = global.persistedJobsRef && global.persistedJobsRef[assemblyJobId];
                  if (_holdCard) {
                    _holdCard.stage = 'metadata_review';
                    _holdCard.metadataQaViolations = _metaCheck.violations;
                    if (typeof saveJobCard === 'function') {
                      try { saveJobCard(assemblyJobId, _holdCard); } catch (_mrc) { /* non-fatal */ }
                    }
                  }
                }
              } else if (reviewHoldEnabled()) {
                log(asmId, '⏸ Gate 5 held for operator review (C0_REVIEW_HOLD) — use POST /job/:id/run-gate5 after approve');
                if (assemblyJobId) {
                  const _revCard = global.persistedJobsRef && global.persistedJobsRef[assemblyJobId];
                  if (_revCard) {
                    _revCard.stage = 'awaiting_review';
                    _revCard.driveUrl = driveUrl;
                    _revCard.state = _revCard.state || {};
                    _revCard.state.savedOutputs = {
                      ...(_revCard.state.savedOutputs || {}),
                      driveUrl,
                      publishCopy: assemblyJobs[asmId]?.publishCopy || _revCard.state.savedOutputs?.publishCopy || null,
                    };
                    if (typeof saveJobCard === 'function') {
                      try { saveJobCard(assemblyJobId, _revCard); } catch (_arc) { /* non-fatal */ }
                    }
                  }
                }
              } else {
