  ...
  const run = async () => {
    ...
    try {
      ...
      if (isShortForm) {
        ...
        // Pro/Agency: prefer brand custom caption font when uploaded
        const _brandIdForFont = req.body.jobSpec?.brandId || req.body.brandId || null;
        if (captionStyle && _brandIdForFont) {
          try {
            const { resolveCaptionFontLocalPath } = require('./services/caption_fonts');
            const customFont = await resolveCaptionFontLocalPath(_brandIdForFont);
            if (customFont) captionStyle = { ...captionStyle, font: customFont };
          } catch (_fontErr) {
            console.warn('[assembly] caption font resolve skipped:', _fontErr.message);
          }
        }
