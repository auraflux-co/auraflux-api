    ...
    try {
      ...
      if (isShortForm) {
        ...
        if (isClipsOnlyComp) {
          ...
          const { applyPortraitBlurPad, burnClipCompHookCaption, resolveClipHookTitle } = require('./assembly_postprocess');
          ...
          for (let ci = 0; ci < clipFiles.length; ci++) {
            ...
            const hookTitle = resolveClipHookTitle(hookItems[ci] || {}, hookClips[ci] || {});
            clipHookTitles.push(hookTitle || '');
            let segPath = pc;
            if (hookTitle) {
