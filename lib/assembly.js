  ...
  const run = async () => {
    try {
      ...
      try {
        ...
        const groupLabels = segGroups.map((g) => {
          const gid = g.groupId || '';
          if (/^intro$/i.test(gid)) return 'Intro';
          if (/^outro$/i.test(gid)) return 'Outro';

          // Extract index: STORY3 → 3, GAME2 → 2, STREAM1 → 1
