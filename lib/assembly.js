...
async function handleAssemble(req, res, saveJobCard) {
  ...
  if (assemblyJobId) {
    ...
    if (holdCard && holdStages.includes(holdCard.stage)) {
    ...
    try {
      ...
      for (let i = 0; i < segsToProcess.length; i++) {
        ...
        if (segType === 'source_clip') {
          ...
          if (clipSlug || seg.pageUrl || (url && url.includes('twitch.tv'))) {
        ...
        try {
          const twitchPage = segType === 'source_clip' && twitchPageForFallback ? twitchPageForFallback : null;
      ...
      // Override DB spec's sceneStructure.items with the request-provided designSpec when available.
      // The DB spec (loaded by jobSpecId) may be a COMPACT_FETCH pre-build spec created before the
      // user selected stories — its sceneStructure.items is in scrape order, NOT picker order.
      // req.body.designSpec comes from the live job card which has the user's selected story order.
      // Using the wrong order makes the sidebar highlight the wrong story throughout the video.
      ...
      const processSegmentToTs = async (inputFilePath, segIdx, localSegIdx) => {
        ...
        if (!_directiveHandled) {
          try {
            ...
            let _sidebarItems = allStories.map(s => ({
              title:    s.title || '',
      ...
      if (groupMp4Paths.length === 1) {
        ...
        const stitchContent = groupMp4Paths.map(f => `file '${f.replace(/'/g, "'\\''")}'`).join('\n');
        fs.writeFileSync(stitchListPath, stitchContent);
        const ffArgs = [
          '-f', 'concat', '-safe', '0', '-i', stitchListPath,
          '-c', 'copy',         // no re-encode — groups are already h264/aac at target spec
          '-movflags', '+faststart',
          '-y', outPath
        ];
        await new Promise((res, rej) => {
        ...
        try { fs.unlinkSync(stitchListPath); } catch(_) {}
      }
      ...
      if (assemblyJobs[asmId].qaOutcome !== 'fail' && outPath) {
        ...
        const _reuseGate1Copy = _existingPublishCopy && !_isShortFormPublish && !clipsOnlyCompJob;
