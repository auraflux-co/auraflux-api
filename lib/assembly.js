...
// chrome_overlay.js Puppeteer functions are @deprecated — no longer called here.
// Chrome burns now go through lib/chrome_overlay_ffmpeg.js (pure FFmpeg drawtext/drawbox).
// burnSceneChromeFromDirective is still imported for the directive path (USE_DIRECTIVE_CHROME).
const { burnSceneChromeFromDirective } = require('./chrome_overlay');
const { resolveChromeCfg, buildAndBurnChrome } = require('./chrome_overlay_ffmpeg');
...
function emitGateResult(jobSpec, result, contentType) {
  try {
    const event = result.passed ? 'gate:pass' : (result.outcome === 'sendback' ? 'gate:sendback' : 'gate:hard_fail');
    pipelineBus.emit(event, {
      ...
      reason:      result.failReason || (result.concerns || []).join('; ') || null
    });
  } catch(e) {}
}
...
async function handleAssemble(req, res, saveJobCard) {
  const { segments, segmentData, labels, transition='crossfade', format='mp4', outputDir, jobTitle, assemblyId, contentType, jobId: assemblyJobId, jobSpecId: reqJobSpecId, sceneTextMap, fullScript, expectedClips: contractExpectedClips = 0, designSpec: reqDesignSpec = null, nbaItems: reqNbaItems = [] } = req.body;
  ...
  const run = async () => {
    try {
          ...
          if (avatarSegsForQA.length > 0) {
            ...
            if (!g2JobSpec) {
              ...
              g2JobSpec = {
                ...
                deliverySpec: {
                  ...
                  categoryId: jobSpec?.deliverySpec?.categoryId || g45JobSpec?.deliverySpec?.categoryId || (contentType === 'news' ? '25' : contentType === 'sports' ? '17' : '24'),
                  scheduledAt: null
                }
              };
            }
            ...
            if (g2TmpPaths.length > 0) {
              ...
              const g2Result = await gate2Worker.run(
                ...
                g2SegLabels  // segment labels for silent segment identification
              );
              await saveGateResult(g2JobSpec.jobId, 'gate2', g2Result);
              emitGateResult(g2JobSpec, g2Result, contentType);
      ...
      const localFiles = [];
      const localFileTypes = []; // Fix 6b: parallel array tracks segType for each entry in localFiles
          ...
          if (seg.localCache && fs.existsSync(seg.localCache)) {
            ...
            if (cacheSize > 10000) {
              ...
              try {
                ...
                localFileTypes.push(segType); // Fix 6b: track type alongside file
      ...
      if (isShortForm) {
        ...
        await new Promise((res, rej) => {
          const args = [
            '-i', avatarConcatPath,
            // Fix 7: Portrait avatar from HeyGen is 720×1280. Scale to fill 1080 wide,
            // then crop height to 960px from top — keeps Bobby G's face/chest visible.
            // scale=1080:1920 then crop=1080:960 (top 960px of 1920)
            '-vf', 'scale=1080:960:force_original_aspect_ratio=increase,crop=1080:960',
            ...ffmpegEncodeArgs(true),
            '-c:a', 'aac', '-ar', '44100', '-ac', '2',
            '-y', avatarHalfPath
        ...
        if (jobSpecCaption && captionText) {
          const baseTypeForCaption = (contentType || 'news').replace(/-short$/, '');
          ...
          if (!captionStyle) {
            ...
            captionStyle = {
              ...
              fontcolor:  jobSpecColor || '#c7af4f',
              boxcolor:   `${jobSpecColor || '#000000'}@${jobSpecCaption.boxOpacity || 0.75}`,
              boxborderw: jobSpecCaption.boxBorderW || 18,
              position:   jobSpecCaption.position || 'y=920'
            };
          } else if (jobSpecColor) {
            // Override color from jobSpec (captionStyle may have stale color from script_gen)
            captionStyle = { ...captionStyle, fontcolor: jobSpecColor };
          }
        }
        ...
        if (captionText && captionStyle) {
          ...
          let dtX, dtY;
          if (captionStyle.position === 'y=920' || captionStyle.position === 'above-split') {
      ...
      for (let i = 0; i < localFiles.length; i++) {
        ...
        // ── Universal chrome burn — reads from job spec, no contentType branching ──
        // Fires for ALL avatar segments across all content types (twitch, news, nba).
        // Directive path (USE_DIRECTIVE_CHROME) is checked first for jobs that have
        // a scene sidecar directive loaded; falls back to the universal spec-driven path.
        // chromeBurnJobSpec was loaded above before this loop using the same lookup pattern
        // as Gate 2/3 (reqJobSpecId || assemblyJobId).

        // Directive path (or any type with a sidecar directive)
        let _directiveHandled = false;
        if (USE_DIRECTIVE_CHROME && assemblyJobId && hasDirectiveForJob(assemblyJobId) && (segTypes[i] || 'avatar') === 'avatar') {
          try {
        ...
        if (!_directiveHandled && (segTypes[i] || 'avatar') === 'avatar') {
          try {
            // Get item data from job spec — authoritative source set by scaffold.
            // Falls back to legacy per-contentType sidebar construction when spec items are absent.
            const specItems = chromeBurnJobSpec?.designSpec?.sceneStructure?.items || [];
            ...
            // Find which item is active based on scene label
            let activeIdx = 0;
            if (specItems.length > 0) {
              ...
              // Legacy fallback: derive activeIdx from contentType-specific label patterns
              if (contentType === 'twitch') {
                const namePart = label
                  .replace(/[_ ](INTRO|REACT|REACTION|CLIP|OUTRO|COLD_OPEN|SETUP|SUMMARY).*$/i, '')
                  .trim().toLowerCase().replace(/_/g, ' ');
                const found = streamerRoster.findIndex(s =>
                ...
                activeIdx = gameNumMatch ? Math.max(0, parseInt(gameNumMatch[1], 10) - 1) : 0;
              } else {
                // news: use STORY# match
                const storyMatch = label.match(/STORY(\d+)/i);
                activeIdx = storyMatch ? Math.max(0, parseInt(storyMatch[1], 10) - 1) : 0;
              }
            }

            // Build allStories for sidebar — from job spec items or legacy per-contentType fallback
            let allStories;
            let overlayTitle;
            let activeCategory;
            ...
            if (specItems.length > 0) {
              allStories = specItems.map((item, idx) => ({
                title:    item.label || item.data?.displayName || `Item ${idx + 1}`,
                category: item.category || chromeSkin.toUpperCase(),
                storyId:  item.sceneId || `item_${idx}`,
                fact:     item.data?.fact || item.data?.matchup || ''
              }));
              const activeItem = specItems[activeIdx] || {};
              overlayTitle = activeItem.label || activeItem.data?.displayName || label || 'CONTENT';
              activeCategory = activeItem.category || chromeSkin.toUpperCase();
            } else if (contentType === 'twitch') {
              // Prefer cardData on INTRO segments (synth tests + live pipeline both carry this)
              const allTwitchIntros = segsToProcess.filter(s => /_INTRO$/i.test(s.label || '') && s.cardData);
              if (allTwitchIntros.length > 0) {
                allStories = allTwitchIntros.map((introSeg, idx) => ({
                  title:    introSeg.cardData.displayName || introSeg.cardData.title || `Streamer ${idx + 1}`,
                  category: 'ON STREAM',
                  storyId:  introSeg.cardData.storyId || `streamer_${idx}`,
                  fact:     [introSeg.cardData.origin, introSeg.cardData.fact].filter(Boolean).join(' · ').slice(0, 60)
                }));
              } else if (streamerRoster.length > 0) {
                allStories = streamerRoster.map((s, idx) => {
                  const parts = [s.origin, s.fact].filter(Boolean);
                  return { title: s.displayName || s.name || `Streamer ${idx + 1}`, category: 'ON STREAM', storyId: `streamer_${idx}`, fact: parts.join(' · ').slice(0, 60) };
                });
              } else {
                allStories = [{ title: 'STREAMER', category: 'ON STREAM', storyId: 'streamer_0', fact: '' }];
              }
              const _curTwitchSeg = segsToProcess.find((s, si) => localFiles[i].includes(`${asmId}_${si}_`));
              overlayTitle = _curTwitchSeg?.cardData?.displayName || _curTwitchSeg?.cardData?.title || allStories[activeIdx]?.title || 'STREAMER';
              activeCategory = 'ON STREAM';
            } else if (contentType === 'nba') {
              // Match both legacy GAME1_INTRO and universal ITEM1_INTRO naming
              const allNbaIntros = segsToProcess.filter(s => (/GAME\d+[_ ]INTRO/i.test(s.label || '') || /ITEM\d+[_ ]INTRO/i.test(s.label || '')) && s.cardData);
              allStories = allNbaIntros.length > 0
                ...
                : [{ title: 'NBA Highlights', category: 'NBA GAME', storyId: 'game_0' }];
              const _nbaSeg = segsToProcess.find((s, si) => localFiles[i].includes(`${asmId}_${si}_`));
              const _nbaCardData = _nbaSeg?.cardData || {};
              overlayTitle = _nbaCardData.title || _nbaCardData.matchup || allStories[activeIdx]?.title || 'NBA Highlights';
              activeIdx = Math.max(0, Math.min(activeIdx, allStories.length - 1));
              activeCategory = 'NBA GAME';
            } else {
              // news legacy fallback
              const allNewsIntros = segsToProcess.filter(s => {
                const lbl = s.label || '';
                return (/STORY\d+_INTRO/i.test(lbl) || /\(INTRO\)/i.test(lbl)) && s.cardData;
              });
              allStories = allNewsIntros.length > 0
                ? allNewsIntros.map((introSeg, idx) => ({
                ...
                : [{ title: 'CONTENT', category: 'WORLD NEWS', storyId: 'story_0' }];
              const _newsSeg = segsToProcess.find((s, si) => localFiles[i].includes(`${asmId}_${si}_`));
              const _newsCardData = _newsSeg?.cardData || {};
              const currentStoryId = _newsCardData.storyId || _newsCardData.title;
              const storyIndex = allStories.findIndex(s => s.storyId === currentStoryId || s.title === _newsCardData.title);
              activeIdx = storyIndex >= 0 ? storyIndex : Math.max(0, Math.min(activeIdx, allStories.length - 1));
              overlayTitle = _newsCardData.title || 'Breaking News Story';
              activeCategory = (_newsCardData.category && _newsCardData.category !== _newsCardData.source) ? _newsCardData.category : 'WORLD NEWS';
            }

            // ── FFmpeg chrome burn (CHROME_OVERLAY_FFMPEG_SPEC.md) ────────────────
            // No Puppeteer, no PNG, no localhost dependency.
            // Two-state timed sidebar hide was a Puppeteer animation workaround — removed.
            // Both intro and body scenes use single buildAndBurnChrome() call.

            // Resolve chromeCfg from customerConfig
            ...
            // chromeSkin maps: twitch→clips, nba→sports, news→news
            const _ctForChrome = chromeSkin === 'twitch' ? 'clips' : chromeSkin === 'nba' ? 'sports' : chromeSkin;
            const _chromeCfg   = resolveChromeCfg(_custConfig, _ctForChrome);

            // Category label from config
            const _categoryLabel = _chromeCfg.flag.categoryLabel[_ctForChrome]
              || activeCategory
              || chromeSkin.toUpperCase();

            // Sidebar items — truncated to maxItems
            const _sidebarItems = allStories.slice(0, _chromeCfg.sidebar.maxItems).map(s => ({
              title:    s.title || '',
              category: s.category || _categoryLabel
            }));

            // Date string for top bar
            const _dateStr = new Date().toLocaleDateString('en-US', {
              month: 'long', day: 'numeric', year: 'numeric'
            }).toUpperCase();

            // Show name from config (contentTypeOverride already merged)
            const _showName = _chromeCfg.name || 'CLIPZWORLD NEWS';

            const chromeParams = {
              showFlag:      true,  // always on for avatar scenes that passed the isColdOpen/isOutro guard above
              showSidebar:   true,
              episodeNumber,
              showName:      _showName,
              dateStr:       _dateStr,
              flagCategory:  _categoryLabel,
              flagTitle:     overlayTitle || allStories[activeIdx]?.title || 'CONTENT',
              sidebarItems:  _sidebarItems,
              activeIdx,
              contentType:   _ctForChrome
            };

            const burnedPath = inputForTS.replace('.mp4', '_chrome.mp4');
...
// Gap #45: OUTRO freeze-hold -- clone last frame for 0.75s (long-form only)
        ...
        try {
          await new Promise((res, rej) => {
            ...
            // Change 2: source crop filter — read from job spec or fall back to contentType heuristic
            const sourceCropFilter = chromeBurnJobSpec?.designSpec?.assembly?.sourceCropFilter
              || (contentType === 'twitch' ? 'crop=1880:1040,' : 'crop=1920:1080,');
            const sourceCropSize = sourceCropFilter;

            // Change 3: Al Jazeera watermark box — read from job spec or fall back (news-specific)
            const _watermarkBox = chromeBurnJobSpec?.designSpec?.assembly?.watermarkBox;
            const watermarkFilter = _watermarkBox
            ...
            // Pillarbox detection: AJ portrait clips (608×1080) must be padded, not stretched.
            // seg.pillarboxFilter is set by news_source when sourceOrientation === 'portrait'.
            // Fall back to building the filter string if only sourceOrientation is flagged.
            const _currentSeg = !isAvatarSeg
              ? segsToProcess.find((s, si) => localFiles[i].includes(`${asmId}_${si}_`))
              : null;
            const _needsPillarbox = !isAvatarSeg && (
              _currentSeg?.pillarboxFilter ||
              _currentSeg?.sourceOrientation === 'portrait'
            );
            ...
            const vfFilter = isAvatarSeg
              ...
              : _needsPillarbox
                // Portrait source clip (e.g. AJ 608×1080): pillarbox to 1920×1080 with black bars
            ...
            // Change 4: clip trim params — read from job spec or fall back to news heuristic
            const clipIntroSkipSecs = chromeBurnJobSpec?.designSpec?.assembly?.clipIntroSkipSecs ?? (contentType === 'news' ? 5 : 0);
            const clipMaxSecs = chromeBurnJobSpec?.designSpec?.assembly?.clipMaxSecs ?? (contentType === 'news' ? 25 : null);

            // ── Fix 10: News source clips — silencedetect trim + hard cap ──
            ...
            const buildTsArgs = async () => {
              ...
              if (clipIntroSkipSecs > 0 && !isAvatarSeg) {
                // Red 3: skip intro branding cards (first clipIntroSkipSecs of every clip)
                // -ss BEFORE -i = fast-seek (keyframe-accurate, no decode overhead)
                // Effective clip window: clipIntroSkipSecs..clipMaxSecs of source
                try {
                  ...
                  if (silenceTrimDur && silenceTrimDur > 0 && silenceTrimDur < NEWS_CLIP_MAX_SECONDS) {
                    ...
                    log(asmId, `  ✂️  Clip ${path.basename(inputForTS)}: skipping ${clipIntroSkipSecs}s intro, trimming to ${finalTrim.toFixed(1)}s (silencedetect, below ${NEWS_CLIP_MAX_SECONDS}s cap)`);
                  } else {
                    finalTrim = NEWS_CLIP_MAX_SECONDS;
                    log(asmId, `  ✂️  Clip ${path.basename(inputForTS)}: skipping ${clipIntroSkipSecs}s intro, capping at ${NEWS_CLIP_MAX_SECONDS}s hard (silencedetect returned ${silenceTrimDur || 'null'})`);
                  }
                  return ['-ss', String(clipIntroSkipSecs), '-i', inputForTS, '-t', finalTrim.toFixed(3), ...baseArgs];
                } catch(trimErr) {
                  log(asmId, `  ⚠️  Clip trim failed (non-fatal): ${trimErr.message} — skipping ${clipIntroSkipSecs}s intro, capping at ${NEWS_CLIP_MAX_SECONDS}s`);
      ...
      // Change 5: read mixMode from job spec; fall back to contentType === 'nba' for Customer 0
      const voiceoverMode = chromeBurnJobSpec?.designSpec?.audio?.mixMode === 'voiceover'
        || contentType === 'nba'; // fallback until c0.json has this
      if (voiceoverMode && tsFiles.length > 0) {
        log(asmId, `  🎙 NBA voiceover V2 — amix: Bobby G narration + music bed over muted clip...`);

        // Pick a random background music track from assets/audio/
        const audioDir = path.join(__dirname, '..', 'assets', 'audio');
        let musicTrack = null;
        try {
          const tracks = fs.readdirSync(audioDir).filter(f => /\.(mp3|wav|m4a)$/i.test(f));
          if (tracks.length > 0) {
            musicTrack = path.join(audioDir, tracks[Math.floor(Math.random() * tracks.length)]);
            log(asmId, `  🎵 Music bed: ${path.basename(musicTrack)}`);
          }
        } catch(e) { /* no music dir — continue without */ }
        ...
        for (let i = 0; i < tsFiles.length - 1; i++) {
          ...
          // avatar followed by source_clip: mix Bobby G audio over clip video
          if (currType === 'avatar' && nextType === 'source_clip') {
            ...
            // Get clip duration for explicit -t (no -shortest truncation)
            let clipDuration = 0;
            try {
              clipDuration = await new Promise((res) => {
                execFile(ffprobePath(), ['-v','quiet','-show_entries','format=duration','-of','default=noprint_wrappers=1:nokey=1', clipTs],
                  {timeout:10000}, (err, stdout) => res(parseFloat(stdout)||0));
              });
            } catch(e) { /* use 0 = ffmpeg decides */ }

            try {
              await new Promise((res, rej) => {
                // V2: 3-input amix
                // Input 0: clip (video used, audio muted via volume=0)
                // Input 1: Bobby G narration at full volume (1.0)
                // Input 2: music bed at 0.20 volume, looped to cover clip duration
                const inputs = ['-i', clipTs, '-i', avatarTs];
                ...
                if (musicTrack) {
                  ...
                  // No music — Bobby G narration only, clip muted
                  filterComplex = '[0:a]volume=0[muted];[1:a]apad[narration];[muted][narration]amix=inputs=2:duration=first:dropout_transition=0[aout]';
                  audioMap = '[aout]';
                }
                ...
                proc.on('close', code => {
                  if (code === 0) {
                    voiceoverFiles[i]   = null;    // avatar video dropped (audio used)
                    voiceoverFiles[i+1] = mixedTs; // clip replaced with voiced version
                    log(asmId, `  🎙 Voiced clip: ${path.basename(mixedTs)} (${clipDuration.toFixed(1)}s)`);
                    res();
                  } else {
                    rej(new Error(`Voiceover V2 mix failed: ${code}`));
                  }
                });
                proc.on('error', rej);
              });
            } catch(e) {
              log(asmId, `  ⚠️  Voiceover V2 failed (non-fatal): ${e.message} — using original`);
            }
          }
        }

        // Rebuild tsFiles — remove nulled avatar segments (their audio was used)
        ...
        log(asmId, `  ✅ NBA voiceover V2 complete — ${tsFiles.length} segments (Bobby G in ${voicedTypes.filter(t=>t==='avatar').length} scenes)`);
      }
      ...
      try {
        if (outPath && fs.existsSync(outPath)) {
          ...
          if (gate3aWorker.canProduce(g3JobSpecWithPath).ready) {
            ...
            emitGateResult(g3JobSpec, gwG3aResult, contentType);
                ...
                if (gwG3bResult && gwG3bResult.outcome === 'mismatch_fixable' && gwG3bResult.mismatches?.length > 0) {
                  ...
                  if (chromeRelated) {
                    ...
                    try {
                      ...
                      const _chromeCfgReBurn = resolveChromeCfg(custConfig, _ctForReBurn);
                      ...
                      const correctedVideoPath = outPath.replace('.mp4', '_g3b_corrected.mp4');
        ...
        if (driveUrl) {
          ...
          try {
            ...
            if (gate4Worker.canProduce(g45JobSpecWithPath).ready) {
              ...
              emitGateResult(g45JobSpec, g4Result, contentType);
          ...
          // Gate 6 removed — publish handled by Gate 4 → Gate 5 above
