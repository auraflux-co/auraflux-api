      ...
      for (let i = 0; i < localFiles.length; i++) {
        ...
        if (!_directiveHandled && (segTypes[i] || 'avatar') === 'avatar') {
          try {
            ...
            const introDur = CONFIG.INTRO_CARD.DURATION_TWITCH; // default 10s
            const burnedPath = inputForTS.replace('.mp4', '_chrome.mp4');

            if (isIntro) {
              // Two-state: lower-third visible, sidebar hidden for first introDur, then sidebar shown
              const overlayVisiblePath = path.join(TMP_DIR, `chrome_vis_${Date.now()}.png`);
              const overlayHiddenPath  = path.join(TMP_DIR, `chrome_hid_${Date.now()}.png`);

              await generateNewscastOverlay(overlayBase, overlayVisiblePath, activeIdx, {
                showLowerThird: true, hideSidebar: true,  episodeNumber, activeCategory, contentType: chromeSkin
              });
              await generateNewscastOverlay(overlayBase, overlayHiddenPath, activeIdx, {
                showLowerThird: true, hideSidebar: false, episodeNumber, activeCategory, contentType: chromeSkin
              });

              const burnArgs = [
                '-i', inputForTS, '-i', overlayVisiblePath, '-i', overlayHiddenPath,
                '-filter_complex',
                `[0:v][1:v]overlay=0:0:enable='lte(t,${introDur})'[mid];[mid][2:v]overlay=0:0:enable='gt(t,${introDur})'[out]`,
                '-map', '[out]', '-map', '0:a',
                ...ffmpegEncodeArgs(true),
                '-pix_fmt', 'yuv420p', '-c:a', 'aac', '-ar', '44100', '-y', burnedPath
              ];
              ...
              await new Promise((res, rej) => {
                ...
                proc.on('close', code => {
                  if (code === 0) res();
                    ...
                    rej(new Error(`Chrome two-state burn failed: ${code} — ${reason}`));
                  }
                });
                proc.on('error', rej);
              });
              ...
              // Single-state: sidebar always visible
              const overlayBodyPath = path.join(TMP_DIR, `chrome_body_${Date.now()}.png`);
              ...
              const burnArgs = [
                '-i', inputForTS, '-i', overlayBodyPath,
              ...
              await new Promise((res, rej) => {
                ...
                proc.on('close', code => {
                  if (code === 0) res();
                    ...
                    const reason = burnStderr.slice(-300).replace(/\n/g, ' ').trim();
                    rej(new Error(`Chrome body burn failed: ${code} — ${reason}`));
                  }
                });
                proc.on('error', rej);
              });
            ...
            if (fs.existsSync(burnedPath) && fs.statSync(burnedPath).size > 10000) {
              ...
              log(asmId, `  🎨 Chrome burned [${activeIdx + 1}/${allStories.length}] skin=${chromeSkin}: ${overlayBase.title}`);
            }
          } catch(e) {
                ...
                if (gwG3bResult && gwG3bResult.outcome === 'mismatch_fixable' && gwG3bResult.mismatches?.length > 0) {
                  ...
                  if (chromeRelated) {
                    ...
                    try {
                      ...
                      // Build correct overlay from job spec designSpec
                      ...
                      const skin = g3JobSpec.designSpec?.chrome?.skin || baseContentType;
                      const episodeNumber = assemblyJobs[asmId]?.episodeNumber || 'Episode 1';

                      // Generate correct full-frame overlay PNG
                      const correctedOverlayPath = path.join(TMP_DIR, `gate3b_corrected_overlay_${asmId}_${Date.now()}.png`);
                      const storyData = {
                        title: 'CORRECTED',
                        category: skin.toUpperCase(),
                        allStories: (assemblyJobs[asmId]?.allStories || [{ title: 'Story', category: skin.toUpperCase(), storyId: 'story_0' }])
                      };

                      await generateNewscastOverlay(storyData, correctedOverlayPath, 0, {
                        showLowerThird: false,
                        hideSidebar: false,
                        episodeNumber,
                        activeCategory: skin.toUpperCase(),
                        contentType: baseContentType
                      });

                      if (fs.existsSync(correctedOverlayPath) && fs.statSync(correctedOverlayPath).size > 1000) {
                        // Burn corrected overlay onto assembled video
                        const correctedVideoPath = outPath.replace('.mp4', '_g3b_corrected.mp4');
                        await new Promise((res, rej) => {
                          const args = [
                            '-i', outPath,
                            '-i', correctedOverlayPath,
                            '-filter_complex', '[0:v][1:v]overlay=0:0[vout]',
                            '-map', '[vout]', '-map', '0:a?',
                            '-c:v', 'libx264', '-preset', 'fast', '-crf', '23',
                            '-c:a', 'copy', '-movflags', '+faststart', '-y', correctedVideoPath
                          ];
                          const proc = execFile(ffmpegPath(), args, { maxBuffer: 100 * 1024 * 1024 });
                          proc.on('close', code => code === 0 ? res() : rej(new Error(`Chrome re-burn failed: ${code}`)));
                          proc.on('error', rej);
                        });
                        ...
                        try { fs.unlinkSync(correctedOverlayPath); } catch(e) {}
                      }
                    } catch(reburnErr) {
