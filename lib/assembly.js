  ...
  const run = async () => {
    ...
    try {
      ...
      if (isShortForm) {
        ...
        if (isClipsOnlyComp) {
          ...
          if (portraitClips.length === 1) {
            ...
            await concatMp4Files(portraitClips, clipsOnlyCompPath, true, 'comp_concat');
          }
          const compDur = await probeDuration(clipsOnlyCompPath);
          log(asmId, `  🎠 Comp assembled: ${compDur.toFixed(1)}s${compDur > 180 ? ' ⚠️ exceeds 180s YouTube Shorts limit' : ''}`);
        }
      ...
      try {
        ...
        await applyPostProcessing({ outPath, isShortForm, skipCaptionsIfSourceCaptioned: clipsOnlyCompJob, asmId, log });
      } catch (ppErr) {
