  ...
  const run = async () => {
    ...
    try {
      ...
      if (assemblyJobs[asmId].qaOutcome !== 'fail' && outPath) {
        ...
        // ── Inject chapter timestamps into publish copy description ──────────────
        if (chaptersString && assemblyJobs[asmId]?.publishCopy?.youtube) {
          try {
        ...
        try {
          ...
          if (!gate3To4Ready) {
            ...
            if (g4Result.uploadSignal && g4Handoff.passed && _schedAt && new Date(_schedAt).getTime() > Date.now()) {
              log(asmId, `⏰ Gate 5 native schedule for ${_schedAt} (CPD-924)`);
              const g5PreflightSched = preflightGateExecution({ jobId: g45JobSpec.jobId, gate: 'gate5', fallbackJobSpec: g45JobSpecWithPath });
              const g5RunSpecSched = {
                ...(g5PreflightSched.jobSpec || g45JobSpecWithPath),
                assembledPath: outPath,
                outputPath: outPath,
                deliverySpec: {
                  ...(g45JobSpec.deliverySpec || {}),
                  scheduledAt: _schedAt,
                },
              };
              if (gate5Worker.canProduce(g5RunSpecSched).ready) {
                const g5Result = await gate5Worker.run(g5RunSpecSched, g4Result);
                try { await saveGR45(g45JobSpec.jobId, 'gate5', g5Result); } catch (e) {}
                emitGateResult(g45JobSpec, g5Result, contentType);
                log(asmId, `📋 Gate 5 scheduled: ${g5Result.passed ? 'PASS' : 'FAIL'}${g5Result.nativeScheduled ? ' (native platform schedule)' : ''}`);
                if (_schedLiveCard) {
                  _schedLiveCard.stage = g5Result.passed ? 'publish_scheduled' : 'gate5_failed';
                  _schedLiveCard.nativeScheduledPublish = !!g5Result.nativeScheduled;
                  _schedLiveCard.driveUrl = driveUrl;
                  _schedLiveCard.state = _schedLiveCard.state || {};
                  _schedLiveCard.state.savedOutputs = { ..._schedLiveCard.state.savedOutputs, driveUrl };
                }
              } else {
                log(asmId, `⚠️  Gate 5 schedule skipped — ${g5PreflightSched.reasons?.join('; ') || 'not ready'}`);
                if (_schedLiveCard) _schedLiveCard.stage = 'publish_scheduled';
              }
              try {
                pipelineBus.emit('gate5:scheduled_hold', {
                  ...
                  publishCopy: assemblyJobs[asmId]?.publishCopy || null,
                  thumbnailDriveUrl: assemblyJobs[asmId]?.thumbnailDriveUrl || null,
                  nativeScheduledPublish: !!(_schedLiveCard?.nativeScheduledPublish),
                });
              } catch (_e) { /* non-fatal */ }
