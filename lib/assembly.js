    ...
    try {
      ...
      if (isShortForm) {
        ...
        await new Promise((res, rej) => {
          const args = [
            '-i', avatarConcatPath,
            // Avatar is a portrait head/bust shot — scale to 70% of half-height so head
            // sits comfortably with breathing room. Center horizontally, pad with black.
            // Target: ~540px wide × 672px tall, centered in 1080×960 canvas.
            '-vf', 'scale=-2:672,pad=1080:960:(ow-iw)/2:(oh-ih)/2:color=black',
            ...ffmpegEncodeArgs(true),
            '-c:a', 'aac', '-ar', '44100', '-ac', '2',
            '-y', avatarHalfPath
          ];
