      ...
      if (isShortForm) {
        ...
        if (isClipsOnlyComp) {
          ...
          for (let ci = 0; ci < clipFiles.length; ci++) {
            ...
            const hookTitle = hookItems[ci]?.title || hookItems[ci]?.headline || hookItems[ci]?.clipTitle || '';
            let segPath = pc;
            if (hookTitle) {
        ...
        try {
          ...
          if (!gate3To4Ready) {
            ...
            if (g4Result.uploadSignal && g4Handoff.passed && gate5Worker.canProduce(g45JobSpecWithPath).ready) {
              const { validatePublishMetadata, reviewHoldEnabled } = require('./gates/metadata_qa');
              const _pc = assemblyJobs[asmId]?.publishCopy || g45JobSpecWithPath.state?.savedOutputs?.publishCopy || {};
              const _yt = _pc.youtube || _pc.platforms?.youtube || _pc;
              const _metaCheck = validatePublishMetadata(g45JobSpecWithPath, {
                title: _yt.bestTitle?.title || _yt.title || (_yt.titles && _yt.titles[0]) || '',
                description: _yt.description || '',
                tags: _yt.tags || [],
              });
