              ...
              if (g2Result.outcome === 'rerender_needed' &&
                ...
                for (const badSeg of badSegments) {
                  ...
                  try {
                    ...
                    // v3 API (migrated from legacy v2 2026-06-09) — <break> tags work natively in `script`
                    const reRenderBody = {
                      type: 'avatar',
                      ...
                      voice_settings: { speed: parseFloat(process.env.HEYGEN_SPEAK_SPEED || '0.85') },
                      resolution: '1080p',
                      aspect_ratio: isPortrait ? '9:16' : '16:9',
                      engine: { type: process.env.HEYGEN_ENGINE || 'avatar_v' }
                    };

                    const submitResp = await axios.post('https://api.heygen.com/v3/videos', reRenderBody,
                    ...
                    if (!newVideoId) throw new Error('No video_id returned from HeyGen re-render');
                    ...
                    for (let attempt = 0; attempt < 18; attempt++) {
                      ...
                      const statusResp = await axios.get(`https://api.heygen.com/v3/videos/${newVideoId}`,
                        { headers: { 'X-Api-Key': HEYGEN_API_KEY }, timeout: 10000 });
                      const status = statusResp.data?.data?.status;
                      if (status === 'completed') { newVideoUrl = statusResp.data.data.video_url; break; }
                      if (status === 'failed') throw new Error(`HeyGen re-render failed for ${badSeg.label}: ${statusResp.data?.data?.failure_message || 'unknown'}`);
                    }

                    if (!newVideoUrl) throw new Error(`Re-render timed out for ${badSeg.label}`);
        ...
        try {
          ...
          if (!gate3To4Ready) {
            ...
            if (g4Result.uploadSignal && g4Handoff.passed && _schedAt && new Date(_schedAt).getTime() > Date.now()) {
              log(asmId, `⏰ Gate 5 held — publish scheduled for ${_schedAt} (CPD-924)`);
              if (_schedLiveCard) {
              ...
              // CPD-936: the mutations above are in-memory only — a restart during the
              // hold window stranded the job (cron fired with no driveUrl). Ask server.js
              // to persist the card to jobs.json + SQLite.
              try {
