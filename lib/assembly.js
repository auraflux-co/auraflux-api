...
function groupSegmentsByLabel(segsToProcess) {
  ...
  if (!hasPrefix) {
    // Strategy B: group by count (4 per group)
    const groups = [];
    const CHUNK = 4;
...
async function handleAssemble(req, res, saveJobCard) {
  ...
  if (assemblyJobId) {
    try {
      ...
      if (applied.overrideCount > 0) {
        ...
        console.log(`[assemble] manual_segments: ${applied.overrideCount} local file(s) from ${applied.dir}`);
      }
    } catch (e) {
  ...
  const run = async () => {
    ...
    try {
      ...
      const durations = [];
      for (const f of localFiles.filter(Boolean)) {
      ...
      // ── Studio laugh segments (Talk Soup): freeze reaction frame + audience track ──
      if (contentType === 'twitch' || String(contentType).includes('twitch')) {
        try {
          ...
          const laughCustomerId = chromeBurnJobSpec?.customerId || 'c0';
          if (isStudioLaughEnabled(laughCustomerId)) {
            const laughCfg = getStudioLaughConfig(laughCustomerId) || {};
            for (let i = 0; i < segsToProcess.length; i++) {
              if (segsToProcess[i].type !== 'studio_laughter') continue;
              let holdIdx = i - 1;
              while (holdIdx >= 0 && !localFiles[holdIdx]) holdIdx--;
              if (holdIdx < 0 || !localFiles[holdIdx]) {
                log(asmId, `⚠️ Studio laugh skipped — no hold video for ${segsToProcess[i].label}`);
                continue;
              }
              const laughPick = segsToProcess[i].laughPickIndex ?? i;
              const laughAudio = await ensureStudioLaughAudio(laughCustomerId, laughPick);
              const laughLabel = segsToProcess[i].label || `laugh_${i}`;
              const laughOut = path.join(TMP_DIR, `${asmId}_${i}_${laughLabel.toLowerCase().replace(/[^a-z0-9]+/g, '_').slice(0, 40)}.mp4`);
              await buildStudioLaughSegment(localFiles[holdIdx], laughOut, laughAudio, {
                maxDurationSec: laughCfg.maxDurationSec || 4.5,
                volume: laughCfg.volume || 0.85
              });
              localFiles[i] = laughOut;
              localFileTypes[i] = 'avatar';
              localFileMeta[i] = segsToProcess[i];
              log(asmId, `🎭 Studio laugh segment ready: ${path.basename(laughOut)} (${path.basename(laughAudio)})`);
            }
          }
        } catch (laughErr) {
          log(asmId, `⚠️ Studio laugh render skipped: ${laughErr.message}`);
        }
      }
      ...
      const processSegmentToTs = async (inputFilePath, segIdx, localSegIdx) => {
        ...
        const { assemblyCropEnabled, assemblyCropFilter } = require('./avatar/echomimic_post');
        const echoAvatarCrop = isAvatarSeg
          && (process.env.AVATAR_ENGINE === 'echomimic' || process.env.ECHOMIMIC_RENDER_MODE === 'pod')
          && assemblyCropEnabled();
        const echoCropPrefix = echoAvatarCrop ? `${assemblyCropFilter()},` : '';
        ...
        const buildTsArgs = async () => {
          ...
          // without -t, aresample async preserves that overhang per segment and ~57 avatar
        ...
        try {
          ...
          if (bookCfg.coldOpen.enabled && _bookCard && coldOpenApproved(_bookCard)) {
            try {
              ...
              const musicPath = resolveMusicBed(bookCfg.coldOpen.musicBedKey);
      ...
      if (assemblyJobs[asmId].qaOutcome !== 'fail' && outPath) {
        ...
        const _publishItems = (req.body.items && req.body.items.length)
          ? req.body.items
          : (chromeBurnJobSpec?.order?.inputs?.items || []);
        if (_reuseGate1Copy) {
          ...
          try {
            ...
            if (publishCopyResp.data && !publishCopyResp.data.error) {
              ...
              const _streamersForSeo = req.body.streamers || chromeBurnJobSpec?.streamers || [];
        ...
        // ── Twitch Soup credits outro (after SEO + chapters — scrolls YT description) ─
        if (_isTwitchLong && assemblyJobId && outPath && fs.existsSync(outPath)) {
          try {
            const { appendCreditsOutroToVideo } = require('./twitch_bookends');
            ...
            if (_creditsResult.appended) {
              ...
              try {
                const { uploadToR2 } = require('./storage');
                const _reUrl = await uploadToR2(outPath, outFile, { folder: `outputs/${assemblyJobId || asmId}` });
                if (_reUrl) {
                  assemblyJobs[asmId].driveUrl = _reUrl;
                  const _live = global.persistedJobsRef?.[assemblyJobId];
                  if (_live) {
                    _live.driveUrl = _reUrl;
                    _live.state = _live.state || {};
                    _live.state.savedOutputs = { ..._live.state.savedOutputs, driveUrl: _reUrl };
                  }
                  log(asmId, `  ☁️  R2 re-uploaded with credits outro`);
                }
              } catch (_ru) {
                log(asmId, `  ⚠️  R2 re-upload after credits failed (non-fatal): ${_ru.message}`);
              }
