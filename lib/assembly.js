...
// Gap #45: OUTRO freeze-hold -- clone last frame for 0.75s (long-form only)
      ...
      try {
        ...
        // Build items in the shape buildTemplateData() expects per content type
        const _specItems = reqDesignSpec?.sceneStructure?.items || [];
        const _thumbItems = _specItems.map(item => ({
          title:    item.label || item.data?.displayName || item.data?.title || item.data?.matchup || '',
          teams:    item.data?.matchup || '',         // NBA: matchup string
        ...
        const _thumbJobSpec = {
          ...
          contentType: contentType || 'news',
          order:       { inputs: { items: _thumbItems } },
