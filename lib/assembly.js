        ...
        if (driveUrl) {
          ...
          try {
            ...
            log(asmId, `⚠️  Gate 4/5 error (non-blocking): ${g45Err.message}`);
              ...
              // Generate publish copy (title, description, tags) — required by Gate 5 for Upload-Post
          // Must happen after Drive upload so we have driveUrl + video duration available
          try {
            ...
            if (publishCopyResp.data && !publishCopyResp.data.error) {
              ...
              // Persist to job spec savedOutputs so Gate 5 can find it after restart
              // g45JobSpec not in scope here — use assemblyJobId directly
          ...
          // Generate designed thumbnail — must be AFTER publishCopy so hook text is available (Twitch)
          try {
            const { generateThumbnail: _genThumb } = require('./thumbnail');
            const _specItems2 = reqDesignSpec?.sceneStructure?.items || [];
            const _thumbItems2 = _specItems2.map(item => ({
            ...
            const _thumbJobSpec2 = {
              jobId:       asmId,
              customerId:  'c0',
              templateId:  (contentType?.includes('short') || format === 'portrait') ? 'short-form' : 'long-form',
              contentType: contentType || 'news',
              order:       { inputs: { items: _thumbItems2 } },
              state:       { savedOutputs: { publishCopy: assemblyJobs[asmId]?.publishCopy || null } }
            };
            const thumbResult2 = await _genThumb(_thumbJobSpec2);
            if (thumbResult2.ok && thumbResult2.driveUrl) {
              assemblyJobs[asmId].thumbnailDriveUrl = thumbResult2.driveUrl;
              const { saveOutput: _saveThumbOut2 } = require('./job_spec');
              try { await _saveThumbOut2(asmId, 'thumbnailDriveUrl', thumbResult2.driveUrl); } catch(e) {}
              log(asmId, `🖼  Thumbnail generated and uploaded to Drive: ${thumbResult2.driveUrl.slice(0, 60)}...`);
            } else {
              log(asmId, `⚠️  Thumbnail generation skipped: ${thumbResult2.error || 'unknown'}`);
            }
          } catch(thumbErr2) {
            log(asmId, `⚠️  Thumbnail error (non-fatal): ${thumbErr2.message}`);
          }
