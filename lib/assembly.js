...
// ── Redis ticker cache helpers ────────────────────────────────────────────────
// Redis is additive — in-memory TICKER_CACHE remains the primary fast path.
// Redis provides cross-process persistence and warm cache after server restarts.
// All Redis calls are non-fatal: failures fall through to in-memory / regeneration.
async function getTickerCache(key) {
  try {
    const redis = getRedisConnection();
    if (!redis) return null;
    const val = await redis.get(`ticker:${key}`);
    return val ? JSON.parse(val) : null;
  } catch (e) {
    return null; // fallback: regenerate
  }
}

async function setTickerCache(key, value) {
  try {
    const redis = getRedisConnection();
    if (!redis) return;
    await redis.set(`ticker:${key}`, JSON.stringify(value), 'EX', 3600); // 1hr TTL
  } catch (e) {
    // non-fatal — in-memory TICKER_CACHE still set
  }
}
...
async function captureTicker(contentType) {
  ...
  // Check Redis cache (warm across server restarts)
  const redisCached = await getTickerCache(contentType);
  if (redisCached && redisCached.path && fs.existsSync(redisCached.path)) {
    const age = Date.now() - (redisCached.cachedAt || 0);
    if (age < TICKER_CACHE_TTL) {
      console.log(`[ticker] Using Redis-cached ${contentType} ticker (age: ${Math.round(age/1000/60)}m)`);
      TICKER_CACHE[contentType] = redisCached; // warm the in-memory cache too
      return redisCached.path;
    }
  }
  ...
  try {
    ...
    const cacheEntry = { path: outPath, cachedAt: Date.now() };
    TICKER_CACHE[contentType] = cacheEntry;
    await setTickerCache(contentType, cacheEntry); // persist to Redis for cross-restart warm cache
