...
function wantsDialogueAwareMix(audio = {}) {
  ...
  // No bed → never duck/attenuate source (stale dialogueAwareMix:true from prior presets was ducking songs to "background")
  if (!bed || bed === 'off') return false;
  if (audio.dialogueAwareMix === true) return true;
  return audio.duckSpeech !== false;
}
