...
/** Sidechain params — mix=0 is no duck; mix=1 is full duck. Keep mix low so clip 1 speech does not mute the bed. */
function sidechainBedParams(audio = {}) {
  const duck = audio.duckSpeech !== false;
  if (!duck) return null;
  return {
    threshold: 0.055,
    ratio: 4,
    attack: 12,
    release: 380,
    // mix=0 → unducked bed only; mix=1 → full sidechain. 0.18 = bed stays under constant speech (clip 1).
    mix: 0.18,
    levelSc: 0.88,
  };
}

function computeBedFloorVolume(bedVol) {
  return Math.max(0.006, Number(bedVol) * 0.35);
}
...
function buildCompAudioFilterParts({
  ...
  if (bedInputIdx != null) {
    ...
    const floorVol = computeBedFloorVolume(bedVol);
    filterParts.push(
      `[${bedInputIdx}:a]atrim=0:${dur},asetpts=PTS-STARTPTS[bedsrc]`,
    );
    filterParts.push(
      `[bedsrc]asplit=2[bedduckin][bedfloorin]`,
    );
    filterParts.push(
      `[bedfloorin]volume=${floorVol.toFixed(4)},copy[bedfloor]`,
    );
    filterParts.push(
      `[bedduckin]volume=${bedVol.toFixed(3)},copy[bedvol]`,
    );
    if (duckParams) {
      const sc = duckParams;
      filterParts.push(
        `[bedvol][main]sidechaincompress=threshold=${sc.threshold}:ratio=${sc.ratio}:attack=${sc.attack}:release=${sc.release}:mix=${sc.mix}:level_sc=${sc.levelSc}[bedduck]`,
      );
      mixInputs.push('[bedduck]', '[bedfloor]');
    } else {
      mixInputs.push('[bedvol]');
    }
  }
