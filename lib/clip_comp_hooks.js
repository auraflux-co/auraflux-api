...
async function mergeObservationPasses(ctx, visual, audio) {
...
function buildHookFromObservationPrompt(ctx, observation, { desktop = false, strict = false, fixDirective = null } = {}) {
  ...
  const strictHint = strict
    ? '\nSTRICT: Be hyper-specific. Must NOT match platform title. 3–8 words.'
    : '';
  const fixHint = fixDirective
    ? `\nPREVIOUS HOOK REJECTED BY QA — fix: ${fixDirective}`
    : '';
  return `Write ONE burned-in on-screen hook from this observation (visual + audio beat together).
...
- 3–8 words preferred — moment only, no streamer name, no colon prefix
- Must reflect VISUAL action AND audio together — not a verbatim quote unless the quote IS the visual beat
...
async function analyzeClipObservationMultiPass(enriched, item, { log } = {}) {
  ...
  try {
    ...
    if (passes >= 3) {
      ...
      return mergeObservationPasses(ctx, visual, audio);
    }
...
async function produceHookWithQa(ctx, observation, junkOpts, { log, clipIndex = 0 } = {}) {
  ...
  for (let attempt = 0; attempt <= maxRetries; attempt++) {
    ...
    const rawHook = await writeHookFromObservation(ctx, observation, {
      strict: attempt > 0,
      fixDirective,
    });
    ...
    if (isJunkHook(hook, junkOpts)) {
      ...
      fixDirective = 'Write a specific moment hook — not the platform title, not streamer name, 3–8 words.';
      continue;
    }
    ...
    if (hookQa.passed) {
      if (log) log(`  ✅ Hook QA pass (${hookQa.score}): "${hook}"`);
      return { hook, hookQa };
    }

    if (log) log(`  ⚠️ Claude hook QA fail (${hookQa.score}): ${(hookQa.violations || []).slice(0, 2).join('; ')}`);
    fixDirective = hookQa.fixDirective
      || 'Rewrite as a specific visual+audio beat — TV-clean, no title copy, no streamer name.';
  }
...
async function analyzeClipMomentBundle(clip, item, { log, clipIndex = 0 } = {}) {
  ...
  try {
    ...
    if (isJunkHook(hook, junkOpts) || (hookQa && !hookQa.passed)) {
...
async function pickLeadStreamerAndTitle(clipEntries, { isComp = false, log } = {}) {
...
{"leadClipIndex":0,"leadStreamer":"name","leadTitleDraft":"...","leadReason":"one sentence"}`;
  ...
  try {
    ...
    const parsed = parseGeminiJsonBlock(raw);
...
async function generateClipCompCreativeBrief(clips, items, opts = {}) {
  ...
  const lead = await pickLeadStreamerAndTitle(clipEntries, { isComp, log });
  return {
    clipCount: count,
    isComp,
...
async function resolveHooksForAssembly({ hookItems = [], hookClips = [], preGenerated = null, preBrief = null, log, forceRegenerate = false } = {}) {
  ...
  if (!forceRegenerate && preBrief && hooksAreUsable(preBrief.clips?.map((c) => c.hook) || preGenerated, count)) {
...
function buildClipCompSeoInput(brief) {
