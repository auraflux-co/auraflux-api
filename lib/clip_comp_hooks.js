...
function normalizeHookLine(streamer, raw, clipTitle = '') {
  let line = sanitizeTvClean(sanitizeHookLineGlyphs(raw).replace(/^["']|["']$/g, '').trim());
...
function applyOperatorCustomHook(card, clipIndex, rawHookText) {
  ...
  const clip = orderedClips[idx] || items[idx] || {};
  const item = items[idx] || orderedClips[idx] || {};
  const ctx = buildClipContext(clip, item);
  const hookText = normalizeHookLine(ctx.streamer, rawHookText, ctx.title);
  if (!hookText) {
    return {
      ok: false,
      error: 'Hook rejected — use moment-only text (no streamer name, no Twitch title copy), max 72 chars, TV-clean.',
    };
  }
