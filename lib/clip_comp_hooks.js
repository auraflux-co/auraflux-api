...
function isJunkHook(text, { streamer, clipTitle, maxWords } = {}) {
...
function buildHookFromObservationPrompt(ctx, observation, { desktop = false, strict = false, fixDirective = null } = {}) {
  ...
  return `${buildExamplesBlock({ examples, patterns })}
...
async function produceHookWithQa(ctx, observation, junkOpts, { log, clipIndex = 0 } = {}) {
  ...
  for (let attempt = 0; attempt <= maxRetries; attempt++) {
    ...
    const { candidates, rawCount } = await generateHookCandidates(ctx, observation, { fixDirective });

    if (log && candidates[0]) {
      log(`  📊 Rank #1: "${candidates[0].text}" (tension ${candidates[0].tensionScore || '?'}) — ${(candidates[0].why || '').slice(0, 60)}`);
    }
    ...
    const isUsable = (text) => {
      ...
      return !isJunkHook(normalized, junkOpts);
    };

    const picked = pickFirstUsableCandidate(candidates, isUsable);
    let rawHook = picked?.text || '';
    ...
    if (isJunkHook(hook, junkOpts)) {
      ...
      fixDirective = `Curiosity gap only — ${hookTargetWords()} words ideal, ${hookMaxWords()} max. No punchline spoil, no title copy, no streamer name, pattern interrupt.`;
      continue;
    }
