...
async function produceHookWithQa(ctx, observation, junkOpts, { log, clipIndex = 0 } = {}) {
  ...
  for (let attempt = 0; attempt <= maxRetries; attempt++) {
    ...
    if (isJunkHook(hook, { ...junkOpts, observation })) {
      if (log) log(`  ⚠️ Hook rejected locally ("${hook || rawHook}")`);
      fixDirective = `Specific curiosity gap — ${hookTargetWords()} words ideal, ${hookMaxWords()} max. Name a concrete object/action from the observation. No mood-only filler (chaos, intense face, sudden roar).`;
      continue;
    }
