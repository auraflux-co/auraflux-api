...
const { sanitizeHookLineGlyphs } = require('./hook_emoji');
...
const { applyClipChannelSeoToLead } = require('./clip_channel_seo');
...
/** Strip Gemini JSON tail accidentally embedded in hook text (CPD hook machine parse leak). */
function stripHookJsonArtifacts(raw) {
  let line = String(raw || '').trim();
  if (!line) return '';
  const jsonTail = line.search(/["']\s*,\s*"(?:formula|psychologyTrigger|visualCue|payoffTease|rank|tensionScore)"\s*:/i);
  if (jsonTail > 0) line = line.slice(0, jsonTail).trim();
...
function normalizeHookLine(streamer, raw, clipTitle = '') {
  let line = stripHookJsonArtifacts(raw);
  line = sanitizeTvClean(sanitizeHookLineGlyphs(line).replace(/^["']|["']$/g, '').trim());
...
/** Operator-typed custom hook — saved as entered; Gemini/TV-clean rules do not apply. */
function normalizeOperatorCustomHook(raw) {
  let line = stripHookJsonArtifacts(raw);
  line = sanitizeHookLineGlyphs(String(line || '').replace(/^["']|["']$/g, '').trim());
  line = line.replace(/^\*+|\*+$/g, '').trim();
  return line;
}

function detectClipSourcePlatform(ctx = {}) {
  const { detectUrlPlatform } = require('./publish_seo_context');
  return detectUrlPlatform(ctx.pageUrl || ctx.clipUrl || ctx.url || '');
}

function buildClipContext(clip = {}, item = {}) {
  const pageUrl = clip.pageUrl || item.pageUrl || clip.url || item.url || '';
  const clipUrl = clip.clipUrl || clip.mp4Url || clip.url || item.clipUrl || item.url || '';
  const platform = clip.platform || item.platform || detectClipSourcePlatform({ pageUrl, clipUrl, url: pageUrl });
  return {
...
/** Prefer resolved MP4, platform page (yt-dlp), or YouTube watch URL — never thumbnail preview MP4. */
function resolveHookVideoUrl(enriched = {}) {
  ...
  if (/youtube\.com\/watch|youtu\.be\/|youtube\.com\/shorts\/|youtube\.com\/live\//i.test(pageUrl)) return pageUrl;
  if (/youtube\.com\/watch|youtu\.be\/|youtube\.com\/shorts\/|youtube\.com\/live\//i.test(rawUrl)) return rawUrl;
...
function buildCompHookAnalysisPrompt(ctx = {}) {
  const platform = ctx.platform || detectClipSourcePlatform(ctx);
  const isYoutube = platform === 'youtube';
  const subject = ctx.streamer || 'unknown';
  const desktop = !isYoutube && isDesktopOrIrlStream(ctx);
  const desktopBlock = desktop
    ...
    : isYoutube
      ? `YouTube long-form or performance clip — describe the actual performance, setting, crowd/stage, and standout moment. This is NOT a Twitch live stream.`
      : `Gameplay clip — describe visible action AND reactions together.`;

  const sourceLabel = isYoutube ? 'YouTube video' : 'Twitch clip';
  return `${sourceLabel} featuring "${subject}". Category: ${ctx.game || 'unknown'}.
...
1. VISUAL: who/what on screen, body language, setting, stage/audience, key action
2. AUDIO: tone, reaction sounds, music/performance, short quote ONLY if speech is central
3. COMBINED BEAT: the single funniest, surprising, or emotional moment (visual + audio together)

Do NOT write a hook yet. Do NOT call this a Twitch stream unless it is clearly live Twitch gameplay. No hype. No talent name in the observation.`;
}
...
async function enrichClipForHook(clip = {}, item = {}) {
  ...
  if (merged.postLiveVod && merged.trimStart != null && merged.trimEnd != null) {
    const pageUrl = merged.pageUrl || merged.url || merged.clipUrl || '';
    const vidMatch = String(pageUrl).match(/(?:v=|youtu\.be\/)([A-Za-z0-9_-]{6,})/);
    if (vidMatch) {
      try {
        const { getOrExtractPreviewMp4 } = require('./post_live/vod_preview');
        const preview = await getOrExtractPreviewMp4({
          videoId: vidMatch[1],
          vodUrl: pageUrl,
          start_s: Number(merged.trimStart) || 0,
          end_s: Number(merged.trimEnd) || (Number(merged.trimStart) || 0) + 30,
        });
        if (preview?.previewUrl) {
          const path = require('path');
          const localPath = path.join(__dirname, '..', 'tmp', path.basename(preview.previewUrl));
          merged.url = localPath;
          merged.clipUrl = localPath;
          merged.mp4Url = localPath;
          merged._postLivePreview = true;
        }
      } catch (_e) { /* yt-dlp preview optional — title fallback still works */ }
    }
    return merged;
  }
...
async function analyzeClipObservation(enriched, item) {
  ...
  const platform = ctx.platform || detectClipSourcePlatform(ctx) || 'twitch';
  const analysis = await geminiAnalyzeClip(videoUrl, enriched.thumbnailUrl, platform === 'youtube' ? 'youtube' : 'twitch', {
...
async function analyzeClipObservationMultiPass(enriched, item, { log } = {}) {
  ...
  let openGeminiClipSession;
  try {
    ({ openGeminiClipSession } = require('./script_gen'));
  } catch (_e) { /* non-fatal */ }
  if (typeof openGeminiClipSession !== 'function') {
    return analyzeClipObservation(enriched, item);
  }
  ...
  try {
    const platform = ctx.platform || detectClipSourcePlatform(ctx) || 'twitch';
    session = await openGeminiClipSession(videoUrl, enriched.thumbnailUrl, platform === 'youtube' ? 'youtube' : 'twitch', meta);
...
async function strictVideoRewatchObservation(enriched, item, { log } = {}) {
  ...
  if (typeof openGeminiClipSession !== 'function') return '';
  let session;
  try {
...
async function analyzeClipMomentBundle(clip, item, { log, clipIndex = 0 } = {}) {
  ...
  try {
    ...
    if (isJunkHook(hook, { ...junkOpts, observation }) || (hookQa && !hookQa.passed)) {
      ...
      if (rewatch) {
        ...
        hook = retry.hook;
        hookQa = retry.hookQa;
        hookCandidates = (retry.hookCandidates && retry.hookCandidates.length)
          ? retry.hookCandidates
          : hookCandidates;
      }
    }

    if (isJunkHook(hook, { ...junkOpts, observation }) || isFallbackHook(hook, GENERIC_FALLBACKS)) {
      const bestCand = (hookCandidates || [])
        .filter((c) => c.text && !isFallbackHook(c.text, GENERIC_FALLBACKS))
        .sort((a, b) => (b.qaScore || 0) - (a.qaScore || 0))[0];
      if (bestCand?.text) {
        hook = normalizeHookLine(ctx.streamer, bestCand.text, ctx.title) || bestCand.text;
        if (log) log(`  📌 Using best QA candidate (${bestCand.qaScore || '?'}): "${hook}"`);
        hookQa = {
          passed: !!bestCand.qaPassed,
          score: bestCand.qaScore || 0,
          violations: bestCand.qaPassed ? [] : ['best_candidate_below_pass'],
          attempts: hookQa?.attempts || 0,
        };
      } else {
  ...
  return {
    ...
    platformTitle: ctx.title || '',
    platform: ctx.platform || detectClipSourcePlatform(ctx) || 'unknown',
    pageUrl: ctx.pageUrl || '',
    game: ctx.game || '',
...
async function pickLeadStreamerAndTitleUnmapped(clipEntries, { isComp = false, log, compCreative = null } = {}) {
...
- Do NOT copy burned hook text verbatim; paraphrase OK
- CPD-1260 MOMENT CONTRACT: title draft MUST reuse the same searchable nouns as the lead clip's burned hook + observation (guest/place/stunt/event — e.g. DreamCon, YourRage, Kai, napkin, traffic). Hook teases; title names the moment for YouTube Search.
...
async function pickLeadStreamerAndTitle(clipEntries, opts) {
  const lead = await pickLeadStreamerAndTitleUnmapped(clipEntries, opts);
  const idx = Number(lead && lead.leadClipIndex) || 0;
  const entry = Array.isArray(clipEntries) ? (clipEntries[idx] || clipEntries[0]) : null;
  return applyClipChannelSeoToLead(lead, entry);
}
...
const CLIP_COMP_BRIEF_TIMEOUT_PER_CLIP_MS = 300000;
const CLIP_COMP_BRIEF_TIMEOUT_MIN_MS = 15000;

/**
 * Brief timeout scales with clip count — multi-pass analysis + Claude QA retries
 * take ~3.5-4 min per clip, so a flat total always lost the race on real jobs.
 * Explicit CLIP_COMP_BRIEF_TIMEOUT_MS overrides (absolute total, not per clip).
 */
function clipCompBriefTimeoutMs(clipCount = 1) {
  const raw = process.env.CLIP_COMP_BRIEF_TIMEOUT_MS;
  if (raw != null && String(raw).trim() !== '') {
    const parsed = parseInt(raw, 10);
    if (Number.isFinite(parsed) && parsed > 0) return parsed;
  }
  const clips = Math.max(1, Math.floor(Number(clipCount) || 1));
  return Math.max(CLIP_COMP_BRIEF_TIMEOUT_MIN_MS, CLIP_COMP_BRIEF_TIMEOUT_PER_CLIP_MS * clips);
}

function hookFromRepurposeScene(clip = {}, item = {}) {
  const label = String(clip.sceneLabel || clip.title || item.title || '').toUpperCase();
  const display = clip.displayName || item.displayName || clip.streamer || 'Streamer';
  const isSource = clip.segmentKind === 'source_clip'
    || /_SETUP_CLIP$|_INTRO_CLIP$|_REACTION_CLIP$/i.test(label);
  const isReaction = /_REACTION$/i.test(label) && !isSource;
  const isSetup = /_SETUP$/i.test(label) && !isSource;
  if (isSource) {
    const beat = label.replace(/_SETUP_CLIP$|_CLIP\d+.*$/i, '').replace(/_/g, ' ').trim();
    const words = beat.split(/\s+/).filter(Boolean).slice(-2).join(' ');
    return normalizeHookLine(display, words ? `${words} Moment` : 'The Clip', clip.title)
      || 'The Clip';
  }
  if (isReaction) return normalizeHookLine(display, 'Wait For It', clip.title) || 'Wait For It';
  if (isSetup) return normalizeHookLine(display, 'Here We Go', clip.title) || 'Here We Go';
  const plain = label.replace(/_/g, ' ').trim();
  return normalizeHookLine(display, plain.slice(0, 48), clip.title) || plain.slice(0, 48) || 'Highlight';
}

function repurposeHookCandidate(hookText) {
  return [{
    text: hookText,
    rank: 0,
    tensionScore: 85,
    why: 'Repurpose scene label',
    formula: 'Repurpose',
    psychologyTrigger: 'scene',
    visualCue: '',
    payoffTease: '',
    selected: true,
    qaPassed: true,
...
/** Ensure each clip has at least one picker candidate when Hook Machine returned a title only. */
function ensureHookCandidatePools(clips = []) {
  return clips.map((c) => {
    const pool = Array.isArray(c.hookCandidates)
      ? c.hookCandidates.filter((x) => x && String(x.text || x).trim())
      : [];
    if (pool.length) return pool;
    const hook = String(c.hook || '').trim();
    return hook ? repurposeHookCandidate(hook) : [];
  });
}

/**
 * Fast brief for Published Shows → CLIPS COMP (YouTube timestamp segments).
 * Skips 3-min Hook Machine — scene picks ARE the edit decision.
 */
async function buildRepurposeClipCompBrief(clips, items, opts = {}) {
  const { log } = opts;
  const count = Math.max(clips.length, items?.length || 0);
  const clipEntries = [];
  for (let i = 0; i < count; i++) {
    const clip = clips[i] || {};
    const item = (items && items[i]) || {};
    const hook = hookFromRepurposeScene(clip, item);
    if (log) log(`  ✓ Repurpose hook clip ${i + 1}: "${hook}"`);
    clipEntries.push({
      index: i,
      streamer: clip.streamer || item.streamer || '',
      displayName: clip.displayName || item.displayName || clip.streamer || 'Streamer',
      platformTitle: clip.sceneLabel || clip.title || '',
      observation: `Repurpose segment ${clip.sceneLabel || clip.title || i + 1} from published episode.`,
      hook,
      hookQa: { passed: true, score: 85, violations: [], attempts: 0 },
      hookCandidates: repurposeHookCandidate(hook),
    });
  }
  const isComp = count > 1;
  const lead = await pickLeadStreamerAndTitle(clipEntries, { isComp, log, compCreative: opts.compCreative || null });
  return {
    ...
    sourceChannel: lead.sourceChannel || null,
    leadTitleDraft: lead.leadTitleDraft,
    leadReason: lead.leadReason || 'repurpose_scene_picks',
    clips: clipEntries,
    generatedAt: new Date().toISOString(),
    repurposeBrief: true,
  };
...
async function generateClipCompCreativeBriefWithTimeout(clips, items, opts = {}) {
  const { log, onLateBrief } = opts;
  let timer;
  const clipCount = Math.max(clips.length, items?.length || 0, 1);
  const timeoutMs = clipCompBriefTimeoutMs(clipCount);
  ...
  try {
    ...
    // Timeout salvage: the real brief keeps running — hand it to the caller when it
    // completes so fallback hooks on the card can be replaced before operator confirm.
    if (typeof onLateBrief === 'function') {
      briefPromise
        .then((brief) => {
          if (log) log('  ♻️ Late brief completed after timeout — salvaging');
          return onLateBrief(brief);
        })
        .catch(() => {});
    }
...
function hooksAreUsable(preGenerated, count, { brief = null, operatorLocked = false } = {}) {
  if (operatorLocked && preGenerated && Array.isArray(preGenerated) && preGenerated.length >= count) {
    return preGenerated.slice(0, count).every((h) => String(h || '').trim());
  }
...
async function resolveHooksForAssembly({
  hookItems = [],
  hookClips = [],
  preGenerated = null,
  preBrief = null,
  ...
  if (forceRegenerate && !operatorLocked) {
    const brief = await generateClipCompCreativeBrief(hookClips, hookItems, { log });
    return { hooks: brief.clips.map((c) => c.hook), brief };
  }
  if (operatorLocked && hooksAreUsable(preGenerated, count, { operatorLocked: true })) {
    const hooks = preGenerated.slice(0, count);
    let brief = preBrief;
    if (brief?.clips?.length) {
      hooks.forEach((h, i) => {
        if (h && brief.clips[i]) brief.clips[i].hook = h;
      });
    } else {
      brief = buildBriefFromHooks(hookClips, hookItems, hooks);
    }
    if (log) log('  ✓ Operator-locked hooks — skipping Hook Machine');
    return { hooks, brief, operatorLocked: true };
  }
  // Operator-selected clipHookTitles always win over Gemini brief defaults (re-assemble path).
  if (hooksAreUsable(preGenerated, count)) {
    const hooks = preGenerated.slice(0, count);
    let brief = preBrief;
    if (brief?.clips?.length) {
      hooks.forEach((h, i) => {
      ...
      if (log) log('  ✓ Using operator clipHookTitles (override brief defaults)');
    } else {
      brief = buildBriefFromHooks(hookClips, hookItems, hooks);
    }
    return { hooks, brief };
  }
  if (preBrief && briefHooksAreQaReady(preBrief, count)) {
  ...
  if (preBrief && hooksAreUsable(preBrief.clips?.map((c) => c.hook) || preGenerated, count, { brief: preBrief })) {
    return { hooks: preBrief.clips.slice(0, count).map((c) => c.hook), brief: preBrief };
  }
  const brief = await generateClipCompCreativeBrief(hookClips, hookItems, { log });
  return { hooks: brief.clips.map((c) => c.hook), brief };
...
function buildBriefFromHooks(clips, items, hooks) {
  ...
  return {
    ...
    leadClipIndex: mapped.leadClipIndex,
    leadStreamer: mapped.leadStreamer,
    sourceChannel: mapped.sourceChannel || null,
    leadTitleDraft: mapped.leadTitleDraft,
    leadReason: mapped.leadReason,
    clips: clipEntries,
    generatedAt: new Date().toISOString(),
  };
}
...
function buildClipCompSeoInput(brief, compCreative = null) {
  ...
  const isDual = brief?.dualSourceStack
    || compCreative?.preset === 'dual_source_stack'
    || compCreative?.layout?.mode === 'dual_source_vstack';
  if (isDual) {
    const top = compCreative?.layout?.paneLabels?.top || brief?.clips?.[0]?.displayName || 'Age 7';
    const bot = compCreative?.layout?.paneLabels?.bottom || brief?.clips?.[1]?.displayName || 'Age 16';
    lines.push(
      'VIDEO FORMAT: Then/Now dual stack (C11) — TOP pane then BOTTOM pane on one 9:16 Short.',
      `On-screen labels: "${top}" (top half) and "${bot}" (bottom half).`,
      `Required YouTube title framing seed: "${brief.leadTitleDraft || `${top} vs ${bot} Then vs Now`}"`,
      'Do NOT invent Twitch streamer drama, chat chaos, or "Local import" as talent.',
      'Title/description should name the Then/Now / age-contrast premise and the real source videos.',
      '',
    );
  }

  const platforms = [...new Set((brief.clips || []).map((c) => c.platform).filter(Boolean))];
  const sourcePlatform = platforms.includes('youtube') && !platforms.includes('twitch')
    ? 'youtube'
    : (platforms.includes('twitch') ? 'twitch' : 'unknown');
  if (sourcePlatform === 'youtube') {
    lines.push(
      'SOURCE PLATFORM: YouTube — this is NOT a Twitch live stream. Describe the performance/video content accurately.',
      'Do NOT say "Twitch stream", "live stream", "chat chaos", or "VOD footage from Twitch" unless explicitly true.',
      '',
    );
  }
  lines.push(
    `Lead talent (SEO / title subject): ${brief.leadStreamer || 'unknown'}`,
    brief.sourceChannel && brief.sourceChannel !== brief.leadStreamer
      ? `Source clip channel (credit only — do NOT lead the YouTube title with this name): ${brief.sourceChannel}`
      : '',
  ...
  lines.push('CPD-1260 MOMENT CONTRACT (mandatory for Search, not just Shorts feed):');
  lines.push('- Pick 2–4 searchable moment keywords from lead burned hook + observation (streamer + guest/place/stunt/event).');
  lines.push('- bestTitle AND the first 1–2 sentences of the YouTube description MUST include those same keywords (paraphrase OK; do not invent a different moment).');
  lines.push('- Burned hook stays short curiosity overlay — title/desc name the moment so viewers can find it via YouTube Search.');
  lines.push('- Prefer query-style titling when natural ("ExtraEmily Stops Traffic with YourRage", "FunnyMike Cheapest iPad Hunt") over vague feed-only jokes.');
  lines.push('- CLIP CHANNEL vs TALENT (CPD-1315): if Source clip channel is set, bestTitle / tags / description opener MUST name the SEO talent (e.g. IShowSpeed), not the clip-farm channel (e.g. Speedy Boykins). Credit the source channel in Featured Streamers only.');
  if (compCreative?.hooks?.rankedList?.enabled) {
...
/**
 * Operator-typed burned hook — bypasses Hook Machine / Claude QA.
 */
function applyOperatorCustomHook(card, clipIndex, rawHookText) {
  const idx = Math.max(0, Number(clipIndex) || 0);
  const hookText = normalizeOperatorCustomHook(rawHookText);
  if (!hookText) {
    return {
      ok: false,
      error: 'Hook text is empty.',
    };
  }

  card.clipHookTitles = Array.isArray(card.clipHookTitles) ? card.clipHookTitles.slice() : [];
  while (card.clipHookTitles.length <= idx) card.clipHookTitles.push('');
  card.clipHookTitles[idx] = hookText;

  const customCand = {
    text: hookText,
    rank: 0,
    tensionScore: 100,
    why: 'Operator custom hook',
    formula: 'Operator Custom',
    psychologyTrigger: 'operator',
    visualCue: '',
    payoffTease: '',
    selected: true,
    qaPassed: true,
  ...
  const pools = Array.isArray(card.clipHookCandidates) ? card.clipHookCandidates.map((p) => (p || []).slice()) : [];
  while (pools.length <= idx) pools.push([]);
  const withoutDup = pools[idx].filter((c) => !c.operatorCustom && String(c.text || '').trim().toLowerCase() !== hookText.toLowerCase());
  pools[idx] = [customCand, ...withoutDup.map((c) => ({ ...c, selected: false }))];
  card.clipHookCandidates = pools;

  if (card.clipCompBrief?.clips?.[idx]) {
    card.clipCompBrief.clips[idx].hook = hookText;
    card.clipCompBrief.clips[idx].hookCandidates = pools[idx];
    card.clipCompBrief.clips[idx].hookQa = {
      passed: true,
      score: 100,
      violations: [],
      attempts: 0,
...
module.exports = {
  ...
  hookFromRepurposeScene,
  repurposeHookCandidate,
  ensureHookCandidatePools,
  buildFallbackClipCompBrief,
  resolveHooksForAssembly,
  isJunkHook,
  normalizeHookLine,
  stripHookJsonArtifacts,
  sanitizeTvClean,
  stripStreamerPrefix,
  hookCopiesClipTitle,
  buildClipContext,
  ...
  observationMinChars,
  applyOperatorCustomHook,
  normalizeOperatorCustomHook,
  stripHookJsonArtifacts,
};
