...
async function renderCreditsOutro({
  ...
  const scrollPxPerSec = Number(cfg.scrollPxPerSec) || 52;
  const yExpr = scroll
    ? `h-t*${Math.max(48, Math.floor(scrollPxPerSec))}`
    : '(h-text_h)/2';
...
const TWITCH_SOUP_MIN_WITH_CREDITS_SEC = 465;
...
async function verifyTwitchSoupPreAssembly({ card, customerId = 'c0', log = () => {} }) {
  ...
  const avatarScenes = (card?.script?.scenes || []).filter((s) => s.type === 'avatar');
  if (avatarScenes.length && heygenJobs.length < avatarScenes.length) {
    blockers.push(`HeyGen ${heygenJobs.length}/${avatarScenes.length} scenes — incomplete before assembly`);
  }
  ...
  for (let i = 0; i < segsToProcess.length; i++) {
    ...
    try {
      const files = fs.readdirSync(tmpDir).filter((f) => (
        f.startsWith(prefix)
        && f.endsWith('.mp4')
        && !/_legacy_|_with_crowd|_muted/i.test(f)
      ));
      if (files.length) {
        files.sort();
