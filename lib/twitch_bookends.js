...
async function appendCreditsOutroToVideo({
  ...
  let existingDur = null;
  try {
    existingDur = await probeDurationSec(mainMp4Path);
  ...
  // CPD-1223 r29: only honor the card flag when the duration probe failed — the
  // flag persists across reassemblies, so a fresh run's MP4 (no credits yet) was
  // skipped because a *previous* run had appended credits to *its* output.
  if (existingDur == null && card?.creditsOutroAppended === true) {
