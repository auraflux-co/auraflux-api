...
function buildClipBreakdown(spec) {
  return (spec.clips || []).map((c, i) => {
    ...
    const trimStart = c.trimStart != null ? Number(c.trimStart) : 0;
    const trimEnd = c.trimEnd != null ? Number(c.trimEnd) : trimStart + useSec;
    const plan = buildLayoutTimePlan({
      trimStart,
      trimEnd,
      layoutSegments: c.layoutSegments || [],
      openingLayout: c.openingLayout || null,
      compCreative: spec.compCreative || {},
    });
    const layoutPlanLabel = formatLayoutPlanSummary(plan, trimStart)
      || (spec.compCreative?.layout?.landscapeSplit === false ? 'full bleed (whole clip)' : 'split (whole clip)');
    return {
      slot: i + 1,
      ...
      trimStart,
      trimEnd,
      trimLabel: `${formatSecLabel(trimStart)}–${formatSecLabel(trimEnd)} on source`,
      layoutPlanLabel,
      footageSec: useSec,
...
function buildAssemblySteps(spec, flags, editorial) {
  ...
  steps.push({
    ...
    detail: (() => {
      const lead = (spec.clips || [])[0];
      if (lead?.layoutSegments?.length) {
        const plan = buildLayoutTimePlan({
          trimStart: lead.trimStart || 0,
          trimEnd: lead.trimEnd,
          layoutSegments: lead.layoutSegments,
          openingLayout: lead.openingLayout,
          compCreative: cc,
        });
        return `Temporal layout — ${formatLayoutPlanSummary(plan, lead.trimStart || 0)}`;
      }
      return PORTRAIT_LAYOUT_LABELS[flags.layoutMode] || flags.layoutMode;
    })(),
  });
  ...
  try {
    ...
    const clean = sourceCleanupSummary(cc.sourceCleanup);
    if (clean) {
      steps.unshift({
        phase: 'layout',
        label: 'Source UI mask',
        detail: `Blur ${clean} before portrait crop`,
      });
    }
  } catch (_) { /* offline */ }
