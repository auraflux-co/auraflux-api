...
async function _getClerkUserMeta(userId) {
  ...
  try {
    ...
    const entry  = {
      planTier:  normaliseTier(meta.planTier) || null,
      role:      meta.role || null,
      apiAccess: meta.apiAccess || meta.api_access || null,
      expiresAt: now + META_CACHE_TTL_MS,
    };
    _metaCache.set(userId, entry);
    return entry;
  } catch {
    return { planTier: null, role: null, apiAccess: null, expiresAt: now + 30_000 };
  }
}
...
function _attachUserFromClaims(req, userId, claimsMeta, email) {
  req.user = {
    ...
    apiAccess: claimsMeta.apiAccess || claimsMeta.api_access || null,
    email:     email || null,
  };
}
...
async function _authFromBearerToken(req, res, next) {
  ...
  try {
    ...
    _attachUserFromClaims(req, payload.sub, {
      ...
      apiAccess: clerkMeta.apiAccess || claimsMeta.apiAccess || claimsMeta.api_access || null,
    }, payload.email);
    return next();
  } catch (e) {
...
function requireAuth(req, res, next) {
  ...
  const jwtApiAccess = claimsMeta.apiAccess || claimsMeta.api_access || null;

  if (jwtPlanTier) {
    // JWT already has planTier — still refresh apiAccess from Clerk when missing
    if (jwtApiAccess) {
      req.user = {
        id:        auth.userId,
        role:      jwtRole || ROLES.CUSTOMER,
        planTier:  jwtPlanTier,
        apiAccess: jwtApiAccess,
        email:     auth.sessionClaims?.email || null,
      };
      return next();
    }
    return _getClerkUserMeta(auth.userId).then((clerkMeta) => {
      req.user = {
        id:        auth.userId,
        role:      jwtRole || clerkMeta.role || ROLES.CUSTOMER,
        planTier:  jwtPlanTier,
        apiAccess: clerkMeta.apiAccess || null,
        email:     auth.sessionClaims?.email || null,
      };
      next();
    }).catch(() => {
      req.user = {
        id: auth.userId, role: jwtRole || ROLES.CUSTOMER, planTier: jwtPlanTier,
        apiAccess: null, email: auth.sessionClaims?.email || null,
      };
      next();
    });
  }
  ...
  _getClerkUserMeta(auth.userId).then((clerkMeta) => {
    req.user = {
      ...
      planTier:  clerkMeta.planTier || 'operate',
      apiAccess: clerkMeta.apiAccess || jwtApiAccess || null,
      email:     auth.sessionClaims?.email || null,
    };
    next();
  }).catch(() => {
    req.user = { id: auth.userId, role: ROLES.CUSTOMER, planTier: 'operate', apiAccess: null, email: null };
    next();
  });
}
