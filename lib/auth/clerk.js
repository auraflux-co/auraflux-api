...
async function _getClerkUserMeta(userId) {
  ...
  try {
    ...
    const entry  = { planTier: meta.planTier || null, role: meta.role || null, expiresAt: now + META_CACHE_TTL_MS };
...
function clerkInit() {
  ...
  if (!hasSecret || !hasPublishable) {
    const missing = [
      !hasSecret      && 'CLERK_SECRET_KEY',
      !hasPublishable && 'CLERK_PUBLISHABLE_KEY (or NEXT_PUBLIC_CLERK_PUBLISHABLE_KEY)',
    ].filter(Boolean).join(', ');
    console.warn(`[auth] ${missing} not set — Clerk auth disabled, all requests unauthenticated`);
    return (req, _res, next) => next();
  }

  // Ensure the bare name is set so @clerk/express SDK can find it
  ...
  // Wrap to catch Clerk handshake/session errors (e.g. browser requests with
  // __clerk_handshake param) so they don't bubble up as unhandled 500 errors.
  // Protected routes still enforce auth via requireAuth() — this only prevents
  // Clerk middleware itself from crashing public routes.
  return (req, res, next) => {
    inner(req, res, (err) => {
      if (err) {
        console.warn('[auth] Clerk middleware error (non-fatal on public route):', err.message);
      }
      next(); // always proceed — requireAuth() enforces auth on protected routes
    });
  };
}
...
function requireAuth(req, res, next) {
  ...
  const jwtPlanTier = claimsMeta.planTier || null;
  ...
  _getClerkUserMeta(auth.userId).then((clerkMeta) => {
    req.user = {
      ...
      planTier: clerkMeta.planTier || 'diy',
      email:    auth.sessionClaims?.email || null,
    };
    next();
  }).catch(() => {
    req.user = { id: auth.userId, role: ROLES.CUSTOMER, planTier: 'diy', email: null };
    next();
  });
}
