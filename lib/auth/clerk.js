...
function requireAuth(req, res, next) {
  ...
  // Role and planTier are stored in Clerk public metadata
  const meta      = auth.sessionClaims?.metadata || auth.sessionClaims?.publicMetadata || {};
  const role      = meta.role      || ROLES.CUSTOMER;
  const planTier  = meta.planTier  || 'diy';
