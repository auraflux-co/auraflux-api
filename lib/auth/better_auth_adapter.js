...
async function _getProfileMeta(accountId) {
  ...
  try {
    ...
    return { planTier: 'operate', role: ROLES.CUSTOMER, apiAccess: null, email: null, expiresAt: now + 30_000 };
  }
}
...
function requireAuth(req, res, next) {
  ...
  const hasApiAccessClaim = Object.prototype.hasOwnProperty.call(claims, 'apiAccess')
    || Object.prototype.hasOwnProperty.call(claims, 'api_access');
  const jwtApiAccess = hasApiAccessClaim
    ? (claims.apiAccess ?? claims.api_access ?? null)
    : undefined;

  if (jwtPlan && jwtRole && hasApiAccessClaim) {
    req.user = {
      id: claims.sub,
      role: jwtRole,
      planTier: jwtPlan,
  ...
  _getProfileMeta(claims.sub)
    .then((meta) => {
      req.user = {
        ...
        apiAccess: hasApiAccessClaim ? jwtApiAccess : (meta.apiAccess || null),
