...
const ChromeTvCardSchema = z.object({
  visible: z.boolean(),
  imageUrl: z.string().optional(),   // og:image URL from article scrape
  headline: z.string().optional(),   // Full article headline, sentence case
  sourceName: z.string().optional()  // Logo/text below image
});
...
function directiveToOverlayParams(directive, context) {
  ...
  return {
    ...
    // Red 4 Fix 3a: expose tvCard so assembly can render the TV card overlay
    tvCard: directive.tvCard?.visible ? {
