 ...
 *
 * Strategy:
 *  1. Check pod desiredStatus via GraphQL.
 *  2. If RUNNING — confirm ComfyUI is reachable; return pod ID.
 *  3. If EXITED — call podResume (same host).
 *  4. If podResume fails "not enough GPUs" — call podFindAndDeployOnDemand
 *     to spin up a replacement on any available machine, then update
 *     RUNPOD_POD_ID in Render environment vars so future calls use the new pod.
 *  5. Wait up to 5 min for ComfyUI to respond.
 *
...
async function ensurePodRunning() {
  ...
  const podConfig = pod; // save for potential recreation

  if (pod.desiredStatus === 'RUNNING') {
    // Already desired running — check if ComfyUI is actually up
    const ready = await _waitForComfyUI(podId, 60_000);
    if (ready) return podId;
    // Proxy up but ComfyUI not responding — fall through to resume attempt
  }
  ...
  // ── 3. GPU shortage — create a new pod on any available machine ───────────
  console.log(`[runpod] Resume blocked (GPU shortage or timeout) — deploying new pod`);
  ...
  console.log(`[runpod] New pod created: ${newPod.id} — waiting for ComfyUI`);
  podId = newPod.id;

  // Update in-process env var immediately so subsequent calls in this run use the new pod
  process.env.RUNPOD_POD_ID = podId;

  // Persist the new pod ID to Render so restarts and future runs pick it up
  _updateRenderPodId(podId).catch((e) =>
    console.warn(`[runpod] Failed to update RUNPOD_POD_ID in Render: ${e.message}`)
  );

  const ready = await _waitForComfyUI(podId, 600_000); // 10 min for cold start + model load
  if (!ready) throw new Error(`ComfyUI on new pod ${podId} did not become ready in 10 min`);
...
async function _updateRenderPodId(newPodId) {
  ...
  console.log(`[runpod] Updated RUNPOD_POD_ID=${newPodId} in Render service ${serviceId}`);
}
