...
async function pollComfyResult(promptId, podId = getPodId(), opts = {}) {
  const { maxWaitMs = 600_000, intervalMs = 5_000 } = opts;
...
// Cache the probe result per pod so we only check once per process lifetime.
const _wan21ProbeCache = {};

/**
 * Returns true if the pod exposes the newer kijai WAN 2.1 node schema
 * (CLIPLoader with type="wan" + WanImageToVideo), false for the legacy
 * WanVideoModelLoader schema. Result is cached per pod ID.
 */
async function _probeWan21Nodes(podId = getPodId()) {
  if (_wan21ProbeCache[podId] !== undefined) return _wan21ProbeCache[podId];
  try {
    const url = `https://${podId}-8188.proxy.runpod.net/object_info/WanImageToVideo`;
    const result = await new Promise((resolve) => {
      const req = https.get(url, { timeout: 8000 }, (res) => {
        let raw = '';
        res.on('data', (c) => { raw += c; });
        res.on('end', () => {
          try { resolve(res.statusCode === 200 && JSON.parse(raw).WanImageToVideo != null); }
          catch { resolve(false); }
        });
      });
    ...
    _wan21ProbeCache[podId] = result;
    return result;
  } catch {
    _wan21ProbeCache[podId] = false;
    return false;
  }
}
 ...
 *
 * WAN 2.2 parameters:
 *   width, height (pixels), numFrames — default 97 (6s @ 16fps, clears portal3a 5s minimum)
 *
 ...
 * @param {string}  [opts.planTier='operate']
 ...
 *
 * @param {Object} opts
 * @param {string} opts.planTier      — job plan tier ('operate'|'guided'|'managed'|'custom')
...
async function generateWanVideo(opts = {}) {
  const {
    ...
    planTier = 'operate',
    podId = getPodId(),
    seed: _seedParam = null,  // null/undefined → random seed below
  ...
  const seed = _seedParam ?? Math.floor(Math.random() * 2 ** 32);
  ...
  // ── WAN 2.1 / 2.2 (self-hosted ComfyUI) ─────────────────────────────────
  // Detect which node schema the pod supports by probing the object_info.
  // Pods with the newer kijai WAN wrapper use CLIPLoader+WanImageToVideo (wan21 workflow).
  // Legacy pods use the older WanVideoModelLoader schema (wan22 workflow).
  const isWan21 = await _probeWan21Nodes(podId);
  const workflowFile = isWan21 ? 'wan_t2v_wan21_workflow.json' : 'wan_t2v_workflow.json';
  const workflowPath = path.join(__dirname, workflowFile);
  const workflow = JSON.parse(fs.readFileSync(workflowPath, 'utf8'));

  if (isWan21) {
    // WAN 2.1 workflow: CLIPLoader(2) → CLIPTextEncode(3/4) → WanImageToVideo(6) → KSampler(7) → SaveAnimatedPNG(9)
    workflow['3'].inputs.text = positivePrompt;
    workflow['4'].inputs.text = negativePrompt;
    workflow['6'].inputs.width = width;
    workflow['6'].inputs.height = height;
    workflow['6'].inputs.length = numFrames;
    workflow['7'].inputs.seed = seed;
    workflow['9'].inputs.filename_prefix = outputPrefix;
  } else {
    // Legacy WAN 2.2 workflow: WanVideoModelLoader → WanVideoSampler → SaveAnimatedWEBP
  ...
  console.log(`[runpod] Using ${workflowFile} on pod ${podId}`);
...
async function _ensurePodRunningImpl() {
  ...
  // Pod was deleted from RunPod — skip straight to fresh deployment.
  // The stored RUNPOD_POD_ID is stale; _registerNewPod will update it.
  if (!pod) {
    console.log(`[runpod] Pod ${podId} not found (deleted?) — deploying fresh pod`);
    return _deployFreshPod(apiKey, podId, {
...
/**
 * Deploy a brand-new pod using the given config defaults.
 * Used when ensurePodRunning detects the tracked pod was deleted.
 */
async function _deployFreshPod(apiKey, oldPodId, defaults = {}) {
  const {
    imageName = 'runpod/comfyui:latest',
    gpuCount = 1,
    containerDiskInGb = 50,
  ...
  const gpuType = 'NVIDIA GeForce RTX 3090';
  console.log(`[runpod] Deploying fresh pod (old pod ${oldPodId} was deleted)`);
  const deployResult = await _graphql(apiKey, `
    mutation {
      podFindAndDeployOnDemand(input: {
        name:              "auraflux-comfyui-auto"
  ...
  if (!newPod?.id) {
    const err = (deployResult?.errors || []).map((e) => e.message).join('; ');
    throw new Error(`podFindAndDeployOnDemand failed (fresh deploy): ${err}`);
  }

  console.log(`[runpod] Fresh pod created: ${newPod.id} — waiting for ComfyUI (up to 10 min)`);
  const ready = await _waitForComfyUI(newPod.id, 600_000);
  if (!ready) throw new Error(`ComfyUI on fresh pod ${newPod.id} did not become ready in 10 min`);

  await _registerNewPod(newPod.id, oldPodId);
  return newPod.id;
}
