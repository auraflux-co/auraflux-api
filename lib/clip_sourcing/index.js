...
// ─── CPD-870: Autonomous clip picker — pre-portal job pre-step ───────────────
/**
 * Autonomously select clips for a job spec before the portal sequence begins.
 *
 * Flow:
 *  1. Reads footage local paths from jobSpec.order.inputs.items[].localPath
 *  2. Runs autoSourceClipManifest to get Gemini-ranked clips
 *  3. Falls back to view-count ordering when no local paths exist or Gemini fails
 *  4. Persists manifest to jobSpec.state.clipManifest and jobSpec.orderedClipUrls
 *  5. Logs clip selection reasoning for operator review
 *
 * Non-fatal: caller must .catch() — if no clips can be selected the job proceeds
 * to manual selection queue (state.clipAutoSelectFailed = true).
 *
 * @param {object} jobSpec  — mutated in-place
 * @returns {Promise<object|null>}  — clip manifest or null on failure
 */
async function runForJob(jobSpec) {
  let logErr;
  try { logErr = require('../utils/logger').logError; } catch { logErr = () => {}; }

  const jobId     = jobSpec?.jobId || 'unknown';
  const items     = jobSpec?.order?.inputs?.items || [];
  const script    = jobSpec?.filledScript || jobSpec?.scaffold || jobSpec?.script?.raw || '';
  const showTitle = jobSpec?.designSpec?.showTitle ||
                    jobSpec?.order?.contentType    ||
                    jobSpec?.contentType           || 'show';
  const planTier  = jobSpec?.planTier || 'operate';

  if (!jobSpec.state) jobSpec.state = {};

  // ── Step 1: Try Gemini footage analysis from local paths ──────────────────
  const footagePaths = items.map((i) => i.localPath).filter(Boolean);
  let clipManifest   = null;
  let selectionLog   = [];

  if (footagePaths.length > 0) {
    try {
      clipManifest = await autoSourceClipManifest(footagePaths, script, showTitle, { planTier });
      if (clipManifest?.clips?.length) {
        selectionLog = clipManifest.clips.map((c) => ({
          label:      c.label || c.id,
          confidence: c.confidence,
          startTime:  c.startTime,
          duration:   c.duration,
          source:     clipManifest.source,
        }));
      }
    } catch (err) {
      logErr('CPD870_GEMINI_ANALYSIS_FAIL', err, { jobId });
    }
  }

  // ── Step 1b: CPD-947 — Twitch URL candidates ranked by Gemini Flash ─────────
  // Fires when no local footage exists but Twitch clip/VOD URLs are present.
  // Gemini ranks candidates by metadata (title, game, viewCount, duration).
  // No video download required — metadata-only ranking via text prompt.
  if (!clipManifest || !clipManifest.clips?.length) {
    const twitchCandidates = _collectTwitchUrlCandidates(items, jobSpec);
    if (twitchCandidates.length > 0) {
      try {
        const contentType = jobSpec?.contentType || '';
        clipManifest = await _rankTwitchCandidatesWithGemini(
          twitchCandidates, contentType, showTitle, jobId, planTier
        );
        if (clipManifest?.clips?.length) {
          selectionLog = clipManifest.clips.map((c) => ({
            label: c.label, confidence: c.confidence, source: 'gemini_metadata_rank',
          }));
        }
      } catch (err) {
        logErr('CPD947_GEMINI_TWITCH_RANK_FAIL', err, { jobId });
      }
    }
  }

  // ── Step 2: URL metadata fallback — order by viewCount ────────────────────
  if (!clipManifest || !clipManifest.clips?.length) {
    const urlItems = items.filter((i) => i.url || i.clipUrl || i.videoUrl);
    if (urlItems.length > 0) {
      const sorted = [...urlItems].sort((a, b) => (b.viewCount || 0) - (a.viewCount || 0));
      const top    = sorted.slice(0, MAX_CANDIDATES);
      clipManifest = {
        clips: top.map((item, i) => ({
          id:                 item.id || `fallback-${i}`,
          path:               item.localPath || null,
          url:                item.url || item.clipUrl || item.videoUrl || null,
          scriptSegmentIndex: i,
          duration:           item.duration || DEFAULT_CLIP_DURATION,
          confidence:         0.5,
          label:              item.title || item.label || `Clip ${i + 1}`,
          startTime:          0,
          tags:               item.tags || [],
        })),
      ...
      selectionLog = clipManifest.clips.map((c) => ({
        label: c.label, confidence: c.confidence, source: 'view_count_fallback',
      }));
    }
  }

  // ── Step 3: Persist to job spec ───────────────────────────────────────────
  if (clipManifest && clipManifest.clips?.length) {
    jobSpec.state.clipManifest       = clipManifest;
    jobSpec.state.clipSelectionLog   = selectionLog;
    jobSpec.state.clipAutoSelectedAt = new Date().toISOString();

    // Populate orderedClipUrls so the assembly layer can consume auto-selected clips
    jobSpec.orderedClipUrls = clipManifest.clips
      .filter((c) => c.path || c.url)
      .map((c, i) => ({
        clipUrl:      c.url  || c.path || '',
        url:          c.url  || c.path || '',
        storyIndex:   i,
        label:        c.label,
        duration:     c.duration,
    ...
    console.log(
      `[clip_sourcing] ${jobId}: auto-selected ${clipManifest.clips.length} clips ` +
      `(source: ${clipManifest.source}, orderedClipUrls: ${jobSpec.orderedClipUrls.length})`
    );
    return clipManifest;
  }

  // ── Step 4: No clips available — flag for manual selection queue ──────────
  console.warn(`[clip_sourcing] ${jobId}: no clips auto-selected — job queued for manual clip selection`);
  jobSpec.state.clipAutoSelectFailed   = true;
  jobSpec.state.clipAutoSelectFailedAt = new Date().toISOString();
  return null;
}

// ─── CPD-947: Twitch Gemini metadata ranker ───────────────────────────────────

const _TWITCH_CLIP_RE = /twitch\.tv\/(?:[^/]+\/clip\/|clip\/)|clips\.twitch\.tv\//i;
const _TWITCH_VOD_RE  = /twitch\.tv\/videos\//i;

/**
 * Collect Twitch clip/VOD URL candidates from both items[] and jobSpec.urls.
 * Returns an array of { url, title, viewCount, durationSeconds, game } objects.
 */
function _collectTwitchUrlCandidates(items, jobSpec) {
  const seen = new Set();
  const out  = [];

  const addItem = (item) => {
    const url = item.url || item.clipUrl || item.videoUrl || '';
    if (!url || seen.has(url)) return;
    if (!_TWITCH_CLIP_RE.test(url) && !_TWITCH_VOD_RE.test(url)) return;
    seen.add(url);
    out.push({
      url,
      title:           item.title   || item.label    || '',
      viewCount:       item.viewCount  != null ? item.viewCount  : null,
      durationSeconds: item.duration   != null ? item.duration   : null,
  ...
  const rawUrls = [
    ...(Array.isArray(jobSpec?.urls) ? jobSpec.urls : []),
    ...(jobSpec?.url ? [jobSpec.url] : []),
  ];
  for (const url of rawUrls) addItem({ url });

  return out;
}

/**
 * CPD-947: Call Gemini Flash to rank Twitch clip candidates by metadata.
 * No video download — passes titles, view counts, games, and thumbnails as text.
 *
 * For contentType 'clips': picks 1 best clip.
 * For VOD/highlight: picks up to 5.
 */
async function _rankTwitchCandidatesWithGemini(candidates, contentType, showTitle, jobId, planTier) {
  const { callGemini } = require('../services/gemini');
  const isVod  = /vod|highlight|video|long/i.test(contentType);
  const pickN  = isVod ? 5 : 1;
  const label  = isVod ? `up to ${pickN} best clips for a highlight reel` : 'the single best clip';

  const candidateList = candidates.slice(0, 30).map((c, i) => {
    const parts = [`${i + 1}. URL: ${c.url}`];
    if (c.title)           parts.push(`   Title: ${c.title}`);
    if (c.game)            parts.push(`   Game: ${c.game}`);
    if (c.viewCount != null) parts.push(`   Views: ${c.viewCount.toLocaleString()}`);
    if (c.durationSeconds != null) parts.push(`   Duration: ${c.durationSeconds}s`);
    return parts.join('\n');
  }).join('\n\n');

  const prompt = `You are selecting Twitch clips for a short-form video production pipeline.

Show/brand: "${showTitle}"
Content type: ${contentType || 'clips'}
Task: Select ${label} from the candidates below.
  ...
  "selected": [
    {
      ...
      "url": "<exact url>",
      "reason": "<1 sentence why this clip was chosen>"
    }
  ]
}

Rules:
- Select exactly ${pickN === 1 ? '1 clip' : `1 to ${pickN} clips`}
- Prefer clips with strong titles, high view counts, and energetic/highlight-worthy content
- For gaming content, prefer skill highlights, funny moments, or viral-worthy reactions
- Return only indices from the provided list`;

  const raw = await callGemini(prompt, { maxOutputTokens: 600, temperature: 0.2 });
  const clean = raw.replace(/```[a-z]*\n?/gi, '').trim();
  const parsed = JSON.parse(clean);

  if (!Array.isArray(parsed.selected) || parsed.selected.length === 0) return null;

  const clips = parsed.selected
    .map((sel) => {
      const idx  = (sel.index || 1) - 1;
      const orig = candidates[idx];
      if (!orig) return null;
      return {
        id:                `twitch-gemini-${idx}-${Date.now()}`,
        url:               orig.url,
        path:              null,
        scriptSegmentIndex: idx,
        duration:          orig.durationSeconds || DEFAULT_CLIP_DURATION,
        confidence:        0.8,
        label:             orig.title || `Clip ${idx + 1}`,
        startTime:         0,
        tags:              orig.game ? [orig.game] : [],
        _geminiReason:     sel.reason || '',
      };
    })
    .filter(Boolean);
  ...
  console.log(
    `[clip_sourcing] ${jobId}: CPD-947 Gemini picked ${clips.length}/${candidates.length} ` +
    `Twitch candidates (${isVod ? 'VOD/multi' : 'clips/single'})`
  );
