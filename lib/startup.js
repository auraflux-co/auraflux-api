...
async function rescueInterruptedJobs() {
  ...
  try {
    ...
    await Promise.all(
      stale.map(({ id, spec }) => {
        // CPD-175: If the job already has outputUrl, the video assembled successfully before
        // the server died — likely a publish or downstream step was interrupted. Mark as
        // 'assembled' (video ready) rather than 'failed' (nothing produced). This prevents
        // E2E tests and the dashboard from treating a successful assembly as a failure.
