...
// CPD-1221: exact drawtext width via node-canvas — the 0.66*fontsize estimate nearly
// doubles the real Barlow Condensed width on mixed-case hooks, shifting the block left.
// Fonts register per path (registerFont must precede canvas creation for that family).
const _registeredMeasureFonts = new Map(); // fontPath → family name (or null on failure)
let _measureCtx = null;
function measureHookTextWidth(text, fontsize, fontPath) {
  const t = String(text || '');
  if (!t) return 0;
  try {
    ...
    if (fontPath && fs.existsSync(fontPath)) {
      if (!_registeredMeasureFonts.has(fontPath)) {
        const fam = `CWNHookMeasure${_registeredMeasureFonts.size}`;
        try {
          registerFont(fontPath, { family: fam });
          _registeredMeasureFonts.set(fontPath, fam);
        } catch {
          _registeredMeasureFonts.set(fontPath, null);
        }
      }
      family = _registeredMeasureFonts.get(fontPath);
    }
    if (!family) return estimateTextWidth(t, fontsize);
    if (!_measureCtx) _measureCtx = createCanvas(8, 8).getContext('2d');
    _measureCtx.font = `${fontsize}px "${family}"`;
    const w = _measureCtx.measureText(t).width;
    if (w > 0) return w;
  } catch { /* canvas unavailable — fall through to estimate */ }
  return estimateTextWidth(t, fontsize);
}
...
function buildHookBurnFilterPlan(lines, style, {
  ...
  cleanLines.forEach((line, lineIdx) => {
    ...
    const textW = measureHookTextWidth(textPart, style.fontsize, style.font);
