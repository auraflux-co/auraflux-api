...
/** Host header for local dashboard — skip Basic Auth so Rob's localhost link stays open. */
...
function skipAuthForLocalHost() {
  const v = String(process.env.BASIC_AUTH_SKIP_LOCAL || '1').toLowerCase();
...
function createBasicAuthMiddleware({
  ...
  return function basicAuthMiddleware(req, res, next) {
    ...
    // Tunnel Host is *.trycloudflare.com / ngrok — those still require auth.
    if (skipAuthForLocalHost() && isLocalHostHeader(req.headers.host)) return next();
