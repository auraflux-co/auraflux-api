...
function writeDirectiveForJob(jobId, directive) {
  ...
  console.log(
    `[directives] Wrote directive for job ${jobId} → ${filePath} (${directive.scenes.length} scenes, ${directive.storyList.length} stories)`
  );
  return filePath;
}
...
function loadDirectiveForJob(jobId) {
  ...
  if (!validation.ok) {
    const err = new Error(
      `Directive file ${filePath} failed Zod validation: ${validation.errors.join('; ')}`
    );
    err.code = 'DIRECTIVE_VALIDATION_FAILED';
    err.validatorErrors = validation.errors;
    throw err;
