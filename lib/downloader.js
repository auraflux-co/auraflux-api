...
const TRUSTED_DOMAINS = [
  ...
  'static-cdn.jtvnw.net',         // Twitch Helix clip CDN (thumbnailToMp4)
  'jtvnw.net',                    // Twitch static / video asset CDN
  'twitch-video-assets',          // Twitch VOD/clip paths on jtvnw CDN
  'twitch.tv',                    // CPD-1014: clip/VOD page URLs (portal1b video review)
  'clips.twitch.tv',              // CPD-1014: legacy clip page host
  'kick.com',                     // Kick clip/VOD page URLs (yt-dlp / Apify resolve)
  ...
  'cloudflarestorage.com',        // Cloudflare R2 presigned avatar output
  'assets.auraflux.co',           // Library R2 staging + compact-fetch clip CDN (CPD-1118)
  'r2.dev',                       // Cloudflare R2 public dev URLs
  'r2.cloudflarestorage.com',     // R2 account subdomain variant
  ...
  'ap.org',                       // AP CDN
  'streamable.com',               // Reddit desk — common embed host
  'reddit.com',                   // Reddit video pages
  'redd.it',                      // v.redd.it short video CDN
  'v.redd.it',                    // Reddit hosted video
];

function isTrustedDownloadUrl(url) {
  if (!url || typeof url !== 'string') return false;
  if (url.startsWith('/') && fs.existsSync(url)) return true;
...
async function downloadFile(url, destPath) {
  ...
  stream.on('data', () => { lastByteAt = Date.now(); });
  ...
  return new Promise((res, rej) => {
    const stallTimer = setInterval(() => {
      if (Date.now() - lastByteAt > STALL_MS) {
        ...
        rej(new Error(`Download stalled — no data for ${Math.round(STALL_MS / 1000)}s`));
      }
    }, 15000);
    ...
    writer.on('finish', done(res));
    writer.on('error', done(rej));
    stream.on('error', done(rej));
  });
}
...
module.exports = { downloadFile, downloadVideoForAnalysis, isTrustedDownloadUrl, TRUSTED_DOMAINS };
