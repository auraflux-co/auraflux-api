...
const TRUSTED_DOMAINS = [
  ...
  'vod.kick.com', // Kick VOD CDN
  'clips.kick.com', // Kick clips CDN
  'cdn.kick.com', // Kick general CDN
  'b-cdn.net', // BunnyCDN (used by Kick and others)
  ...
  'media.w3.org', // W3C media samples (used in E2E tests)
  'commondatastorage.googleapis.com', // Google public test videos
  'r2.dev', // Cloudflare R2 public buckets (our own storage)
  'r2-dev', // Cloudflare R2 variant
];
...
async function downloadFile(url, destPath) {
  ...
  if (isHls) {
    return new Promise((res, rej) => {
      ...
      proc.on('close', (code) =>
        code === 0 ? res() : rej(new Error(`FFmpeg HLS download failed with code ${code}`))
      );
      proc.on('error', rej);
    });
  }
...
async function downloadVideoForAnalysis(url, destPath, { maxSecs = 90 } = {}) {
  ...
  return new Promise((res, rej) => {
    ...
    const proc = execFile(ffmpegPath(), args, {
      timeout: (maxSecs + 60) * 1000,
      maxBuffer: 50 * 1024 * 1024,
    });
    proc.on('close', (code) => {
      if (code !== 0)
        return rej(new Error(`FFmpeg download failed (code ${code}) for ${url.slice(0, 80)}`));
      const size = fs.existsSync(destPath) ? fs.statSync(destPath).size : 0;
      if (size < 1000)
        return rej(
          new Error(`Downloaded file too small (${size} bytes) — stream may be unavailable`)
        );
      res();
    });
    proc.on('error', rej);
