...
function isEditorialContentType(contentType) {
  const base = String(contentType || '').replace(/-short$/, '').replace(/-vod-comp$/, '');
  return base.includes('sports') || ['nba', 'basketball', 'boxing', 'hockey', 'nhl'].some(t => base.includes(t))
    || base.includes('news');
}

/** Editorial timeline for sports/news OR twitch vod_comp / editorial flag (CPD-1091). */
function shouldUseClipCompEditorial(contentType, compCreative = null) {
  if (!editorialEnabled()) return false;
  if (isEditorialContentType(contentType)) return true;
  const c = compCreative || {};
  if (c.delivery?.format === 'vod_comp') return true;
  if (c.editorial?.enabled) return true;
  return false;
}
...
function resolveTemplateKey(contentType, compCreative = null) {
  const base = String(contentType || '').replace(/-short$/, '').replace(/-vod-comp$/, '');
  if (base.includes('news')) return 'news';
  if (compCreative?.delivery?.format === 'vod_comp' || base.includes('twitch') || base === 'clips') return 'twitch';
  return 'sports';
}
...
function buildSpokenScripts(contentType, plan, compCreative = null) {
  const cfg = loadEditorialConfig();
  const key = resolveTemplateKey(contentType, compCreative);
  ...
  const streamer = compCreative?.hooks?.rankedList?.streamer
    || plan.streamer
    || 'the stream';
  return {
    introText: fillTemplate(tpl.introTts, { dateLong, topic, streamer }),
    outroText: fillTemplate(tpl.outroTts, { dateLong, topic, streamer }),
