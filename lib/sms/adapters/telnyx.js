...
const { Telnyx, TelnyxWebhook } = require('telnyx');
...
async function sendSms({ to, from: fromNumber, body }) {
  ...
  const msg = await client.messages.send({ from: number, to, text: body });
  return {
    sid:    msg.data?.id || msg.id,
    status: msg.data?.to?.[0]?.status || msg.data?.status || msg.status || 'queued',
  };
}
