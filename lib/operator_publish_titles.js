...
function buildTitleCandidatesFromPublishCopy(pc, card = null) {
  ...
  if (yt.bestTitle?.title) {
    const withShorts = yt.title || `${yt.bestTitle.title} #Shorts`;
    if (!out.some((c) => c.text.toLowerCase() === withShorts.toLowerCase())) {
      add(withShorts, { source: 'bestTitle' });
    }
  }
  if (card?.clipCompBrief?.leadTitleDraft) {
