...
function reapplyOperatorTitleIfLocked(card) {
  ...
  const isShort = isShortJob(card);
  const lockedTitle = card.title ? normalizeOperatorTitle(card.title, { isShort }) : '';
  const title = (sel?.operatorCustom && sel.text)
    || lockedTitle
    || (sel?.text || '');
  if (!title) return pc;
  setYoutubeTitle(pc, title, {
    reason: sel?.operatorCustom ? 'Operator typed YouTube title' : 'Operator selected title',
    operatorCustom: !!(sel?.operatorCustom || lockedTitle),
  });
  card.publishCopy = pc;
  return pc;
}
