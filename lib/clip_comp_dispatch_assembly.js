...
async function runClipCompHookGeneration(jobId, card, {
  ...
  if (!clipCompBurnedHooksEnabled(card)) {
    ...
    try {
      const { isDualSourceStackMode, enrichDualSourceSeoBrief } = require('./clip_comp_dual_source');
      if (isDualSourceStackMode(card.compCreative || { preset: card.compCreativePreset })) {
        enrichDualSourceSeoBrief(card);
        console.log(`[clip-comp] ${jobId}: C11 dual SEO brief — "${card.clipCompBrief?.leadTitleDraft || ''}"`);
      }
