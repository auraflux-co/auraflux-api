...
function needsHookBeforeAssembly(card = {}) {
  ...
  // whisper_only (e.g. FableFlow): no burned hooks — skip Hook Machine + hook_review
  if (!clipCompBurnedHooksEnabled(card)) return false;
...
async function runClipCompHookGeneration(jobId, card, {
  ...
  if (!clipCompBurnedHooksEnabled(card)) {
    ...
    card.clipHookTitles = Array.from({ length: n }, () => '');
    card.clipHookCandidates = Array.from({ length: n }, () => []);
    card.clipCompBrief = null;
