...
function createJobSpec(params) {
  const {
    ...
    expectedSynth = false,
    addOns = {},
    planTier = null,   // 'diy' | 'dwy' | 'dfy' | 'custom' — baked in at job creation
  } = params;
  ...
  const resolvedPlanTier = planTier || customerConfig.planTier || 'diy';
  let featureFlags = {};
  ...
  if (sourceType === 'wan_gen') {
    ...
    if (!isFeatureEnabledFn(requiredFeature, resolvedPlanTier)) {
      throw new Error(`[job_spec] AI video generation (${genType || 'text'}) requires plan ${genType === 'image' ? 'dfy' : 'dwy'} or higher (current: ${resolvedPlanTier})`);
    }
  }
