...
// ── CPD-872: Required features for QA compliance ──────────────────────────────

/**
 * Build the list of features that QA must verify are present in the assembled video.
 * Baked into jobSpec at creation; consumed by portal_gpt4o_qa.js for compliance check.
 *
 * @param {{ addOns, extensions, designSpec, templateId, productionProfile }}
 * @returns {Array<{ key: string, label: string, description: string }>}
 */
function deriveRequiredFeatures({ addOns = {}, extensions = {}, designSpec = {}, templateId = '', productionProfile = '', brandId = null }) {
  const features = [];

  const expectedAspect = (productionProfile === 'vertical_reel') ? '9:16' : '16:9';
  features.push({
    key:         'correct_aspect_ratio',
    label:       'Correct aspect ratio',
    description: `Video must be ${expectedAspect} for ${productionProfile || 'standard'} profile`,
  });

  // Short-form = vertical_reel profile (TikTok/Reels/Shorts) OR legacy 'short-form' templateId
  const isShort = productionProfile === 'vertical_reel' || templateId === 'short-form';
  features.push({
    key:         'correct_duration',
    label:       'Duration in expected range',
    description: isShort
      ? 'Short-form (vertical/TikTok/Reels): 30–90 seconds. ACCEPT any video 30s or longer as passing this check.'
      : 'Long-form (horizontal/YouTube): 90–600 seconds',
  });

  if (addOns?.tts?.active === true) {
    features.push({
      key:         'tts_audio',
      label:       'TTS voice-over audible',
      description: 'Whisper transcript must contain narration, not just silence',
    });
  }

  if (addOns?.branding?.active === true ||
      designSpec?.chrome?.hasLogo === true ||
      designSpec?.chrome?.hasTopBar === true) {
    features.push({
      key:         'logo_overlay',
      label:       'Brand logo/overlay visible',
      description: 'Sub-brand logo (Twitch avatar) must be visible in the lower-third area',
    });
  }

  if (brandId) {
    features.push({
      key:         'platform_watermark',
      label:       'AuraFlux platform watermark',
      description: 'AuraFlux logo must be visible in the bottom-right corner of the video',
    });
  }

  if (designSpec?.chrome?.captionStyle && designSpec.chrome.captionStyle !== 'none') {
    features.push({
      key:         'captions',
      label:       'Captions burned in',
      description: 'Visible caption text must appear in frames during narrated segments',
    });
  }

  if (extensions?.heygen_ext?.ordered === true) {
    features.push({
      key:         'avatar_video',
      label:       'HeyGen avatar segment present',
      description: 'A talking-head avatar segment must be visible in the video',
    });
  }

  if (designSpec?.chrome?.hasTopBar === true || designSpec?.chrome?.hasFlag === true) {
    features.push({
      key:         'intro_card',
      label:       'Intro card present at start',
      description: 'A branded intro card must appear in the first 5 seconds',
    });
  }
 ...
 *
 * Required params:
 *   customerId   string    — Clerk user ID e.g. 'user_3DeZEr79TWcmvEvCMlu8lmM1FdU'
 *   templateId   string    — 'long-form' | 'short-form'
 *   contentType  string    — e.g. 'news', 'clips', 'sports'
 *
 * Optional params:
 *   createdBy    string    — 'dashboard' | 'api' | 'agent' | 'e2e_script'  (default: 'api')
...
function buildPortalMap({ providers, sourceType, stageMap, expectedSynth, contentType }) {
  ...
  // CPD-405: compilation carousel — skip script gen (portal1), video reviewer (portal1b),
  // and avatar render (portal2); assembly fires after portal0, then 3a/3b/4/5 run normally.
  const isCarousel   = contentType === 'compilation_carousel';

  const portal1Active  = !isCarousel && (isCommentary || hasScriptProvider);
  const portal1bActive = !isCarousel && (isCommentary || !!(providers.videoReviewer));
  const portal2Active  = !isCarousel && (isCommentary || hasAvatarProvider || (stageMap?.avatar?.active === true));
  const portal3Active  = isCommentary || isCarousel || hasAssemblyProvider;
  // portal4 is a broadcast commentary QA gate — not applicable for pure clip compilations
  // where there is no scripted commentary to verify against source material.
  const isClipsOnly   = contentType === 'clips';
  const portal4Active = !isClipsOnly && (isCommentary || isCarousel || (hasAssemblyProvider && !expectedSynth));
  const portal5Active = isCommentary || isCarousel || hasUploadProvider;
...
function buildExtensionsMap(providers) {
  return {
    ...
    // CPD-431: GPT-4o deep quality + creative review — always ordered; self-skips if OPENAI_API_KEY absent.
    gpt4o_qa_ext: {
      key: 'gpt4o_qa_ext',
      label: 'GPT-4o Production Quality Review (Extension)',
      ordered: true,  // always runs — extension skips gracefully when key not set
      provider: 'openai',
      reason: 'Extension: deep quality + creative review via GPT-4o + Whisper. Non-blocking.',
    },
    // CPD-431: Twelve Labs Pegasus video QA — always ordered; self-skips if TWELVE_LABS_API_KEY absent.
    twelvelabs_qa_ext: {
      key: 'twelvelabs_qa_ext',
      label: 'Twelve Labs Pegasus Video QA (Extension)',
      ordered: true,  // always runs — extension skips gracefully when key not set
      provider: 'twelvelabs',
      reason: 'Extension: visual/audio/production/content quality analysis via Pegasus 1.2. Non-blocking.',
    },
  };
}
...
function createJobSpec(params) {
  const {
    ...
    brandName = null,  // CPD-1006: display name for chrome lower-third
    templateId,
    contentType,
    createdBy = 'api',
    sourceType = 'none',
    ...
    publishMeta = {},  // CPD-505: customer-provided publish metadata (title, description, tags, privacy, tiktokCaption)
    productionProfile: _productionProfileParam = null,  // CPD-872: optional — derived from contentType if not provided
  } = params;
  ...
  // CPD-872 fix: resolve productionProfile — use caller-supplied value or derive from contentType.
  // Matches LEGACY_CONTENT_TYPE_TO_PRODUCTION_PROFILE in pipeline_routing.js (no import to avoid circular dep).
  const _CT_TO_PROFILE = { clips: 'vertical_reel', news: 'broadcast_desk', sports: 'live_event' };
  const productionProfile = _productionProfileParam || _CT_TO_PROFILE[baseContentType] || 'broadcast_desk';
  ...
  const jobSpec = {
    ...
    order: {
      ...
      // CPD-505: customer-provided publish metadata — used by portal5 before fallback to publishCopy
      publish: {
        title:          publishMeta.title          || null,
        description:    publishMeta.description    || null,
        tags:           Array.isArray(publishMeta.tags) ? publishMeta.tags : (publishMeta.tags ? String(publishMeta.tags).split(',').map(t => t.trim()).filter(Boolean) : []),
        privacyStatus:  publishMeta.privacyStatus  || publishMeta.privacy || null,
        tiktokCaption:    publishMeta.tiktokCaption    || null,
        instagramCaption: publishMeta.instagramCaption  || null,
        scheduledPublishAt: publishMeta.scheduledPublishAt || null,
      },
    },
    ...
    addOns: {
      ...
      // CPD-947: Gemini autonomous clip picker — persisted so worker.js can read it
      clipSourcing: {
        active: !!(addOns?.clipSourcing?.active === true),
      },
    },
    ...
    // CPD-872: Feature list baked in at creation — portal_gpt4o_qa.js verifies each is present
    requiredFeatures: deriveRequiredFeatures({
      addOns,
      extensions: (() => {
        const ext = buildExtensionsMap(providers);
        if (addOns?.heygen?.active === true) ext.heygen_ext.ordered = true;
        if (addOns?.tts?.active === true) ext.tts_ext.ordered = true;
        return ext;
      })(),
      designSpec,
      templateId: resolvedTemplateId,
  ...
  try {
    ...
    try {
      ...
      // Detect source platform from URLs so platform brand colors are baked into the frozen cfg.
      const _freezeUrls = [
        ...(sourceConfig?.urls || []),
        ...(jobSpec?.order?.inputs?.sources || []),
      ];
      const _freezePlatform = detectSourcePlatform(_freezeUrls)
        || (contentType === 'clips' && chromeContentType === 'clips' ? null : null);
      if (_freezePlatform && !jobSpec.sourcePlatform) jobSpec.sourcePlatform = _freezePlatform;
      const resolvedChromeCfg = resolveChromeCfg(customerConfig, chromeContentType, _freezePlatform);
...
function updateJobSpec(jobId, patch) {
  ...
  if (!current) {
    ...
    seedJobSpecFromScript(jobId, { jobId, customerId: _card.customerId , templateId: null }).catch(() => {});
...
function saveGateResult(jobId, gate, result) {
  ...
  if (!current) {
    ...
    if (_card2) {
      ...
      _seed(jobId, { jobId, customerId: _card2.customerId , templateId: null }).catch(() => {});
      current = getJobSpec(jobId);
    }
  }
  if (!current) {
...
function failJob(jobId, gate, rootCause, restartGate) {
  ...
  if (!current) {
    ...
    if (_cardFail) {
      seedJobSpecFromScript(jobId, { jobId, customerId: _cardFail.customerId , templateId: null }).catch(() => {});
      current = getJobSpec(jobId);
    }
  }
  if (!current) {
