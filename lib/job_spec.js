 ...
 */
function buildPortalMap({ providers, sourceType, stageMap, expectedSynth }) {
  const hasAvatarProvider = !!(providers.avatar || stageMap?.avatar?.provider);
  const hasUploadProvider = !!(providers.upload || stageMap?.upload?.provider);
  const hasScriptProvider = !!(providers.script || stageMap?.script?.provider);
  const hasAssemblyProvider = !!(providers.assembly || stageMap?.assembly?.provider);

  // Portal 1 is skippable if the customer provides their own script
  // (sourceType='upload' does not by itself skip script gen — only an explicit own_script flag would;
  //  for now we activate script gen whenever a provider is configured)
  const portal1Active = hasScriptProvider;
  const portal1bActive = !!(providers.videoReviewer); // not yet wired in C0

  // Portal 2 only fires when segments exist (requires avatar/render provider)
  const portal2Active = hasAvatarProvider || (stageMap?.avatar?.active === true);
  ...
  return {
    ...
    portal3a: {
      key: 'portal3a',
      label: 'Assembly QA',
      active: hasAssemblyProvider,
      skippable: false,
      provider: hasAssemblyProvider ? 'gemini' : null,
      reason: hasAssemblyProvider ? null : 'No assembly provider — portal skipped',
    },
    portal3b: {
      key: 'portal3b',
      ...
      active: hasAssemblyProvider,
      skippable: false,
      provider: hasAssemblyProvider ? 'internal' : null,
      reason: hasAssemblyProvider ? null : 'No assembly provider — portal 3b skipped',
    },
    portal4: {
      key: 'portal4',
      label: 'Broadcast Ready',
      active: hasAssemblyProvider && !expectedSynth,
      skippable: false,
      provider: hasAssemblyProvider ? 'gemini' : null,
      reason: expectedSynth
    ...
    portal5: {
      ...
      active: hasUploadProvider,
      skippable: true,
      provider: hasUploadProvider ? (providers.upload || 'upload_post') : null,
      reason: hasUploadProvider ? null : 'No upload provider configured — portal skipped',
    },
  };
}
...
function createJobSpec(params) {
  ...
  const jobSpec = {
    ...
    portals: buildPortalMap({ providers, sourceType, stageMap: mergedStageMap, expectedSynth }),

    // extensions — add-ons activated by the job spec (not always-on portals)
    extensions: buildExtensionsMap(providers),
...
function linkScriptJob(semanticJobId, scriptJobId) {
  try {
    ...
    const r = db
      ...
      .run(scriptJobId, semanticJobId);
    if (!r.changes) {
      console.warn(
        `[job_spec] linkScriptJob: no jobs row updated for semantic id ${semanticJobId}`
      );
      return;
    }
