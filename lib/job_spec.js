...
function createJobSpec(params) {
  ...
  const preClipsPerStreamer = params.clipsPerStreamer
    || (preItems[0]?.targetClipsPerStreamer > 0 ? preItems[0].targetClipsPerStreamer : undefined)
    || (preItems[0]?.clipsPerStreamer > 0 ? preItems[0].clipsPerStreamer : undefined)
    || (preItems[0]?.clips?.length > 0 ? preItems[0].clips.length : undefined);
      ...
      const scaffoldJobSpec = {
        ...
        order: {
          ...
          inputs: {
            ...
            items: mappedItems,
            ...(preClipsPerStreamer ? { clipsPerStreamer: preClipsPerStreamer } : {}),
          }
        }
      };
...
function syncJobCardScriptGateSnapshot(jobId) {
  try {
    ...
    const { loadJob, getGateResults, saveJob } = require('./db');
    ...
    console.warn(`[job_spec] syncJobCardScriptGateSnapshot(${jobId}): ${e.message}`);
  }
}
...
function appendGate1ScriptAttempt(jobId, { attempt, script }) {
  ...
  const { updateJobSpec } = require('./db');
