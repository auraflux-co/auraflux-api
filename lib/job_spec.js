...
function buildPortalsMap(jobSpec = {}) {
  ...
  // Peaks / clip-comp rewrites contentType to twitch-short|news-short|sports-short after create —
  // still treat as clip path so portal4 stays on (CPD-1046).
  const isClipComp =
    isClips ||
    jobSpec.productionPath === 'short_compile_clips' ||
    !!jobSpec.compositionSpec?.clipComp ||
  ...
  if (expected) {
    ...
    defaults = {
      portal0:  true,
      portal1:  scriptActive && !isClipComp,
      portal1b: false,
      portal2:  avatarActive,
      portal3a: true,
      portal3b: true,
      portal4:  !isClipComp && scriptActive,
      portal5:  !staging,
    };
  }

  if (isClipComp || !scriptActive) {
    ...
    if (isClipComp) defaults.portal4 = true;
  ...
  for (const key of PORTAL_ORDER) {
    ...
    portals[key] = {
      ...
      reason: active ? null : (staging && key === 'portal5'
        ...
        : isClipComp && key === 'portal4' && !active ? 'clip compilation — portal4 inactive (override)'
        : null),
    };
  }
