 ...
 *   pipeline:retry_attempt { jobId, gate, stage, attempt, maxAttempts } — uniform retry marker (e.g. Gate 1 script loop)
 ...
 *   publish:platform_done { jobId, platform, request_id, outcome } — Gate 5 per-platform Upload-Post poll terminal
 *   publish:all_done       { jobId, anySuccess, allFailed, platforms } — Gate 5 finished all platform uploads
...
const TIMELINE_STRING_MAX = Math.max(500, parseInt(process.env.JOB_TIMELINE_STRING_MAX || '4000', 10) || 4000);
const TIMELINE_ARRAY_MAX  = Math.max(4, parseInt(process.env.JOB_TIMELINE_ARRAY_MAX || '16', 10) || 16);
...
function appendJobRunTimeline(type, payload) {
  try {
    ...
    const extra = (payload != null && typeof payload === 'object' && !Array.isArray(payload))
  ...
  if (data != null && typeof data === 'object' && !Array.isArray(data)) {
    ...
    if (payload.jobId && typeof payload.jobId === 'string') {
      try {
        ...
        payload.canonicalJobId = resolveCanonicalJobId(payload.jobId);
      } catch (_e) {
