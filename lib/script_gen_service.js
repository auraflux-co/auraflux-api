...
async function generateJobScript(jobSpec) {
  ...
  if (!urls.length) throw new Error('[script_gen_service] No source URLs in job spec');
  ...
  const orderedClipUrls = analyses.map((a, i) => ({
    url:              a.url,
    clipUrl:          a.url,
    storyIndex:       i,
    pageUrl:          '',
  ...
  const userPrompt   = _buildUserPrompt({ topic, tone, contentType, formType, analyses: analysisBlock, clipCount: urls.length });
...
function _buildUserPrompt({ topic, tone, contentType, formType, analyses, clipCount }) {
  const isShort = formType === 'short' || contentType.includes('-short');
  const sectionNames = Array.from({ length: clipCount }, (_, i) => `STORY${i + 1}_CLIP`).join(', ');
