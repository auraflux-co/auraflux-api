...
function formatRecommendPromptBlock(ctx = {}) {
  if (!ctx.sampleSize) {
    const fallback = ['(No Content Memory yet — use category guidance above; avoid generic compilation titles.)'];
    if (ctx.competitorPatterns) fallback.push('', ctx.competitorPatterns);
  ...
  if (ctx.competitorPatterns) {
    lines.push('', ctx.competitorPatterns);
  }
