...
function resolveChromeCfg(customerConfig, contentType, sourcePlatform) {
  ...
  // Apply platform brand colors (lowest priority override — customer config wins if it
  // explicitly sets contentTypeOverrides for this platform/contentType).
  // Only applies when sourcePlatform is known AND the customer config didn't already
  // set platform-specific color overrides.
  if (sourcePlatform && PLATFORM_CHROME_COLORS[sourcePlatform] && !overrides.colors) {
