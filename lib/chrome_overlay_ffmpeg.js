...
if (!FONT_AVAILABLE.headline || !FONT_AVAILABLE.body || !FONT_AVAILABLE.small) {
  console.warn(
    '[chrome-ffmpeg] WARN: Custom font files missing. Falling back to FFmpeg built-in sans.'
  );
...
// ── Platform brand palettes — applied on top of customer colors when sourcePlatform is set ──
// Colors are chosen for legibility on the dark broadcast chrome (dark background, accent readability).
const PLATFORM_CHROME_COLORS = {
  twitch:    { primary: '#6441a5', accent: '#bf94ff', active: '#9147ff', text: '#ffffff', background: '#0e0e10' },
  youtube:   { primary: '#cc0000', accent: '#ffffff', active: '#ff0000', text: '#ffffff', background: '#0f0f0f' },
  kick:      { primary: '#1a3a1f', accent: '#53fc18', active: '#53fc18', text: '#ffffff', background: '#0d0d0d' },
  instagram: { primary: '#833ab4', accent: '#fd1d1d', active: '#fcb045', text: '#ffffff', background: '#1a1a2e' },
  tiktok:    { primary: '#010101', accent: '#69c9d0', active: '#ee1d52', text: '#ffffff', background: '#010101' },
  twitter:   { primary: '#15202b', accent: '#1da1f2', active: '#1da1f2', text: '#ffffff', background: '#15202b' },
  facebook:  { primary: '#1877f2', accent: '#ffffff', active: '#42b72a', text: '#ffffff', background: '#18191a' },
};

/**
 * Detect source platform from a list of URLs or inventory paths.
 * Returns the platform slug ('twitch'|'youtube'|'kick'|'instagram'|'tiktok'|null).
 */
function detectSourcePlatform(urls) {
  const list = (Array.isArray(urls) ? urls : [urls]).filter(Boolean).map(String);
  if (list.some((u) => /twitch\.tv|video_inventory\/twitch\//i.test(u)))    return 'twitch';
  if (list.some((u) => /youtube\.com|youtu\.be|video_inventory\/youtube\//i.test(u))) return 'youtube';
  if (list.some((u) => /kick\.com|video_inventory\/kick\//i.test(u)))       return 'kick';
  if (list.some((u) => /instagram\.com|video_inventory\/instagram\//i.test(u))) return 'instagram';
  if (list.some((u) => /tiktok\.com|video_inventory\/tiktok\//i.test(u)))   return 'tiktok';
  if (list.some((u) => /twitter\.com|x\.com|video_inventory\/twitter\//i.test(u))) return 'twitter';
  return null;
}

// ── Resolve effective chromeCfg from customerConfig + contentType overrides ─
function resolveChromeCfg(customerConfig, contentType, sourcePlatform) {
  ...
  const cfg = {
    ...
    flag: {
      ...
      categoryLabel: base.flag?.categoryLabel || {
        news:   'BREAKING NEWS',
        clips:  'HIGHLIGHTS',
        sports: 'SPORTS',
      },
  ...
  // Apply platform brand colors — source platform is the most specific signal we have.
  // This overrides generic contentTypeOverrides.colors (e.g. all-clips purple) with the
  // actual brand color of the originating platform (e.g. YouTube red, Kick green).
  // Customer can still set contentTypeOverrides.platformColors.{platform} for explicit
  // per-platform overrides that take priority over the built-in PLATFORM_CHROME_COLORS defaults.
  const platformColorOverride = overrides.platformColors?.[sourcePlatform];
  if (platformColorOverride) {
    cfg.colors = { ...cfg.colors, ...platformColorOverride };
  } else if (sourcePlatform && PLATFORM_CHROME_COLORS[sourcePlatform]) {
    cfg.colors = { ...cfg.colors, ...PLATFORM_CHROME_COLORS[sourcePlatform] };
  }
...
function fingerprintResolvedChromeCfg(cfg) {
  ...
  return crypto.createHash('sha1').update(JSON.stringify(pick)).digest('hex').slice(0, 12);
}
...
function buildChromeFilterChain(chromeParams, chromeCfg) {
  const {
    ...
    showName = chromeCfg.name || 'AuraFlux',
    dateStr = new Date()
  ...
  // Episode number (left)
  filters.push(
    `drawtext=${FF_HEADLINE}text='${escapeDrawtext(episodeNumber)}':fontsize=22:fontcolor=${COL_EPISODE_SHOW}:x=32:y=14`
  );
  // Show name (center)
  filters.push(
    `drawtext=${FF_HEADLINE}text='${escapeDrawtext(showName)}':fontsize=22:fontcolor=${COL_EPISODE_SHOW}:x=(w-text_w)/2:y=14`
  );
  // LIVE badge
  ...
  // Date (right of LIVE)
  filters.push(
    `drawtext=${FF_HEADLINE}text='${escapeDrawtext(dateStr)}':fontsize=18:fontcolor=${COL_TOP_TEXT}:x=1740:y=15`
  );
  ...
  if (showFlag) {
    ...
    const splitAt =
      Number(chromeCfg.flag.twoLineSplitAtChars) > 0
        ? Number(chromeCfg.flag.twoLineSplitAtChars)
        : 24;
    const fs1 =
      Number(chromeCfg.flag.fontSizeSingleLine) > 0
        ? Number(chromeCfg.flag.fontSizeSingleLine)
        : 28;
    const fs2 =
      Number(chromeCfg.flag.fontSizeDoubleLine) > 0
    ...
    filters.push(
      `drawbox=x=${flagLm}:y=${flagTop0}:w=${flagW}:h=${row1H}:color=${COL_ACCENT}:t=fill`
    );
    filters.push(
      `drawtext=${FF_SMALL}text='${escapeDrawtext(category)}':fontsize=14:fontcolor=${COL_FLAG_CAT}:x=${flagLm + 32}:y=${flagTop0 + 12}`
    );
    ...
    filters.push(
      `drawbox=x=${flagLm}:y=${row2Y}:w=${flagW}:h=${row2Height}:color=${COL_PRIMARY}:t=fill`
    );
    filters.push(
      `drawbox=x=${flagLm}:y=${row2Y}:w=${chromeCfg.flag.borderWidth}:h=${row2Height}:color=${COL_ACCENT}:t=fill`
    );
    filters.push(
      `drawtext=${FF_BODY}text='${escapeDrawtext(titleLine1)}':fontsize=${titleFs}:fontcolor=${COL_TEXT}:x=${flagLm + 16}:y=${line1Y}`
    );
    if (needsTwoLines && titleLine2) {
      filters.push(
        `drawtext=${FF_BODY}text='${escapeDrawtext(titleLine2)}':fontsize=${titleFs}:fontcolor=${COL_TEXT}:x=${flagLm + 16}:y=${line2Y}`
      );
    }
  }
  ...
  if (showSidebar) {
    ...
    const panelH = chromeCfg.sidebar.maxItems * sbIH + (chromeCfg.sidebar.maxItems - 1) * sbGap;
    // Always draw sidebar shell so chrome remains visible even with 0 items.
    // Keep sidebar shell visibly present for vision QA even when item count is zero.
    filters.push(
      `drawbox=x=${sbLeft}:y=${sbTop}:w=${sbW}:h=${panelH}:color=${cssToFfmpeg(c.background, '0.62')}:t=fill`
    );
    ...
    if (items.length === 0) {
      filters.push(
        `drawbox=x=${sbLeft + 12}:y=${sbTop + 10}:w=${sbW - 24}:h=40:color=${COL_DARK}:t=fill`
      );
      filters.push(
        `drawtext=${FF_SMALL}text='ON AIR':fontsize=18:fontcolor=${COL_ACCENT}:x=${sbLeft + 22}:y=${sbTop + 18}`
      );
    }
    items.forEach((item, i) => {
      ...
      if (isActive) {
        ...
        filters.push(
          `drawbox=x=${sbLeft}:y=${itemY}:w=${sbW}:h=${sbIH}:color=${COL_PRIMARY}:t=fill`
        );
        filters.push(
          `drawbox=x=${sbLeft}:y=${itemY}:w=${chromeCfg.sidebar.activeBorderWidth}:h=${sbIH}:color=${COL_ACTIVE}:t=fill`
        );
        filters.push(
          `drawtext=${FF_SMALL}text='\\u25B6 ON AIR':fontsize=12:fontcolor=${COL_ACTIVE}:x=${sbLeft + 14}:y=${itemY + 10}`
        );
        filters.push(
          `drawtext=${FF_BODY}text='${itemTitle}':fontsize=22:fontcolor=${COL_TEXT}:x=${sbLeft + 14}:y=${itemY + 30}`
        );
      } else {
        // Inactive item — dark background + accent border
        filters.push(`drawbox=x=${sbLeft}:y=${itemY}:w=${sbW}:h=${sbIH}:color=${COL_DARK}:t=fill`);
        filters.push(
          `drawbox=x=${sbLeft}:y=${itemY}:w=${chromeCfg.sidebar.borderWidth}:h=${sbIH}:color=${COL_ACCENT}:t=fill`
        );
        filters.push(
          `drawtext=${FF_SMALL}text='${itemCategory}':fontsize=12:fontcolor=${COL_ACCENT}:x=${sbLeft + 14}:y=${itemY + 10}`
        );
        filters.push(
          `drawtext=${FF_BODY}text='${itemTitle}':fontsize=22:fontcolor=${COL_TEXT}:x=${sbLeft + 14}:y=${itemY + 30}`
        );
  ...
  if (logoExists) {
    ...
    filters.push(
      `drawbox=x=${logoX - 4}:y=${logoY - 4}:w=${logoSize + 8}:h=${logoSize + 8}:color=${COL_BACKGROUND}:t=fill`
    );
    filters.push(
      `drawbox=x=${logoX - 4}:y=${logoY - 4}:w=${logoSize + 8}:h=3:color=${COL_ACCENT}:t=fill`
    );
    filters.push(
      `drawbox=x=${logoX - 4}:y=${logoY - 4}:w=3:h=${logoSize + 8}:color=${COL_ACCENT}:t=fill`
    );
    ...
    const logoY = chromeCfg.logo.y ?? 910;
    filters.push(
      `drawbox=x=${logoX}:y=${logoY}:w=${logoSize}:h=${logoSize}:color=${COL_PRIMARY}:t=fill`
    );
    ...
    filters.push(
      `drawtext=${FF_HEADLINE}text='CWN':fontsize=24:fontcolor=${COL_ACCENT}:x=${logoX + 12}:y=${logoY + 30}`
    );

    // No logo — just the draw filters
    if (logoAssetRel && !logoExists) {
      console.warn(
        `[chrome-ffmpeg] WARN: Logo asset not found: ${logoAsset} — skipping logo overlay`
      );
    }
    const drawChain = filters.join(',');
    extraInputs = [];
