...
function buildChromeFilterChain(chromeParams, chromeCfg) {
  ...
  if (showFlag) {
    ...
    // Flag height is fixed at spec max — never expand, reduce font for 2-line titles
    const totalFlagH = flagH; // always 88px — Bobby G face starts at y=120, topBar=48, flag=88 → bottom=136
    ...
    // 2-line titles use fontsize=20 so both lines fit within fixed 50px row height
    const row2Y    = topBarH + row1H;
    const titleFs  = needsTwoLines ? 20 : 28;
    ...
    filters.push(`drawtext=${FF_BODY}text='${escapeDrawtext(titleLine1)}':fontsize=${titleFs}:fontcolor=${COL_TEXT}:x=16:y=${row2Y + 6}`);
    if (needsTwoLines && titleLine2) {
      filters.push(`drawtext=${FF_BODY}text='${escapeDrawtext(titleLine2)}':fontsize=${titleFs}:fontcolor=${COL_TEXT}:x=16:y=${row2Y + 28}`);
    }
  }
