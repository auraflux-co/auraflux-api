...
function resolveChromeCfg(customerConfig, contentType) {
  ...
  const cfg = {
    ...
    sidebar: {
      ...
      thumbWidth:      base.sidebar?.thumbWidth      || 120,
      thumbHeight:     base.sidebar?.thumbHeight     || 68,
      thumbPadLeft:    base.sidebar?.thumbPadLeft    || 14,
      thumbTextGap:    base.sidebar?.thumbTextGap    || 10
    },
    logo: {
...
function buildChromeFilterChain(chromeParams, chromeCfg) {
  ...
  const thumbW   = chromeCfg.sidebar.thumbWidth  || 120;
  const thumbH   = chromeCfg.sidebar.thumbHeight || 68;
  const thumbPad = chromeCfg.sidebar.thumbPadLeft || 14;
  const thumbGap = chromeCfg.sidebar.thumbTextGap || 10;
  ...
  if (showSidebar) {
    ...
    items.forEach((item, i) => {
      ...
      const itemTitle    = escapeDrawtext(truncate(item.title || '', 28));
      const itemCategory = escapeDrawtext((item.category || '').toUpperCase());
      const hasThumb     = item.thumbPath && fs.existsSync(item.thumbPath);
      const textX        = sbLeft + thumbPad + (hasThumb ? thumbW + thumbGap : 0);
      ...
      if (isActive) {
        ...
        if (hasThumb) {
          const ty = itemY + Math.floor((sbIH - thumbH) / 2);
          filters.push(`drawbox=x=${sbLeft + thumbPad}:y=${ty}:w=${thumbW}:h=${thumbH}:color=${COL_DARK}:t=fill`);
          thumbOverlays.push({ path: item.thumbPath, x: sbLeft + thumbPad, y: ty });
        }
        filters.push(`drawtext=${FF_SMALL}text='\\u25B6 ON AIR':fontsize=12:fontcolor=${COL_ACTIVE}:x=${textX}:y=${itemY + 10}`);
        filters.push(`drawtext=${FF_BODY}text='${itemTitle}':fontsize=22:fontcolor=${COL_TEXT}:x=${textX}:y=${itemY + 30}`);
      } else {
        ...
        filters.push(`drawtext=${FF_SMALL}text='${itemCategory}':fontsize=12:fontcolor=${COL_ACCENT}:x=${textX}:y=${itemY + 10}`);
        filters.push(`drawtext=${FF_BODY}text='${itemTitle}':fontsize=22:fontcolor=${COL_TEXT}:x=${textX}:y=${itemY + 30}`);
      }
    });
  ...
  // ── Logo shell + sidebar thumb filter chain ────────────────────────────────
  ...
  const extraInputs = [];
  thumbOverlays.forEach((t) => {
    extraInputs.push('-loop', '1', '-framerate', '30', '-i', t.path);
  });
  if (logoExists) {
    extraInputs.push('-loop', '1', '-framerate', '30', '-i', logoAsset);
  }

  const parts = [`[0:v]${drawChain}[base]`];
  let stream = 'base';
  let inputIdx = 1;

  thumbOverlays.forEach((t, ti) => {
    const scaled = `thumb${ti}`;
    const out = `vthumb${ti}`;
    parts.push(`[${inputIdx}:v]scale=${thumbW}:${thumbH}[${scaled}]`);
    parts.push(`[${stream}][${scaled}]overlay=x=${t.x}:y=${t.y}:shortest=1:format=auto[${out}]`);
    stream = out;
    inputIdx += 1;
  });

  if (logoExists) {
    const logoScale = logoOpacity < 1.0
      ? `[${inputIdx}:v]scale=${logoSize}:-2,format=rgba,colorchannelmixer=aa=${logoOpacity}[logo]`
      : `[${inputIdx}:v]scale=${logoSize}:-2,format=rgba[logo]`;
    parts.push(logoScale);
    parts.push(`[${stream}][logo]overlay=x=${logoX}:y=${logoY}:shortest=1:format=auto[out]`);
  } else if (thumbOverlays.length > 0) {
    const last = parts.pop();
    parts.push(last.replace(`[${stream}]`, `[base]`).replace(new RegExp(`\\[${stream}\\]$`), '[out]'));
  } else {
    parts[0] = `[0:v]${drawChain}[out]`;
  }
