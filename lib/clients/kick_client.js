...
class KickClient {
  ...
  async getClips(username, limit = DEFAULT_LIMIT) {
    ...
    return items.slice(0, cap).map((c) => ({
      ...
      // CPD-351: Use stable page URL as primary so yt-dlp handles downloads at assembly time.
      // Kick direct API c.clip_url is usually a CDN URL that may expire; page URL is stable.
      url:          `https://kick.com/clip/${c.id}`,
      cdnUrl:       c.clip_url || null,
