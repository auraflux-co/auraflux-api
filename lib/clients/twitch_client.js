...
// Process-level token cache so a single refresh serves all concurrent requests
...
class TwitchClient {
  constructor(options = {}) {
    ...
    // Prefer the process-level cached token; fall back to env var for backwards compat
    this.token = options.token || _cachedToken || process.env.TWITCH_TOKEN;
  ...
  /**
   * Obtain a fresh app access token using the client credentials grant.
   * Serialised so multiple concurrent requests share one refresh round-trip.
   * @returns {Promise<string>} New access token
   */
  async refreshToken() {
    if (!this.clientId || !this.clientSecret) {
      throw new Error('TWITCH_CLIENT_ID and TWITCH_CLIENT_SECRET are required to refresh the token');
    }
    ...
    _tokenRefreshPromise = (async () => {
      try {
        const resp = await axios.post(
          `https://id.twitch.tv/oauth2/token?client_id=${encodeURIComponent(this.clientId)}&client_secret=${encodeURIComponent(this.clientSecret)}&grant_type=client_credentials`,
          null,
          { timeout: 10000 },
        );
        const newToken = resp.data?.access_token;
        if (!newToken) throw new Error('Twitch token refresh returned no access_token');
        _cachedToken = newToken;
        this.token = newToken;
        console.log('[TwitchClient] App access token refreshed successfully');
        return newToken;
      } finally {
  ...
  /**
   * Return the current bearer token, refreshing if it is expired/missing.
   * Falls back to TWITCH_TOKEN env var if no secret is available (legacy).
   * @returns {Promise<string>}
   */
  async ensureToken() {
    if (this.token) {
      // Quick validate — Twitch tokens expire after 60 days
      const valid = await this.validateToken();
      if (valid) return this.token;
      console.warn('[TwitchClient] Token invalid/expired — attempting refresh');
    }
    if (this.clientSecret) {
      return this.refreshToken();
    }
    throw new Error('Twitch token is missing or expired. Set TWITCH_CLIENT_SECRET to enable auto-refresh.');
  }
  ...
  async getUserByLogin(login) {
    if (!this.clientId) {
      throw new Error('TWITCH_CLIENT_ID required for Helix API');
    }
   ...
   */
  async getClips(broadcasterId, count = 10, startedAt = null) {
    if (!this.clientId) {
      throw new Error('TWITCH_CLIENT_ID required for Helix API');
    }
    const token = await this.ensureToken();
    ...
    // that may have been deleted. Use the caller-supplied startedAt when available
    // (matches the user's date-range filter so recent clips aren't buried behind
    // high-view-count older clips). Fall back to 90 days for "All time" browsing.
    const since = startedAt || new Date(Date.now() - 90 * 24 * 60 * 60 * 1000).toISOString();
    return withRetry(
      async () => {
        const resp = await axios.get(
          `https://api.twitch.tv/helix/clips?broadcaster_id=${broadcasterId}&first=${Math.min(count, 100)}&started_at=${since}`,
          {
            headers: {
              'Client-Id': this.clientId,
              Authorization: `Bearer ${token}`,
