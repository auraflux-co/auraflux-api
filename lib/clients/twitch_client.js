...
class TwitchClient {
  ...
  /**
   * Get recent VODs (archives) for a broadcaster via Helix.
   * @param {string} broadcasterId - Twitch user id
   * @param {number} count - max 100
   * @param {{ type?: 'archive'|'highlight'|'upload' }} [opts]
   * @returns {Promise<Array>}
   */
  async getVideos(broadcasterId, count = 20, { type = 'archive' } = {}) {
    if (!this.clientId || !this.token) {
      throw new Error('Twitch Client ID and Token required for Helix API');
    }
    if (!broadcasterId) throw new Error('broadcasterId required');

    await this.checkRateLimit();

    const target = Math.min(Math.max(1, Number(count) || 20), 500);
    return withRetry(
      async () => {
        const out = [];
        let cursor = null;
        while (out.length < target) {
          const first = Math.min(100, target - out.length);
          ...
          const resp = await this._helixGet(`https://api.twitch.tv/helix/videos?${qs}`);
