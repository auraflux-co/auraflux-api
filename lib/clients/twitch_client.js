...
/** Shared Helix window — all TwitchClient instances share one limit (avoids 429 storms). */
const _sharedHelixWindow = [];
const SHARED_HELIX_MAX_PER_MIN = Number(process.env.TWITCH_HELIX_MAX_PER_MIN) || 400;

async function sharedHelixRateLimit(logLabel = 'TwitchClient') {
  const now = Date.now();
  const oneMinuteAgo = now - 60000;
  while (_sharedHelixWindow.length && _sharedHelixWindow[0] <= oneMinuteAgo) {
    _sharedHelixWindow.shift();
  }
  if (_sharedHelixWindow.length >= SHARED_HELIX_MAX_PER_MIN) {
    const waitMs = _sharedHelixWindow[0] + 60000 - now;
    if (waitMs > 0) {
      console.log(`[${logLabel}] Shared Helix rate limit, waiting ${waitMs}ms`);
      await new Promise((resolve) => setTimeout(resolve, waitMs));
    }
  }
  _sharedHelixWindow.push(Date.now());
}
...
class TwitchClient {
  constructor(options = {}) {
    ...
    this.maxRequestsPerMinute = options.maxRequestsPerMinute || SHARED_HELIX_MAX_PER_MIN;
  }
  ...
  async checkRateLimit() {
    await sharedHelixRateLimit('TwitchClient');
  }
...
function getSharedTwitchClient() {
  if (!_defaultClient) _defaultClient = new TwitchClient();
  return _defaultClient;
}

module.exports = TwitchClient;
module.exports.getSharedTwitchClient = getSharedTwitchClient;
module.exports.sharedHelixRateLimit = sharedHelixRateLimit;
