...
class GeminiClient {
  ...
  async uploadFile(filePath) {
    ...
    if (fileSize > this.maxFileSize) {
      throw new Error(`File size ${(fileSize/1024/1024).toFixed(1)}MB exceeds ${(this.maxFileSize/1024/1024)}MB limit`);
    }
    ...
    try {
      ...
      console.log(`[GeminiClient] Uploaded file: ${file.name} (${(fileSize/1024/1024).toFixed(1)}MB)`);
  ...
  async deleteFile(fileName) {
    try {
      await axios.delete(
        `${this.baseUrl}/v1beta/${fileName}?key=${this.apiKey}`,
        { timeout: 10000 }
      );
