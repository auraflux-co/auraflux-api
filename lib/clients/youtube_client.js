...
const MAX_LIMIT = 50;
/** Max videos when paging the channel uploads playlist (Clip Picker full catalog). */
const MAX_PLAYLIST_PAGES = 20; // 20 × 50 = 1000
...
class YouTubeClient {
  ...
  /**
   * Uploads playlist id (UU…) for a channel — cheap alternative to search.list.
   * Cost: 1 unit (channels.list contentDetails).
   */
   ...
   * opts.publishedAfter — ISO date filter (skip older than this while paging newest-first)
    ...
    while (out.length < target && pages < MAX_PLAYLIST_PAGES) {
      ...
      for (const vid of videoIds) {
        ...
        if (publishedAfter && pubAt && pubAt < publishedAfter) {
          // Uploads playlist is newest-first — older than window means stop paging.
  ...
  /**
   * Full channel uploads via UU playlist (1 unit/page). Falls back to search.list.
   */
  async getChannelUploads(channelId, limit = DEFAULT_LIMIT, opts = {}) {
    const uploadsId = await this.getUploadsPlaylistId(channelId);
    if (!uploadsId) {
      return this.getContent(channelId, Math.min(limit, MAX_LIMIT), opts);
    }
    const { type = 'all' } = opts;
    // Over-fetch when filtering Shorts so long-form can still fill `limit`.
    const scan = type === 'video' || type === 'short'
      ? Math.min(Math.max(limit * 3, limit), MAX_UPLOADS_LIMIT)
      : Math.min(Math.max(limit, 1), MAX_UPLOADS_LIMIT);
    const raw = await this.getPlaylistVideos(uploadsId, scan, opts);
    if (type === 'short') return raw.filter((v) => v.isShort).slice(0, limit);
    if (type === 'video') return raw.filter((v) => !v.isShort).slice(0, limit);
