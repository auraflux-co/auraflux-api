...
const MAX_LIMIT = 50;
/** Uploads playlist scan cap for Peaks All-time / wide windows (C0 parity). */
const MAX_UPLOADS_LIMIT = 500;
const MAX_PLAYLIST_PAGES = 20;
const MAX_LONG_RETURN = 200;
...
class YouTubeClient {
  ...
  /**
   * Recent long-form uploads (≥ minDurationSec, not Shorts).
   * Prefers the channel uploads playlist (paginated) over search.list.
   * opts: { minDurationSec, maxDurationSec, publishedAfter, limit }
   * Third-arg number still accepted as minDurationSec for backward compat.
   */
  async getRecentLongVideos(channelId, limitOrOpts = DEFAULT_LIMIT, minDurationSecOrOpts = 180) {
    if (!this.apiKey) throw new Error('YOUTUBE_API_KEY not configured');
    let opts = {};
    let want = DEFAULT_LIMIT;
    if (limitOrOpts && typeof limitOrOpts === 'object') {
      opts = limitOrOpts;
      want = opts.limit != null ? opts.limit : DEFAULT_LIMIT;
    } else {
      want = limitOrOpts;
      if (minDurationSecOrOpts && typeof minDurationSecOrOpts === 'object') {
        opts = minDurationSecOrOpts;
      } else {
        opts = { minDurationSec: minDurationSecOrOpts };
      }
    }
    const minDurationSec = opts.minDurationSec != null ? Number(opts.minDurationSec) : 180;
    const maxDurationSec = opts.maxDurationSec != null && opts.maxDurationSec !== ''
      ? Number(opts.maxDurationSec)
      : null;
    const publishedAfter = opts.publishedAfter || null;
    want = Math.min(Math.max(1, Number(want) || DEFAULT_LIMIT), MAX_LONG_RETURN);
    // Oversample raw uploads so Shorts-heavy channels still fill `want` long-form.
    const scan = Math.min(Math.max(want * 3, 50), MAX_UPLOADS_LIMIT);

    /** @type {Array<{ isShort?: boolean, duration?: number, id?: string, title?: string, thumbnailUrl?: string, publishedAt?: string, url?: string, viewCount?: number }> } */
    let candidates = [];
    try {
      const chRes = await this._http.get('/channels', {
        params: this._params({ part: 'contentDetails', id: channelId }),
      });
      this._logQuota('channels.list contentDetails', 1);
      const uploadsId = chRes.data?.items?.[0]?.contentDetails?.relatedPlaylists?.uploads;
      if (uploadsId) {
        candidates = await this.getPlaylistVideos(uploadsId, scan, { publishedAfter });
      }
    } catch (err) {
      console.warn('[YouTubeClient] uploads playlist lookup failed:', err.message);
    }

    if (!candidates.length) {
      candidates = await this.getRecentVideos(channelId, Math.min(scan, MAX_LIMIT));
    }

    const longForm = candidates.filter((v) => {
      if (v.isShort) return false;
      const d = v.duration || 0;
      if (d < minDurationSec) return false;
      if (maxDurationSec != null && Number.isFinite(maxDurationSec) && d > maxDurationSec) return false;
      return true;
    });
    return {
      videos: longForm.slice(0, want),
      scanned: candidates.length,
      shortsSkipped: candidates.filter((v) => v.isShort || (v.duration || 0) < minDurationSec).length,
    };
  }
  ...
  /**
   * Fetch videos from a specific playlist (paginated).
   * opts.publishedAfter — ISO date; uploads are newest-first so we stop paging when older.
   * Cost: 1 unit/page (playlistItems.list) + N (videos.list)
   */
  async getPlaylistVideos(playlistId, limit = DEFAULT_LIMIT, opts = {}) {
    if (!this.apiKey) throw new Error('YOUTUBE_API_KEY not configured');
    const { publishedAfter } = opts;
    const target = Math.min(Math.max(1, Number(limit) || DEFAULT_LIMIT), MAX_UPLOADS_LIMIT);
    const out = [];
    let pageToken = null;
    let pages = 0;

    while (out.length < target && pages < MAX_PLAYLIST_PAGES) {
      const pageSize = Math.min(50, target - out.length);
      ...
      const res = await this._http.get('/playlistItems', { params });
      ...
      for (const vid of videoIds) {
        ...
        if (publishedAfter && pubAt && pubAt < publishedAfter) {
          hitOlderThanFilter = true;
          continue;
        }
        const isShort = detectShort(durIso);
        out.push({
      ...
      if (!pageToken || hitOlderThanFilter) break;
    }
