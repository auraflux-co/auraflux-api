...
class YouTubeClient {
   ...
   * Prefers the channel uploads playlist (cheap, reliable) over search.list,
   * and oversamples so Shorts-heavy channels still surface longer VODs.
   */
  async getRecentLongVideos(channelId, limit = DEFAULT_LIMIT, minDurationSec = 180) {
    if (!this.apiKey) throw new Error('YOUTUBE_API_KEY not configured');
    const want = Math.min(Math.max(1, limit), MAX_LIMIT);
    const scan = Math.min(Math.max(want * 5, 50), MAX_LIMIT);
    ...
    try {
      ...
      if (uploadsId) {
        candidates = await this.getPlaylistVideos(uploadsId, scan);
      }
    } catch (err) {
    ...
    if (!candidates.length) {
      candidates = await this.getRecentVideos(channelId, scan);
    }

    const longForm = candidates.filter(
      (v) => !v.isShort && (v.duration || 0) >= minDurationSec,
    );
    return {
