...
/** @type {string|null|undefined} undefined = uncached */
...
async function resolveApifyProxyPassword() {
  ...
  if (cachedApifyProxyPassword !== undefined) return cachedApifyProxyPassword;

  const token = process.env.APIFY_API_TOKEN;
  if (!token) {
    cachedApifyProxyPassword = null;
    return null;
  }
  ...
  try {
    ...
    cachedApifyProxyPassword = body?.data?.proxy?.password || null;
  } catch {
    cachedApifyProxyPassword = null;
  }
