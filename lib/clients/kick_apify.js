...
function _normalise(item) {
  ...
  // CDN URL (Apify direct link) — good for portal0 probing but may expire by assembly time.
  // Store as cdnUrl; use the stable Kick page URL as primary url for yt-dlp downloads.
  const cdnUrl   = item.videoUrl || item.clipUrl || null;
  const pageUrl  = isVod
    ? (videoId ? `https://kick.com/video/${videoId}` : cdnUrl)
    : (clipId  ? `https://kick.com/clip/${clipId}`   : cdnUrl);
