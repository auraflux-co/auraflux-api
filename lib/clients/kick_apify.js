...
/**
 * Live channel metadata via Apify channel_details mode (CPD-1065).
 * Does not return playback_url — use kick_live_resolver for HLS ingest.
 */
async function fetchKickChannelDetails(username) {
  const token = process.env.APIFY_API_TOKEN;
  if (!token) throw new Error('APIFY_API_TOKEN not set');
  ...
  return {
    slug,
    channelName: item.channelName || slug,
    displayName: item.displayName || slug,
    isLive: !!item.isLive,
    currentViewers: item.currentViewers || item.viewers || 0,
    currentStreamTitle: item.currentStreamTitle || item.title || null,
    currentCategory: item.currentCategory || item.category || null,
    scrapedAt: item.scrapedAt || null,
  };
}

module.exports = { fetchKickContent, fetchKickChannelDetails };
