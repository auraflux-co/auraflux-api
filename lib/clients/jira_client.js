...
class JiraClient {
  ...
  async createIssue({ summary, description = '', issueType = 'Task', labels = [] }) {
    const resp = await axios.post(`${this.baseUrl}/issue`, {
      fields: {
    ...
    return { key: resp.data.key, id: resp.data.id, url: `https://${new URL(this.baseUrl).hostname}/browse/${resp.data.key}` };
  }
  ...
  async searchIssues(jql, maxResults = 50) {
    const resp = await axios.post(`${this.baseUrl}/issue/search`, { jql, maxResults }, { auth: this.auth });
