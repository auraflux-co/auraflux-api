 ...
 * CPD-1293: runs BEFORE Gate 3a so QA + operator preview see Whisper/look.
 * R2 upload still follows Gate 3. Thumbnails run in parallel with Gate 3a
 * on the postprocessed file.
 *
 * Features ported:
 ...
 *   3. Portrait brand-pad + Gemini smart-crop — utility for 16:9→9:16 conversion
...
async function _applyLoudnorm(inputPath, outputPath) {
  ...
  // CPD-1222: linear=true silently falls back to dynamic mode when measured LRA
  // exceeds the target — dynamic gain pumping ducked speech right after loud clips
  // ("sound went low" on the quip after a Twitch clip). Widen the target LRA to the
  // measured value so long-form mixes stay in linear (single-gain) mode.
  const targetLra = measured
    ? Math.min(20, Math.max(11, Math.ceil(Number(measured.input_lra) || 11)))
    : 11;
  const filter = measured
    ? [
        `loudnorm=I=-16:TP=-1.5:LRA=${targetLra}`,
...
// ─── 3. Portrait brand-pad + smart crop (utility — opt-in) ───────────────────

function normalizeBrandPadHex(hex, fallback = '0d1424') {
  const raw = String(hex || fallback).replace(/^#/, '').replace(/^0x/i, '');
  return /^[0-9a-fA-F]{6}$/.test(raw) ? `0x${raw.toLowerCase()}` : `0x${fallback}`;
}

/**
 * Convert 16:9 landscape to 9:16 with solid brand-pad fill (#0d1424) and a
 * full-width sharp clip centered vertically (smart-crop horizontal offset).
 */
async function applyPortraitBlurPad(inputPath, outputPath, {
  jobId = 'pp',
  log = null,
  bottomCropPct = 0,
  padColor = null,
  ...
  const dur = Math.max(0.04, Number(durationSec) || 60).toFixed(3);
  const padHex = normalizeBrandPadHex(
    padColor
      || process.env.CWN_BRAND_PAD_COLOR
      || CONFIG.VISUAL_LAYOUTS?.SHORT_FORM?.BRAND_PAD_COLOR
      || '0x0d1424',
  );

  const fgX = `(W-w)*${Math.max(0.15, Math.min(0.85, cx)).toFixed(3)}`;
  const filter = [
    `color=c=${padHex}:s=1080x1920:d=${dur},format=yuv420p[bg]`,
    `[0:v]${srcCrop}scale=1080:-2:force_original_aspect_ratio=decrease,setsar=1[fg]`,
    `[bg][fg]overlay=${fgX}:(H-h)/2,setsar=1,format=yuv420p[vout]`,
  ].join(';');
  ...
  if (log) {
    log(`[postprocess] portrait brand-pad applied (pad=${padHex}, subject cx=${cx.toFixed(2)}${cropPct > 0 ? `, bottom crop ${(cropPct * 100).toFixed(0)}%` : ''})`);
  }
  return outputPath;
}
...
function _wrapHookLines(rawText, maxLines = 2, maxCharsPerLine = 36) {
  ...
  if (String(rawText || '').includes('|')) {
    ...
    if (forced.length > 1) {
      return _rebalanceHookLines(forced.slice(0, maxLines), maxCharsPerLine);
    }
  }
  ...
  return _rebalanceHookLines(lines, maxCharsPerLine);
}

/** Pull words so line 2 is a real phrase, not a 1–3 char orphan. */
function _rebalanceHookLines(lines, maxCharsPerLine = 36) {
  if (!Array.isArray(lines) || lines.length < 2) return lines || [];
  const out = lines.map((l) => String(l || '').trim()).filter(Boolean);
  if (out.length < 2) return out;

  const orphanMax = 4; // "U!", "OK", "?!"
  let guard = 0;
  while (guard++ < 12 && out.length >= 2) {
    const last = out[out.length - 1];
    const prev = out[out.length - 2];
    const lastWords = last.split(/\s+/).filter(Boolean);
    const prevWords = prev.split(/\s+/).filter(Boolean);
    // Orphan = tiny crumb OR a single short token alone on line 2.
    const lastIsOrphan = last.length <= orphanMax || (lastWords.length === 1 && last.length <= 10);
    if (!lastIsOrphan) break;

    // Can't steal words — collapse orphan onto previous line.
    if (prevWords.length < 2) {
      out[out.length - 2] = `${prev} ${last}`.trim();
      out.pop();
      break;
    }

    // Move last word of prev onto last until line 2 is a real phrase.
  ...
  // Only collapse to one line when the whole hook still fits the char budget.
  if (out.length === 2 && `${out[0]} ${out[1]}`.length <= maxCharsPerLine) {
    return [`${out[0]} ${out[1]}`.trim()];
  }
...
function _buildHookDrawtextFilters(lines, style, sharpBottom, { hookPlacement = 'bottom', hookMidY = 680 } = {}) {
  ...
  const midFrame = hookPlacement === 'ranked_mid' || hookPlacement === 'full_bleed_mid' || hookPlacement === 'split_seam';
...
async function applyClipCompTransform(inputPath, outputPath, { contentType, designSpec, asmId = 'pp', log = null, previewFast = false } = {}) {
  ...
  // CPD-1291: Review uses ultrafast — assembly keeps quality encode.
...
async function applyPostProcessing({
  ...
  designSpec = null,
  compCreative = null,
  /** Music preflight result — high-confidence music → skip Whisper (bad timing on singing). */
  musicPreflight = null,
  asmId = 'pp',
  log = null,
}) {
  ...
  const musicFlagged = Array.isArray(musicPreflight?.flagged) && musicPreflight.flagged.length > 0;
  if (wantsCaptions && musicFlagged) {
    wantsCaptions = false;
    _log(log, asmId, '  📝 Music-dominant source (preflight) — skipping Whisper captions (timing unreliable on singing)');
  }
  ...
  const applyTransform = shouldApplyClipCompTransform(isClipComp, designSpec, compCreative);
  // CPD-1293: do not early-return before transform — Review burns look even when captions off.
  if (!wantsCaptions && !wantsLoudnorm && !applyTransform) return applied;

  const useClipCompTemplate = isClipComp && !applyTransform;
  if (useClipCompTemplate) {
    _log(log, asmId, '  📐 Clip-comp template mode — no look/transform ordered');
  }
  ...
  // Ensure transform sees live operator creative (anim text / look) — designSpec alone can be stale
  const designForTransform = {
    ...(designSpec || {}),
    compCreative: {
      ...((designSpec && designSpec.compCreative) || {}),
      ...(compCreative || {}),
      animatedText: (compCreative && compCreative.animatedText)
        || (designSpec && designSpec.compCreative && designSpec.compCreative.animatedText)
        || null,
      look: (compCreative && compCreative.look)
        || (designSpec && designSpec.compCreative && designSpec.compCreative.look)
        || null,
    },
  };
  ...
  if (applyTransform) {
    ...
    try {
      const ok = await applyClipCompTransform(outPath, txOut, {
        contentType,
        designSpec: designForTransform,
        asmId,
        log: (m) => _log(log, asmId, m),
      });
      if (ok && await _avSyncOk(txOut)) {
        fs.renameSync(txOut, outPath);
        applied.transform = true;
        const animN = (designForTransform.compCreative?.animatedText?.items || []).length;
        _log(log, asmId, `  ✅ Clip-comp transform applied (grade + vignette + grain + badge${animN ? ` + ${animN} anim punches` : ''})`);
      } else if (ok) {
