...
async function _burnCaptionsWhisper(inputPath, outputPath, style = 'default', promptBias = '') {
...
function buildClipCompHookStyle(designSpec, contentType) {
  ...
  const accent = cap.colors?.[typeKey] || cap.colors?.clips || '#6441A5';
  const boxOpacity = cap.boxOpacity ?? 0.82;
  return {
    font: cap.font || '/System/Library/Fonts/Supplemental/Arial Bold Italic.ttf',
...
function _formatHookLines(rawText, maxLines = 2) {
  const clean = String(rawText || '').replace(/\s+/g, ' ').trim();
  if (!clean || maxLines <= 1) return clean;
...
async function burnClipCompHookCaption(inputPath, outputPath, { text, designSpec, contentType, log = null }) {
  const hookText = _formatHookLines(text, 2);
  if (!hookText) return inputPath;
  ...
  const dtY = `(${sharpBottom}-text_h-${Math.round(style.yOffset)})`;
  const drawArgs = [
    `fontfile=${style.font}`,
    `text=${escapedText}`,
  ...
  try {
    ...
    if (log) log(`  💬 Hook title burned: "${hookText.replace(/\n/g, ' | ')}"`);
    return outputPath;
  } catch (e) {
 ...
 * @param {boolean}  opts.clipCompTransform — apply color grade / vignette / grain / badge
...
async function applyPostProcessing({
  ...
  // Transform pass BEFORE captions — distinct look + @handle badge (silent comps, no avatar).
  if (clipCompTransform) {
