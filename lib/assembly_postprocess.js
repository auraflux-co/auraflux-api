 ...
 */
async function _whisperTranscribe(inputPath, responseFormat = 'srt') {
...
async function transcribeVideo(inputPath, { jobId = 'pp', log = null } = {}) {
  try {
    const text = await _whisperTranscribe(inputPath, 'text');
...
async function _burnCaptionsWhisper(inputPath, outputPath, style = 'default') {
  ...
  try {
    const srt = await _whisperTranscribe(inputPath, 'srt');
...
module.exports = { applyPostProcessing, applyPortraitBlurPad, transcribeVideo };
