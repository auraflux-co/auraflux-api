 ...
 */
/**
 * Build an SRT caption file from job spec script text when Whisper audio transcription
 * returns nothing (silent clip or clips with no speech). Distributes script words evenly
 * across video duration at ~5 words per caption line.
 * Returns SRT string or null if no usable text found.
 */
async function _buildSrtFromScript(jobSpec, videoPath, log) {
  const scriptText = _val(jobSpec, 'script', null) ||
                     _val(jobSpec, 'finalTitle', null) ||
                     _val(jobSpec, 'publishCopy.tiktok', null) ||
                     _val(jobSpec, 'publishCopy.youtube', null) ||
                     null;
  if (!scriptText || typeof scriptText !== 'string' || scriptText.trim().length < 20) return null;

  const { execFile: _ef } = require('child_process');
  const durationSecs = await new Promise((res) => {
    _ef(
      require('./ffmpeg_utils').ffprobePath(),
      ['-v', 'quiet', '-print_format', 'json', '-show_format', videoPath],
      (err, stdout) => {
        if (err) return res(30);
        try { res(parseFloat(JSON.parse(stdout).format.duration) || 30); }
        catch (_) { res(30); }
      }
    );
  });

  const words = scriptText.replace(/[^\w\s']/g, ' ').trim().split(/\s+/).filter(Boolean);
  ...
  const secsPerChunk = durationSecs / chunks.length;
  const toTime = (s) => {
  ...
  chunks.forEach((chunk, i) => {
    const start = i * secsPerChunk;
    const end   = Math.min((i + 1) * secsPerChunk - 0.1, durationSecs - 0.1);
    srt += `${i + 1}\n${toTime(start)} --> ${toTime(end)}\n${chunk}\n\n`;
  });

  if (log) log(`[postprocess] captions: script fallback built ${chunks.length} caption lines from ${words.length} words`);
  return srt;
}
...
async function _burnCaptionsWhisper(inputPath, log, jobSpec) {
  ...
  try {
    ...
    // CPD-978: Build Whisper prompt from streamers.json roster on-air names
    // Providing real names as prior context steers Whisper away from phonetic mishearings.
    let whisperPrompt = '';
    try {
      const rosterPath = require('path').join(__dirname, '../data/streamers.json');
      if (fs.existsSync(rosterPath)) {
        const { roster } = JSON.parse(fs.readFileSync(rosterPath, 'utf8'));
        const names = (roster || []).map((r) => r.onAirName || r.displayName).filter(Boolean);
        if (names.length) whisperPrompt = `Streamers mentioned: ${names.join(', ')}.`;
      }
    } catch (_) {}
    ...
    let srtContent = whisperResp.data;
    if (!srtContent || typeof srtContent !== 'string' || !srtContent.includes('-->')) {
      if (log) log('[postprocess] captions: Whisper returned empty SRT — trying script fallback');
      srtContent = await _buildSrtFromScript(jobSpec, inputPath, log);
      if (!srtContent) {
        if (log) log('[postprocess] captions: no script text available for fallback — skipping captions');
        return inputPath;
      }
      if (log) log('[postprocess] captions: using script-derived captions (no audio transcript)');
    }

    // CPD-978: SRT phonetic post-pass — replace phonetic respellings with display names
    // HeyGen scripts write phonetic field (e.g. "Yawn-uh") so the avatar pronounces correctly;
    // Whisper then transcribes that audio phonetically into the SRT. We fix captions here.
    try {
      const rosterPath2 = require('path').join(__dirname, '../data/streamers.json');
      if (fs.existsSync(rosterPath2)) {
        const { roster } = JSON.parse(fs.readFileSync(rosterPath2, 'utf8'));
        for (const r of (roster || [])) {
          if (r.phonetic && (r.onAirName || r.displayName)) {
            const correct = r.onAirName || r.displayName;
            // Replace phonetic literal (e.g. "Yawn-uh") and common variations
            const variants = [r.phonetic, r.twitchUsername].filter(Boolean);
            for (const v of variants) {
              const re = new RegExp(v.replace(/[-/\\^$*+?.()|[\]{}]/g, '\\$&'), 'gi');
              srtContent = srtContent.replace(re, correct);
            }
          }
        }
      }
    ...
    // CPD-978: FontSize 10/11 — libass default PlayResY=288, so FontSize=18 renders ~120px on
    // a 1920-tall video. Drop to 10/11 (≈65-75px) for readable but not overbearing captions.
    const styleMap = {
      ...
      default:  'FontName=Arial,FontSize=11,PrimaryColour=&H00FFFFFF,Bold=1,BorderStyle=3,OutlineColour=&H80000000',
    };
...
/**
 * CPD-442: Apply PiP (picture-in-picture) face-cam overlay.
 * Downloads the secondary video from addOns.pip.videoUrl, overlays it scaled to
 * 1/4 of main video dimensions in the corner specified by addOns.pip.position.
 */
async function _applyPipOverlay(inputPath, outputPath, jobSpec, log) {
  const videoUrl = _val(jobSpec, 'addOns.pip.videoUrl', null);
  const position = _val(jobSpec, 'addOns.pip.position', 'bottom-right');
  if (!videoUrl) return;

  // Download the pip video to a temp file
  const pipTmp = inputPath.replace(/\.mp4$/, '') + '_pip_src.mp4';
  try {
    const https = require('https');
    const http  = require('http');
    await new Promise((resolve, reject) => {
      const dest = fs.createWriteStream(pipTmp);
      const proto = videoUrl.startsWith('https') ? https : http;
      proto.get(videoUrl, (res) => {
        if (res.statusCode >= 400) { reject(new Error(`pip download HTTP ${res.statusCode}`)); return; }
        res.pipe(dest);
    ...
    const filterComplex = `[1:v]scale=iw/4:ih/4[pip];[0:v][pip]overlay=${overlayExpr}[vout]`;
    ...
    _log(log, `  [postprocess] PiP overlay applied (position=${position})`);
  } finally {
    try { if (fs.existsSync(pipTmp)) fs.unlinkSync(pipTmp); } catch (_) {}
  }
}
...
async function applyPostProcessingEffects(jobSpec, inputPath, log) {
  ...
  const wantsPip    = _val(jobSpec, 'addOns.pip.active', false) && _val(jobSpec, 'addOns.pip.videoUrl', null);
  const hasFilters  = vfChain && !vfChain.includes('__PORTRAIT_BLUR_PAD__') && !vfChain.includes('__PIP_OVERLAY__');
  const hasAudio    = !!afChain;
  const hasEncOpts  = wantsH265 || (wantsMetadata && encodeOpts.title);

  const nothingToDo = !wantsPortrait && !wantsSquare && !wantsLoudnorm && !hasFilters && !hasAudio && !hasEncOpts && !wantsPip;
  if (nothingToDo) {
    return inputPath;
  }
  ...
  try {
    ...
    // CPD-442: PiP face-cam overlay (requires secondary video input — must run after all single-input passes)
    if (wantsPip) {
      const pipOut = _tmpPath(inputPath, 'pip');
      tempPaths.push(pipOut);
      await _applyPipOverlay(currentPath, pipOut, jobSpec, log);
      currentPath = pipOut;
    }
