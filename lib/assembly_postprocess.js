...
async function transcribeVideo(inputPath, { jobId = 'pp', log = null } = {}) {
  try {
    ...
    const text = await _whisperTranscribe(inputPath, 'text', _streamerNamePromptBias(roster));
...
async function _burnCaptionsWhisper(inputPath, outputPath, style = 'default') {
  ...
  try {
    const roster = _loadStreamerRoster();
    const srt = await _whisperTranscribe(inputPath, 'srt', _streamerNamePromptBias(roster));
    if (!srt || typeof srt !== 'string' || !srt.includes('-->')) {
      throw new Error('Whisper returned empty SRT');
    }
    fs.writeFileSync(srtPath, _correctStreamerNames(srt, roster), 'utf8');
...
/**
 * Full-bleed 9:16 crop (C2 / C5 / C9–C11). Subject-centred when smart_crop available.
 * Uses 720x1280 on Render memory budget (same as assembly_service vertical crop).
 */
async function applyPortraitFullBleed(inputPath, outputPath, { jobId = 'pp', log = null, width = 720, height = 1280 } = {}) {
  let cx = 0.5;
  let cy = 0.5;
  try {
    const { detectSubjectCentre } = require('./services/smart_crop');
    const centre = await detectSubjectCentre(inputPath, jobId);
    if (centre) {
      if (Number.isFinite(centre.cx)) cx = centre.cx;
      if (Number.isFinite(centre.cy)) cy = centre.cy;
    }
  } catch { /* centre fallback */ }

  const w = Math.max(360, Number(width) || 720);
  const h = Math.max(640, Number(height) || 1280);
  // Normalize subject into crop window: scale+crop then shift crop origin toward subject.
  const filter =
    `scale=${w}:${h}:force_original_aspect_ratio=increase,` +
    `crop=${w}:${h}:(iw-ow)*${Math.max(0.05, Math.min(0.95, cx)).toFixed(3)}:(ih-oh)*${Math.max(0.05, Math.min(0.95, cy)).toFixed(3)},` +
  ...
  if (log) log(`[postprocess] portrait full-bleed applied (cx=${cx.toFixed(2)}, cy=${cy.toFixed(2)})`);
  return outputPath;
}

/**
 * Apply punch/look colorbalance after portrait layout (C9/C10).
 */
async function applyLookColorPass(inputPath, outputPath, { lookName = null, log = null } = {}) {
  ...
  try {
    ...
    frag = buildColorbalanceFrag(look.colorbalance);
  } catch (_) { /* optional */ }
  if (!frag) {
    fs.copyFileSync(inputPath, outputPath);
    return outputPath;
  ...
  if (log) log(`[postprocess] look color applied (${lookName})`);
  return outputPath;
}

/**
 * Route portrait layout from compositionSpec.compCreative.layout.mode.
 */
async function applyClipCompPortraitLayout(inputPath, outputPath, {
  layoutMode = 'blur_pad',
  jobId = 'pp',
  lookName = null,
  log = null,
} = {}) {
  const mode = String(layoutMode || 'blur_pad');
  const mid = lookName ? `${outputPath}.prelook.mp4` : outputPath;
  if (mode === 'full_bleed_crop' || mode === 'split_screen' || mode === 'dual_source_stack') {
    // split_screen / dual_source need full C0 clip_comp_layout — full-bleed is the prod MVP stand-in
    await applyPortraitFullBleed(inputPath, mid, { jobId, log });
  } else {
    await applyPortraitBlurPad(inputPath, mid, { jobId, log });
  }
  if (lookName && lookName !== 'auto') {
    await applyLookColorPass(mid, outputPath, { lookName, log });
    try { if (mid !== outputPath && fs.existsSync(mid)) fs.unlinkSync(mid); } catch (_) {}
  }
  return outputPath;
}
...
/**
 * CPD-1187 / CPD-431: pipeline_assembly.js entry point.
 * Delegates to applyPostProcessing and writes savedOutputs flags on jobSpec.
 */
async function applyPostProcessingEffects(jobSpec, inputPath, log) {
  if (!inputPath || !fs.existsSync(inputPath)) return inputPath;

  const isShortForm =
    jobSpec?.order?.output?.formFactor === 'short' ||
    (jobSpec?.contentType || '').includes('-short') ||
    jobSpec?.templateId === 'short-form';
  const asmId = jobSpec?.jobId || 'pp';
  const applied = await applyPostProcessing({
    outPath: inputPath,
    isShortForm,
    asmId,
    log: typeof log === 'function' ? (_id, msg) => log(msg) : null,
  });

  if (!jobSpec.state) jobSpec.state = {};
  if (!jobSpec.state.savedOutputs) jobSpec.state.savedOutputs = {};
  if (applied.loudnorm) jobSpec.state.savedOutputs.loudnormApplied = true;
  if (applied.captions) jobSpec.state.savedOutputs.captionsApplied = true;
...
module.exports = {
  ...
  _correctStreamerNames, _streamerNamePromptBias,
};
