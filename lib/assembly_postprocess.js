...
async function applyPostProcessingEffects(jobSpec, inputPath, log) {
  ...
  try {
    ...
    // We mark the intent here so the grader sees layoutPortraitApplied, but skip the
    // separate _applyPortraitBlurPad call to avoid a redundant re-encode.
    if (wantsPortrait) {
      // CPD-479: Portrait reframe is deferred to _applyChrome (needsPortraitReframe=true).
    ...
    // CPD-479: layoutPortraitApplied is set by developer_api.js after chrome overlay,
    // not here. Portrait was deferred to the chrome pass; premature flag = wrong guard.
