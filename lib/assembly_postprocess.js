...
function filterFfmpegPath() {
  ...
  const dockerWrapper = path.join(__dirname, '..', 'bin', 'ffmpeg-docker');
  const candidates = [
  ...
  _filterFfmpegPath = dockerWrapper;
  return _filterFfmpegPath;
}
...
async function _burnCaptionsWhisper(inputPath, outputPath, style = 'default', promptBias = '', { clipCompTemplate = false } = {}) {
  const srtPath = inputPath.replace(/\.mp4$/, '_captions.srt');

  try {
    const roster = _loadStreamerRoster();
    const bias = promptBias || _streamerNamePromptBias(roster);
    const srt = await _whisperTranscribe(inputPath, 'srt', bias);
    ...
    let forceStyle = CAPTION_STYLES[style] || CAPTION_STYLES.default;
    if (clipCompTemplate) {
      const { clipCompWhisperCaptionStyleSuffix } = require('./clip_comp_template');
      forceStyle += clipCompWhisperCaptionStyleSuffix();
    }
...
function _wrapHookLines(rawText, maxLines = 2, maxCharsPerLine = 36) {
  const clean = stripDrawtextUnsafe(String(rawText || '').replace(/\s+/g, ' '));
  if (!clean) return [];
...
function _buildHookDrawtextFilters(lines, style, sharpBottom) {
  const lineHeight = Math.round(style.fontsize * 1.18);
  const blockHeight = lines.length * lineHeight;
  const baseY = sharpBottom - style.yOffset - blockHeight;
 ...
 */
async function burnClipCompHookCaption(inputPath, outputPath, { text, designSpec, contentType, log = null }) {
  ...
  const sharpBottom = CONFIG.VISUAL_LAYOUTS?.SHORT_FORM?.CLIP_COMP_SHARP_BOTTOM || 1264;
  const vf = _buildHookDrawtextFilters(lines, style, sharpBottom);
...
async function applyPostProcessing({
  ...
  // CPD-937: twitch clip comps — skip whisper when creator already burned captions.
  // Sports/news: always apply our caption layer (transformative + readable).
  // Twitch clip comps: ALWAYS apply ClipzWorld whisper — creator subs are not our layer.
  if (wantsCaptions && skipCaptionsIfSourceCaptioned && !isBroadcastClipComp && !isClipComp) {
    try {
  ...
  if (isBroadcastClipComp && wantsCaptions) {
    ...
    _log(log, asmId, `  📝 Twitch clip comp — forcing ClipzWorld whisper captions`);
  }
