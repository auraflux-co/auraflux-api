...
// AuraFlux pipeline performance metrics
...
function finalizeJobMetrics(jobId) {
  ...
  // Fire-and-forget Postgres write for each stage metric
  try {
    const { saveMetric } = require('./db');
    if (typeof saveMetric === 'function') {
      for (const stage of jobMetrics[jobId].stages) {
        saveMetric(jobId, stage.stage, stage.wallTimeMs, stage).catch(() => {});
      }
    }
  } catch (e) {
