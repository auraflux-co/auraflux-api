...
async function uploadToR2(localPath, fileName, opts = {}) {
  ...
  if (stat.size >= MULTIPART_THRESHOLD) {
    ...
    upload.on('httpUploadProgress', (p) => {
      if (p.total && opts.onProgress) opts.onProgress(p);
      else if (p.total && p.loaded && p.loaded % (512 * 1024 * 1024) < (p.part || 1)) {
...
module.exports = { uploadToR2, presignR2, deleteFromR2, detectMimeType, isR2Configured };
