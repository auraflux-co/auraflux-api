...
function parseR2ObjectUrl(url) {
  if (!url || typeof url !== 'string') return null;
  const base = url.split('?')[0];
  try {
    const parsed = new URL(base);
    const assetsDomain = process.env.R2_ASSETS_DOMAIN;
    if (assetsDomain && parsed.hostname === assetsDomain) {
...
/**
 * Return a fresh presigned GET URL for an R2 object referenced by a stored URL.
 * Strips expired query params and re-signs with current credentials.
 */
async function getPresignedDownloadUrl(url, expiresInSec = 3600) {
  const parsed = parseR2ObjectUrl(url);
  if (!parsed?.key) {
    throw new Error(`Cannot parse R2 object URL: ${String(url).slice(0, 120)}`);
  }
  return presignR2(parsed.key, { bucket: parsed.bucket, expiresIn: expiresInSec });
}

module.exports = { uploadToR2, presignR2, getPresignedDownloadUrl, parseR2ObjectUrl, detectMimeType };
