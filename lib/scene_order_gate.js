...
const { buildScaffoldRows, buildHeyGenSceneRows } = require('./scene_scaffold_panel');
...
function buildSceneOrderPreflight({ card, script, contentType, rundown } = {}) {
  ...
  const heygenScaffold = buildHeyGenSceneRows({ card, script: raw, contentType: ct, rundown: rundownSrc });
  ...
  if (heygenScaffold.mergeCount > 0) {
    notes.push(
      `HeyGen merges ${heygenScaffold.mergeCount} streamer block(s) — ${heygenScaffold.scriptSceneCount} script scenes → ${heygenScaffold.heygenSceneCount} renders (no 009/055 flash)`,
    );
  }
  ...
  return {
    ...
    notes,
    rows,
    heygenRows: heygenScaffold.rows,
    heygenMergeCount: heygenScaffold.mergeCount,
    heygenSceneCount: heygenScaffold.heygenSceneCount,
    scriptSceneCount: heygenScaffold.scriptSceneCount,
    expectedHeaders,
    foundHeaders,
    scriptHash: hash,
    heygenConfirmed,
...
function hasSceneOrderConfirm(card, gate, hash) {
  if (gate === 'heygen') {
    return !!(card?.sceneOrderHeygenConfirmedAt && card.sceneOrderScriptHash === hash);
  }
  if (card?.sceneOrderAssemblyConfirmedAt && card.sceneOrderScriptHash === hash) return true;
  // First assembly after HeyGen: heygen confirm with unchanged script satisfies assembly gate.
  if (card?.sceneOrderHeygenConfirmedAt && card.sceneOrderScriptHash === hash) return true;
  return false;
}
...
function assertSceneOrderGate(card, gate = 'assembly', script) {
  ...
  const raw = script || card?.script?.raw || '';
  const hash = scriptHash(raw);
  if (!hasSceneOrderConfirm(card, gate, hash)) {
    return {
      ok: false,
