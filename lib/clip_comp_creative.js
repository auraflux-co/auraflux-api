...
const PRESET_DEFAULTS = {
  ...
  serpent_ranked: {
    ...
    audio: { clipAudio: true, musicBed: 'low_trap', musicBedVolume: 0.10, cutSfx: 'serpent_pack', duckSpeech: true },
    editorial: { enabled: false, introCard: false, ttsBridges: false },
    effects: { transform: false, gagOverlays: false },
    delivery: { format: 'short', vodTargetMin: 8, vodTargetMax: 20, relatedVideoParentId: null, playlistSeries: null },
  },
...
/** Rank overlay slot count follows lineup size (2–8); classic Stream Serpent target is 5. */
function syncRankedListToClipCount(compCreative, clipCount) {
  ...
  merged.hooks.rankedList.slotCount = Math.max(2, Math.min(8, Math.floor(n)));
  return merged;
}

function rankedListClipGuidance(compCreative) {
  if (!compCreative?.hooks?.rankedList?.enabled) return null;
  return 'Ranked list counts down to #1 — lineup size sets the list (classic Top 5 = pick 5 clips).';
}
