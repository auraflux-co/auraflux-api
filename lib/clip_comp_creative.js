...
const PRESET_DEFAULTS = {
  ...
  serpent_ranked: {
    ...
    hooks: { mode: 'hook_only', rankedList: { enabled: true, streamer: '', theme: 'MOMENTS', slotCount: 5, titlePattern: 'WAIT_FOR_NO_1' } },
    captions: { whisper: false, style: 'phrase_full_bleed' },
    audio: { clipAudio: true, musicBed: 'low_trap', musicBedVolume: 0.04, cutSfx: 'serpent_pack', duckSpeech: true },
    editorial: { enabled: false, introCard: false, ttsBridges: false },
    effects: { transform: false, gagOverlays: false },
    delivery: { format: 'short', vodTargetMin: 8, vodTargetMax: 20, relatedVideoParentId: null, playlistSeries: null },
  },
  serpent_ranked_vod: {
    ...
    layout: { mode: 'full_bleed_crop', logo: 'off' },
    hooks: { mode: 'hook_only', rankedList: { enabled: true, streamer: '', theme: 'MOMENTS', slotCount: 10, titlePattern: 'WAIT_FOR_NO_1' } },
    captions: { whisper: false, style: 'phrase_full_bleed' },
    audio: { clipAudio: true, musicBed: 'low_trap', musicBedVolume: 0.04, cutSfx: 'serpent_pack', duckSpeech: true },
...
function compCreativeGate3Expectations(compCreative) {
  ...
  return {
    ...
    hasTicker: false,
    hasLogo,
    logoPosition: logoMode === 'corner' ? 'top-right' : 'top-blur-fold',
    ...
    formatDescription: ranked
      ? `Stream Serpent ranked clip comp — full-bleed portrait, header + countdown column (Top ${rankedOverlay.slotCount}), burned hooks mid-frame, logo OFF, NO broadcast chrome.`
      : fullBleed
...
function resolveCompCreativeFromContext({ body = {}, designSpec = null, jobCard = null } = {}) {
  ...
  // Stale job-card musicBedVolume (e.g. 0.1 from an old run) must not override preset defaults.
  if (!body.compCreative?.audio?.musicBedVolume && VALID_PRESETS.has(merged.preset)) {
