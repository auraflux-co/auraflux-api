...
const PRESET_DEFAULTS = {
  ...
  // Age / then-now: two different sources stacked — top plays then bottom (hold opposite pane)
  dual_source_stack: {
    preset: 'dual_source_stack',
    layout: {
      mode: 'dual_source_vstack',
      logo: 'corner',
    ...
    hooks: { mode: 'whisper_only', rankedList: { enabled: false, streamer: '', theme: 'MOMENTS', slotCount: 1, titlePattern: 'WAIT_FOR_NO_1' } },
    captions: { whisper: true, style: 'phrase_full_bleed' },
    audio: { clipAudio: true, musicBed: 'off', musicBedVolume: 0.12, cutSfx: 'off', duckSpeech: true },
    editorial: { enabled: false, introCard: false, ttsBridges: false },
    effects: { transform: false, gagOverlays: false },
    delivery: { format: 'short', vodTargetMin: 8, vodTargetMax: 20, relatedVideoParentId: null, playlistSeries: null },
  },
  ...
  // CPD-1287 — FableFlow (@FableFlow2) editor-less Speed Short recipe
  fableflow_speed: {
    preset: 'fableflow_speed',
    layout: {
      // CPD-1289 — split default so two people fit; operator can switch to full_bleed solo
      mode: 'split_screen',
      logo: 'corner',
      logoCorner: 'top_right',
      landscapeSplit: true,
      topPaneSubject: 'stacked_figures',
    },
    hooks: {
      mode: 'whisper_only',
      rankedList: { enabled: false, streamer: '', theme: 'MOMENTS', slotCount: 1, titlePattern: 'WAIT_FOR_NO_1' },
    },
    captions: { whisper: true, style: 'phrase_full_bleed' },
    audio: {
      clipAudio: true,
      // CPD-1295: complementary bed outside dialogue; dialogueAwareMix gates under speech
      musicBed: 'neutral_lofi',
      bedStyle: 'complement',
      musicBedVolume: 0.10,
      bedMixWeight: 0.14,
      cutSfx: 'whoosh',
      duckSpeech: true,
      dialogueAwareMix: true,
      sourceOutsideSpeechGain: 0.32,
      bedInSpeechGain: 0,
    },
    look: { preset: 'punch' },
    transition: { style: 'cut', durationSec: 0.35 },
    // UI hint: auto-suggest Beats→FX after preview is ready
    beatSync: { suggestOnPreview: true, source: 'clip', maxPeaks: 6 },
    editorial: { enabled: false, introCard: false, ttsBridges: false },
    effects: { transform: true, gagOverlays: false },
    delivery: { format: 'short', vodTargetMin: 8, vodTargetMax: 20, relatedVideoParentId: null, playlistSeries: null },
  },
  // iss_ahVNGr1DzfdZ — Reaction Short (Transform 5/5 recipe from Compose QA)
  reaction_short: {
    ...
    // Anim-text owns punch lines — burned CWN hook card OFF (was competing with WATCH THIS / WAIT FOR IT)
    hooks: {
      mode: 'whisper_only',
      rankedList: { enabled: false, streamer: '', theme: 'MOMENTS', slotCount: 1 },
    },
    captions: { whisper: true, style: 'phrase_full_bleed' },
    audio: {
      clipAudio: true,
      // Source clip audio (performances / reactions) wins — trap bed fought AGT/rock cuts
      musicBed: 'off',
      musicBedVolume: 0.10,
      bedMixWeight: 0.12,
      cutSfx: 'whoosh',
      duckSpeech: false,
      dialogueAwareMix: false,
      highlightSfx: { enabled: false, drops: [] },
    },
    look: { preset: 'punch' },
    transition: { style: 'cut', durationSec: 0.35 },
    beatSync: { suggestOnPreview: true, autoExecute: true, source: 'clip', maxPeaks: 6 },
    speedFeel: 'punch_pause',
    // Ratio hints (0–1 of trim); EXECUTE densifies + expands to wall-clock if needed
    animatedText: {
      enabled: true,
      items: [
...
const PRESET_LABELS = {
  classic_blur_pad: 'Classic ClipzWorld (brand-pad)',
  ...
  dual_source_stack: 'Then/Now stack (Age top → bottom)',
  twitch_comp_vod: 'Comp VOD 8–20 min (edited comp)',
  fableflow_speed: 'FableFlow · Speed Short (editor-less)',
  reaction_short: 'Reaction Short (anim punches + whisper)',
...
const COMP_CREATIVE_CATALOG = {
  classic_blur_pad: {
    ...
    tagline: 'Brand-pad portrait · ClipzWorld navy bands',
    buttons: ['Short', 'Comp'],
    ...
    layout: 'Brand-pad (#0d1424 bands) + logo in top fold',
    audio: 'Clip audio only',
    outcome: '4-clip streamer comp OR solo brand-pad Shorts',
  },
  full_bleed: {
    code: 'C2',
    name: 'Full Bleed',
  ...
  dual_source_stack: {
    code: 'C11',
    name: 'Then / Now Stack',
    tagline: 'Two sources stacked — top plays first, then bottom (Age 7 → Age 16)',
    buttons: ['Comp'],
    shortClips: 'Not for this preset — use Comp with exactly 2 clips',
    compClips: 'Exactly 2 · clip 1 = TOP · clip 2 = BOTTOM',
    compVodClips: null,
    whisper: true,
    hooks: 'Whisper only · pane labels (Age 7 / Age 16)',
    layout: 'Persistent top/bottom stack · top plays for Mark Out (or switch sec) while bottom holds · then bottom plays',
    audio: 'Active pane audio (phase 1 top · phase 2 bottom)',
    outcome: 'Before/after Age Short — both panes visible the whole time; playback switches at top trim end',
  },
  twitch_comp_vod: {
    code: 'C6',
  ...
  fableflow_speed: {
    code: 'C9',
    name: 'FableFlow Speed',
    tagline: 'Editor-less Speed Short — match @FableFlow2 Feed recipe',
    buttons: ['Short'],
    shortClips: '1 VOD peak → 1 Short (primary)',
    compClips: 'Not for this preset — use Short only',
    compVodClips: null,
    whisper: true,
    hooks: 'Whisper captions only (no burned CWN hook card)',
    layout: 'Default Split for two-shot (Speed+brother); switch to full screen for solo',
    audio: 'ES music bed + light whoosh · Punch look (visible in preview) · Beats→FX suggested',
    outcome: 'Faceless Speed Short: peak → Split/frame → Beats→FX → EXECUTE → Fable-style title at publish',
  },
  reaction_short: {
    code: 'C10',
    name: 'Reaction Short',
    tagline: 'Reaction packaging — vivid punch, anim-text beats, source audio (no competing hook card)',
    buttons: ['Short'],
    shortClips: '1 clip / VOD peak → 1 Short',
    compClips: 'Not for this preset — use Short only',
    compVodClips: null,
    whisper: true,
    hooks: 'Whisper only · 4 timed anim lines (WATCH THIS → THE DROP) — no burned hook card',
    layout: 'Full-bleed solo 9:16 (reaction / stage / performance)',
    audio: 'Source clip audio (bed off) · punch look · auto Beats→FX + punch_pause on EXECUTE',
    outcome: 'Reaction Short: peak → full-bleed → EXECUTE auto-burns speed/beats/punch/anim (Gap fixes optional)',
  },
  custom: {
    code: 'C7',
...
const COMP_LINEUP_TARGETS = {
  ...
  dual_source_stack: { lineupSlots: 2, minClips: 2, maxClips: 2, minDurationSec: 0, maxDurationSec: 120 },
  ...
  fableflow_speed: { lineupSlots: 1, minClips: 1, maxClips: 1, minDurationSec: 0, maxDurationSec: 90 },
  reaction_short: { lineupSlots: 1, minClips: 1, maxClips: 1, minDurationSec: 0, maxDurationSec: 90 },
...
function mergeCompCreative(opts = {}) {
  ...
  merged.sourceCleanup = deepMerge(
    DEFAULT_SOURCE_CLEANUP,
    merged.sourceCleanup || opts.overrides?.sourceCleanup || {},
  );
...
function compCreativeChips(compCreative) {
  ...
  try {
    ...
    const clean = sourceCleanupSummary(c.sourceCleanup);
    if (clean) chips.push('UI mask');
  } catch (_) { /* offline */ }
  return chips;
}
...
function compCreativeGate3Expectations(compCreative) {
  ...
  const dualStack = layoutMode === 'dual_source_vstack' || c.preset === 'dual_source_stack';
  ...
  return {
    ...
    logoPosition: logoMode === 'off'
      ? 'off'
      : (logoMode === 'corner' || fullBleed || splitScreen || dualStack)
    ...
    formatDescription: ranked
      ...
      : dualStack
        ? 'Then/Now dual-source stack — TOP + BOTTOM panes are two different sources shown simultaneously (vstack); hold-then-switch intentional; Age labels OK; logo per preset; NO broadcast chrome; NOT sequential end-to-end stitch.'
        : splitScreen
        ...
        : fullBleed
          ...
          : 'Brand-pad portrait clip comp — sharp clip + ClipzWorld navy (#0d1424) bands, logo in top fold when enabled, NO broadcast chrome.',
  };
}
...
function resolvePresetLayoutMode(preset) {
  ...
  if (p === 'dual_source_stack') return 'dual_source_vstack';
  if (p === 'facecam_split') return 'split_screen';
  if (p === 'full_bleed' || p === 'fableflow_speed' || p === 'reaction_short' || p.includes('serpent') || p === 'dahbluh_clean' || p === 'twitch_comp_vod') {
...
function finalizeCompCreativeForAssembly(compCreative, {
  ...
  // Dual-source Age stack — never coerce into facecam split_screen; bed always off
  if (merged.preset === 'dual_source_stack' || merged.layout.mode === 'dual_source_vstack') {
    merged.preset = 'dual_source_stack';
    merged.layout.mode = 'dual_source_vstack';
    merged.layout.landscapeSplit = false;
    merged.layout.paneLabels = merged.layout.paneLabels || { top: 'Age 7', bottom: 'Age 16' };
    if (merged.layout.paneLabelAnchor !== 'bottom') merged.layout.paneLabelAnchor = 'top';
    if (merged.layout.switchAtSec != null) {
      const sw = Number(merged.layout.switchAtSec);
      merged.layout.switchAtSec = Number.isFinite(sw) && sw > 0 ? sw : null;
    }
    ...
    merged.hooks = { ...(merged.hooks || {}), mode: 'whisper_only' };
  ...
  if (hasLandscape) {
    ...
    const operatorSingleCrop = layout.cropZoom != null
      || layout.cropCx != null
      || layout.cropCy != null;
    const singleView = layout.landscapeSplit === false
      || (operatorSingleCrop && layout.mode !== 'split_screen');
    if (singleView) {
      merged.layout.landscapeSplit = false;
      delete merged.layout.facecamRect;
      delete merged.layout.bottomPaneRect;
      ...
      const zoom = Number(merged.layout.cropZoom);
      if (Number.isFinite(zoom)) merged.layout.cropZoom = Math.max(0.25, Math.min(4, zoom));
      else delete merged.layout.cropZoom;
...
function resolveCompCreativeFromContext({ body = {}, designSpec = null, jobCard = null } = {}) {
  ...
  // Compose Music bed OFF always wins — never revive preset low_trap / ranked bed
  const shippedBed = body.compCreative?.audio?.musicBed
    ?? cardCreative?.audio?.musicBed
    ?? null;
  if (shippedBed === 'off' || merged.composeShipped || merged.operatorLocked) {
    if (shippedBed === 'off' || merged.audio?.musicBed === 'off') {
      merged.audio = { ...(merged.audio || {}), musicBed: 'off', bedPerSegment: false };
    }
  }
  // Ranked presets: bed volume comes from PRESET_DEFAULTS, not stale card fields.
  if (merged.audio?.musicBed && merged.audio.musicBed !== 'off'
      && ['serpent_ranked', 'serpent_ranked_vod'].includes(merged.preset)) {
  ...
  } else if (merged.audio?.musicBed && merged.audio.musicBed !== 'off'
      && body.compCreative?.audio?.musicBedVolume == null && VALID_PRESETS.has(merged.preset)) {
