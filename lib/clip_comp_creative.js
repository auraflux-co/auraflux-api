...
const PRESET_DEFAULTS = {
  ...
  reaction_short: {
    ...
    // Timings are ratio hints (0–1 of trim); Compose hydrate expands to wall-clock sec
    animatedText: {
      enabled: true,
      items: [
        { text: 'WATCH THIS', ratio: 0.08, duration: 4.5, style: 'scale_pop', fontSize: 76 },
        { text: 'WAIT FOR IT', ratio: 0.32, duration: 4.0, style: 'scale_pop', fontSize: 76 },
        { text: 'UNBELIEVABLE', ratio: 0.55, duration: 4.0, style: 'scale_pop', fontSize: 76 },
        { text: 'THE DROP', ratio: 0.78, duration: 3.5, style: 'scale_pop', fontSize: 76 },
      ],
    },
    editorial: { enabled: false, introCard: false, ttsBridges: false },
    effects: { transform: true, gagOverlays: false },
...
const COMP_CREATIVE_CATALOG = {
  ...
  reaction_short: {
    ...
    audio: 'Source clip audio (bed off) + whoosh · vivid · Beats→FX on clip audio · punch_pause',
    outcome: 'Reaction Short: peak → full-bleed → Gap fixes / Beats→FX → Review → Gemini Compare → EXECUTE',
  },
  custom: {
    code: 'C7',
