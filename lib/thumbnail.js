...
/**
 * Build template render data from jobSpec for a given customer's content types.
 ...
 * @param {Object} customerConfig   - full customer config (for brand strings)
 * @returns {Object} render data
 */
function buildTemplateData(jobSpec, thumbnailConfig, customerConfig) {
  ...
  if (cfg.templateType === 'overlay') {
    // clips / overlay style: use customer background, stamp episode and date
    const showName = customerConfig?.showName || 'AuraFlux';
    return {
  ...
  const showName = customerConfig?.showName || 'AuraFlux';
  const chromeName =
    customerConfig?.templates?.['long-form']?.designDefaults?.chrome?.name || showName;
  return {
    templateType: 'html',
    ...
    await page.evaluate(
      (data, isNBA) => {
        if (isNBA) {
          ...
          if (categoryEl) categoryEl.textContent = data._chromeName || data._showName || '';
          const brandEl = document.querySelector('.thumb-brand');
          if (brandEl) brandEl.textContent = data._showName || '';
...
async function renderOverlayToPng(renderData, outputPath) {
  ...
  const safeText = (hookText || 'AURAFLUX').replace(/[':]/g, ' ').replace(/\\/g, '').slice(0, 80);
  const safeEpisode =
...
async function generateThumbnail(jobSpec) {
  ...
  try {
    ...
    const renderData = buildTemplateData(jobSpec, thumbnailConfig, customerConfig);
    ...
    // Upload — R2 for C1+ (Render), Drive for C0 (localhost)
    let uploadUrl = null;
    try {
      uploadUrl = await uploadFile(pngPath, pngName, {
        customerConfig,
        folder: 'thumbnails',
        label: `Thumbnail - ${jobId}`,
      });
    } catch (uploadErr) {
      logError('THUMBNAIL_UPLOAD_FAIL', uploadErr, { jobId });
      return {
        ok: true,
        pngPath,
        driveUrl: null,
    ...
    return { ok: true, pngPath, driveUrl: uploadUrl, error: null };
  } catch (err) {
