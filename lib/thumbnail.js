 ...
 * Extracts candidate frames from the assembled short, lets Gemini pick the
 * most engaging one (fallback: 35% frame), and uploads it via the same R2
 * contract as generateThumbnail. NEVER throws.
 *
 * @param {Object} jobSpec   - needs jobSpec.jobId for naming/foldering
 * @param {string} videoPath - local path to the assembled short
 * @returns {Promise<{ ok: boolean, pngPath: string|null, driveUrl: string|null, error: string|null }>}
 */
async function generateShortFrameThumbnail(jobSpec, videoPath) {
  ...
  try {
    ...
    if (!pick.ok) {
      return { ok: false, pngPath: null, driveUrl: null, error: pick.error || 'frame pick failed' };
    }
