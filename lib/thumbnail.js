...
function buildTemplateData(jobSpec, thumbnailConfig) {
  ...
  const ALIASES = { twitch: 'clips', 'twitch-short': 'clips', nba: 'sports', 'nba-short': 'sports', 'sports-short': 'sports', 'news-short': 'news' };
  ...
  if (cfg.templateType === 'overlay') {
    ...
    return {
      ...
      backgroundPath: path.join(ROOT_DIR, cfg.backgroundPath || 'assets/longform_twitch_Thumbnail.png'),
      contentType,
      hookText:       (
    ...
    return {
      ...
      injectData: {
        matchups:      (jobSpec.order?.inputs?.items || [])
                         .map(i => i.matchup || i.teams || i.title || i.displayName || '')
                         .filter(Boolean)
                         .join(' · '),
  ...
  // news (default) — use first item that has a usable imageUrl
  const newsItems = jobSpec.order?.inputs?.items || [];
  const leadItem = newsItems.find(it => it?.imageUrl && it.imageUrl.startsWith('http')) || newsItems[0] || {};
  return {
    templateType:  'html',
    templatePath:  path.join(ROOT_DIR, cfg.templatePath || 'templates/cwn_news_tool.html'),
    injectData: {
      headline:  (leadItem.title || leadItem.displayName || '').slice(0, 60),
...
async function renderOverlayToPng(renderData, outputPath) {
  ...
  const safeText = (hookText || 'CLIPZWORLD NEWS')
    ...
    .replace(/\\/g, '')
    .slice(0, 80);
  const safeEpisode = String(episodeNumber || '').replace(/[^0-9]/g, '').slice(0, 4) || '1';
  ...
  const drawtextFilter = contentType === 'clips'
    ? [
      // Cover old baked-in date text area — hardcoded x for JPEG/PNG compat (W variable fails on stills)
      "drawbox=x=400:y=565:w=480:h=120:color=0x6441A5@0.92:t=fill",
...
async function generateThumbnail(jobSpec) {
  ...
  try {
    ...
    const contentType = CONTENT_TYPE_ALIASES[rawContentType] || String(rawContentType).replace(/-short$/, '');
    ...
    // Upload to R2 (CPD-887) — field stays named driveUrl for job-card compat
    let driveUrl = null;
    try {
      const { uploadToR2 } = require('./storage');
      driveUrl = await uploadToR2(pngPath, pngName, { folder: `thumbnails/${jobId}` });
    } catch (uploadErr) {
      ...
      return { ok: true, pngPath, driveUrl: null, error: `R2 upload failed: ${uploadErr.message}` };
    }
...
/**
 * Plain portrait frame — no show/EP/tagline burn. Used when Gemini pick fails.
 */
async function extractPlainPortraitFrame(videoPath, outputPath, atRatio = 0.35) {
  const duration = await new Promise((resolve, reject) => {
    execFile(ffmpegPath(), ['-i', videoPath], { maxBuffer: 10 * 1024 * 1024 }, (err, _stdout, stderr) => {
...
/**
 * CPD-938 — short-form thumbnail pulled from the actual video.
 * Gemini frame pick; fallback is plain portrait extract (never VOD designed template).
 */
async function generateShortFrameThumbnail(jobSpec, videoPath) {
  const jobId = jobSpec?.jobId || 'unknown';
  try {
    if (!videoPath || !fs.existsSync(videoPath)) {
      return { ok: false, pngPath: null, driveUrl: null, error: `video not found: ${videoPath}` };
    }

    if (!fs.existsSync(OUTPUT_DIR)) fs.mkdirSync(OUTPUT_DIR, { recursive: true });
    const jpgName = `thumbnail_${jobId}_${Date.now()}.jpg`;
    const jpgPath = path.join(OUTPUT_DIR, jpgName);

    const { pickBestThumbnailFrame } = require('./services/frame_intel');
    const pick = await pickBestThumbnailFrame(videoPath, jpgPath, jobId);
    if (!pick.ok) {
      try {
        await extractPlainPortraitFrame(videoPath, jpgPath);
      } catch (fallbackErr) {
        return { ok: false, pngPath: null, driveUrl: null, error: pick.error || fallbackErr.message || 'frame pick failed' };
      }
    }
    ...
    try {
      const { uploadToR2 } = require('./storage');
      driveUrl = await uploadToR2(jpgPath, jpgName, { folder: `thumbnails/${jobId}` });
    } catch (uploadErr) {
      logError('THUMBNAIL_DRIVE_UPLOAD_FAIL', uploadErr, { jobId });
      return { ok: true, pngPath: jpgPath, driveUrl: null, error: `R2 upload failed: ${uploadErr.message}` };
    }

    return { ok: true, pngPath: jpgPath, driveUrl, error: null };
  } catch (err) {
    logError('THUMBNAIL_FRAME_FAIL', err, { jobId });
    return { ok: false, pngPath: null, driveUrl: null, error: err.message };
  }
}

module.exports = { generateThumbnail, generateShortFrameThumbnail };
