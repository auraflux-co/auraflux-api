...
function kickApiGetHttps(accessToken, path) {
  return new Promise((resolve, reject) => {
    const req = https.request(
      {
      ...
      (res) => {
        ...
        res.on('end', () => {
          ...
          if (status < 200 || status >= 300) {
            reject(new Error(
              `Kick GET ${path} failed (${status})${text ? `: ${text.slice(0, 120)}` : ''}`,
            ));
            return;
          }
          if (!text) {
            reject(new Error(`Kick GET ${path} empty response (${status})`));
            return;
          }
          try {
            resolve(JSON.parse(text));
          } catch {
            reject(new Error(`Kick GET ${path} non-JSON (${status}): ${text.slice(0, 120)}`));
          }
        });
        res.on('error', reject);
      },
    ...
    req.on('error', reject);
    req.setTimeout(25000, () => req.destroy(new Error('Kick API request timeout')));
    req.end();
  });
}
...
async function kickApiGet(accessToken, path) {
  ...
  for (let attempt = 1; attempt <= 2; attempt++) {
    try {
      return await kickApiGetHttps(accessToken, path);
    } catch (err) {
      lastErr = err;
      if (!isRetryableKickTokenError(err) || attempt === 2) throw err;
      await sleep(400);
