...
async function publishDirect({ platform, videoPath, metadata = {}, jobSpec }) {
  ...
  const tokens = await loadTokens(customerId, platform);
  if (!tokens) {
    throw new Error(
      `No ${platform} OAuth tokens found for customer ${customerId}. ` +
...
async function canPublishDirect(platform, jobSpec) {
  ...
  const valid = await hasValidToken(jobSpec?.customerId, platform);
