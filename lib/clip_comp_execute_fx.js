...
function densifyAnimatedText(animatedText, durationSec) {
  // Compose shipped Anim text OFF — never invent WATCH THIS pack
  if (!animatedText || animatedText.enabled === false) {
    return { enabled: false };
  }
  const dur = Math.max(8, Number(durationSec) || 30);
  const existing = expandAnimRatios(animatedText?.items || [], dur);
  if (!existing.length && !animatedText.text) {
    return { enabled: false };
  }
...
function strengthenCompCreativeForExecute(compCreative, { durationSec = 30 } = {}) {
  ...
  // Compose shipped controls always win — never invent FX the operator turned off
  if (c.composeShipped || c.operatorLocked) {
    if (c.animatedText && c.animatedText.enabled === false) {
      c.animatedText = { enabled: false };
    }
    return c;
  }
  if (preset !== 'reaction_short' && !c.effects?.autoExecuteFx) return c;

  // Honor explicit operator look — do not upgrade auto/vivid to punch when Compose set it
  const lookName = c.look?.preset || 'auto';
  if (!c.look?.preset) {
    c.look = { ...(c.look || {}), preset: 'punch' };
  } else if (lookName === 'vivid' || lookName === 'crisp') {
    // mild bump only when preset left default vivid from reaction hydrate without lock
    c.look = { ...(c.look || {}), preset: lookName };
  }
  ...
  if (c.animatedText?.enabled !== false) {
    c.look = {
      ...c.look,
      filmGrainStrength: Math.min(9, Math.max(7, Number(c.look?.filmGrainStrength) || 8)),
    };
  }
  // Only default speed when Compose did not ship a feel
  if (c.speedFeel == null) {
    c.speedFeel = 'punch_pause';
  }
  // Do not force auto beats when operator disabled them
  const beatsOff = c.beatSync?.autoExecute === false;
  c.beatSync = {
    ...(c.beatSync || {}),
    suggestOnPreview: true,
    autoExecute: beatsOff ? false : (c.beatSync?.autoExecute !== false),
