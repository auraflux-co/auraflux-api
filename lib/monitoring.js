...
function startListening() {
  ...
  pipelineBus.on('gate:pass', ({ jobId, gate, score, outcome, contentType, customerId, durationMs }) => {
    ...
    // NR: gate pass event — track pass rate per gate per customer
    nrEvent('GateResult', {
      jobId,
  ...
  pipelineBus.on('gate:hard_fail', ({ jobId, gate, reason, contentType, customerId }) => {
    const existing = activeJobs.get(jobId) || {};
    activeJobs.set(jobId, {
      ...existing,
      lastActivity: Date.now(),
