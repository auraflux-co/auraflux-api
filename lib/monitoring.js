...
function startListening() {
  ...
  pipelineBus.on('gate:sendback', ({ jobId, gate, attempt, fixDirective, contentType, customerId }) => {
    const existing = activeJobs.get(jobId) || {};
    ...
    // NR: sendback event — track how often QA agents send work back and why
    nrEvent('GateSendback', {
      jobId,
      customerId: customerId || existing.customerId || 'unknown',
      contentType: contentType || existing.contentType || 'unknown',
      gate: String(gate),
      attempt: attempt || sendbackCount,
    ...
    recordGateWhyFromBus('gate:sendback', {
      ...
      attempt: attempt || sendbackCount, fixDirective, reason: null, concerns: [], deductions: []
    });
  });
