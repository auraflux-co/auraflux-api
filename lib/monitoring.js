...
function recordGateWhyFromBus(eventName, data) {
  ...
  const passed = eventName === 'portal:pass';
...
async function escalate({ jobId, gate, reason, trail = [] }) {
  ...
  // Write escalation report for Cursor/Claude Code visibility (same format whether local or Render)
  try {
    const fs = require('fs');
    const path = require('path');
    const reportsDir = path.join(__dirname, '..', 'docs', 'reports', 'roo');
    fs.mkdirSync(reportsDir, { recursive: true });
    const ts = new Date().toISOString().replace(/[:.]/g, '-').slice(0, 19);
    const trailLines = (trail || [])
      .map((r) => `- Gate ${r?.gate ?? '?'}: ${r?.outcome ?? '?'} (score: ${r?.score ?? '?'})`)
      .join('\n');
    const report = [
      `# Pipeline Escalation Report`,
      ``,
      `**Job:** ${jobId}`,
      `**Gate:** ${gate}`,
      `**Time:** ${escalationEntry.escalatedAt}`,
      `**Reason:** ${safeReason}`,
      ``,
      `## Gate Trail`,
      ``,
    ...
    fs.writeFileSync(path.join(reportsDir, `escalation_${ts}.md`), report, 'utf8');
  } catch (_e) {
    /* non-fatal */
  }
  ...
  pipelineBus.emit('portal:escalate', { jobId, gate, reason: safeReason, trail });
...
function determineRestartGate({ jobId, savedOutputs = [] }) {
  ...
  for (const gateKey of PORTAL_ORDER) {
...
function startListening() {
  ...
  pipelineBus.on(
    ...
    ({
      ...
      recordGateWhyFromBus('portal:pass', {
        jobId,
        gate,
        score,
        outcome,
  ...
  pipelineBus.on(
    ...
    ({ jobId, gate, reason, contentType, customerId, concerns, deductions }) => {
      ...
      recordGateWhyFromBus('portal:hard_fail', {
        jobId,
        gate,
        score: null,
        outcome: 'hard_fail',
    ...
    ({ jobId, gate, attempt, fixDirective, contentType, customerId, qaTier }) => {
      if (qaTier === QA_TIER_OPS) {
        logError(
          ...
          new Error('portal:sendback received qaTier ops — use portal:ops_sendback'),
          { jobId, gate }
        );
        return;
      }
      ...
      recordGateWhyFromBus('portal:sendback', {
        jobId,
        gate,
        score: null,
        outcome: 'sendback',
  ...
  pipelineBus.on('portal:escalate', ({ jobId, gate, reason, trail }) => {
