...
const rooBridge = require('./roo_bridge');
...
function recordGateWhyFromBus(eventName, data) {
  ...
  const reasons = (data.deductions || []).map(d => (typeof d === 'string' ? d : d.reason) || '').filter(Boolean);
...
async function escalate({ jobId, gate, reason, trail = [] }) {
  ...
  const safeReason = (reason == null || reason === '') ? 'No escalation reason provided' : String(reason);
  ...
  if (rootCauseType === 'auto_fix') {
    ...
    logError('MONITORING_CODE_FIX_NEEDED', new Error(`${jobId} requires code fix at gate ${gate}: ${safeReason}`), { jobId, gate, reason: safeReason });
...
async function killJobCleanly({ jobId, failedGate, rootCause, trail = [] }) {
  ...
  try {
    ...
    requestAgentInterventionLastResort(jobId, `unfixable_job_kill gate=${failedGate}: ${rootCause}`);
  } catch (_e) { /* non-fatal */ }
}
...
function startListening() {
  ...
  pipelineBus.on('gate:pass', ({ jobId, gate, score, outcome, contentType, customerId, durationMs, concerns, deductions }) => {
    const existing = activeJobs.get(jobId) || {};
    ...
    recordGateWhyFromBus('gate:pass', {
      jobId, gate, score, outcome, contentType, customerId: customerId || existing.customerId, durationMs, concerns, deductions
    });
  });

  pipelineBus.on('gate:hard_fail', ({ jobId, gate, reason, contentType, customerId, concerns, deductions }) => {
    const existing = activeJobs.get(jobId) || {};
    activeJobs.set(jobId, {
      ...existing,
      lastActivity: Date.now(),
    ...
    const hardFailReason = reason
      || (Array.isArray(deductions) && deductions.length
        ? deductions.map(d => (typeof d === 'string' ? d : d.reason) || '').filter(Boolean).join('; ')
        : '')
    ...
    recordGateWhyFromBus('gate:hard_fail', {
      jobId, gate, score: null, outcome: 'hard_fail', contentType, customerId: customerId || existing.customerId,
      reason: hardFailReason, concerns, deductions
    });
  });

  // Tier 1 — QA review/report only. Tier 2 uses gate:ops_sendback (no qa_cycle / kill ladder).
  pipelineBus.on('gate:sendback', ({ jobId, gate, attempt, fixDirective, contentType, customerId, qaTier }) => {
    if (qaTier === QA_TIER_OPS) {
      logError('MONITORING_SENDBACK_TIER_MISMATCH', new Error('gate:sendback received qaTier ops — use gate:ops_sendback'), { jobId, gate });
      return;
    }
    const existing = activeJobs.get(jobId) || {};
    ...
    nrEvent('GateSendback', {
      ...
      fixDirectiveFields: fixDirective ? Object.keys(fixDirective).filter(k => Array.isArray(fixDirective[k]) && fixDirective[k].length > 0).join(',') : ''
    });
    recordGateWhyFromBus('gate:sendback', {
      jobId, gate, score: null, outcome: 'sendback', contentType, customerId: customerId || existing.customerId,
      attempt: attempt || cycle.sendbacks, fixDirective, reason: null, concerns: [], deductions: []
    });
  });

  // Tier 2 — operational / worker / internal (not formal QA review; does not advance qa_cycle)
  pipelineBus.on('gate:ops_sendback', ({ jobId, gate, attempt, fixDirective, contentType, customerId, reason }) => {
    const existing = activeJobs.get(jobId) || {};
    activeJobs.set(jobId, {
      ...existing,
      lastActivity: Date.now(),
    ...
    try {
      whyLedger.recordWhyLedger({
        ...
        evidenceDigest: { attempt, fixDirectiveKeys: fixDirective ? Object.keys(fixDirective) : undefined },
        ...
        if (typeof gateWorker.prepare === 'function') {
          ...
          Promise.resolve(gateWorker.prepare(specRef))
            .then(() => {
              ...
              if (ds.synthVerifyRequired !== undefined) patch.synthVerifyRequired = ds.synthVerifyRequired;
              ...
              try {
                ...
                console.warn(`[monitoring] gate3a synth fields persist failed (${specRef.jobId}): ${pe.message}`);
              }
            })
            .catch(e => {
  ...
  silenceChecker = setInterval(() => {
    ...
    for (const [jobId, state] of activeJobs.entries()) {
      ...
      if (silentMs > SILENCE_THRESHOLD_MS) {
        ...
        // CPD-901: jobs already terminal in the DB are not running workers —
        // re-assembly Gate 2 events re-register the parent jobSpecId with no
        // later events, which produced a fake 'silent worker' kill every run.
        try {
          const { getJobStatus } = require('./db');
          const dbStatus = (getJobStatus(jobId) || '').toLowerCase();
          if (['killed', 'published', 'done', 'completed', 'failed'].includes(dbStatus)) {
            activeJobs.delete(jobId);
            console.log(`[monitoring] ${jobId}: silence check skipped — DB status '${dbStatus}' is terminal (deregistered)`);
            continue;
          }
        } catch (_e) { /* DB unavailable — fall through to normal escalation */ }
...
rooBridge.attachToBus(pipelineBus);
