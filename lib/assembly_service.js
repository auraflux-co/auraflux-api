...
// CPD-351: Detect Kick page URLs (kick.com/clip/{id} or kick.com/video/{id}) that
// require yt-dlp for download. Kick CDN URLs from Apify expire quickly; using the
// stable page URL with yt-dlp avoids download failures at assembly time.
function _isKickPageUrl(url) {
  if (!url || typeof url !== 'string') return false;
  return /^https?:\/\/(?:www\.)?kick\.com\/(clip|video)\//.test(url);
}
...
async function _downloadWithYtdlp(jobId, url, destPath) {
  ...
  console.log(`[assembly_service] ${jobId}: downloading Twitch clip via yt-dlp: ${url.slice(0, 80)}`);
  return new Promise((resolve, reject) => {
    execFile(
      ytdlp,
      [
      ...
      (err) => {
        if (err) return reject(new Error(`yt-dlp clip download failed: ${err.message}`));
        resolve();
      }
    );
  });
...
async function _downloadClips(jobId, urls) {
  ...
  for (let i = 0; i < urls.length; i++) {
    ...
    try {
      ...
      } else if (_isKickPageUrl(url) || _isYouTubeUrl(url)) {
        // CPD-351: Kick page URLs (kick.com/clip/ or kick.com/video/) and YouTube watch
        // URLs require yt-dlp. Kick CDN URLs from Apify expire; the stable page URL is
        // used instead. YouTube watch URLs return HTML for direct HTTP — yt-dlp only.
        console.log(`[assembly_service] ${jobId}: platform page URL — downloading via yt-dlp: ${url.slice(0, 80)}`);
        await _downloadWithYtdlp(jobId, url, tmpPath);
