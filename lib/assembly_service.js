...
async function _downloadWithYtdlp(jobId, url, destPath) {
  ...
  if (proxyUrl) {
    ...
    // CPD-353: YouTube bot-detection bypass — android/mweb client avoids datacenter IP blocks
 ...
 * android/mweb first; optional useCookies uses YOUTUBE_COOKIES_BASE64 + Deno EJS.
 */
...
async function extractVodClips(vodUrl, opts = {}) {
  ...
  // Peaks / COMPACT windows: skip get-url — go straight to --download-sections.
  let streamUrl;
  ...
  for (let i = 0; i < clipCount; i++) {
    ...
    if (useSectionDownload) {
      try {
        ...
        if (!process.env.YOUTUBE_COOKIES_BASE64 && !process.env.YTDLP_PROXY) throw androidErr;
        console.warn(`[assembly_service] ${jobId}: android/mweb section failed (${androidErr.message.slice(0, 140)}) — retry cookies`);
