...
// Prevents two concurrent heavy FFmpeg passes (concat + chrome) from OOM-ing the process.
...
function _applyChrome(inputPath, outputPath, opts = {}) {
  return new Promise((resolve, reject) => {
    ...
    // instead of running _applyPortraitBlurPad as a separate FFmpeg invocation.
    ...
    // For portrait jobs with needsPortraitReframe=true, prepend blur-pad reframe so
    // the entire portrait layout + chrome happens in a single FFmpeg decode/encode pass.
    // This replaces the previous two-pass approach (_applyPortraitBlurPad + _applyChrome).
    let filterComplex;
    const inputs = ['-i', inputPath];
    if (logoPath) inputs.push('-i', logoPath);

    if (needsPortraitReframe) {
      // Step 1: Blur-pad 16:9 to 9:16; step 2: apply chrome on reframed stream
      const portraitSteps = [
      ...
      if (logoPath) {
        ...
        const fallbackText = `drawtext=text='AuraFlux':fontsize=${logoTextSize}:fontcolor=white@0.75:x=w-text_w-20:y=${logoTextY}${fontAttr}`;
        filterComplex = [portraitSteps, `[reframed]${baseFilters},${fallbackText}[vout]`].join(';');
      }
    } else if (logoPath) {
      filterComplex = [
      ...
      filterComplex = `[0:v]${fallbackFilters}[vout]`;
    }
...
// CPD-479: Wrap assembleForJob with the assembly semaphore so at most one
// heavy FFmpeg assembly runs concurrently on the 512 MB Render instance.
...
module.exports = { assembleForJob: assembleForJobQueued, mixTtsIntoVideo, mixAlignedMultiClipTts, extractVodClips, applyChrome: _applyChrome };
