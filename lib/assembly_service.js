...
function _applyChrome(inputPath, outputPath, opts = {}) {
  return new Promise((resolve, reject) => {
    ...
    if (logoPath) {
      // filter_complex: scale to max 720p (OOM guard on Render), chrome on base, brand logo overlay
      const filterComplex = [
      ...
      const fallbackFilters = `scale=-2:min(ih\\,720),${baseFilters},drawtext=text='AuraFlux':fontsize=${logoTextSize}:fontcolor=white@0.75:x=w-text_w-20:y=${logoTextY}${fontAttr}`;
