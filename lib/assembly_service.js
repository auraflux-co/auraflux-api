...
// signed CDN URL for a clip. Uses ShareClipRenderStatus (2026+ Twitch clip API).
async function _resolveTwitchClipGql(slug) {
  const TwitchClient = require('./clients/twitch_client');
  const client = new TwitchClient();
  const { mp4Url } = await client.resolveClipMp4(slug, 'low');
  return mp4Url;
}
...
async function _downloadWithYtdlp(jobId, url, destPath) {
  ...
  if (proxyUrl) {
    ...
    // CPD-1312: android client (ANDROID_VR media URLs 403). Same as media_download.js.
    extraArgs.push('--extractor-args', 'youtube:player_client=android,ios');
    console.log(`[assembly_service] ${jobId}: using android/ios client for YouTube clip download`);
  }
  ...
  for (let i = 0; i < urls.length; i++) {
    ...
    try {
      if (_isTwitchCdnUrl(url)) {
        ...
        // YouTube watch URLs require yt-dlp. _downloadWithYtdlp uses android/ios client
        // as a bot-detection bypass when no proxy is configured.
        console.log(`[assembly_service] ${jobId}: YouTube URL — downloading via yt-dlp (android/ios): ${url.slice(0, 80)}`);
...
async function extractVodClips(vodUrl, opts = {}) {
  ...
  const streamUrl = await new Promise((resolve, reject) => {
    ...
    //
    //   3. android/ios player client — CPD-1312 (ANDROID_VR media URLs 403).
    ...
    if (proxyUrl) {
      ...
      if (cookiesB64) {
        ...
        // Priority 3: android/ios client (CPD-1312)
        extraArgs.push('--extractor-args', 'youtube:player_client=android,ios');
      }
    }
  ...
  const isYouTubeVod = /youtube\.com|youtu\.be/.test(vodUrl);
  const EVEN_DIM_VF = 'scale=trunc(iw/2)*2:trunc(ih/2)*2';

  for (let i = 0; i < clipCount; i++) {
    const startSec = startSeconds[i];
    const tsEntry = vodClipTimestamps?.[i];
    const clipDurSecs = tsEntry?.end_s != null && Number.isFinite(tsEntry.end_s)
      ? Math.max(1, Number(tsEntry.end_s) - Number(startSec))
      : maxClipSecs;
    const clipPath = path.join(TMP_DIR, `vod_clip_${jobId}_${i}.mp4`);
    console.log(`[assembly_service] ${jobId}: extracting clip ${i + 1}/${clipCount} at ${Math.round(startSec)}s (${clipDurSecs.toFixed(1)}s)`);

    const runFfmpegExtract = (encode) => new Promise((resolve, reject) => {
      const args = [
        '-fflags', '+genpts',
        '-reconnect', '1',
        ...
        '-ss', String(startSec),
        '-i', streamUrl,
        '-t', String(clipDurSecs),
      ];
      if (encode) {
        const vf = isVertical
          ? 'scale=1080:1920:force_original_aspect_ratio=increase,crop=1080:1920'
          : `scale=-2:480:force_original_aspect_ratio=decrease,${EVEN_DIM_VF}`;
        args.push(
      ...
      args.push('-movflags', '+faststart', '-y', clipPath);
      execFile(ffmpegPath(), args, { timeout: 900000 }, (err) => {
    ...
    let extracted = false;
    // YouTube progressive URLs (itag 18): stream copy avoids libx264 odd-width failures.
    if (isYouTubeVod && vodClipTimestamps?.length) {
      try {
        await runFfmpegExtract(false);
        if (fs.existsSync(clipPath) && _fileSizeMb(clipPath) > 0.1) extracted = true;
      } catch (copyErr) {
        console.warn(`[assembly_service] ${jobId}: clip ${i + 1} stream-copy failed (${copyErr.message.slice(0, 120)}) — re-encoding`);
        try { fs.unlinkSync(clipPath); } catch (_) {}
      }
    }
    if (!extracted) {
      await runFfmpegExtract(true);
    }
