...
function _downloadFile(url, destPath) {
  return new Promise((resolve, reject) => {
    axios.get(url, { responseType: 'stream', timeout: 60000, maxRedirects: 5 })
      .then((response) => {
...
function _concatClips(localPaths, outputPath) {
  return new Promise((resolve, reject) => {
    ...
    // Single clip: just re-encode to ensure codec compatibility
    if (localPaths.length === 1) {
      ...
      execFile(ffmpegPath(), args, { timeout: 120000 }, (err) => {
        if (err) return reject(new Error(`FFmpeg re-encode failed: ${err.message}`));
        resolve();
      });
      return;
    }
