...
async function extractVodClips(vodUrl, opts = {}) {
  ...
  // CPD-200: simpler format for Twitch HLS. CPD-290: YouTube bot-check on Render IPs.
  // Strategy: YTDLP_PROXY → cookies+EJS → ANDROID_VR clients without cookies.
  ...
  const runGetUrl = (args, cookiePath) => new Promise((resolveGet, rejectGet) => {
    execFile(
      ytdlp,
      [
        '--get-url',
      ...
      (err, stdout) => {
        if (cookiePath) try { fs.unlinkSync(cookiePath); } catch (_) {}
  ...
  let streamUrl;
  try {
    streamUrl = await runGetUrl(extraArgs, cookieFilePath);
  } catch (firstErr) {
    if (!isYouTube || !cookieFilePath) throw firstErr;
    console.warn(`[assembly_service] ${jobId}: cookie get-url failed (${firstErr.message.slice(0, 120)}) — retry android client`);
    streamUrl = await runGetUrl(
