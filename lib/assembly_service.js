...
function _concatClips(localPaths, outputPath) {
  return new Promise((resolve, reject) => {
    ...
    // Multiple clips: use concat demuxer with stream copy (no re-encode).
    // Twitch clips are uniformly h264/aac — stream copy is safe and avoids
    // the memory spike from a libx264 re-encode that can OOM constrained instances.
    // If stream copy fails (e.g. incompatible codecs), fall back to re-encode.
    ...
    execFile(ffmpegPath(), streamCopyArgs, { timeout: 300000 }, (err) => {
      if (!err) {
        try { fs.unlinkSync(listPath); } catch (_) {}
        return resolve();
      }
      // Stream copy failed — fall back to re-encode at 720p to avoid OOM
      console.warn(`[assembly_service] stream copy failed (${err.message.slice(0, 80)}), falling back to re-encode`);
      const reencodeArgs = [
        '-f', 'concat', '-safe', '0',
        '-i', listPath,
      ...
      execFile(ffmpegPath(), reencodeArgs, { timeout: 600000 }, (err2) => {
        try { fs.unlinkSync(listPath); } catch (_) {}
...
/**
 * CPD-185: Burn a minimal AuraFlux broadcast chrome overlay onto a video using FFmpeg drawtext.
 * Adds: top bar with show name, bottom-right AuraFlux logo watermark, accent bottom line.
 *
 * This is intentionally minimal — just enough for Gemini QA to detect branding presence.
 * Full production chrome (flags, sidebars, episodic titles) is rendered in the full assembly.js path.
 *
 * @param {string} inputPath   - source video path
 * @param {string} outputPath  - destination path (may equal inputPath if caller renames)
 * @param {object} [opts]
 * @param {string} [opts.showName]    - show name for top bar (default: 'AuraFlux')
...
function _applyChrome(inputPath, outputPath, opts = {}) {
  return new Promise((resolve, reject) => {
    ...
    // Layout constants — vertically-taller videos need bigger UI elements
    const topBarH  = isVertical ? 80 : 56;
    const fontSize = isVertical ? 38 : 30;
    const logoSize = isVertical ? 30 : 24;
    const bottomY  = isVertical ? 'h-60' : 'h-44';

    // Prefer LiberationSans-Bold (installed via fonts-liberation in the Docker image).
    // Fall back to DejaVuSans-Bold, then omit fontfile to use FFmpeg's built-in font.
    const fontFile = [
    ...
    const filters = [
      ...
      `drawbox=x=0:y=0:w=iw:h=${topBarH}:color=0x0f172a@0.92:t=fill`,
      // Show name centred in top bar
      `drawtext=text='${rawName}':fontsize=${fontSize}:fontcolor=white:x=(w-text_w)/2:y=(${topBarH}-text_h)/2${fontAttr}`,
      // Bottom accent bar
      `drawbox=x=0:y=h-4:w=iw:h=4:color=0x6366f1@0.9:t=fill`,
      // AuraFlux logo watermark bottom-right
      `drawtext=text='AuraFlux':fontsize=${logoSize}:fontcolor=0x6366f1@0.85:x=w-text_w-24:y=${bottomY}${fontAttr}`,
    ].join(',');
  ...
  await new Promise((resolve, reject) => {
    execFile(
      ...
      [
        ...
        '-i', audioPath,
        '-filter_complex',
        '[0:a]volume=0.12[duck];[1:a]volume=2.2,apad[narr];[duck][narr]amix=inputs=2:duration=first:dropout_transition=2[aout]',
        '-map', '0:v',
        '-map', '[aout]',
        '-c:v', 'copy',
        '-c:a', 'aac', '-b:a', '192k',
...
async function extractVodClips(vodUrl, opts = {}) {
  const { clipCount = 3, maxClipSecs = 60, jobId = `vod_${Date.now()}` } = opts;
  ...
  const vodDurationSecs = await new Promise((resolve) => {
    execFile(
      ...
      [
        ...
        '-of', 'csv=p=0',
        '-timeout', '30000000', // 30s ffprobe timeout (microseconds)
        streamUrl,
      ],
      { timeout: 45000 },
      (err, stdout) => {
  ...
  const usableStart = Math.floor(effectiveDuration * 0.05);
