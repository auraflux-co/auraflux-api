...
async function extractVodClips(vodUrl, opts = {}) {
  ...
  const streamUrl = await new Promise((resolve, reject) => {
    ...
    // CPD-290: YouTube requires cookie auth on Render's data center IPs.
    // ANDROID_VR bypass (CPD-288) no longer works as of May 2026. If
    // YOUTUBE_COOKIES_BASE64 is set, decode and write to a temp file and pass
    // --cookies to yt-dlp. Cookie file is cleaned up after the URL resolves.
    ...
    if (isYouTube) {
      ...
      if (cookiesB64) {
        ...
        // Fallback: try ANDROID_VR — may still work on some IPs
