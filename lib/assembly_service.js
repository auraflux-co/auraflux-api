...
async function extractVodClips(vodUrl, opts = {}) {
  ...
  const streamUrl = await new Promise((resolve, reject) => {
    ...
    // CPD-288: YouTube bot detection — use ANDROID_VR/ANDROID clients which bypass
    // IP-based bot detection on Render's data center IPs. These clients are less
    // aggressively filtered by YouTube compared to the default web client.
    const extraArgs = /youtube\.com|youtu\.be/.test(vodUrl)
...
async function _assembleCompactClipSpec(jobId, jobSpec, clipSpec) {
  ...
  for (let i = 0; i < ordered.length; i++) {
    ...
    const _isPlatformUrl = /twitch\.tv\/videos\/|kick\.com\/video\/|youtube\.com\/watch|youtu\.be\//.test(clip.url);
    ...
    if (_isPlatformUrl) {
      ...
      const [extractedPath] = await extractVodClips(clip.url, {
        clipCount: 1,
      ...
      fs.renameSync(extractedPath, rawPath);
