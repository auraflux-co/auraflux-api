...
async function assembleForJob(jobSpec) {
  ...
  // ── Clip-comp creative portrait (Peaks / C1–C11) ─────────────────────────
  // Bake layout before concat so C9 full-bleed/split ≠ C1 blur-pad.
  let _compCreative = null;
  let _clipCompPortraitApplied = false;
  try {
    const wantsClipComp = !!(
      jobSpec.clipsOnly
      || jobSpec.productionPath === 'short_compile_clips'
      || jobSpec.compositionSpec?.compCreative
      || jobSpec.compositionSpec?.compCreativePreset
      || jobSpec.featureConfig?.compose?.preset
    );
    if (wantsClipComp && localPaths.length) {
      const { mergeCompCreative } = require('./clip_comp_creative');
      const presetKey = jobSpec.compositionSpec?.compCreativePreset
        || jobSpec.compositionSpec?.compCreative?.preset
        || jobSpec.featureConfig?.compose?.preset
        || 'classic_blur_pad';
      _compCreative = mergeCompCreative({
        ...(jobSpec.compositionSpec?.compCreative || {}),
        preset: presetKey,
      });
      jobSpec.compositionSpec = {
        ...(jobSpec.compositionSpec || {}),
        compCreativePreset: _compCreative.preset,
        compCreative: _compCreative,
      };
      if (jobSpec.designSpec) jobSpec.designSpec.compCreative = _compCreative;

      const { applyClipCompPortraitLayout } = require('./assembly_postprocess');
      const layoutMode = _compCreative.layout?.mode || 'blur_pad';
      const lookName = _compCreative.look?.preset || null;
      const portraitPaths = [];
      for (let i = 0; i < localPaths.length; i++) {
        const out = path.join(TMP_DIR, `${jobId}_cc_portrait_${i}.mp4`);
        await applyClipCompPortraitLayout(localPaths[i], out, {
          layoutMode,
          jobId,
          lookName,
          log: (m) => console.log(`[assembly_service] ${jobId}: ${m}`),
        });
        portraitPaths.push(out);
        try { if (localPaths[i] !== out) fs.unlinkSync(localPaths[i]); } catch (_) {}
      }
      localPaths.splice(0, localPaths.length, ...portraitPaths);
      _clipCompPortraitApplied = true;
      console.log(`[assembly_service] ${jobId}: clip-comp portrait (${_compCreative.preset} / ${layoutMode})`);
    }
  } catch (ccErr) {
    console.warn(`[assembly_service] ${jobId}: clip-comp portrait skipped: ${ccErr.message}`);
  }
  ...
  if (assembledVideoUrl) jobSpec.state.savedOutputs.cleanVideoUrl = assembledVideoUrl;
  if (_clipCompPortraitApplied) {
    jobSpec.state.savedOutputs.layoutPortraitApplied = true;
    jobSpec.state.savedOutputs.compCreativePreset = _compCreative?.preset || null;
  }
  ...
  // Skip when clip-comp portrait already applied (Peaks / C1–C11).
  const _needsVertical = !_clipCompPortraitApplied && (
  ...
  if (_needsVertical) {
    ...
    try {
      const { exportProfileForTier } = require('./services/plan_entitlements');
      const _exportProfile = exportProfileForTier(jobSpec.planTier || jobSpec.plan_tier || 'operate');
      await _applyVerticalCrop(outputPath, croppedPath, _exportProfile);
...
// Derive Twitch clip MP4 URL via Helix (CPD-349 thumbnailToMp4 public CDN).
// Policy: assembly uses Helix only until clipzworldnews test channel is ready — no GQL.
async function _resolveTwitchClipHelixCdnUrl(slug) {
  const TwitchClient = require('./clients/twitch_client');
  const client = new TwitchClient();
  return client.resolveClipCdnFromHelix(slug);
}

// Extract clip id from Helix CDN URL (clips-media-assets2.twitch.tv/{id}.mp4).
function _extractTwitchClipIdFromCdnUrl(url) {
  if (!url) return '';
  const m = url.match(/clips-media-assets2\.twitch\.tv\/([^/?]+)\.mp4/);
  return m ? m[1] : '';
}

// Download a Twitch clip by resolving Helix metadata → public CDN URL.
async function _downloadTwitchClipDirect(jobId, url, destPath) {
  const slug = _extractTwitchSlug(url) || _extractTwitchClipIdFromCdnUrl(url);
  if (!slug) throw new Error(`Cannot extract Twitch clip id from: ${url.slice(0, 100)}`);
  console.log(`[assembly_service] ${jobId}: resolving Twitch clip via Helix (id: ${slug})`);
  const mp4Url = await _resolveTwitchClipHelixCdnUrl(slug);
  console.log(`[assembly_service] ${jobId}: downloading Twitch clip from Helix CDN URL`);
  return _downloadFile(mp4Url, destPath);
}

// Helix CDN first (metadata via Helix); yt-dlp on page URL when CDN file is invalid.
// Matches portal0: Helix for resolution, yt-dlp when clips-media-assets2 probe fails.
async function _downloadTwitchClipPage(jobId, pageUrl, destPath) {
  try {
    await _downloadTwitchClipDirect(jobId, pageUrl, destPath);
    const dur = await _getVideoDuration(destPath).catch(() => 0);
    if (dur >= 0.5) return;
    console.warn(
      `[assembly_service] ${jobId}: Helix CDN clip invalid duration (${dur}s) — falling back to yt-dlp`
    );
    try { fs.unlinkSync(destPath); } catch (_) {}
  } catch (helixErr) {
    console.warn(
      `[assembly_service] ${jobId}: Helix CDN download failed (${helixErr.message.slice(0, 80)}) — falling back to yt-dlp`
    );
  }
  await _downloadWithYtdlp(jobId, pageUrl, destPath);
}


/**
 * Scans execFile/subprocess stderr (or Error.message) for YouTube datacenter /
 * bot-check signatures used across yt-dlp wrappers.
 * @param {string} stderr
 * @returns {boolean}
 */
function isDatacenterBlock(stderr) {
  if (!stderr) return false;
  const s = String(stderr);
  if (s.includes('[YOUTUBE_IP_BLOCKED]')) return true;
  return /bot|sign in|confirm|429|403|unavailable|blocked/i.test(s);
}
...
async function _downloadWithYtdlp(jobId, url, destPath) {
  ...
  if (proxyUrl) {
    ...
    // CPD-353: android/mweb client — extractor-args, not --format.
    extraArgs.push('--extractor-args', 'youtube:player_client=android,mweb');
    console.log(`[assembly_service] ${jobId}: using android/mweb client for YouTube clip download`);
  }
    ...
    execFile(
      ...
      (err, _stdout, stderr) => {
        if (err) {
          ...
          if (_isYouTubeUrl(url) && isDatacenterBlock(detail)) {
...
/** Format seconds → HH:MM:SS for yt-dlp --download-sections. */
function _ytdlpSectionHms(sec) {
...
/**
 * Download a YouTube time window via --download-sections (Peaks / VOD extract).
 * Prefer YTDLP_PROXY; else android/mweb; optional useCookies → YOUTUBE_COOKIES_BASE64 + Deno EJS.
 */
async function _downloadYoutubeSection(jobId, url, destPath, startS, endS, { useCookies = false } = {}) {
  const ytdlp = process.env.YTDLP_PATH || 'yt-dlp';
  const start = Math.max(0, Math.floor(Number(startS) || 0));
  let end = Math.floor(Number(endS) || (start + 45));
  if (end <= start) end = start + 45;
  if (end - start > 180) end = start + 180;
  const section = `*${_ytdlpSectionHms(start)}-${_ytdlpSectionHms(end)}`;
  const extraArgs = [];
  let cookieFilePath = null;
  const proxyUrl = process.env.YTDLP_PROXY;
  if (proxyUrl) {
    extraArgs.push('--proxy', proxyUrl);
  } else if (useCookies && process.env.YOUTUBE_COOKIES_BASE64) {
    cookieFilePath = path.join(TMP_DIR, `yt_cookies_${jobId}.txt`);
    fs.writeFileSync(cookieFilePath, Buffer.from(process.env.YOUTUBE_COOKIES_BASE64, 'base64').toString('utf8'));
    extraArgs.push('--cookies', cookieFilePath);
    extraArgs.push('--remote-components', 'ejs:github');
    extraArgs.push('--no-update');
  } else {
    extraArgs.push('--extractor-args', 'youtube:player_client=android,mweb');
  }
  const mode = proxyUrl ? 'proxy' : (useCookies ? 'cookies' : 'android/mweb');
  console.log(`[assembly_service] ${jobId}: YouTube section ${section} via ${mode}`);
  try {
    await new Promise((resolve, reject) => {
      execFile(
        ytdlp,
        ...
        (err, _stdout, stderr) => {
          if (err) {
            const detail = String(stderr || err.message || '').replace(/\s+/g, ' ').slice(0, 240);
            if (isDatacenterBlock(detail)) {
              return reject(new Error(`[YOUTUBE_IP_BLOCKED] yt-dlp YouTube section failed (${mode}): ${detail}`));
            }
            return reject(new Error(`yt-dlp YouTube section failed (${mode}): ${detail}`));
          }
          resolve();
        },
      );
    ...
    if (cookieFilePath) {
      try { fs.unlinkSync(cookieFilePath); } catch (_) { /* ignore */ }
    }
  }
}
  ...
  for (let i = 0; i < urls.length; i++) {
    ...
    try {
      if (_isTwitchCdnUrl(url)) {
        ...
        // if expired/403, re-resolve clip id via Helix thumbnailToMp4 CDN.
        console.log(`[assembly_service] ${jobId}: Twitch CDN URL — attempting direct download, Helix fallback on failure`);
        try {
          await _downloadFile(url, tmpPath);
        } catch (_cdnErr) {
          console.warn(`[assembly_service] ${jobId}: CDN download failed (${_cdnErr.message.slice(0, 60)}) — re-resolving via Helix`);
          await _downloadTwitchClipPage(jobId, url, tmpPath);
        }
      } else if (_isTwitchClipPageUrl(url)) {
        // Helix CDN first; yt-dlp on page URL when CDN file is invalid (portal0 parity).
        await _downloadTwitchClipPage(jobId, url, tmpPath);
        ...
        // YouTube watch URLs require yt-dlp. _downloadWithYtdlp uses android/mweb client
        // as a bot-detection bypass when no proxy is configured.
        console.log(`[assembly_service] ${jobId}: YouTube URL — downloading via yt-dlp (android/mweb): ${url.slice(0, 80)}`);
 ...
 */
function _applyVerticalCrop(inputPath, outputPath, exportProfile = {}) {
  const width = Number(exportProfile.width) || 1080;
  const height = Number(exportProfile.height) || 1920;
  const fps = Number(exportProfile.fps) || 60;
  // Memory-safe encode for Render starter: 1 thread + veryfast + Level 4.2 for 1080p60.
  const { PROFILE_HIGH_42 } = require('./ffmpeg_builder');
  return new Promise((resolve, reject) => {
    execFile(
      ffmpegPath(),
      [
 ...
 */
/**
 * CPD-1006: Build FFmpeg filter_complex for chrome + dual-logo overlay.
 * - Sub-brand avatar: lower-third left (when brandLogoPath set)
 * - AuraFlux watermark: always bottom-right when platformLogoPath set
 */
function _buildDualLogoFilterComplex({
  baseStream,
  baseFilters,
  brandLogoPath,
  platformLogoPath,
  ...
  if (baseFilters) {
    parts.push(`[${stream}]${baseFilters}[chromed]`);
    stream = 'chromed';
  }

  if (brandLogoPath) {
    parts.push(`[${brandInputIdx}:v]scale=${brandLogoW}:-1,format=rgba,colorchannelmixer=aa=0.92[brandlogo]`);
    parts.push(`[${stream}][brandlogo]overlay=x=20:y=${ltBarY}+12[withbrand]`);
    stream = 'withbrand';
  }

  if (platformLogoPath) {
    parts.push(`[${platformInputIdx}:v]scale=${platformLogoW}:-1,format=rgba,colorchannelmixer=aa=0.80[platlogo]`);
    parts.push(`[${stream}][platlogo]overlay=x=W-w-16:y=H-h-16[vout]`);
  } else {
    const ftxt = `drawtext=text='AuraFlux':fontsize=${logoTextSize}:fontcolor=white@0.75:x=w-text_w-16:y=H-th-16${fontAttr}`;
    parts.push(`[${stream}]${ftxt}[vout]`);
  }
...
async function _applyChrome(inputPath, outputPath, opts = {}) {
  ...
  return new Promise((resolve, reject) => {
    ...
    // CPD-509 / CPD-1006: sub-brand logo (Twitch avatar) + AuraFlux platform watermark
    const { AURAFLUX_WATERMARK_PATH } = require('./services/branding_assets');
    const fs = require('fs');
    const brandLogoPath = opts.brandLogoPath && fs.existsSync(opts.brandLogoPath)
      ? opts.brandLogoPath
      : null;
    const platformLogoPath = opts.platformWatermark !== false &&
      fs.existsSync(AURAFLUX_WATERMARK_PATH)
      ? AURAFLUX_WATERMARK_PATH
      : null;

    const brandingActive = opts.brandingActive !== false;
    ...
    const brandLogoW      = isVertical ? 64 : 48;
    const platformLogoW   = isVertical ? 44 : 36;
    ...
    // opts.hasTopBar=false (default for clip/extract jobs) suppresses the top bar so
    // portal3a doesn't penalise it as an unexpected chrome element.
    const showTopBar = opts.hasTopBar !== false;
    const baseFilters = brandingActive ? [
      ...(showTopBar ? [
    ...
    // Collect logo inputs: brand first, platform second
    const logoInputs = [];
    if (brandLogoPath) logoInputs.push(brandLogoPath);
    if (platformLogoPath) logoInputs.push(platformLogoPath);
    const brandInputIdx    = brandLogoPath ? 1 : null;
    const platformInputIdx = platformLogoPath ? (brandLogoPath ? 2 : 1) : null;
    ...
    if (needsPortraitReframe) {
      ...
      const portraitOverlay = _buildDualLogoFilterComplex({
        baseStream: 'reframed',
      ...
      const fcFinal = [portraitSteps, portraitOverlay].join(';');
      execArgs = ['-i', inputPath, ...logoInputs.flatMap((p) => ['-i', p]), '-filter_complex', fcFinal, '-map', '[vout]', '-map', '0:a?'];
    } else if (brandLogoPath || platformLogoPath || baseFilters) {
      const landscapeOverlay = _buildDualLogoFilterComplex({
        baseStream: 'scaled',
        baseFilters: baseFilters || null,
        brandLogoPath,
        platformLogoPath,
      ...
      const fc = [`[0:v]scale=-2:min(ih\\,720)[scaled]`, landscapeOverlay].join(';');
      execArgs = ['-i', inputPath, ...logoInputs.flatMap((p) => ['-i', p]), '-filter_complex', fc, '-map', '[vout]', '-map', '0:a?'];
    } else {
      const vf = `scale=-2:min(ih\\,720),drawtext=text='AuraFlux':fontsize=${logoTextSize}:fontcolor=white@0.75:x=w-text_w-16:y=H-th-16${fontAttr}`;
...
/**
 * Seek/trim a remote or local stream URL into a clip file (get-url + ffmpeg path).
 */
function _ffmpegTrimFromStream(jobId, streamUrl, clipPath, startSec, durationSec, isVertical) {
  return new Promise((resolve, reject) => {
    execFile(
      ffmpegPath(),
      [
      ...
      (err) => {
        if (err) {
          ...
          if (isDatacenterBlock(detail)) {
            return reject(new Error(`[YOUTUBE_IP_BLOCKED] Clip extraction failed: ${detail.slice(0, 160)}`));
          }
          return reject(new Error(`Clip extraction failed: ${err.message}`));
        }
        resolve();
      },
    );
...
async function extractVodClips(vodUrl, opts = {}) {
  ...
  // Prefer YTDLP_PROXY; else android/mweb. Cookies are a Render fallback only (after android fails).
  const isYouTube = /youtube\.com|youtu\.be/.test(vodUrl);
  const hasTimestamps = !!(vodClipTimestamps && vodClipTimestamps.length >= clipCount);
  const extraArgs = [];
  const proxyUrl = process.env.YTDLP_PROXY;
  if (proxyUrl) {
    extraArgs.push('--proxy', proxyUrl);
    ...
    extraArgs.push('--extractor-args', 'youtube:player_client=android,mweb');
    console.log(`[assembly_service] ${jobId}: using android/mweb client for YouTube VOD resolve`);
  }

  const runGetUrl = (args) => new Promise((resolveGet, rejectGet) => {
    execFile(
      ytdlp,
      [
        '--get-url',
      ...
      (err, stdout, stderr) => {
        if (err) {
          const detail = String(stderr || err.message || '').replace(/\s+/g, ' ').slice(0, 200);
          if (isDatacenterBlock(detail)) {
            return rejectGet(new Error(`[YOUTUBE_IP_BLOCKED] yt-dlp URL resolve failed: ${detail}`));
          }
          return rejectGet(new Error(`yt-dlp URL resolve failed: ${detail}`));
        }
        const url = (stdout || '').split('\n').find((l) => l.trim().startsWith('http'));
        if (!url) return rejectGet(new Error('yt-dlp returned no stream URL'));
        resolveGet(url.trim());
      },
    );
  });

  // Peaks / COMPACT windows: prefer --download-sections; get-url+ffmpeg is fallback.
  let streamUrl;
  let useSectionDownload = isYouTube && hasTimestamps;
  if (useSectionDownload) {
    console.log(`[assembly_service] ${jobId}: YouTube timestamps present — section download (skip get-url)`);
  } else {
    try {
      streamUrl = await runGetUrl(extraArgs);
      console.log(`[assembly_service] ${jobId}: stream URL resolved (${String(streamUrl).slice(0, 80)}...)`);
    } catch (getUrlErr) {
      if (!isYouTube) throw getUrlErr;
      console.warn(`[assembly_service] ${jobId}: get-url failed (${getUrlErr.message.slice(0, 160)}) — section download`);
      useSectionDownload = true;
    }
  }

  let effectiveDuration = 1200;
  if (!useSectionDownload) {
    ...
    effectiveDuration = vodDurationSecs > 10 ? vodDurationSecs : 1200;
  ...
  if (hasTimestamps) {
    ...
    endSeconds = vodClipTimestamps.slice(0, clipCount).map((t) =>
      t.end_s != null ? t.end_s : (Number(t.start_s) || 0) + maxClipSecs
    );
    console.log(`[assembly_service] ${jobId}: using ${startSeconds.length} viewer-highlight timestamps: ${startSeconds.join(', ')}s`);
  } else if (useSectionDownload) {
    throw new Error('YouTube section download requires vodClipTimestamps (Peaks windows)');
  } else {
    const skipByPercent  = Math.floor(effectiveDuration * 0.30);
    const skipMin        = Math.min(1800, Math.floor(effectiveDuration * 0.20));
    const skipCap        = Math.min(Math.floor(effectiveDuration * 0.40), 3600);
  ...
  async function resolveStreamUrlLazy() {
    if (streamUrl) return streamUrl;
    streamUrl = await runGetUrl(extraArgs);
    console.log(`[assembly_service] ${jobId}: stream URL resolved for splice fallback (${String(streamUrl).slice(0, 80)}...)`);
    return streamUrl;
  }

  for (let i = 0; i < clipCount; i++) {
    const startSec = startSeconds[i];
    const endSec = endSeconds
      ? endSeconds[i]
      : startSec + maxClipSecs;
    const durationSec = Math.max(1, (endSeconds ? endSec - startSec : maxClipSecs));
    ...
    if (useSectionDownload) {
      let sectionErr = null;
      try {
        await _downloadYoutubeSection(jobId, vodUrl, clipPath, startSec, endSec, { useCookies: false });
      } catch (androidErr) {
        sectionErr = androidErr;
        if (process.env.YOUTUBE_COOKIES_BASE64 || process.env.YTDLP_PROXY) {
          console.warn(`[assembly_service] ${jobId}: android/mweb section failed (${androidErr.message.slice(0, 140)}) — retry cookies/proxy`);
          try {
            await _downloadYoutubeSection(jobId, vodUrl, clipPath, startSec, endSec, { useCookies: true });
            sectionErr = null;
          } catch (cookieErr) {
            sectionErr = cookieErr;
          }
        }
      }

      if (sectionErr) {
        // Bandwidth-aware fallback: get-url + ffmpeg splice (not full VOD download).
        console.warn(`[assembly_service] ${jobId}: section download failed (${sectionErr.message.slice(0, 140)}) — get-url + ffmpeg splice`);
        try {
          const url = await resolveStreamUrlLazy();
          await _ffmpegTrimFromStream(jobId, url, clipPath, startSec, durationSec, isVertical);
        } catch (spliceErr) {
          if (isDatacenterBlock(sectionErr.message) || isDatacenterBlock(spliceErr.message)) {
            throw new Error(
              `[YOUTUBE_IP_BLOCKED] YouTube section and splice failed. ` +
              `Set YTDLP_PROXY (residential). Last errors: section=${sectionErr.message.slice(0, 100)}; splice=${spliceErr.message.slice(0, 100)}`
            );
          }
          throw spliceErr;
        }
      ...
      if (fs.existsSync(clipPath) && _fileSizeMb(clipPath) > 0.1) {
        ...
        console.log(`[assembly_service] ${jobId}: clip ${i + 1}/${clipCount} section-downloaded (${_fileSizeMb(clipPath).toFixed(1)} MB)`);
      }
      continue;
    }

    await _ffmpegTrimFromStream(jobId, streamUrl, clipPath, startSec, durationSec, isVertical);
    if (fs.existsSync(clipPath) && _fileSizeMb(clipPath) > 0.1) {
      clipPaths.push(clipPath);
...
async function _assembleCompactClipSpec(jobId, jobSpec, clipSpec) {
  ...
  for (let i = 0; i < ordered.length; i++) {
    ...
    if (clip.cdnUrl && _isTwitchCdnUrl(clip.cdnUrl)) {
      await _downloadFile(clip.cdnUrl, rawPath);
      console.log(`[assembly_service] ${jobId}: COMPACT clip ${i + 1} downloaded via Helix CDN: ${clip.cdnUrl.slice(0, 60)}`);
    } else if (_isTwitchClipPageUrl(clip.url)) {
      await _downloadTwitchClipPage(jobId, clip.url, rawPath);
      console.log(`[assembly_service] ${jobId}: COMPACT clip ${i + 1} downloaded via Helix: ${clip.url.slice(0, 60)}`);
