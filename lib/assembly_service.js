...
async function extractVodClips(vodUrl, opts = {}) {
  ...
  const vodLocal = path.join(TMP_DIR, `vod_${jobId}.mp4`);

  // Step 1 — Download VOD (best quality up to 720p to keep size manageable)
  console.log(`[assembly_service] ${jobId}: downloading VOD from ${vodUrl}`);
  await new Promise((resolve, reject) => {
    execFile(
      ytdlp,
      [
      ...
      (err) => {
        if (err) return reject(new Error(`yt-dlp VOD download failed: ${err.message}`));
        resolve();
      },
    );
  });

  if (!fs.existsSync(vodLocal)) throw new Error('yt-dlp produced no output file');
  console.log(`[assembly_service] ${jobId}: VOD downloaded (${_fileSizeMb(vodLocal).toFixed(1)} MB)`);

  // Step 2 — Probe VOD duration
  const vodDurationSecs = await new Promise((resolve) => {
    execFile(
      ffmpegPath().replace('ffmpeg', 'ffprobe'),
      ['-v', 'error', '-show_entries', 'format=duration', '-of', 'csv=p=0', vodLocal],
      { timeout: 30000 },
      (err, stdout) => {
  ...
  if (vodDurationSecs < 10) throw new Error(`VOD too short to extract clips: ${vodDurationSecs}s`);
  console.log(`[assembly_service] ${jobId}: VOD duration ${Math.round(vodDurationSecs)}s`);

  // Step 3 — Extract N evenly-spaced clips from the middle third of the VOD
  // Skip first and last 5% to avoid stream startup/outro artifacts.
  const usableStart = Math.floor(vodDurationSecs * 0.05);
  const usableEnd   = Math.floor(vodDurationSecs * 0.95);
  ...
  for (let i = 0; i < clipCount; i++) {
    ...
    if (fs.existsSync(clipPath) && _fileSizeMb(clipPath) > 0.1) {
      ...
      console.log(`[assembly_service] ${jobId}: extracted clip ${i + 1}/${clipCount} (${Math.round(startSec)}s)`);
    }
  }

  // Clean up the full VOD download
  try { fs.unlinkSync(vodLocal); } catch (_) {}

  if (!clipPaths.length) throw new Error('No clips extracted from VOD');
  return clipPaths;
}

module.exports = { assembleForJob, mixTtsIntoVideo, extractVodClips };
