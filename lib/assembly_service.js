...
/**
 * Download a YouTube time window via android/mweb + --download-sections (Peaks / VOD extract).
 * Same client as _downloadWithYtdlp — no cookies.
 */
async function _downloadYoutubeSection(jobId, url, destPath, startS, endS) {
  const ytdlp = process.env.YTDLP_PATH || 'yt-dlp';
  ...
  console.log(`[assembly_service] ${jobId}: YouTube section ${section} via android/mweb`);
  return new Promise((resolve, reject) => {
    execFile(
      ytdlp,
      [
      ...
      (err) => {
        if (err) return reject(new Error(`yt-dlp YouTube section failed: ${err.message.slice(0, 160)}`));
        resolve();
      },
    );
  });
...
async function extractVodClips(vodUrl, opts = {}) {
  ...
  // CPD-200 / CPD-353: Match _downloadWithYtdlp — android/mweb on YouTube, no cookies.
  // Cookies-first was a Peaks detour and breaks Render (web clients + bot check).
  // Prefer YTDLP_PROXY when set; otherwise android/mweb like the 6-month job path.
  ...
  const runGetUrl = (args) => new Promise((resolveGet, rejectGet) => {
    execFile(
      ...
      (err, stdout) => {
        if (err) return rejectGet(new Error(`yt-dlp URL resolve failed: ${err.message}`));
  ...
  try {
    ...
    // Proven job path for bounded windows: android/mweb + --download-sections (no cookies, no full VOD).
    console.warn(`[assembly_service] ${jobId}: get-url failed (${getUrlErr.message.slice(0, 120)}) — android/mweb section download`);
    useSectionDownload = true;
  }

  // Probe VOD duration only when we have a stream URL (section-download path uses timestamps).
  ...
  // CPD-246: Use highlight timestamps when available — else evenly-spaced cuts.
  let startSeconds;
  let endSeconds = null;
  ...
  for (let i = 0; i < clipCount; i++) {
    ...
    if (useSectionDownload) {
      await _downloadYoutubeSection(jobId, vodUrl, clipPath, startSec, endSec);
