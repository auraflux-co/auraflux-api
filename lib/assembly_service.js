  ...
  for (let i = 0; i < urls.length; i++) {
    ...
    try {
      ...
      if (_isTwitchClipPageUrl(url)) {
        // Throttle sequential yt-dlp calls to avoid Twitch GQL rate-limiting on multi-clip jobs.
        if (i > 0) await new Promise((r) => setTimeout(r, 2000));
        let ytdlpErr;
        for (let attempt = 0; attempt < 3; attempt++) {
          if (attempt > 0) {
            console.log(`[assembly_service] ${jobId}: yt-dlp retry ${attempt}/2 after 8s backoff for clip ${i}`);
...
async function _assembleCompactClipSpec(jobId, jobSpec, clipSpec) {
  ...
  for (let i = 0; i < ordered.length; i++) {
    ...
    if (_isPlatformUrl) {
      // Use yt-dlp to extract a single clip segment using the trim points as timestamps.
      // CPD-347: Retry up to 3 times with 8s backoff — Twitch GQL rate-limits rapid
      // sequential yt-dlp calls from datacenter IPs (same issue as _downloadClips).
      if (i > 0) await new Promise((r) => setTimeout(r, 2000)); // pre-call throttle for clips 2+
