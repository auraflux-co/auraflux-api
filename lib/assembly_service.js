...
function _concatClips(localPaths, outputPath) {
  return new Promise((resolve, reject) => {
    ...
    // clips differ in codec level, profile, or timebase — only clip 1's video frames
    // are decodable (e.g. 26s) while the container reports the full duration (e.g. 71.4s).
    // CPD-229 fixed audio stream copy but video stream copy is equally unreliable.
    // libx264 decode+re-encode normalises all codec parameters so every clip's video
    // data is correctly written and decodable. CRF 22 veryfast preserves near-source
    // quality; dynaudnorm is then run separately to smooth loudness boundaries.
    const primaryArgs = [
      '-f', 'concat', '-safe', '0',
      '-i', listPath,
