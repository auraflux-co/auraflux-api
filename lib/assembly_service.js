...
async function _assembleCompactClipSpec(jobId, jobSpec, clipSpec) {
  ...
  for (let i = 0; i < ordered.length; i++) {
    ...
    if (_isTwitchClipPageUrl(clip.url)) {
      // CPD-347: Twitch clip pages — download full clip via yt-dlp, then apply ffmpeg trim below.
      // Render datacenter IPs are rate-limited to ~2 Twitch GQL calls/minute; portal0 + clip 0
      // consume both slots, so clips 2+ need a 60s gap before their first yt-dlp attempt.
      if (i > 0) await new Promise((r) => setTimeout(r, 60000));
      let _ytdlpErr;
