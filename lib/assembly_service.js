  ...
  for (let i = 0; i < urls.length; i++) {
    ...
    try {
      ...
      if (_isTwitchCdnUrl(url)) {
        // CPD-899: Twitch CDN (cloudfront.net/nauth/) URLs expire in ~30 minutes.
        // If we stored a CDN URL in portal0 (legacy), attempt direct download; on
        // 403/expired fall back to yt-dlp re-resolve so we still get the clip.
        console.log(`[assembly_service] ${jobId}: Twitch CDN URL detected — attempting direct download, yt-dlp fallback on failure`);
        try {
          await _downloadFile(url, tmpPath);
        } catch (_cdnErr) {
          console.warn(`[assembly_service] ${jobId}: Twitch CDN download failed (${_cdnErr.message.slice(0, 60)}) — re-resolving via yt-dlp`);
  ...
  for (let i = 0; i < ordered.length; i++) {
    ...
    if (_isTwitchClipPageUrl(clip.url)) {
      ...
      if (!_downloaded) {
        ...
        // CPD-465: capped at 2 attempts (was 4) — 4×120s+3×30s = 9.5min tied up memory causing OOM.
        if (i > 0) await new Promise((r) => setTimeout(r, 60000));
        let _ytdlpErr;
...
module.exports = { assembleForJob: assembleForJobQueued, assembleCarousel: assembleCarouselQueued, mixTtsIntoVideo, mixAlignedMultiClipTts, extractVodClips, applyChrome: _applyChrome };
