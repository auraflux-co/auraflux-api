...
function _concatClips(localPaths, outputPath) {
  return new Promise((resolve, reject) => {
    ...
    // CPD-229: Force audio re-encode to AAC 44100Hz/stereo during concat.
    // Twitch clips from different streamers can have different audio sample rates
    // (44100Hz vs 48000Hz) or channel layouts. Stream-copy concat (-c copy) silently
    // drops audio from clips 2..N when parameters differ. The result is a video with
    // full-length video track but only the first clip's audio duration — e.g. 71s video
    // with 26s audio. mixTtsIntoVideo then sees amix duration=first=26s and truncates.
    // Forcing -c:a aac re-encode ensures consistent audio across all clips.
    // Video is still stream-copied (fast, no quality loss).
    ...
    // Video stream is copied (fast); only audio is re-encoded (~3-5s/min overhead).
    ...
    execFile(ffmpegPath(), streamCopyArgs, { timeout: 300000 }, (err) => {
      ...
      // Stream copy failed — fall back to re-encode at 720p
