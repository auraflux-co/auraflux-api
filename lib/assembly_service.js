...
function _concatClips(localPaths, outputPath) {
  return new Promise((resolve, reject) => {
    ...
    const vFilters = localPaths.map((_, i) =>
      `[${i}:v]scale=${CONCAT_W}:${CONCAT_H}:force_original_aspect_ratio=decrease,` +
...
async function mixTtsIntoVideo(videoPath, audioPath, jobId = 'unknown') {
  ...
  const audioPadSecs = videoDur ? Math.ceil(videoDur) + 2 : 300;
...
async function extractVodClips(vodUrl, opts = {}) {
  ...
  if (vodClipTimestamps && vodClipTimestamps.length >= clipCount) {
    ...
    // Use max(25% of VOD, 900s min) but cap at 30 min (1800s) so we don't over-skip short VODs.
    const skipByPercent  = Math.floor(effectiveDuration * 0.25);
    const skipMin        = Math.min(900, Math.floor(effectiveDuration * 0.15)); // at least 15%
    const skipCap        = 1800; // never skip more than 30 min regardless of VOD length
