...
function getPipelineQueue() {
  return _makeQueue(QUEUES.PIPELINE);
}
function getGateQueue() {
  return _makeQueue(QUEUES.GATE);
}
function getHeyGenQueue() {
  return _makeQueue(QUEUES.HEYGEN_POLL);
}
function getAssemblyQueue() {
  return _makeQueue(QUEUES.ASSEMBLY);
}
function getUploadQueue() {
  return _makeQueue(QUEUES.UPLOAD);
}
function getMonitoringQueue() {
  return _makeQueue(QUEUES.MONITORING);
}
...
async function addGateJob(queueName, gateName, jobId, payload) {
  ...
  try {
    ...
    canonicalJobId = resolveCanonicalJobIdSync(jobId);
  } catch (_e) {
