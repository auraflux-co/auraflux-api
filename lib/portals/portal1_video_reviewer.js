'use strict';
/**
 * Portal 1b — Gemini video reviewer (independent of the script-writing Gemini).
 * Watches the same source clip the job confirmed at Portal 0 and checks whether
 * the spoken script invents verifiable facts that contradict the video.
 *
 * Controlled by env PORTAL1_VIDEO_REVIEW (default on). Set PORTAL1_VIDEO_REVIEW=0 to skip.
 * Backward-compat: GATE1_VIDEO_REVIEW=0 is also respected.
 *
 * Source downloads are cached on disk (keyed by URL + max seconds) so Portal 1
...
const GEMINI_MODEL =
  process.env.GEMINI_SCRIPT_MODEL || process.env.GEMINI_MODEL || 'gemini-2.5-flash';
const GEMINI_APIKEY = process.env.GEMINI_API_KEY;

const DEFAULT_MAX_SECS = 90;
const CACHE_TTL_MS = Math.max(
  60_000,
  parseInt(process.env.GATE1_VIDEO_CACHE_TTL_MS || String(20 * 60 * 1000), 10)
);
...
function cacheRootPath() {
  ...
  return path.join(os.tmpdir(), 'auraflux_gate1b_video_cache');
}
...
function contentTypeNeedsVideoReview(ct) {
  ...
  // Video review applies to long-form content types where clips are the primary source.
  // 'sports' covers NBA/basketball, 'news' covers any article-driven content.
  return s.includes('sports') || s.includes('nba') || s.includes('news');
}
...
async function reviewScriptAgainstVideo({ jobSpec, script, gate0Report }) {
  const videoReviewEnabled = process.env.PORTAL1_VIDEO_REVIEW ?? process.env.GATE1_VIDEO_REVIEW;
  if (videoReviewEnabled === '0') {
    return { skipped: true, reason: 'PORTAL1_VIDEO_REVIEW=0' };
  }
  if (!GEMINI_APIKEY) {
  ...
  const tmpPath = path.join(
    os.tmpdir(),
    `gate1b_${jobId.replace(/[^a-z0-9_-]/gi, '_')}_${Date.now()}.mp4`
  );
...
Return JSON ONLY:
{
  ...
  "observedSummary": "one sentence what the video actually shows (teams/moment, no invented scores)"
    ...
    captureAIMemoryTrace({
      ...
      metadata: { contentType, source: 'lib/portals/portal1_video_reviewer' },
    });
    ...
    const observedSummary =
      typeof parsed.observedSummary === 'string' ? parsed.observedSummary : '';
