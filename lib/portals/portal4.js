'use strict';
/**
 * lib/portals/portal4.js — Portal 4: Gemini Full Video Broadcast Ready
 *
...
async function preflightForUpload(assembledPath, jobSpec) {
  ...
  if (!assembledPath) {
    ...
    logError('PORTAL4_PREFLIGHT_NO_PATH', new Error(result.failReason), {
      jobId: jobSpec?.jobId,
    });
    return result;
  }
  ...
  try {
    ...
    logError('PORTAL4_PREFLIGHT_FILE_MISSING', new Error(result.failReason), {
      jobId: jobSpec?.jobId,
      assembledPath,
    });
    return result;
  ...
  if (result.sizeBytes < MIN_FILE_SIZE) {
    ...
    logError('PORTAL4_PREFLIGHT_TOO_SMALL', new Error(result.failReason), {
      jobId: jobSpec?.jobId,
  ...
  if (result.duration === null) {
    ...
    logError('PORTAL4_PREFLIGHT_NO_DURATION', new Error(result.failReason), {
      jobId: jobSpec?.jobId,
      assembledPath,
    });
    return result;
  ...
  if (result.duration < 5) {
    ...
    logError('PORTAL4_PREFLIGHT_SHORT_VIDEO', new Error(result.failReason), {
      jobId: jobSpec?.jobId,
  ...
  if (!result.sha256) {
    ...
    logError('PORTAL4_PREFLIGHT_HASH_FAIL', new Error(result.failReason), {
      jobId: jobSpec?.jobId,
      assembledPath,
    });
    return result;
 ...
 * Falls back to AuraFlux platform defaults (neutral dark theme) if nothing is set.
 * Each customer configures their brand via jobSpec.designSpec.chrome.skinOverrides
 * or their customerConfig.designDefaults.chrome colors.
 *
 * Returns:
 * {
 *   primaryColor: string,
...
function resolveChromeSkinOverride(jobSpec) {
  ...
  // Per-job override: jobSpec.designSpec.chrome.skinOverrides takes highest precedence
  if (overrides && typeof overrides === 'object') {
    return {
      primaryColor: overrides.primaryColor || PLATFORM_DEFAULTS.primaryColor,
      accentColor:  overrides.accentColor  || PLATFORM_DEFAULTS.accentColor,
      textColor:    overrides.textColor    || PLATFORM_DEFAULTS.textColor,
      skinName:     overrides.skinName     || PLATFORM_DEFAULTS.skinName,
    };
  }

  // Fallback: read from customerConfig designDefaults
  try {
    const { loadCustomerConfig } = require('../customerConfig');
    const cfg = loadCustomerConfig(jobSpec?.customerId);
    const chromeCfg = cfg?.designDefaults?.chrome;
    if (chromeCfg) {
      return {
        primaryColor: chromeCfg.colors?.primary  || PLATFORM_DEFAULTS.primaryColor,
        accentColor:  chromeCfg.colors?.accent   || PLATFORM_DEFAULTS.accentColor,
        textColor:    chromeCfg.colors?.text      || PLATFORM_DEFAULTS.textColor,
        skinName:     chromeCfg.name             || PLATFORM_DEFAULTS.skinName,
      };
    }
  } catch (_) { /* no config — use platform defaults */ }

  const skin = jobSpec?.designSpec?.chrome?.skin;
  return { ...PLATFORM_DEFAULTS, skinName: skin || PLATFORM_DEFAULTS.skinName };
}
...
// CPD-892: suggestive mode — when Gemini upload or analysis fails, pass_with_notes instead of blocking.
// Set PORTAL4_SUGGESTIVE_MODE=true (or customer qaMode='suggestive') to enable.
const PORTAL4_SUGGESTIVE_MODE_DEFAULT = process.env.PORTAL4_SUGGESTIVE_MODE === 'true';
...
function canProduce(jobSpec) {
  ...
  if (!assembledPath) {
    ...
    // Files > MAX_GEMINI_UPLOAD_BYTES (480MB) are handled in run() with a Portal 3a fallback.
    // No upper-bound rejection here — run() handles all oversized files gracefully.
  }

  // Portal 3b must have passed
...
function commit(jobSpec) {
  ...
  return {
    committed: `I will watch the complete assembled video and confirm it is broadcast ready — overall pacing end to end, audio quality across full runtime, no compounding issues, content accuracy vs Portal 1 committed script summary, brand representation. I verify thumbnailDriveUrl is present in savedOutputs (thumbnail generated in parallel during Portal 3a) — missing thumbnail is a warning not a blocker. Pass authorizes upload to: ${platforms.join(', ')}.`,
  };
}
 ...
 */
function buildUpstreamContext(priorPortalReports) {
 ...
 */
async function analyzeFullVideo(fileUri, jobSpec, priorPortalReports, jobId, platforms) {
  ...
  const [gate0Report, gate1Report] = priorPortalReports || [];
  ...
  // Fix 9: Read contentType and chrome context from jobSpec — Portal 4 must know station brand
  const contentType = jobSpec?.contentType || jobSpec?.order?.contentType || 'unknown';
  const chromeName =
    jobSpec?.designSpec?.chrome?.name || jobSpec?.designSpec?.chrome?.skin || 'AuraFlux';
  ...
  // Show name comes from customerConfig or chrome settings — no hardcoded C0 show names
  let showName = jobSpec?.designSpec?.chrome?.name || chromeName;
  ...
  try {
    ...
    showName = _sCfg?.showName || showName;
  } catch (_e) {}
  ...
  const g3aReport = priorPortalReports?.find
    ? priorPortalReports.find((r) => r?.portal === 'portal3a')
    : priorPortalReports?.gate3a;
  const g3bReport = priorPortalReports?.find
    ? priorPortalReports.find((r) => r?.portal === 'portal3b')
    : priorPortalReports?.gate3b;
  const g2Report = priorPortalReports?.find
    ? priorPortalReports.find((r) => r?.portal === 'portal2')
    : priorPortalReports?.gate2;

  const priorContext =
    [
      gate0Report?.passed ? `Portal 0: confirmed format=${confirmedFormat}, sources verified` : null,
      gate1Report?.passed ? `Portal 1: script passed style QA` : null,
      g2Report?.passed ? `Portal 2: all renders passed (score ${g2Report.score ?? 'n/a'})` : null,
      g3aReport?.passed
        ? `Portal 3a: assembly qualitative check passed (score ${g3aReport.score ?? 'n/a'})`
        : null,
      g3bReport?.passed ? `Portal 3b: commitment verification passed` : null,
      ...(g3aReport?.upstreamContext?.downstreamHeadsUp
        ? [`Portal 3a flagged for Portal 4: ${g3aReport.upstreamContext.downstreamHeadsUp}`]
        : []),
      ...(g3aReport?.sampleFindings
        ? Object.entries(g3aReport.sampleFindings)
            .filter(([, f]) => f?.issues?.length)
            .map(([label, f]) => `Portal 3a ${label} issues: ${f.issues.join('; ')}`)
        : []),
    ]
      .filter(Boolean)
  ...
  const isClipComp =
    ...
    jobSpec?.productionPath === 'long_compile_clips' ||
    jobSpec?.designSpec?.chrome?.layout === 'clip-comp';

  const productionSteps = isClipComp
    ? `1. Portal 0 (ffprobe): confirmed source clip URLs and format=${confirmedFormat}
2. Portal 1: skipped — clip compilation (no avatar script)
3. FFmpeg assembly: concatenated source clips with editorial transitions, captions, and brand logo overlay
4. Portal 3a (Gemini): reviewed 3 sample points (EARLY/MIDDLE/LATE) for freeze, black frames, audio, logo/captions
5. Portal 3b: verified assembly matched committed design spec
6. You are now reviewing the complete clip compilation`
    : `1. Portal 0 (ffprobe): confirmed source videos, detected format=${confirmedFormat}
2. Portal 1 (Claude): reviewed script style — voice, names, accuracy, outro
...
6. Portal 3a (Gemini): reviewed 3 sample points (EARLY/MIDDLE/LATE) for freeze, clips, audio, chrome
7. Portal 3b (Claude): verified assembly matched committed design spec
8. You are now reviewing the complete assembled video`;

  const watchCriteria = isClipComp
    ? `WATCH THE ENTIRE VIDEO and evaluate:
1. Pacing: editorial transitions feel natural? no dead air or abrupt cuts?
2. Audio: consistent volume across clips? no silent gaps, clipping, or music-rights red flags?
3. Visual quality: any frozen frames, black frames, or severe compression artifacts Portal 3a missed?
4. Captions: readable throughout, not cut off, match spoken/on-screen content?
5. Brand logo: visible and correctly placed per design spec (do not flag intentional clip-comp layout)?
6. Content match: clip order and visuals match ordered items: ${itemsOrdered}
7. BROADCAST READY: comfortable publishing this clip compilation tonight?`
    : `WATCH THE ENTIRE VIDEO and evaluate:
...
3. Compounding issues: anything recurring across segments that Portal 3a's 3 samples might have missed?
...
8. BROADCAST READY: comfortable airing this tonight?`;
  ...
  const visualSpecChecks = isClipComp
    ? `VISUAL SPEC CHECKS — clip compilation (no avatar chrome):
- Verify captions are legible and not obscuring critical action
- Verify brand logo placement matches clip-comp layout (typically top-blur-fold or corner logo)
- Verify clip transitions (fade/zoom) render cleanly without flash frames
- Flag frozen or black segments — hard blocker
- Flag missing logo when design spec ordered brand overlay`
...
- sidebarAccurate=false where titles are completely wrong is a BLOCKER — missing/wrong sidebar titles confuse viewers about what they're watching.`;
  ...
  "blockers": ["BLOCKING issues — each must be specific and actionable, leave [] if none"],
  "score": 0-100
}`
    : `{
  "pacingOk": boolean,
  ...
  const prompt = `You are Portal 4 — the broadcast QA director and upload authorization gate for the AuraFlux video production pipeline.
...
- Content accuracy issue: describe what commentary says vs what clip shows → Portal 1 sendback with clip analysis
...
- Content type: ${contentType}${isClipComp ? ' (clip compilation — no avatar)' : ''}
- Show name: ${showName}
- Is short-form: ${isShort}
- Station brand: ${chromeName}${isClipComp ? '' : ' (always on ticker right side — CORRECT branding, never flag it)'}
- Scene count: ${totalScenes || 'unknown'}, source clips: ${clipCount || (jobSpec?.order?.inputs?.items || []).length}
- Items ordered: ${itemsOrdered}
${isClipComp ? '' : `- Flag category label: "${categoryLabel}" — must appear on every avatar scene flag row 1
...
${sourceList}
${gate1Report?.passed ? `- Script summary: ${scriptSummary}` : ''}
...
IMPORTANT: uploadSignal must always be present — set it to true if broadcastReady=true and blockers is empty, false otherwise. Never omit it.`;
  ...
  try {
    ...
    logError('PORTAL4_GEMINI_ERROR', err, { jobId, portal: 4 });
    return null;
  }
}
 ...
 * @param {Object} jobSpec
 * @param {string} assembledPath
 * @param {Object[]} priorPortalReports - [gate0, gate1, gate2, gate3a, gate3b]
...
async function run(jobSpec, assembledPath, priorPortalReports) {
  ...
  // Check thumbnail — generated in parallel during Portal 3a, should be ready by now
  ...
  // CPD-37/CPD-84: apply plan-based feature gate — skip (pass with note) if below min plan
  try {
    const { isFeatureEnabled } = require('../services/feature_gate');
    if (!isFeatureEnabled('portal.full_video_qa', jobSpec?.planTier)) {
      const note = `[portal4] full-video QA not enabled for plan '${jobSpec?.planTier || 'unknown'}' — skipping, uploadSignal deferred to Portal 5`;
      console.log(note);
      return { ...baseOutput, passed: true, broadcastReady: true, uploadSignal: true, notes: [note], completedAt: now() };
    }
  } catch (_e) { /* non-fatal — proceed with QA if gate check fails */ }

  const readiness = canProduce(jobSpec);
  if (!readiness.ready) {
    const reason = `Portal 4 not ready: ${readiness.reasons.join('; ')}`;
    logError('PORTAL4_NOT_READY', new Error(reason), { jobId, portal: 4 });
    return { ...baseOutput, notes: [reason], completedAt: now() };
  }

  // Portal 3b escalation blocks Portal 4 — fixable mismatches do not
  ...
  if (gate3bReport?.outcome === 'mismatch_escalate') {
    const reason =
      'Portal 3b escalation — fundamental spec mismatch, upload signal NOT issued, human review required';
    logError('PORTAL4_GATE3B_ESCALATE', new Error(reason), { jobId, portal: 4 });
    return {
      ...baseOutput,
      notes: [reason],
      completedAt: now(),
  ...
  if (jobSpec?.expectedSynth) {
    const note =
      'expectedSynth/lab — Portal 4 full-video broadcast QA skipped; uploadSignal false until a production assemble';
    return {
      portal: 4,
      jobId,
      passed: true,
  ...
  if (!preflight.passed) {
    logError('PORTAL4_PREFLIGHT_FAIL', new Error(preflight.failReason), { jobId, portal: 4 });
    return {
      ...baseOutput,
      preflightPassed: false,
      notes: [
  ...
  // Large file check — fall back to Portal 3a findings if file exceeds Gemini upload limit
  const fileSize = fs.existsSync(targetPath) ? fs.statSync(targetPath).size : 0;
  if (fileSize > MAX_GEMINI_UPLOAD_BYTES) {
    const g3aFindings = priorPortalReports ? priorPortalReports[3] : null;
    const g3aPassed = g3aFindings?.passed || false;
    const sizeLabel = `${(fileSize / 1024 / 1024).toFixed(0)}MB`;
    logError(
      'PORTAL4_FILE_TOO_LARGE_FALLBACK',
      new Error(
        `${sizeLabel} exceeds ${MAX_GEMINI_UPLOAD_BYTES / 1024 / 1024}MB Gemini limit — using Portal 3a fallback`
      ),
      { jobId, portal: 4, fileSize }
    );
    return {
      portal: 4,
      jobId,
      ...
      notes: [
        `File size ${sizeLabel} exceeds Gemini upload limit — broadcast check based on Portal 3a findings`,
      ],
      upstreamContext: buildUpstreamContext(priorPortalReports),
      completedAt: now(),
    };
  }

  // Resolve suggestive mode — env var global default or per-job qaMode config (CPD-892)
  const { loadCustomerConfig: _loadCfg } = require('../customerConfig');
  const _qaMode = jobSpec?.qaMode || _loadCfg(jobSpec?.customerId)?.qaMode;
  const _suggestive = PORTAL4_SUGGESTIVE_MODE_DEFAULT || _qaMode === 'suggestive';
  ...
  try {
    ...
    logError('PORTAL4_UPLOAD_FAIL', err, { jobId, portal: 4 });
    if (_suggestive) {
      const note = `Video upload to Gemini failed: ${err.message}. Suggestive mode — passing with operator review flag.`;
      console.log(`[portal4] ${jobId}: ${note}`);
      return {
        ...baseOutput,
        passed: true,
        outcome: 'pass_with_notes',
  ...
  if (!analysis) {
    ...
    logError('PORTAL4_NO_ANALYSIS', new Error(note), { jobId, portal: 4 });
    if (_suggestive) {
      console.log(`[portal4] ${jobId}: no Gemini analysis result. Suggestive mode — passing with operator review flag.`);
      return {
        ...baseOutput,
        passed: true,
        outcome: 'pass_with_notes',
        uploadSignal: true,
        notes: [note, 'Suggestive mode — proceeding to portal5 with operator review flag.'],
        completedAt: now(),
      };
    }
    return {
  ...
  if (analysis.flagAccurate === false) {
    ...
    logError('PORTAL4_FLAG_INACCURATE', new Error(flagBlocker), { jobId, portal: 4 });
  }
  ...
  if (analysis.sidebarAccurate === false) {
    ...
    logError('PORTAL4_SIDEBAR_INACCURATE', new Error(sidebarBlocker), { jobId, portal: 4 });
  }
  ...
  let passed = broadcastReady;
  // Prefer Gemini's explicit uploadSignal if present; fall back to derived value from broadcastReady.
  // Portal 5 checks gate4Report.uploadSignal === true (hard check) — must always be defined.
  ...
  let uploadSignal = blockers.length > 0 ? false : geminiSignal;

  if (!passed) {
    const blockerText = blockers.length
      ? blockers.join('; ')
      : 'Broadcast not ready (no blockers listed)';
    logError('PORTAL4_NOT_BROADCAST_READY', new Error(`Blockers: ${blockerText}`), {
      jobId,
      portal: 4,
      score,
      blockers,
  ...
  // In suggestive QA mode, downgrade hard-fails to pass_with_notes so operators can review
  // rather than blocking upload entirely. Structural blockers (freeze, corrupt video) still block.
  let outcome = passed ? 'pass' : 'hard_fail';
  if (!passed && _suggestive) {
    console.log(`[portal4] ${jobId}: suggestive mode — downgrading hard_fail to pass_with_notes (score=${score})`);
    passed = true;
    outcome = 'pass_with_notes';
    uploadSignal = true;
  }
  ...
  return {
    ...
    upstreamContext: {
      ...
      downstreamHeadsUp: notes.length > 0 ? `Portal 4 notes for operator: ${notes.join('; ')}` : null,
    },
    completedAt: now(),
  };
}
