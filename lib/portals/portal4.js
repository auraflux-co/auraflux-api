...
async function preflightForUpload(assembledPath, jobSpec) {
  ...
  const integrity = await probeMp4DecodeIntegrity(assembledPath);
  if (!integrity.ok) {
    result.failReason = `preflightForUpload: decode integrity failed — ${integrity.errors.join('; ')}`;
    logError('PORTAL4_PREFLIGHT_DECODE_FAIL', new Error(result.failReason), {
      jobId: jobSpec?.jobId,
      assembledPath,
    });
    return result;
