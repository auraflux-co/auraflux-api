...
/**
 * CPD-419: Build a YouTube chapter timestamp block from per-segment data.
 * @param {Array<{label:string, durationSeconds:number}>} segs
 * @returns {string|null}
 */
function buildTimestampBlock(segs) {
  ...
  const lines = segs.map((s) => {
    ...
    const sc = elapsed % 60;
    const line = `${m}:${String(sc).padStart(2, '0')} ${s.label || 'Segment'}`;
    elapsed += Math.round(s.durationSeconds || 0);
    return line;
  });
  return lines.join('\n');
}
...
const POLL_INTERVAL_MS = parseInt(process.env.UPLOADPOST_POLL_INTERVAL_MS || '10000', 10);
const POLL_TIMEOUT_MS = parseInt(process.env.UPLOADPOST_POLL_TIMEOUT_MS || '900000', 10); // 15 min — large videos
const UPLOADPOST_STATUS_PATH = process.env.UPLOADPOST_STATUS_PATH || '/uploadposts/status';
const TERMINAL_UPLOADPOST_STATUSES = new Set(['completed', 'done', 'success']);

function extractUploadPostPlatformResult(data, platform) {
  const results = Array.isArray(data?.results) ? data.results : [];
  return results.find((r) => String(r.platform || '').toLowerCase() === platform)
    || (results.length === 1 ? results[0] : null);
}

/** @returns {{ jobId: string|null, url: string|null }|null} */
function parseUploadPostTerminalSuccess(data, platform) {
  const status = String(data?.status || '').toLowerCase();
  if (!TERMINAL_UPLOADPOST_STATUSES.has(status)) return null;
  const platformResult = extractUploadPostPlatformResult(data, platform);
  if (platformResult && platformResult.success === false) return null;
  const jobId = data?.job_id || data?.jobId
    || platformResult?.job_id
    || platformResult?.platform_post_id
    || null;
  return {
    jobId: jobId ? String(jobId) : null,
    url: platformResult?.url || platformResult?.post_url || data?.url || null,
  };
}

async function fetchUploadPostStatusPayload(requestId) {
  const response = await axios.get(`${UPLOADPOST_BASE_URL}${UPLOADPOST_STATUS_PATH}`, {
...
function validatePrePublish(metadata) {
  ...
  if (platforms.includes('tiktok')) {
    ...
    if (metadata.scheduledAt && rules.supportsScheduling) {
      const scheduledDate = new Date(metadata.scheduledAt);
      if (scheduledDate <= new Date()) {
        violations.push('TikTok: scheduledAt must be a future date');
      }
    }
  }
...
// ─── Direct publish (CPD-86) ──────────────────────────────────────────────────
// Preferred path when customer has connected their platform accounts via OAuth.
// Falls back to Upload-Post when direct tokens are not available.
// CPD-1020: require the adapter core — ../publish resolves to lib/publish.js (legacy copy gen), not index.js
const { canPublishDirect, publishDirect } = require('../publish/index');
...
async function submitUpload(platform, driveUrl, metadata, corrected, pipelineJobId = null, profile = null) {
  ...
  const privacyStatus = isFutureSchedule
    ? 'private'
    : (corrected.privacyStatus || metadata.privacyStatus || 'private');
  ...
  if (platform === 'tiktok') {
    ...
    form.append('privacy_level', isFutureSchedule ? 'SELF_ONLY' : 'PUBLIC_TO_EVERYONE');
  ...
  if (isFutureSchedule && platform !== 'youtube') {
    form.append('scheduled_date', new Date(scheduledAt).toISOString());
  }
...
async function pollForJobId(requestId, platform, pipelineJobId) {
  ...
  while (Date.now() < deadline) {
    ...
    try {
      ...
      const terminal = parseUploadPostTerminalSuccess(response.data, platform);
      if (terminal) {
        try {
          pipelineBus.emit('publish:platform_done', {
            ...
            request_id: String(requestId),
            outcome: 'completed',
            platformJobId: terminal.jobId,
          });
        } catch (_e2) {
          /* non-fatal */
        }
        return { jobId: terminal.jobId || requestId };
      }
      if (status === 'failed') {
  ...
  try {
    const finalData = await fetchUploadPostStatusPayload(requestId);
    const terminal = parseUploadPostTerminalSuccess(finalData, platform);
    if (terminal) {
      console.log(`[portal5] ${pipelineJobId}: Upload-Post ${platform} completed after poll window (${requestId})`);
      return { jobId: terminal.jobId || requestId };
    }
  } catch (finalErr) {
    logError('PORTAL5_POLL_FINAL_CHECK', finalErr, { platform, requestId, jobId: pipelineJobId });
  }
...
async function uploadWithRetries(platform, driveUrl, metadata, corrected, jobId, profile = null) {
  ...
  for (let attempt = 1; attempt <= MAX_RETRIES; attempt++) {
    try {
      ...
      return { jobId: platformJobId, requestId: String(requestId), failed: false, failReason: null };
    } catch (err) {
      lastErr = err;
      logError('PORTAL5_UPLOAD_ATTEMPT_FAIL', err, { jobId, portal: 5, platform, attempt });
      if (/Poll timeout/i.test(err.message || '')) break;
...
/**
 * directPublishWithRetries — CPD-86
 * Use the customer's connected OAuth tokens to publish directly to the platform.
 * Retries up to MAX_RETRIES times with exponential backoff.
 */
async function directPublishWithRetries(platform, videoUrl, metadata, jobSpec, jobId) {
  let lastErr;
  ...
  for (let attempt = 1; attempt <= MAX_RETRIES; attempt++) {
    try {
      try {
        pipelineBus.emit('pipeline:retry_attempt', {
          jobId, portal: 5,
          stage: `direct_publish_${platform}`,
          attempt, maxAttempts: MAX_RETRIES,
        });
      } catch (_e) { /* non-fatal */ }

      const directMeta = { ...metadata };
      if (platform === 'youtube' && metadata.scheduledAt) {
        directMeta.publishAt = new Date(metadata.scheduledAt).toISOString();
        directMeta.privacyStatus = 'private';
      }

      const r = await publishDirect({ platform, videoPath: videoUrl, metadata: directMeta, jobSpec });
      return { jobId: r.platformJobId || r.id || null, failed: false, failReason: null, url: r.url };
    } catch (err) {
      lastErr = err;
      logError('PORTAL5_DIRECT_PUBLISH_FAIL', err, { jobId, portal: 5, platform, attempt });
      if (attempt < MAX_RETRIES) {
        const delay = Math.min(RETRY_DELAY_MS * Math.pow(2, attempt - 1), RETRY_DELAY_MAX_MS);
  ...
  logError('PORTAL5_DIRECT_PUBLISH_EXHAUSTED', lastErr, { jobId, portal: 5, platform, attempts: MAX_RETRIES });
  return { jobId: null, failed: true, failReason: lastErr?.message || 'direct publish failed after retries' };
}
...
async function run(jobSpec, gate4Report) {
  ...
  validatePortalInput('5', jobSpec);

  // When called via _adaptLegacyPortal (only jobSpec passed), read gate4 report from portalReports
  if (!gate4Report && jobSpec?.portalReports?.portal4) {
    gate4Report = jobSpec.portalReports.portal4;
  }
  ...
  // Legacy clip jobs skipped portal4 — R2 URL was treated as publish-safe (CPD-869 gap).
  // CPD-1046: when portal4 is active, require gate4.uploadSignal; fallback only if portal4 inactive.
  const portal4Inactive = jobSpec?.portals?.portal4?.active === false;
  const clipsVideoReady = portal4Inactive &&
    !gate4Report &&
    !!(jobSpec?.assembledVideoUrl || jobSpec?.state?.savedOutputs?.r2VideoUrl);
  if (!clipsVideoReady && (!gate4Report || gate4Report.uploadSignal !== true)) {
  ...
  if (!readiness.ready) {
    ...
    // If the only blocker is "no platforms specified", this is a valid operator choice —
    // the job was produced for review/download without a publish destination.
    // Treat as a graceful skip so the job reaches complete status without uploading.
    const onlyNoPlatforms =
      readiness.reasons.length > 0 &&
      readiness.reasons.every(
        (r) =>
          r.includes('No platforms specified') ||
          r.includes('UPLOADPOST_API_KEY') ||
          r.includes('UPLOADPOST_PROFILE') ||
          r.includes('driveUrl missing')
      );
    if (onlyNoPlatforms) {
      console.log(`[portal5] ${jobId}: no publish platforms configured — skipping upload (pass_with_notes)`);
      return {
        portal: 5,
        jobId,
        passed: true,
        outcome: 'skip_no_platforms',
        platforms: {},
        prePublishValidation: { passed: true, violations: [] },
        notes: ['No publish platforms configured — job ready for review/download'],
        completedAt: now(),
      };
    }
  ...
  // CPD-419: Append YouTube chapter timestamps if per-segment data was stored by assembly
  const _segDurs = jobSpec.state?.savedOutputs?.segmentDurations || null;
  const _baseDesc = ytMeta.description || jobSpec.order?.publish?.description || '';
  const _chapterBlock = _segDurs && platforms.includes('youtube') ? buildTimestampBlock(_segDurs) : null;
  const _description = _chapterBlock ? `${_baseDesc}\n\nCHAPTERS:\n${_chapterBlock}`.trim() : _baseDesc;
  ...
  const metadata = {
    ...
    scheduledAt:
      jobSpec.scheduledPublishAt ||
      jobSpec.deliverySpec?.scheduledAt ||
      jobSpec.order?.publish?.scheduledAt ||
      null,
    caption:
      jobSpec.order?.publish?.tiktokCaption ||
      jobSpec.order?.publish?.instagramCaption ||
  ...
  for (const platform of platforms) {
    ...
    // CPD-86: prefer direct publish (customer OAuth) over Upload-Post proxy
    const directCheck = await canPublishDirect(platform, jobSpec).catch(() => ({ canDirect: false, reason: 'canPublishDirect threw' }));
    let result;
    if (directCheck.canDirect) {
      result = await directPublishWithRetries(platform, driveUrl, metadata, jobSpec, jobId);
    } else {
      result = await uploadWithRetries(platform, driveUrl, metadata, corrected, jobId, uploadPostProfile);
    }

    platformResults[platform] = { attempted: true, directPublish: !!directCheck.canDirect, ...result };
  }
  ...
  const nativeScheduled = !!(metadata.scheduledAt && new Date(metadata.scheduledAt).getTime() > Date.now());
...
async function retryPlatformUpload(jobSpec, platform) {
  ...
  // CPD-419: Append YouTube chapter timestamps on retry too
  const _retrySegDurs = jobSpec.state?.savedOutputs?.segmentDurations || null;
  const _retryBaseDesc = ytMeta.description || jobSpec.order?.publish?.description || '';
  const _retryChapters = _retrySegDurs && platform === 'youtube' ? buildTimestampBlock(_retrySegDurs) : null;
  const metadata = {
    title: ytMeta.title || ytMeta.titles?.[0] || jobSpec.order?.publish?.title || jobSpec.title || '',
    description: _retryChapters ? `${_retryBaseDesc}\n\nCHAPTERS:\n${_retryChapters}`.trim() : _retryBaseDesc,
    tags: ytMeta.tags || ytMeta.hashtags || jobSpec.order?.publish?.tags || [],
    categoryId: jobSpec.deliverySpec?.categoryId || jobSpec.order?.publish?.categoryId || '24',
  ...
  // CPD-86: prefer direct publish on retry too
  const directCheck = await canPublishDirect(platform, jobSpec).catch(() => ({ canDirect: false }));
  let result;
  if (directCheck.canDirect) {
    result = await directPublishWithRetries(platform, driveUrl, metadata, jobSpec, jobId);
  } else {
    result = await uploadWithRetries(platform, driveUrl, metadata, validation.corrected, jobId, resolveUploadPostProfile(jobSpec));
  }
