...
const { validateYoutubeTagsSeo } = require('../gates/metadata_qa');
...
async function submitUpload(platform, driveUrl, metadata, corrected, pipelineJobId = null, profile = null) {
  ...
  const { resolveYouTubePrivacy, resolveTikTokPrivacy } = require('../publish_privacy');
  const jobSpecStub = {
    contentType: metadata.contentType,
    clipsOnly: metadata.clipsOnly,
    deliverySpec: { visibility: metadata.privacyStatus },
    order: { publish: { privacyStatus: metadata.privacyStatus } },
  };
  const privacyStatus = isFutureSchedule
    ? 'private'
    : resolveYouTubePrivacy(jobSpecStub, corrected);
...
async function directPublishWithRetries(platform, videoUrl, metadata, jobSpec, jobId) {
  ...
  for (let attempt = 1; attempt <= MAX_RETRIES; attempt++) {
    try {
      ...
      const r = await publishDirect({ platform, videoPath: videoUrl, metadata, jobSpec });
