...
async function run(jobSpec, gate4Report) {
  ...
  if (thumbState && thumbState.status === 'pending') {
    const msg = `Gate 5 HOLD: thumbnail approval is pending for job ${jobId} — customer must approve via POST /jobs/${jobId}/thumbnail/approve`;
  ...
  if (!validation.passed) {
    logError(
      'GATE5_VALIDATION_FAIL',
      new Error(`Violations: ${validation.violations.join('; ')}`),
      { jobId, gate: 5, violations: validation.violations }
    );
    try {
      pipelineBus.emit('publish:failed_validation', {
        jobId,
  ...
  for (const platform of platforms) {
    if (!['youtube', 'tiktok', 'instagram'].includes(platform)) {
      logError('GATE5_UNKNOWN_PLATFORM', new Error(`Unknown platform: ${platform}`), {
        jobId,
        gate: 5,
        platform,
      });
  ...
  const anySuccess = Object.values(platformResults).some(
    (r) => r.attempted && !r.failed && r.jobId
  );
  const allFailed =
    platforms.length > 0 &&
  ...
  if (allFailed) {
    logError('GATE5_ALL_PLATFORMS_FAILED', new Error('All platform uploads failed'), {
      jobId,
      gate: 5,
      platformResults,
    });
...
function prepare(jobSpec) {
  ...
  try {
    ...
    console.log(
      `[gate5] Ready for job ${jobId} — will deliver to ${platforms.join(', ') || '(none set yet)'} via ${profile}`
    );
  } catch (e) {
...
async function retryPlatformUpload(jobSpec, platform) {
  ...
  const result = await uploadWithRetries(platform, driveUrl, metadata, validation.corrected, jobId);
  return { ok: !result.failed, platformJobId: result.jobId, failReason: result.failReason };
}

module.exports = { canProduce, commit, run, prepare, validatePrePublish, retryPlatformUpload };
