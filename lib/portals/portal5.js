...
async function run(jobSpec, gate4Report) {
  ...
  console.log(`[portal5:diag] ${jobId}: run() entered, gate4Report param=${!!gate4Report}, portalReports.portal4=${!!jobSpec?.portalReports?.portal4}`);
  ...
  console.log(`[portal5:diag] ${jobId}: gate4Report resolved, uploadSignal=${gate4Report?.uploadSignal}, assembledPath=${!!jobSpec?.assembledPath}, r2VideoUrl=${!!(jobSpec?.r2VideoUrl || jobSpec?.state?.savedOutputs?.r2VideoUrl)}`);
  ...
  console.log(`[portal5:diag] ${jobId}: past thumbnail check, checking uploadSignal...`);
  ...
  if (!readiness.ready) {
    ...
    if (onlyNoPlatforms) {
      console.log(`[portal5:diag] ${jobId}: onlyNoPlatforms=true, reasons=${JSON.stringify(readiness.reasons)}`);
