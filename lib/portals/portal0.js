...
async function probeWithYtdlp(url) {
  ...
  return new Promise((resolve) => {
    execFile(
      ytdlp,
      ['--dump-json', '--no-playlist', '--no-warnings', probeUrl],
...
async function run(jobSpec) {
  ...
  for (const item of items) {
    ...
    // Multi-clip throttle: 2s pre-call delay + 5s retry on first failure prevents GQL
    // rate-limiting where rapid sequential calls return null for clips 2+.
    ...
    if (!ytdlpProbe) {
      // First attempt failed — check if this is a Twitch URL and retry with longer backoff
      const isTwitchUrl =
        /clips\.twitch\.tv/.test(effectiveUrl) ||
        /twitch\.tv.*\/clip\//.test(effectiveUrl) ||
        /cloudfront\.net\/nauth\//.test(effectiveUrl);
      if (isTwitchUrl) {
        console.log(`[portal0] ${jobId}: yt-dlp probe failed for item ${itemId} — retrying after 6s backoff`);
        await new Promise((r) => setTimeout(r, 6000));
        ytdlpProbe = await probeWithYtdlp(effectiveUrl);
        if (ytdlpProbe) {
          console.log(`[portal0] ${jobId}: item ${itemId} confirmed via yt-dlp (retry) — ${ytdlpProbe.duration}s`);
        }
      }
    }
    if (ytdlpProbe) {
