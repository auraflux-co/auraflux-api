...
async function run(jobSpec) {
  ...
  for (const item of items) {
    ...
    // Twitch VOD page URLs (twitch.tv/videos/NNNNN) — same bypass as clips.
    // yt-dlp probe fails on the page URL; assembly_service handles yt-dlp download at job time.
    const isTwitchVodUrl = /twitch\.tv\/videos\/\d+/.test(effectiveUrl);
    ...
    // Library. assembly_service downloads via yt-dlp android/mweb client at job time.
    ...
    if (isPageUrl(effectiveUrl) && !isTwitchClipUrl && !isTwitchVodUrl && !isKickClipUrl) {
    ...
    // Step 1b: Twitch clip/VOD page fallback — accept without ffprobe.
    // yt-dlp probe failed but checkUrlReachable confirmed the page exists.
    // ffprobe cannot probe HTML pages; assembly_service handles yt-dlp download at job time.
    if (isTwitchClipUrl || isTwitchVodUrl) {
      confirmedSources.push({
        itemId,
        url:      effectiveUrl,
        duration: null,
      ...
      detectedFormats.push('clip');
      console.log(`[portal0] ${jobId}: item ${itemId} confirmed via reachability (${isTwitchVodUrl ? 'Twitch VOD' : 'Twitch clip'}) — ${effectiveUrl.slice(0, 80)}`);
      continue;
    }
