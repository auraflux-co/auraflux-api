 ...
 * to their direct CDN MP4 URL via yt-dlp --get-url. Called by Portal 0 before the
 * isPageUrl rejection so clips from the Source Library don't block the pipeline.
 * Returns the resolved CDN URL string, or null if resolution fails.
 */
async function resolveTwitchClipUrl(url) {
  const isTwitchClip =
  ...
  return new Promise((resolve) => {
    execFile(
      ytdlp,
      ['--get-url', '--format', 'best[height<=720]/best', '--no-playlist', '--no-warnings', url],
      { timeout: 30000, maxBuffer: 2 * 1024 * 1024 },
      (err, stdout) => {
        if (err) return resolve(null);
        const resolved = (stdout || '').split('\n').find((l) => l.trim().startsWith('http'));
        resolve(resolved ? resolved.trim() : null);
      },
    );
  });
}
...
async function run(jobSpec) {
  ...
  for (const item of items) {
    ...
    // Step 0: Reject web page URLs — these must be resolved to media URLs before Portal 0.
    // CPD-292: Twitch clip URLs (clips.twitch.tv / twitch.tv/user/clip/) are page URLs
    // that yt-dlp can resolve to CDN MP4. Attempt resolution before failing.
    if (isPageUrl(url)) {
      const resolved = await resolveTwitchClipUrl(url);
      if (resolved) {
        console.log(`[portal0] ${jobId}: resolved Twitch clip URL via yt-dlp → ${resolved.slice(0, 80)}`);
        url = resolved; // continue with direct CDN URL
      } else {
