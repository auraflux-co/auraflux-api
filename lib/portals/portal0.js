...
async function run(jobSpec) {
  ...
  for (const item of items) {
    ...
    // Twitch clip page URLs (clips.twitch.tv, twitch.tv/.../clip/) are allowed to fall through
    // to checkUrlReachable — yt-dlp may have been rate-limited but assembly_service will retry.
    if (isPageUrl(effectiveUrl) && !isTwitchClipUrl) {
    ...
    // yt-dlp was rate-limited (above) but checkUrlReachable confirmed the clip page exists.
    // ffprobe cannot probe HTML, so we skip Step 2 and let assembly_service handle the
    // actual yt-dlp download with its own retry logic at job execution time.
    if (isTwitchClipUrl) {
      confirmedSources.push({
        itemId,
        url:      effectiveUrl,
