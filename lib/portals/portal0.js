 ...
 *
 * Handles both watch-page URLs (twitch.tv/:channel/clip/:slug) and CDN URLs.
 * Returns { duration, width, height, format, directUrl } or null on failure.
 */
async function probeWithYtdlp(url) {
  // Only Twitch PAGE URLs need yt-dlp — direct CDN URLs (cloudfront.net/nauth,
  // clips-media-assets2.twitch.tv with sig/token) are probed with ffprobe directly.
  const isTwitch =
