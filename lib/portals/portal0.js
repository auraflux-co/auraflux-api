...
async function probeSource(url) {
  ...
  // Probe the downloaded temp file; if that fails (e.g. moov atom not in first 8MB)
  // fall back to probing the original URL directly via streaming.
  return new Promise((resolve, reject) => {
    ...
    execFile(ffprobePath(), args, { timeout: 15000 }, (err, stdout) => {
      try { fs.unlinkSync(tmpFile); } catch (_) {}
      if (err) {
        // Temp file probe failed (likely moov atom not in first 8MB = non-faststart mp4).
        // Fall back to probing the original URL directly.
        const urlArgs = [
          '-v','quiet',
          '-protocol_whitelist', 'file,http,https,tcp,tls,crypto',
          '-select_streams','v:0',
