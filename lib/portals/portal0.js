...
async function probeSource(url) {
  ...
  // Probe the downloaded temp file
  return new Promise((resolve, reject) => {
...
function canProduce(jobSpec) {
  ...
  // Items must be present, UNLESS the job is topic-only (no source URLs supplied,
  // script generated from topic+tone directly). In that case portal0 has nothing
  // to verify — the script_gen_service already ran and produced filledScript.
  const items = jobSpec.order?.inputs?.items;
  const isTopicOnly = (!items || !Array.isArray(items) || items.length === 0) &&
    !(jobSpec.sourceConfig?.urls?.length) &&
    !(jobSpec.order?.inputs?.url);
  if (!isTopicOnly && (!items || !Array.isArray(items) || items.length === 0)) {
