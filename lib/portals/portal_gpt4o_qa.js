...
passed=true means the video meets production standards for delivery. passed=false means at least one critical issue must be fixed before the video is shown to the customer.`;
}
...
async function runWorker({ jobSpec, workerAttempt = 1 }) {
  ...
  if (!jobSpec?.addOns?.gpt4o_qa_ext?.ordered) {
  ...
  try {
    ...
    console.log(`[gpt4o_qa:${jobId}] GPT-4o score=${review.overallScore} passed=${review.passed} fixable=${review.fixableIssues?.length || 0}`);
