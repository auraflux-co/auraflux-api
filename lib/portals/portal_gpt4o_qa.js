...
/**
 * CPD-949: Upload extracted QA frame JPEGs to R2.
 * Returns array of { timestamp, url } — url is null for frames that failed to upload.
 * Non-fatal: callers should continue if this throws.
 * Only runs when R2_ACCESS_KEY_ID is present (skips on C0 localhost).
 *
 * @param {Array<{timestamp: number, path: string}>} frames
 * @param {string} jobId
 * @returns {Promise<Array<{timestamp: number, url: string|null}>>}
 */
async function _uploadFramesToR2(frames, jobId) {
  if (!process.env.R2_ACCESS_KEY_ID) return frames.map((f) => ({ timestamp: f.timestamp, url: null }));
  const { uploadToR2 } = require('../storage');
  const results = [];
  for (let i = 0; i < frames.length; i++) {
    const f = frames[i];
    try {
      const url = await uploadToR2(f.path, `frame_${i}.jpg`, {
        folder:      `jobs/${jobId}/qa`,
        contentType: 'image/jpeg',
        bucket:      process.env.R2_VIDEO_BUCKET,
      });
      results.push({ timestamp: f.timestamp, url: url || null });
    } catch (_e) {
      console.warn(`[gpt4o_qa:${jobId}] R2 upload failed for frame ${i}: ${_e.message}`);
      results.push({ timestamp: f.timestamp, url: null });
    }
  }
  return results;
}
...
/**
 * Attach the nearest R2 screenshot URL to each featureCompliance or fixableIssue entry.
 * Matches by closest timestamp within 2 seconds, falls back to the first frame.
 *
 * @param {Array} entries  — featureCompliance[] or fixableIssues[]
 * @param {string} tsField — field name that contains the timestamp string ("MM:SS" or null)
 * @param {Array<{timestamp: number, url: string|null}>} frameUrls
 */
function _attachScreenshotUrls(entries, tsField, frameUrls) {
  if (!Array.isArray(entries) || !frameUrls?.length) return;
  const hasUrls = frameUrls.some((f) => f.url);
  if (!hasUrls) return;

  for (const entry of entries) {
    const tsStr = entry[tsField];
    let targetSec = null;
    ...
    if (targetSec != null) {
      ...
      for (const f of frameUrls) {
        if (!f.url) continue;
        const diff = Math.abs(f.timestamp - targetSec);
        if (diff < minDiff) { minDiff = diff; best = f.url; }
      }
    }
    // Fallback: first available frame
    if (!best) best = (frameUrls.find((f) => f.url) || {}).url || null;
    entry.screenshotUrl = best;
  }
}
...
async function runWorker({ jobSpec, workerAttempt = 1 }) {
  ...
  try {
    ...
    // ── Step 1b: CPD-949 — Upload frames to R2 ────────────────────────────
    let frameUrls = [];
    try {
      frameUrls = await _uploadFramesToR2(frames, jobId);
      const uploaded = frameUrls.filter((f) => f.url).length;
      if (uploaded > 0) {
        console.log(`[gpt4o_qa:${jobId}] uploaded ${uploaded}/${frames.length} QA frames to R2`);
        if (!jobSpec.state) jobSpec.state = {};
        jobSpec.state.qaScreenshots = frameUrls;
      }
    } catch (_e) {
      console.warn(`[gpt4o_qa:${jobId}] R2 frame upload failed (non-fatal): ${_e.message}`);
    }
    ...
    // CPD-949: Attach R2 screenshot URLs to compliance entries and fixable issues
    _attachScreenshotUrls(review.featureCompliance, 'timestamp', frameUrls);
    _attachScreenshotUrls(review.fixableIssues,     'timestamp', frameUrls);
    ...
    const _hasMissingFeatures = missingFeatures.length > 0;
    return {
      passed:              !_hasMissingFeatures,
      outcome:             _hasMissingFeatures ? 'operator_review' : (review.passed !== false ? 'pass' : 'pass_with_notes'),
      score:               review.overallScore,
      review,
...
function isPass(result) {
  // If GPT-4o confirmed that required features are missing from the output,
  // route to operator review — the video cannot be delivered without those features.
  // A low score alone does not block (subjective quality); missing features do.
  if (result && result.missingFeatureCount > 0) return false;
  return true;
}
