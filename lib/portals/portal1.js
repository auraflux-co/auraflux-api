...
function getThresholds(jobSpec) {
  ...
  const t = getGateThresholds(customerId, templateId, 'gate1', {
...
async function run(jobSpec, filledScript, gate0Report) {
  ...
  if (unfilledSlots.length > 0) {
    ...
    logError('GATE1_UNFILLED_SLOTS', new Error(reason), {
      jobId,
      gate: 1,
  ...
  if (isSports && allConfirmedPlayers && allConfirmedPlayers.size > 0 && script) {
    ...
    if (deterministicRosterViolations.length > 0) {
      ...
      logError(
        'GATE1_ROSTER_VIOLATION_DETERMINISTIC',
  ...
  if (cTypeCheck.includes('twitch') && requiredDisplayNames.length > 0 && script) {
    for (const { wrong, correct } of requiredDisplayNames) {
      ...
      if (handleOccurrences >= 2) {
        ...
        logError(
          'GATE1_TWITCH_HANDLE_VIOLATION',
          new Error(`Handle "${wrong}" instead of display name "${correct}"`),
          { jobId, gate: 1, wrong, correct, occurrences: handleOccurrences }
        );
      }
    }
  }
  ...
  if (cTypeCheck.includes('news') && confirmedArticleTitles.length > 0 && script) {
    ...
    for (const org of knownNewsOrgs) {
      if (
        ...
        logError('GATE1_NEWS_SOURCE_VIOLATION', new Error(`Wrong source org "${org}"`), {
          jobId,
          gate: 1,
          wrongOrg: org,
    ...
    if (uncoveredArticles > 0 && confirmedArticleTitles.length >= 2) {
      ...
      if (uncoveredArticles > Math.floor(confirmedArticleTitles.length / 2)) {
        ...
        logError(
          'GATE1_NEWS_TOPIC_DRIFT',
          new Error(`${uncoveredArticles} articles have zero keyword coverage in script`),
          { jobId, gate: 1, uncoveredArticles, totalArticles: confirmedArticleTitles.length }
        );
      }
    }
  }
  ...
  if (score > 0 && process.env.GEMINI_API_KEY) {
    ...
    try {
      ...
      if (!parsed) {
        logError(
          'GATE1_JSON_PARSE_FAIL',
          new Error('Gate 1 Gemini response was not valid JSON after repair attempts'),
          {
            jobId,
            gate: 1,
        ...
        try {
          ...
          recordWhyLedger({
            ...
            reasons: [
              'Gate 1 (Gemini) returned non-JSON; neutral JSON fallback applied before scoring',
            ],
      ...
      if (parsed.lockedIntroCorrect === false) {
        ...
        deductions.push({
          points: -15,
          reason: 'Locked intro text incorrect or missing (Gate 1 model verification)',
        });
      }
      ...
      if (parsed.lockedOutroCorrect === false) {
        ...
        deductions.push({
          points: -15,
          reason: 'Locked outro text incorrect or missing (Gate 1 model verification)',
        });
      }
      ...
      if (
        ...
        if (parsed.softAccuracyIssues.length > 0) {
          logError(
            'GATE1_SPORTS_ACCURACY_SOFT',
            new Error(`${parsed.softAccuracyIssues.length} soft accuracy issues`),
            { jobId, gate: 1, issues: parsed.softAccuracyIssues }
          );
        }
      }
      ...
      if (
        ...
        logError(
          'GATE1_ROSTER_VIOLATION',
          new Error(`${parsed.rosterViolations.length} roster violations`),
          { jobId, gate: 1, violations: parsed.rosterViolations }
        );
      }
      ...
      if (
        ...
        logError(
          'GATE1_TWITCH_ENTITY_VIOLATION',
          new Error(`${parsed.entityNameErrors.length} entity name violations`),
          { jobId, gate: 1, violations: parsed.entityNameErrors }
        );
      }
      ...
      if (
        ...
        logError(
          'GATE1_NEWS_SOURCE_VIOLATION_GEMINI',
          new Error(`${parsed.sourceViolations.length} source violations`),
          { jobId, gate: 1, violations: parsed.sourceViolations }
        );
      }
    } catch (err) {
  ...
  if (score >= PASS_THRESHOLD) {
    ...
    logError('GATE1_SENDBACK', new Error(`Score ${score} — sendback`), {
      jobId,
      gate: 1,
      score,
      deductions,
    ...
    if (isSportsContent && !fabricationFail && !hasRosterViolation) {
      ...
      logError('GATE1_SPORTS_SENDBACK', new Error(`Score ${score} — sports sendback`), {
        jobId,
        gate: 1,
        score,
        deductions,
      ...
      logError('GATE1_HARD_FAIL', new Error(`Score ${score} — hard fail`), {
        jobId,
        gate: 1,
        score,
        deductions,
