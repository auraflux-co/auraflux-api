...
async function run(jobSpec, filledScript, gate0Report) {
  ...
  // Check prohibited structural language patterns (-10 each occurrence).
  // Skip for C1 API customers whose voice config explicitly allows all language
  // (prohibitedWords:[]) — broadcast CTA rules (subscribe, follow-us) don't apply to social/clips content.
  const hypeWordsForCheck = getHypeWords(jobSpec);
  const skipCTAPatterns = Array.isArray(hypeWordsForCheck) && hypeWordsForCheck.length === 0;
  const prohibitedFound = skipCTAPatterns ? [] : findProhibitedLanguage(script);
