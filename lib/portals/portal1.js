 ...
 *
 * 'strict'    — all style rules deduct from the score (C0 broadcast news default).
 ...
 *
 * Precedence:
 *   1. jobSpec.writingQaMode (future API param — 'strict' | 'suggestive')
 *   2. customerConfig.qaMode (per-customer JSON setting)
 *   3. C0 → 'strict', all others → 'suggestive'
 */
function getWritingQaMode(jobSpec) {
  ...
  // C0 is always strict (broadcast news house rules); all other customers default to suggestive
  // until they configure writing preferences through the API, dashboard, or copilot.
  return customerId === 'c0' ? 'strict' : 'suggestive';
}
