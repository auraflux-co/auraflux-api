...
// Platform defaults (used when customerConfig has no explicit qaThresholds.gate1).
// c1_default.json sets 60/40; these are the safety-net if no config resolves.
...
// Platform-level default prohibited words: none.
// Each customer defines their own via customerConfig.voice.prohibitedWords.
// 'strict' qaMode customers who set prohibitedWords get them enforced;
// 'suggestive' customers (the default) receive them as advisory notes only.
const DEFAULT_HYPE_WORDS = [];

// Structural prohibited patterns — checked only when qaMode === 'strict'.
// Customers on 'suggestive' mode receive these as writingSuggestions, not deductions.
...
// Platform-level default required outros: none.
// Customers configure their sign-off line via customerConfig.voice.outroLine.
// No outro requirement is enforced unless the customer explicitly sets one.
...
function getHypeWords(jobSpec) {
  ...
  // prohibitedWords=[] means customer explicitly allows all language (e.g. c1_default clips/social).
  // prohibitedWords=undefined means fall back to broadcast news defaults.
  if (Array.isArray(voice.prohibitedWords)) return voice.prohibitedWords;
  return DEFAULT_HYPE_WORDS;
}

/**
 * Determine whether Portal 1 style checks are hard gates ('strict') or advisory-only ('suggestive').
 *
 * 'strict'    — all style rules deduct from the score. Opt-in only via jobSpec or customerConfig.
 * 'suggestive'— factual/fabrication checks still deduct; style checks generate
 *               writingSuggestions in the report but do NOT lower the score.
 *
 * Precedence:
 *   1. jobSpec.writingQaMode  (API param — 'strict' | 'suggestive')
 *   2. customerConfig.qaMode  (per-customer JSON setting)
 *   3. Default: 'suggestive'  — no style blocking until customer configures writing preferences
 *
 * Note: 'strict' mode is an opt-in. C0 (ClipzWorld News) only runs on localhost, not this platform.
 */
function getWritingQaMode(jobSpec) {
  if (jobSpec?.writingQaMode === 'strict' || jobSpec?.writingQaMode === 'suggestive') {
    return jobSpec.writingQaMode;
  }
  const customerId = jobSpec?.customerId;
  const templateId =
    jobSpec?.order?.templateId ||
  ...
  if (customerId) {
    try {
      ...
      const cfg = loadCustomerConfig(customerId, templateId);
      if (cfg?.qaMode === 'strict' || cfg?.qaMode === 'suggestive') return cfg.qaMode;
    } catch (_) { /* no config — fall through */ }
  }
  return 'suggestive';
}
...
function getRequiredOutro(jobSpec) {
  ...
  const voice = getVoiceConfig(customerId, templateId);
  // outroLine === '' means "no locked outro for this customer" (e.g. c1_default).
  // outroLine === undefined means customer config doesn't define one → use default.
  if (voice.outroLine !== undefined) return voice.outroLine || null;
...
function tryParseGate1QaJson(rawText) {
  ...
  // Truncated Gemini output — salvage booleans/arrays from partial JSON
  const partial = salvageTruncatedGate1QaJson(s);
  if (partial) return partial;
  return null;
}

/** Best-effort parse when Gemini truncates mid-response. */
function salvageTruncatedGate1QaJson(s) {
  if (!s || typeof s !== 'string' || !s.includes('{')) return null;
  const pickBool = (key) => {
    const m = s.match(new RegExp(`"${key}"\\s*:\\s*(true|false)`, 'i'));
  ...
  const pickStringArray = (key) => {
    ...
    if (!m) return [];
    try {
      const wrapped = JSON.parse(`[${m[1]}${m[1].trim().endsWith('"') ? '' : '"..."'}]`.replace(/,\s*$/, ''));
      return Array.isArray(wrapped) ? wrapped.filter(Boolean) : [];
    } catch (_e) {
      return m[1].split(',').map((x) => x.replace(/^[\s"]+|[\s"]+$/g, '')).filter(Boolean);
    }
  };
  if (!/lockedIntroCorrect|sceneStructureCorrect|fabricationFound/i.test(s)) return null;
  return {
    lockedIntroCorrect: pickBool('lockedIntroCorrect'),
    lockedOutroCorrect: pickBool('lockedOutroCorrect'),
    prohibitedWordsFound: pickStringArray('prohibitedWordsFound'),
    fabricationFound: pickBool('fabricationFound') === true,
    examples: pickStringArray('examples'),
    sceneStructureCorrect: pickBool('sceneStructureCorrect') !== false,
    itemAccuracyIssues: pickStringArray('itemAccuracyIssues'),
    softAccuracyIssues: pickStringArray('softAccuracyIssues'),
    clipMismatches: [],
  };
}
...
function canProduce(jobSpec) {
  ...
  // lockedOutro and sceneHeaders are required only for HeyGen avatar-render jobs.
  // speakerName defaults to 'Bobby G' for ALL jobs (createJobSpec fallback) — it is
  // NOT a reliable signal for avatar workflows. Only addOns.heygen.active is definitive.
  // C1 API url_list jobs never have HeyGen ordered; their script is validated by content
  // checks below (format, placeholders, quality) — not by voice/scene structure metadata.
  const hasAvatarWorkflow = !!jobSpec?.addOns?.heygen?.active;
...
/**
 * Strip <<<CWN_NBA_TS_LOCK>>> ... <<<CWN_NBA_TS_UNLOCK>>> blocks from a string.
 * These blocks contain instruction text (not filled content) so must not be
 * scanned for unfilled [DIALOGUE] placeholders.
 */
function stripLockedBlocks(text) {
  return text.replace(/<<<CWN_NBA_TS_LOCK>>>[\s\S]*?<<<CWN_NBA_TS_UNLOCK>>>/g, '');
}
...
function findUnfilledSlots(script) {
  ...
  for (const { header, body } of sections) {
    // Strip locked instruction blocks before checking — they may contain
    // "[DIALOGUE]" in guidance text which is not an unfilled slot.
    if (stripLockedBlocks(body).includes('[DIALOGUE]')) {
      unfilledHeaders.push(header);
    }
  }
  // Also catch raw [DIALOGUE] outside headers (strip locked blocks first)
  const headerNames = new Set(unfilledHeaders);
  if (stripLockedBlocks(script).includes('[DIALOGUE]') && headerNames.size === 0) {
...
function validateStructureAgainstJobSpec(script, jobSpec) {
  ...
  // Only enforce clip-slot count when a pre-generated scaffold was committed.
  // C1 API jobs (url_list source, no preItems) skip scaffold generation so
  // sceneHeaders is empty — their scripts contain inline narration, not slot markers.
  const isScaffolded = expectedHeadersList.length > 0;
  if (expectedClipCount > 0 && isScaffolded) {
...
function findNameErrors(script, jobSpec) {
  ...
  for (const item of items) {
    ...
    // Only use real platform handles (Twitch username/handle) — never item.id which is just
    // the stringified array index ("0", "1", ...) and creates false positives on scores/stats.
    const handle = item.handle || item.twitchUsername;
...
${
  contentTypeForCheck.includes('twitch')
    ? `\nTWITCH ACCURACY — ENTITY GROUNDING (operator edits script — do NOT hard-block):
- AUTHORIZED FACTS list confirmed streamer display names, optional games, and clip titles from metadata.
- If the script uses a Twitch handle (lowercase username) instead of the streamer's display name → flag as entityNameError (wrong entity name).
- Game names, crew names, or incidental details NOT listed in AUTHORIZED FACTS → add to softAccuracyIssues (-10 soft note). Do NOT set fabricationFound for these unless they CONTRADICT a listed fact.
- fabricationFound=true ONLY when the script contradicts AUTHORIZED FACTS (wrong streamer identity, describes totally different clip than the confirmed title).
- Operator has full creative control — unverifiable color commentary is softAccuracyIssues, not fabrication.
- Set "entityNameErrors" for wrong-name instances: { wrong, correct }.`
    : ''
}
${
...
async function run(jobSpec, filledScript, gate0Report) {
  ...
  const contentTypeEarly = jobSpec?.contentType || jobSpec?.order?.contentType || '';
  const isTwitchContent = contentTypeEarly.includes('twitch');
  ...
  let script = filledScript || jobSpec.filledScript || jobSpec.scaffold || null;
  if (typeof script === 'string') {
    const { prepareScriptForGate1 } = require('../scaffold');
    const streamers = (jobSpec?.order?.inputs?.items || []).map((it) => ({
      streamer: it.twitchUsername || it.handle || it.displayName,
      displayName: it.displayName || it.name,
    }));
    script = prepareScriptForGate1(script, {
      expectedHeaders: expectedSceneHeaders(jobSpec),
      lockedIntro: jobSpec?.designSpec?.voice?.lockedIntro || null,
      lockedOutro: jobSpec?.designSpec?.voice?.lockedOutro || null,
      streamers,
    });
  }
  ...
  // Determine QA mode: 'strict' enforces style rules as score deductions (C0 default);
  // 'suggestive' converts style checks to advisory notes that return in the report
  // without lowering the score or blocking the job.  Only factual/fabrication checks
  // remain hard in suggestive mode.
  const qaMode = getWritingQaMode(jobSpec);
  const writingSuggestions = []; // populated in suggestive mode; included in portal report
  ...
  let pendingVideoFabricationNotes = [];
  try {
    ...
    if (!vReview.skipped && vReview.fabricationFound) {
      ...
      if (isTwitchContent) {
        pendingVideoFabricationNotes = ex.slice(0, 3);
        logError('PORTAL1_VIDEO_FABRICATION_SOFT', new Error(reason), { jobId, gate: '1b' });
        upstreamContext.escalatedConcerns.push(reason);
        if (vReview.observedSummary) {
          upstreamContext.downstreamHeadsUp = `Video reviewer summary: ${vReview.observedSummary}`;
        }
      } else {
  ...
  if (pendingVideoFabricationNotes.length > 0) {
    for (const issue of pendingVideoFabricationNotes) {
      score -= 10;
      deductions.push({ points: -10, reason: `Video QA note (operator may edit): ${issue}` });
    }
  }
  ...
  if (cTypeCheck.includes('twitch') && requiredDisplayNames.length > 0 && script) {
    for (const { wrong, correct } of requiredDisplayNames) {
      const handle = String(wrong || '').toLowerCase();
      if (!handle || handle === String(correct || '').toLowerCase()) continue;
      // Handles are lowercase logins — do not match scene headers (CINNA_INTRO).
      const handleOccurrences = (script.match(new RegExp(`\\b${handle.replace(/[.*+?^${}()|[\]\\]/g, '\\$&')}\\b`, 'g')) || []).length;
  ...
  // ── Style checks — mode-aware ─────────────────────────────────────────────
  // In 'strict' mode: deduct from score (C0 broadcast news behavior).
  // In 'suggestive' mode: collect as advisory notes, no score impact.
  // This lets the job pass while surfacing style guidance the customer can
  // act on once they configure their writing preferences.
  ...
  for (const v of hypeViolations) {
    if (qaMode === 'strict') {
      ...
      writingSuggestions.push({
        type: 'voice_style',
        note: `${v.violation} in ${v.section} — consider adjusting tone to match your brand voice`,
      });
    }
  }

  // Entity name errors — always hard (factual accuracy, not style)
  ...
  // Prohibited structural language patterns (broadcast CTA: subscribe, follow-us, etc.)
  ...
  for (const p of prohibitedFound) {
    if (qaMode === 'strict') {
      ...
      writingSuggestions.push({
        ...
        note: `"${p.label}" (${p.count}x) — CTA patterns like this are fine for social content; configure prohibited patterns via API or dashboard to enforce custom rules`,
      });
    }
  }

  // Outro check — short-form has NO outro scene, skip entirely
  const isShortForm =
    (jobSpec?.order?.formType || '').includes('short') ||
    (jobSpec?.order?.templateId || '').includes('short') ||
    (jobSpec?.contentType || '').includes('-short');
  let outroCheck = { ok: true, required: null, found: null }; // short-form: outro always ok
  if (!isShortForm) {
    ...
    if (!outroCheck.ok) {
      if (qaMode === 'strict') {
        ...
        writingSuggestions.push({
          type: 'outro',
          note: `No consistent sign-off detected — consider setting a brand outro line via API (voice.outroLine) or dashboard writing preferences`,
        });
      }
    }
  }

  // Locked intro check — reads from jobSpec.designSpec.voice.lockedIntro
  // Short-form has no locked intro (null → check skipped automatically)
  ...
  if (!isShortForm) {
    ...
    if (!introCheck.ok && introCheck.required) {
      if (qaMode === 'strict') {
        ...
        writingSuggestions.push({
          type: 'locked_intro',
          note: `No locked intro phrase detected — configure a consistent opening hook via API (voice.lockedIntro) or dashboard writing preferences`,
        });
      }
    }
  }
  ...
  if (score > 0 && process.env.GEMINI_API_KEY) {
    ...
    try {
      ...
      if (parsed.fabricationFound && parsed.examples?.length > 0 && !videoGroundingClear) {
        const reason = `Commentary accuracy note: ${parsed.examples.slice(0, 3).join('; ')}`;
        if (cTypeCheck.includes('twitch')) {
          for (const issue of parsed.examples.slice(0, 3)) {
            score -= 10;
            deductions.push({
              points: -10,
              reason: `Commentary note (operator may edit): ${issue}`,
            });
          }
          logError('PORTAL1_FABRICATION_SOFT', new Error(reason), { jobId, portal: 1 });
          upstreamContext.escalatedConcerns.push(reason);
        } else {
      ...
      // Twitch: soft accuracy issues from Gemini (game names, unlisted details) — never hard-block
      if (
        ...
        for (const issue of parsed.softAccuracyIssues.slice(0, 3)) {
          score -= 10;
          deductions.push({ points: -10, reason: `Twitch accuracy note: ${issue}` });
        }
      }
  ...
  if (score >= PASS_THRESHOLD) {
    ...
    if (isSports && !fabricationFail && !hasRosterViolation) {
      outcome = 'sendback';
    ...
    } else if (cTypeCheck.includes('twitch')) {
      outcome = 'sendback';
      score = Math.max(score, SENDBACK_THRESHOLD);
      fixDirective = {
        attempt: 1,
        committed: commit(jobSpec).committed,
        delivered: `Twitch script scored ${score}/100 — held for operator edit (content notes do not block pipeline)`,
        mismatches: deductions.map((d) => ({
          field: d.reason.includes('ENTITY') ? 'entity_name' : 'commentary_accuracy',
          committed: 'Operator-editable commentary',
          delivered: d.reason,
          fix: d.reason,
        })),
        nameErrors,
      ...
      logError('PORTAL1_TWITCH_SENDBACK', new Error(`Score ${score} — twitch sendback (operator edit)`), {
        jobId,
        portal: 1,
        score,
        deductions,
  ...
  return {
    ...
    outcome,
    qaMode,
    writingSuggestions: writingSuggestions.length > 0 ? writingSuggestions : undefined,
    fixDirective,
    upstreamContext: {
      ...upstreamContext,
