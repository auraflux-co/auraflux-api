...
async function run(jobSpec, gate3aReport, priorPortalReports) {
  ...
  // Clip comps use top-blur-fold logo (not broadcast chrome) — skip reburn trigger.
  const isClipCompChrome =
    chromeCfg.layout === 'clip-comp' || chromeCfg.logoPosition === 'top-blur-fold';
  if (committedLogoPos && chromeCommitted && !isClipCompChrome && (gate3aReport?.score ?? 100) < 70) {
    mismatches.push({
  ...
  // In suggestive QA mode, downgrade escalate to fixable — operator reviews notes rather than blocking
  const qaMode = jobSpec?.qaMode || loadCustomerConfig(jobSpec?.customerId)?.qaMode;
  if (outcome === 'mismatch_escalate' && qaMode === 'suggestive') {
    console.log(`[portal3b] ${jobId}: suggestive mode — downgrading mismatch_escalate to mismatch_fixable`);
    outcome = 'mismatch_fixable';
  }
