  ...
  for (let attempt = 0; attempt <= MAX_TTS_RETRIES; attempt++) {
    ...
    try {
      ...
      if (status === 401 || status === 403) {
        ...
        // Try Gemini TTS as fallback before degrading to skip.
        const isIpFlag = errBody.includes('detected_unusual_activity') ||
          (Buffer.isBuffer(rawData) && rawData.toString().includes('detected_unusual_activity'));
        if (isIpFlag) {
          console.log(`[tts_ext:${jobId}] ElevenLabs IP flagged — attempting Gemini TTS fallback`);
          try {
