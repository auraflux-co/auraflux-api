...
async function runWorker({ jobSpec } = {}) {
  ...
  try {
    ...
    return {
      passed: false,
      outcome: 'skip',
      reason: `ElevenLabs SDK unavailable — TTS skipped: ${e.message}`,
      audioPath: null,
      charCount: 0,
    };
  }
  ...
  try {
    ...
    // 401/403 = bad key, 429 = rate limit, 5xx = ElevenLabs down — all are graceful skips,
    // not job failures. The job produces output without a voiceover rather than failing entirely.
    return {
      passed: false,
      outcome: 'skip',
      reason: `ElevenLabs API error — TTS skipped: ${err.message}`,
      audioPath: null,
      charCount: 0,
    };
  }
