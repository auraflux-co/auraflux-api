 ...
 */
async function runFFmpegDefectScan(videoPath, jobId) {
  ...
  const freezeResult = await new Promise((resolve) => {
    ...
    const proc = require('child_process').spawn(
      ...
      // true freezes. d=3: require 3s of freeze (was 2s) to reduce false positives.
      ['-i', videoPath, '-vf', 'freezedetect=n=0.003:d=3', '-f', 'null', '-'],
      { timeout: 120000 }
    );
    proc.stderr.on('data', (d) => { stderr += d.toString(); });
    proc.on('close', () => resolve(stderr));
...
async function run(jobSpec, assembledPath, priorPortalReports) {
  ...
  try {
    ffmpegDefectScan = await runFFmpegDefectScan(targetPath, jobId);
  } catch (scanErr) {
