...
async function runFFmpegDefectScan(videoPath, jobId) {
  ...
  const blackMatches = [...blackResult.matchAll(/black_start:([\d.]+)/g)];
  const blackFrames = [];
  for (const m of blackMatches) {
    const start = parseFloat(m[1]);
    if (start < 1) continue; // ignore opening black frame (common)
    const tsStr = new Date(start * 1000).toISOString().substr(11, 8);
    blackFrames.push({ type: 'black_frame', timestamp: tsStr, detail: `Black frame at ${tsStr}` });
  }
  // Only flag as defect if black frames appear mid-video (likely encoding error)
  // Opening and closing black frames are normal
  if (videoDur > 0 && blackFrames.some(b => parseFloat(b.timestamp) < videoDur - 2)) {
    defects.push(...blackFrames.filter(b => parseFloat(b.timestamp) < videoDur - 2));
  }
