...
async function runFFmpegDefectScan(videoPath, jobId) {
  ...
  for (const m of freezeMatches) {
    ...
    if (start < 3) continue;
    // Skip freezes that start in the last 4s — Twitch/YouTube clips commonly end on a
    // static last frame (clip capture ends). This is not a production defect.
    if (videoDur > 0 && start >= (videoDur - 4)) continue;
    const tsStr = new Date(start * 1000).toISOString().substr(11, 8);
    defects.push({
      type: 'frozen_frame',
  ...
  try {
    ...
    const lufsMatch = ebur128Result.match(/I:\s*([-\d.]+)\s*LUFS/);
