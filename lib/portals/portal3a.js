'use strict';
/**
 * lib/gates/gate3a.js — Portal 3a: Gemini Assembly QA (Qualitative)
 *
...
const { resolveChromeCfg, fingerprintResolvedChromeCfg, detectSourcePlatform } = require('../chrome_overlay_ffmpeg');
const { ffmpegPath } = require('../ffmpeg_utils');
const { validatePortalInput } = require('../portal_contract');
const { getManifestSummary, computeChapters } = require('../services/processing_manifest');
...
async function _writeGate3aSynthPreviewPng(jobId, sourceMp4) {
  ...
  if (sourceMp4 && fs.existsSync(sourceMp4)) {
    await new Promise((res, rej) => {
      execFile(
        FFMPEG,
        ['-y', '-ss', '2', '-i', sourceMp4, '-frames:v', '1', '-q:v', '2', outPng],
        { timeout: 60000 },
        (err) => (err ? rej(err) : res())
      );
    });
...
// Platform defaults — overridden by customerConfig.qaThresholds.gate3a
...
function getThresholds(jobSpec) {
  ...
  const t = getPortalThresholds(customerId, templateId, 'gate3a', {
    pass: DEFAULT_PASS_THRESHOLD,
    manualReview: DEFAULT_PASS_WITH_NOTES_THRESHOLD,
  });
  return {
...
/**
 * CPD-431: FFmpeg deterministic defect scan — runs on the FULL assembled video
 * before any AI sampling. Catches hard defects that AI models miss or guess at:
 *   - Audio silence gaps > 3s (catches the audio-drops-before-end bug)
 *   - Frozen frames > 2s (hard visual failure)
 *   - Missing audio track
 *   - Audio stream shorter than video by > 5s (truncated audio)
 *
 * Returns { clean: boolean, defects: Array<{ type, timestamp, detail }> }
 */
async function runFFmpegDefectScan(videoPath, jobId, contentType = '') {
  ...
  const hasAudio = await new Promise((resolve) => {
    execFile(
      ...
      { timeout: 15000 },
      (err, stdout) => resolve(!err && stdout.trim().startsWith('audio'))
    );
  });

  if (!hasAudio) {
    defects.push({ type: 'no_audio_track', timestamp: '0:00', detail: 'No audio stream detected in assembled video' });
    return { clean: false, defects };
  }

  // ── 2. Get video duration and audio duration ───────────────────────────────
  const [videoDur, audioDur] = await Promise.all([
    new Promise((resolve) => {
      execFile(ffprobePath(), ['-v', 'quiet', '-select_streams', 'v:0',
    ...
    new Promise((resolve) => {
      execFile(ffprobePath(), ['-v', 'quiet', '-select_streams', 'a:0',
        '-show_entries', 'stream=duration', '-of', 'csv=p=0', videoPath],
        { timeout: 15000 }, (err, stdout) => {
          const d = parseFloat(stdout.trim());
          resolve(isNaN(d) ? 0 : d);
  ...
  if (videoDur > 0 && audioDur > 0 && (videoDur - audioDur) > 5) {
    const gapSecs = Math.round(videoDur - audioDur);
    const gapTs = new Date(audioDur * 1000).toISOString().substr(11, 8);
    defects.push({
      type: 'audio_truncated',
      timestamp: gapTs,
      detail: `Audio ends ${gapSecs}s before video ends — silent final ${gapSecs}s`,
    });
  }

  // ── 3. Silence detection (skip first 5s — avoid false positive on fade-in) ─
  // n=-45dB: slightly less strict than -50dB to avoid flagging natural low-energy moments.
  // d=5: require 5s of silence (was 3s) — streaming clips often have brief quiet moments
  //       between commentary that are not audio dropouts.
  const silenceResult = await new Promise((resolve) => {
    let stderr = '';
    const proc = require('child_process').spawn(
      FFMPEG,
      ['-i', videoPath, '-af', 'silencedetect=n=-45dB:d=5', '-f', 'null', '-'],
      { timeout: 120000 }
    );
  ...
  const silenceMatches = [...silenceResult.matchAll(
    /silence_start:\s*([\d.]+)/g
  )];
  for (const m of silenceMatches) {
    const start = parseFloat(m[1]);
    if (start < 5) continue; // ignore opening fade
    // Skip silence starting in the last 4s — clips often end on a silent tail frame.
    if (videoDur > 0 && start >= (videoDur - 4)) continue;
    const tsStr = new Date(start * 1000).toISOString().substr(11, 8);
    defects.push({
      type: 'silence_gap',
      timestamp: tsStr,
      detail: `Silence detected starting at ${tsStr} (>5s) — possible audio dropout`,
    });
  }

  // ── 4. Freeze detection ───────────────────────────────────────────────────
  // For clips contentType (gaming/stream highlights), use d=5 — loading screens,
  // lobby screens, and score screens regularly hold for 3-4s in legitimate gaming content.
  // For all other content types, use d=3.
  const freezeDuration = (contentType === 'clips') ? 5 : 3;
  const freezeResult = await new Promise((resolve) => {
    let stderr = '';
    const proc = require('child_process').spawn(
      FFMPEG,
      // n=0.003: 0.3% pixel difference threshold — filters out natural gaming/stream
      // static scenes (menus, pause screens, talking-head moments) while still catching
      // true freezes. d varies by contentType (5s for clips, 3s otherwise).
      ['-i', videoPath, '-vf', `freezedetect=n=0.003:d=${freezeDuration}`, '-f', 'null', '-'],
      { timeout: 120000 }
    );
    proc.stderr.on('data', (d) => { stderr += d.toString(); });
    proc.on('close', () => resolve(stderr));
  ...
  const freezeMatches = [...freezeResult.matchAll(
    /freeze_start:\s*([\d.]+)/g
  )];
  for (const m of freezeMatches) {
    const start = parseFloat(m[1]);
    // Skip freezes in the first 3s — intro frames, loading screens, and vertical-crop
    // artifacts at the start of gaming clips are not production defects.
    if (start < 3) continue;
    // Skip freezes that start in the last 8s — Twitch/YouTube clips commonly end on a
    // static last frame (score screen, game transition). Gaming clips frequently freeze
    // in the final 5-8s as the clip capture ends. Was 4s, extended to 8s (CPD-869).
    if (videoDur > 0 && start >= (videoDur - 8)) continue;
    const tsStr = new Date(start * 1000).toISOString().substr(11, 8);
    defects.push({
      type: 'frozen_frame',
      timestamp: tsStr,
      detail: `Frozen frame detected at ${tsStr} (>3s) — static video`,
    });
  }

  // ── 5. Black frame detection (CPD-438) ───────────────────────────────────
  const blackResult = await new Promise((resolve) => {
    let stderr = '';
    const proc = require('child_process').spawn(
      FFMPEG,
      ['-i', videoPath, '-vf', 'blackdetect=d=0.05:pix_th=0.1', '-f', 'null', '-'],
      { timeout: 60000 }
    );
    proc.stderr.on('data', (d) => { stderr += d.toString(); });
    proc.on('close', () => resolve(stderr));
  ...
  // CPD-481: capture start, end, and duration so we can filter brief blips
  const blackMatches = [...blackResult.matchAll(/black_start:([\d.]+)\s+black_end:([\d.]+)\s+black_duration:([\d.]+)/g)];
  const blackFrames = [];
  for (const m of blackMatches) {
    const start    = parseFloat(m[1]);
    const duration = parseFloat(m[3]);
    // Skip stream-startup window (first 15s) — Twitch/YouTube capture begins with buffer frames
    if (start < 15) continue;
    // Skip very brief encoder blips (< 0.3s) — single dropped frames, not a real blackout
    if (duration < 0.3) continue;
    const tsStr = new Date(start * 1000).toISOString().substr(11, 8);
    blackFrames.push({ type: 'black_frame', timestamp: tsStr, detail: `Black frame at ${tsStr} (${duration.toFixed(2)}s)` });
  }
  // Only flag as defect if black frames appear mid-video (not last 5s — stream capture end)
  if (videoDur > 0 && blackFrames.some(b => parseFloat(b.timestamp) < videoDur - 5)) {
    defects.push(...blackFrames.filter(b => parseFloat(b.timestamp) < videoDur - 5));
  }

  // ── 6. EBU R128 loudness measurement (CPD-438) ───────────────────────────
  // Non-blocking: store loudness reading in result for grader, do not add to defects
  let loudnessLUFS = null;
  try {
    const ebur128Result = await new Promise((resolve) => {
      let stderr = '';
      const proc = require('child_process').spawn(
        FFMPEG,
        ['-i', videoPath, '-af', 'ebur128=peak=true', '-f', 'null', '-'],
        { timeout: 90000 }
      );
      proc.stderr.on('data', (d) => { stderr += d.toString(); });
      proc.on('close', () => resolve(stderr));
    ...
    // Match the Summary block's integrated loudness — not the per-frame I: readings
    // which start at -70 LUFS during the first transient silence before stabilising.
    const lufsMatch = ebur128Result.match(/Integrated loudness:[\s\S]*?I:\s*([-\d.]+)\s*LUFS/);
    if (lufsMatch) loudnessLUFS = parseFloat(lufsMatch[1]);
  } catch {}

  console.log(`[portal3a:ffmpeg_scan] ${jobId}: ${defects.length === 0 ? 'clean' : `${defects.length} defect(s): ${defects.map(d => d.type).join(', ')}`}${loudnessLUFS != null ? ` | loudness: ${loudnessLUFS} LUFS` : ''}`);
  return {
...
function canProduce(jobSpec) {
  ...
  if (!assembledPath) {
    const asmErr = jobSpec.assemblyFailReason ? `Assembly failed: ${jobSpec.assemblyFailReason}` : 'No assembledPath in jobSpec — assembled video path required';
    reasons.push(asmErr);
...
/**
 * Build the exact Gemini user prompt for one Portal 3a sample (offline replay + tracing).
...
function buildGate3aGeminiSamplePrompt(jobSpec, label, ctx) {
  ...
  // Read what chrome elements the job spec says are present — opt-in only, default false.
  // job_spec.js now defaults these to false, so this fallback is a safety net only.
  const hasTopBar = chromeCfg?.hasTopBar === true;
  const hasFlag = chromeCfg?.hasFlag === true;
  const hasSidebar = chromeCfg?.hasSidebar === true;
  const hasTicker = chromeCfg?.hasTicker === true;
  const hasLogo = chromeCfg?.hasLogo === true;
  ...
  // CPD-486: clip-only jobs have no avatar — describe the actual production model
  // so Gemini doesn't look for avatar faces or broadcast chrome that will never appear.
  const assembledDesc = isSplitScreen
    ...
    : isClipOnly
      ? `${confirmedFormat === '9:16' ? 'portrait (9:16)' : 'landscape (16:9)'} — source clip with branding overlay. NO avatar segments. NO broadcast chrome (top bar / flag / sidebar / ticker). ${hasLogo ? `AuraFlux logo (${logoSize}px, ${logoPosition}) only.` : 'Branding overlay only.'}`
      : `landscape (16:9) — avatar segments + source clips + broadcast chrome overlay${hasTicker ? ' + ticker' : ''}.`;

  const prompt = `You are Portal 3a — the qualitative assembly reviewer for the AuraFlux production pipeline.

── HOW THIS WORK WAS PRODUCED ────────────────────────────────────────────────
1. Portal 0 confirmed source videos and detected format (${confirmedFormat})
${isClipOnly
  ? `2. No script generation — this is a clip-only compilation job (no avatar, no voiceover)
3. No HeyGen avatar renders — source clip is the entire content
4. FFmpeg assembled: ${assembledDesc}`
  : `2. Portal 1 (Claude) reviewed the script for style quality
3. Portal 2 (ffprobe) confirmed all HeyGen avatar renders passed quality checks
4. FFmpeg assembled: ${assembledDesc}`
}
5. You are reviewing the assembled result at 3 sample points

YOUR GOAL: Help this job pass to Portal 4. Identify exactly what is wrong and how assembly can fix it. A targeted fix directive is success. A vague rejection is failure on your part.

YOUR ROLE: You review this ${label.toUpperCase()} sample (${SAMPLE_DURATION}s). Portal 4 watches the full video. Your scope is technical quality at this sample point — not editorial content.
...
YOU DO NOT CHECK: script content, names, editorial choices, pacing across the full runtime — those are Portal 1 and Portal 4's jobs.
...
- Include "downstreamHeadsUp" so Portal 4 knows exactly what to watch for in the full video
...
${
  !hasFlag && !hasSidebar
    ? '- NO flag, NO sidebar — do NOT expect or check for these elements.'
    : `- Flag should show: category "${jobSpec?.designSpec?.voice?.categoryLabel || jobSpec?.designSpec?.chrome?.categoryLabel || 'CATEGORY'}" + active item title for current scene position
...
2. Source clips: visible? ${isClipOnly ? 'This is a clip-only job — the ENTIRE video should be source clip footage (no avatar talking head).' : 'not just talking head the entire time?'}
3. Audio: FFmpeg has already verified no silence gaps or dropout. Your job is visual only — set audioContinuous=true unless you hear obvious corruption (distortion, static, wrong audio entirely). Do NOT flag normal pauses or quiet moments.
4. ${
    isSplitScreen
      ? `Layout: portrait split-screen correct? Top half = ${splitTop}, bottom half = ${splitBottom}? Logo (${logoSize}px) visible at ${logoPosition}?`
      : isClipOnly
        ? `Branding: ${hasLogo ? `AuraFlux logo visible at ${logoPosition}?` : 'no logo expected'} ${hasTopBar || hasFlag || hasSidebar ? 'Unexpected chrome elements (top bar / flag / sidebar) — report if present.' : 'No top bar, flag, or sidebar expected — report if you see any.'}`
...
${isSplitScreen ? '5. Portrait layout: top/bottom split correct? caption visible near split line?\n' : ''}${
    !isSplitScreen && !isClipOnly
      ? `5. Flag accuracy: Does the top-left flag show the correct category label AND the correct active item title for this scene position?
6. Sidebar accuracy: Does the sidebar show the correct item titles matching the ordered items?`
      : !isSplitScreen && isClipOnly
        ? `5. Content quality: Is this clearly watchable streaming content (gameplay, IRL, reaction, etc.)? If it appears to be a test pattern, black screen, or corrupted file, flag it.`
        : ''
  }
...
${
  isSplitScreen
    ? `- This is a 9:16 portrait split-screen. Top 50% = ${splitTop}. Bottom 50% = ${splitBottom}.
...
- If you see a top bar, flag, or sidebar — those are NOT in spec. Report them as violations.`
    : isClipOnly
      ? `- This is a CLIP-ONLY job. The entire video should be source clip footage — NO avatar, NO talking head.
- Per spec: NO top bar, NO flag, NO sidebar, NO ticker.
- Logo: ${hasLogo ? `AuraFlux logo (${logoSize}px) should be visible near ${logoPosition}` : 'none expected'}.
- chromeVisible=true if the logo is present (or branding overlay is visible). chromeCorrect=true if no unexpected broadcast chrome elements appear.
- If you see a talking head avatar taking up significant screen space — that is a production error. Set sourceClipsVisible=false.
- If you see a top bar, flag, or sidebar — those are NOT in spec for this clip-only job. Report them as violations.
- The source clip should fill the frame (full-screen, ${confirmedFormat === '9:16' ? '9:16 portrait' : '16:9 landscape'}).`
      : `- Top bar (y=0, h=48px): ${hasTopBar ? `left side must show episode number, center must show show name matching skin "${expectedSkin}"` : 'NOT in spec — report if present'}
...
Return JSON only:
{
  ...
  "chromeDescription": "${isSplitScreen ? `describe portrait split layout (top=${splitTop}, bottom=${splitBottom}) and logo visibility` : isClipOnly ? 'describe what you see in the frame — is it source clip footage? any unexpected elements (avatar, top bar, flag, sidebar)?' : 'what you actually see in the chrome elements (top bar, flag, sidebar, logo) — be specific'}",
  ${
    !isSplitScreen && !isClipOnly
  ...
  "brandingRubric": {
    "logoVisible": boolean,
    "logoPositionCorrect": boolean,
    "logoPositionObserved": "describe where exactly the logo is on screen",
    "showNameReadable": boolean,
    "showNameText": "exact text you can read in the top bar or lower-third, or null if not visible",
    "lowerThirdVisible": boolean,
    "lowerThirdText": "exact text in the lower-third label, or null if not visible",
    "aspectRatioCorrect": boolean,
    "aspectRatioObserved": "16:9 or 9:16 or other — describe what you see"
  },
  "artifactChecks": {
    "blockingArtifacts": boolean,
    "colorBanding": boolean,
    "greenOrMagentaFrames": boolean,
    "alphaBleedthrough": boolean,
    "hardCutGlitch": boolean,
    "anyArtifact": boolean
  },
  ...
  "downstreamHeadsUp": "one sentence for Portal 4 to watch for specifically, or null if clean",
  "score": 0-100
}

SCORING RUBRIC — use this to set your score:
- 100: perfect sample — all checks pass, branding correct, no artifacts
- 90-99: minor cosmetic issue that won't affect viewer experience
- 80-89: noticeable issue (slightly wrong lower-third text, logo partially clipped) but video is watchable
- 70-79: significant issue — wrong branding, missing logo, aspect ratio mismatch — viewer would notice
- 60-69: major issue — content visible but quality is clearly below production standard
- <60: hard stop — freeze, encoding failure, wrong content, broken chrome that cannot be shipped
Only a freeze (freezeDetected=true) is an AUTOMATIC hard fail regardless of score.`;
 ...
 */
async function run(jobSpec, assembledPath, priorPortalReports) {
  const jobId = jobSpec?.jobId || 'unknown';
  const now = () => new Date().toISOString();

  // Load thresholds from customerConfig — universal, not c0-specific
  const { passThreshold: PASS_THRESHOLD, passWithNotesThreshold: PASS_WITH_NOTES_THRESHOLD } =
  ...
  console.log(`[portal3a] ${jobId}: run() invoked — qaMode=${jobSpec?.qaMode} assembledPath=${jobSpec?.assembledPath || jobSpec?.outputPath}`);
  // CPD-483: validate upstream outputs before doing any work
  // CPD-487: FATAL validation — if assembledPath is missing, abort immediately with a clear
  // error attributed to the assembly stage, not portal3a. Previously this was non-strict
  // (warn-only), which allowed the pipeline to continue and surface a misleading portal3a fail.
  validatePortalInput('3a', jobSpec, { strict: true });
  const readiness = canProduce(jobSpec);
  if (!readiness.ready) {
    const reason = `Portal 3a not ready: ${readiness.reasons.join('; ')}`;
    logError('GATE3A_NOT_READY', new Error(reason), { jobId, gate: '3a' });
    console.log(`[portal3a] ${jobId}: canProduce FAILED — ${reason}`);
    return { ...baseOutput, failReason: reason, completedAt: now() };
  }
  console.log(`[portal3a] ${jobId}: canProduce OK — proceeding to Gemini analysis`);
  ...
  if (jobSpec?.expectedSynth) {
    const uc = {
      ...
      downstreamHeadsUp:
        'expectedSynth/lab — broadcast-style Gemini sampling not run (product policy)',
    };
    return {
      gate: '3a',
      jobId,
      ...
      ffmpegAlarm: { fired: false, targetTimestamp: null, issue: null },
      upstreamContext: uc,
      notes: ['expectedSynth/lab — Portal 3a broadcast sampling skipped (product policy)'],
      completedAt: now(),
    };
  }

  // Commentary assembly mode — CPD-74
  // When job spec declares assembly.mode: 'commentary', route to script-cued B-roll assembler
  // before running the standard Gemini QA path.
  if (jobSpec?.assembly?.mode === 'commentary') {
    const {
      ...
      stubClipManifest,
      parseScriptSegments,
    } = require('../services/commentary_assembly');

    const validation = validateCommentaryJobSpec(jobSpec);
    if (!validation.valid) {
      logError('PORTAL3A_COMMENTARY_INVALID', new Error(validation.reason), { jobId });
      return {
        ...baseOutput,
        outcome: 'hard_fail',
    ...
    // Use persisted manifest first; otherwise auto-source from footage (CPD-73)
    let clipManifest = jobSpec?.state?.clipManifest;
    if (!clipManifest) {
      const footagePaths = (jobSpec?.order?.inputs?.items || [])
        .map((i) => i.localPath)
        .filter(Boolean);
      if (footagePaths.length > 0) {
        try {
          const { autoSourceClipManifest } = require('../clip_sourcing');
          clipManifest = await autoSourceClipManifest(
            footagePaths,
            script,
            jobSpec?.designSpec?.showTitle || jobSpec?.order?.contentType || 'show',
            { planTier: jobSpec?.planTier || 'operate' },
          );
          if (clipManifest && jobSpec.state) {
            jobSpec.state.clipManifest = clipManifest;
          }
        } catch (e) {
          logError('CPD73_AUTO_SOURCE_FAIL', e, { jobId });
        }
      }
      if (!clipManifest) {
        clipManifest = stubClipManifest(parseScriptSegments(script).length);
      }
    }

    const assemblyResult = commentaryAssemble(script, clipManifest, jobSpec.assembly || {});

    // Persist segment timeline to job spec state for assembly layer to consume
    if (jobSpec.state) {
      jobSpec.state.commentarySegments = assemblyResult.segments;
      jobSpec.state.commentaryAssembly = {
        segmentCount: assemblyResult.scriptSegmentCount,
        matchedCount: assemblyResult.matchedCount,
        unmatchedCount: assemblyResult.unmatched.length,
        unmatched: assemblyResult.unmatched,
        timingWarnings: assemblyResult.timingWarnings,
        totalDuration: assemblyResult.totalDuration,
        clipManifestSource: clipManifest.source || 'unknown',
        assembledAt: now(),
      };
    }

    const hasUnmatched = assemblyResult.unmatched.length > 0;
    const outcome = hasUnmatched ? 'pass_with_notes' : 'pass';
    const notes = [
      `Commentary assembly: ${assemblyResult.matchedCount}/${assemblyResult.scriptSegmentCount} segments matched`,
      ...(assemblyResult.timingWarnings.length > 0
        ? [`Timing warnings: ${assemblyResult.timingWarnings.join('; ')}`]
        : []),
      ...(hasUnmatched
        ? [`Unmatched segments (no clip found): [${assemblyResult.unmatched.join(', ')}]`]
        : []),
      ...(clipManifest.source === 'stub'
        ? ['Using stub clip manifest — CPD-73 clip sourcing not yet implemented']
        : []),
    ];
    ...
    return {
      ...
      jobId,
      passed: true,
      score: hasUnmatched ? 75 : 90,
      outcome,
      sampleFindings: {
        early: { commentary: true, segments: assemblyResult.segments.slice(0, 2) },
        middle: { commentary: true, segments: assemblyResult.segments.slice(1, 3) },
        late: {
          commentary: true,
          segments: assemblyResult.segments.slice(-2),
        },
      },
      ffmpegAlarm: { fired: false, targetTimestamp: null, issue: null },
      upstreamContext: {
        ...
        confirmedClean: [
          ...upstreamContext.confirmedClean,
          'commentary_assembly_planned',
        ],
        downstreamHeadsUp: hasUnmatched
          ? `Commentary: ${assemblyResult.unmatched.length} unmatched segment(s) — assembly will use black placeholder`
          : null,
      },
      notes,
      commentaryAssembly: assemblyResult,
      completedAt: now(),
    };
  }
  ...
  const confirmedFormat =
    ...
    '16:9';
  const expectedSkin =
    jobSpec?.designSpec?.chrome?.skin || jobSpec?.order?.designSpec?.chrome?.skin || null;
  ...
  // ── CPD-431: FFmpeg deterministic defect scan — runs on full file before AI ──
  // Catches hard defects deterministically: silence gaps, frozen frames, missing
  // audio track, truncated audio. AI cannot reliably detect these — ffmpeg owns them.
  let ffmpegDefectScan = { clean: true, defects: [] };
  try {
    ffmpegDefectScan = await runFFmpegDefectScan(targetPath, jobId, jobSpec?.contentType || '');
  } catch (scanErr) {
    console.warn(`[portal3a] ${jobId}: ffmpeg defect scan threw (non-fatal) — ${scanErr.message}`);
  }

  // Persist loudness + black frame scan data for grader (CPD-438)
  const ffmpegScanMeta = {
    loudnessLUFS: ffmpegDefectScan.loudnessLUFS ?? null,
    blackFrames:  ffmpegDefectScan.blackFrames  ?? [],
    defects:      ffmpegDefectScan.defects,
    scannedAt:    now(),
  };
  try {
    const { saveOut45 } = require('../job_spec');
    await saveOut45(jobId, 'portalReports.portal3a.ffmpegScan', ffmpegScanMeta);
  } catch {}

  if (!ffmpegDefectScan.clean) {
    const defectSummary = ffmpegDefectScan.defects.map((d) => `${d.type}@${d.timestamp}: ${d.detail}`).join('; ');
    logError('GATE3A_FFMPEG_DEFECT', new Error(defectSummary), { jobId, gate: '3a', defects: ffmpegDefectScan.defects });
    console.error(`[portal3a] ${jobId}: FFmpeg defect scan HARD FAIL — ${defectSummary}`);
    // Hard fail — no point running expensive Gemini if the file is broken
    return {
      ...baseOutput,
      passed: false,
      outcome: 'hard_fail',
      score: 0,
      ffmpegDefects: ffmpegDefectScan.defects,
      ffmpegScan: ffmpegScanMeta,
      failReason: `FFmpeg defect detected: ${defectSummary}`,
      completedAt: now(),
    };
  }

  // Sample points: EARLY (10%), MID-EARLY (40%), MID-LATE (75%), LATE (90%)
  // 4 points instead of 3 — 75% catches the "last-minute dropout" window
  // Audio continuity is now owned by ffmpeg scan above; Gemini owns visual/content checks only.
  const SAMPLE_DURATION = 10;
  const samplePoints = [
    { label: 'early',    startFn: (d) => Math.max(0, d * 0.10 - SAMPLE_DURATION / 2) },
    { label: 'midEarly', startFn: (d) => Math.max(0, d * 0.40 - SAMPLE_DURATION / 2) },
    { label: 'midLate',  startFn: (d) => Math.max(0, d * 0.75 - SAMPLE_DURATION / 2) },
  ...
  const sampleFindings = { early: null, midEarly: null, midLate: null, late: null };
  let totalScore = 100;
  const deductions = [];
  ...
  const priorContext =
    [
      gate0Report?.passed
        ? `Portal 0: sources confirmed, format=${gate0Report.confirmedFormat || confirmedFormat}`
        : null,
      gate1Report?.passed
        ? `Portal 1: script passed style QA (score ${gate1Report.score ?? 'n/a'})`
        : null,
      gate2Report?.passed
        ? `Portal 2: all renders passed quality check (score ${gate2Report.score ?? 'n/a'})`
        : null,
      ...(gate2Report?.upstreamContext?.downstreamHeadsUp
        ? [`Portal 2 flagged: ${gate2Report.upstreamContext.downstreamHeadsUp}`]
        : []),
    ]
      .filter(Boolean)
      .join('\n') || 'No prior gate reports available';

  // CPD-486: clip-only jobs (contentType === 'clips') have no avatar segments — the assembled
  // output is source clip + branding overlay only.  Tell Gemini the right production model so
  // it doesn't look for avatar faces or broadcast chrome that will never appear.
  const isClipOnly = (jobSpec?.contentType || jobSpec?.order?.contentType || '') === 'clips';
  ...
  for (const { label, startFn } of samplePoints) {
    ...
    const { prompt, meta: g3PromptMeta } = buildGate3aGeminiSamplePrompt(
      jobSpec,
      label,
      gate3aPromptCtx
    );
    ...
    if (!finding.audioContinuous) {
      ...
      logError(
        'GATE3A_AUDIO_DROPOUT',
        new Error(`Audio dropout at ${label} (Gemini flag — ffprobe confirmed audio present)`),
        { jobId, gate: '3a', label }
      );
    }
  }
  ...
  if (expectedClips > 0 && !freezeFound) {
    const anyClipsVisible = Object.values(sampleFindings).some(
      (f) => f && f.sourceClipsVisible === true
    );
    ...
    if (!anyClipsVisible && !hasSceneStructure) {
      ...
      totalScore -= 15;
      logError('GATE3A_NO_CLIPS_IN_ANY_SAMPLE', new Error(reason), {
        jobId,
        gate: '3a',
        expectedClips,
      });
      ...
      logError(
        'GATE3A_CLIPS_NOT_IN_SAMPLES',
        new Error(
          `Clips not visible in sampled frames (expected at scenes ${clipSceneIndices.join(',')}) — samples may have landed on avatar segments`
        ),
        { jobId, gate: '3a', expectedClips, clipSceneIndices }
      );
    }
  }
  ...
  // content integrity failure (e.g. wrong game, wrong player) and must block Portal 4.
  if (isSplitScreen && !freezeFound) {
    const wrongClipSamples = Object.values(sampleFindings).filter(
      (f) => f && f.clipContentMatchesScript === false
    );
    if (wrongClipSamples.length > 0) {
      const descriptions = wrongClipSamples
        .map((f) => f.clipContentDescription || 'wrong content')
        .join('; ');
  ...
  // Chrome strict only fires when the job spec EXPLICITLY declares a top bar (opt-in).
  // Jobs without a designSpec (e.g. Operate API jobs) have chromeCfg.hasTopBar=undefined
  // and must not fail chrome checks — no chrome was ever requested.
  if (chromeStrict && chromeCfg && chromeCfg.hasTopBar === true && !freezeFound) {
    ...
    if (strictViolations.length > 0) {
      const hasMultiSampleConsistency = ['chromeCorrect', 'flagAccurate', 'sidebarAccurate'].some(
        (k) => strictViolations.filter((v) => v[k]).length >= 2
      );
  ...
  const allUploadsFailedCount = deductions.filter(
    (d) => d.reason && d.reason.includes('Could not upload') && d.reason.includes('Gemini')
  ).length;
  ...
  if (allSampleUploadsFailed && !freezeFound && !chromeBlocker) {
    ...
    logError(
      'GATE3A_ALL_UPLOADS_FAILED_PASSTHROUGH',
      new Error('All Gemini uploads failed — treating as pass_with_notes for operator review'),
      { jobId, gate: '3a', totalScore }
    );
  } else if (freezeFound) {
    outcome = 'hard_fail';
    ...
    logError('GATE3A_CHROME_BLOCKER', new Error('Chrome/flag/sidebar failed strict spec check'), {
      jobId,
      gate: '3a',
      sampleFindings,
    });
    ...
    outcome = 'hard_fail';
    logError(
      'GATE3A_HARD_FAIL_SCORE',
      new Error(`Score ${totalScore} < ${PASS_WITH_NOTES_THRESHOLD}`),
      { jobId, gate: '3a', totalScore }
    );
  }

  // In suggestive QA mode, downgrade score/content hard-fails to pass_with_notes.
  // Freeze and chrome-strict failures still block (those indicate a broken video, not style mismatches).
  const qaMode = jobSpec?.qaMode || loadCustomerConfig(jobSpec?.customerId)?.qaMode;
  console.log(`[portal3a] ${jobId}: final outcome=${outcome} score=${Math.round(totalScore)} freezeFound=${freezeFound} chromeBlocker=${chromeBlocker} qaMode=${qaMode} passed=${passed}`);
  if (outcome === 'hard_fail' && !freezeFound && !chromeBlocker && qaMode === 'suggestive') {
    console.log(`[portal3a] ${jobId}: suggestive mode — downgrading hard_fail to pass_with_notes (score=${Math.round(totalScore)})`);
    outcome = 'pass_with_notes';
    passed = true;
    totalScore = Math.max(totalScore, PASS_WITH_NOTES_THRESHOLD);
  }

  // ── Feature Delivery Verification ─────────────────────────────────────────
  // Compare what was ordered (jobSpec) against what the processingManifest says
  // was applied. Missing features deduct from the score and are surfaced to the
  // grader and downstream portals so they can justify the grade.
  let featureVerification = null;
  let chapters = [];
  try {
    const manifestSummary = getManifestSummary(jobSpec);
    chapters = manifestSummary.chapters || [];

    const missingFeatures = manifestSummary.featuresNotApplied || [];
    const orderedCount   = (manifestSummary.featuresOrdered || []).length;
    const appliedCount   = (manifestSummary.featuresApplied || []).length;

    featureVerification = {
      featuresOrdered:    manifestSummary.featuresOrdered,
      featuresApplied:    manifestSummary.featuresApplied,
      featuresNotApplied: missingFeatures,
      allFeaturesDelivered: manifestSummary.allFeaturesDelivered,
      manifestPresent:    manifestSummary.manifestPresent,
      clipSegmentCount:   manifestSummary.clipSegmentCount,
      chapters,
      scoreJustification: _buildScoreJustification(sampleFindings, manifestSummary, totalScore),
    };

    // Penalise score for features that were ordered but not applied.
    // Each missing feature deducts up to 5 points (capped at 20 total deduction).
    if (missingFeatures.length > 0 && orderedCount > 0) {
      const deductPerFeature = Math.min(5, 20 / missingFeatures.length);
      const totalMissingDeduct = Math.min(20, missingFeatures.length * deductPerFeature);
      totalScore = Math.max(0, totalScore - totalMissingDeduct);
      const missingStr = missingFeatures.join(', ');
      deductions.push({ reason: `Features ordered but not applied (−${totalMissingDeduct.toFixed(0)}pts): ${missingStr}`, points: totalMissingDeduct });
      console.log(`[portal3a] ${jobId}: feature gap — ordered [${manifestSummary.featuresOrdered}], applied [${manifestSummary.featuresApplied}], missing [${missingStr}] — deducting ${totalMissingDeduct.toFixed(0)}pts`);

      // Escalate outcome if passing grade was based on an under-delivered job
      if (passed && totalScore < PASS_THRESHOLD) {
        outcome = 'pass_with_notes';
        passed  = true; // still passes — operator reviews the gap
      }
    }

    // Log chapters for operator visibility
    if (chapters.length > 0) {
      console.log(`[portal3a] ${jobId}: chapter breakdown (${chapters.length} chapters):`);
      for (const ch of chapters) {
        console.log(`  ${ch.startTimestamp} — ${ch.title} (${ch.type})`);
      }
    }
  } catch (_manifestErr) {
    // Never let manifest reading break portal delivery
  }

  const concerns = deductions.map((d) => d.reason);
  const downstreamHeadsUp =
    concerns.length > 0 ? `Portal 3a notes for gate3b: ${concerns.join('; ')}` : null;
  ...
  return {
    ...
    upstreamContext: {
      ...
      confirmedClean: passed
        ? [...upstreamContext.confirmedClean, 'assembly_qualitative']
        : upstreamContext.confirmedClean,
      escalatedConcerns:
        outcome === 'hard_fail'
...
async function prepare(jobSpec) {
  ...
  try {
    if (!GEMINI_APIKEY) {
      console.warn(
        `[gate3a] prepare() warning: GEMINI_API_KEY not set — video analysis will fail for job ${jobId}`
      );
    }
    ...
    const expectedClipCount =
      ...
      0;
    const expectedSkin = chrome?.skin || null;
    const sceneHeaders = jobSpec?.designSpec?.sceneStructure?.sceneHeaders || [];
    ...
    const itemTitles = (jobSpec?.order?.inputs?.items || [])
      ...
      .join(', ');
    const categoryLabel =
      jobSpec?.designSpec?.voice?.categoryLabel || chrome?.categoryLabel || null;

    console.log(
      `[gate3a] Ready for job ${jobId} — ${sceneHeaders.length} scenes, clip positions: [${clipScenePositions.join(',')}], skin=${expectedSkin}, format=${format}, items: ${itemTitles || 'none'}, categoryLabel: ${categoryLabel || 'n/a'}`
    );

    // ── Synth prebuild verification ───────────────────────────────────────────
    // Fingerprint = resolved customer chrome (customerConfig + content-type overrides), same inputs as FFmpeg burn.
    ...
    const threshold =
      jobSpec?.designSpec?.chrome?.synthVerifyThreshold || synthHistory.DEFAULT_THRESHOLD;
    ...
    try {
      ...
      if (frozenResolved && typeof frozenResolved === 'object') {
        ...
        const p3aSrcPlatform = jobSpec?.sourcePlatform
          || (chromeSkin === 'twitch' ? 'twitch' : null)
          || detectSourcePlatform([
              ...(jobSpec?.sourceConfig?.urls || []),
              ...(jobSpec?.order?.inputs?.sources || []),
            ]);
        resolvedChrome = resolveChromeCfg(loadCustomerConfig(customerId), ctForChrome, p3aSrcPlatform);
        chromeLayoutHash = fingerprintResolvedChromeCfg(resolvedChrome);
      }
    } catch (e) {
      console.warn(
        `[gate3a] Could not resolve chrome for synth fingerprint (${jobId}): ${e.message}`
      );
      return;
    }
    ...
    if (synthHistory.shouldSkipSynthForHash(hash, threshold)) {
      console.log(
        `[gate3a] Synth prebuild SKIPPED for job ${jobId} — resolved-chrome hash ${hash} verified ${entry.successCount}/${threshold} times (last: ${entry.lastVerified})`
      );
      ...
      try {
        ...
        console.log(
          `[gate3a] Synth preview image: ${rel} (from ${entry?.lastOutputPath ? 'last verified output' : 'placeholder — no cached synth MP4 path'})`
        );
      } catch (pe) {
    ...
    console.log(
      `[gate3a] Synth prebuild REQUIRED for job ${jobId} — resolved-chrome hash ${hash} verified ${runCount}/${threshold} times (first-run threshold not yet met)`
    );
    ...
    if (_synthInFlight.has(hash)) {
      console.log(
        `[gate3a] Synth prebuild already in-flight for hash ${hash} — waiting for result (job ${jobId})`
      );
    ...
    if (synthResult.ok) {
      ...
      console.log(
        `[gate3a] ✅ Synth prebuild PASSED for job ${jobId} — resolved-chrome hash ${hash} (${newCount}/${threshold} successes)`
      );
      if (jobSpec.designSpec) jobSpec.designSpec.synthVerified = true;
      ...
      if (newCount >= threshold) {
        console.log(
          `[gate3a] 🔓 Chrome hash ${hash} now fully verified (${newCount}/${threshold}) — future jobs will skip synth prebuild`
        );
      }
    } else {
      console.error(
        `[gate3a] ❌ Synth prebuild FAILED for job ${jobId} — resolved-chrome hash ${hash}: ${synthResult.error}`
      );
...
async function runSynthAssembly({ jobSpec, contentType, format, jobId }) {
  ...
  async function makeSynth(filename, source, size) {
    ...
    if (fs.existsSync(outPath) && fs.statSync(outPath).size >= SYNTH_CACHE_MIN_BYTES)
      return outPath;
    await new Promise((res, rej) => {
      _execFile(
        FFMPEG,
        [
          ...
          'lavfi',
          '-i',
          `${source}=duration=${SYNTH_DUR_SEC}:size=${size}:rate=30`,
          '-f',
          'lavfi',
          '-i',
          `sine=f=440:b=4:duration=${SYNTH_DUR_SEC}`,
          '-c:v',
          'libx264',
          '-preset',
          'veryfast',
  ...
  if (fs.statSync(clipPath).size < SYNTH_CACHE_MIN_BYTES) {
    return {
      ok: false,
      error: `Synth clip unexpectedly small (${fs.statSync(clipPath).size}b < ${SYNTH_CACHE_MIN_BYTES}) — check FFmpeg`,
    };
  }
...
function _synthCardData(baseContentType) {
  ...
  if (baseContentType === 'twitch' || baseContentType === 'clips') {
    return {
      title: 'Synth Streamer',
      displayName: 'Synth',
      category: 'ON STREAM',
      storyId: 'synth_0',
      origin: 'Test',
      fact: 'Prebuild verification',
    };
  }
  // news default
...
/**
 * Build a human-readable score justification explaining why the portal gave
 * the score it did — including chapter breakdown and feature delivery evidence.
 * This is surfaced in portal3a.featureVerification.scoreJustification.
 */
function _buildScoreJustification(sampleFindings, manifestSummary, rawScore) {
  const lines = [];
  lines.push(`Score: ${Math.round(rawScore)}/100`);
  lines.push('');

  // Features delivered
  const ordered = manifestSummary.featuresOrdered || [];
  const applied = manifestSummary.featuresApplied || [];
  const missing = manifestSummary.featuresNotApplied || [];
  if (ordered.length > 0) {
    lines.push('Features ordered: ' + ordered.join(', '));
    lines.push('Features applied: ' + (applied.length > 0 ? applied.join(', ') : '(none recorded)'));
    if (missing.length > 0) {
      lines.push('⚠️  Features NOT applied: ' + missing.join(', '));
    } else {
      lines.push('✅ All ordered features delivered');
    }
    lines.push('');
  }

  // Chapter breakdown
  const chapters = manifestSummary.chapters || [];
  if (chapters.length > 0) {
    lines.push(`Chapter breakdown (${chapters.length} chapters):`);
    for (const ch of chapters) {
      lines.push(`  ${ch.startTimestamp || '0:00'} — ${ch.title} [${ch.type}]`);
    }
    lines.push('');
  }

  // Clip segments
  if (manifestSummary.clipSegmentCount > 0) {
    lines.push(`Source clips used: ${manifestSummary.clipSegmentCount}`);
    if (manifestSummary.sourceUrls?.length) {
      lines.push('Sources: ' + manifestSummary.sourceUrls.map((u) => u.split('/').pop()).join(', '));
    }
    lines.push('');
  }

  // Gemini sample verdicts
  const findings = Object.entries(sampleFindings || {}).filter(([, v]) => v);
  if (findings.length > 0) {
    lines.push('Gemini QA sample verdicts:');
    for (const [label, finding] of findings) {
      const verdict = finding?.freezeDetected ? '❌ FREEZE' : finding?.score >= 70 ? '✅ pass' : '⚠️  low';
      lines.push(`  ${label}: ${verdict} (score ${finding?.score ?? 'n/a'})`);
    }
  }
