...
async function runFFmpegDefectScan(videoPath, jobId) {
  ...
  const silenceResult = await new Promise((resolve) => {
    ...
    const proc = require('child_process').spawn(
      FFMPEG,
      ['-i', videoPath, '-af', 'silencedetect=n=-50dB:d=3', '-f', 'null', '-'],
      { timeout: 120000 }
    );
  ...
  for (const m of silenceMatches) {
    ...
    defects.push({
      type: 'silence_gap',
      timestamp: tsStr,
      detail: `Silence detected starting at ${tsStr} (>${3}s) — possible audio dropout`,
    });
  }
  ...
  const freezeResult = await new Promise((resolve) => {
    ...
    const proc = require('child_process').spawn(
      FFMPEG,
      ['-i', videoPath, '-vf', 'freezedetect=n=0.001:d=2', '-f', 'null', '-'],
      { timeout: 120000 }
    );
    proc.stderr.on('data', (d) => { stderr += d.toString(); });
    proc.on('close', () => resolve(stderr));
  ...
  for (const m of freezeMatches) {
    ...
    defects.push({
      type: 'frozen_frame',
      timestamp: tsStr,
      detail: `Frozen frame detected at ${tsStr} (>2s) — static video`,
    });
  }
