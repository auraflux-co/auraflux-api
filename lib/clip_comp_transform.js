...
function buildClipCompEffectsSpec(contentType = 'twitch-short', designSpec = {}) {
  ...
  const lookName = designSpec?.compCreative?.look?.preset
    ...
    || 'auto';
  const creativeGrain = Number(designSpec?.compCreative?.look?.filmGrainStrength);
  const defaultGrain = designSpec?.compCreative?.preset === 'reaction_short' ? 8 : 7;
  const grainStrength = Math.min(10, Number.isFinite(creativeGrain) && creativeGrain > 0
    ? creativeGrain
    : parseInt(process.env.CLIP_COMP_GRAIN_STRENGTH || String(defaultGrain), 10));
