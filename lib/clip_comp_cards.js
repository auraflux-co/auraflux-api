 ...
 * Legacy fallback only — clip comps should use lib/clip_comp_hooks (Gemini).
 * Never passthrough platform clip titles.
 */
function resolveClipHookTitle(item = {}, clipEntry = {}) {
  ...
  if (streamer) {
    const fallback = stripDrawtextUnsafe(`${streamer}: unexpected moment`);
