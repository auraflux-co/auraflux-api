...
/** Strip emoji / unsupported glyphs before FFmpeg drawtext (shows ▯ boxes otherwise). */
function stripDrawtextUnsafe(s) {
  return String(s || '')
...
/**
 * Pick a hook line for burned captions — reject junk titles like "d".
 */
function resolveClipHookTitle(item = {}, clipEntry = {}) {
  const streamer = item.displayName || item.streamer || clipEntry.displayName || clipEntry.streamer || '';
  const candidates = [
    item.title, item.headline, item.clipTitle,
    clipEntry.title, clipEntry.clipTitle,
  ].map((t) => stripDrawtextUnsafe(t)).filter(Boolean);

  for (const c of candidates) {
    const words = c.split(/\s+/).filter(Boolean);
    if (c.length >= 10 && words.length >= 2 && !/^[a-z]$/i.test(c)) return c;
  }
  if (streamer) {
    const fallback = stripDrawtextUnsafe(`${streamer} — viral Twitch moment`);
