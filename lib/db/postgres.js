...
async function listJobsByCustomer(customerId, limit = 50) {
  ...
  const { rows } = await pool.query(
    `SELECT id, customer_id, status, job_spec, created_at, updated_at, scheduled_start_at
     FROM jobs WHERE customer_id = $1 ORDER BY created_at DESC LIMIT $2`,
    [customerId, limit]
  );
  return rows;
}
...
async function createTemplate(customerId, { name, description, contentType, platforms, jobSpec, recurrenceType, recurrenceDay, recurrenceTime }) {
  ...
  // CPD-445: $7/$8/$9 need explicit ::TEXT casts — these params appear in both the
  // VALUES list and in CASE WHEN expressions; without casts pg 16 deduces conflicting
  // types for $7 (recurrence_type column vs string comparison) → error 42P18.
  const { rows } = await pool.query(
    `INSERT INTO job_templates
     ...
     VALUES ($1,$2,$3,$4,$5::TEXT[],$6::JSONB,$7::TEXT,$8::TEXT,$9::TEXT,
