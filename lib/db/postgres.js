...
async function resolveCanonicalJobId(jobId) {
  ...
  const { rows } = await pool.query(
    'SELECT id, script_job_id FROM jobs WHERE id = $1',
    [jobId]
  );
...
async function jobIdsLinkedToCanonical(canonicalId) {
  ...
  const { rows } = await pool.query(
    'SELECT id FROM jobs WHERE script_job_id = $1',
    [canonicalId]
  );
...
async function loadAllJobs() {
  const pool = getPool();
  const { rows } = await pool.query(
    'SELECT card FROM jobs ORDER BY created_at DESC LIMIT 200'
  );
  return rows.map((r) => (typeof r.card === 'string' ? JSON.parse(r.card) : r.card)).filter(Boolean);
}
...
async function getJobBySpec(jobId) {
  ...
  if (!rows.length || !rows[0].job_spec) {
    ({ rows } = await pool.query(
      'SELECT job_spec FROM jobs WHERE script_job_id = $1 LIMIT 1',
      [jobId]
    ));
  }
  if (!rows.length || !rows[0].job_spec) {
...
async function markJobPublished(jobId, driveUrl) {
  const pool = getPool();
  await pool.query(
    'UPDATE jobs SET drive_url = $1, published_at = $2, status = $3 WHERE id = $4',
    [driveUrl || null, Date.now(), 'published', jobId]
  );
}
