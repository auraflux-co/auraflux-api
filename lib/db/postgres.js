...
async function updateJobSpec(jobId, jobSpec, { force = false } = {}) {
  const pool = getPool();
...
async function savePublishResult(jobId, platform, { platformJobId, driveUrl, title, status, requestId }) {
  const pool = getPool();
  const cid = await resolveCanonicalJobId(jobId);
  await pool.query(
    `INSERT INTO publish_results (job_id, platform, platform_job_id, drive_url, title, status, published_at, created_at, request_id)
 ...
 * Returns an array: [{ platform, platformJobId, driveUrl, title, status, publishedAt, requestId }]
 */
async function getPublishResults(jobId) {
  const pool = getPool();
  const cid = await resolveCanonicalJobId(jobId).catch(() => jobId);
  const { rows } = await pool.query(
    `SELECT platform, platform_job_id, drive_url, title, status, published_at, created_at, request_id
 ...
 */
const { PLAN_DEFAULTS: _ENTITLEMENT_DEFAULTS } = require('../services/plan_entitlements');
const PLAN_DEFAULTS = {
  growth:  { credits_included: _ENTITLEMENT_DEFAULTS.growth.credits_included,  overage_price_cents: _ENTITLEMENT_DEFAULTS.growth.overage_price_cents },
  operate: { credits_included: _ENTITLEMENT_DEFAULTS.operate.credits_included, overage_price_cents: _ENTITLEMENT_DEFAULTS.operate.overage_price_cents },
  guided:  { credits_included: _ENTITLEMENT_DEFAULTS.guided.credits_included,  overage_price_cents: _ENTITLEMENT_DEFAULTS.guided.overage_price_cents },
  managed: { credits_included: _ENTITLEMENT_DEFAULTS.managed.credits_included, overage_price_cents: _ENTITLEMENT_DEFAULTS.managed.overage_price_cents },
  custom:  { credits_included: _ENTITLEMENT_DEFAULTS.custom.credits_included,  overage_price_cents: _ENTITLEMENT_DEFAULTS.custom.overage_price_cents },
};

function _entitlementCols(tier) {
  const d = _ENTITLEMENT_DEFAULTS[tier] || _ENTITLEMENT_DEFAULTS.operate;
  return {
    vod_hours_included: d.vod_hours_included,
    max_channels: d.max_channels,
    queue_priority: d.queue_priority,
    export_vertical_w: d.export_vertical_w,
    export_vertical_h: d.export_vertical_h,
    export_fps: d.export_fps,
    has_account_manager: !!d.has_account_manager,
  };
}
...
async function getBrandsForAccount(accountId) {
  ...
  const { rows } = await pool.query(
    `SELECT
       ...
       b.creative_profile, b.publish_strategy_rules,
       cp.tier,
       cp.credits_included,
...
async function updateBrand(brandId, accountId, fields) {
  ...
  if (fields.creative_profile !== undefined) {
    setClauses.push(`creative_profile = $${paramIdx++}::jsonb`);
    params.push(JSON.stringify(fields.creative_profile));
  }
  if (fields.publish_strategy_rules !== undefined) {
    setClauses.push(`publish_strategy_rules = $${paramIdx++}::jsonb`);
    params.push(JSON.stringify(fields.publish_strategy_rules));
  }
  if (fields.calendar_plan !== undefined) {
    setClauses.push(`calendar_plan = $${paramIdx++}::jsonb`);
    params.push(JSON.stringify(fields.calendar_plan));
  }
...
/**
 * Ensure a client_plans row exists for a sub-brand (CPD-353 / username-only source channels).
 * Idempotent — safe to call before PATCH source_channels on new brands.
 */
async function ensureClientPlanForBrand(brandId, accountId, tier = 'operate') {
  const existing = await getClientPlanByBrand(brandId);
  if (existing) return existing;
  const pool = getPool();
  const { credits_included, overage_price_cents } = PLAN_DEFAULTS[tier] || PLAN_DEFAULTS.operate;
  const { rows } = await pool.query(
    `INSERT INTO client_plans
       (client_id, brand_id, account_id, tier, credits_included, overage_price_cents, billing_anchor_day, active, source_channels)
     VALUES ($1, $2, $3, $4, $5, $6, 1, TRUE, '{}')
     RETURNING *`,
    [`${accountId}:${brandId}`, brandId, accountId, tier, credits_included, overage_price_cents],
  );
  return rows[0];
}
...
async function getOrCreateClientPlan(clientId, tier = 'operate') {
  ...
  const { credits_included, overage_price_cents } = PLAN_DEFAULTS[t] || PLAN_DEFAULTS.operate;
  const ent = _entitlementCols(t);

  const { rows } = await pool.query(
    `INSERT INTO client_plans (
       client_id, tier, credits_included, overage_price_cents, billing_anchor_day, active,
       vod_hours_included, max_channels, queue_priority,
       export_vertical_w, export_vertical_h, export_fps, has_account_manager
     )
     ...
     ON CONFLICT (client_id) DO UPDATE
       SET tier                = EXCLUDED.tier,
           ...
           vod_hours_included  = EXCLUDED.vod_hours_included,
           max_channels        = EXCLUDED.max_channels,
           queue_priority      = EXCLUDED.queue_priority,
           export_vertical_w   = EXCLUDED.export_vertical_w,
           export_vertical_h   = EXCLUDED.export_vertical_h,
           export_fps          = EXCLUDED.export_fps,
           has_account_manager = EXCLUDED.has_account_manager,
           active              = TRUE
     RETURNING *`,
    [
      clientId, t, credits_included, overage_price_cents, 1,
      ent.vod_hours_included, ent.max_channels, ent.queue_priority,
      ent.export_vertical_w, ent.export_vertical_h, ent.export_fps, ent.has_account_manager,
    ]
  );
  return rows[0];
}
 ...
 */
async function updateClientPlanTier(clientId, tier, stripeSubscriptionId = null, stripeCustomerId = null, opts = {}) {
  const pool = getPool();
  const t = (tier || 'operate').toLowerCase();
  const { credits_included, overage_price_cents } = PLAN_DEFAULTS[t] || PLAN_DEFAULTS.operate;
  const ent = _entitlementCols(t);
  const managedAddon = opts.managedAddon;
  // Legacy full Managed tier always implies Managed add-on semantics.
  const managedVal = managedAddon == null
    ? (t === 'managed' ? true : null)  // null = leave column unchanged
    : !!managedAddon;
  const hasAm = managedVal === true ? true : ent.has_account_manager;
  const { rowCount } = await pool.query(
    `UPDATE client_plans
     SET tier = $1,
         credits_included       = $2,
         ...
         stripe_subscription_id = COALESCE($4, stripe_subscription_id),
         stripe_customer_id     = COALESCE($6, stripe_customer_id),
         vod_hours_included     = $7,
         max_channels           = $8,
         queue_priority         = $9,
         export_vertical_w      = $10,
         export_vertical_h      = $11,
         export_fps             = $12,
         has_account_manager    = CASE WHEN COALESCE($14::boolean, managed_addon) THEN TRUE ELSE $13 END,
         managed_addon          = COALESCE($14::boolean, managed_addon)
     WHERE client_id = $5`,
    [
      t, credits_included, overage_price_cents, stripeSubscriptionId || null, clientId, stripeCustomerId || null,
      ent.vod_hours_included, ent.max_channels, ent.queue_priority,
      ent.export_vertical_w, ent.export_vertical_h, ent.export_fps, hasAm,
      managedVal,
    ]
  );
  return (rowCount || 0) > 0;
...
/** Attach / detach Managed add-on without changing base plan tier. */
async function setManagedAddon(clientId, enabled, stripeSubscriptionId = null, stripeCustomerId = null) {
  const pool = getPool();
  const { rowCount } = await pool.query(
    `UPDATE client_plans
     SET managed_addon          = $2,
         has_account_manager    = CASE WHEN $2 THEN TRUE ELSE has_account_manager END,
         stripe_subscription_id = COALESCE($3, stripe_subscription_id),
         stripe_customer_id     = COALESCE($4, stripe_customer_id)
     WHERE client_id = $1`,
    [clientId, !!enabled, stripeSubscriptionId || null, stripeCustomerId || null],
  );
  return (rowCount || 0) > 0;
}
...
async function listJobsByCustomer(customerId, limit = 50, brandId = null) {
  ...
  const { rows } = await pool.query(
    `SELECT id, customer_id, brand_id, status, job_spec, created_at, updated_at, scheduled_start_at
     FROM jobs
     WHERE customer_id = $1
       AND ($3::uuid IS NULL OR brand_id = $3 OR brand_id IS NULL)
