...
async function listJobsByCustomer(customerId, limit = 50) {
  const pool = getPool();
  const { rows } = await pool.query(
    'SELECT id, customer_id, status, job_spec, created_at, updated_at FROM jobs WHERE customer_id = $1 ORDER BY created_at DESC LIMIT $2',
    [customerId, limit]
  );
  return rows;
}
...
async function loadJobRow(jobId) {
  const pool = getPool();
  const { rows } = await pool.query(
    'SELECT id, customer_id, status, job_spec, created_at, updated_at FROM jobs WHERE id = $1',
    [jobId]
  );
  return rows[0] || null;
}
...
async function createTemplate(customerId, { name, description, contentType, platforms, jobSpec, recurrenceType, recurrenceDay, recurrenceTime }) {
  const pool = getPool();
  // Precompute derived columns in JS to avoid Postgres type-inference errors when the
  // same parameter ($7) appears both as a VARCHAR column value and in a CASE comparison.
  const recurrenceActive = recurrenceType != null && recurrenceType !== 'once';
  const nextFireAt = (recurrenceType != null && recurrenceTime != null)
