...
async function upsertJobRow(jobId, jobSpec) {
  ...
  const serialized = JSON.stringify(jobSpec);
  const now = Date.now();
  // content_type is NOT NULL in the schema — pull from jobSpec or fall back to 'news'
  ...
  await pool.query(
    `INSERT INTO jobs (id, customer_id, template_id, content_type, form_type, status, stage, job_spec, created_at, updated_at)
     VALUES ($1, $2, $3, $4, $5, 'queued', 'queued', $6, $7, $7)
     ON CONFLICT (id) DO UPDATE SET
       ...
       form_type    = EXCLUDED.form_type,
       updated_at   = EXCLUDED.updated_at`,
    [jobId, jobSpec.customerId || null, jobSpec.templateId || null, contentType, formType, serialized, now]
  );
}
