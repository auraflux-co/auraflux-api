 ...
 * @returns {Promise<boolean>}
 */
async function updateBrandPlanTier(brandId, tier, stripeSubscriptionId = null) {
  const pool = getPool();
  const { credits_included, overage_price_cents } = PLAN_DEFAULTS[tier] || PLAN_DEFAULTS.operate;
  const { rowCount } = await pool.query(
    `UPDATE client_plans
     ...
     WHERE brand_id = $5`,
    [tier, credits_included, overage_price_cents, stripeSubscriptionId || null, brandId],
  );
  return (rowCount || 0) > 0;
}
  ...
  const { rowCount } = await pool.query(
    `UPDATE client_plans
     SET tier = $1,
         ...
         stripe_subscription_id = COALESCE($4, stripe_subscription_id)
     WHERE client_id = $5`,
    [tier, credits_included, overage_price_cents, stripeSubscriptionId || null, clientId]
  );
  return (rowCount || 0) > 0;
}
