...
function buildComparePrompt({ sourceDuration, shortDuration, mode = 'video' }) {
  const watchMode = mode === 'stills'
    ? `Compare labeled SOURCE vs SHORT still frames. Source ≈ ${sourceDuration}s. Short ≈ ${shortDuration}s.`
    : `You are given TWO full videos with audio:
1) SOURCE — the original clip (≈ ${sourceDuration}s). Watch it completely first.
2) SHORT — our composed 9:16 ClipzWorld Short (≈ ${shortDuration}s). Watch it completely second.
Judge motion, audio bed/SFX, beat flashes, animated text timing — not just a freeze-frame.`;

  const beatsAxis = mode === 'stills'
    ? 'music_beats: brightness flashes / beat-timed text if visible; else score 2 + "uncertain from stills"'
    : 'music_beats: score from motion + audio — beat flashes, punch/shake, bed/SFX sync; do NOT say uncertain from stills';
...
Score EVERY axis 1–5 (integer). Keep each notes field ≤20 words. Never omit an axis.
Axes:
- look_grade: color/Punch vs source
- crop_layout: 9:16 framing across the Short
...
async function uploadVideoToGemini(filePath, apiKey) {
  ...
  const mb = fileBuffer.length / (1024 * 1024);
  if (mb > 1800) {
    throw new Error(`Video too large for Gemini upload (${mb.toFixed(0)}MB) — trim first`);
  }
  const boundary = `cwn_cmp_${Date.now()}`;
...
async function waitForGeminiFileActive(file, apiKey, { maxWaitMs = 180_000 } = {}) {
  const axios = require('axios');
  const start = Date.now();
  let cur = file;
  while (Date.now() - start < maxWaitMs) {
    if (cur.state === 'ACTIVE') return cur;
    if (cur.state === 'FAILED') throw new Error(`Gemini file FAILED: ${cur.name}`);
    await new Promise((r) => setTimeout(r, 2500));
    const resp = await axios.get(
      `https://generativelanguage.googleapis.com/v1beta/${cur.name}?key=${apiKey}`,
      { timeout: 20_000 },
    );
    cur = resp.data;
  }
  throw new Error(`Gemini file stuck PROCESSING: ${file.name}`);
}

async function deleteGeminiFileQuiet(fileName, apiKey) {
...
async function callGeminiCompare(parts, { maxOutputTokens = 4096, timeoutMs = 300_000 } = {}) {
...
async function compareShortVsOriginal(opts = {}) {
  ...
  const mode = opts.mode === 'stills' ? 'stills' : 'video';
  const sourceDur = await probeDurationSec(sourcePath);
  const shortDur = await probeDurationSec(shortPath);

  if (mode === 'video') {
    try {
      return await compareViaFullVideo({
        sourcePath,
        shortPath,
        sourceDur,
        shortDur,
      ...
      console.warn(`[compare-original] video mode failed, falling back to stills: ${videoErr.message}`);
      // fall through to stills
    }
  }

  return compareViaStills(opts, { sourceDur, shortDur });
}

async function compareViaFullVideo({
  sourcePath,
  shortPath,
  sourceDur,
  shortDur,
  ...
  // Gemini ~45min with audio — Compose Review windows are usually ≤3min
  if (sourceDur > 45 * 60 || shortDur > 45 * 60) {
    throw new Error(
      `Clip too long for full-video compare (source ${Math.round(sourceDur)}s / short ${Math.round(shortDur)}s; max ~45min)`,
    );
  }

  const uploaded = [];
  try {
    console.log(`[compare-original] uploading SOURCE (${Math.round(sourceDur)}s) + SHORT (${Math.round(shortDur)}s) to Gemini…`);
    const srcFile = await uploadVideoToGemini(sourcePath, apiKey);
    uploaded.push(srcFile.name);
    await waitForGeminiFileActive(srcFile, apiKey);

    const shortFile = await uploadVideoToGemini(shortPath, apiKey);
    uploaded.push(shortFile.name);
    await waitForGeminiFileActive(shortFile, apiKey);

    const parts = [
      { text: 'SOURCE video (watch completely first — original clip with audio):' },
      { fileData: { mimeType: 'video/mp4', fileUri: srcFile.uri } },
      { text: 'SHORT video (watch completely second — our composed 9:16 Short with audio):' },
      { fileData: { mimeType: 'video/mp4', fileUri: shortFile.uri } },
      {
        text: buildComparePrompt({
          sourceDuration: Math.round(sourceDur * 10) / 10,
          shortDuration: Math.round(shortDur * 10) / 10,
          mode: 'video',
        }),
      },
    ];

    const { report, rawText, finishReason } = await callGeminiCompare(parts, {
    ...
    if (!isCompleteCompareReport(report) || String(finishReason || '').toUpperCase() === 'MAX_TOKENS') {
      ...
      throw new Error(
        `incomplete video compare (finish=${finishReason || 'n/a'}; missing=${missing.join(',') || 'scores'})`,
      );
    }
    ...
    return {
      ...
      meta: {
        ...
        shortDurationSec: Math.round(shortDur * 10) / 10,
        model: GEMINI_MODEL,
        mode: 'video',
        finishReason: finishReason || null,
      },
      rawText: includeRaw ? rawText : undefined,
    };
  } finally {
    for (const name of uploaded) {
      await deleteGeminiFileQuiet(name, apiKey);
    }
  }
}

async function compareViaStills(opts = {}, pre = {}) {
  const sourcePath = opts.sourcePath;
  const shortPath = opts.shortPath;
  ...
  try {
    ...
    for (let i = 0; i < attempts.length; i++) {
      ...
      parts.push({
        text: buildComparePrompt({
          sourceDuration: source.durationSec || pre.sourceDur,
          shortDuration: short.durationSec || pre.shortDur,
          mode: 'stills',
        }),
      });
