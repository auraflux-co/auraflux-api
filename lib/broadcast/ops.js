...
function buildOpsSnapshot({ persistedJobs, gridRunning, tvRunning, health } = {}) {
  ...
  return {
    ...
    contentBoard: (() => {
      try {
        const { buildContentBoard } = require('./content_board');
        return buildContentBoard({ persistedJobs });
      } catch (_) {
        return null;
      }
    })(),
    playbook: (() => {
      try {
        const { buildProgrammingPlaybook } = require('./programming_playbook');
        return buildProgrammingPlaybook();
      } catch (_) {
        return null;
      }
    })(),
    ...
    tokens: (() => {
      try {
        const { syncTokenHealth } = require('./token_health');
        return syncTokenHealth();
      } catch (_) {
        return null;
      }
    })(),
