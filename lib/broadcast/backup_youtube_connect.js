...
/** @returns {{ render: boolean, doppler: boolean, local: boolean }} */
async function persistBackupRefreshToken(refreshToken) {
  const out = { render: false, doppler: false, local: false };
  ...
  if (!out.render) {
    process.env.YOUTUBE_BACKUP_REFRESH_TOKEN = refreshToken;
    out.local = true;
  }
  try {
