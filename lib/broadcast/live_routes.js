...
function registerLiveBroadcastRoutes(app, state = {}) {
  const { requireBroadcastOperator } = require('./operator_auth');
  app.use((req, res, next) => {
    if (req.method === 'GET' || req.method === 'HEAD' || req.method === 'OPTIONS') return next();
    return requireBroadcastOperator(req, res, next);
  });
  ...
  app.get('/live-grid/youtube-listing', async (req, res) => {
    try {
      ...
      const { redactListing } = require('./listing_redact');
      const listing = redactListing(readYoutubeListing());
  ...
  app.post('/live-grid/start', async (req, res) => {
    try {
      ...
      res.json({
        ok: true,
        status,
        youtubeGoLiveWait: status.youtubeGoLiveWait || mgr._youtubeGoLiveWait || null,
      });
    } catch (e) {
  ...
  app.post('/live-grid/stop', async (req, res) => {
    ...
    if (stopOpts.endBroadcast) {
      try {
        const { clearResume } = require('../live_grid/resume_state');
        const { clearWasLive } = require('../live_grid/was_live_env');
        clearResume();
        clearWasLive();
      } catch (_) {}
    }
  ...
  app.get('/live-grid/solo-listings', (req, res) => {
    try {
      ...
      const { redactSoloListings } = require('./listing_redact');
      res.json({
        ok: true,
        enabled: soloStreamsEnabled(),
        configured: soloStreamsConfigured(),
        listings: redactSoloListings(readSoloListings()),
      });
    } catch (e) {
      res.status(500).json({ ok: false, error: e.message });
    }
  ...
  app.post('/live-grid/solo-listings', (req, res) => {
    try {
      ...
      const { redactListing } = require('./listing_redact');
      const { fleetPoolSize } = require('../live_grid/fleet_pool');
      const maxSlot = fleetPoolSize();
      const { quadrant, broadcastId, watchUrl, rtmpUrl, streamId, label } = req.body || {};
      const q = Number(quadrant);
      if (!Number.isInteger(q) || q < 1 || q > maxSlot) {
        return res.status(400).json({ ok: false, error: `quadrant must be 1-${maxSlot}` });
      }
      if (broadcastId == null) {
        return res.status(400).json({ ok: false, error: 'broadcastId required (empty string clears)' });
      }
      const listing = persistSoloListing(q - 1, { broadcastId, watchUrl, rtmpUrl, streamId, label });
      res.json({ ok: true, listing: redactListing(readSoloListingForQuadrant(q - 1)), saved: redactListing(listing) });
    } catch (e) {
      res.status(400).json({ ok: false, error: e.message });
    }
  });

  app.get('/live-grid/solo-bindings', (req, res) => {
    try {
      const { streamerLockEnabled, activeBindings, loadRegistry } = require('../live_grid/solo_streamer_registry');
      const { readSoloListings } = require('../live_grid/solo_listings_env');
      const { redactSoloListings } = require('./listing_redact');
      const mgr = getGrid();
      res.json({
        ok: true,
        enabled: streamerLockEnabled(),
        pool: redactSoloListings(readSoloListings()),
        bindings: activeBindings(),
        registryPath: require('../live_grid/solo_streamer_registry').registryPath(),
        assignments: mgr?._lastAssignments || null,
      });
    } catch (e) {
      res.status(500).json({ ok: false, error: e.message });
    }
  ...
  app.post('/live-grid/solo-bindings', async (req, res) => {
    const mgr = getGrid();
    if (!mgr?.running) return res.status(400).json({ ok: false, error: 'Live grid not running' });
    try {
      const result = await mgr.applySoloBindingMap(req.body || {});
      res.json(result);
    } catch (e) {
      res.status(400).json({ ok: false, error: e.message });
    }
  ...
  app.post('/live-grid/solo-go', async (req, res) => {
    const mgr = getGrid();
    if (!mgr?.running) return res.status(400).json({ ok: false, error: 'Live grid not running' });
    try {
      const { createBroadcasts } = req.body || {};
      const result = await mgr.startSoloStreams({ allowCreateBroadcasts: createBroadcasts === true });
      res.json(result);
    } catch (e) {
      res.status(400).json({ ok: false, error: e.message });
    }
  ...
  app.get('/live-grid/encode-contract', (req, res) => {
    const mgr = getGrid();
    if (!mgr?.running) return res.status(400).json({ ok: false, error: 'Live grid not running' });
    try {
      const { BENCHMARK_STEPS } = require('../live_grid/encode_contract');
      res.json({ ok: true, steps: BENCHMARK_STEPS, contract: mgr.getEncodeContract() });
    } catch (e) {
      res.status(500).json({ ok: false, error: e.message });
    }
  });

  app.post('/live-grid/benchmark/apply-step', async (req, res) => {
    const mgr = getGrid();
    if (!mgr?.running) return res.status(400).json({ ok: false, error: 'Live grid not running' });
    try {
      const step = req.body?.step;
      if (step == null) return res.status(400).json({ ok: false, error: 'step required (0-4)' });
      const result = await mgr.applyBenchmarkStep(Number(step));
      res.json(result);
    } catch (e) {
      res.status(400).json({ ok: false, error: e.message });
    }
  ...
  app.post('/live-grid/solo-focus', async (req, res) => {
    const mgr = getGrid();
    if (!mgr?.running) return res.status(400).json({ ok: false, error: 'Live grid not running' });
    try {
      const {
      ...
      if (quadrant == null) {
        return res.status(400).json({ ok: false, error: 'quadrant required (1-4)' });
      }
      const result = await mgr.focusSoloQuadrant(Number(quadrant), {
        broadcastId,
        bitrateK,
        maxrateK,
        bufsizeK,
  ...
  app.post('/live-grid/solo-unfocus', async (req, res) => {
    const mgr = getGrid();
    if (!mgr?.running) return res.status(400).json({ ok: false, error: 'Live grid not running' });
    try {
      const result = await mgr.clearSoloFocusMode();
      res.json(result);
    } catch (e) {
      res.status(400).json({ ok: false, error: e.message });
    }
  ...
  app.post('/live-grid/refresh-all-seo', async (req, res) => {
    const mgr = getGrid();
    if (!mgr?.running) return res.status(400).json({ ok: false, error: 'Live grid not running' });
    try {
      const {
      ...
      const result = await mgr.refreshAllYoutubeSeo(reason || 'operator', {
        thumbnail: thumbnail === true,
        mainBroadcastId,
        soloBroadcastIds,
        setPublic: setPublic !== false,
        discover: discover === true,
        persistDiscovered: persistDiscovered === true,
      });
      res.json(result);
    } catch (e) {
      res.status(500).json({ ok: false, error: e.message });
  ...
  app.get('/live-grid/discover-broadcasts', async (req, res) => {
    const mgr = getGrid();
    if (!mgr?.running) return res.status(400).json({ ok: false, error: 'Live grid not running' });
    try {
      const result = await mgr.discoverYoutubeBroadcasts();
      res.json(result);
    } catch (e) {
      res.status(500).json({ ok: false, error: e.message });
    }
  ...
  app.post('/live-grid/sync-solo-listings', async (req, res) => {
    const mgr = getGrid();
    if (!mgr?.running) return res.status(400).json({ ok: false, error: 'Live grid not running' });
    try {
      const { quadrants, discover, persistDiscovered, dryRun, mode } = req.body || {};
      const result = await mgr.syncSoloListingsFromGrid({
        quadrants: Array.isArray(quadrants) ? quadrants : undefined,
        discover: discover !== false,
        persistDiscovered: persistDiscovered !== false,
        dryRun: dryRun === true,
        mode: mode === 'lite' ? 'lite' : 'discoverable',
      });
      res.json({ ok: true, ...result });
    } catch (e) {
      res.status(500).json({ ok: false, error: e.message });
  ...
  app.post('/live-grid/refresh-solo-seo', async (req, res) => {
    const mgr = getGrid();
    if (!mgr?.running) return res.status(400).json({ ok: false, error: 'Live grid not running' });
    try {
      const { soloBroadcastIds, setPublic } = req.body || {};
      const result = await mgr.refreshSoloYoutubeSeo({
        soloBroadcastIds,
        setPublic: setPublic !== false,
      });
      res.json({ ok: true, ...result });
  ...
  app.post('/live-grid/apply-encode-profile', async (req, res) => {
    const mgr = getGrid();
    if (!mgr?.running) return res.status(400).json({ ok: false, error: 'Live grid not running' });
    try {
      const result = await mgr.applyEncodeProfile(req.body || {});
      res.json(result);
    } catch (e) {
      res.status(400).json({ ok: false, error: e.message });
    }
  ...
  /** Stop all live fleet slots; roster must mark fleetPaused or slot paused (CPD-1077). */
  app.post('/live-grid/fleet/pause-all', async (req, res) => {
    const mgr = getGrid();
    if (!mgr?.running || !mgr.fleetOrchestrator) {
      return res.json({ ok: true, message: 'Fleet not running', stopped: [] });
    }
    try {
      const { localFleetSlots, currentFleetId, isFleetPaused, fleetPausedReason } = require('../live_grid/solo_roster_fleet');
      if (!isFleetPaused()) {
        return res.status(400).json({
          ok: false,
          error: 'Set fleetPaused:true in config/solo_roster_fleet.json before pause-all (prevents auto-restart on tick)',
        });
      }
      const fleetId = mgr.fleetOrchestrator.fleetId || currentFleetId();
      const slots = localFleetSlots(fleetId);
      const stopped = [];
      for (const slotDef of slots) {
        const st = mgr.fleetOrchestrator._slotState?.get(slotDef.localIndex);
        if (st?.phase === 'live' || st?.phase === 'starting') {
          await mgr.fleetOrchestrator.stopSlot(slotDef, 'operator_pause');
          stopped.push({ slot: slotDef.slot, login: slotDef.login });
        }
      }
      await mgr.fleetOrchestrator.tick?.();
      res.json({
        ok: true,
        message: stopped.length
          ? `Stopped ${stopped.length} slot(s) — ${fleetPausedReason()}`
          : `Fleet idle — ${fleetPausedReason()}`,
        stopped,
        fleet: mgr.fleetOrchestrator.status(),
      });
    } catch (e) {
      res.status(500).json({ ok: false, error: e.message });
    }
  ...
  /** Stop live Kick fleet slots; roster must mark them paused (CPD-1067). */
  app.post('/live-grid/fleet/kick-pause', async (req, res) => {
    const mgr = getGrid();
    if (!mgr?.running || !mgr.fleetOrchestrator) {
      return res.status(400).json({ ok: false, error: 'Fleet not running' });
    }
    try {
      const { localFleetSlots, currentFleetId, isSlotPaused } = require('../live_grid/solo_roster_fleet');
      const fleetId = mgr.fleetOrchestrator.fleetId || currentFleetId();
      const kickSlots = localFleetSlots(fleetId).filter((s) => s.platform === 'kick');
      const stopped = [];
      for (const slotDef of kickSlots) {
        if (!isSlotPaused(slotDef)) {
          return res.status(400).json({
            ok: false,
            error: `Slot ${slotDef.slot} (@${slotDef.login}) not marked paused in roster config`,
          });
        }
        const st = mgr.fleetOrchestrator._slotState?.get(slotDef.localIndex);
        if (st?.phase === 'live' || st?.phase === 'starting') {
          await mgr.fleetOrchestrator.stopSlot(slotDef, 'kick_paused');
          stopped.push({ slot: slotDef.slot, login: slotDef.login });
        }
      }
      await mgr.fleetOrchestrator.tick?.();
      res.json({
        ok: true,
        message: stopped.length
          ? `Stopped ${stopped.length} Kick slot(s); roster pause active`
          : 'Kick slots already idle; roster pause active',
        stopped,
        fleet: mgr.fleetOrchestrator.status(),
      });
    } catch (e) {
  ...
  /**
   * Post-deploy roster sync — re-read config bindings, clear idle slot broadcast pins,
   * update pool labels. Skips slots still live/starting under the OLD login (operator
   * must wait for logoff). CPD-1068.
   */
  app.post('/live-grid/fleet/roster-refresh', async (req, res) => {
    const mgr = getGrid();
    if (!mgr?.running || !mgr.fleetOrchestrator) {
      return res.status(400).json({ ok: false, error: 'Fleet not running' });
    }
    try {
      const {
        ...
        currentFleetId,
      } = require('../live_grid/solo_roster_fleet');
      const { applyLoginSlotMap } = require('../live_grid/solo_streamer_registry');
      const { readSoloListingForPoolSlot, persistSoloListing } = require('../live_grid/solo_listings_env');

      const fleetId = mgr.fleetOrchestrator.fleetId || currentFleetId();
      const slots = localFleetSlots(fleetId);
      const bindings = loginSlotMapForBindings(fleetId);
      const skipped = [];
      const cleared = [];

      for (const slotDef of slots) {
        const q = slotDef.localIndex;
        const st = mgr.fleetOrchestrator._slotState.get(q) || { phase: 'idle' };
        if ((st.phase === 'live' || st.phase === 'starting') && st.login && st.login !== slotDef.login) {
          skipped.push({
            slot: slotDef.slot,
            phase: st.phase,
            oldLogin: st.login,
            newLogin: slotDef.login,
            reason: 'wait_for_logoff',
          });
          continue;
        }
        mgr.fleetOrchestrator._slotState.set(q, { phase: 'idle', login: slotDef.login });
        cleared.push({ slot: slotDef.slot, login: slotDef.login, localPool: slotDef.localPool });
        mgr.feeders?.setQuadrant(q, null);

        const listing = readSoloListingForPoolSlot(slotDef.localPool);
        if (listing?.broadcastId && listing?.rtmpUrl) {
          persistSoloListing(slotDef.localPool - 1, {
      ...
      applyLoginSlotMap(bindings, slots.map((s) => s.login));
      mgr.fleetOrchestrator._saveState();
      await mgr.fleetOrchestrator.tick();
      ...
      res.json({
        ...
        message: skipped.length
          ? `${cleared.length} slot(s) refreshed; ${skipped.length} skipped (old streamer still live — wait for logoff)`
          : `Roster refreshed for ${cleared.length} slot(s)`,
      });
    } catch (e) {
      res.status(500).json({ ok: false, error: e.message });
    }
  ...
  /** Fleet slot health scores — viewer experience + Kick ingest (CPD-1070). */
  app.get('/live-grid/fleet/health', (req, res) => {
    const mgr = getGrid();
    if (!mgr) {
      return res.json({ ok: true, running: false, sidecar: true, health: null });
    }
    try {
      const { buildFleetHealth } = require('../live_grid/fleet_health');
      const fleetId = mgr.fleetOrchestrator?.fleetId || process.env.LIVE_GRID_FLEET_ID || 'a';
      const health = buildFleetHealth(mgr.status(), fleetId);
      res.json({ ok: true, running: !!mgr.running, health });
    } catch (e) {
      res.status(500).json({ ok: false, error: e.message });
    }
  });
  ...
  app.get('/live-grid/kick/resolve', async (req, res) => {
    const slug = String(req.query.slug || req.query.login || '').trim().toLowerCase();
    if (!slug) return res.status(400).json({ ok: false, error: 'slug or login query required' });
    try {
      const {
      ...
      } = require('../clients/kick_live_resolver');
      const oauthCtx = await loadKickOAuthContext();
      const proxyPwd = await resolveApifyProxyPassword();
      let kickApiError = null;
      let kickApiPlayback = false;
      try {
        const data = await fetchKickChannelApi(slug);
        kickApiPlayback = !!data?.playback_url;
      } catch (e) {
        kickApiError = e.message;
      }
      const resolved = await resolveKickLivePlayback(slug);
      const { kickStreamlinkIngestEnabled } = require('../live_grid/kick_config');
      res.json({
        ok: true,
        slug,
        kickIngest: kickStreamlinkIngestEnabled() ? 'streamlink' : 'hls',
        oauthCustomerId: resolveKickOAuthCustomerId(),
        oauthConnected: !!oauthCtx?.tokens?.accessToken,
        proxyConfigured: !!(process.env.KICK_PROXY_URL || proxyPwd),
        proxySource: process.env.KICK_PROXY_URL
          ? 'KICK_PROXY_URL'
          : proxyPwd
            ? (process.env.APIFY_PROXY_PASSWORD ? 'APIFY_PROXY_PASSWORD' : 'apify_api')
            : 'none',
        kickApiPlayback,
        kickApiError,
        resolved,
  ...
  app.post('/live-grid/quadrant/:n/kick', async (req, res) => {
    const mgr = getGrid();
    if (!mgr?.running) return res.status(400).json({ ok: false, error: 'Live grid not running' });
    const q = Number(req.params.n) - 1;
    const { login, slug, label, title, lockWhenOffline } = req.body || {};
    const kickSlug = login || slug;
    if (!kickSlug) return res.status(400).json({ ok: false, error: 'login or slug required' });
    try {
      const quadrant = await mgr.setQuadrantKick(q, kickSlug, { label, title, lockWhenOffline });
      const offline = quadrant?.offline === true;
      const { saveResumeFromManager } = require('../live_grid/resume_state');
      saveResumeFromManager(mgr);
      res.json({ ok: true, quadrant, locked: true, platform: 'kick', offline });
    } catch (e) {
      res.status(400).json({ ok: false, error: e.message });
    }
  });

  app.post('/live-grid/quadrant/:n/url', async (req, res) => {
    const mgr = getGrid();
    if (!mgr?.running) return res.status(400).json({ ok: false, error: 'Live grid not running' });
    const q = Number(req.params.n) - 1;
    ...
    try {
      const { isKickPageUrl, kickSlugFromUrl } = require('../clients/kick_live_resolver');
      if (isKickPageUrl(url)) {
        const kickSlug = login || kickSlugFromUrl(url);
        const quadrant = await mgr.setQuadrantKick(q, kickSlug, { label, title });
        return res.json({ ok: true, quadrant, locked: true, platform: 'kick', resolved: true });
      }
      const quadrant = mgr.setQuadrantUrl(q, url, label, {
        title,
        login,
        kickSlug: req.body?.kickSlug || (login && isKickPageUrl(url) ? login : undefined),
        kickPageUrl: req.body?.kickPageUrl,
      });
...
async function autoResumeLiveGrid(state = {}) {
  ...
  try {
    ...
    if (saved.source === 'env_fallback') {
      console.log('[broadcast-sidecar] auto-resume from env fallback (LIVE_GRID_WAS_LIVE)');
    }
    await mgr.start(startOpts);
    mgr.feeders?.syncNameFiles();
    await mgr.poller?.pollOnce().catch(() => {});
    await mgr._reconcileFeedersFromPoller?.('auto_resume').catch(() => {});
    if (saved.runtime?.soloFocus) {
      await mgr.applySoloFocusFromResume(saved.runtime.soloFocus).catch((e) => {
        console.warn(`[broadcast-sidecar] hero focus restore failed: ${e.message}`);
      });
    }
    saveResumeFromManager(mgr);
    console.log(`[broadcast-sidecar] live grid auto-resumed → ${mgr.broadcast?.watchUrl || 'RTMP'}`);
    return { resumed: true, watchUrl: mgr.broadcast?.watchUrl || null, source: saved.source || 'file' };
  } catch (e) {
