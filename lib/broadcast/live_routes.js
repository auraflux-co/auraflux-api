...
function resolveLiveGridStopOpts(body = {}) {
  ...
  return { skipEndBroadcast: !endBroadcast && gridUsesRtmpBypass(), endBroadcast };
}
...
function registerLiveBroadcastRoutes(app, state = {}) {
  ...
  app.post('/live-grid/start', async (req, res) => {
    try {
      ...
      try {
        const { saveResumeFromManager, applyResumeRuntime } = require('../live_grid/resume_state');
        if (req.body?._resumeRuntime) applyResumeRuntime(mgr, req.body._resumeRuntime);
...
async function autoResumeLiveGrid(state = {}) {
  ...
  try {
    ...
    applyResumeRuntime(mgr, saved.runtime);
