...
async function readLocalFeedReport() {
  ...
  const hlsLive = hlsPreviewLive();
  const hlsReady = gridRunning && hlsLive;
  const hlsStale = gridRunning && hlsPreviewReady() && !hlsLive;
  ...
  return {
    ...
    composed: {
      ...
      note: hlsReady
        ...
        : hlsStale
          ? 'HLS playlist is stale — master is not teeing to preview. Restart grid or run master-refresh.'
