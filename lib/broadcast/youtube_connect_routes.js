...
function registerYoutubeConnectRoutes(app) {
  ...
  app.get('/connect/youtube', requireBroadcastOperatorPage, (req, res) => {
    ...
    const state = operatorAuthRequired() ? createOAuthState() : 'c0';
    res.redirect(ytDirect.buildAuthUrl(ytRedirectUri(req), state));
  });

  app.get('/connect/youtube/callback', requireOAuthCallbackState, async (req, res) => {
  ...
  app.get('/connect/youtube/backup', requireBroadcastOperatorPage, (req, res) => {
    const ytDirect = require('../services/youtube_direct');
    ...
    const state = operatorAuthRequired() ? createOAuthState() : 'backup';
    res.redirect(ytDirect.buildAuthUrlForProfile('backup', ytBackupRedirectUri(req), state));
  });

  app.get('/connect/youtube/backup/callback', requireOAuthCallbackState, async (req, res) => {
    const { code, error } = req.query;
    if (error) return res.status(400).send(`Google OAuth error: ${error}`);
    if (!code) return res.status(400).send('Missing authorization code');
    try {
      ...
      if (saved.local) notes.push('Token set in sidecar process env — run sync script to persist to Render/Doppler');
      if (!saved.render && !saved.doppler && !saved.local) {
