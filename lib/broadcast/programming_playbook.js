...
const { buildViewBandInsights } = require('./view_band_insights');
...
function wiringStatus(formatId, catalog) {
  ...
  switch (formatId) {
    ...
    case 'gaming_showcase':
      return { status: 'partial', detail: 'Event feed picker + URL feeder wired — Q0 pulls allowlisted live feeds at 6pm' };
...
function buildProgrammingPlaybook() {
  ...
  const principles = [
    ...
    {
      title: 'Feed vs Produced vs Watch party',
      body: 'Feed = raw match/court/chase/wire. Produced = named show or host layer. Watch party = branded wrapper around someone else\'s feed. ClipzWorld Tier A targets produced + commentary — not raw feeds on Twitch TV.',
    },
    {
  ...
  return {
    ...
    tiers,
    principles,
    viewBands: buildViewBandInsights(),
