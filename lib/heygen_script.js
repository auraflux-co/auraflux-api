...
function prepareHeyGenScript(raw, opts = {}) {
  ...
  text = text
    ...
    .replace(/\s{2,}/g, ' ')
    .trim();
  // CPD-1223 r25 QA: streamer intro scenes open a stitch group — takes that start
  // speaking on frame 1 (mouth open, mid-gesture) make the hard cut from the previous
  // reaction read as a visual jerk. A leading break makes HeyGen render a neutral
  // settle beat before speech, matching the approved Cinna/Emily handoffs.
  const { isStreamerIntroScene } = require('./heygen_spoken_sanitize');
  if (isStreamerIntroScene(opts.sceneName) && !/^<break\s/i.test(text)) {
