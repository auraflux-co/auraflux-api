...
function syntheticNewsScenario() {
  const sceneHeaders = [
    'INTRO',
    'STORY1_INTRO', 'STORY1_CLIP', 'STORY1_REACT',
    'STORY2_INTRO', 'STORY2_CLIP', 'STORY2_REACT',
    'OUTRO'
  ];
  const jobSpec = {
    ...
    designSpec: {
      ...
      voice: {
        ...
        lockedIntro: "I'm Bobby G, and this is Because the Light was on. I'm told this is the news.",
...
function syntheticNbaScenario() {
  const sceneHeaders = [
    'INTRO',
    'GAME1_INTRO', 'GAME1_NARRATION', 'GAME1_RECAP', 'GAME1_REACTION',
    'OUTRO'
  ];
  const jobSpec = {
    ...
    order: {
      ...
      inputs: {
        items: [
          {
            away: 'LAL', home: 'BOS',
            awayScore: 102, homeScore: 99,
            awayAbbr: 'LAL', homeAbbr: 'BOS',
    ...
    designSpec: {
      ...
      voice: {
        ...
        lockedIntro: "What's up, ClipzWorld! Grab your shades, because we're heading to the Other Side of the Pillow.",
        lockedOutro: 'Goodnight and good luck.'
      },
      sceneStructure: {
        sceneHeaders,
  ...
  const gate0 = {
    ...
    clipAnalyses: [
      { analysis: 'Fast break dunk, crowd noise, score shown 24-22 first quarter.' }
    ],
    upstreamContext: { confirmedClean: [], escalatedConcerns: [] }
  };
  const gate1Report = { passed: true, score: 89 };
  const gate2Report = { passed: true, score: 90, upstreamContext: { downstreamHeadsUp: 'Watch sidebar titles on recap beat' } };
  return { jobSpec, script, gate0, gate1Report, gate2Report };
}
...
function runOfflinePromptReplay() {
  ...
  const g3CtxNews = gate3aPromptCtxFrom(news.jobSpec, news.gate0, news.gate1Report, news.gate2Report);
  const newsRow = {
    name: 'news_longform',
    gate1: {
      gate: 'gate1',
...
function writePromptReplayReport(report, outDir) {
  ...
  for (const s of report.scenarios) {
    ...
    lines.push('', '**Gate 1 preview (first 500 chars)**', '', '```', s.gate1.promptPreviewFirst500, '```', '');
    for (const [k, v] of Object.entries(s.gemini)) {
