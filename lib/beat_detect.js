...
function suggestionsFromPeaks(peaks, {
  ...
  // Gemini Core_fx gap: brief red impact tint on peaks (CPD-1283).
  const flashes = list.map((p) => ({
    atSec: p.atSec,
    duration: 0.28,
    strength: Math.min(0.35, 0.16 + (p.score || 1) * 0.04),
  }));
...
async function analyzeBeatsOnMusicBed(musicBedKey, opts = {}) {
  ...
  const result = await analyzeBeatsOnFile(bedPath, {
    ...opts,
    maxSec: opts.maxSec != null ? opts.maxSec : 45,
    sourceLabel: `music_bed:${path.basename(bedPath)}`,
  });
  return { ...result, bedPath, musicBed: musicBedKey };
}
