...
async function renderCompositionPreview({
  ...
  try {
    ...
    const previewDur = previewDurationSec(clip);
    const layoutSegments = Array.isArray(clip.layoutSegments) ? clip.layoutSegments : [];
    const hasTimedLayout = layoutSegments.length > 0;
    ...
    rawPath = await applySourceCleanup(rawPath, cleanPath, { compCreative, log });
    ...
    if (hasTimedLayout) {
      log(`[composition-preview] timed layout: ${layoutSegments.length} breakpoint(s), ${previewDur}s window`);
      await applyPortraitLayoutTimed(rawPath, layoutPath, {
        compCreative,
        trimStart,
        trimEnd: Math.min(trimEnd, trimStart + previewDur),
        layoutSegments,
        openingLayout: clip.openingLayout || null,
        deliveryAspect,
        sourceFilePreTrimmed: true,
        log: (m) => log(m),
      });
      effectiveMode = layoutSegments[layoutSegments.length - 1]?.mode || compCreative?.layout?.mode;
    } else {
    ...
    await extractJpegFrame(logoPath, outJpeg, jpegAt);
