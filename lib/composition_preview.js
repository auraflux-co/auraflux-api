...
async function downscalePreviewSource(inputPath, outputPath, log) {
  ...
  log('[composition-preview] downscaled to 960w @ 30fps for fast preview');
  return outputPath;
}
...
async function scalePreviewMp4(inputPath, outputPath, deliveryAspect = '9:16', maxWidth = 540) {
...
async function renderCompositionPreview({
  ...
  try {
    ...
    try {
      await scalePreviewMp4(logoPath, outMp4, deliveryAspect, fullTrimWindow ? 720 : 540);
