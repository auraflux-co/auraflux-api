...
const LOGO_PATH = (() => {
  try {
    const { AURAFLUX_WATERMARK_PATH, CWN_LOGO_PATH } = require('./services/branding_assets');
    return AURAFLUX_WATERMARK_PATH || CWN_LOGO_PATH;
  } catch (_) {
    return path.join(__dirname, '../assets/brand_logo.png');
  }
})();
...
async function applyNearFinalPreviewStack(inputPath, tmpDir, {
  ...
  try {
    const { applyClipCompTransform } = require('./apply_clip_comp_transform');
...
/**
 * C11 Then/Now Review — assemble both clips via dual_source_stack (not clips[0] only).
 * Live dual panes are a guide; near-final must be the stacked hold→switch MP4.
 */
async function renderDualSourceTimelinePreview({
  clips,
  compCreativePreset = 'dual_source_stack',
  compCreativeOverrides = {},
  deliveryAspect = '9:16',
  twitchClient = null,
  log = console.log,
}) {
  return runTimelinePreviewExclusive(async () => {
    const { isDualSourceStackMode, assembleDualSourceStack } = require('./clip_comp_dual_source');
    if (!Array.isArray(clips) || clips.length < 2) {
      throw new Error('dual_source_stack Review needs exactly 2 clips (TOP + BOTTOM)');
    }
    const clipA = clips[0];
    const clipB = clips[1];
    const compCreative = mergeCompCreative({
      preset: compCreativePreset || 'dual_source_stack',
      overrides: { ...compCreativeOverrides, previewFast: true },
      streamerHint: clipA?.streamer || clipA?.displayName || null,
    });
    if (!isDualSourceStackMode(compCreative)) {
      throw new Error('renderDualSourceTimelinePreview called without dual_source_stack creative');
    }

    fs.mkdirSync(PREVIEW_DIR, { recursive: true });
    const jobId = `tlprev_dual_${Date.now()}`;
    const outJpeg = path.join(PREVIEW_DIR, `${jobId}.jpg`);
    const outMp4 = path.join(PREVIEW_DIR, `${jobId}.mp4`);
    const workDir = fs.mkdtempSync(path.join(os.tmpdir(), 'comp-dual-prev-'));
    const tmpDirs = [workDir];

    try {
      const resolveOne = async (clip, tag) => {
        const mp4Url = clip.resolvedMp4 || await resolveClipMp4(clip, twitchClient);
        const localClipPath = clip.localClipPath || await findLocalAssemblyClipPath(clip);
        const previewDur = timelinePreviewDurationSec(clip);
        const snippet = await downloadClipSnippet({
          mp4Url,
          pageUrl: clip.pageUrl || clip.url,
          trimStart: clip.trimStart,
        ...
        tmpDirs.push(snippet.tmpDir);
        const scaledPath = path.join(snippet.tmpDir, `scaled_${tag}.mp4`);
        const rawPath = await downscalePreviewSource(snippet.rawPath, scaledPath, log);
        // Snippet is already trimmed — assembleDualSourceStack reads 0..dur.
        return {
          path: rawPath,
          meta: {
            ...clip,
            trimStart: 0,
            trimEnd: previewDur,
            title: clip.title || tag,
          },
          previewDur,
        };
      };

      log('[composition-preview] dual_source_stack Review — resolving TOP + BOTTOM');
      const top = await resolveOne(clipA, 'top');
      const bottom = await resolveOne(clipB, 'bottom');

      const dual = await assembleDualSourceStack({
        clipFiles: [top.path, bottom.path],
        clipMetas: [top.meta, bottom.meta],
        compCreative,
        asmId: jobId,
        tmpDir: workDir,
        log: (m) => log(m),
      ...
      let gradedPath = dual.outputPath;
      try {
        const logoPath = path.join(workDir, `${jobId}_logo.mp4`);
        await applyLogoOverlay(dual.outputPath, logoPath, compCreative);
        if (fs.existsSync(logoPath) && fs.statSync(logoPath).size > 500) gradedPath = logoPath;
      } catch (logoErr) {
        log(`[composition-preview] dual logo skipped: ${logoErr.message}`);
      }

      const jpegAt = Math.min(1.2, Math.max(0.4, (dual.totalDur || 2) * 0.08));
      await extractJpegFrame(gradedPath, outJpeg, jpegAt);
      let previewVideoUrl = null;
      let previewVideoError = null;
      try {
        await scalePreviewMp4(gradedPath, outMp4, deliveryAspect, 1080, { allowCopy: true });
        previewVideoUrl = `/composition/preview/file/${path.basename(outMp4)}`;
      } catch (vidErr) {
        previewVideoError = vidErr.message;
        log(`[composition-preview] dual video encode skipped: ${vidErr.message}`);
      }

      const base64 = fs.readFileSync(outJpeg).toString('base64');
      const totalDur = Number(dual.totalDur) || (top.previewDur + bottom.previewDur);
      return {
        ok: true,
        previewPath: outJpeg,
        previewUrl: `/composition/preview/file/${path.basename(outJpeg)}`,
        ...
        previewVideoError,
        previewVideoMime: previewVideoUrl ? 'video/mp4' : null,
        previewDurationSec: totalDur,
        fullTrimWindow: true,
        nearFinalPreview: true,
        nearFinalApplied: ['dual_source_stack'],
        nearFinalMissing: ['whisper_captions', 'beats_fx'],
        trimStart: 0,
        trimEnd: totalDur,
        dualSourceStack: {
          ...
          hasBottomAudio: dual.hasBottomAudio,
        },
        timedLayoutPreview: false,
        base64,
        mimeType: 'image/jpeg',
        mode: 'dual_source_vstack',
        requestedMode: 'dual_source_vstack',
        landscapeAutoSplit: false,
        layoutSummary: {
          mode: 'dual_source_vstack',
          nearFinal: true,
          nearFinalApplied: ['dual_source_stack'],
      ...
      for (const d of tmpDirs) {
        try { fs.rmSync(d, { recursive: true, force: true }); } catch (_) { /* noop */ }
      }
    }
  });
}
...
module.exports = {
  ...
  renderDualSourceTimelinePreview,
