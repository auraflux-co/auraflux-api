...
function getManualDir(jobId) {
  const root = process.env.C0_MANUAL_SEGMENTS_ROOT
    ? path.resolve(process.env.C0_MANUAL_SEGMENTS_ROOT)
    : MANUAL_ROOT;
  return path.join(root, String(jobId || 'unknown'));
}
...
function shouldUseManualCheckpoint(card) {
  ...
  const semanticId = String(card?.jobSpecId || card?.specId || '');
...
function useC0ImmediateManualHold(card) {
  ...
  // Sim renders write local MP4s — no operator drop folder needed.
  if (process.env.HEYGEN_SIM_MODE === 'true') return false;
...
function applyManualOverrides(jobId, segmentData, options = {}) {
  const {
    partialSceneLabels = null,
    searchDir = null,
  } = options;
  const allowed = partialSceneLabels?.length
    ? new Set(partialSceneLabels.map((l) => String(l || '').toUpperCase().trim()))
    : null;
  const dir = searchDir || getManualDir(jobId);
  if (!fs.existsSync(dir)) return { segmentData, overrideCount: 0, dir, partial: !!allowed };
  ...
  const patched = (segmentData || []).map((seg, i) => {
    const segLabelNorm = String(seg.label || '').toUpperCase().trim();
    if (allowed && !allowed.has(segLabelNorm)) return seg;
  ...
  return { segmentData: patched, overrideCount, dir, partial: !!allowed, allowedLabels: partialSceneLabels || null };
}
