  ...
  for (const scene of scriptScenes) {
    if (scene.type === 'source_clip') {
      ...
      if (scene.hasClipInsert) {
        ...
        const clip = streamerKey
          ? nextClipForStreamer(streamerKey)
          : orderedClipUrls[fallbackClipIdx++] || null;
...
function applyManualOverrides(jobId, segmentData) {
  ...
  const patched = (segmentData || []).map((seg, i) => {
    ...
    if (
      ...
      //   Only accept if the folder label matches the segment label (or the folder has no label)
      //   — prevents an old OUTRO at ordinal 6 from being assigned to a missing scene
      //   simply because ordinals happen to align.
      const byHeygenOrd = nestedExports.find((n) =>
        n.ord === heygenOrdinal && (!n.label || !segLabel || n.label === segLabel)
      );
      ...
      //   Requires label agreement when both the folder and segment have a label — prevents
      //   HeyGen batch folders (which happen to have ordinal numbers) from polluting manual slots.
      const byAbsoluteIdx = nestedExports.find((n) =>
        n.ord === i && (!n.label || !segLabel || n.label === segLabel)
      );
