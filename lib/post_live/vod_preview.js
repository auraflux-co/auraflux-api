...
function ytdlpAuthArgs(vodUrl, jobId, { preferAndroid = false } = {}) {
  ...
  if (proxyUrl) {
    ...
    // preferAndroid: retry when cookies+web still get bot-blocked on datacenter IPs
    if (cookiesB64 && !preferAndroid) {
      ...
      // Cookies force web clients; YouTube n-challenge needs Deno + EJS (image has both).
...
async function extractPreviewWithYtdlp({ vodUrl, dest, startS, endS, jobId }) {
  ...
  const formatLadder =
    'bestvideo[height<=1080][vcodec^=avc1]+bestaudio[ext=m4a]/bestvideo[height<=720][vcodec^=avc1]+bestaudio[ext=m4a]/bestvideo[height<=480][vcodec^=avc1]+bestaudio[ext=m4a]/best[height<=720]/best';

  async function attempt(preferAndroid) {
    const { extraArgs, cookieFilePath } = ytdlpAuthArgs(vodUrl, jobId, { preferAndroid });
    try {
  ...
  try {
    ...
    console.warn(`[post-live/preview] ${jobId}: cookie/web path failed (${firstErr.message}) — retry android client`);
    await attempt(true);
  }
