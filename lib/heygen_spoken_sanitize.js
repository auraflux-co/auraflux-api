...
let _roster = null;
let _rosterMtimeMs = 0;

function loadRoster() {
  const rosterPath = path.join(__dirname, '..', 'data', 'streamers.json');
  // CPD-1223: mtime-based invalidation — a process-lifetime cache meant phonetic
  // edits to streamers.json silently didn't apply until the next restart.
  let mtimeMs = 0;
  try { mtimeMs = fs.statSync(rosterPath).mtimeMs; } catch { /* empty */ }
  if (_roster && mtimeMs === _rosterMtimeMs) return _roster;
  _roster = [];
  _rosterMtimeMs = mtimeMs;
  try {
    _roster = JSON.parse(fs.readFileSync(rosterPath, 'utf8'))?.roster || [];
  } catch { /* empty */ }
  return _roster;
}
