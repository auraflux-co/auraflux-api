...
async function buildPortraitCompSegments({
  ...
  for (let ci = 0; ci < clipFiles.length; ci++) {
    ...
    if (hasTimedLayout) {
      ...
      // CPD-1293: staged peak / R2 window files are already 0-based — match Review.
      // Never -ss absolute VOD offsets on a ~30–45s window MP4 (black panes / EOF).
      const { isStagedVodPeakClip } = require('./content_library/stable_mp4');
      const stagedPeak = isStagedVodPeakClip(clipMeta)
        || !!(clipMeta.vodPeakWindow || clipMeta.stagedUrl || clipMeta.r2Url);
      const trimStartN = Number(clipMeta.trimStart) || 0;
      const trimEndN = clipMeta.trimEnd != null ? Number(clipMeta.trimEnd) : null;
      const sourceFilePreTrimmed = !!stagedPeak;
      const layoutTrimStart = sourceFilePreTrimmed ? 0 : clipMeta.trimStart;
      const layoutTrimEnd = sourceFilePreTrimmed && trimEndN != null && trimStartN > 0
        ? Math.max(0.04, trimEndN - trimStartN)
        : clipMeta.trimEnd;
      await applyPortraitLayoutTimed(layoutInput, pc, {
        compCreative,
        trimStart: layoutTrimStart,
        trimEnd: layoutTrimEnd,
        layoutSegments,
        openingLayout,
        deliveryAspect,
        sourceFilePreTrimmed,
        log: (m) => log(`  ${m}`),
      });
