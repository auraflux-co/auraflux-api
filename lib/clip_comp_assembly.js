...
async function buildPortraitCompSegments({
  ...
  for (let ci = 0; ci < clipFiles.length; ci++) {
    ...
    if (hasTimedLayout) {
      ...
      // CPD-1293: staged peak windows are already the Short trim — match Review preTrimmed=true.
      const stagedPeak = !!(clipMeta.vodPeakWindow || clipMeta.stagedUrl || clipMeta.r2Url);
      const trimStartN = Number(clipMeta.trimStart) || 0;
      const trimEndN = clipMeta.trimEnd != null ? Number(clipMeta.trimEnd) : null;
      const sourceFilePreTrimmed = stagedPeak && trimStartN === 0 && trimEndN != null;
