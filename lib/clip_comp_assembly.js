...
async function buildPortraitCompSegments({
  ...
  for (let ci = 0; ci < clipFiles.length; ci++) {
    ...
    const hasTimedLayout = Array.isArray(layoutSegments) && layoutSegments.length > 0;
    ...
    let facecamRect;
    if (hasTimedLayout) {
      log(`  [layout] temporal plan: ${layoutSegments.length} breakpoint(s) on clip ${ci + 1}`);
      await applyPortraitLayoutTimed(layoutInput, pc, {
        compCreative,
        trimStart: clipMeta.trimStart,
        trimEnd: clipMeta.trimEnd,
        layoutSegments,
        openingLayout: clipMeta.openingLayout || null,
        deliveryAspect,
