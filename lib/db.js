...
function _migrateJobsTable(db) {
  ...
  const newColumns = [
    ...
    // CPD-48: Content scheduling
    { name: 'publish_mode',         ddl: "ALTER TABLE jobs ADD COLUMN publish_mode TEXT DEFAULT 'immediate'" },
    { name: 'scheduled_publish_at', ddl: 'ALTER TABLE jobs ADD COLUMN scheduled_publish_at INTEGER' },
    { name: 'actual_published_at',  ddl: 'ALTER TABLE jobs ADD COLUMN actual_published_at INTEGER' },
  ];
...
function saveJob(jobId, card) {
  ...
  stmt.run(jobId, contentType, formType, status, stage, createdAt, now, JSON.stringify(card));
}
...
function saveMetric(jobId, stage, durationMs, data) {
  ...
  db.prepare(
    ...
    VALUES (?, ?, ?, ?, ?)
  `
  ).run(cid, stage, durationMs ?? null, data ? JSON.stringify(data) : null, Date.now());
}
...
function seedJobSpecFromScript(jobId, jobSpecObj) {
  ...
  if (
    exists.job_spec != null &&
    String(exists.job_spec).trim() !== '' &&
    exists.job_spec !== 'null'
  ) {
    return false;
  }
  ...
  `
  ).run(JSON.stringify(spec), spec.customerId || null, spec.templateId || null, now, jobId);
  return true;
}
...
function getGateResults(jobId) {
  ...
  const rows = db
    .prepare(`SELECT gate, result FROM gate_results WHERE job_id IN (${ph}) ORDER BY id ASC`)
    .all(...ids);
...
/**
 * Merge latest gate0/gate1 rows from SQLite into the jobs.card JSON so dashboards
 * and /job-spec consumers do not contradict gate_results when Portal 1 fails
 * (previously the card was only written on Portal 1 pass).
 */
function syncJobCardScriptGateSnapshot(jobId) {
  try {
    if (!jobId || typeof jobId !== 'string' || !jobId.startsWith('script_')) return;
    const card = loadJob(jobId);
    if (!card) return;
    const gr = getGateResults(jobId);
    const next = { ...card, lastGateSyncAt: new Date().toISOString() };
    if (gr.gate0) {
      next.gate0Passed = !!gr.gate0.passed;
    }
    if (gr.gate1) {
      next.gate1Score = gr.gate1.score;
      next.gate1Passed = !!gr.gate1.passed;
      next.gate1Outcome = gr.gate1.outcome || (gr.gate1.passed ? 'pass' : 'fail');
      const fd = gr.gate1.fixDirective;
      if (fd && typeof fd.delivered === 'string') {
        next.gate1Summary = fd.delivered.slice(0, 800);
      }
    }
    saveJob(jobId, next);
  } catch (e) {
    console.warn(`[db] syncJobCardScriptGateSnapshot(${jobId}): ${e.message}`);
  }
}
...
/**
 * Save a Portal 5 publish result per platform.
 */
...
function markJobPublished(jobId, driveUrl) {
  const db = getDb();
  db.prepare('UPDATE jobs SET drive_url = ?, published_at = ?, status = ? WHERE id = ?').run(
    driveUrl || null,
    Date.now(),
    'published',
    jobId
...
/**
 * Set publish_mode and scheduled_publish_at on a job.
 *
 * @param {string} jobId
 * @param {'immediate'|'scheduled'} publishMode
 * @param {number|null}  scheduledAt — Unix epoch ms (or null to clear)
 */
function updateJobPublishSchedule(jobId, publishMode, scheduledAt) {
  const db = getDb();
  db.prepare(`
    UPDATE jobs
    SET    publish_mode = ?, scheduled_publish_at = ?
    WHERE  id = ?
  `).run(publishMode, scheduledAt ?? null, jobId);
}

/**
 * Return jobs that are in 'ready_to_publish' status, have publish_mode='scheduled',
 * and whose scheduled_publish_at <= now.
 *
 * Used by the scheduling cron job.
 *
 * @returns {Object[]}
 */
function getJobsDueForScheduledPublish() {
  const db  = getDb();
  const now = Date.now();
  return db.prepare(`
    SELECT *
...
/**
 * Mark a scheduled job as published.
 */
function markJobActuallyPublished(jobId) {
  const db  = getDb();
  const now = Date.now();
  db.prepare(`
    UPDATE jobs
...
module.exports = {
  ...
  // Voice profiles — stubs on C0; overridden by Postgres adapter on C1+
  getVoiceProfile: async () => null,
  saveVoiceProfile: async () => false,
  // Billing / Credits / Stripe — C0 stubs (Postgres-only on C1+ Render)
  // These are overridden below when DATABASE_URL is present.
  getPool: () => { throw new Error('[db] Postgres pool not available in C0 mode — DATABASE_URL not set'); },
  getCreditBalance: async () => null,
  getClientPlan: async () => null,
  logCreditEvent: async () => {},
  getActivePacks: async () => [],
  deductPackCredits: async () => {},
  getOrCreateBillingPeriod: async () => null,
  insertCreditPack: async () => null,
  hasStripeEvent: async () => false,
  recordStripeEvent: async () => {},
  updateClientPlanTier: async () => false,
  updatePublishStatus: async () => {},
};

// ── C1+ Postgres billing/voice overlay ───────────────────────────────────────
// The job pipeline (job_spec.js, assembly.js, etc.) remains on SQLite + persistent
// disk until job_spec.js is refactored to be async (CPD-51 phase 2).
// Billing, credits, Stripe, and voice profiles use Postgres on Render now.
if (process.env.DATABASE_URL) {
  const pg = require('./db/postgres');
  const pgOverrides = {
    getPool:                  pg.getPool,
    getCreditBalance:         pg.getCreditBalance,
    getClientPlan:            pg.getClientPlan,
    logCreditEvent:           pg.logCreditEvent,
    getActivePacks:           pg.getActivePacks,
    deductPackCredits:        pg.deductPackCredits,
    getOrCreateBillingPeriod: pg.getOrCreateBillingPeriod,
    insertCreditPack:         pg.insertCreditPack,
    hasStripeEvent:           pg.hasStripeEvent,
    recordStripeEvent:        pg.recordStripeEvent,
    updateClientPlanTier:     pg.updateClientPlanTier,
    updatePublishStatus:      pg.updatePublishStatus,
    getVoiceProfile:          pg.getVoiceProfile,
    saveVoiceProfile:         pg.saveVoiceProfile,
  };
  Object.assign(module.exports, pgOverrides);
}
