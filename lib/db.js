...
const SCHEMA = `
  ...
  -- CPD-1219: per-streamer YouTube search cache (one API search per streamer per day)
  CREATE TABLE IF NOT EXISTS competitor_search_log (
...
function initDb() {
  ...
  // CPD-1219: streamer tags on competitor videos
  _migrateCompetitorVideosTable(_db);
...
/**
 * CPD-1219 — competitor_videos.streamers: JSON array of canonical twitch
 * logins detected in the video title (tagged at sync time).
 */
function _migrateCompetitorVideosTable(db) {
  const existingColumns = db.pragma('table_info(competitor_videos)').map(c => c.name);
  if (!existingColumns.includes('streamers')) {
    try {
      db.exec('ALTER TABLE competitor_videos ADD COLUMN streamers TEXT');
      console.log('[db] Migration: added column competitor_videos.streamers');
    } catch (e) {
      console.warn(`[db] Migration warning for competitor_videos.streamers: ${e.message}`);
    }
  }
}
