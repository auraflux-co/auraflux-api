...
const EFFECT_ORDER = [
  ...
  'curves',        // RGB curves
  'color_balance', // shadows/highlights balance / look tint
  'impact_tint',   // CPD-1283: timed red impact flashes
  'bw',            // black and white
  'vignette',      // vignette
  ...
  'zoom_punch',          // CPD-415/1279: zoom punch-in
  'camera_shake',        // CPD-1280: timed crop wobble
...
const AUDIO_EFFECTS = {
  ...
  speed(jobSpec) {
    // CPD-1281: keep A/V in sync when video.speedRamp is set without audio.speed.
    const s = _val(jobSpec, 'effects.audio.speed', null)
      ?? _val(jobSpec, 'effects.video.speedRamp', null);
    if (!s || s === 1) return null;
    const { buildAtempoChain } = require('./speed_ramps');
    return buildAtempoChain(s);
  },
  ...
  sound_effects(jobSpec) {
    // Timed highlight SFX are mixed in clip_comp_audio_mix (multi-input amix).
    // Marker so active-effects / grader can see the feature is ordered.
    const cfg = _val(jobSpec, 'addOns.sound_effects', null)
      || _val(jobSpec, 'effects.audio.sound_effects', null)
      || _val(jobSpec, 'compCreative.audio.highlightSfx', null);
    if (!cfg || cfg.enabled === false) return null;
    const drops = Array.isArray(cfg.drops) ? cfg.drops : [];
    if (!drops.length && !cfg.style) return null;
    return '__SOUND_EFFECTS_AMIX__';
  },
};
...
const VIDEO_EFFECTS = {
  ...
  // CPD-1283: CapCut-style look tint (whole-clip colorbalance)
  color_balance(jobSpec) {
    const cfg = _val(jobSpec, 'effects.color.colorBalance', null) ||
                _val(jobSpec, 'addOns.color.colorBalance', null);
    if (!cfg) return null;
    const { buildColorbalanceFrag } = require('./look_presets');
    return buildColorbalanceFrag(cfg);
  },

  // CPD-1283: timed red impact flashes (Gemini Core_fx gap)
  impact_tint(jobSpec) {
    const cfg = _val(jobSpec, 'addOns.impact_tint', null) ||
                _val(jobSpec, 'effects.video.impact_tint', null);
    if (!cfg || cfg.enabled === false) return null;
    const { normalizeImpactTints, buildImpactTintFilter } = require('./look_presets');
    const flashes = normalizeImpactTints(cfg);
    if (!flashes.length) return null;
    return buildImpactTintFilter(flashes);
  },
  ...
  speed_ramp(jobSpec) {
    // Constant whole-clip factor only. Timed segments use applySpeedRamps (CPD-1281).
    const factor = _val(jobSpec, 'effects.video.speedRamp', null);
    if (!factor || factor === 1) return null;
    const ramps = _val(jobSpec, 'effects.video.speedRamps', null);
    if (Array.isArray(ramps) && ramps.length) return null;
    const { buildSetpts } = require('./speed_ramps');
    return buildSetpts(factor);
  },
  ...
  transitions(jobSpec) {
    // Multi-input xfade cannot live in a single -vf chain. Clip comps use
    // lib/clip_comp_transitions.js (CPD-1284). API jobs use assembly_service.
    // Return a sentinel so getActiveEffects / manifests can see the order.
    const on = _val(jobSpec, 'effects.transitions', false)
      || _val(jobSpec, 'addOns.effects.transitions', false)
      || _val(jobSpec, 'compCreative.transition.style', null);
    if (!on || on === 'cut' || on === 'off' || on === 'none') return null;
    return '__TRANSITIONS_XFADE__';
  },
  ...
  zoom_punch(jobSpec) {
    ...
    if (!cfg || cfg.enabled === false) return null;
    // CPD-1279 / CPD-415: real zoompan from punch points (Compose/AI).
    const { normalizeZoomPunches, buildZoomPunchFilter } = require('./zoom_keyframes');
    const punches = normalizeZoomPunches(cfg);
    if (!punches.length) return null;
    const w = Number(cfg.width) || Number(_val(jobSpec, 'delivery.width', 1080)) || 1080;
    const h = Number(cfg.height) || Number(_val(jobSpec, 'delivery.height', 1920)) || 1920;
    const fps = Number(cfg.fps) || Number(_val(jobSpec, 'delivery.fps', 30)) || 30;
    return buildZoomPunchFilter(punches, { width: w, height: h, fps });
  },

  // CPD-1280: timed camera shake (crop wobble)
  camera_shake(jobSpec) {
    const cfg = _val(jobSpec, 'addOns.camera_shake', null) ||
                _val(jobSpec, 'effects.video.camera_shake', null);
    if (!cfg || cfg.enabled === false) return null;
    const { buildShakeFilter, normalizeShakes } = require('./camera_fx');
    const shakes = normalizeShakes(cfg);
    if (!shakes.length) return null;
    return buildShakeFilter(shakes);
  },
  ...
  animated_text_effects(jobSpec) {
    const cfg = _val(jobSpec, 'addOns.animated_text_effects', null)
      ...
      || _val(jobSpec, 'compCreative.animatedText', null);
    if (!cfg || (cfg.enabled === false && !Array.isArray(cfg.items) && !Array.isArray(cfg))) return null;
    const { buildAnimatedTextFilter } = require('./animated_text');
    return buildAnimatedTextFilter(cfg);
  },
...
function buildAudioFilterChain(jobSpec) {
  // Same sentinels as buildVideoFilterChain — not real FFmpeg -af fragments.
  const SENTINELS = new Set([
    '__PORTRAIT_BLUR_PAD__', '__CAPTIONS_WHISPER__', '__CAPTIONS_BURNIN__', '__CAPTIONS_STYLED__',
    '__TRANSITIONS_XFADE__', '__SOUND_EFFECTS_AMIX__',
  ]);
  const parts = [];
