 ...
 *   generateChromeOverlay {function}
...
async function burnChrome(ctx) {
  ...
  try {
    ...
    const allNbaIntros = segsToProcess.filter(
      (s) => /GAME\d+[_ ]INTRO/i.test(s.label || '') && s.cardData
    );
    const allStories =
      allNbaIntros.length > 0
        ? allNbaIntros.map((introSeg, idx) => {
            ...
            const matchup = raw
              ...
              .slice(0, 40);
            return {
              title: matchup || `Game ${idx + 1}`,
              category: 'NBA GAME',
              storyId: introSeg.cardData?.storyId || `game_${idx}`,
            };
          })
        : [
            {
              title: cardData.matchup || cardData.title || 'NBA Highlights',
              category: 'NBA GAME',
              storyId: 'game_0',
            },
          ];
    ...
    const byStoryId = allStories.findIndex(
      (s) => cardData?.storyId && s.storyId === cardData.storyId
    );
    const byTitle = allStories.findIndex(
      (s) => cardData?.matchup && s.title.toLowerCase() === String(cardData.matchup).toLowerCase()
    );
    ...
    const overlayBase = {
      title:
        candidateTitle && !/^(story|intro|nba highlights)$/i.test(candidateTitle)
          ? candidateTitle
          : fallbackTitle,
      category: 'NBA GAME',
    ...
    if (isGameIntro) {
      ...
      await generateChromeOverlay(overlayBase, overlayVisiblePath, activeGameIndex, {
        showLowerThird: true,
        hideSidebar: false,
        episodeNumber,
      ...
      await generateChromeOverlay(overlayBase, overlayHiddenPath, activeGameIndex, {
        showLowerThird: true,
        hideSidebar: false,
        episodeNumber,
        activeCategory: 'NBA GAME',
      ...
      if (fs.existsSync(burnedPath) && fs.statSync(burnedPath).size > 10000) {
        ...
        log(
          asmId,
          `  🏀 NBA chrome burned (two-state) [game ${activeGameIndex + 1}/${allStories.length}]: ${overlayBase.title}`
        );
      }
    } else {
      ...
      await generateChromeOverlay(overlayBase, overlayBodyPath, activeGameIndex, {
