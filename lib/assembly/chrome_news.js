...
async function burnChrome(ctx) {
  ...
  // Fix 7: omitBackground:true in generateChromeOverlay() produces real RGBA PNG.
  ...
  if (!_directiveHandled) {
    ...
    try {
      ...
      const storyIndex = allStories.findIndex(
        (s) => s.storyId === currentStoryId || s.title === cardData.title
      );
      ...
      const activeCategory =
        cardData.category && cardData.category !== cardData.source
          ? cardData.category
          : 'WORLD NEWS';
      ...
      const overlayBase = {
        title:
          candidateTitle && !/^(story|intro|breaking news story)$/i.test(candidateTitle)
            ? candidateTitle
            : safeFallbackTitle,
      ...
      if (isStoryIntro) {
        ...
        await generateChromeOverlay(overlayBase, overlayVisiblePath, activeStoryIndex, {
          showLowerThird: true,
          hideSidebar: false,
          episodeNumber,
          activeCategory,
        ...
        await generateChromeOverlay(overlayBase, overlayHiddenPath, activeStoryIndex, {
          showLowerThird: true,
          hideSidebar: false,
          episodeNumber,
          activeCategory,
        ...
        await new Promise((res, rej) => {
          ...
          proc.on('close', (code) => {
            if (code === 0) res();
              ...
              console.error(
                `[intro-burn] FFmpeg exit ${code} for news two-state overlay: ${reason}`
              );
        ...
        if (fs.existsSync(burnedPath) && fs.statSync(burnedPath).size > 10000) {
          ...
          log(
            asmId,
            `  📰 NEWS two-state overlay burned [${activeStoryIndex + 1}/${allStories.length}]: ${cardData.title || 'story'}`
          );
        }
      } else if (isStoryBody) {
        ...
        await generateChromeOverlay(overlayBase, overlayBodyPath, activeStoryIndex, {
          showLowerThird: true,
          hideSidebar: false,
          episodeNumber,
          activeCategory,
        ...
        await new Promise((res, rej) => {
          ...
          proc.on('close', (code) => {
            if (code === 0) res();
              ...
              const reason = burnStderr.slice(-300).replace(/\n/g, ' ').trim();
              console.error(
                `[story-body-burn] FFmpeg exit ${code} for news body overlay: ${reason}`
              );
        ...
        await new Promise((res, rej) => {
          ...
          proc.on('close', (code) => {
            if (code === 0) res();
              ...
              const reason = burnStderr.slice(-300).replace(/\n/g, ' ').trim();
              console.error(
                `[intro-burn] FFmpeg exit ${code} for news single-state overlay: ${reason}`
              );
