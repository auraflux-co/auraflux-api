...
async function burnChrome(ctx) {
  ...
  try {
    ...
    const byStoryId = allStories.findIndex((s) => s.storyId === (currentSeg?.cardData?.storyId || currentSeg?.storyId));
    const activeIdx = activeStreamerIndex >= 0 ? activeStreamerIndex : (byStoryId >= 0 ? byStoryId : 0);
    ...
    const overlayBase = {
      title:      candidateTitle && !/^(story|intro|streamer)$/i.test(candidateTitle) ? candidateTitle : fallbackTitle,
      category:   'ON STREAM',
    ...
    if (isStreamerIntro) {
      ...
      await generateNewscastOverlay(overlayBase, overlayVisiblePath, activeIdx, {
        showLowerThird: true, hideSidebar: false,  episodeNumber, activeCategory: 'ON STREAM', contentType: 'twitch', baselinePreset: '0415'
      });
      ...
      await generateNewscastOverlay(overlayBase, overlayBodyPath, activeIdx, {
        showLowerThird: !isColdOrOutro, hideSidebar: isColdOrOutro, episodeNumber, activeCategory: 'ON STREAM', contentType: 'twitch', baselinePreset: '0415'
      });
