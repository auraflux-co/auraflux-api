...
const { normalizeContentType, acceptsLegacyContentType } = require('./content_type');

/** All content types accepted by assemble / generate-full-script routes. */
const PIPELINE_CONTENT_TYPES = [
  'twitch', 'sports', 'nba', 'news',
  'twitch-short', 'sports-short', 'nba-short', 'news-short',
];
 ...
 * @returns {Function} Express middleware
 */
function validateContentType(allowed = PIPELINE_CONTENT_TYPES) {
  return (req, res, next) => {
    const contentType = req.body.contentType || req.query.contentType;
    if (contentType && !allowed.includes(contentType) && !acceptsLegacyContentType(contentType, allowed)) {
