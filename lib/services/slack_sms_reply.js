...
const { outboundLineForInbound } = require('./telnyx_line_routing');
...
async function sendThreadSms({ channel, threadTs, mapping, replyText }) {
  const fromLine = outboundLineForInbound(mapping.to_number);
  await sms.sendSms({
  ...
  if (botConfigured()) {
    await postMessage({
      channel,
      threadTs,
      text: `✅ Sent to ${mapping.from_number} from ${fromLine}`,
    });
  }

  return { handled: true, to: mapping.from_number, from: fromLine };
}
