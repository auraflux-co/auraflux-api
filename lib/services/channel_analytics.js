...
function hasMonetaryScope(tokens) {
  return !!(tokens?.scope || '').includes('yt-analytics-monetary.readonly');
}
...
async function fetchPerVideoAnalytics(channelId, opts = {}) {
  ...
  try {
    const summaryStart = opts.reportingStartDate || opts.startDate;
    const summaryEnd = opts.reportingEndDate || opts.endDate;
    if (summaryStart && summaryEnd) {
      channelSummary = await fetchChannelSummary(channelId, {
        startDate: summaryStart,
        endDate: summaryEnd,
        accessToken,
        includeRevenue: hasMonetaryScope(tokens),
      });
    } else {
      const windowDays = Number(process.env.CWN_NORTH_STAR_WINDOW_DAYS) || 28;
      const nsEnd = endDate;
      const nsStart = (() => {
...
async function fetchChannelSummary(channelId, opts = {}) {
  ...
  const tokens = loadTokens();
  const baseMetrics = 'views,engagedViews,estimatedMinutesWatched,subscribersGained,subscribersLost,likes,comments,shares';
  const metrics = opts.includeRevenue !== false && hasMonetaryScope(tokens)
    ? `${baseMetrics},estimatedRevenue`
    : baseMetrics;
