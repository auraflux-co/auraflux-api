...
function applyPublishRequestToSpec(spec, body = {}) {
  ...
  if (Object.keys(publishMeta).length) {
    ...
    if (publishMeta.youtubePlaylistId !== undefined) {
      spec.order.publish.youtubePlaylistId = publishMeta.youtubePlaylistId;
    }
    if (publishMeta.youtubePlaylistTitle !== undefined) {
      spec.order.publish.youtubePlaylistTitle = publishMeta.youtubePlaylistTitle;
    }
    if (publishMeta.playlistStrategyBadge !== undefined) {
      spec.order.publish.playlistStrategyBadge = publishMeta.playlistStrategyBadge;
    }
  }
  if (body.youtubePlaylistId) {
    spec.order = spec.order || {};
    spec.order.publish = spec.order.publish || {};
    spec.order.publish.youtubePlaylistId = body.youtubePlaylistId;
    if (body.youtubePlaylistTitle) spec.order.publish.youtubePlaylistTitle = body.youtubePlaylistTitle;
    if (body.playlistStrategyBadge) spec.order.publish.playlistStrategyBadge = body.playlistStrategyBadge;
  }
  return spec;
}
...
async function executeApprovePublish({ db, jobId, spec, platforms, retryPlatformUpload }) {
  ...
  // Playlist strategy — add YouTube VOD to matched playlist after successful upload
  try {
    const playlistId = spec.order?.publish?.youtubePlaylistId;
    const yt = results?.youtube || results?.YouTube;
    const videoId = yt?.videoId || yt?.id || yt?.platformJobId || null;
    if (playlistId && videoId && publishResultsHadSuccess({ youtube: yt })) {
      const { addVideoToPlaylist } = require('./youtube_direct');
      const pl = await addVideoToPlaylist(videoId, playlistId);
      if (!spec.state) spec.state = {};
      spec.state.playlistStrategy = { playlistId, videoId, result: pl, at: new Date().toISOString() };
    }
  } catch (e) {
    if (!spec.state) spec.state = {};
    spec.state.playlistStrategyError = String(e.message || e).slice(0, 200);
  }
