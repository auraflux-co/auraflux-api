 ...
 *   const diy   = await getPlan('operate');
 *
...
const PRICE_ENV_KEYS = {
  ...
  diy:     'STRIPE_PRICE_OPERATE',
  dwy:     'STRIPE_PRICE_GUIDED',
  dfy:     'STRIPE_PRICE_MANAGED',
};
 ...
 * PlanDef shape:
 * {
 *   id:                  string   // 'operate' | 'guided' | 'managed' | custom tier
 ...
 *   price_usd:           number   // dollars (unit_amount / 100)
...
async function fetchPlansFromStripe() {
  ...
  for (const product of products.data) {
    ...
    plans[tier] = {
      ...
      // Stripe unit_amount is cents — expose dollars to the app UI
      price_usd:           typeof price.unit_amount === 'number'
 ...
 * Returns the tier string ('operate', 'guided', etc.) or null.
 */
