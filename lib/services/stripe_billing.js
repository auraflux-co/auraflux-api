...
async function upgradeExistingSubscription(stripeCustomerId, newPriceId, clientId, planId) {
  ...
  // CPD-387: correctly identify the plan subscription item.
  //
  // Strategy (in order):
  //  1. Single-item subscription → always safe to take it (AuraFlux sells one
  //     plan per subscription; this is the common case today).
  //  2. Multi-item subscription → match by known plan price IDs from env vars.
  //  3. No match → bail out; caller falls through to a new checkout session.
  //
  // NOTE: the previous OR condition `|| i.price.id === data[0]?.price?.id`
  // compared each item to itself and always matched the first item — removed.
  let planItem;
  if (sub.items.data.length === 1) {
    planItem = sub.items.data[0];
  } else {
    const addonPriceId = process.env.STRIPE_PRICE_MANAGED_ADDON;
    const knownPlanPriceIds = new Set(
      ['STRIPE_PRICE_GROWTH', 'STRIPE_PRICE_OPERATE', 'STRIPE_PRICE_GUIDED', 'STRIPE_PRICE_MANAGED']
        .map((k) => process.env[k])
        .filter(Boolean)
        .filter((id) => id !== addonPriceId),
    );
    planItem = sub.items.data.find((i) => knownPlanPriceIds.has(i.price.id));
  }

  if (!planItem) return null; // ambiguous — fall through to checkout
...
const PLANS = {
  growth:  { id: 'growth',  label: 'Growth / Creator',       credits: 30,   price_usd: 299,  brands: 1, collab_mode: 'guides' },
  operate: { id: 'operate', label: 'Pro Operator / Agency',  credits: 50,   price_usd: 999,  brands: 1, collab_mode: 'guides' },
  guided:  { id: 'guided',  label: 'Guided',                 credits: 200,  price_usd: 2499, brands: 3, collab_mode: 'full' },
  managed: { id: 'managed', label: 'Managed',                credits: 1000, price_usd: 4499, brands: 5, collab_mode: 'full', has_account_manager: true },
};
 ...
 */
function getManagedAddonPriceId() {
  return process.env.STRIPE_PRICE_MANAGED_ADDON || null;
}

function isManagedAddonPriceId(priceId) {
  const addon = getManagedAddonPriceId();
  return !!(priceId && addon && priceId === addon);
}

async function resolveSubscriptionTier(priceId) {
  if (!priceId) return null;
  // Managed add-on is not a base plan — never map it to tier=managed.
  if (isManagedAddonPriceId(priceId)) return null;
...
/**
 * Inspect subscription line items → base plan tier + Managed add-on flag.
 * Legacy full Managed (STRIPE_PRICE_MANAGED alone) → tier=managed, managedAddon=true.
 */
async function resolveSubscriptionState(subscription) {
  const items = subscription?.items?.data || [];
  const addonPriceId = getManagedAddonPriceId();
  let managedAddon = false;
  let baseTier = null;
  ...
  for (const item of items) {
    ...
    if (tier) baseTier = tier;
  }

  if (baseTier === 'managed') managedAddon = true;
  return { tier: baseTier, managedAddon };
}

/**
 * Attach Managed add-on to the customer's active Creator/Studio subscription.
 * Returns { ok, subscription } or { ok:false, reason, checkoutUrl? }.
 */
async function addManagedAddonToSubscription(clientId, email, successUrl, cancelUrl) {
  const priceId = getManagedAddonPriceId();
  if (!priceId) {
    throw new Error('STRIPE_PRICE_MANAGED_ADDON is not configured');
  }
  ...
  const sub = subs.find((s) => (s.items?.data || []).length > 0) || null;

  if (!sub) {
    return {
      ok: false,
      reason: 'no_active_subscription',
      message: 'Subscribe to Creator or Studio first, then add Managed.',
    };
  }

  const already = (sub.items.data || []).some((i) => i.price?.id === priceId);
  if (already) {
    return { ok: true, alreadyAttached: true, subscriptionId: sub.id };
  }

  // Add a second line item — do NOT replace the base plan item.
  try {
    ...
    await stripe.subscriptions.update(sub.id, {
      metadata: { ...(sub.metadata || {}), clientId, managedAddon: 'true' },
    });
    const { setManagedAddon } = require('../db/postgres');
    await setManagedAddon(clientId, true, sub.id, customerId);
    return { ok: true, subscriptionId: sub.id, attachedInPlace: true };
  } catch (err) {
    const session = await stripe.checkout.sessions.create({
      mode:                      'subscription',
      customer:                  customerId,
      automatic_payment_methods: { enabled: true },
      line_items:                [{ price: priceId, quantity: 1 }],
      metadata:                  { clientId, addon: 'managed', planId: 'managed_addon' },
      subscription_data: {
        metadata: { clientId, addon: 'managed', managedAddon: 'true' },
      },
      success_url: successUrl,
      cancel_url:  cancelUrl,
    });
...
module.exports = {
  ...
  addManagedAddonToSubscription,
  getManagedAddonPriceId,
  isManagedAddonPriceId,
  attemptAutoTopup,
};
