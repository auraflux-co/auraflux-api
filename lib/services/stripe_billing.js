...
async function createCheckoutSession(clientId, packId, successUrl, cancelUrl, quantity = 1, email = null) {
  ...
  // CPD-381: resolve customer so pack purchases are linked to the same Stripe
  // Customer object used by subscriptions and the payment-method page.
  let customerParam = {};
  if (email) {
    try {
      const cid = await getOrCreateStripeCustomer(clientId, email);
      customerParam = { customer: cid };
    } catch { /* non-fatal — checkout works without a customer */ }
  }
...
async function createSubscriptionCheckoutSession(clientId, planId, successUrl, cancelUrl, brandId = null, email = null) {
  ...
  // CPD-381: link checkout to the customer so invoices + auto top-up work
  let customerParam = {};
  if (email) {
    try {
      const cid = await getOrCreateStripeCustomer(clientId, email);
