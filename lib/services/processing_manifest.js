...
function _deriveOrderedFeatures(spec) {
  const features = [];
  // Branding
  if (spec.addOns?.branding?.active) features.push('branding');
  ...
  if (eff.zoom)        features.push('effects.zoom');
  if (eff.transitions) features.push('effects.transitions');
  if (eff.ken_burns)   features.push('effects.ken_burns');
  // Audio
