'use strict';
/**
 * Production cron (CPD-1053) — auto-run calendar production slots.
 * Shorts + live scheduling always; long-form only when avatar/HeyGen is unblocked.
 *
 * Disable: PRODUCTION_CRON=off
 * On Render: defaults to off until PRODUCTION_CRON=on (avoid surprise cloud jobs).
 */

const TICK_MS = 60 * 1000;
let _timer = null;

function productionCronDefault() {
  if (process.env.PRODUCTION_CRON) return process.env.PRODUCTION_CRON;
  return process.env.RENDER ? 'off' : 'on';
}

function startProductionCron({ baseUrl, getPersistedJobs, saveJobCard } = {}) {
  if (String(productionCronDefault()).toLowerCase() === 'off') {
...
module.exports = { startProductionCron, stopProductionCron, TICK_MS, productionCronDefault };
