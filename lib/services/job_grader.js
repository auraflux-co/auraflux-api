...
const CHECKS = [
  // ── Critical gates (job must reach these states) ──────────────────────────
  {
    ...
    implemented: true,
    run(spec) {
      const url = spec.outputUrl || spec.state?.savedOutputs?.driveUrl;
  ...
  },
  {
    ...
    run(spec) {
      ...
      return avg >= 80 ? pass(`avg ${avg}`) : fail(`avg score ${avg} — below 80 threshold`);
    },
  },
  ...
  },
  {
    ...
    run(spec) {
      ...
      // portal3a confirms chromeVisible in its report
