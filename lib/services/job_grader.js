...
const CHECKS = [
  ...
  },
  {
    ...
    // not falling back to the platform default. When designSpec.chrome.name is
    ...
    run(spec) {
      if (!_featureActive(spec, 'branding')) return skip('branding not requested');
      const chromeName = spec.designSpec?.chrome?.name;
      if (!chromeName) {
        return warn(
          'designSpec.chrome.name not set — chrome overlay fell back to platform default ("AuraFlux"). ' +
          'Set designSpec.chrome.name to the streamer\'s show name before applying branding.'
        );
      }
