...
const CHECKS = [
  // ── Critical gates (job must reach these states) ──────────────────────────
  {
    ...
    implemented: true,
    run(spec) {
      // CPD-598: topic-only compose jobs produce a script, not a video — skip this check.
      // Video output is supplied in a subsequent fetch/upload job by the customer.
      if (spec.isTopicOnly) return skip('Topic-only compose job — script is the deliverable, no video output expected');
  ...
  {
    // CPD-QA: Verify branding chrome is configured with a real show name,
    // not falling back to the platform default. When designSpec.chrome.showName is
    // absent the assembly falls back to 'AuraFlux' text in BOTH the top bar and
    // the lower-third, making the output look like an unbranded platform demo.
    id: 'branding_config',
    label: 'Branding chrome name configured (not platform fallback)',
    weight: 2,
    implemented: true,
    run(spec) {
      if (!_featureActive(spec, 'branding')) return skip('branding not requested');
      // pipeline_assembly.js uses this fallback chain for the chrome name:
      //   designSpec.chrome.streamer → order.inputs.streamer → brandName
      // job_spec.js writes the brand show name into designSpec.chrome.showName.
      const chromeName = spec.designSpec?.chrome?.showName ||
                         spec.designSpec?.chrome?.name ||
                         spec.designSpec?.chrome?.streamer ||
                         spec.order?.inputs?.streamer ||
                         spec.brandName;
      if (!chromeName) {
        return warn(
          'No chrome name available (designSpec.chrome.showName, chrome.streamer, order.inputs.streamer, ' +
          'and brandName are all empty) — overlay fell back to platform default ("AuraFlux").'
        );
      }
      return pass(`chrome name: "${chromeName}"`);
    },
  },
  {
    // CPD-QA: Verify source duration is appropriate for the template format.
    // A 43-second clip submitted to a longform template produces a meaningless
    // output — the grader should surface this immediately.
    id: 'source_fit',
    label: 'Source duration fits template format',
    weight: 3,
    implemented: true,
    run(spec) {
      const sourceDuration = spec.state?.savedOutputs?.sourceDurationSeconds
        || spec.order?.inputs?.sourceDuration
        || null;
      const format = spec.order?.layoutMode || spec.addOns?.layout?.portrait ? 'portrait' : 'landscape';
      const isLongform = spec.order?.format === 'longform'
        || spec.order?.contentType === 'longform'
        || (spec.addOns?.layout?.portrait === false && !spec.addOns?.layout?.portrait);

      if (sourceDuration === null) return skip('source duration not recorded in savedOutputs');

      if (isLongform && sourceDuration < 300) {
        return fail(
          `Longform template received ${sourceDuration}s source — ` +
          `this template expects a VOD (≥300s). Output will not represent a real longform product.`
        );
      }
      if (!isLongform && sourceDuration > 600) {
        return warn(
          `Short-form template received ${sourceDuration}s source — ` +
          `pipeline may have trimmed it but the source was unusually long for this template.`
        );
      }
      return pass(`${sourceDuration}s source is appropriate for ${isLongform ? 'longform' : 'short-form'} template`);
    },
  },
  {
    id: 'dynamic',
  ...
  {
    id: 'zoom_punch',
    label: 'Zoom punch-in effect (CPD-415/1279)',
    weight: 2,
    implemented: true,
    run(spec) {
      if (!_featureActive(spec, 'zoom_punch')) return skip('zoom_punch not requested');
      const effects = spec.state?.savedOutputs?.postProcessEffects || [];
      if (effects.includes('video.zoom_punch') || effects.includes('zoom_punch')) return pass();
      const punches = spec.addOns?.zoom_punch || spec.effects?.video?.zoom_punch;
      if (punches) return warn('zoom_punch declared — confirm portrait FX pass');
      return warn('zoom_punch active but no punches recorded');
    },
  },
  {
    id: 'animated_text_effects',
    label: 'Animated text effects (CPD-416/1285)',
    weight: 2,
    implemented: true,
    run(spec) {
      if (!_featureActive(spec, 'animated_text_effects')) return skip('animated_text_effects not requested');
      const cfg = spec.addOns?.animated_text_effects
        || spec.effects?.overlay?.animated_text_effects
        || spec.compCreative?.animatedText;
      const items = cfg?.items || (Array.isArray(cfg) ? cfg : null);
      if (items && items.length) return pass();
      if (cfg?.enabled || cfg?.text) return pass();
      return warn('animated_text_effects ordered but no overlay items');
    },
  },
  {
    id: 'sound_effects',
    label: 'Sound effects (CPD-417/1286)',
    weight: 2,
    implemented: true,
    run(spec) {
      if (!_featureActive(spec, 'sound_effects')) return skip('sound_effects not requested');
      const cfg = spec.addOns?.sound_effects
        || spec.effects?.audio?.sound_effects
        || spec.compCreative?.audio?.highlightSfx;
      const drops = cfg?.drops || [];
      if (drops.length) return pass();
      if (cfg?.enabled || cfg?.style) return warn('sound_effects ordered — confirm amix pass');
      return warn('sound_effects active but no drops');
    },
  },
  {
    id: 'lower_thirds',
  ...
  },
  {
    ...
    implemented: true,
    run(spec) {
      // Skip if thumbnail approval was not ordered — most jobs don't request it.
      const ordered = spec.addOns?.thumbnailApproval?.active === true ||
                      spec.extensions?.thumbnail_ext?.ordered === true;
      if (!ordered) return skip('thumbnail approval not ordered');
  ...
  },
  {
    ...
    run(spec) {
      if (!_featureActive(spec, 'captions.whisper') && !_featureActive(spec, 'captions')) return skip('not requested');
      // Check processingManifest first — it records what assembly actually applied
      const manifest = spec.state?.processingManifest;
      if (manifest) {
        const captionsApplied = (manifest.featuresApplied || []).some((f) => f === 'captions' || f.startsWith('captions'));
        if (captionsApplied) return pass('captions confirmed in processingManifest — applied by assembly_postprocess');
        return warn('captions ordered but not found in processingManifest.featuresApplied — check assembly_effects captions path');
      }
      // Legacy: check effects array
      const effects = spec.state?.savedOutputs?.postProcessEffects || [];
      if (effects.some((e) => e.includes('captions'))) return pass('captions effect recorded in postProcessEffects');
      const transcript = spec.state?.savedOutputs?.transcript || spec.state?.savedOutputs?.gpt4oQA?.transcript;
      if (!transcript) return warn('captions ordered — not yet confirmed applied (no manifest and no transcript)');
      return pass('transcript present — captions assumed applied');
    },
  },
  ...
  // ── Colour & visual effects (CPD-433) ─────────────────────────────────────
  {
    ...
    weight: 2, implemented: true,
    run(spec) {
      const hasLut     = _featureActive(spec, 'color.lut');
      const hasPreset  = spec.colorGrade?.active && spec.colorGrade?.preset;
      const hasEq      = spec.colorGrade?.active && spec.colorGrade?.eq;
      if (!hasLut && !hasPreset && !hasEq) return skip('not requested');
      // Check processingManifest for confirmation
      const manifest = spec.state?.processingManifest;
      if (manifest) {
        const gradeApplied = (manifest.featuresApplied || []).some((f) => f === 'colorGrade' || f === 'video.eq' || f === 'video.lut');
        if (gradeApplied) {
          const preset = spec.colorGrade?.preset;
          return pass(`colorGrade${preset ? ` (${preset})` : ''} confirmed in processingManifest`);
        }
        return warn(`colorGrade${spec.colorGrade?.preset ? ` (${spec.colorGrade.preset})` : ''} ordered but not in processingManifest — check assembly_effects eq/lut path`);
      }
      const effects = spec.state?.savedOutputs?.postProcessEffects || [];
      if (effects.some((e) => e.includes('eq') || e.includes('lut'))) return pass('color grade effect in postProcessEffects');
      const lutPath = spec.effects?.color?.lutPath || spec.addOns?.color?.lutPath;
      return lutPath ? pass('lut3d filter wired') : warn('colorGrade ordered — not confirmed applied (no manifest)');
    },
  },
  {
  ...
  },
  {
    ...
    weight: 2, implemented: true,
    run(spec) {
      if (!_featureActive(spec, 'captions.burnin') && !_featureActive(spec, 'captions')) return skip('not requested');
      const manifest = spec.state?.processingManifest;
      if (manifest) {
        const captionsApplied = (manifest.featuresApplied || []).some((f) => f === 'captions' || f.startsWith('captions'));
        if (captionsApplied) return pass('captions burn-in confirmed in processingManifest');
        return warn('captions.burnin ordered but not found in processingManifest — check assembly captions path');
      }
      const effects = spec.state?.savedOutputs?.postProcessEffects || [];
      if (effects.some((e) => e.includes('captions'))) return pass('captions effect recorded in postProcessEffects');
      return warn('captions.burnin ordered — not confirmed applied (no manifest)');
    },
  },
  ...
  // ── QA upgrades (CPD-438) ─────────────────────────────────────────────────
  {
    ...
    weight: 2, implemented: true,
    run(spec) {
      // Clips content uses source audio we don't generate — platforms auto-normalize on upload.
      // Loudness compliance only applies to TTS/avatar jobs where we own the audio mix.
      const ct = spec.baseContentType || spec.contentType || '';
      if (ct === 'clips') return skip('clips content — source audio loudness not operator-controlled');
  ...
  },
  {
    ...
    run(spec) {
      const scan = spec.portalReports?.portal3a?.ffmpegScan;
      // portal3a does not yet populate ffmpegScan in its report — skip rather than
      // warn so that jobs don't lose a grade point for a data gap we can't control.
      if (!scan) return skip('ffmpegScan not populated by portal3a — cannot check');
  ...
  // ── Encoding & delivery (CPD-439) ─────────────────────────────────────────
  {
    ...
    run(spec) {
      ...
      // outputSizeBytes is not yet populated after R2 upload — skip rather than
      // warn so jobs don't lose grade points for a data gap we can't control.
      if (!sizeBytes) return skip('outputSizeBytes not tracked after R2 upload — cannot check bitrate');
      const platforms = spec.order?.publish?.platforms || [];
  ...
  // ── CPD-QA: Feature delivery verification (backed by processingManifest) ──
  {
    id: 'features_delivered', label: 'All ordered features delivered',
    weight: 5, implemented: true,
    run(spec) {
      // Check portal3a's featureVerification report first (most authoritative)
      const fv = spec.portalReports?.portal3a?.featureVerification;
      if (fv) {
        const missing = fv.featuresNotApplied || [];
        if (!fv.manifestPresent) {
          return warn('processingManifest not found — legacy job or assembly did not run manifest hooks yet');
        }
        if (fv.featuresOrdered?.length === 0) return skip('no enhanced features ordered (branding-only or clips job)');
        if (missing.length === 0) return pass(`all ${fv.featuresOrdered.length} ordered features confirmed delivered: ${fv.featuresOrdered.join(', ')}`);
        return fail(`${missing.length} feature(s) ordered but NOT delivered: ${missing.join(', ')} — ordered: ${fv.featuresOrdered.join(', ')}`);
      }
      // Fallback: check processingManifest directly
      const manifest = spec.state?.processingManifest;
      if (!manifest) return warn('processingManifest absent — assembly did not run manifest hooks yet');
      const ordered = manifest.featuresOrdered || [];
      const applied = manifest.featuresApplied || [];
      if (ordered.length === 0) return skip('no enhanced features ordered');
      const missing = ordered.filter((o) => !applied.some((a) => a === o || a.split(':')[0] === o.split(':')[0]));
      if (missing.length === 0) return pass(`all ${ordered.length} ordered features confirmed delivered`);
      return fail(`features ordered but NOT applied: ${missing.join(', ')}`);
    },
  },
  {
    id: 'chapters_documented', label: 'Chapter breakdown generated for score justification',
    weight: 3, implemented: true,
    run(spec) {
      // Chapters are required when there are multiple source clips or any ordered feature.
      const fv = spec.portalReports?.portal3a?.featureVerification;
      const chapters = fv?.chapters || spec.portalReports?.portal3a?.chapters || [];
      const segmentCount = spec.state?.savedOutputs?.segmentDurations?.length || 0;
      const orderedFeatures = fv?.featuresOrdered?.length || 0;
      const needsChapters = segmentCount > 1 || orderedFeatures > 0;

      if (!needsChapters) return skip('single-clip job with no ordered features — chapters not required');
      if (chapters.length === 0) {
        // No chapters yet means manifest wasn't written or portal3a didn't generate them
        const manifest = spec.state?.processingManifest;
        if (!manifest) return warn('chapters not generated — processingManifest absent (assembly did not run manifest hooks yet)');
        return warn(`chapters not generated despite ${segmentCount} clip segment(s) — portal3a may not have run yet`);
      }
      const chapterLines = chapters.map((c) => `${c.startTimestamp || '0:00'} ${c.title}`).join('; ');
      return pass(`${chapters.length} chapters documented: ${chapterLines}`);
    },
  },
];
...
function _featureActive(spec, key) {
  ...
  // CPD-QA: Direct spec fields set by developer_api.js from addOns
  // These are not stored under spec.addOns but directly on the spec.
  // Map grader feature keys to their direct spec path equivalents.
  const directFieldMap = {
    'captions.whisper': () => spec.captions?.active === true && (spec.captions?.style === 'animated' || spec.captions?.style === 'whisper'),
    'captions.burnin':  () => spec.captions?.active === true && spec.captions?.style === 'burnin',
    'captions':         () => spec.captions?.active === true,
    'audio.loudnorm':   () => spec.audioOpts?.loudnorm === true,
    'audio.duck':       () => spec.audioOpts?.duck === true,
    'color.lut':        () => !!(spec.colorGrade?.active && spec.colorGrade?.lutPath),
    'color.eq':         () => !!(spec.colorGrade?.active && (spec.colorGrade?.preset || spec.colorGrade?.eq)),
    'colorGrade':       () => spec.colorGrade?.active === true,
    'video.ken_burns':  () => spec.effects?.zoom === true || spec.effects?.ken_burns === true,
    'layout.portrait':  () => spec.effects?.layout?.portrait === true,
  };
  const check = directFieldMap[key];
  if (check) {
