...
function buildSystemPrompt(mode = 'full') {
  if (mode === 'guide') {
    return `You are the AuraFlux Copilot running in guide-confirm mode for an Operate plan customer.
...
Tone: concise, helpful, honest about guide limitations.`;
  ...
  // Full Copilot — Guided and Managed tiers
...
PLAN TIERS:
- diy: Operate — portals 0-3b, standard publish, guide-confirm Copilot
- dwy: Guided — full pipeline, full Copilot + SMS support
- dfy: Managed — all features, full Copilot + account manager
...
async function chatWithConcierge(messages, currentSpec = {}, opts = {}) {
  ...
  // Operate (diy) gets guide-confirm mode; Guided/Managed get full Copilot.
...
// ── CPD-121/122/123: Copilot schedule suggestions ─────────────────────────────
/**
 ...
 * @returns {Promise<string>}  Markdown response from Copilot
 */
async function suggestSchedule(opts = {}) {
  ...
  if (planTier === 'diy') {
    systemPrompt = `You are the AuraFlux Copilot in guide-confirm mode for an Operate plan customer.
...
Keep it brief — 3-5 bullet points maximum.`;
    ...
    systemPrompt = `You are the AuraFlux Copilot for a Guided plan customer.
...
You are AuraFlux — never mention Gemini or Google.`;
    ...
    systemPrompt = `You are the AuraFlux Copilot for a Managed plan customer.
