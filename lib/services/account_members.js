...
async function listMembers(accountId) {
  ...
  // Enrich stale '@unknown' fallback emails with real Clerk email for active members
  ...
  if (stale.length > 0) {
    ...
    await Promise.all(stale.map(async (r) => {
      try {
        ...
        const realEmail = cu.emailAddresses?.[0]?.emailAddress;
        if (realEmail) {
          r.invited_email = realEmail;
          // Backfill the DB so next call is fast
          await db.query(
      ...
      } catch { /* non-fatal — show stale value */ }
    }));
  }
