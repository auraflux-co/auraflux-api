...
function reportingRangeMatches(a, b) {
  if (!a || !b) return false;
...
async function buildChannelStatsReport({ handle = DEFAULT_HANDLE, refresh = false, startDate, endDate, days } = {}) {
  ...
  if (!refresh) {
    const cachedReport = await readCache(handle);
    const fetchedAt = cachedReport?.fetchedAt || cachedReport?.catalog?.fetchedAt;
    if (cachedReport?.ok && cachedReport?.catalog?.items?.length && fetchedAt) {
      const age = Date.now() - new Date(fetchedAt).getTime();
      if (age < CACHE_TTL_MS && reportingRangeMatches(cachedReport.reportingRange, reportingRange)) {
        return cachedReport;
      }
    }
  }
