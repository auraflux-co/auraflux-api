...
async function ringOnlineAgents({ inboundLeg, caller, auraLine }) {
  ...
  if (!agents.length) return { rang: 0 };
...
async function handleVoiceControlEvent(evt) {
  ...
  if (eventType === 'call.initiated' && direction === 'incoming') {
    try {
      ...
      if (!agents.length) {
        await notifySlack(
          `📞 Inbound call to *${auraLine}* from *${from}* — no agents online. <${takeUrl}|Open phone>`,
          process.env.SLACK_SMS_CHANNEL_ID,
        );
        return { handled: true, flow: 'inbound_no_agents' };
      }

      const { rang } = await ringOnlineAgents({ inboundLeg: callControlId, caller: from, auraLine });
      await notifySlack(
        `📞 Inbound call to *${auraLine}* from *${from}* — ringing ${rang} agent(s). <${takeUrl}|Open phone>`,
        process.env.SLACK_SMS_CHANNEL_ID,
      );
      return { handled: true, flow: 'inbound_webrtc_ring', rang };
    } catch (err) {
