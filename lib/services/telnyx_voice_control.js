...
/**
 * Place outbound call from user-selected AuraFlux line.
 */
...
async function handleVoiceControlEvent(evt) {
  ...
  if (eventType === 'call.initiated' && direction === 'incoming') {
    try {
      await notifySlack(`📞 Inbound call to *${to}* from *${from}*`);
  ...
  if (flow === 'outbound_operator') {
    try {
      ...
      const from = clientState.from;
      if (!dest || !from) throw new Error('missing destination or from line');
