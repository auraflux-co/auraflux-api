'use strict';
/**
 * lib/services/telnyx_voice_control.js
 *
 * Click-to-call via Telnyx Call Control:
 *   Outbound /call: ring operator phone → when answered, dial destination → bridge
 *   Inbound PSTN: answer → ring operator → bridge
 *
 * Audio is on the operator's phone (or WebRTC page in a future phase) — Slack is the UI.
 */
...
/**
 * Start outbound click-to-call: ring operator, then destination on answer.
 */
async function startOutboundClickToCall({ destination, slackUserId }) {
  const operator = getOperatorPhone();
  if (!operator) {
    throw new Error('SLACK_OPERATOR_PHONE not set — add your cell for click-to-call');
  }
  ...
  await notifySlack(`📞 Calling your phone — you'll be connected to *${destination}* when you answer.`);
...
async function handleVoiceControlEvent(evt) {
  ...
  try {
      ...
      const operator = getOperatorPhone();
      if (!operator) {
        await notifySlack(`📞 Inbound call from *${from}* — set SLACK_OPERATOR_PHONE to forward.`);
        return { handled: true, reason: 'no_operator_phone' };
      }
      await dialLeg({
        to: operator,
        clientState: {
          flow: 'inbound_operator',
          inboundLeg: callControlId,
          caller: from,
        },
      });
      await notifySlack(`📞 Incoming call from *${from}* → ringing operator.`);
      return { handled: true, flow: 'inbound' };
    } catch (err) {
