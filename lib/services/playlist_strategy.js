...
function matchPlaylistStrategy(rules, ctx = {}) {
  ...
  return {
    matched: false,
    reason: list.length
      ? 'No rule matched tags/audio bed — add a matching rule in Settings → Brand, or override playlist on publish.'
      : 'No playlist strategy rules on this brand — add rules in Settings → Brand.',
  };
}
