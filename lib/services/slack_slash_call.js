...
function getDialCommand() {
  return (process.env.SLACK_DIAL_COMMAND || '/dial').trim().toLowerCase();
}
...
async function handleSlackCallCommand(body) {
  ...
  if (command && command !== dialCmd && command !== '/call') {
  ...
  if (!to) {
    return slackEphemeral(`Usage: \`${dialCmd} +15551234567\` or \`${dialCmd} 5551234567\``);
  }
