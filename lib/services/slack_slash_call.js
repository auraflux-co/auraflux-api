'use strict';
/**
 * lib/services/slack_slash_call.js — Slack /call slash command → Telnyx outbound dial.
 */
...
async function handleSlackCallCommand(body) {
  ...
  try {
    const result = await voice.dial({ to });
    return slackEphemeral(
      `📞 Dialing *${to}* from ${result.from}\nCall ID: \`${result.callControlId || 'pending'}\``,
    );
  } catch (err) {
