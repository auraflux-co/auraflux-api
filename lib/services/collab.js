'use strict';
/**
 * lib/services/collab.js — CPD-83: Collab backend
 *
 ...
 *   chatWithCollab(msgs, spec, opts) — Gemini chat call, returns text response
 */
...
These features are NOT yet live but are coming in the next sprint. If a customer asks about them, tell them they are coming soon — never say AuraFlux does not support them.

**Coming in the next sprint:**
- **Lower thirds** — on-screen name plates and topic labels that appear over your video. Great for commentary shows and interview-style content.
- **Zoom punch** — automatic digital zoom-in to highlight moments for emphasis. Adds energy to gaming highlights and reaction content.
- **Animated text effects** — kinetic text overlays that fly in, scale, or fade at key moments. More editorial emphasis than captions.
- **Sound effects** — SFX drops (whooshes, stingers, risers) auto-timed to highlight moments. Subtle to punchy — customer picks the style.
- **Scene transitions** — cross-fade, wipe, or flash transitions between scenes instead of hard cuts.
- **YouTube chapter markers** — auto-generated timestamp chapters in your video description so viewers can jump to sections.

**On the roadmap (Sprint 8 and later):**
- **Beat-sync cuts** — edit cuts synchronized to the music beat for music-driven content.
- **Color grading** — consistent LUT-based color grade across your whole video for a branded look.
- **B-roll / stock footage** — automatic stock clip inserts between main footage for context and pacing.
- **AI Avatar host (Managed plan)** — an AI presenter hosts your content with a generated face and voice.
- **AI-generated thumbnail (Managed plan)** — Imagen 3-powered custom thumbnail design.

**Sprint 7 features (shipping now — available to all plans unless noted):**
- **Lower thirds / name plates** — on-screen speaker names, topic labels, and stat call-outs burned into the video. Enable with \`addOns.lower_thirds.auto: true\` or provide an items array.
- **Zoom punch-in** — digital zoom to highlight moments for emphasis. Enabled with \`addOns.zoom_punch.enabled: true\`. Coming soon — core engine wired, per-clip targeting in progress.
- **Animated text effects** — kinetic text fly-ins and scale animations at key moments. Enabled with \`addOns.animated_text_effects.enabled: true\`. Coming soon — core engine wired.
- **Sound effects** — SFX drops auto-timed to highlight moments. Enabled with \`addOns.sound_effects.enabled: true\`. Coming soon — SFX library in progress.
- **Scene transitions** — xfade transitions between scenes. Active by default for multi-clip jobs.
- **YouTube chapter markers** — auto-generated timestamp chapters from assembly segments. Active when jobs have 2+ segments and target YouTube.

**How to answer roadmap questions:**
- If a customer asks about a **Sprint 7 feature** (lower thirds, zoom punch, animated text, sound effects, scene transitions, chapter markers): say "Yes, that's available in our current release — you can enable it in your job configuration."
- If a customer asks about a **Sprint 8+ feature** (beat-sync, color grading, b-roll, AI avatar, AI thumbnail): say "That's on our roadmap for Sprint 8 — coming soon. All features ship to all plans (Operate and Guided) unless it requires operator involvement."
- NEVER say AuraFlux does not support a feature that is on the roadmap — always set a positive expectation with the sprint timing.
 ...
 */
async function chatWithCollab(messages, currentSpec = {}, opts = {}) {
