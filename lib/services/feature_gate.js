 ...
 *   growth = operate < guided < managed < custom
 */
...
const TIER_RANK = {
  ...
  growth: 1, operate: 1, guided: 2, managed: 3, custom: 99,
...
const FEATURE_PLANS = {
  ...
  'composition.near_final_preview': {
    min_plan:    'operate',
    label:       'Compose near-final preview',
    description: 'Peaks Review burns layout + look + FX stack before Create Short (C0 graduation iss_tY3Ii28wvWBK)',
  },
  ...
  'thumbnail.peak_picker': {
    min_plan:    'operate',
    label:       'Peak-frame thumbnail picker',
    description: 'Extract 3 high-candidate keyframes from audio/motion peaks for Shorts/Reels/TikTok cover',
  },
  'sponsor.markers': {
    min_plan:    'operate',
    label:       'Sponsor & ad marker detection',
    description: 'Scan transcripts for sponsor phrases, nudge trim boundaries, optional lower-third overlay',
  },
  'brand.creative_profile': {
    min_plan:    'operate',
    label:       'Brand creative profiles',
    description: 'Per-brand fonts, colors, watermark, caption style, and audio bed defaults',
  },
  'review.guest_share': {
    min_plan:    'operate',
    label:       'Guest review share links',
    description: 'Tokenized public review URLs for async guest approve / request re-cut',
  },
  'publish.playlist_strategy': {
    min_plan:    'operate',
    label:       'Playlist strategy routing',
    description: 'Auto-match publish playlists from tag / audio-bed rules',
  },
  'portal.web_research': {
    min_plan:    'operate',
 ...
 *
 * @param {string} planTier
 * @returns {Object}  e.g. { 'thumbnail.imagen': false, 'thumbnail.designed': true, ... }
 */
