...
/** Retry on backup GCP project when primary quota or OAuth credentials are unavailable. */
async function withQuotaFailover(run) {
  try {
    return await run('primary');
  } catch (err) {
    if (hasBackupProfile() && shouldFailoverToBackup(err)) {
      console.warn('[youtube_direct] primary unavailable — failing over to backup GCP project');
...
async function applyLiveBroadcastSeoDiscoverable(broadcastId, seo = {}, {
  ...
  const syntheticOff = String(process.env.LIVE_GRID_LIVE_SYNTHETIC_MEDIA ?? 'off').toLowerCase() !== 'on';
  if (syntheticOff) {
    try {
      await setContainsSyntheticMedia(broadcastId, false);
      log('containsSyntheticMedia → false');
    } catch (e) {
      log(`synthetic media flag skipped: ${e.response?.data?.error?.message || e.message}`);
    }
  }
...
async function applyLiveBroadcastSeo(broadcastId, seo = {}, { thumbnailPath, log = () => {}, playlistId, membersOnlyChat, setPublic } = {}) {
  ...
  if (syntheticOff) {
    try {
      await setContainsSyntheticMedia(broadcastId, false);
      log('containsSyntheticMedia → false (not AI-generated)');
      out.syntheticMedia = false;
    } catch (e) {
      log(`synthetic media flag skipped: ${e.response?.data?.error?.message || e.message}`);
    }
  }
...
/**
 * Remove a never-went-live broadcast from Studio (ready/created → delete).
 * Live/testing → endLiveBroadcast (complete, VOD kept).
 */
async function cancelLiveBroadcast(broadcastId) {
  if (!broadcastId) return { ok: false, reason: 'missing_id' };
  const st = await getBroadcastStatus(broadcastId);
  if (!st) return { ok: false, reason: 'not_found' };
  if (st.lifeCycleStatus === 'live' || st.lifeCycleStatus === 'testing') {
    await endLiveBroadcast(broadcastId);
    return { ok: true, action: 'ended', lifeCycleStatus: st.lifeCycleStatus };
  }
  return withQuotaFailover(async (profile) => {
    const accessToken = await getAccessTokenForProfile(profile);
    await axios.delete(`${YT_API}/liveBroadcasts?id=${encodeURIComponent(broadcastId)}`,
      { headers: { Authorization: `Bearer ${accessToken}` } });
    return { ok: true, action: 'deleted', lifeCycleStatus: st.lifeCycleStatus };
  });
}
...
/** Concurrent viewers while live — null if ended, hidden, or not yet live. */
async function getLiveConcurrentViewers(videoId) {
  if (!videoId) return null;
  return withQuotaFailover(async (profile) => {
    const accessToken = await getAccessTokenForProfile(profile);
    const res = await axios.get(`${YT_API}/videos?part=liveStreamingDetails,statistics&id=${videoId}`,
      { headers: { Authorization: `Bearer ${accessToken}` } });
    const v = res.data.items?.[0];
    if (!v) return null;
    const live = v.liveStreamingDetails || {};
    const concurrent = live.concurrentViewers != null
      ? parseInt(live.concurrentViewers, 10)
      : null;
    return {
      concurrentViewers: Number.isFinite(concurrent) ? concurrent : null,
      viewCount: parseInt(v.statistics?.viewCount || '0', 10),
      actualStartTime: live.actualStartTime || null,
      actualEndTime: live.actualEndTime || null,
      lifeCycleStatus: live.actualEndTime ? 'complete' : (live.actualStartTime ? 'live' : null),
    };
  });
}
