...
/**
 * Download Twitch avatar and upload to R2 (no DB write).
 * @returns {Promise<{ ok: boolean, imageUrl?: string, error?: string, twitchLogin?: string, displayName?: string }>}
 */
async function fetchTwitchAvatarToR2({ brandId, accountId, twitchLogin }) {
  if (!brandId || !accountId || !twitchLogin) {
    return { ok: false, error: 'brandId, accountId, twitchLogin required' };
  }
  try {
    ...
    logError('[brand_twitch_sync] fetchTwitchAvatarToR2 failed', err, { brandId, accountId, twitchLogin });
