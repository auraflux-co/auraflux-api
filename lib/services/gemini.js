...
/**
 * Multi-turn chat via Gemini. Accepts a `contents` array of
 * `{role, parts}` messages plus an optional `systemInstruction` string.
 ...
 */
async function callGeminiChat(contents, opts = {}) {
  const client = opts.model && opts.model !== DEFAULT_MODEL
    ? new GeminiClient(process.env.GEMINI_API_KEY, { model: opts.model })
    : getClient();
  return client.chat(contents, opts);
}
