...
async function generateConnectUrl(customerId, { redirectUrl, platforms } = {}) {
  ...
  if (platforms?.length) {
    ...
    // Give Upload-Post a moment to process the disconnect before generating
    // a new connect URL. Without this delay, the PKCE session may not be
    // cleared in time, producing "missing code or verifier" on the next attempt.
    if (didDisconnect) {
