...
async function generateConnectUrl(customerId, { redirectUrl, platforms } = {}) {
  ...
  if (platforms?.length) {
    ...
    for (const platform of platforms) {
      try {
        await disconnectPlatform(customerId, platform);
        didDisconnect = true;
        console.log(`[uploadpost] pre-disconnect ${platform} succeeded`);
      } catch (err) {
        console.warn(`[uploadpost] pre-disconnect ${platform} failed (non-fatal): ${err?.message}`);
      }
    }
    // Give Upload-Post time to propagate the disconnect before generating a new
    // connect URL. 600ms was occasionally too short; 1500ms gives their API
    // enough time to clear the PKCE session so the next connect starts clean.
    if (didDisconnect) {
