...
function startStreamScheduler({ baseUrl, windows = {}, getPersistedJobs } = {}) {
  if (String(process.env.STREAM_SCHEDULER || 'on').toLowerCase() === 'off') {
    log('disabled via STREAM_SCHEDULER=off');
    return null;
  }

  let calWindows = windows;
  try {
    const { getStreamWindows } = require('../calendar/slot_jobs');
    calWindows = getStreamWindows();
    if (windows.tv) calWindows.tv = windows.tv;
    if (windows.grid) calWindows.grid = windows.grid;
  } catch (_) {}
  ...
  const streamDefs = [
    {
      ...
      fallback: parseWindow(process.env.LIVE_TV_WINDOW, { start: 12 * 60, end: 18 * 60 }),
    },
    {
      name: 'live-grid',
      windowKey: 'grid',
      statusPath: '/live-grid/status', startPath: '/live-grid/start', stopPath: '/live-grid/stop',
      fallback: parseWindow(process.env.LIVE_GRID_WINDOW, { start: 18 * 60, end: 3 * 60 }),
    },
  ];
  ...
  function resolveWindows() {
    ...
    try {
      const { getStreamWindows } = require('../calendar/slot_jobs');
      calW = getStreamWindows();
    } catch (_) {}
    return streamDefs.map((def) => ({
      ...def,
      window: calW[def.windowKey] || def.fallback,
    }));
  }

  const streams = resolveWindows().map((def) => ({
    ...def,
    state: { lastStartKey: null, lastPrepareKey: null, lastPrepareRefresh: null, startAttempts: {}, wasInWindow: null },
  }));

  async function tick() {
    const now = nowET();
    let freshWindows = null;
    try {
      const { getStreamWindows } = require('../calendar/slot_jobs');
      freshWindows = getStreamWindows();
    } catch (_) {}
    for (const s of streams) {
      if (freshWindows) {
        s.window = freshWindows[s.windowKey] || null;
      }
      ...
      try {
        ...
        if (s.name === 'live-grid') {
          const {
            inPrepareWindow, loadPrepared, scheduleAheadEnabled, gridStartIsoFromMinutes,
          } = require('../live_grid/prepared_broadcast');
          if (scheduleAheadEnabled() && !running) {
            const prepKey = windowKey(now.minutes, s.window, now.dateKey);
            if (inPrepareWindow(now.minutes, s.window) && s.state.lastPrepareKey !== prepKey) {
              s.state.lastPrepareKey = prepKey;
              if (!loadPrepared()) {
                try {
                  let prepareBody = {};
                  try {
                    const { getLiveGridStartBody } = require('../calendar/slot_jobs');
                    prepareBody = getLiveGridStartBody(typeof getPersistedJobs === 'function' ? getPersistedJobs() : {});
                  } catch (_) {}
                  prepareBody.scheduledStartTime = gridStartIsoFromMinutes(s.window.start);
                  prepareBody.windowKey = prepKey;
                  await axios.post(`${baseUrl}/live-grid/prepare`, prepareBody);
                  log(`${s.name}: schedule-ahead prepare for window ${prepKey}`);
                } catch (e) {
                  log(`${s.name}: prepare failed — ${e.response?.data?.error || e.message}`);
                }
              } else if (s.state.lastPrepareRefresh !== prepKey) {
                s.state.lastPrepareRefresh = prepKey;
                try {
                  await axios.post(`${baseUrl}/live-grid/prepare/refresh`, {});
                  log(`${s.name}: refreshed prepared broadcast SEO/thumbnail`);
                } catch (e) {
                  log(`${s.name}: prepare refresh failed — ${e.response?.data?.error || e.message}`);
                }
              }
            }
          }
        ...
        if (action === 'start') {
          ...
          let startBody = {};
          try {
            const { getLiveTvStartBody, getLiveGridStartBody } = require('../calendar/slot_jobs');
            if (s.name === 'live-tv') {
              startBody = getLiveTvStartBody();
            } else if (s.name === 'live-grid') {
              const jobs = typeof getPersistedJobs === 'function' ? getPersistedJobs() : {};
              startBody = getLiveGridStartBody(jobs);
              try {
                const { loadPrepared, scheduleAheadEnabled } = require('../live_grid/prepared_broadcast');
                if (scheduleAheadEnabled() && loadPrepared()) startBody.usePrepared = true;
              } catch (_) {}
            }
          } catch (_) {
            if (s.name === 'live-tv') {
              try {
                ...
                if (curated?.videos?.length) {
                  startBody = { videos: curated.videos, curated: curated.curated !== false };
                }
              } catch (_) {}
            }
          }
          await axios.post(`${baseUrl}${s.startPath}`, startBody);
  ...
  for (const s of streams) {
    if (s.window) log(`${s.name}: window ${fmt(s.window.start)}–${fmt(s.window.end)} ET`);
    else log(`${s.name}: window=off — auto scheduling paused for today`);
  }
  ...
  return {
    status() {
      ...
      try {
        const { getStreamWindows } = require('../calendar/slot_jobs');
        fresh = getStreamWindows();
      } catch (_) {}
      return streams.map((s) => {
        const window = (fresh && fresh[s.windowKey]) || s.window;
        if (!window) {
          return {
            name: s.name,
            window: null,
            ...
            inWindow: false,
            next: null,
            pauseReason: fresh?.pauseReason || null,
          };
        }
        return {
          name: s.name,
          window,
          paused: false,
          inWindow: inWindow(now.minutes, window),
          next: nextBoundary(now, window),
        };
      });
    },
