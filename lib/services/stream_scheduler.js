...
function startStreamScheduler({ baseUrl, windows = {}, getPersistedJobs } = {}) {
  ...
  const streams = [
    {
      name: 'live-tv',
      window: calWindows.tv || parseWindow(process.env.LIVE_TV_WINDOW, { start: 12 * 60, end: 18 * 60 }),
    ...
    {
      name: 'live-grid',
      window: calWindows.grid || parseWindow(process.env.LIVE_GRID_WINDOW, { start: 18 * 60, end: 3 * 60 }),
