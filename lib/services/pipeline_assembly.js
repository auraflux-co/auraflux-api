...
async function runAssemblyAndPostProcess(spec, jobId, opts = {}) {
  ...
  if (spec.addOns?.branding?.active === true && !spec.addOns?.tts?.active && spec.assembledPath) {
    try {
      ...
      const { showName, streamerName } = _resolveChromeName(spec); // CPD-571
      ...
      await applyChrome(spec.assembledPath, chromedPath, {
        ...
        // Respect job spec chrome config — clip/extract jobs have hasTopBar=false by default
...
async function runTtsMixAndChrome(spec, jobId, ttsAudioPath, opts = {}) {
  ...
  try {
    ...
    // CPD-871: brand logo in TTS path — mirror the same download logic as non-TTS path
    let _ttsBrandLogoPath = null;
    if (spec.brandId && spec.customerId) {
      try {
        const { getBrand } = require('../db/postgres');
        const _ttsBrand = await getBrand(spec.brandId, spec.customerId);
        if (_ttsBrand?.image_url) {
          const _tmpLogo = require('path').join(require('os').tmpdir(), `brand_logo_tts_${spec.brandId}_${Date.now()}.png`);
          const _axios   = require('axios');
          const _resp    = await _axios.get(_ttsBrand.image_url, { responseType: 'arraybuffer', timeout: 15000 });
          require('fs').writeFileSync(_tmpLogo, _resp.data);
          _ttsBrandLogoPath = _tmpLogo;
          _log('brand logo downloaded for chrome overlay (TTS path, CPD-871)');
        }
      } catch (_logoErr) {
        _warn(`brand logo download failed in TTS path (using default) — ${_logoErr.message}`);
      }
    }
...
async function ensureChromeApplied(spec, jobId, opts = {}) {
  ...
  // CPD-586: only apply safety-net chrome when branding was explicitly ordered
  if (spec.addOns?.branding?.active !== true) return;
  ...
  try {
    ...
    // CPD-871: brand logo in safety-net chrome path
    let _safetyLogoPath = null;
    if (spec.brandId && spec.customerId) {
      try {
        const { getBrand } = require('../db/postgres');
        const _sbrand = await getBrand(spec.brandId, spec.customerId);
        if (_sbrand?.image_url) {
          const _tmpLogo = require('path').join(require('os').tmpdir(), `brand_logo_safety_${spec.brandId}_${Date.now()}.png`);
          const _axios   = require('axios');
          const _resp    = await _axios.get(_sbrand.image_url, { responseType: 'arraybuffer', timeout: 15000 });
          require('fs').writeFileSync(_tmpLogo, _resp.data);
          _safetyLogoPath = _tmpLogo;
        }
      } catch (_logoErr) {
        _warn(`brand logo download failed in safety-net chrome (using default) — ${_logoErr.message}`);
      }
    }
