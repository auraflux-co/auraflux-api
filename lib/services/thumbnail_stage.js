 ...
 * Four generation paths run in parallel and results are presented to the
 ...
 *     method:                'frame' | 'designed' | 'imagen' | 'custom' | null,
 ...
 */
async function extractCandidateFrames(videoPath, jobId, { planTier = 'operate' } = {}) {
  ...
  let offsets = FRAME_OFFSETS_PCT.map((pct, i) => ({
    offsetSeconds: Math.max(1, Math.floor(duration * pct)),
    score: parseFloat((1 - Math.abs(i - 2) / 4).toFixed(3)),
    method: 'frame',
  }));

  try {
    const { pickPeakThumbnailOffsets } = require('./thumbnail_peak_frames');
    if (isFeatureEnabled('thumbnail.peak_picker', planTier)) {
      const peakOffsets = await pickPeakThumbnailOffsets(videoPath, { planTier });
      if (peakOffsets.length) {
        // Prefer 3 peak candidates; keep designed/imagen paths separate
        offsets = peakOffsets.slice(0, 3);
      }
    }
  } catch (e) {
    logError('THUMBNAIL_PEAK_OFFSETS_FAIL', e, { jobId });
  }

  const frames = [];
  for (let i = 0; i < offsets.length; i++) {
    const { offsetSeconds: offset, score, method } = offsets[i];
    ...
    try {
      ...
      if (fs.existsSync(framePath) && fs.statSync(framePath).size > 500) {
        frames.push({
          ...
          score: score != null ? score : parseFloat((1 - Math.abs(i - 1) / 3).toFixed(3)),
          method: method || 'frame',
        });
      }
    } catch (e) {
 ...
 */
async function resolveAssembledVideoPath(jobSpec) {
  const jobId = jobSpec.jobId;
  let videoPath = jobSpec.state?.savedOutputs?.assembledVideoPath
  ...
  try {
    ...
    videoPath = await restoreAssembledPathFromCheckpoint(jobSpec, jobId);
    return videoPath;
  } catch (e) {
    logError('THUMBNAIL_R2_RESTORE_FAIL', e, { jobId });
    return null;
  }
}

/**
 * Ensure peak/frame candidates exist for creator Review (R2-safe).
 * If stage already has frame candidates and force=false, returns existing state.
 */
async function ensureThumbnailCandidates(jobSpec, { force = false, framesOnly = true } = {}) {
  const existing = jobSpec.state?.thumbnail;
  const frameCand = (existing?.candidates || []).filter((c) => (c.method || 'frame') === 'frame' || typeof c.index === 'number');
  if (!force && frameCand.some((c) => c.url)) {
    return { passed: true, outcome: 'thumbnail_already_ready', thumbnail: existing, reused: true };
  }
  return initiateApprovalStage(jobSpec, { framesOnly });
}

async function initiateApprovalStage(jobSpec, opts = {}) {
  const framesOnly = opts.framesOnly === true;
  const jobId     = jobSpec.jobId;
  const videoPath = await resolveAssembledVideoPath(jobSpec);
  ...
  // ── Frame extraction (local path or R2 restore) ─────────────────────────────
  if (videoPath && fs.existsSync(videoPath)) {
    try {
      const frames = await extractCandidateFrames(videoPath, jobId, {
        planTier: jobSpec.planTier || jobSpec.plan_tier || 'operate',
      });
    ...
    console.warn(`[thumbnail_stage:${jobId}] No assembled video (local or R2) — skipping frame extraction`);
  }

  // Creator peak-picker path: frames only (fast). Full pipeline still runs designed+Imagen.
  if (!framesOnly) {
    // ── Run designed + Imagen in parallel ─────────────────────────────────────
    const [designedResult, imagenResult] = await Promise.all([
...
module.exports = {
  ...
  ensureThumbnailCandidates,
  resolveAssembledVideoPath,
  approveThumbnail,
