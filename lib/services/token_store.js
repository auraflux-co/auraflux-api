...
/**
 * Save (upsert) OAuth tokens for a customer+platform pair.
 *
 * @param {object} p
...
async function saveTokens({
  ...
  await db.query(
    `INSERT INTO platform_oauth_tokens
       (customer_id, platform, access_token, refresh_token, token_expiry,
...
/**
 * Load decrypted tokens for a customer+platform pair.
 ...
 */
async function loadTokens(customerId, platform) {
  const res = await db.query(
    `SELECT access_token, refresh_token, token_expiry, scope,
            platform_user_id, platform_handle, raw_meta
...
/**
 * Remove stored tokens for a customer+platform (disconnect).
 */
async function deleteTokens(customerId, platform) {
...
/**
 * Check if a customer has a valid (non-expired) token for a platform.
 */
async function hasValidToken(customerId, platform) {
  const res = await db.query(
    `SELECT 1 FROM platform_oauth_tokens
      WHERE customer_id = $1 AND platform = $2
        AND (token_expiry IS NULL OR token_expiry > NOW() + INTERVAL '5 minutes')
      LIMIT 1`,
    [customerId, platform]
  );
...
/**
 * List all connected platforms for a customer (for settings UI display).
 */
async function listConnectedPlatforms(customerId) {
  const res = await db.query(
    `SELECT platform, platform_handle, platform_user_id, token_expiry, updated_at
       FROM platform_oauth_tokens
      WHERE customer_id = $1
