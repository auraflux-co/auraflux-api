...
function buildCopilotEstimate(credits, breakdown, balance) {
  ...
  const parts = [];
  if (breakdown.tts || breakdown.wan_t2v || breakdown.heygen_std || breakdown.heygen_iv || breakdown.shoppable) {
    const aiCost = breakdown.tts + breakdown.wan_t2v + breakdown.heygen_std + breakdown.heygen_iv + breakdown.shoppable;
