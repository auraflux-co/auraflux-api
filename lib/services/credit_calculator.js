 ...
 * @param {string}  [opts.planTier]      — 'operate' | 'guided' | 'managed' | 'custom' (CPD-128)
 *
 * @returns {{ credits: number, breakdown: object, discountFactor: number }}
 */
function calculateCreditCost({ durationMins = 1, aiFeature = null, addOns = {}, planTier = 'operate' }) {
 ...
 */
function deriveFeatures({ addOns = {}, sourceType = '', contentType = '', planTier = 'operate' }) {
  let aiFeature = null;
  ...
  const flatAddOns = {
    script:            !!(addOns?.script?.active === true),   // only charge when explicitly enabled by job spec or wizard
    ...
    imagen_thumbnail:  !!(addOns?.imagenThumbnail?.active) && planTier !== 'operate',
  };
...
function buildCollabEstimate(credits, breakdown, balance) {
  ...
  const flatTotal = breakdown.script + breakdown.research + breakdown.content_fetch +
    breakdown.narrative_clip + breakdown.imagen_thumbnail;
