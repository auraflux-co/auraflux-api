 ...
 * The Puppeteer functions in this file are @deprecated.
 * All chrome burns now go through lib/chrome_overlay_ffmpeg.js (pure FFmpeg drawtext/drawbox).
 * assembly.js no longer imports generateChromeOverlay or generateChromeOverlay from here.
 * burnSceneChromeFromDirective is still used for the USE_DIRECTIVE_CHROME code path.
 *
 * After 2 successful smoke test runs across all three content types, the Puppeteer
 * functions and their puppeteer dependency can be removed.
 *
 * DO NOT add new callers of generateChromeOverlay or generateChromeOverlay.
 */
...
// generateChromeOverlay          (was 11801)
...
async function generateTwitchLongformThumbnail(options = {}) {
  ...
  try {
    const data = JSON.parse(
      fs.readFileSync(path.join(__dirname, '..', 'data/streamers.json'), 'utf8')
    );
    roster = data.roster || [];
  } catch (e) {
  ...
  for (let i = 0; i < n; i++) {
    ...
    const imgSrc = fs.existsSync(localPath) ? localPath : streamer.profileImage || null;
    ...
    if (imgSrc) {
      try {
        ...
        console.warn(
          `[twitch-thumbnail] Profile image failed for ${streamer.displayName}: ${e.message}`
        );
      }
    } else {
      ctx.fillStyle = '#1a2540';
  ...
  ctx.shadowColor = 'rgba(0,0,0,0.9)';
  ctx.shadowBlur = 8;
  ctx.fillText('TWITCH SOUP', W / 2, H - 32);
  ...
  const outputPath = path.join(
    OUTPUT_DIR,
    `thumbnail_twitch_longform_ep${episodeNum}_${Date.now()}.png`
  );
  const buf = canvas.toBuffer('image/png');
  fs.writeFileSync(outputPath, buf);

  console.log(
    `[twitch-thumbnail] ✅ Generated: ${path.basename(outputPath)} (Episode ${episodeNum}, ${dateStr}, ${n} streamers)`
  );
...
async function generateNewsNbaThumbnail(options = {}) {
  const { createCanvas, loadImage } = require('canvas');
  const { contentType, episodeNum, date, title, storyImage, source } = options;
  ...
  if (backgroundImage) {
    const srcW = backgroundImage.width,
      srcH = backgroundImage.height;
    const scale = Math.max(W / srcW, H / srcH);
    const drawW = srcW * scale,
      drawH = srcH * scale;
    const drawX = (W - drawW) / 2,
...
async function burnSceneChromeFromDirective(scene, inputTs, asmId, jobId) {
  ...
  await new Promise((res, rej) => {
    ...
    proc.on('close', (code) =>
      code === 0 ? res() : rej(new Error(`Directive chrome burn failed: ${code}`))
    );
    proc.on('error', rej);
  });
  try {
...
async function generateChromeOverlayFromDirective(directive, context) {
  ...
  await _renderChromeOverlay(storyData, outputPath, storyIndex, {
    showLowerThird,
    hideSidebar,
    episodeNumber,
    activeCategory,
...
/**
 * @deprecated Use lib/chrome_overlay_ffmpeg.js buildAndBurnChrome() instead.
 * This Puppeteer-based renderer is superseded by the FFmpeg drawtext/drawbox engine.
 * Kept for reference during the 2-smoke-test migration period. Do not add new callers.
 */
async function _renderChromeOverlay(storyData, outputPath, storyIndex = 0, options = {}) {
  const {
    ...
    activeCategory = null,
    contentType = 'news',
    baselinePreset = process.env.CWN_OVERLAY_BASELINE_PRESET || '0415',
  } = options;
  ...
  try {
    ...
    await page.evaluate(
      async (data, activeIndex, opts) => {
        // Baseline lock: use the 2026-04-15 newsroom look as the default long-form preset.
        document.body.classList.remove('baseline-0415');
        if (String(opts.baselinePreset || '').trim() === '0415') {
          document.body.classList.add('baseline-0415');
        }
        ...
        if (skin) {
          ...
          root.style.setProperty('--navy', skin.navy);
        ...
        const BAD_TITLE_RE = /^(story|intro|segment|streamer|nba highlights|breaking news story)$/i;
        const BAD_CATEGORY_RE = /^(story|intro)$/i;
        const normalizeText = (v) =>
          (v == null ? '' : String(v)).replace(/[_-]+/g, ' ').replace(/\s+/g, ' ').trim();
        const cleanTitle = (candidate, fallback) => {
          const c = normalizeText(candidate).replace(/^story\s*\d+\s*/i, '');
          if (!c || BAD_TITLE_RE.test(c)) return normalizeText(fallback);
          return c;
        };
        const cleanCategory = (candidate, fallback) => {
          const c = normalizeText(candidate).toUpperCase();
          if (!c || BAD_CATEGORY_RE.test(c)) return normalizeText(fallback).toUpperCase();
          return c;
        };
        ...
        if (activeStory) {
          ...
          if (ltHeadline)
            ltHeadline.textContent = cleanTitle(
              activeStory.title,
              data.title || `${opts.activeCategory || 'SEGMENT'} 1`
            );
          const ltCat = document.querySelector('.lower-third .lt-category');
          if (ltCat)
            ltCat.textContent = cleanCategory(
              activeStory.category || opts.activeCategory,
              opts.activeCategory || 'WORLD NEWS'
            );
        } else {
        ...
        if (ltCategory) {
          ltCategory.textContent = cleanCategory(
            opts.activeCategory || data.category,
            'WORLD NEWS'
          );
        }
        ...
        if (segName && opts.activeCategory) {
          segName.textContent = cleanCategory(opts.activeCategory, 'WORLD NEWS');
        }
        ...
        if (data.allStories && data.allStories.length > 0) {
          ...
          if (storyList) {
            ...
            data.allStories.forEach((story, idx) => {
              ...
              const sTitle = cleanTitle(
                story.title || story.text,
                `${opts.activeCategory || 'SEGMENT'} ${idx + 1}`
              );
              const sCat =
                idx === activeIndex
                  ? '▶ ON AIR'
                  : cleanCategory(
                      story.category || opts.activeCategory,
                      opts.activeCategory || 'WORLD NEWS'
                    );
              storyItem.innerHTML = `
              <div class="story-item-cat">${sCat}</div>
              <div class="story-item-text">${sTitle}</div>
      ...
      storyData,
      storyIndex,
      { showLowerThird, hideSidebar, episodeNumber, activeCategory, contentType, baselinePreset }
    );
    ...
    console.log(
      `[newscast-overlay] ✅ Generated overlay (story ${storyIndex}, lowerThird=${showLowerThird}): ${outputPath}`
    );
  } finally {
    await browser.close();
  }
 ...
 * This Puppeteer-based renderer is superseded by the FFmpeg drawtext/drawbox engine.
 *
 * Universal chrome overlay generator — reads templateFile from jobSpec.designSpec.
 * Falls back to the default newscast template if designSpec is not available.
 */
async function generateChromeOverlay(jobSpec, storyData, outputPath, storyIndex = 0, options = {}) {
  // Read template from job spec if available, fall back to default newscast template
  const templateFile =
    jobSpec?.designSpec?.chrome?.templateFile || 'tools/clipzworld_newscast.html';
  const contentType =
    jobSpec?.baseContentType || jobSpec?.contentType || options.contentType || 'news';

  // Pass through to existing function with contentType from job spec
  return _renderChromeOverlay(storyData, outputPath, storyIndex, {
    ...options,
    contentType,
    templateFile, // Will be used when generateChromeOverlay is updated to read it
  });
}
