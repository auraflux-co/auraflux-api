 ...
 * assembly.js no longer imports generateNewscastOverlay or generateChromeOverlay from here.
 ...
 *
 * DO NOT add new callers of generateNewscastOverlay or generateChromeOverlay.
 */
...
async function generateNewscastOverlay(storyData, outputPath, storyIndex = 0, options = {}) {
  ...
  try {
    ...
    await page.evaluate(async (data, activeIndex, opts) => {
      ...
      const skinMap = {
        twitch: { navy: '#2a1f47', gold: '#7d5bbe', gold2: '#9b7de3', red: '#C9082A', showName: 'TWITCH SOUP' },
        nba:    { navy: '#102a58', gold: '#1a4fa8', gold2: '#4b7cc6', red: '#C9082A', showName: 'OTHER SIDE OF THE PILLOW' }
      };
      ...
      const normalizeText = (v) => (v == null ? '' : String(v)).replace(/[_-]+/g, ' ').replace(/\s+/g, ' ').trim();
      ...
      if (activeStory) {
        ...
        if (ltHeadline) ltHeadline.textContent = cleanTitle(activeStory.title, data.title || `${opts.activeCategory || 'SEGMENT'} 1`);
        const ltCat = document.querySelector('.lower-third .lt-category');
        if (ltCat) ltCat.textContent = cleanCategory(activeStory.category || opts.activeCategory, opts.activeCategory || 'WORLD NEWS');
      } else {
        // Fallback: use storyData directly
        const ltHeadline = document.querySelector('.lower-third .lt-headline');
        if (ltHeadline) ltHeadline.textContent = cleanTitle(data.title, `${opts.activeCategory || 'SEGMENT'} 1`);
      }
      ...
      if (ltCategory) {
        ltCategory.textContent = cleanCategory(opts.activeCategory || data.category, 'WORLD NEWS');
      }
      ...
      if (data.allStories && data.allStories.length > 0) {
        ...
        if (storyList) {
          ...
          data.allStories.forEach((story, idx) => {
            ...
            const sTitle = cleanTitle(story.title || story.text, `${opts.activeCategory || 'SEGMENT'} ${idx + 1}`);
            const sCat = idx === activeIndex
              ? '▶ ON AIR'
              : cleanCategory(story.category || opts.activeCategory, opts.activeCategory || 'WORLD NEWS');
            storyItem.innerHTML = `
      ...
      // ── Sidebar pagination ───────────────────────────────────────
      // paginateSidebar() is defined in the HTML template (clipzworld_newscast.html).
      // It applies the sliding window (default 5 visible items) and shows a "n / total"
      // indicator when there are more streamers than the window fits.
      if (typeof paginateSidebar === 'function') {
        paginateSidebar(activeIndex);
      }

    }, storyData, storyIndex, { showLowerThird, hideSidebar, episodeNumber, activeCategory, contentType, baselinePreset });
...
async function generateChromeOverlay(jobSpec, storyData, outputPath, storyIndex = 0, options = {}) {
  ...
  const templateFile = jobSpec?.designSpec?.chrome?.templateFile || 'tools/clipzworld_newscast.html';
  const contentType = jobSpec?.baseContentType || jobSpec?.contentType || options.contentType || 'news';

  // Pass through to existing function with contentType from job spec
  return generateNewscastOverlay(storyData, outputPath, storyIndex, {
    ...options,
    contentType,
    templateFile  // Will be used when generateNewscastOverlay is updated to read it
  });
}
