  ...
  try {
    ...
    await page.evaluate(async (data, activeIndex, opts) => {
      ...
      if (activeStory) {
        ...
        if (ltHeadline) {
            const raw = activeStory.title || data.title || 'Breaking News';
            ltHeadline.textContent = (opts.contentType === 'twitch' || opts.contentType === 'nba') ? raw.toUpperCase() : raw;
          }
        ...
        const ltHeadline = document.querySelector('.lower-third .lt-headline');
        if (ltHeadline) {
            const raw = data.title || 'Breaking News';
            ltHeadline.textContent = (opts.contentType === 'twitch' || opts.contentType === 'nba') ? raw.toUpperCase() : raw;
          }
      }
