 ...
 * @returns {string|null}
 */
function buildShakeFilter(shakes, { width = 1080, height = 1920 } = {}) {
  ...
  // CPD-1293: crop-for-shake must rescale back to target portrait (was shipping 1044×1884).
  return `crop=iw-${pad * 2}:ih-${pad * 2}:x='${pad}+(${xOsc})':y='${pad}+(${yOsc})',scale=${w}:${h}:flags=lanczos,setsar=1,format=yuv420p`;
}

/**
 * Combine optional zoom_punch + camera_shake into one -vf chain.
 */
function buildCameraFxFilter({
  zoomPunch = null,
  cameraShake = null,
  impactTint = null,
  width = 1080,
  ...
  const shake = buildShakeFilter(shakes, { width, height });
  if (shake) parts.push(shake);
  const flashes = normalizeImpactTints(impactTint);
  const tint = buildImpactTintFilter(flashes);
...
async function applyCameraFx(inputPath, outputPath, {
  ...
  const vf = buildCameraFxFilter({ zoomPunch, cameraShake, impactTint, width, height, fps });
  if (!vf) {
    const fs = require('fs');
  ...
  if (log) {
    ...
    const t = normalizeImpactTints(impactTint).length;
    log(`[camera-fx] CPD-1280/1283 applied (${p} punch, ${s} shake, ${t} tint flash)`);
  }
  return outputPath;
}
