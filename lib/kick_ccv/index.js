...
function buildVodUrl(slug, vodId) {
  ...
  const s = String(slug).trim().toLowerCase().replace(/^@/, '');
  const id = String(vodId).trim();
  if (!s || !id) return null;
  return `https://kick.com/${s}/videos/${id}`;
}

/** Clip window around peak for Peaks compose (seconds). */
function peakWindow(peakOffsetSec, { padBefore = 20, padAfter = 25 } = {}) {
  const peak = Math.max(0, Math.floor(Number(peakOffsetSec) || 0));
  const start = Math.max(0, peak - padBefore);
  const end = Math.max(start + 5, peak + padAfter);
  return { start_sec: start, end_sec: end, peak_sec: peak };
}
...
async function finalizeOfflineStreams(liveSlugs) {
  ...
  // Retry live/ended, plus linked rows missing ids or still carrying percent-encoded slugs from older buildVodUrl.
  const open = await dbQuery(
    `SELECT id, kick_slug, kick_stream_id, vod_id, vod_url, status
       FROM kick_ccv_streams
      WHERE status IN ('live', 'ended')
         OR (status = 'linked' AND (vod_id IS NULL OR vod_url IS NULL OR vod_url LIKE '%25%'))`,
  );
  const results = [];
  for (const row of open.rows || []) {
    ...
    // Livestream UUID from /users/livestreams becomes the VOD id after the stream ends.
    // Official Kick public API has no VOD list — capture while live is the only harden path.
...
async function listPeaksForBrand({ brandId, kickSlug, limit = 10 } = {}) {
  ...
  return (result.rows || []).map((r) => ({
    ...
    ...(() => {
      const w = peakWindow(r.peak_offset_sec);
      return { startSec: w.start_sec, endSec: w.end_sec, peakSec: w.peak_sec };
    })(),
  }));
}
