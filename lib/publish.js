...
function buildYoutubeTagExpansionPool({ streamers = [], title = '', contentType = '', seoKind = '' } = {}) {
  ...
  const isReaction = kind === 'reaction' || ct.includes('reaction');
  const streamerTags = [];
  const titleTags = [];
  const typeTags = [];
  ...
  if (isReaction) typeTags.push(...YOUTUBE_TAG_EXPANSION_BY_TYPE.reaction);
  ...
  const base = isReaction ? [] : YOUTUBE_TAG_EXPANSION_BASE;
...
function isTwitchClipShort(contentType, isShort = false, seoContext = null) {
  ...
  if (seoContext?.seoKind === 'reaction') return false;
...
function expandShortDescriptionIfNeeded(desc, { streamers = [], cc = {}, isShort = false, contentType = '', seoContext = null } = {}) {
  ...
  const isReaction = ctx.seoKind === 'reaction';
...
function ensurePublishMetadataComplete(metadata, { streamers = [], cc = {}, isShort = false, clipCount = 0, compCreative = null, contentType = '', clipCompBrief = null } = {}) {
  if (!metadata || typeof metadata !== 'object') return metadata;
...
function finalizeYoutubePublishMetadata(yt, cc = {}, opts = {}) {
  ...
  const isReactionSeo = seoCtx.seoKind === 'reaction';
  ...
  const isTwitchContent = !isReactionSeo && (ct.includes('twitch') || ct === 'clips' || ct.includes('streamer'));
...
function buildPublishCopySystemPrompt({
  ...
  const isReactionSeo = ctx.seoKind === 'reaction' || ctx.isReactionShort;
...
- The host (Bobby G) provides dry, deadpan commentary — titles and descriptions reflect this voice, not wire-copy` : '';
  ...
  const isTwitchClipShortForm = isShort && !isReactionSeo
...
- CPD-1260 MOMENT CONTRACT: burned hook + bestTitle + first 1–2 description sentences MUST sync on the same searchable keywords (streamer + guest/place/stunt/event). Search traffic depends on this; Shorts-feed-only joke titles underperform.` : '';
  ...
  const reactionShortSeoGuidance = isReactionSeo ? `
REACTION / PERFORMANCE SHORT — NOT TWITCH (mandatory):
...
- thumbnailTextIdeas: 2-5 words, mobile-readable, episode tease not Shorts hook copy` : '';
  ...
  const channelContext = isReactionSeo
...
${geminiBriefGuidance}${reactionShortSeoGuidance}${rankedListSeoGuidance}${nbaSeoGuidance}${clipCompSportsGuidance}${newsSeoGuidance}${twitchClipShortDescriptionGuidance}${twitchClipTitleGuidance}${twitchLongFormGuidance}
...
async function handleGeneratePublishCopy(req, res) {
  ...
  const isReactionSeo = seoContext.seoKind === 'reaction';
  ...
  // Build streamer credits block (Twitch only — skip for YouTube reaction / performance Shorts)
  const streamerCredits = !isReactionSeo
  ...
  const descriptorKey = isReactionSeo ? 'reaction' : baseContentType;
  ...
  const systemPrompt = buildPublishCopySystemPrompt({
    ...
    streamerCredits,
    chaptersBlock,
    contentType: isReactionSeo ? 'reaction' : baseContentType,
  ...
  try {
    ...
    ensurePublishMetadataComplete(metadata, {
      ...
      compCreative,
      clipCompBrief,
      contentType: isReactionSeo ? 'reaction-short' : (contentType || baseContentType),
    });
