...
const { resolvePublishSeoContext } = require('./publish_seo_context');
...
function _parseJsonFromModelText(text, label = 'publish-copy') {
  const trimmed = (text || '').trim();
  const jsonMatch = trimmed.match(/\{[\s\S]*\}/);
  if (!jsonMatch) throw new Error(`Could not parse JSON from ${label} response`);
  try {
    return JSON.parse(jsonMatch[0]);
  } catch (e1) {
    return JSON.parse(_repairJson(jsonMatch[0]));
  }
}

const GEMINI_PUBLISH_MODEL = process.env.GEMINI_PUBLISH_MODEL
  || process.env.GEMINI_MODEL
  || 'gemini-2.5-flash';

function _openAiQuotaError(err) {
  if (!err) return false;
  return err.status === 429
    || err.code === 'insufficient_quota'
    || /exceeded your current quota|insufficient_quota|rate limit/i.test(err.message || '');
}

async function _geminiChatJson(systemPrompt, userPrompt, label = 'publish-copy') {
  const apiKey = process.env.GEMINI_API_KEY;
  if (!apiKey) throw new Error('GEMINI_API_KEY not set — cannot fall back from OpenAI quota error');
  const url = `https://generativelanguage.googleapis.com/v1beta/models/${GEMINI_PUBLISH_MODEL}:generateContent?key=${apiKey}`;
  ...
  if (!text.trim()) throw new Error(`${label}: empty Gemini response`);
  console.warn(`[${label}] OpenAI unavailable — used Gemini ${GEMINI_PUBLISH_MODEL} fallback`);
  try {
    return _parseJsonFromModelText(text, label);
  } catch (parseErr) {
    throw new Error(`${label}: Gemini JSON parse failed (${parseErr.message})`);
  }
}
...
async function _generateMetadataJson(systemPrompt, label = 'publish-copy') {
  ...
  for (let attempt = 1; attempt <= 2; attempt++) {
    ...
    try {
      ...
      if (_openAiQuotaError(apiErr) && process.env.GEMINI_API_KEY) {
        try {
          return await _geminiChatJson(systemPrompt, 'Generate the metadata JSON now.', label);
        } catch (gemErr) {
          throw new Error(`OpenAI quota exceeded and Gemini fallback failed: ${gemErr.message}`);
        }
      }
      const isQuota429 = _openAiQuotaError(apiErr);
      if (isQuota429 && attempt < 2) {
        console.warn(`[${label}] OpenAI 429/quota on attempt ${attempt} — retrying in 8s (check billing if this persists)`);
...
- Output ONLY valid JSON, no markdown`;
  ...
  try {
    ...
    if (_openAiQuotaError(apiErr) && process.env.GEMINI_API_KEY) {
      const result = await _geminiChatJson(systemPrompt, 'Generate the short-form caption JSON now.', 'short-caption');
      if (Array.isArray(result.hashtags)) {
        result.hashtags = result.hashtags.map((h) => String(h).replace(/^#/, ''));
      }
      return result;
    }
    throw apiErr;
...
async function handlePublish(req, res) {
  ...
  const { strictPrivateOnly, resolveYouTubePrivacy, resolveTikTokPrivacy } = require('./publish_privacy');
  const jobSpecStub = { contentType, clipsOnly: !!req.body.clipsOnly };
  const effectivePrivacyStatus = strictPrivateOnly()
    ? resolveYouTubePrivacy(jobSpecStub, { privacyStatus })
    : (privacyStatus || 'private');
  const effectiveTiktokPrivacy = strictPrivateOnly()
    ? resolveTikTokPrivacy(jobSpecStub, { tiktokPrivacy }, scheduledAt)
  ...
  try {
    ...
    if (platforms.includes('youtube')) {
      ...
      form.append('privacyStatus', effectivePrivacyStatus || 'private');
    ...
    if (platforms.includes('tiktok')) {
      ...
      form.append('privacy_level', effectiveTiktokPrivacy);
...
function buildYoutubeTagExpansionPool({ streamers = [], title = '', contentType = '', seoKind = '' } = {}) {
  const ct = String(contentType || '').toLowerCase();
  const kind = String(seoKind || '').toLowerCase();
  const isSourceVideo = kind === 'source-video' || kind === 'reaction' || ct.includes('youtube');
  const streamerTags = [];
  const titleTags = [];
  const typeTags = [];
  ...
  if (isSourceVideo) typeTags.push(...YOUTUBE_TAG_EXPANSION_BY_TYPE.reaction);
  ...
  for (const name of names) {
    ...
    if (isSourceVideo) {
      streamerTags.push(lower, slug, `${lower} performance`, `${lower} reaction`, `${slug} viral`);
    } else {
  ...
  const base = isSourceVideo ? [] : YOUTUBE_TAG_EXPANSION_BASE;
  return [...streamerTags, ...titleTags, ...typeTags, ...base]
...
function isTwitchClipShort(contentType, isShort = false, seoContext = null) {
  if (!isShort) return false;
  if (seoContext?.seoKind === 'source-video' || seoContext?.sourceVideo) return false;
...
function expandShortDescriptionIfNeeded(desc, { streamers = [], cc = {}, isShort = false, contentType = '', seoContext = null } = {}) {
  let text = String(desc || '').trim();
  const ctx = seoContext || resolvePublishSeoContext({ contentType, streamers });
  const isSourceVideo = ctx.seoKind === 'source-video' || ctx.sourceVideo;
  const twitchClipShort = isTwitchClipShort(contentType, isShort, ctx);
  if (twitchClipShort) text = sanitizeTwitchClipShortCopy(text);
  if (!isShort || countWords(text) >= 125) return text;
  const names = (streamers || [])
    .map((s) => (typeof s === 'string' ? s : (s.displayName || s.streamer || s.username || '')))
    .filter(Boolean);
  const namesStr = names.length ? names.join(', ') : (ctx.subjectName || 'today\'s featured talent');
  const filler = isSourceVideo
    ? [
      `This YouTube Short captures a standout moment from ${namesStr}'s performance — the kind of viral beat viewers replay and share.`,
      'ClipzWorld News curates reaction and performance highlights into quick vertical Shorts so you catch the emotional peak without watching the full source video.',
      'From goosebumps vocals to unexpected twists, this Short focuses on the moment that made the clip worth sharing.',
      'Follow @clipzworldnews for more reaction highlights, talent moments, and viral performances.',
      'Subscribe on YouTube, follow us on TikTok and Instagram, and comment with your favorite part of this Short.',
      'Ring the bell so you never miss the next viral performance moment.',
    ].join('\n\n')
    : twitchClipShort
    ? [
...
function streamerDisplayToken(s) {
  if (typeof s === 'string') return s.split(/\s+/)[0];
  return String(s.displayName || s.streamer || s.twitchUsername || s.login || '').split(/\s+/)[0];
}

function titleMentionsStreamer(title, streamers = []) {
  const titleLower = String(title || '').toLowerCase();
  return streamers.some((s) => {
    const token = streamerDisplayToken(s).toLowerCase();
...
function ensureStreamerInTitle(title, streamers = []) {
  ...
  const first = streamerDisplayToken(streamers[0]);
  if (!first) return t;
...
function ensureStreamerInClipCompTitle(title, streamers = []) {
  ...
  const token = streamerDisplayToken(streamers[0]);
  if (!token) return t;
...
/** Long-form Twitch Soup episode title — possessive headline, never Shorts colon prefix. */
function ensureLongFormTwitchTitle(title, streamers = []) {
  return ensureStreamerInClipCompTitle(title, streamers);
}

/** Fill gaps so metadata QA passes before operator review. */
function applyClipCompYoutubeTitles(metadata, streamers, fixFn, { isShort = true } = {}) {
  const seen = new Set();
  const touch = (yt) => {
    ...
    if (Array.isArray(yt.titles)) {
      ...
      if (isShort && yt.titles[0] && !/#Shorts/i.test(yt.titles[0])) {
        yt.titles[0] = `${yt.titles[0]} #Shorts`;
      }
    }
  };
  ...
  if (typeof metadata.youtube === 'string') {
    ...
    metadata.youtube = isShort ? `${fixFn(bare, streamers)} #Shorts`.trim() : fixFn(bare, streamers);
  }
}

/** Long-form VOD titles — strip #Shorts, optional Twitch streamer possessive fix. */
function applyLongFormYoutubeTitles(metadata, streamers, { isTwitch = false } = {}) {
  const fixFn = isTwitch
    ? ensureLongFormTwitchTitle
    : (t) => String(t || '').trim().replace(/\s+#Shorts\s*$/i, '');
  applyClipCompYoutubeTitles(metadata, streamers, fixFn, { isShort: false });
}

function ensurePublishMetadataComplete(metadata, {
  streamers = [], cc = {}, isShort = false, clipCount = 0, compCreative = null,
  contentType = '', clipCompBrief = null, orderedClipUrls = [], segmentData = [], items = [],
} = {}) {
  if (!metadata || typeof metadata !== 'object') return metadata;
  ...
  const ct = String(metadata.contentType || '');
  const isTwitchShort = isShort && ct.includes('twitch');
  const isTwitchLongForm = !isShort && ct.includes('twitch');
  const isRankedList = !!(compCreative?.hooks?.rankedList?.enabled);
  const multiClipComp = isShort && clipCount >= 2 && !isRankedList;
  ...
  if (isTwitchShort) {
    applyClipCompYoutubeTitles(metadata, streamers, wrapTitleFix(ensureStreamerInClipCompTitle), { isShort: true });
  } else if (isTwitchLongForm) {
    applyLongFormYoutubeTitles(metadata, streamers, { isTwitch: true });
  } else if (!isShort) {
    applyLongFormYoutubeTitles(metadata, streamers, { isTwitch: false });
  } else {
    applyClipCompYoutubeTitles(metadata, streamers, wrapTitleFix(ensureStreamerInTitle), { isShort: true });
  }
...
function finalizeYoutubePublishMetadata(yt, cc = {}, opts = {}) {
  if (!yt) return;
  const seoCtx = opts.seoContext || resolvePublishSeoContext({
    contentType: opts.contentType,
    compCreative: opts.compCreative,
    streamers: opts.streamers,
  });
  const isSourceVideoSeo = seoCtx.seoKind === 'source-video' || seoCtx.sourceVideo;
  let desc = normalizeDescriptionNewlines(yt.description || '');
  if (isTwitchClipShort(opts.contentType, opts.isShort, seoCtx)) {
  ...
  const isTwitchContent = !isSourceVideoSeo && (ct.includes('twitch') || ct === 'clips' || ct.includes('streamer'));
  ...
  if (!Array.isArray(yt.hashtags) || yt.hashtags.length < 3) {
    yt.hashtags = opts.isShort
      ? (isSourceVideoSeo
        ? ['#Shorts', '#Reaction', '#Viral', '#AGT', '#ClipzWorldNews']
        : ['#Shorts', '#TwitchClips', '#FYP', '#StreamerLife', '#ClipzWorldNews'])
      : ['#TwitchSoup', '#TwitchClips', '#ClipzWorldNews', '#StreamerLife', '#Gaming'];
  }
  desc = appendSocialLinksBlock(desc, cc);
  if (opts.compCreative) {
    const { enrichPublishDescription } = require('./clip_comp_publish');
    desc = enrichPublishDescription(desc, opts.compCreative);
  }
...
function buildPublishCopySystemPrompt({
  ...
  const ctx = seoContext || resolvePublishSeoContext({
    contentType,
    clipCompBrief,
    compCreative,
  });
  const isSourceVideoSeo = ctx.seoKind === 'source-video' || ctx.sourceVideo;
...
- The host (Bobby G) provides dry, deadpan commentary — titles and descriptions reflect this voice, not wire-copy` : '';
  ...
  const isTwitchClipShortForm = isShort && !isSourceVideoSeo
    && (contentType === 'twitch' || contentType === 'clips' || (contentType || '').includes('twitch'));

  const geminiBriefGuidance = (clipCompBrief?.leadTitleDraft || (clipCompBrief?.clips || []).some((c) => c.observation)) ? `
...
- ${clipCompBrief.isComp && !(compCreative?.hooks?.rankedList?.enabled) ? 'Multi-clip comp — keep " and more..." in bestTitle when appropriate' : 'Use lead title draft framing — ranked lists use Top N / countdown language, not generic "and more..." only'}
- Use each clip's Observation lines for description bullets — facts only
- Burned hooks are on-screen overlays — paraphrase for YouTube title/description (never paste verbatim), but KEEP the same moment nouns
...
- CPD-1260 MOMENT CONTRACT: burned hook + bestTitle + first 1–2 description sentences MUST sync on the same searchable keywords (streamer + guest/place/stunt/event). Search traffic depends on this; Shorts-feed-only joke titles underperform.` : '';

  const sourceVideoSeoGuidance = isSourceVideoSeo ? `
SOURCE VIDEO SHORT — NOT TWITCH (mandatory — applies to any YouTube/VOD-sourced clip, any Compose preset):
- Source is ${ctx.sourcePlatform === 'youtube' ? 'a YouTube video / performance clip' : 'edited reaction footage'} — NOT a Twitch live stream
- NEVER say "Twitch stream", "live on stream", "chat went crazy", "VOD footage", or "featured streamers on Twitch" unless explicitly true in the Gemini observations
- Category should be Viral Entertainment or Podcast — NOT Twitch unless the source is clearly Twitch gameplay
- Title + description must name the ACTUAL moment (performance, audition, song, reaction beat) from Gemini observations below
- Tags: talent name, event/show, song/performance type, reaction keywords — NOT "twitch clips", "twitch soup", "streamer drama"
- Featured talent: credit ${ctx.subjectName || 'the performer'} — do NOT invent twitch.tv URLs for YouTube-only sources
- If Gemini observations are present below, they are ground truth — OpenAI must not override with generic Twitch compilation copy` : '';

  const geminiObservationBlock = ctx.hasGeminiObservations && ctx.geminiObservationBlock
    ? `\n\nGEMINI VIDEO OBSERVATIONS (ground truth — description + tags MUST match these facts):\n${ctx.geminiObservationBlock}\n`
    : '';

  const rankedListSeoGuidance = (isShort && compCreative?.hooks?.rankedList?.enabled) ? (() => {
    const rl = compCreative.hooks.rankedList;
    const slotCount = Math.max(2, Number(rl.slotCount) || 5);
    const streamer = String(rl.streamer || clipCompBrief?.leadStreamer || 'Streamer').trim();
    const theme = String(rl.theme || 'FUNNIEST').trim();
    let { buildRankedListHeader, buildRankedListTitleDraft } = require('./clip_comp_titles');
    const header = buildRankedListHeader(compCreative);
    const titleSeed = buildRankedListTitleDraft(compCreative) || clipCompBrief?.leadTitleDraft || '';
    return `
STREAM SERPENT RANKED LIST — OVERRIDES GENERIC COMP TITLE RULES (mandatory):
- Video is a Top ${slotCount} ranked countdown of ${streamer}'s ${theme} Twitch moments — viewer waits for #1
- On-screen burned header: "${header}"
- bestTitle.title MUST use ranked framing like: "${titleSeed}" — include Top ${slotCount}, streamer name, and countdown/wait-for-#1 hook
- Do NOT use a single-moment title ("${streamer}'s Mirror Revelation and more...") — this is a ranked list Short
- Description first sentence MUST mention Top ${slotCount} / ranked / funniest moments / countdown — then tease #1 payoff
- hookMoment should describe why #1 is worth waiting for, not just one clip from the middle of the list`;
  })() : '';
...
- Description tone: quick highlight reel / viral moments — not a show recap or episode summary` : '';
  ...
  const twitchClipTitleGuidance = isTwitchClipShortForm ? `
...
WINNERS (500+ views + Search): ONE specific searchable moment — streamer name natural in the phrase (not "Name:" prefix).
  ✓ "ExtraEmily Stops Traffic with YourRage" · "ExtraEmily's Napkin Moment with Kai Cenat" · "ExtraEmily DreamCon Disguise Fail"
  ✓ "ExtraEmily's Rave Outfit Stuns Viewers" · "Wrong Shirt Gift Gone Wrong"
  ✓ "FunnyMike's Father's Day Chocolate Chaos" · "FunnyMike's Cheapest iPad Hunt" · "FunnyMike Making It Rain"
  ✓ "Youngin's Reaction to FunnyMike" · "FunnyMike's Before and After Streamer U Interview"
  ✗ "ExtraEmily: Back for old clips" · "jasontheween: ExtraEmily's Rave Outfit" · copying Twitch clip titles verbatim
  ✗ Vague feed-only jokes that drop event/guest nouns (hurts YouTube Search; pack stays Shorts-feed dependent)
...
- CPD-1260: first 150 chars of description = engaging open that repeats the title's moment keywords (streamer + guest/place/stunt)
...
- thumbnailTextIdeas: same rule — 2-5 words, one moment, never generic compilation filler` : '';
  ...
  const isTwitchLongForm = !isShort && (contentType === 'twitch' || contentType === 'clips' || (contentType || '').includes('twitch'));

  const twitchLongFormGuidance = isTwitchLongForm ? `
TWITCH SOUP LONG-FORM EPISODE — YOUTUBE TITLE RULES (critical):
- This IS a Twitch Soup episode on ClipzWorld News — NOT a YouTube Short. NEVER append #Shorts to any title
- Title the lead moment with streamer possessive ("Jason's Spelling Bee Slip Stuns the Crew") — NEVER "Name: moment" colon prefix
- Multi-streamer episode: lead with the #1 clip's moment; imply more chaos ("... and More Wild Moments" or "| Twitch Soup")
- WINNERS: "ExtraEmily's Wrong Shirt Gift Goes Viral | Twitch Soup" · "Spell Check Disaster Leaves Jason Speechless"
- LOSERS: generic "Best Twitch Moments" · "[Streamer]: [verbatim clip title]" · anything with #Shorts
- Description MUST follow long-form Twitch Soup structure (Welcome to ${cc.showName}, per-streamer breakdown, timestamps if provided)
- thumbnailTextIdeas: 2-5 words, mobile-readable, episode tease not Shorts hook copy` : '';

  const channelContext = isSourceVideoSeo
    ? `ClipzWorld News (YouTube: ${cc.youtubeUrl || cc.handle}, TikTok: ${cc.tiktokUrl || cc.tiktokHandle}, Instagram: ${cc.instagramUrl || cc.instagramHandle}) — reaction / performance Shorts curated from source video`
    : isTwitchClipShortForm
...
${geminiBriefGuidance}${sourceVideoSeoGuidance}${rankedListSeoGuidance}${nbaSeoGuidance}${clipCompSportsGuidance}${newsSeoGuidance}${twitchClipShortDescriptionGuidance}${twitchClipTitleGuidance}${twitchLongFormGuidance}
Creative brief + script excerpt (up to 4000 chars):
${scriptExcerpt}${geminiObservationBlock}
${streamerCredits ? `\nStreamers featured:\n${streamerCredits}` : ''}
${intelligenceBlock ? `\nHISTORICAL PERFORMANCE (Content Memory — adapt patterns that drove views, do not copy verbatim):\n${intelligenceBlock}\n` : ''}
${chaptersBlock}
...
function buildPublishScriptFromCard(card = {}) {
  if (card.clipCompBrief) {
    try {
      ...
      const seo = buildClipCompSeoInput(card.clipCompBrief, card.compCreative || null);
      if (seo && seo.trim()) return seo.trim();
...
async function handleGeneratePublishCopy(req, res) {
  ...
  if (clipCompBrief) {
    try {
      const { buildClipCompSeoInput } = require('./clip_comp_hooks');
      scriptExcerpt = buildClipCompSeoInput(clipCompBrief, compCreative).slice(0, 4000);
  ...
  const seoContext = resolvePublishSeoContext({
    ...
    streamers,
    items,
    orderedClipUrls: req.body.orderedClipUrls || [],
    segmentData: req.body.segmentData || [],
  });
  const isSourceVideoSeo = seoContext.seoKind === 'source-video' || seoContext.sourceVideo;
  ...
  // Build streamer credits block (Twitch only — skip for YouTube / source-video Shorts)
  const streamerCredits = !isSourceVideoSeo
    && (baseContentType === 'twitch' || (contentType || '').includes('twitch'))
    && streamers.length
  ...
  const contentDescriptors = {
    twitch: isShort
      ? `Twitch clips compilation featuring: ${streamers.join(', ') || 'multiple streamers'}`
      : `Twitch Soup long-form episode featuring streamers: ${streamers.join(', ') || 'multiple streamers'}`,
    youtube: isShort
      ? `YouTube-sourced Short featuring: ${seoContext.subjectName || streamers.join(', ') || 'featured talent'} — describe the actual video moment, NOT a Twitch stream`
      : `YouTube / source video highlight featuring: ${seoContext.subjectName || streamers.join(', ') || 'featured talent'}`,
    reaction: isShort
      ? `Reaction / performance YouTube Short featuring: ${seoContext.subjectName || streamers.join(', ') || 'featured talent'} — describe the actual video moment, NOT a Twitch stream`
      : `Reaction / performance highlight featuring: ${seoContext.subjectName || streamers.join(', ') || 'featured talent'}`,
    nba:    buildSportsContext(items),
    sports: buildSportsContext(items),
    news:   buildNewsContext(items),
  };
  const descriptorKey = isSourceVideoSeo ? 'youtube' : baseContentType;
  const cd = contentDescriptors[descriptorKey] || contentDescriptors.news;
  ...
  const systemPrompt = buildPublishCopySystemPrompt({
    ...
    streamerCredits,
    chaptersBlock,
    contentType: isSourceVideoSeo ? 'youtube' : baseContentType,
  ...
  try {
    ...
    ensurePublishMetadataComplete(metadata, {
      ...
      compCreative,
      clipCompBrief,
      contentType: isSourceVideoSeo ? 'youtube-short' : (contentType || baseContentType),
    });
...
async function generatePublishCopyFromScript({ script, contentType, formType, items = [], jobId, platforms = ['youtube', 'tiktok', 'instagram'], designSpec = {} }) {
  ...
  const contentDescriptors = {
    twitch: isShort
      ? `Twitch clips compilation featuring: ${streamers.map(s => (typeof s === 'object' ? s.displayName || s.title : s) || '').filter(Boolean).join(', ') || 'multiple streamers'}`
      : `Twitch Soup long-form episode featuring streamers: ${streamers.map(s => (typeof s === 'object' ? s.displayName || s.title : s) || '').filter(Boolean).join(', ') || 'multiple streamers'}`,
  ...
  const primaryStreamer = streamers.length
    ? (typeof streamers[0] === 'object'
      ? (streamers[0].displayName || streamers[0].streamer || streamers[0].login || '')
      : String(streamers[0]))
    : null;
  ...
  try {
    ...
    const intel = intelligence.getPublishIntelligenceContext({
      contentType: baseContentType,
      streamer: primaryStreamer || null,
      formFactor: isShort ? 'short' : 'long',
    });
    intelligenceContext = intel.intelligenceContext;
    intelligenceBlock = intel.promptBlock || intelligenceContext.promptBlock || '';
  } catch (e) {
    console.warn(`[publish-copy/g1] intelligence context skipped: ${e.message}`);
  }
  ...
  finalizeYoutubePublishMetadata(metadata.youtube, cc, { streamers, isShort, contentType: contentType || baseContentType });
  ensurePublishMetadataComplete(metadata, {
    streamers,
    cc,
    isShort,
    clipCount: items.length,
    compCreative,
    contentType: contentType || baseContentType,
  });
  ...
  if (jobId) {
    try {
      const intelligence = require('./intelligence');
      intelligence.recordPublishGenerationDecisions(jobId, metadata, intelligenceContext);
    } catch (_dec) { /* non-fatal */ }
  }
...
module.exports = {
  ...
  resolvePublishContentType: require('./publish_seo_context').resolvePublishContentType,
};
