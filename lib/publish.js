...
function ensurePublishMetadataComplete(metadata, { streamers = [], cc = {}, isShort = false, clipCount = 0, compCreative = null } = {}) {
  if (!metadata || typeof metadata !== 'object') return metadata;
...
${geminiBriefGuidance}${rankedListSeoGuidance}${nbaSeoGuidance}${clipCompSportsGuidance}${newsSeoGuidance}${twitchClipShortDescriptionGuidance}${twitchClipTitleGuidance}
...
async function generatePublishCopyFromScript({ script, contentType, formType, items = [], jobId, platforms = ['youtube', 'tiktok', 'instagram'], designSpec = {} }) {
  ...
  ensurePublishMetadataComplete(metadata, { streamers, cc, isShort, clipCount: items.length, compCreative });
