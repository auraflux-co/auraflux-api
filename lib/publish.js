...
Rules:
- caption: 90-150 chars, starts with the most compelling fact or hook
...
- hashtags: mix of broad (Shorts, FYP, ForYou) + topic-specific; no # prefix in array values
...
/** Post-process YouTube metadata — hashtags in description, social links, metrics. */
...
function ensurePublishMetadataComplete(metadata, { streamers = [], cc = {}, isShort = false } = {}) {
  ...
  if (!metadata.tiktok.caption || metadata.tiktok.caption.length < 40) {
    metadata.tiktok.caption = `${hook}\n\n${cc.tiktokUrl || 'https://www.tiktok.com/@clipzworldstreams'}`;
  }
...

{
  ...
  "tiktok": {
    "caption": "<90-150 chars hook. End with ${cc.tiktokUrl || cc.tiktokHandle}. 4-6 hashtags inline.>"
  }` : ''}${needsInstagram ? `,
  "instagram": {
    "caption": "<Hook in first 125 chars. Full description with line breaks. End with ${cc.instagramUrl || cc.instagramHandle} + 10-15 hashtags.>"
  }` : ''}
}
