...

{
  ...
  "youtube": {
    ...
    "playlistDescription": "<60-80 word playlist description for '${cc.showName}' YouTube playlist. Tone: dry, punchy, ${cc.host} voice.>",
...
- Bobby G voice: flat/dry, no hype words, short sentences — never explain the joke
- Channel is ${cc.handle} — never use any other handle`;
  ...
  try {
    ...
    if (
      metadata.youtube &&
      Array.isArray(metadata.youtube.titles) &&
      metadata.youtube.titles.length
    ) {
    ...
    if (needsYouTube && metadata.youtube) {
      if (metadata.youtube.title && metadata.youtube.title.length > 100) {
        console.warn(
          `[publish-copy] YouTube title too long (${metadata.youtube.title.length} chars), truncating...`
        );
        metadata.youtube.title = metadata.youtube.title.substring(0, 97) + '...';
      }
...
async function generatePublishCopyFromScript({
  ...
  const signOff = isShortScript ? 'Subscribe. Appreciate you.' : "I'm Bobby G. See you tomorrow. — ClipzWorld News";
  const voiceRules = `\nBOBBY G VOICE RULES:\n- Show: "${cc.showName}" on ClipzWorld News\n- Host: Bobby G — dry, flat delivery. Jon Stewart crossed with Norm MacDonald.\n- Never use: "incredible", "amazing", "insane", "wild", "crazy", "epic", "unbelievable"\n- Short declarative sentences. State the fact. Move on.\n- Never explain the joke. Let the content speak.\n- Descriptions end with exactly: "${signOff}"\n- Channel handle: ${cc.handle}`;
...

{
  ...
  "youtube": {
    ...
    "description": "<Full YouTube description: 200-350 words. Lead with the hook moment. Summarise each segment/streamer/story in 1-2 sentences. Include subscribe CTA. End with ${cc.handle}. Use line breaks for readability.${streamerCredits ? ' Include streamer Twitch URLs section.' : ''}>",
    "playlistDescription": "<60-80 word playlist description for '${cc.showName}' YouTube playlist. Tone: dry, punchy, ${cc.host} voice.>",
    "tags": [<10-15 YouTube search tags as strings, no # prefix, mix broad + specific>],
    "hashtags": [<5-8 hashtags WITH # prefix for description footer, platform-appropriate>],
    "thumbnailTextIdeas": [
