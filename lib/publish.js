...
Rules:
- caption: 90-150 chars, starts with the most compelling fact or hook; name at least one streamer when script features streamers
...
/** TikTok: hook + optional line + dense hashtags + profile URL. */
function buildTikTokCaption({ hook, synopsis = '', hashtags = [], cc = {} } = {}) {
  const hookLine = String(hook || '').replace(/#\w+/g, '').trim().slice(0, 160);
  const syn = String(synopsis || '').replace(/#\w+/g, '').trim().split('\n')[0].slice(0, 200);
  const tagLine = formatHashtagLine(hashtags.slice(0, 14));
  const url = cc.tiktokUrl || CHANNEL_SOCIAL.tiktok.url;
  return [hookLine, syn, tagLine, url].filter(Boolean).join('\n\n');
}
...
function finalizeShortSocialCaptions(metadata, { streamers = [], cc = {}, contentType = 'twitch' } = {}) {
  ...
  const hook = yt.title || yt.bestTitle?.title || '';
  const synopsis = (yt.description || '').split('\n\n')[0] || '';
  const ytTags = (yt.hashtags || []).map((h) => String(h).replace(/^#/, ''));
  const pack = buildSocialHashtagPack({ streamers, ytHashtags: ytTags, contentType, max: 22 });

  if (!metadata.tiktok) metadata.tiktok = {};
  if (!metadata.instagram) metadata.instagram = {};

  metadata.tiktok.caption = buildTikTokCaption({ hook, synopsis, hashtags: pack, cc });
  metadata.instagram.caption = buildInstagramCaption({ hook, synopsis, hashtags: pack, cc });
...
function expandShortDescriptionIfNeeded(desc, { streamers = [], cc = {}, isShort = false } = {}) {
...
function ensurePublishMetadataComplete(metadata, { streamers = [], cc = {}, isShort = false } = {}) {
  if (!metadata || typeof metadata !== 'object') return metadata;
  const flat = normalizePublishCopyShape(metadata);
  Object.assign(metadata, flat);
  if (!metadata.youtube) metadata.youtube = {};
  ...
  yt.title = ensureStreamerInTitle(yt.title || baseTitle, streamers);
  if (yt.bestTitle?.title) yt.bestTitle.title = ensureStreamerInTitle(yt.bestTitle.title, streamers);
  if (Array.isArray(yt.titles) && yt.titles[0]) yt.titles[0] = yt.title;

  finalizeYoutubePublishMetadata(yt, cc, { streamers, isShort });
...
- The host (Bobby G) provides dry, deadpan commentary — titles and descriptions reflect this voice, not wire-copy` : '';
  ...
  return `You are a YouTube Growth Strategist, News Editor, SEO Specialist, and Viral Content Producer working for ClipzWorld News (show: ${cc.showName}, host: ${cc.host}, YouTube: ${cc.youtubeUrl || cc.handle}, TikTok: ${cc.tiktokUrl || cc.tiktokHandle}, Instagram: ${cc.instagramUrl || cc.instagramHandle}).
...
${nbaSeoGuidance}${clipCompSportsGuidance}${newsSeoGuidance}
...

{
  ...
  "youtube": {
    ...
    "description": "${isShort
      ? `YouTube Shorts SEO description — REQUIRED 125-250 words (never a one-liner). Structure: (1) First 150 characters = engaging hook + primary keyword (search snippet). (2) 150-200 words on streamers/moments/context with secondary keywords woven naturally. (3) CTA (subscribe, comment, follow). (4) Plain URLs: ${cc.youtubeUrl || cc.handle}, ${cc.tiktokUrl || cc.tiktokHandle}, ${cc.instagramUrl || cc.instagramHandle}. (5) End with exactly 3-5 hashtags INLINE (#Shorts #TwitchClips etc.) — duplicate the same tags in the hashtags array.`
  ...
  "tiktok": {
    "caption": "<TikTok ONLY — 90-160 char hook naming streamers, then blank line, then 12-15 hashtags inline (#FYP #TwitchClips #StreamerName etc.), then blank line, then ${cc.tiktokUrl || cc.tiktokHandle}. NEVER paste the YouTube description here.>"
  }` : ''}${needsInstagram ? `,
  "instagram": {
    "caption": "<Instagram Reels ONLY — hook in first 125 chars naming streamers, 1-2 sentence synopsis (NOT the full YT essay), blank line, 15-20 hashtags inline, blank line, ${cc.instagramUrl || cc.instagramHandle}. NEVER paste the YouTube description verbatim.>"
  }` : ''}
}
...
async function generatePublishCopyFromScript({ script, contentType, formType, items = [], jobId, platforms = ['youtube', 'tiktok', 'instagram'], designSpec = {} }) {
  ...
  ensurePublishMetadataComplete(metadata, { streamers, cc, isShort });
