...
async function handleGeneratePublishCopy(req, res) {
  ...
  // Channel config: show names, host, handle
  const channelConfig = {
    twitch: { showName: 'Twitch Soup', handle: '@clipznashite', host: 'Bobby G' },
    nba:    { showName: 'Other Side of the Pillow', handle: '@clipznashite', host: 'Bobby G' },
    news:   { showName: 'Because the Light Was On', handle: '@clipznashite', host: 'Bobby G' },
  };
  const cc = channelConfig[contentType] || channelConfig.news;
  ...
  const streamerCredits = contentType === 'twitch' && streamers.length
    ? streamers.map(s => {
        ...
        const display = entry ? (entry.displayName || s) : s;
        return `• ${display}: ${url}`;
      }).join('\n')
    : '';
