...
async function generateShortFormCaption(script, contentType) {
  ...
  const systemPrompt = `You write ultra-short social captions for ClipzWorld News (@clipzworldnews) short-form vertical videos (≤60 seconds).
...
- NEVER mention file paths, HTML files, or internal tool names — use @clipzworldnews if referencing the channel
...
- YouTube Shorts description: under 120 chars, factual hook + ${cc.handle} only` : '';
...
- Shorts descriptions: factual hook + ${cc.handle} — never "Only on tools/..." or similar
...
async function handleGeneratePublishCopy(req, res) {
  ...
  let channelConfig = {
    ...
    clips:  { showName: 'Twitch Soup', handle: '@clipzworldnews', tiktokHandle: '@clipznashite', host: 'ClipzWorld News' },
    sports: { showName: 'Other Side of the Pillow', handle: '@clipzworldnews', tiktokHandle: '@clipznashite', host: 'ClipzWorld News' },
    news:   { showName: 'Because the Light Was On', handle: '@clipzworldnews', tiktokHandle: '@clipznashite', host: 'ClipzWorld News' },
  };
  try {
