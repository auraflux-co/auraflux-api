...
- The host (Bobby G) provides dry, deadpan commentary — titles and descriptions reflect this voice, not wire-copy` : '';
  ...
  return `You are a YouTube Growth Strategist, News Editor, SEO Specialist, and Viral Content Producer working for ClipzWorld News (show: ${cc.showName}, host: ${cc.host}, handle: ${cc.handle}).
...

{
  ...
  "youtube": {
    ...
    "description": "${isShort ? '1-2 punchy sentences (under 120 chars total). Hook first, then ' + cc.handle + '. Hashtags go in the hashtags field — NOT inline here.' : 'YouTube description: open with primary keywords in the first sentence; summarize the video; include EVERY important name, event, organization, team, and streamer; subscribe CTA; a question that encourages comments; end with 3-5 hashtags on the last line. 250-400 words.'}",
...
async function generatePublishCopyFromScript({ script, contentType, formType, items = [], jobId, platforms = ['youtube', 'tiktok', 'instagram'], designSpec = {} }) {
  ...
  const pcResponse = await openai.chat.completions.create({
    model: 'gpt-4o',
    max_tokens: 6000,
    messages: [
      { role: 'system', content: systemPrompt },
  ...
  const text = (pcResponse.choices[0]?.message?.content || '').trim();
  const jsonMatch = text.match(/\{[\s\S]*\}/);
