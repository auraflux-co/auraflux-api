...
const { buildYoutubeLanguagePromptBlock, enforcePublishCaptionLanguage } = require('./youtube_language_policy');
...
- Output ONLY valid JSON, no markdown, no explanation`;
  ...
  if (result.caption) {
    const cap = enforcePublishCaptionLanguage(result.caption);
    if (cap.sanitized) {
      console.warn('[publish] Short-form caption sanitized for YouTube vulgar language policy');
    }
    result.caption = cap.caption;
  }
  if (result.altText) {
    const alt = enforcePublishCaptionLanguage(result.altText);
    result.altText = alt.caption;
  }
  return result;
}
...
- The host (Bobby G) provides dry, deadpan commentary — titles and descriptions reflect this voice, not wire-copy` : '';
  ...
  const clipsSeoGuidance = isGamingContentType(contentType) ? `
TWITCH/GAMING SEO — FAIL CLOSED (CPD-1013):
- NEVER use generic tech/news phrasing ("platform update", "breaking developments", "tech industry", "infrastructure summit")
- Titles MUST name the streamer(s) or a specific gameplay moment visible in the script excerpt
- If the script excerpt lacks concrete gameplay details, do NOT invent a news-style headline — stay faithful to clip content` : '';
...
${nbaSeoGuidance}${newsSeoGuidance}${clipsSeoGuidance}
...
// CPD-1013: Reject generic tech/news copy when gaming/clips script lacks real analysis.
const GENERIC_GAMING_PUBLISH_PATTERNS = [
  /platform update/i,
  /breaking developments in the tech world/i,
  /tech industry landscape/i,
  /major announcements reshaped the industry/i,
  /infrastructure summit/i,
  /global infrastructure/i,
];

function isGamingContentType(contentType) {
  const ct = (contentType || '').toLowerCase();
  return ct.includes('clip') || ct.includes('twitch') || ct.includes('streamer') || ct === 'gaming';
}

function scriptExcerptIsPlaceholder(scriptExcerpt) {
  if (!scriptExcerpt || typeof scriptExcerpt !== 'string') return true;
  return /Video clip \d+ for "/i.test(scriptExcerpt);
}

function rejectGenericGamingPublishCopy(metadata, { contentType, scriptExcerpt, items = [] }) {
  if (!isGamingContentType(contentType)) return;

  if (scriptExcerptIsPlaceholder(scriptExcerpt)) {
    throw new Error('[publish-copy] Rejected: script excerpt is placeholder-only (Gemini did not analyze clip)');
  }

  const yt = metadata?.youtube || {};
  const title = yt.bestTitle?.title || yt.title || '';
  const desc = yt.description || '';
  const combined = `${title}\n${desc}`;

  for (const pattern of GENERIC_GAMING_PUBLISH_PATTERNS) {
    if (pattern.test(combined)) {
      throw new Error(
        `[publish-copy] Rejected generic/off-topic gaming publish copy (matched ${pattern})`
      );
    }
  }

  const streamerNames = items
    .map((it) => (typeof it === 'string' ? it : (it.title || it.displayName || it.streamer || '')))
    .filter(Boolean);
  if (streamerNames.length && title) {
    const titleLower = title.toLowerCase();
    const namesStreamer = streamerNames.some((n) => titleLower.includes(String(n).toLowerCase().slice(0, 4)));
    if (!namesStreamer && /update|developments|industry|summit/i.test(titleLower)) {
      throw new Error('[publish-copy] Rejected: gaming title lacks streamer/game grounding');
    }
  }
}
...
async function handleGeneratePublishCopy(req, res) {
  ...
  let channelConfig = {
    ...
    clips:  { showName: 'Twitch Soup', handle: '@clipzworldnews', tiktokHandle: '@clipznashite', host: 'Bobby G' },
  ...
  const contentDescriptors = {
    ...
    clips:  `Twitch clips compilation featuring: ${streamers.join(', ') || 'multiple streamers'}`,
    streamer: `Twitch clips compilation featuring: ${streamers.join(', ') || 'multiple streamers'}`,
    nba:    buildNbaContext(items),
    news:   buildNewsContext(items),
  };
  const cdKey = ['twitch', 'clips', 'streamer', 'nba', 'news'].find((k) => (contentType || '').includes(k)) || contentType;
  const cd = contentDescriptors[cdKey] || contentDescriptors.twitch;
  ...
  try {
    ...
    rejectGenericGamingPublishCopy(metadata, {
      contentType,
      scriptExcerpt,
      items: streamers.length ? streamers : items,
    });
    ...
    const payload =
      platforms.length === 1 && platforms[0] === 'youtube' && metadata.youtube
        ? { ok: true, ...metadata.youtube, platforms: { youtube: metadata.youtube }, contentType, formType }
        : { ok: true, platforms: metadata, contentType, formType };
    ...
    if (req.body.jobId) {
      try {
        const db = require('./db');
        const row = await db.loadJobRow(req.body.jobId);
        if (row) {
          const spec =
            typeof row.job_spec === 'string' ? JSON.parse(row.job_spec) : row.job_spec || {};
          spec.state = spec.state || {};
          spec.state.savedOutputs = spec.state.savedOutputs || {};
          spec.state.savedOutputs.publishCopy = payload.platforms || payload;
          spec.updatedAt = new Date().toISOString();
          await db.updateJobSpec(req.body.jobId, spec);
          payload.savedToJob = true;
        }
      } catch (saveErr) {
        console.warn('[publish-copy] jobId save failed:', saveErr.message);
      }
    }
...
async function generatePublishCopyFromScript({ script, contentType, formType, items = [], jobId, platforms = ['youtube', 'tiktok', 'instagram'], designSpec = {} }) {
  ...
  if (designSpec?.voice?.showName) {
    ...
    if (channelConfig[vcKey]) channelConfig[vcKey].showName = normalizeShowName(designSpec.voice.showName);
  }
  ...
  const contentDescriptors = {
    ...
    clips:  `Twitch clips compilation featuring: ${streamers.map(s => (typeof s === 'object' ? s.displayName || s.title : s) || '').filter(Boolean).join(', ') || 'multiple streamers'}`,
    streamer: `Twitch clips compilation featuring: ${streamers.map(s => (typeof s === 'object' ? s.displayName || s.title : s) || '').filter(Boolean).join(', ') || 'multiple streamers'}`,
    nba:    buildNbaContextG1(items),
    news:   buildNewsContextG1(items),
  };
  const cdKey = ['twitch', 'clips', 'streamer', 'nba', 'news'].find((k) => (baseContentType || '').includes(k)) || baseContentType;
  const cd = contentDescriptors[cdKey] || contentDescriptors.twitch;
  ...
  const { applySupportPromoToPublishCopy } = require('./clipzworld_support');
  return applySupportPromoToPublishCopy(result);
}
