...
// Implements the fully-automatic pipeline: Gate 1 → HeyGen render → heygen:all_complete.
async function startHeyGenPoller(jobId, card) {
  ...
  if (allAlreadyComplete) {
    console.log(`[heygen-poller:${jobId}] ⏭️ All segments already completed — emitting heygen:all_complete`);
    try {
      pipelineBus.emit('heygen:poll_terminal', { jobId, outcome: 'skipped_external_poll', reason: 'all_segments_already_complete' });
    } catch (_e) { /* non-fatal */ }
    ...
    if (shouldUseManualCheckpoint(updatedCard)) {
      ...
      updatedCard.manualSegments = {
        enabled: true, status: 'awaiting_upload', manualDir: dir, manifestPath, operatorGuideDir,
        sourceClipPrefetchLogPath: prefetch.logPath, overlaysDir, overlaysReadme,
        overlayPreviewCopies: copiedPreviews || [], segmentCount: manifest.segments.length,
        requestedAt: new Date().toISOString(),
      };
      saveJobCard(jobId, updatedCard);
      console.log(`[heygen-poller:${jobId}] 🛑 c0 manual checkpoint — ${dir} — resume POST /job/${jobId}/manual-segments/resume`);
    ...
    pipelineBus.emit('heygen:all_complete', {
      jobId, contentType: card.contentType || 'twitch', segmentUrls, card: updatedCard, segmentData,
    });
    return;
  }
  ...
  nrEvent('HeyGenPollStart', { jobId, executionMode: 'inline', segmentCount: videoJobs.length, contentType: card.contentType || 'twitch', maxPollMinutes: MAX_POLL_MINUTES });

  // Seed pending rows in heygen_renders DB table (non-fatal)
  try {
  ...
  const poll = async () => {
    ...
    if (pollCount > MAX_POLLS) {
      ...
      try { pipelineBus.emit('heygen:poll_terminal', { jobId, outcome: 'timeout', reason: 'max_polls_exceeded' }); } catch (_e) {}
      unregisterPoller(jobId);
      return;
    }
    ...
    try {
      ...
      const silentSuspects = completed.filter((s) => {
        ...
        return name.includes('_CLIP') && !name.includes('_CLIP1') && !name.includes('_CLIP2') &&
               !name.includes('_CLIP3') && !name.includes('SETUP') && !name.includes('REACTION') && !name.includes('RECAP');
      });
      if (silentSuspects.length > 0) {
        console.warn(`[heygen-poller:${jobId}] ⚠️  SILENT RENDER DETECTED: ${silentSuspects.map((s) => s.sceneName).join(', ')} — source_clip scenes submitted to HeyGen, will be excluded from assembly`);
      }

      // Persist completed/failed statuses to DB (non-fatal)
      try {
        for (const s of statuses) {
          if (s.status === 'completed' || s.status === 'failed') {
            saveHeyGenRender(jobId, s.video_id, s.sceneName, s.status, s.status === 'completed' ? { videoUrl: s.video_url } : {});
          }
        }
      } catch (_e) {}
      ...
      nrEvent('HeyGenPollTick', { jobId, pollCount, completed: completed.length, pending: pending.length, failed: failed.length, total: videoJobs.length, contentType: card.contentType || 'twitch' });
      try {
        pipelineBus.emit('heygen:poll_tick', { jobId, attempt: pollCount, allComplete: completed.length === videoJobs.length, pending: pending.length, failed: failed.length, total: videoJobs.length });
      } catch (_e) {}

      if (failed.length > 0) {
        console.error(`[heygen-poller:${jobId}] ❌ ${failed.length} segment(s) failed: ${failed.map((f) => f.sceneName).join(', ')} — keep polling (HeyGen sometimes recovers)`);
      }
      ...
      console.log(`[heygen-poller:${jobId}] ✅ All ${videoJobs.length} segments completed — building segmentData`);
      ...
      if (scriptScenes.length > 0) {
        for (const scene of scriptScenes) {
          if (scene.type === 'source_clip') {
            ...
            if (clip && (clip.url || clip.clipUrl)) {
              segmentData.push({
                url: clip.clipUrl || clip.url || '', pageUrl: clip.pageUrl || '',
                label: clip.label || scene.name || `CLIP_${clipIdx}`, type: 'source_clip',
                clipUrl: clip.clipUrl || clip.url || '',
                clipTimingTargets: Array.isArray(clip.clipTimingTargets) ? clip.clipTimingTargets : [],
                clipTimingFormat: clip.clipTimingFormat || 'none',
              });
            ...
            if (avatarSeg && avatarSeg.video_url) {
              ...
              if (scene.hasClipInsert) {
                ...
                if (clip && (clip.url || clip.clipUrl)) {
                  segmentData.push({
                    url: clip.clipUrl || clip.url || '', pageUrl: clip.pageUrl || '',
                    label: clip.label || `${sceneKey}_CLIP`, type: 'source_clip',
                    clipUrl: clip.clipUrl || clip.url || '',
                    clipTimingTargets: Array.isArray(clip.clipTimingTargets) ? clip.clipTimingTargets : [],
                    clipTimingFormat: clip.clipTimingFormat || 'none',
                  });
      ...
      for (const avatarSeg of sortedAvatarSegs) {
        ...
        if (_ct === 'news' && /STORY(\d+)_INTRO/i.test(_sceneName)) {
          ...
          if (item) seg.cardData = { title: item.title || `Story ${storyIdx + 1}`, category: 'WORLD NEWS', storyId: `story_${storyIdx + 1}`, imageUrl: item.thumbnailUrl || item.imageUrl || null, heroImageUrl: item.heroImageUrl || item.thumbnailUrl || null, source: item.source || '' };
          ...
          seg.cardData = { title: nbaItem?.title || nbaItem?.matchup || rawName || `Game ${gameIdx + 1}`, matchup: nbaItem?.matchup || rawName || `Game ${gameIdx + 1}`, category: 'NBA GAME', storyId: `game_${gameIdx + 1}`, gameId: nbaItem?.gameId || null };
        } else if (_ct === 'twitch' && /[_ ]INTRO$/i.test(_sceneName)) {
          const namePart = _sceneName.replace(/[_ ]INTRO$/i, '').replace(/_/g, ' ').toLowerCase();
          const streamer = (card.streamers || []).find((s) => (s.displayName || '').toLowerCase() === namePart || (s.twitchUsername || '').toLowerCase() === namePart) || (card.streamers || [])[0];
          if (streamer) seg.cardData = { title: streamer.displayName || namePart, category: 'ON STREAM', storyId: `streamer_${namePart.replace(/\s+/g, '_')}`, fact: [streamer.origin, streamer.fact].filter(Boolean).join(' · ').slice(0, 60), imageUrl: streamer.profileImage || null, twitchUsername: streamer.twitchUsername || streamer.username || null };
        }
      }

      console.log(`[heygen-poller:${jobId}] Built segmentData: ${segmentData.length} segments (${sortedAvatarSegs.length} avatar + ${clipIdx} clips)`);

      const updatedCard = persistedJobs[jobId] || card;
      updatedCard.heygen = updatedCard.heygen || {};
      updatedCard.heygen.videoJobs = statuses.map((s) => ({ ...(videoJobs.find((j) => j.video_id === s.video_id) || {}), status: s.status, video_url: s.video_url }));
      updatedCard.stage = 'all_sent';
      saveJobCard(jobId, updatedCard);
      ...
      if (shouldUseManualCheckpoint(updatedCard)) {
        ...
        const { dir, manifestPath, manifest, overlaysDir, overlaysReadme, copiedPreviews, operatorGuideDir } = man;
        let prefetch = { logPath: path.join(dir, 'source_clip_prefetch.log') };
        try { prefetch = await prefetchManualSourceClips(dir, segmentData, { jobId }); } catch (e) { console.error(`[heygen-poller:${jobId}] prefetch failed (non-fatal): ${e.message}`); }
        updatedCard.stage = 'awaiting_manual_segments';
        updatedCard.manualSegments = { enabled: true, status: 'awaiting_upload', manualDir: dir, manifestPath, operatorGuideDir, sourceClipPrefetchLogPath: prefetch.logPath, overlaysDir, overlaysReadme, overlayPreviewCopies: copiedPreviews || [], segmentCount: manifest.segments.length, requestedAt: new Date().toISOString() };
        saveJobCard(jobId, updatedCard);
        try { pipelineBus.emit('heygen:poll_terminal', { jobId, outcome: 'manual_checkpoint_waiting', reason: 'c0_manual_segment_hold' }); } catch (_e) {}
        console.log(`[heygen-poller:${jobId}] 🛑 manual checkpoint — ${dir} — resume POST /job/${jobId}/manual-segments/resume`);
        unregisterPoller(jobId);
        return;
      }

      const segmentUrls = sortedAvatarSegs.map((s) => s.video_url).filter(Boolean);
      nrEvent('HeyGenSegmentsReady', { jobId, contentType: card.contentType || 'twitch', segmentCount: videoJobs.length, segmentUrlCount: segmentUrls.length });
      try { pipelineBus.emit('heygen:poll_terminal', { jobId, outcome: 'all_segments_ready', reason: null }); } catch (_e) {}
      pipelineBus.emit('heygen:all_complete', { jobId, contentType: card.contentType || 'twitch', segmentUrls, card: updatedCard, segmentData });
      unregisterPoller(jobId);
      console.log(`[heygen-poller:${jobId}] 📡 heygen:all_complete emitted — Gate 2 + assembly handed off`);
    } catch (pollErr) {
...
function resumeInFlightPollers() {
  ...
  if (candidates.length > MAX_RESUME_POLLERS) {
    console.warn(`[startup-resume] ⚠️  ${candidates.length} jobs eligible — capping at ${MAX_RESUME_POLLERS}. Remaining ${candidates.length - MAX_RESUME_POLLERS} need manual re-trigger from dashboard.`);
  }
  ...
  for (const [jobId, card] of candidates.slice(0, MAX_RESUME_POLLERS)) {
    ...
    if (videoJobs.every((vj) => vj.status === 'completed' && vj.video_url)) {
      ...
      setTimeout(() => {
        try { pipelineBus.emit('heygen:poll_terminal', { jobId, outcome: 'all_segments_ready', reason: 'startup_resume_card_complete' }); } catch (_e) {}
        pipelineBus.emit('heygen:all_complete', { jobId, contentType, formType, segmentUrls, card, segmentData: null });
      }, 2000);
    } else {
      console.log(`[startup-resume:${jobId}] Resuming poller (${videoJobs.length} segments, not all complete)`);
      startHeyGenPoller(jobId, card).catch((e) => console.error(`[startup-resume:${jobId}] Poller error: ${e.message}`));
    }
    resumed++;
  }

  if (resumed > 0) console.log(`[startup-resume] Resumed ${resumed} in-flight job(s) (cap: ${MAX_RESUME_POLLERS})`);
  ...
  if (scriptReadyCandidates.length > 0) {
    ...
    console.log(`[startup-resume] ${scriptReadyCandidates.length} script_ready job(s) — auto-sending to HeyGen (cap: ${MAX_SCRIPT_READY_RESUME})`);
    ...
    (async () => {
      for (const card of toResume) {
        ...
        if (!script) { console.warn(`[startup-resume:${jobId}] No script in card — skipping`); continue; }
        ...
        try {
          ...
          if (heygenResult?.videoJobs?.length) {
            ...
            if (useC0ImmediateManualHold(card)) {
              ...
              console.log(`[startup-resume:${jobId}] ✅ c0 immediate manual hold (${heygenResult.videoJobs.length} scenes sent) — ${prep.manualSegments.manualDir}`);
            } else {
              card.stage = 'all_sent';
              saveJobCard(jobId, card);
