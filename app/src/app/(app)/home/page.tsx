...
import { MemberHomeHub } from '@/components/member-portal/member-home-hub';
...
export default async function DashboardPage() {
  ...
  const tierBadge = (
    <span
      className={`inline-flex items-center px-2.5 py-0.5 rounded text-[11px] font-bold tracking-widest uppercase ${TIER_BADGE_COLORS[planTier] ?? TIER_BADGE_COLORS.operate}`}
    >
      {tierLabel(planTier)}
    </span>
  );
  ...
  return (
    <PageShell>
      <PageHeader title={`Welcome back, ${firstName}`} badge={tierBadge} />
      <Suspense>
