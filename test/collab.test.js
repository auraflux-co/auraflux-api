'use strict';
/**
 * test/collab.test.js — CPD-83 / CPD-309: AI Concierge backend unit tests
 *
 * Tests: getPortalContracts, validateJobSpec, buildSystemPrompt,
 *        buildSystemStatus, chatWithCollab (mocked)
 */
...
describe('chatWithCollab', () => {
  ...
  it('returns text from Gemini response', async () => {
    const result = await chatWithCollab(
  ...
  it('throws when Gemini is not configured', async () => {
    ...
    await expect(
      chatWithCollab([{ role: 'user', content: 'Hi' }], {}, { planTier: 'guided' })
  ...
  it('throws when plan does not support concierge', async () => {
    ...
    await expect(
      chatWithCollab([{ role: 'user', content: 'Hi' }], {}, { planTier: 'operate' })
