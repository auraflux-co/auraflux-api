...
describe('COMP_CREATIVE_CATALOG labels', () => {
  it('lists C1–C10 with codes and select labels', () => {
    const list = getCompCreativeCatalogList();
    assert.equal(list.length, 10);
    const codes = list.map((e) => e.code).sort();
    assert.deepEqual(codes, ['C1', 'C10', 'C2', 'C3', 'C4', 'C5', 'C6', 'C7', 'C8', 'C9']);
    list.forEach((e) => {
  ...
  it('C10 Reaction Short is Short-only Transform recipe', () => {
    const e = getCompCreativeCatalogEntry('reaction_short');
    assert.equal(e.code, 'C10');
    assert.deepEqual(e.buttons, ['Short']);
    assert.match(e.name, /Reaction/i);
  });
