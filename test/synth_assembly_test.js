...
function makeSynth(filename, duration = 10, source = 'testsrc', size = '1920x1080') {
  ...
  return new Promise((res, rej) => {
    execFile(FFMPEG, [
      '-f', 'lavfi', '-i', `${source}=duration=${duration}:size=${size}:rate=30`,
      '-f', 'lavfi', '-i', `sine=f=440:b=4:duration=${duration}`,
      '-c:v', 'libx264', '-preset', 'ultrafast', '-crf', '18',
      '-c:a', 'aac', '-b:a', '128k',
      '-pix_fmt', 'yuv420p',
      '-y', outPath
    ], { timeout: 30000 }, (err) => err ? rej(err) : res(outPath));
  });
}

function url(filename) { return `${STATIC}/${filename}`; }

function avatar(label, filename) {
  return { url: url(filename), label, type: 'avatar' };
}
function clip(label, filename) {
  return { url: url(filename), label, type: 'source_clip',
           clipUrl: url(filename), pillarboxFilter: null, orientation: 'landscape' };
}
...
function nbaPayload() {
  ...
  const games = [
    { title: 'LAKERS VS CELTICS', category: 'NBA GAME', storyId: 'game_1', matchup: 'Lakers vs Celtics' }
  ];
  return {
    ...
    segmentData: [
      { ...avatar('INTRO',                        'av_10s.mp4') },
      { ...avatar('GAME1_LAKERS_VS_CELTICS_INTRO','av_10s.mp4'),
        cardData: { ...games[0] } },
      { ...avatar('GAME1_LAKERS_VS_CELTICS_NARRATION', 'av_10s.mp4'),
        cardData: { ...games[0] } },
      { ...clip(  'GAME1_CLIP',                   'clip_16x9_10s.mp4') },
      { ...avatar('GAME1_LAKERS_VS_CELTICS_REACTION',  'av_10s.mp4'),
        cardData: { ...games[0] } },
      { ...avatar('OUTRO',                        'av_10s.mp4') },
    ]
  };
...
async function run() {
  ...
  const payloads = { news: newsPayload, twitch: twitchPayload, nba: nbaPayload, short: shortPayload };
  const results  = [];

  for (const type of RUN) {
    const payload = withSegments(payloads[type]());
    console.log(`▶ Running ${type.toUpperCase()} assembly test (${payload.segmentData.length} segments)...`);
    try {
      ...
      for (let i = 0; i < 200; i++) {
        ...
        if (d.outputPath) { out = d.outputPath; break; }
        if (d.status === 'failed' && !d.outputPath) { throw new Error(d.error || 'Assembly failed — no output file'); }
  ...
  for (const r of results) {
    console.log(`  ${r.pass ? '✅' : '❌'} ${r.type.toUpperCase()}${r.error ? ' — ' + r.error : ''}`);
  }
