...
if (
  ...
  !/^REPLACE/i.test(String(process.env.NEW_RELIC_LICENSE_KEY))
) {
  process.env.NR_PIPELINE_SERVICE =
    process.env.NR_PIPELINE_SERVICE || 'auraflux-heygen-poller-docker';
...
const { shouldUseManualCheckpoint, writeManualManifest, prefetchManualSourceClips } = require(
  path.join(__dirname, '..', 'lib', 'manual_segment_workflow')
);
...
if (!jobId) {
  console.error('[poller] Usage: heygen-poller.js <jobId>');
  process.exit(1);
}
if (!HEYGEN_API_KEY) {
  console.error('[poller] HEYGEN_API_KEY not set');
  process.exit(1);
}
...
function buildSegmentData(card, sortedAvatarSegs) {
  ...
  for (const avatarSeg of sortedAvatarSegs) {
    ...
    if (
      (card.contentType || 'twitch') === 'news' &&
      /STORY(\d+)_INTRO/i.test(avatarSeg.sceneName)
    ) {
    ...
    const isClipTrigger =
      /SETUP/i.test(avatarSeg.sceneName) || /NARRATION/i.test(avatarSeg.sceneName);
  ...
  console.log(
    `[poller:${jobId}] Built segmentData: ${segmentData.length} entries (${sortedAvatarSegs.length} avatar + ${clipIdx} source_clips)`
  );
  return segmentData;
}
...
async function poll(videoJobs, pollCount) {
  if (pollCount > MAX_POLLS) {
    console.error(
      `[poller:${jobId}] ⏰ Timeout after ${MAX_POLL_MINUTES}min — giving up. POST /job/${jobId}/resubmit-heygen to retry.`
    );
  ...
  console.log(
    `[poller:${jobId}] Poll ${pollCount}: ${completed.length}/${videoJobs.length} completed, ${pending.length} pending, ${failed.length} failed`
  );
  ...
  if (failed.length > 0) {
    console.error(
      `[poller:${jobId}] ❌ ${failed.length} failed: ${failed.map((f) => f.sceneName).join(', ')}`
    );
    try {
      const card = readCard();
    ...
    if (statuses.every((s) => TERMINAL.has(s.status)) && completed.length === 0) {
      console.warn(
        `[poller:${jobId}] All segments terminal, 0 completed — stopping. POST /job/${jobId}/resubmit-heygen to retry.`
      );
  ...
  if (shouldUseManualCheckpoint(card)) {
    ...
    writeCard(card);
    console.log(
      `[poller:${jobId}] 🛑 c0 manual checkpoint — ${dir} (see read_me/) — resume POST /job/${jobId}/manual-segments/resume`
    );
    try {
      pipelineBus.appendJobTimelineEvent('heygen:poll_terminal', {
        jobId,
  ...
  try {
    ...
    console.log(
      `[poller:${jobId}] 📡 Handoff sent to ${CWN_SERVER_URL}/internal/heygen-complete — pipeline continues`
    );
...
async function main() {
  const card = readCard();
  if (!card) {
    console.error(`[poller] Job not found: ${jobId}`);
    process.exit(1);
  }

  const videoJobs = card.heygen?.videoJobs || [];
  if (!videoJobs.length) {
    console.error(`[poller] No videoJobs on card: ${jobId}`);
    process.exit(1);
  }

  console.log(
    `[poller:${jobId}] 🔄 Starting — ${videoJobs.length} segments, polling every ${POLL_INTERVAL_MS / 1000}s (max ${MAX_POLL_MINUTES}min)`
  );
