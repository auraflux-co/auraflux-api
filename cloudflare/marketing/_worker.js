...
// Last known-good deployment with: chat widget + sign-up CTAs + correct content (368KB)
// Updated whenever a new full deployment (with all rewrites) is confirmed working.
const FRAMER_FALLBACK = 'https://3f42170e.auraflux-marketing.pages.dev';
...
export default {
  async fetch(request, env, ctx) {
    ...
    // ── Proxy from last known-good Framer snapshot ────────────────────────
    // env.ASSETS only works when static files are uploaded alongside the worker.
    // For pure-worker deployments (no static file upload), ASSETS returns 404.
    // Always use the snapshot fallback — it IS the correct Framer content with
    // all the CTAs, brand rewrites, and navigation already applied by that deployment's worker.
    let response;
    try {
