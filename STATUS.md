...
| Cline | **Fix 1/4 — feat(news): build orderedClipUrls for News in generate-full-script**: Root cause of 22_avatar_0_clips — `orderedClipUrls` was only populated in the Twitch block. News block now maps `items[]` → `orderedClipUrls[]` with url/clipUrl/pageUrl/label/streamer/title fields, filtered to items with valid clip URLs. Fixes the 0-clip assembly for all News long-form jobs. | `server.js`, `STATUS.md` | pending | 2026-04-12 5:00 PM ET |
