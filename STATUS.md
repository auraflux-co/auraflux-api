...
| Cline | Fix ticker missing in assembled video: add `captureTicker(contentType)` pre-warm call in `startHeyGenPoller()` before `axios.post('/assemble')`. `TICKER_CACHE` is in-memory and cleared on server restart — ticker was always empty when auto-assembly fired after a restart. Pre-warm fires async (non-blocking) for long-form only. Fix clip ordering bug in `restoreJobsFromServer()`: was checking `isReaction` (matched both `CLIP_REACTION` and `REACTION`), inserting source clips BEFORE reaction scenes. Fixed to check `isSetup` and insert clips AFTER SETUP scenes, matching `startHeyGenPoller()` interleaving logic. Fixes: no ticker in assembled video + audio/scene mismatch (Bobby G talking about wrong clip). | `server.js`, `cwn_production.html` | pending | 2026-04-11 9:58 AM ET |
