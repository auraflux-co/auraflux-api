...
**Last Updated:** 2026-04-11 (5:15 PM ET)
**Branch:** main | **Latest Commit:** `699b4cb` (archive) + Cline pending TV card + ticker fix. **4th smoke test reviewed** — TV card left edge overlaps Bobby G's shoulder at current `{x:1040, y:100, w:840, h:472}`, and ticker is visibly shaky due to 15fps/30fps frame rate mismatch. New handoff `CLINE_HANDOFF_TICKER_STABILITY_TV_REPOSITION.md`: TV card → `{x:1160, y:100, w:720, h:405}` (exact 16:9 aspect per Rob's math verification), ticker FPS 15→30, cache bust. **Rob decided on Jira+Confluence** as the long-term task tracking stack; migration parked until 12-test suite stable.
...
| Cline | **fix(layout): TV card 840×472→720×405 exact 16:9 + ticker FPS 15→30 for smoother scroll**: Rob reviewed 4th smoke test — TV card left edge at x=1040 overlapped Bobby G shoulder; Option (c) {x:1160,y:100,w:720,h:405} is mathematically exact 16:9 (720÷405=1.7778), right edge at 1880 (40px margin). Ticker FPS 15→30 eliminates hold-and-jump stutter (~70% of visible shake). Long-term drawtext replacement parked as Task #22 (post-12-test). server.js unchanged — config-driven filters from 33ed559 auto-propagate. | `lib/config.js`, `STATUS.md` | pending | 2026-04-11 4:54 PM ET |
---
