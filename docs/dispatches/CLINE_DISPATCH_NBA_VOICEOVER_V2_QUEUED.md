...
- **Adds `pickNBAMusicTrack()` helper** — reads `assets/audio/nba/` at assembly time, picks a track (round-robin or random, final choice in the spec), validates it loads, returns an absolute path for the FFmpeg filter graph. Handles empty-dir case with a loud error instead of a silent fallback.
...
3. **`assets/audio/nba/` populated** — at least one `.mp3`, `.wav`, or `.m4a` file present. Empty directory = `pickNBAMusicTrack()` will throw, and the dispatch's happy path assumes a valid track exists. Rob is responsible for dropping in the music beds before dispatch fires.
